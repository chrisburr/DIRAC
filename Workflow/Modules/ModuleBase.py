...
class ModuleBase(object):
    ...
    def execute(self):
        ...
        # This catches everything that is voluntarily thrown within the modules, so an error
            ...
            if len(rte.args) > 1:
                ...
                return S_ERROR(
                    rte.args[1], rte.args[0]
                )  # rte[1] should be an error code
    ...
    def _resolveInputWorkflow(self):
        ...
        self.inputDataList = [
            lfn.strip("LFN:") for lfn in self.InputData.split(";") if lfn
        ]
            ...
            for outputF in self.workflow_commons["outputList"]:
                try:
                    if (
                        ...
                        and outputF["outputDataType"].lower()
                        == self.inputDataType.lower()
                    ):
    ...
    def getCandidateFiles(self, outputList, outputLFNs, fileMask, stepMask=""):
        ...
        for outputFile in outputList:
            if (
                ...
                self.log.error(
                    "Ignoring malformed output data specification", str(outputFile)
                )
        ...
        for lfn in outputLFNs:
            if os.path.basename(lfn) in fileInfo.keys():
                ...
                self.log.verbose(
                    "Found LFN %s for file %s" % (lfn, os.path.basename(lfn))
                )
    ...
    def _applyMask(self, candidateFilesIn, fileMask, stepMask):
        ...
        if fileMask and fileMask != [""]:
            for fileName, metadata in candidateFiles.items():
                if (
                    metadata["type"].lower() not in fileMask
                ):  # and ( fileName.split( '.' )[-1] not in fileMask ) ):
            ...
            self.log.info(
                "No outputDataFileMask provided, the files with all the extensions will be considered"
            )
        ...
        if stepMask and stepMask != [""]:
            ...
            self.log.info(
                "No outputDataStep provided, the files output of all the steps will be considered"
            )
    ...
    def _checkSanity(self, candidateFiles):
        ...
        if notPresentKeys:
            for fileName_keys in notPresentKeys:
                self.log.error(
                    "File %s has missing %s" % (fileName_keys[0], fileName_keys[1])
                )
            raise ValueError
    ...
    def _checkLocalExistance(self, fileList):
        ...
        if notPresentFiles:
            self.log.error(
                "Output data file list %s does not exist locally" % notPresentFiles
            )
            raise os.error
    ...
    def generateFailoverFile(self):
        ...
        if len(self.request):
            ...
            if not isValid["OK"]:
                raise RuntimeError(
                    "Failover request is not valid: %s" % isValid["Message"]
                )
            else:
                ...
                if requestJSON["OK"]:
                    ...
                    if result["OK"]:
                        ...
                        self.log.error(
                            "No digest? That's not sooo important, anyway:",
                            result["Message"],
                        )
                else:
