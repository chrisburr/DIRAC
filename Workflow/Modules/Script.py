...
class Script(ModuleBase):
    ...
    def _executeCommand(self):
        ...
        if status:
            ...
            self.log.info(
                "%s execution completed with status %s" % (self.command, status)
            )
        ...
        self.log.info(
            "Output written to %s, execution complete." % (self.applicationLog)
        )
    ...
    def _exitWithError(self, status):
        ...
        raise RuntimeError(
            ...
            % (os.path.basename(self.executable).split("_")[0], status),
            status,
        )
