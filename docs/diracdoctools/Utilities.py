...
def writeLinesToFile(filename, lines):
    ...
    if oldContent is None or oldContent != newContent:
        ...
        LOG.debug(
            "Not updating file content for %s", os.path.join(os.getcwd(), filename)
        )
...
def setUpReadTheDocsEnvironment(moduleName="DIRAC"):
    ...
    if not os.path.exists(os.path.join("../../", moduleName)):
        diracLink = os.path.abspath(
            os.path.join(os.getcwd(), "../", buildfolder, moduleName)
        )
...
def runCommand(command):
    ...
    try:
        result = subprocess.check_output(
            shlex.split(command), stderr=subprocess.STDOUT, universal_newlines=True
        )
        if "NOTICE:" in result:
            lines = []
            LOG.warn(
                "NOTICE in output for: %s; cleaning output from datestamp..", command
            )
