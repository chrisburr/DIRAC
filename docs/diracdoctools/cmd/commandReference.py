...
class CommandReference(object):
    ...
    def getScripts(self):
        ...
        for scriptPath in scripts:
            ...
            for mT in self.sectionDicts:
                if any(pattern in scriptPath for pattern in mT[PATTERN]) and not any(
                    pattern in scriptPath for pattern in mT[EXCLUDE]
                ):
    ...
    def createFilesAndIndex(self, sectionDict):
        ...
        for script in sectionDict[SCRIPTS]:
            ...
            if self.createScriptDocFiles(
                script, sectionPath, scriptName, referencePrefix=prefix
            ):
        ...
        writeLinesToFile(
            os.path.join(self.config.docsPath, sectionDict[SECTION_PATH], "index.rst"),
            sectionIndexRST,
        )
    ...
    def createFiles(self, sectionDict):
        ...
        for script in sectionDict[SCRIPTS] + sectionDict[MANUAL]:
            ...
            prefix = prefix + "_" if prefix else ""
            if (
                self.createScriptDocFiles(
                    script, sectionPath, scriptName, referencePrefix=prefix
                )
                and scriptName not in commandList
            ):
    ...
    def cleanExistingIndex(self, sectionDict):
        ...
        """
        existingCommands = {
            os.path.basename(com).replace(".py", "")
        ...
        if superfluousCommands:
            ...
            for com in superfluousCommands:
                ...
                if os.path.exists(commandDocPath):
                    shutil.move(
                        commandDocPath, os.path.join(sectionPath, "obs_" + com + ".rst")
                    )
            self.exitcode = 1
    ...
    def createScriptDocFiles(self, script, sectionPath, scriptName, referencePrefix=""):
        ...
        for line in helpMessage.splitlines():
            ...
            # strip general options from documentation
                ...
                LOG.debug(
                    "Found general options in line %r", line
                ) if self.debug else None
                genOptions = True
                continue
            elif genOptions and line.startswith(" "):
                LOG.debug(
                    "Skipping General options line %r", line
                ) if self.debug else None
                continue
