...
class CodeReference(object):
    ...
    def mkPackageRst(
        self, filename, modulename, fullmodulename, subpackages=None, modules=None
    ):
        ...
        if subpackages:
            LOG.info(
                "Module %r with subpackages: %r", fullmodulename, ", ".join(subpackages)
            )
    ...
    def mkDummyRest(self, classname, fullclassname):
        ...
        lines.append("")
        lines.append(
            "This is an empty file, because we cannot parse this file correctly or it causes problems."
        )
    ...
    def getsubpackages(self, abspath, direc):
        ...
        for dire in direc:
            ...
            if dire.lower() == "docs" or "/docs" in dire.lower():
                ...
                continue
            if os.path.exists(
                os.path.join(self.config.sourcePath, abspath, dire, "__init__.py")
            ):
    ...
    def getmodules(self, abspath, _direc, files):
        ...
        for filename in files:
            if (
                ...
                or any(
                    f.lower() in filename.lower() for f in self.config.code_ignoreFiles
                )
            ):
    ...
    def cleanDoc(self):
        ...
        LOG.info(
            "Removing existing code documentation: %r", self.config.code_targetPath
        )
    ...
    def createDoc(self, buildtype="full"):
        ...
        if (
            os.path.exists(self.config.code_targetPath)
            and os.environ.get("READTHEDOCS", "False") == "True"
        ):
            self.cleanDoc()
        ...
        for root, direc, files in os.walk(self.config.sourcePath):
            configTemplate = [
                os.path.join(root, _) for _ in files if _ == "ConfigTemplate.cfg"
            ]
            ...
            if not fullmodulename.startswith(self.config.moduleName):
                fullmodulename = (
                    ".".join([self.config.moduleName, fullmodulename])
                ).strip(".")
            ...
            if firstModule:
                ...
                self.createCodeDocIndex(
                    ...
                    modules=self.getmodules(codePath, direc, files),
                    buildtype=buildtype,
                )
            ...
            for filename in files:
                ...
                if any(f in filename for f in self.config.code_dummyFiles):
                    ...
                    self.mkDummyRest(
                        filename.split(".py")[0], fullclassname.split(".py")[0]
                    )
                    continue
                elif (
            ...
            if configTemplate:
                shutil.copy(
                    configTemplate[0],
                    os.path.join(self.config.code_targetPath, docPath),
                )
        ...
        if buildtype == "limited":
            ...
            if subpackages:
                ...
                for package in systemPackages:
                    lines.append(
                        "   %s/%s_Module.rst" % (package, package.split("/")[-1])
                    )
