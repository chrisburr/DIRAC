...
class GithubInterface(object):
    ...
    def checkRate(self):
        ...
        LOGGER.getChild("Rate").info(
            "Remaining calls to github API are %s of %s",
            rate["rate"]["remaining"],
            rate["rate"]["limit"],
        )
    ...
    def parseOptions(self):
        ...
        conf_parser.add_argument(
            "-d", "--debug", action="count", dest="debug", help="d, dd, ddd", default=0
        )
        ...
        if self.sinceLatestTag:
            ...
            if (
                parsed.date.tzinfo is None
                or parsed.date.tzinfo.utcoffset(parsed.date) is None
            ):
        ...
        self.useGitlab = (
            parsed.gitlab
            if isinstance(parsed.gitlab, bool)
            else parsed.gitlab.lower() == "true"
        )
        ...
        if self.deployRelease:
            if not (self.releaseNotes and self.tagName):
                raise RuntimeError(
                    "--deployRelease requires --releaseNotes and --tagName"
                )
    ...
    def getGithubLatestTagDate(self):
        ...
        sortedTags = sorted(
            tags, key=lambda tag: LooseVersion(tag["name"]), reverse=True
        )
    ...
    def getNotesFromPRs(self, prs):
        ...
        for pr in prs:
            ...
            if mergeDate < self.startDate:
                continue
            rawReleaseNotes[baseBranch].update(
                {prID: dict(comment=comment, mergeDate=mergeDate)}
            )
    ...
    def createGithubRelease(self):
        ...
        result = req2Json(
            url=self._github("releases"), parameterDict=releaseDict, requestType="POST"
        )
    ...
    def createGitlabRelease(self):
        ...
        result = req2Json(
            url=self._gitlab("releases"), parameterDict=releaseDict, requestType="POST"
        )
