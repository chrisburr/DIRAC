...
class Configuration(object):
    ...
    def __init__(self, confFile, sections=None):
        ...
        if K_CODE in sections:
            self.code_targetPath = self._fullPath(
                config.get(K_CODE, "docs_target_path")
            )
            self.code_customDocsPath = self._fullPath(
                config.get(K_CODE, "customdocs_folder")
            )
            self.code_privateMembers = listify(
                config.get(K_CODE, "document_private_members")
            )
        ...
        if K_COMMANDS in sections:
            self.com_ignore_commands = listify(
                config.get(K_COMMANDS, "ignore_commands")
            )
            self.com_module_docstring = listify(
                config.get(K_COMMANDS, "add_module_docstring")
            )
...
class CLParser(object):
    def __init__(self):
        ...
        self.parser = argparse.ArgumentParser(
            "DiracDocTool", formatter_class=argparse.RawTextHelpFormatter
        )
