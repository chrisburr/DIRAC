...
def getGUID(fileNames, directory=""):
    ...
    gLogger.verbose(
        "Will look for POOL XML Catalog GUIDs in %s for %s"
    ...
    if not generated:
        gLogger.info(
            "Found GUIDs from POOL XML Catalogue for all files: %s"
            % ", ".join(fileNames)
        )
    else:
        gLogger.info(
...
def getType(fileNames, directory=""):
    ...
    gLogger.verbose(
        "Will look for POOL XML Catalog file types in %s for %s"
        % (directory, ", ".join(fileNames))
    )
    ...
    if not generated:
        gLogger.info(
            "Found Types from POOL XML Catalogue for all files: %s"
            % ", ".join(fileNames)
        )
    else:
        gLogger.info(
    ...
    for pattern in patterns:
        ...
        for fname in fileList:
            if fname.endswith(".bak"):
                ...
                gLogger.debug(
                    "Unpacking catalog XML file %s" % (os.path.join(directory, fname))
                )
