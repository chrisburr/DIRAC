...
class FCConditionParser(object):
    ...
    class PluginOperand(object):
        ...
        def __init__(self, tokens):
            ...
            _class = objLoader.loadObject(
                "Resources.Catalog.ConditionPlugins.%s" % self.pluginName,
                self.pluginName,
            )
    ...
    def __call__(self, catalogName, operationName, lfns, condition=None, **kwargs):
        ...
        """
        self.log.debug(
            "Testing %s on %s for %s lfns" % (operationName, catalogName, len(lfns))
        )

        conditionStr = (
            condition
            if condition is not None
            else self.__getConditionFromCS(catalogName, operationName)
        )
        ...
        if conditionStr:
            for lfn in lfns:
                try:
                    evaluatedLfns[lfn] = self.__evaluateCondition(
                        conditionStr, lfn=lfn, **kwargs
                    )
                except Exception as e:
                    self.log.exception(
                        "Exception while evaluation conditions", lException=e
                    )
