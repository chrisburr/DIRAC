...
def getClientCertInfo():
    ...
    if not res["OK"]:
        gLogger.error(
            "getClientCertInfo: Failed to get client proxy information.", res["Message"]
        )
        return res
    proxyInfo = res["Value"]
...
def getDNFromUID(userID):
    ...
    if res == 0:
        ...
        gLogger.debug(
            "LcgFileCatalogClient.getDNFromUID: UID %s maps to %s." % (userID, dn)
        )
        return S_OK(dn)
    else:
...
def getRoleFromGID(groupID, path=None):
    ...
    if res == 0:
        ...
        if role == "lhcb":
            role = "lhcb/Role=user"
        gLogger.debug(
            "LcgFileCatalogClient.getRoleFromGID: GID %s maps to %s." % (groupID, role)
        )
        return S_OK(role)
    else:
...
def getDNUserID(dn):
    ...
    for userMap in users if not error else []:
        if userMap.username == dn:
            ...
            break
    return returnCode(
        userid is None, userid, errMsg="DN does not exist" if not error else ""
    )
...
class LcgFileCatalogClient(FileCatalogClientBase):
    ...
    def __init__(self, **options):
        ...
        if importedLFC is None:
            try:
                ...
                importedLFC = True
                gLogger.debug(
                    "LcgFileCatalogClient.__init__: Successfully imported lfc module."
                )

            except ImportError:
                importedLFC = False
                gLogger.exception(
                    "LcgFileCatalogClient.__init__: Failed to import lfc module."
                )
    ...
    def hasAccess(self, lfns, opType):
        ...
        failed = perms["Failed"]
        successful = dict(
            (path, perms["Successful"][path].get(opType, False))
            for path in perms["Successful"]
        )
                ...
                if not res["OK"]:
                    ...
                    if not res["OK"]:
                        ...
                        try:
                            ...
                            print(
                                "key not found: __getACLInformation returned incomplete dictionary",
                                KeyError,
                            )
                            failed[path] = lfcPerm
                            continue
        ...
        for lfnList in breakListIntoChunks(lfns, 1000):
            ...
            for index in range(len(errCodes)):
                ...
                if errCode == 0:
                    ...
                    if res["OK"]:
                        failed[lfn] = (
                            "The supplied GUID is already used by %s" % res["Value"]
                        )
                else:
    ...
    def removeReplica(self, lfns):
        ...
        for lfn, message in res["Value"]["Failed"].items():
            if "PFN" not in lfns[lfn]:  # Change only if PFN is not there
                failed[
                    lfn
                ] = message  # The replicas are not available, mark the lfn as failed
    ...
    def setReplicaProblematic(self, lfns, revert=False):
        ...
        while lfnsSEs:
            ...
            for lfn in res["Value"]["Failed"]:
                failed.setdefault(lfn, {})[batch[lfn]["SE"]] = res["Value"]["Failed"][
                    lfn
                ]
    ...
    def createDataset(self, datasets):
        ...
        for datasetName, lfns in datasets.items():
            ...
            if not res["OK"]:
                ...
                return S_ERROR(
                    "LcgFileCatalogClient.createDataset: This dataset already exists."
                )
    ...
    def __executeOperation(self, path, method):
        ...
        if not fcn:
            return S_ERROR(
                "Unable to invoke %s, it isn't a member function of LcgFileCatalogClient"
                % method
            )
        res = fcn(path)
    ...
    def __getLFNForPFN(self, pfn):
        ...
        return returnCode(
            error, self.__getLfnForGUID(fstat.guid) if not error else None
        )
    ...
    def __getACLInformation(self, path):
        results, objects = lfc.lfc_getacl(
            self.__fullLfn(path), 256
        )  # lfc.CNS_ACL_GROUP_OBJ)
        if results == -1:
        ...
        for obj in objects:
            if obj.a_type == lfc.CNS_ACL_USER_OBJ:
                ...
                errStr = (
                    "LcgFileCatalogClient.__getACLInformation: ACL type not considered."
                )
    ...
    def __getFileReplicas(self, lfn, allStatus):
        ...
        return returnCode(
            ...
            dict(
                [
                    (replica.host, replica.sfn)
                    for replica in replicaObjects
                    if allStatus or replica.status != "P"
                ]
            )
            if not error
            else None,
    ...
    def __getFileReplicaStatus(self, lfn, se):
        ...
        return returnCode(
            status is None,
            status,
            errMsg="No replica at supplied site" if not error else "",
        )
    ...
    def __checkAddFile(self, lfn, pfn, size, se, guid, checksum):
        ...
        if not res["OK"]:
            ...
            if replicas.get(se) != pfn:
                return S_ERROR(
                    "This LFN %s is already registered with another SE/PFN" % lfn
                )
            return S_OK(False)
    ...
    def __getDirectorySize(self, path, longOutput=False):
        ...
        while loop.pop():
            ...
            if S_ISDIR(entry.filemode):
                ...
                if (
                    (not permissions & S_IWUSR)
                    and (not permissions & S_IWGRP)
                    and (not permissions & S_IWOTH)
                ):
                ...
                if not fileInfo:
                    ...
                    for replica in fileInfo:
                        if replica.host not in pathDict["SiteUsage"]:
                            pathDict["SiteUsage"][replica.host] = {
                                "Files": 0,
                                "Size": 0,
                            }
    ...
    def __createDataset(self, datasetName, lfns):
        ...
        for link, error in res["Value"]["Failed"].items():
            gLogger.error(
                "LcgFileCatalogClient.__createDataset: Failed to create link",
                "for %s: %s" % (link, error),
            )
    ...
    def __removeFilesFromDataset(self, datasetName, lfns):
        ...
        for link, error in res["Value"]["Failed"].items():
            gLogger.error(
                "LcgFileCatalogClient.__removeFilesFromDataset: Failed to remove link",
                "%s: %s" % (link, error),
            )
