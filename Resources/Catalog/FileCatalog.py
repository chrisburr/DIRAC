...
class FileCatalog(object):
    ...
    def w_execute(self, *parms, **kws):
        ...
        for catalogName, oCatalog, master in self.writeCatalogs:
            ...
            if self.call in self.no_lfn_methods:
                ...
                res = self.condParser(
                    catalogName, self.call, fileInfo, condition=condition
                )
                ...
                if master:
                    if any([not valid for valid in condEvals.values()]):
                        gLogger.error(
                            "The master catalog is not valid for some LFNS", condEvals
                        )
                        return S_ERROR(
                            "The master catalog is not valid for some LFNS %s"
                            % condEvals
                        )

                validLFNs = dict(
                    (lfn, fileInfo[lfn]) for lfn in condEvals if condEvals[lfn]
                )
    ...
    def r_execute(self, *parms, **kws):
        ...
        if not successful and not failed:
            return S_ERROR(
                DErrno.EFCERR, "Failed to perform %s from any catalog" % self.call
            )
            ...
            if re.search("Read", catalogConfig["AccessType"]):
                if catalogConfig["Master"]:
                    self.readCatalogs.insert(
                        0, (catalogName, oCatalog, catalogConfig["Master"])
                    )
                else:
                    self.readCatalogs.append(
                        (catalogName, oCatalog, catalogConfig["Master"])
                    )
    ...
    def _getCatalogConfigDetails(self, catalogName):
        ...
        if not result["OK"]:
            errStr = (
                "FileCatalog._getCatalogConfigDetails: Failed to get catalog options."
            )
        ...
        if "Status" not in catalogConfig:
            warnStr = (
                "FileCatalog._getCatalogConfigDetails: 'Status' option not defined."
            )
