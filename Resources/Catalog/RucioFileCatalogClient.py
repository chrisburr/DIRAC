...
class RucioFileCatalogClient(FileCatalogClientBase):
    ...
    def __init__(self, **options):
        ...
        proxyInfo = {"OK": False}
        if os.getenv("RUCIO_AUTH_TYPE") == "x509_proxy" and not os.getenv(
            "X509_USER_PROXY"
        ):
            ...
            if proxyInfo["OK"]:
                ...
                sLog.debug(
                    "X509_USER_PROXY not defined. Using %s" % proxyInfo["Value"]["path"]
                )
        try:
            self._client = Client()
            ...
            if os.getenv("RUCIO_AUTH_TYPE") == "x509_proxy":
                ...
                if proxyInfo["OK"]:
                    ...
                    sLog.debug(
                        "Switching to account %s mapped to proxy %s" % (username, dn)
                    )
        ...
        try:
            ...
            sLog.error(
                "Cannot instantiate RucioFileCatalog interface",
                "error : %s" % repr(err),
            )
            ...
            try:
                ...
                for rep in self.client.list_replicas(didList):
                    if rep:
                        ...
                        for rse in rep["rses"]:
                            if self.convertUnicode:
                                result["Value"]["Successful"][lfn][str(rse)] = str(
                                    rep["rses"][rse][0]
                                )
                            else:
                                result["Value"]["Successful"][lfn][rse] = rep["rses"][
                                    rse
                                ][0]
                    else:
                        for did in didList:
        ...
        for lfn in lfns:
            try:
                ...
                if meta["did_type"] == "CONTAINER":
                    ...
                    for child in self.client.list_content(
                        ...
                        if child["type"] in ["DATASET", "CONTAINER"]:
                            ...
                            if verbose:
                                result["Value"]["Successful"][lfn]["SubDirs"][
                                    childName
                                ] = {
                                    "Status": "-",
                            ...
                            result["Value"]["Successful"][lfn]["Files"][childName] = {
                                "Mode": 509
                            }
                            if verbose:
                                pass
                    ...
                    file_dict = {}
                    for file_did in self.client.list_files(
                        scope=did["scope"], name=did["name"]
                    ):
                        guid = file_did["guid"]
                    ...
                    for rep in self.client.list_replicas(
                        ...
                        if rep:
                            ...
                            if self.convertUnicode:
                                name = str(name)
                            result["Value"]["Successful"][lfn]["Files"][name] = {
                                "Metadata": {},
                                "Replicas": {},
                            }
                            result["Value"]["Successful"][lfn]["Files"][name][
            ...
            try:
                ...
                if meta["did_type"] == "FILE":
                    ...
                    for rule in self.client.list_did_rules(did["scope"], did["name"]):
                        rid = rule["id"]
                        self.client.update_replication_rule(
                            rid, options={"lifetime": -86400}
                        )
                    successful[lfn] = True
                elif meta["did_type"] == "DATASET":
                    rules = {}
                    for lock in self.client.get_dataset_locks(
                        did["scope"], did["name"]
                    ):
            ...
            try:
                ...
                if meta["did_type"] == "FILE":
                    ...
                    for child in self.client.list_content(
                        ...
                        if self.convertUnicode:
                            childName = str(childName)
                        resDict["Successful"][lfn]["SubDirs"][
                            childName
                    ...
                    for child in self.client.list_files(
                        scope=did["scope"], name=did["name"]
                    ):
                    ...
                    for rep in self.client.list_replicas(
                        ...
                        for rse in rep["rses"]:
                            if rse not in resDict["Successful"][lfn]["SiteUsage"]:
                                resDict["Successful"][lfn]["SiteUsage"][rse] = {
                                    "Files": 0,
                                    "Size": 0,
                                }
