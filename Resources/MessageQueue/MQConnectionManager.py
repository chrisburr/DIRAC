...
class MQConnectionManager(object):
    ...
    def addNewMessenger(self, mqURI, messengerType):
        ...
        def generateMessengerId(msgT):
            return msgT + str(
                max(msgIdToInt(self.__getAllMessengersIdWithType(msgT), msgT) or [0])
                + 1
            )
    ...
    def unsubscribe(self, connector, destination, messengerId):
        ...
        return connector.unsubscribe(
            parameters={"destination": destination, "messengerId": messengerId}
        )
    ...
    def stopConnection(self, mqURI, messengerId):
        ...
        try:
            ...
            if not self.__removeMessenger(conn, dest, messengerId):
                return S_ERROR(
                    EMQCONN,
                    "Failed to stop the connection!The messenger %s does not exist!"
                    % messengerId,
                )
            else:
                if "consumer" in messengerId:
                    result = self.unsubscribe(
                        connector, destination=dest, messengerId=messengerId
                    )
                    if not result["OK"]:
                        return result
    ...
    def __getMessengersIdWithType(self, mqConnection, mqDestination, messengerType):
        ...
        return [
            p
            for p in self.__getMessengersId(mqConnection, mqDestination)
            if messengerType in p
        ]
    ...
    def __addMessenger(self, mqConnection, destination, messengerId):
        ...
        if self.__connectionExists(mqConnection):
            ...
            self.__connectionStorage[mqConnection] = {
                "MQConnector": None,
                "destinations": {destination: [messengerId]},
            }
        return True
