...
class StompMQConnector(MQConnector):
    ...
    RECONNECT_SLEEP_INITIAL = (
        1  # [s]  Initial delay before reattempting to establish a connection.
    )
    RECONNECT_SLEEP_INCREASE = (
        0.5  # Factor by which sleep delay is increased 0.5 means increase by 50%.
    )
    ...
    def setupConnection(self, parameters=None):
        ...
        reconnectSleepInitial = self.parameters.get(
            "ReconnectSleepInitial", StompMQConnector.RECONNECT_SLEEP_INITIAL
        )
        reconnectSleepIncrease = self.parameters.get(
            "ReconnectSleepIncrease", StompMQConnector.RECONNECT_SLEEP_INCREASE
        )
        reconnectSleepMax = self.parameters.get(
            "ReconnectSleepMax", StompMQConnector.RECONNECT_SLEEP_MAX
        )
        reconnectSleepJitter = self.parameters.get(
            "ReconnectSleepJitter", StompMQConnector.RECONNECT_SLEEP_JITTER
        )
        reconnectAttemptsMax = self.parameters.get(
            "ReconnectAttemptsMax", StompMQConnector.RECONNECT_ATTEMPTS_MAX
        )
    ...
    def subscribe(self, parameters=None):
        ...
        for connection in self.connections.itervalues():
            try:
                ...
                connection.subscribe(destination=dest, id=mId, ack=ack, headers=headers)
            except Exception as e:
    ...
    def unsubscribe(self, parameters):
        ...
        if fail:
            return S_ERROR(
                EMQUKN, "Failed to unsubscribe from at least one destination"
            )
    ...
    def on_disconnected(self):
        ...
        try:
            if self.callback:
                ...
                if res["OK"]:
                    self.log.info(
                        "Reconnection successful to broker", "%s" % self.serverIP
                    )
                else:
                    self.log.error(
                        "Error reconnectiong broker", "%s: %s" % (self.serverIP, res)
                    )
