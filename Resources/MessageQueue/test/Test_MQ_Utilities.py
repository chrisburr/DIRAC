...
SIMILAR_QUEUE_CONFIG.update(
    {
        ...
        "%s/%s/Queues/%s1/Acknowledgement"
        % (ROOT_PATH, MQSERVICE_NAME, QUEUE_NAME): True,
    }
)
...
AMBIGIOUS_QUEUE_CONFIG.update(
    {
        ...
        "%s/%s/Queues/%s/Acknowledgement"
        % (ROOT_PATH, DIFFERENT_MQSERVICE_NAME, QUEUE_NAME): True,
    }
)
...
class Test_getMQParamFromCSSuccessTestCase(unittest.TestCase):
    ...
    def test_getQueue(self):
        ...
        result = module.getMQParamsFromCS(
            mqURI=MQSERVICE_NAME + "::" + QUEUE_TYPE + "::" + QUEUE_NAME
        )
    ...
    def test_getTopic(self):
        ...
        result = module.getMQParamsFromCS(
            mqURI=MQSERVICE_NAME + "::" + TOPIC_TYPE + "::" + TOPIC_NAME
        )
        self.assertTrue(result["OK"])
    ...
    def test_getDestinationType(self):
        self.assertEqual(
            module.getDestinationType("bblabl.ch::Topics::MyTopic"), "Topics"
        )
        self.assertEqual(
            module.getDestinationType("bblabl.ch::Queues::MyQueue"), "Queues"
        )
    ...
    def test_getDestinationAddress(self):
        self.assertEqual(
            module.getDestinationAddress("bblabl.ch::Topics::MyTopic"), "/topic/MyTopic"
        )
        self.assertEqual(
            module.getDestinationAddress("bblabl.ch::Queues::MyQueue"), "/queue/MyQueue"
        )
...
class Test_getMQParamFromCSFailureTestCase(unittest.TestCase):
    ...
    def test_getQueueByInvalidName(self):
        ...
        result = module.getMQParamsFromCS(
            mqURI=MQSERVICE_NAME + "::" + QUEUE_TYPE + "::" + "InvalidName"
        )
        self.assertFalse(result["OK"])
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(
        Test_getMQParamFromCSSuccessTestCase
    )
    suite.addTests(
        unittest.defaultTestLoader.loadTestsFromTestCase(
            Test_getMQParamFromCSFailureTestCase
        )
    )
    suite.addTests(
        unittest.defaultTestLoader.loadTestsFromTestCase(
            Test_generateDefaultCallbackTestCase
        )
    )
