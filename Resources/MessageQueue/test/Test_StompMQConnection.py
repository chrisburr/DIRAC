...
SSL_PARAMETERS.update(
    {"SSLVersion": SSL_VERSION, "HostCertificate": HOSTCERT, "HostKey": HOSTKEY}
)
...
class StompMQConnectorSuccessTestCase(unittest.TestCase):
    ...
    def test_createStompListener(self):
        ...
        listener = module.StompListener(
            testCallback, ACKNOWLEDGEMENT, connection, MESSENGERID
        )
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(
        StompMQConnectorSuccessTestCase
    )
    suite.addTests(
        unittest.defaultTestLoader.loadTestsFromTestCase(
            StompMQConnectorFailureTestCase
        )
    )
