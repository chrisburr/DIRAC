...
class ProxyStorage(StorageBase):
        ...
        for src_url in urls.keys():
            ...
            if not res["OK"]:
                ...
                if not res["OK"]:
                    ...
                    if destSize == -1:
                        errStr = (
                            "ProxyStorage.getFile: Failed to get the local file size."
                        )
                        gLogger.error(errStr, dest_file)
                        failed[src_url] = errStr
                    else:
    ...
    def putFile(self, path, sourceSize=0):
        ...
        if sourceSize:
         ...
         a replication is requested."
            ...
            return client.callProxyMethod(
                self.name, "putFile", [path], {"sourceSize": sourceSize}
            )
    ...
    def pinFile(self, path, lifetime=60 * 60 * 24):
        client = RPCClient(self.url)
        return client.callProxyMethod(
            self.name, "pinFile", [path], {"lifetime": lifetime}
        )
    ...
    def removeDirectory(self, path, recursive=False):
        client = RPCClient(self.url)
        return client.callProxyMethod(
            self.name, "removeDirectory", [path], {"recursive": recursive}
        )
    ...
    def __executeOperation(self, url, method):
        ...
        if not fcn:
            return S_ERROR(
                "Unable to invoke %s, it isn't a member function of ProxyStorage"
                % method
            )
        res = fcn([url])
        if not res["OK"]:
