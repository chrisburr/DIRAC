...
class StorageElementItem(object):
    ...
    __deprecatedArguments = [
        "singleFile",
        "singleDirectory",
    ]  # Arguments that are now useless
    ...
    def __init__(self, name, plugins=None, vo=None, hideExceptions=False):
        ...
        proxiedProtocols = gConfig.getValue(
            "/LocalSite/StorageElements/ProxyProtocols", ""
        ).split(",")
        self.useProxy = (
            gConfig.getValue(
        ...
        if self.valid:
            ...
            self.localAccessProtocolList = (
                accessProto if accessProto else self.__dmsHelper.getAccessProtocols()
            )
            ...
            self.localWriteProtocolList = (
                writeProto if writeProto else self.__dmsHelper.getWriteProtocols()
            )
    ...
    def checksumType(self):
        ...
        """
        self.log.getSubLogger("checksumType").verbose(
            "get checksum type for %s." % self.name
        )
        return (
            self.options["ChecksumType"].upper()
            if "ChecksumType" in self.options
            else gConfig.getValue(
                "/Resources/StorageElements/ChecksumType", "ADLER32"
            ).upper()
        )
    ...
    def getOccupancy(self, unit="MB", **kwargs):
        ...
        if not filteredPlugins:
            return S_ERROR(
                errno.EPROTONOSUPPORT, "No storage plugins to query the occupancy"
            )
        ...
        if occupancyPlugin:
            res = ObjectLoader().loadObject(
                "Resources.Storage.OccupancyPlugins.%s" % occupancyPlugin,
                occupancyPlugin,
            )
            if not res["OK"]:
                return S_ERROR(
                    errno.EPROTONOSUPPORT,
                    "Failed to load occupancy plugin %s" % occupancyPlugin,
                )
        ...
        for storage in filteredPlugins:
            ...
            if res["OK"]:
                ...
                if "SpaceReservation" not in occupancyDict:
                    occupancyDict["SpaceReservation"] = self.options.get(
                        "SpaceReservation"
                    )
    ...
    def checkOccupancy(self, occupancyDict, unit):
        ...
        if set(occupancyDict) & mandatoryParams != mandatoryParams:
            ...
            msg = "Missing mandatory parameters %s" % str(
                mandatoryParams - set(occupancyDict)
            )
            log.error(msg)
            return S_ERROR(msg)
        ...
        if unit != "B":
            for space in ["Total", "Free"]:
                ...
                if convertedSpace == -sys.maxsize:
                    msg = "Error converting %s space from MB to %s: %s" % (
                        space,
                        unit,
                        occupancyDict[space],
                    )
    ...
    def status(self):
        ...
        retDict["Read"] = not (
            "ReadAccess" in self.options
            and self.options["ReadAccess"] not in ("Active", "Degraded")
        )
        retDict["Write"] = not (
            "WriteAccess" in self.options
            and self.options["WriteAccess"] not in ("Active", "Degraded")
        )
        retDict["Remove"] = not (
            "RemoveAccess" in self.options
            and self.options["RemoveAccess"] not in ("Active", "Degraded")
        )
        if retDict["Read"]:
            retDict["Check"] = True
        else:
            retDict["Check"] = not (
                "CheckAccess" in self.options
                and self.options["CheckAccess"] not in ("Active", "Degraded")
            )
        diskSE = True
        tapeSE = False
    ...
    def isValid(self, operation=None):
        ...
        log.verbose(
            "Determining if the StorageElement %s is valid for VO %s"
            % (self.name, self.vo)
        )
        ...
        if not self.valid:
            log.debug("Failed to create StorageElement plugins.", self.errorReason)
            return S_ERROR(
                "SE.isValid: Failed to create StorageElement plugins: %s"
                % self.errorReason
            )
        ...
        if "VO" in self.options and self.vo not in self.options["VO"]:
            ...
            return S_ERROR(
                errno.EACCES,
                "StorageElement.isValid: StorageElement is not allowed for VO",
            )
        log.verbose(
        ...
        if (not operation) and (not reading) and (not writing) and (not checking):
            ...
            return S_ERROR(
                errno.EACCES, "SE.isValid: Read, write and check access not permitted."
            )
        ...
        elif operation in self.writeMethods or (
            operation.lower() in ("write", "writeaccess")
        ):
            operation = "WriteAccess"
        elif operation in self.removeMethods or (
            operation.lower() in ("remove", "removeaccess")
        ):
            operation = "RemoveAccess"
        elif operation in self.checkMethods or (
            operation.lower() in ("check", "checkaccess")
        ):
            operation = "CheckAccess"
        else:
            log.debug("The supplied operation is not known.", operation)
            return S_ERROR(
                DErrno.ENOMETH, "SE.isValid: The supplied operation is not known."
            )
        ...
        if operation == "CheckAccess":
            if not reading:
                if not checking:
                    ...
                    return S_ERROR(
                        errno.EACCES,
                        "SE.isValid: Check access not currently permitted.",
                    )
        if operation == "ReadAccess":
            if not reading:
                log.debug("Read access not currently permitted.")
                return S_ERROR(
                    errno.EACCES, "SE.isValid: Read access not currently permitted."
                )
        if operation == "WriteAccess":
            if not writing:
                log.debug("Write access not currently permitted.")
                return S_ERROR(
                    errno.EACCES, "SE.isValid: Write access not currently permitted."
                )
        if operation == "RemoveAccess":
            if not removing:
                log.debug("Remove access not currently permitted.")
                return S_ERROR(
                    errno.EACCES, "SE.isValid: Remove access not currently permitted."
                )
        return S_OK()

    def getPlugins(self):
        """Get the list of all the plugins defined for this Storage Element"""
        self.log.getSubLogger("getPlugins").verbose(
            "Obtaining all plugins of %s." % self.name
        )
        if not self.valid:
    ...
    def getStorageParameters(self, plugin=None, protocol=None):
        ...
        log.verbose("Obtaining storage parameters for %s for %s." % (self.name, reqStr))
    ...
    def __getAllProtocols(self, protoType):
        ...
        return set(
            reduce(
                lambda x, y: x + y,
                [plugin.protocolParameters[protoType] for plugin in self.storages],
            )
        )
    ...
    def generateTransferURLsBetweenSEs(self, lfns, sourceSE, protocols=None):
        ...
        sourceSEStorages = sorted(
            ...
            key=lambda x: getIndexInList(
                x.getParameters()["Protocol"], commonProtocols
            ),
        )
            ...
            return S_OK(
                {
                    ...
                    "Failed": failed,
                    "Protocols": (nativeSrcProtocol, nativeDestProtocol),
                }
            )
        ...
        for lfn in lfns:
            if self.useCatalogURL:
                ...
                if not result["OK"]:
                    ...
                    failed[lfn] = (
                        "%s %s" % (failed[lfn], errStr) if lfn in failed else errStr
                    )
                else:
                    urlDict[result["Value"]] = lfn
    ...
    def __filterPlugins(self, methodName, protocols=None, inputProtocol=None):
        ...
        if methodName in self.readMethods + self.checkMethods:
            ...
            if protocols:
                ...
                for plugin in self.storages:
                    if (
                        set(plugin.protocolParameters.get("OutputProtocols", []))
                        & setProtocol
                    ):
                        log.debug(
        ...
        for plugin in self.storages:
            ...
            if (
                not (pluginName in self.remotePlugins)
                and not localSE
                and not pluginName == "Proxy"
            ):
                # If the SE is not local then we can't use local protocols
                log.debug(
                    "Local protocol not appropriate for remote use: %s." % pluginName
                )
                continue
    ...
    def __executeMethod(self, lfn, *args, **kwargs):
        ...
        successful = {}
        failed = {}
        filteredPlugins = self.__filterPlugins(
        ...
        for storage in filteredPlugins:
            ...
            log.verbose(
                "Generating %s protocol URLs for %s." % (len(lfnDict), pluginName)
            )
            ...
            if not urlDict:
                log.verbose(
                    "__executeMethod No urls generated for protocol %s." % pluginName
                )
            else:
                log.verbose(
                ...
                self.addAccountingOperation(
                    urlsToUse, startDate, elapsedTime, storageParameters, res
                )

                if not res["OK"]:
                    errStr = "Completely failed to perform %s." % self.methodName
                    log.verbose(
                        errStr, "with plugin %s: %s" % (pluginName, res["Message"])
                    )
                    for lfn in urlDict.values():
                        if lfn not in failed:
                            failed[lfn] = ""
                        failed[lfn] = (
                            "%s %s" % (failed[lfn], res["Message"])
                            if failed[lfn]
                            else res["Message"]
                        )
    ...
    def addAccountingOperation(
        self, lfns, startDate, elapsedTime, storageParameters, callRes
    ):
        """
        ...
        baseAccountingDict["User"] = (
            getProxyInfo().get("Value", {}).get("username", "unknown")
        )
        ...
        if not callRes["OK"]:
            ...
            if self.methodName in ("putFile", "getFile"):
                ...
                totalSize = sum(
                    val.get("Size", 0) for val in succ.values() if isinstance(val, dict)
                )
                totalSucc = sum(
                    val.get("Files", 0)
            ...
            if callRes["Value"]["Failed"]:
                ...
                if not accRes["OK"]:
                    self.log.error(
                        "Could not send failed accounting report", accRes["Message"]
                    )
