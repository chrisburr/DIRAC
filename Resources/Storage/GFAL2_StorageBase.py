...
class GFAL2_StorageBase(StorageBase):
    ...
    def __init__(self, storageName, parameters):
        ...
        # file size
        self.disableTransferChecksum = (
            True if (parameters.get("DisableChecksum") == "True") else False
        )
        ...
        self.ctx.set_opt_boolean(
            "GRIDFTP PLUGIN",
            "SESSION_REUSE",
            os.environ.get("DIRAC_GFAL_GRIDFTP_SESSION_REUSE", "no").lower()
            in ["true", "yes"],
        )
        ...
        self.stageTimeout = gConfig.getValue(
            "/Resources/StorageElements/StageTimeout", 12 * 60 * 60
        )
        # gfal2Timeout, amount of time it takes until an operation times out
        self.gfal2Timeout = gConfig.getValue(
            "/Resources/StorageElements/GFAL_Timeout", 100
        )
        # set the gfal2 default protocols, e.g. used when trying to retrieve transport url
        self.defaultLocalProtocols = gConfig.getValue(
            "/Resources/StorageElements/DefaultProtocols", []
        )

        # # set checksum type, by default this is 0 (GFAL_CKSM_NONE)
        self.checksumType = gConfig.getValue(
            "/Resources/StorageElements/ChecksumType", "0"
        )
    ...
    def exists(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.exists: Checking the existence of %s path(s)" % len(urls)
        )
    ...
    def __singleExists(self, path):
        ...
        try:
            self.ctx.stat(
                path
            )  # If path doesn't exist this will raise an error - otherwise path exists
    ...
    def isFile(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.isFile: checking whether %s path(s) are file(s)."
            % len(urls)
        )
    ...
    def __putSingleFile(self, src_file, dest_url, sourceSize):
        ...
        if any(
            src_file.startswith(protocol + ":")
            for protocol in self.protocolParameters["InputProtocols"]
        ):
            src_url = src_file
            if not sourceSize:
        ...
        if not res["OK"]:
            log.debug(
                "Failed to create destination folder %s" % dest_path, res["Message"]
            )
            return res
        ...
        try:
            ...
            # no checksum check, compare file sizes for verfication
                ...
                if destSize == sourceSize:
                    ...
                    return S_ERROR(
                        "%s srcSize: %s destSize: %s" % (errStr, sourceSize, destSize)
                    )
        except gfal2.GError as e:
            # ##
    ...
    def getFile(self, path, localPath=False):
        ...
        self.log.debug(
            "GFAL2_StorageBase.getFile: Trying to download %s files." % len(urls)
        )
        ...
        try:
            ...
            if useChecksum:
                ...
                if destSize == remoteSize:
                    ...
                    errStr = (
                        "File sizes don't match. Something went wrong. Removing local file %s"
                        % dest_file
                    )
            ...
            errStr = "Could not copy %s to %s, [%d] %s" % (
                src_url,
                dest,
                e.code,
                e.message,
    ...
    def removeFile(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.removeFile: Attempting to remove %s files" % len(urls)
        )
    ...
    def getFileSize(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.getFileSize: Trying to determine file size of %s files"
            % len(urls)
        )
    ...
    def getFileMetadata(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.getFileMetadata: trying to read metadata for %s paths"
            % len(urls)
        )
    ...
    def _getSingleFileMetadata(self, path):
        ...
        """
        self.log.debug(
            "GFAL2_StorageBase._getSingleFileMetadata: trying to read metadata for %s"
            % path
        )
        ...
        if not metaDict["File"]:
            errStr = (
                "GFAL2_StorageBase._getSingleFileMetadata: supplied path is not a file"
            )
    ...
    def __getSingleMetadata(self, path):
        ...
        if self._defaultExtendedAttributes is not None:
            res = self._getExtendedAttributes(
                path, attributes=self._defaultExtendedAttributes
            )
            if not res["OK"]:
    ...
    def prestageFile(self, path, lifetime=86400):
        ...
        self.log.debug(
            "GFAL2_StorageBase.prestageFile: Attempting to issue stage requests for %s file(s)."
            % len(urls)
        )
    ...
    def __prestageSingleFile(self, path, lifetime):
        ...
        try:
            (status, token) = self.ctx.bring_online(
                path, lifetime, self.stageTimeout, True
            )
    ...
    def prestageFileStatus(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.prestageFileStatus: Checking the staging status for %s file(s)."
            % len(urls)
        )
    ...
    def pinFile(self, path, lifetime=86400):
        ...
        self.log.debug(
            "GFAL2_StorageBase.pinFile: Attempting to pin %s file(s)." % len(urls)
        )
        failed = {}
        successful = {}
        for url in urls:
    ...
    def __pinSingleFile(self, path, lifetime):
        ...
        try:
            ...
            errStr = (
                "GFAL2_StorageBase.__pinSingleFile: Error occured while pinning file"
            )
            log.debug(errStr, "%s %s" % (path, repr(e)))
            return S_ERROR(e.code, "%s %s" % (errStr, repr(e)))
        finally:
    ...
    def releaseFile(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.releaseFile: Attempting to release %s file(s)."
            % len(urls)
        )
    ...
    def __getChecksum(self, path, checksumType=None):
        ...
        if not checksumType:
            errStr = (
                "No checksum type set by the storage element. Can't retrieve checksum"
            )
    ...
    def __parseStatInfoFromApiOutput(self, statInfo):
        ...
        if metaDict["File"]:
            ...
            metaDict["LastAccess"] = (
                self.__convertTime(statInfo.st_atime) if statInfo.st_atime else "Never"
            )
            metaDict["ModTime"] = (
                self.__convertTime(statInfo.st_mtime) if statInfo.st_mtime else "Never"
            )
            metaDict["StatusChange"] = (
                self.__convertTime(statInfo.st_ctime) if statInfo.st_ctime else "Never"
            )
    ...
    def createDirectory(self, path):
        ...
        failed = {}
        self.log.debug(
            "createDirectory: Attempting to create %s directories." % len(urls)
        )
        for url in urls:
    ...
    def isDirectory(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.isDirectory: checking whether %s path(s) are directory(ies)."
            % len(urls)
        )
    ...
    def listDirectory(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.listDirectory: Attempting to list %s directories"
            % len(urls)
        )
    ...
    def getDirectory(self, path, localPath=False):
        ...
        log.debug(
            "Attempting to get local copies of %s directories. %s" % (len(urls), urls)
        )
    ...
    def __getSingleDirectory(self, src_dir, dest_dir):
        ...
        if not res["OK"]:
            log.debug(
                "Failed to find the source directory: %s %s" % (res["Message"], src_dir)
            )
            return res
        ...
        for sFile in sFilesDict:
            ...
            res = self._getSingleFile(
                ...
                disableChecksum=self.disableTransferChecksum,
            )
            if res["OK"]:
                filesReceived += 1
        ...
        log.debug(
            "Trying to recursively download the %s directories" % len(subDirsDict)
        )
    ...
    def putDirectory(self, path):
        ...
        for destDir, sourceDir in urls.iteritems():
            ...
            if res["OK"]:
                if res["Value"]["AllPut"]:
                    log.debug(
                        "Successfully put directory to remote storage: %s" % destDir
                    )
                    successful[destDir] = {
                        "Files": res["Value"]["Files"],
                        "Size": res["Value"]["Size"],
                    ...
                    log.debug(
                        "Failed to put entire directory to remote storage.", destDir
                    )
                    failed[destDir] = {
                        "Files": res["Value"]["Files"],
                        "Size": res["Value"]["Size"],
                ...
                log.debug(
                    "Completely failed to put directory to remote storage.", destDir
                )
    ...
    def __putSingleDirectory(self, src_directory, dest_directory):
        ...
        if not os.path.isdir(src_directory):
            errStr = (
                "The supplied source directory does not exist or is not a directory."
            )
        ...
        for fileName in contents:
            ...
            # localPath is another folder, start recursion
                ...
                if not res["OK"]:
                    log.debug(
                        "Failed to put directory to storage. Skipping", res["Message"]
                    )
                    allSuccessful = False
                else:
    ...
    def __removeSingleDirectory(self, path, recursive=False):
        ...
        if (recursive and allRemoved) or (
            not recursive and removedAllFiles and not subDirsDict
        ):
            try:
                status = self.ctx.rmdir(path)
                if status < 0:
                    errStr = (
                        "Error occured while removing directory. Status: %s" % status
                    )
                    log.debug(errStr)
                    allRemoved = False
            except gfal2.GError as e:
    ...
    def getDirectorySize(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.getDirectorySize: Attempting to get size of %s directories"
            % len(urls)
        )
    ...
    def __getSingleDirectorySize(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.__getSingleDirectorySize: Attempting to get the size of directory %s"
            % path
        )
        ...
        self.log.debug(
            "GFAL2_StorageBase.__getSingleDirectorySize: Successfully obtained size of %s."
            % path
        )
        subDirectories = len(res["Value"]["SubDirs"])
        return S_OK(
            {"Files": directoryFiles, "Size": directorySize, "SubDirs": subDirectories}
        )
    ...
    def getDirectoryMetadata(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.getDirectoryMetadata: Attempting to fetch metadata."
        )
    ...
    def __getSingleDirectoryMetadata(self, path):
        ...
        """
        self.log.debug(
            "GFAL2_StorageBase.__getSingleDirectoryMetadata: Fetching metadata of directory %s."
            % path
        )
