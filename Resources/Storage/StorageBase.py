...
class StorageBase(object):
    ...
    def __init__(self, name, parameterDict):
        ...
        if hasattr(self, "_INPUT_PROTOCOLS"):
            self.protocolParameters["InputProtocols"] = getattr(
                self, "_INPUT_PROTOCOLS"
            )
        else:
            self.protocolParameters["InputProtocols"] = [
                self.protocolParameters["Protocol"],
                "file",
            ]

        if hasattr(self, "_OUTPUT_PROTOCOLS"):
            self.protocolParameters["OutputProtocols"] = getattr(
                self, "_OUTPUT_PROTOCOLS"
            )
        else:
            self.protocolParameters["OutputProtocols"] = [
                self.protocolParameters["Protocol"]
            ]
    ...
    def constructURLFromLFN(self, lfn, withWSUrl=False):
        ...
        if voLFN != self.se.vo and voLFN != "SandBox" and voLFN != "Sandbox":
            return S_ERROR(
                "LFN (%s) path must start with VO name (%s)" % (lfn, self.se.vo)
            )
    ...
    def _isInputURL(self, url):
        ...
        if (
            not urlDict["Protocol"]
            and "file" in self.protocolParameters["InputProtocols"]
        ):
            return S_OK(True)
