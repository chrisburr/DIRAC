...
class fake_gConfig(object):
    ...
    def getOptionsDict(self, path):
        ...
        options = dict(
            (opt, val)
            for opt, val in csSection.iteritems()
    ...
    def getOptions(self, path):
        ...
        if not csSection:
            return S_ERROR("Not a valid section")
        options = [
            opt for opt, val in csSection.iteritems() if not isinstance(val, dict)
        ]
        return S_OK(options)
...
class StorageFactoryStandaloneTestCase(unittest.TestCase):
    ...
    def test_standalone(self, _sf_generateStorageObject, _rss_getSEStatus):
        ...
        self.assertDictEqual(
            storages["StorageOptions"], {"BackendType": "Eos", "SEType": "T0D1"}
        )
...
class StorageFactorySimpleInheritance(unittest.TestCase):
    ...
    def test_simple_inheritance(self, _sf_generateStorageObject, _rss_getSEStatus):
        ...
        self.assertDictEqual(
            storages["StorageOptions"],
            {"BackendType": "Eos", "SEType": "T0D1", "BaseSE": "CERN-BASE"},
        )
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(
        StorageFactoryStandaloneTestCase
    )
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(
            StorageFactorySimpleInheritance
        )
    )
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(StorageFactoryWeirdDefinition)
    )
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(StorageFactoryPureAbstract)
    )
