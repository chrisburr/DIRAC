...
class fake_SRM2Plugin(StorageBase):
    ...
    def getTransportURL(self, path, protocols=False):
        return S_OK(
            {"Successful": dict.fromkeys(path, "srm:getTransportURL"), "Failed": {}}
        )
...
class fake_XROOTPlugin(StorageBase):
    ...
    def getTransportURL(self, path, protocols=False):
        return S_OK(
            {"Successful": dict.fromkeys(path, "root:getTransportURL"), "Failed": {}}
        )
...
class fake_GSIFTPPlugin(StorageBase):
    ...
    def getTransportURL(self, path, protocols=False):
        return S_OK(
            {"Successful": dict.fromkeys(path, "gsiftp:getTransportURL"), "Failed": {}}
        )


def mock_StorageFactory_generateStorageObject(
    storageName, pluginName, parameters, hideExceptions=False
):
...
class TestBase(unittest.TestCase):
    ...
    def setUp(self, _mk_generateStorage, _mk_isLocalSE, _mk_addAccountingOperation):
        ...
        self.testCfgFileName = os.path.join(
            tempfile.gettempdir(), "test_StorageElement.cfg"
        )
        cfgContent = """
    DIRAC
    {
    ...
    def test_03_multiProtocolThirdParty(self, _mk_isLocalSE, _mk_addAccounting):
        ...
        res = self.seD.replicateFile(
            {lfn: srcUrl}, sourceSize=123, inputProtocol="root"
        )
