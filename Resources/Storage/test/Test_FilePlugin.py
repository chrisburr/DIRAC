...
def mock_StorageFactory_getConfigStorageName(
    storageName, referenceType, seConfigPath=None
):
    resolvedName = storageName
    return S_OK(resolvedName)


def mock_StorageFactory_getConfigStorageOptions(
    storageName, derivedStorageName=None, seConfigPath=None
):
...
def mock_StorageFactory_getConfigStorageProtocols(
    storageName, derivedStorageName=None, seConfigPath=None
):
...
class TestBase(unittest.TestCase):
    ...
    def setUp(
        ...
        with open(
            os.path.join(self.srcPath, self.existingFile.replace("/test/", "")), "w"
        ) as f:
        ...
        with open(
            os.path.join(self.srcPath, self.subFile.replace("/test/", "")), "w"
        ) as f:
    ...
    def test_02_FileTest(self, mk_isLocalSE, mk_addAccounting):
        ...
        self.assertTrue(
            os.strerror(errno.ENOENT) in res["Value"]["Failed"][self.nonExistingFile],
            res,
        )
        ...
        self.assertTrue(
            os.strerror(errno.EISDIR) in res["Value"]["Failed"][self.subDir], res
        )
        ...
        self.assertTrue(res["OK"], res)
        self.assertEqual(
            res["Value"]["Successful"][self.existingFile], self.existingFileSize
        )
        self.assertTrue(
            os.strerror(errno.ENOENT) in res["Value"]["Failed"][self.nonExistingFile],
            res,
        ...
        self.assertTrue(
            os.path.exists(
                os.path.join(self.destPath, os.path.basename(self.existingFile))
            )
        )
        self.assertEqual(res["Value"]["Successful"][self.subFile], self.subFileSize)
        self.assertTrue(
            os.path.exists(os.path.join(self.destPath, os.path.basename(self.subFile)))
        )
        self.assertTrue(
            os.strerror(errno.ENOENT) in res["Value"]["Failed"][self.nonExistingFile],
            res,
    ...
    def test_04_putDirectory(self, mk_isLocalSE, mk_addAccounting):
        ...
        self.assertTrue(res["OK"], res)
        self.assertEqual(
            res["Value"]["Successful"][self.subDir],
            {"Files": 1, "Size": self.subFileSize, "SubDirs": 0},
        )
        self.assertEqual(
            res["Value"]["Successful"]["/test"],
            {"Files": 1, "Size": self.existingFileSize, "SubDirs": 1},
        )
        self.assertTrue(
            os.strerror(errno.ENOENT) in res["Value"]["Failed"][self.nonExistingFile],
            res,
        ...
        self.assertTrue(
            os.strerror(errno.ENOTDIR) in res["Value"]["Failed"][self.existingFile], res
        )
        self.assertTrue(
            os.strerror(errno.ENOENT) in res["Value"]["Failed"][nonExistingDir], res
        )
        ...
        self.assertEqual(
            res["Value"]["Successful"][self.subDir],
            {"Files": [self.subFile], "SubDirs": []},
        )
        self.assertEqual(
            res["Value"]["Successful"]["/test"],
            {"Files": [self.existingFile], "SubDirs": [self.subDir]},
        )
        self.assertTrue(
            os.strerror(errno.ENOENT) in res["Value"]["Failed"][self.nonExistingFile],
            res,
        ...
        self.assertEqual(
            res["Value"]["Successful"][self.subDir],
            {"Files": 1, "Size": self.subFileSize},
        )
        self.assertTrue(
            os.path.exists(self.destPath + self.subFile.replace("/test", ""))
        )
        self.assertEqual(
            res["Value"]["Failed"][self.nonExistingFile], {"Files": 0, "Size": 0}
        )
        self.assertEqual(
            res["Value"]["Failed"][self.existingFile], {"Files": 0, "Size": 0}
        )
        self.assertEqual(
            res["Value"]["Failed"][nonExistingDir], {"Files": 0, "Size": 0}
        )
        ...
        self.assertTrue(res["OK"], res)
        self.assertEqual(
            res["Value"]["Failed"][nonExistingDir],
        ...
        self.assertTrue(res["OK"], res)
        self.assertEqual(
            res["Value"]["Failed"][self.nonExistingFile],
            {"FilesRemoved": 0, "SizeRemoved": 0},
        )
        ...
        self.assertTrue(res["OK"], res)
        self.assertEqual(
            res["Value"]["Failed"][self.existingFile],
            {"FilesRemoved": 0, "SizeRemoved": 0},
        )
        ...
        self.assertEqual(
            res["Value"]["Successful"]["/test"],
            {"FilesRemoved": 1, "SizeRemoved": self.subFileSize},
        )
