...
class FileTestCases(StorageElementTestCase):
    def test_exists(self):
        ...
        removeFileRes = returnSingleResult(
            self.storageElement.removeFile(destinationFilePath)
        )
        # Check removed file
        missingExistsRes = returnSingleResult(
            self.storageElement.exists(destinationFilePath)
        )
        # Check directories are handled properly
        destinationDir = os.path.dirname(destinationFilePath)
        directoryExistsRes = returnSingleResult(
            self.storageElement.exists(destinationDir)
        )
    ...
    def test_isFile(self):
        ...
        missingIsFileRes = returnSingleResult(
            self.storageElement.isFile(destinationFilePath)
        )
        # Check directories are handled properly
        destinationDir = os.path.dirname(destinationFilePath)
        directoryIsFileRes = returnSingleResult(
            self.storageElement.isFile(destinationDir)
        )
    ...
    def test_getFile(self):
        ...
        getFileRes = returnSingleResult(
            self.storageElement.getFile(destinationFilePath)
        )
        # Now remove the destination file
        removeFileRes = returnSingleResult(
            self.storageElement.removeFile(destinationFilePath)
    ...
    def test_getFileMetadata(self):
        ...
        getFileMetadataRes = returnSingleResult(
            self.storageElement.getFileMetadata(destinationFilePath)
        )
        # Now remove the destination file
        removeFileRes = returnSingleResult(
            self.storageElement.removeFile(destinationFilePath)
        ...
        # Get metadata for a removed file
        getMissingFileMetadataRes = returnSingleResult(
            self.storageElement.getFileMetadata(destinationFilePath)
        )
        # Check directories are handled properly
        destinationDir = os.path.dirname(destinationFilePath)
        directoryMetadataRes = returnSingleResult(
            self.storageElement.getFileMetadata(destinationDir)
        )
    ...
    def test_getFileSize(self):
        ...
        putFileRes = returnSingleResult(self.storageElement.putFile(fileDict))
        # Get the file metadata
        getFileSizeRes = returnSingleResult(
            self.storageElement.getFileSize(destinationFilePath)
        )
        # Now remove the destination file
        removeFileRes = returnSingleResult(
            self.storageElement.removeFile(destinationFilePath)
        ...
        # Get metadata for a removed file
        getMissingFileSizeRes = returnSingleResult(
            self.storageElement.getFileSize(destinationFilePath)
        )
        # Check directories are handled properly
        destinationDir = os.path.dirname(destinationFilePath)
        directorySizeRes = returnSingleResult(
            self.storageElement.getFileSize(destinationDir)
        )
    ...
    def test_getURL(self):
        ...
        getMissingTurlRes = self.storageElement.getURL(
            destinationFilePath, protocol="dips"
        )
        ...
        self.assertTrue(
            type(getTurlRes["Value"]["Successful"][destinationFilePath])
            in six.string_types
        )
        # Check that the removal was done correctly
        self.assertTrue(removeFileRes["OK"])
...
class DirectoryTestCases(StorageElementTestCase):
    ...
    def test_isDirectory(self):
        ...
        self.assertTrue(
            nonExistantDirRes["Value"]["Failed"][nonExistantDir]
    ...
    def test_listDirectory(self):
        ...
        self.assertEqual(
            len(listDirRes["Value"]["Successful"][destDirectory]["Files"].keys()),
            self.numberOfFiles,
        )
        ...
        if removeDirRes["Value"]["Successful"][destDirectory]["FilesRemoved"]:
            self.assertEqual(
                removeDirRes["Value"]["Successful"][destDirectory]["FilesRemoved"],
                self.numberOfFiles,
            )
            self.assertEqual(
                removeDirRes["Value"]["Successful"][destDirectory]["SizeRemoved"],
                self.numberOfFiles * sizeOfLocalFile,
            )
        self.assertTrue(
            type(removeDirRes["Value"]["Successful"][destDirectory]["FilesRemoved"])
            in six.integer_types
        )
        self.assertTrue(
            type(removeDirRes["Value"]["Successful"][destDirectory]["SizeRemoved"])
            in six.integer_types
        )
    ...
    def test_getDirectoryMetadata(self):
        ...
        self.assertEqual(
            metadataDirRes["Value"]["Successful"][destDirectory]["Type"], "Directory"
        )
        # Perform the checks for the remove directory operation
        self.assertTrue(removeDirRes["OK"])
        self.assertTrue(removeDirRes["Value"])
    ...
    def test_getDirectorySize(self):
        ...
        self.assertTrue(
            type(getDirSizeRes["Value"]["Successful"][destDirectory]["Files"])
            in six.integer_types
        )
        self.assertTrue(
            type(getDirSizeRes["Value"]["Successful"][destDirectory]["Size"])
            in six.integer_types
        )
        # Perform the checks for the remove directory operation
        self.assertTrue(removeDirRes["OK"])
    ...
    def test_getDirectory(self):
        ...
        for _dir in dirDict:
            if putDirRes["Value"]["Successful"][_dir]["Files"]:
                self.assertEqual(
                    putDirRes["Value"]["Successful"][_dir]["Files"], self.numberOfFiles
                )
                self.assertEqual(
                    putDirRes["Value"]["Successful"][_dir]["Size"],
                    self.numberOfFiles * sizeOfLocalFile,
                )
                self.assertTrue(
                    type(putDirRes["Value"]["Successful"][_dir]["Files"])
                    in six.integer_types
                )
                self.assertTrue(
                    type(putDirRes["Value"]["Successful"][_dir]["Size"])
                    in six.integer_types
                )
        ...
        for _dir in dirDict:
            if getDirRes["Value"]["Successful"][_dir]["Files"]:
                self.assertEqual(
                    getDirRes["Value"]["Successful"][_dir]["Files"], self.numberOfFiles
                )
                self.assertEqual(
                    getDirRes["Value"]["Successful"][_dir]["Size"],
                    self.numberOfFiles * sizeOfLocalFile,
                )
                self.assertTrue(
                    type(getDirRes["Value"]["Successful"][_dir]["Files"])
                    in six.integer_types
                )
                self.assertTrue(
                    type(getDirRes["Value"]["Successful"][_dir]["Size"])
                    in six.integer_types
                )
        # Perform the checks for the remove directory operation
        self.assertTrue(removeDirRes["OK"])
