...
class GFAL2_GSIFTPStorage(GFAL2_StorageBase):
    ...
    def getURLBase(self, withWSUrl=False):
        ...
        return self.__addDoubleSlash(
            super(GFAL2_GSIFTPStorage, self).getURLBase(withWSUrl=withWSUrl)
        )

    def constructURLFromLFN(self, lfn, withWSUrl=False):
        """Overwrite to add the double slash"""
        return self.__addDoubleSlash(
            super(GFAL2_GSIFTPStorage, self).constructURLFromLFN(
                lfn=lfn, withWSUrl=withWSUrl
            )
        )
    ...
    def getCurrentURL(self, fileName):
        """Overwrite to add the double slash"""
        return self.__addDoubleSlash(
            super(GFAL2_GSIFTPStorage, self).getCurrentURL(fileName)
        )
