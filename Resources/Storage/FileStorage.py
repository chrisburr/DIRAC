...
class FileStorage(StorageBase):
    ...
    def exists(self, path):
        ...
        urls = res["Value"]
        self.log.debug(
            "FileStorage.exists: Checking the existence of %s path(s)" % len(urls)
        )
    ...
    def isFile(self, path):
        ...
        self.log.debug(
            "FileStorage.isFile: Determining whether %s paths are files." % len(urls)
        )
        successful = {}
        failed = {}
        ...
        for dest_url, src_file in path.items():
            try:
                ...
                if sourceSize and (sourceSize != fileSize):
                    ...
                    failed[
                        dest_url
                    ] = "Source and destination file sizes do not match (%s vs %s)." % (
                        sourceSize,
                        fileSize,
                    )
                else:
    ...
    def removeFile(self, path):
        ...
        urls = res["Value"]
        gLogger.debug(
            "FileStorage.removeFile: Attempting to remove %s files." % len(urls)
        )
    ...
    def removeDirectory(self, path, recursive=False):
        ...
        self.log.debug(
            "FileStorage.removeDirectory: Attempting to remove %s directories."
            % len(urls)
        )
        ...
        for url in urls:
            if recursive:
                ...
                for root, _dirs, files in os.walk(url):
                    nbOfFiles += len(files)
                    totalSize += sum(
                        os.path.getsize(os.path.join(root, fn)) for fn in files
                    )
                try:
                    shutil.rmtree(url)
                    successful[url] = {
                        "FilesRemoved": nbOfFiles,
                        "SizeRemoved": totalSize,
                    }
                except OSError as ose:
    ...
    def listDirectory(self, path):
        ...
        self.log.debug(
            "FileStorage.listDirectory: Attempting to list %s directories." % len(urls)
        )
