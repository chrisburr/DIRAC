...
class S3Storage(StorageBase):
        ...
        for src_key in keys:
            try:
                ...
                dest_file = os.path.join(
                    localPath if localPath else os.getcwd(), fileName
                )
    ...
    def _presigned_putFile(self, urls, sourceSize=0):
        ...
        urlAdlers = {
            url: {"x-amz-meta-checksum": fileAdler(src_file)}
            for url, src_file in urls.items()
        }

        res = self.S3GatewayClient.createPresignedUrl(
            self.name, "put_object", urlAdlers
        )
        if not res["OK"]:
            return res
            ...
            try:
                ...
                with open(src_file, "rb") as src_fd:
                    ...
                    response = requests.post(
                        presignedURL, data=presignedFields, files=files
                    )
    ...
    def _presigned_getFileMetadata(self, urls):
        ...
        for url, presignedURL in presignedURLs.items():
            try:
                ...
                responseMetadata = {
                    headerKey.lower(): headerVal
                    for headerKey, headerVal in response.headers.items()
                }
    ...
    def getFileSize(self, urls):
        ...
        successful = {
            url: metadata["Size"]
            for url, metadata in res["Value"]["Successful"].items()
        }
    ...
    listDirectory = (
        ...
        removeDirectory
    ) = getDirectorySize = getDirectoryMetadata = putDirectory = notAvailable
    ...
    def getTransportURL(self, urls, protocols):
        ...
        return self.S3GatewayClient.createPresignedUrl(
            self.name, "get_object", urls, expiration=60 * 60 * 24
        )
            ...
            try:
                if s3_method != "put_object":
                    response = self.s3_client.generate_presigned_url(
                        ...
                        Params={"Bucket": self.bucketName, "Key": key},
