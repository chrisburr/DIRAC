...
class WLCGAccountingJson(object):
    ...
    def getOccupancy(self, **kwargs):
        ...
        if not os.path.isfile(filePath):
            return S_ERROR(
                "No WLCGAccountingJson file of %s is downloaded." % (self.name)
            )
        ...
        try:
            ...
            return S_ERROR(
                errno.ENOMSG,
                "Issue finding storage shares. %s in %s at %s."
                % (repr(e), occupancyLFN, self.name),
            )
        ...
        if spaceReservation:
            ...
            for storage in self.se.storages:
                ...
                for share in storageShares:
                    shareLen.append(
                        (share, len(os.path.commonprefix([share["path"][0], basePath])))
                    )
        ...
        try:
            ...
            return S_ERROR(
                errno.ENOMSG,
                "Issue finding Total or Free space left. %s in %s storageshares."
                % (repr(e), spaceReservation),
            )
