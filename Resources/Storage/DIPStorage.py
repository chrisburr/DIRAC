...
class DIPStorage(StorageBase):
    ...
    def setParameters(self, parameters):
        ...
        if "CheckSum" in parameters and parameters["CheckSum"].lower() in [
            "0",
            "no",
            "false",
            "off",
            ...
            if res["OK"]:
                if res["Value"]["Exists"]:
                    if res["Value"]["Type"] == "File":
                        gLogger.debug(
                            "DIPStorage.isFile: Successfully obtained metadata for %s."
                            % url
                        )
                        successful[url] = True
                    else:
                ...
                gLogger.error(
                    "DIPStorage.isFile: Failed to get metadata for url",
                    "%s: %s" % (url, res["Message"]),
                )
                failed[url] = res["Message"]
        resDict = {"Failed": failed, "Successful": successful}
    ...
    def getFileSize(self, path):
        ...
        failed = {}
        gLogger.debug(
            "DIPStorage.getFileSize: Attempting to obtain size for %s files."
            % len(urls)
        )
    ...
    def listDirectory(self, path):
        ...
        failed = {}
        gLogger.debug(
            "DIPStorage.listDirectory: Attempting to list %s directories." % len(urls)
        )
        serviceClient = RPCClient(self.url)
        for url in urls:
    ...
    def getDirectory(self, path, localPath=False):
        ...
        for src_dir in urls:
            ...
            if res["OK"]:
                ...
                gLogger.error(
                    "DIPStorage.getDirectory: Failed to get entire directory.", src_dir
                )
