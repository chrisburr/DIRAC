...
class RFIOStorage(StorageBase):
    ...
    def exists(self, path):
        ...
        urls = res["Value"]
        gLogger.debug(
            "RFIOStorage.exists: Determining the existance of %s files." % len(urls)
        )
        comm = "nsls -d"
        for url in urls:
            comm = " %s %s" % (comm, url)
    ...
    def isFile(self, path):
        ...
        urls = res["Value"]
        gLogger.debug(
            "RFIOStorage.isFile: Determining whether %s paths are files." % len(urls)
        )
        successful = {}
        failed = {}
        comm = "nsls -ld"
    ...
    def __getPathMetadata(self, urls):
        gLogger.debug(
            "RFIOStorage.__getPathMetadata: Attempting to get metadata for %s paths."
            % (len(urls))
        )
        comm = "nsls -ld"
        for url in urls:
        ...
        if not res["OK"]:
            errStr = (
                "RFIOStorage.__getPathMetadata: Completely failed to get path metadata."
            )
    ...
    def __getFileMetadata(self, urls):
        gLogger.debug(
            "RFIOStorage.__getPathMetadata: Attempting to get additional metadata for %s files."
            % (len(urls))
        )
        ...
        if not res["OK"]:
            errStr = (
                "RFIOStorage.__getFileMetadata: Completely failed to get cached status."
            )
            gLogger.error(errStr, res["Message"])
            return S_ERROR(errStr)
        else:
    ...
    def __getFile(self, src_url, dest_file):
        ...
        if os.path.exists(dest_file):
            gLogger.debug(
                "RFIOStorage.getFile: Local file already exists %s. Removing..."
                % dest_file
            )
            os.remove(dest_file)
        ...
        gLogger.debug(
            "RFIOStorage.getFile: Executing transfer of %s to %s" % (src_url, dest_file)
        )
        ...
        if res["OK"]:
            ...
            if returncode == 0:
                gLogger.debug(
                    "RFIOStorage.__getFile: Got file from storage, performing post transfer check."
                )
                localSize = getSize(dest_file)
                if localSize == remoteSize:
                    gLogger.debug(
                        "RFIOStorage.getFile: Post transfer check successful."
                    )
    ...
    def __putFile(self, src_file, dest_url, sourceSize):
        ...
        if res["Value"]:
            ...
            if not res["OK"]:
                gLogger.debug(
                    "RFIOStorage.__putFile: Failed to remove remote file %s." % dest_url
                )
            else:
                gLogger.debug(
                    "RFIOStorage.__putFile: Removed remote file %s." % dest_url
                )
        ...
        gLogger.debug(
            "RFIOStorage.putFile: Executing transfer of %s to %s" % (src_file, turl)
        )
        ...
        if res["OK"]:
            gLogger.debug(
                "RFIOStorage.__putFile: Removed remote file remnant %s." % dest_url
            )
        else:
            gLogger.debug(
    ...
    def removeFile(self, path):
        ...
        for urls in listOfLists:
            gLogger.debug(
                "RFIOStorage.removeFile: Attempting to remove %s files." % len(urls)
            )
    ...
    def getFileMetadata(self, path):
        ...
        urls = res["Value"]
        gLogger.debug(
            "RFIOStorage.getFileMetadata: Obtaining metadata for %s files." % len(urls)
        )
    ...
    def getFileSize(self, path):
        ...
        urls = res["Value"]
        gLogger.debug(
            "RFIOStorage.getFileSize: Determining the sizes for  %s files." % len(urls)
        )
        res = self.__getPathMetadata(urls)
        if not res["OK"]:
            return res
    ...
    def prestageFile(self, path):
        ...
        if res["OK"]:
            ...
            errStr = (
                "RFIOStorage.prestageFile: Completely failed to issue stage requests."
            )
            gLogger.error(errStr, res["Message"])
            return S_ERROR(errStr)
        resDict = {"Failed": failed, "Successful": successful}
    ...
    def isDirectory(self, path):
        ...
        urls = res["Value"]
        gLogger.debug(
            "RFIOStorage.isDirectory: Determining whether %s paths are directories."
            % len(urls)
        )
        res = self.__getPathMetadata(urls)
        if not res["OK"]:
    ...
    def getDirectory(self, path, localPath=False):
        ...
        failed = {}
        gLogger.debug(
            "RFIOStorage.getDirectory: Attempting to get local copies of %s directories."
            % len(urls)
        )
        for src_directory in urls:
            ...
            if res["OK"]:
                if res["Value"]["AllGot"]:
                    gLogger.debug(
                        "RFIOStorage.getDirectory: Successfully got local copy of %s"
                        % src_directory
                    )
                    successful[src_directory] = {
                        "Files": res["Value"]["Files"],
                        "Size": res["Value"]["Size"],
                    }
                else:
                    gLogger.error(
                        "RFIOStorage.getDirectory: Failed to get entire directory.",
                        src_directory,
                    )
                    failed[src_directory] = {
                        "Files": res["Value"]["Files"],
                        "Size": res["Value"]["Size"],
                    }
            else:
                gLogger.error(
                    "RFIOStorage.getDirectory: Completely failed to get local copy of directory.",
                    src_directory,
                )
    ...
    def __getDir(self, srcDirectory, destDirectory):
        ...
        if not res["OK"]:
            errStr = (
                "RFIOStorage.__getDir: Failed to find the supplied source directory."
            )
        ...
        if not res["Value"]["Successful"][srcDirectory]:
            errStr = (
                "RFIOStorage.__getDir: The supplied source directory does not exist."
            )
            gLogger.error(errStr, srcDirectory)
            return S_ERROR(errStr)
    ...
    def putDirectory(self, path):
        ...
        failed = {}
        gLogger.debug(
            "RFIOStorage.putDirectory: Attemping to put %s directories to remote storage."
            % len(urls)
        )
        for destDir, sourceDir in urls.items():
            ...
            if res["OK"]:
                if res["Value"]["AllPut"]:
                    gLogger.debug(
                        "RFIOStorage.putDirectory: Successfully put directory to remote storage: %s"
                        % destDir
                    )
                    successful[destDir] = {
                        "Files": res["Value"]["Files"],
                    ...
                    gLogger.error(
                        "RFIOStorage.putDirectory: Failed to put entire directory to remote storage.",
                        destDir,
                    )
                    failed[destDir] = {
                        "Files": res["Value"]["Files"],
                ...
                gLogger.error(
                    "RFIOStorage.putDirectory: Completely failed to put directory to remote storage.",
                    destDir,
                )
    ...
    def __putDir(self, src_directory, dest_directory):
        ...
        if not os.path.isdir(src_directory):
            errStr = (
                "RFIOStorage.__putDir: The supplied source directory does not exist."
            )
    ...
    def createDirectory(self, path):
        ...
        failed = {}
        gLogger.debug(
            "RFIOStorage.createDirectory: Attempting to create %s directories."
            % len(urls)
        )
        for url in urls:
            ...
            if res["OK"]:
                gLogger.debug(
                    "RFIOStorage.createDirectory: Successfully created directory on storage: %s"
                    % url
                )
                successful[url] = True
            else:
                gLogger.error(
                    "RFIOStorage.createDirectory: Failed to create directory on storage.",
                    url,
                )
    ...
    def listDirectory(self, path):
        ...
        urls = res["Value"]
        gLogger.debug(
            "RFIOStorage.listDirectory: Attempting to list %s directories." % len(urls)
        )
        ...
        for directory in directories:
            ...
            if res["OK"]:
                ...
                errStr = (
                    "RFIOStorage.listDirectory: Completely failed to list directory."
                )
    ...
    def getDirectoryMetadata(self, path):
        ...
        urls = res["Value"]
        gLogger.debug(
            "RFIOStorage.getDirectoryMetadata: Attempting to get metadata for %s directories."
            % len(urls)
        )
        res = self.isDirectory(urls)
        if not res["OK"]:
    ...
    def getDirectorySize(self, path):
        ...
        urls = res["Value"]
        gLogger.debug(
            "RFIOStorage.getDirectorySize: Attempting to get size of %s directories."
            % len(urls)
        )
        ...
        for directory, dirDict in res["Value"]["Successful"].items():
            ...
            for _fileURL, fileDict in filesDict.items():
                ...
                directoryFiles += 1
            gLogger.debug(
                "RFIOStorage.getDirectorySize: Successfully obtained size of %s."
                % directory
            )
    ...
    def __checkArgumentFormat(self, path):
        ...
        if isinstance(path, six.string_types):
            ...
            return S_ERROR(
                "RFIOStorage.__checkArgumentFormat: Supplied path is not of the correct format."
            )
        return S_OK(urls)
    ...
    def __executeOperation(self, url, method):
        ...
        if not fcn:
            return S_ERROR(
                "Unable to invoke %s, it isn't a member funtion of RFIOStorage" % method
            )
        res = fcn(url)
        if not res["OK"]:
            return res
