...
class GFAL2_SRM2Storage(GFAL2_StorageBase):
    ...
    def __init__(self, storageName, parameters):
        ...
        self.gfal2requestLifetime = gConfig.getValue(
            "/Resources/StorageElements/RequestLifeTime", 100
        )
        ...
        if "ProtocolsList" in parameters:
            ...
            self.log.debug(
                "GFAL2_SRM2Storage: No protocols provided, using the default protocols."
            )
    ...
    def __setSRMOptionsToDefault(self):
        ...
        self.ctx.set_opt_integer(
            "SRM PLUGIN", "REQUEST_LIFETIME", self.gfal2requestLifetime
        )
    ...
    def _updateMetadataDict(self, metadataDict, attributeDict):
        ...
        metadataDict["Accessible"] = (
            not metadataDict["Lost"]
            and metadataDict["Cached"]
            and not metadataDict["Unavailable"]
        )
    ...
    def getOccupancy(self, *parms, **kws):
        ...
        if not self.spaceToken:
            self.log.info(
                "getOccupancy: SpaceToken not defined for this SE. Falling back to the default getOccupancy."
            )
        ...
        spaceTokenAttr = (
            "spacetoken.description?%s" % self.protocolParameters["SpaceToken"]
        )
        ...
        try:
            ...
            return S_ERROR(
                getattr(e, "code", errno.EINVAL), "%s %s" % (errStr, repr(e))
            )
