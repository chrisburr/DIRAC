...
class StorageFactory(object):
    ...
    def getStorage(self, parameterDict, hideExceptions=False):
        ...
        return self.__generateStorageObject(
            storageName, pluginName, parameterDict, hideExceptions=hideExceptions
        )
    ...
    def _getConfigStorageName(
        self, storageName, referenceType, seConfigPath=SE_CONFIG_PATH
    ):
        """
        ...
        if not res["OK"]:
            errStr = (
                "StorageFactory._getConfigStorageName: Failed to get storage options"
            )
            gLogger.error(errStr, res["Message"])
            return S_ERROR(errStr)
        if not res["Value"]:
            errStr = (
                "StorageFactory._getConfigStorageName: Supplied storage doesn't exist."
            )
    ...
    def _getConfigStorageOptions(
        self, storageName, derivedStorageName=None, seConfigPath=SE_CONFIG_PATH
    ):
        """
        ...
        for seName in (
            (storageName, derivedStorageName) if derivedStorageName else (storageName,)
        ):
            storageConfigPath = cfgPath(seConfigPath, seName)
            res = gConfig.getOptions(storageConfigPath)
            if not res["OK"]:
                errStr = (
                    "StorageFactory._getStorageOptions: Failed to get storage options."
                )
            ...
            for option in set(res["Value"]) - set(
                ...
                default = (
                    [] if option in ["VO", "AccessProtocols", "WriteProtocols"] else ""
                )
    ...
    def _getConfigStorageProtocols(
        self, storageName, derivedStorageName=None, seConfigPath=SE_CONFIG_PATH
    ):
        """
        ...
        if derivedStorageName:
            ...
            res = self.__getProtocolsSections(
                derivedStorageName, seConfigPath=SE_CONFIG_PATH
            )
            if not res["OK"]:
                return res
    ...
    def _getConfigStorageProtocolDetails(
        ...
        if not res["OK"]:
            errStr = (
                "StorageFactory.__getProtocolDetails: Failed to get protocol options."
            )
    ...
    def __generateStorageObject(
        ...
        result = objectLoader.loadObject(
            "Resources.Storage.%sStorage" % storageType,
            storageType + "Storage",
        ...
        try:
            ...
            errStr = (
                "StorageFactory._generateStorageObject: Failed to instantiate %s: %s"
                % (storageName, x)
            )
