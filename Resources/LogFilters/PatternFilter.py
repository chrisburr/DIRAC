...
class PatternFilter(object):
    ...
    def __init__(self, optionDict):
        ...
        self._accept = [
            w.strip() for w in optionDict.get("Accept", "").split(",") if w.strip()
        ]
        self._reject = [
            w.strip() for w in optionDict.get("Reject", "").split(",") if w.strip()
        ]
    ...
    def __filter(self, record):
        ...
        accepted = not self._accept or any(
            w in msg for w in self._accept for msg in msgPs
        )
