    ...
    def test_getProxy(self):
        ...
        for proxyProvider, log in [
            ...
            for name, email, res in [
                ...
                gLogger.info(
                    "\nFullName: %s" % name or "absent",
                    "Email: %s.." % email or "absent",
                )
                # Create user DN
            ...
            for dn, name, res in [
                (
                    "/O=DIRAC/OU=DIRAC CA/CN=user_3/emailAddress=some@mail.org",
                    "user_3",
                    True,
                ),
                (
                    "/O=Dirac/OU=DIRAC CA/CN=user/emailAddress=some@mail.org",
                    "user",
                    True,
                ),
                (
                    "/O=Dirac/OU=Without supplied field/emailAddress=some@mail.org",
                    "not_suplied",
                    False,
                ),
