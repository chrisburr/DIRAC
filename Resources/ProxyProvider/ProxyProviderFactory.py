...
class ProxyProviderFactory(object):
    ...
    def getProxyProvider(self, proxyProvider):
        ...
        result = getInfoAboutProviders(
            of="Proxy", providerName=proxyProvider, option="all", section="all"
        )
        if not result["OK"]:
            return result
        ppDict = result["Value"]
        ...
        if not ppType:
            return S_ERROR(
                "Cannot find information for ProxyProvider %s" % proxyProvider
            )
        self.log.verbose(
            "Creating ProxyProvider of %s type with the name %s"
        ...
        if not result["OK"]:
            self.log.error(
                "Failed to load object", "%s: %s" % (subClassName, result["Message"])
            )
            return result
