...
class DIRACCAProxyProvider(ProxyProvider):
    def __init__(self, parameters=None):
        ...
        self.fields2nid[
            "domainComponent"
        ] = -1  # Add DN description that is not liested in X509.X509_Name
    ...
    def setParameters(self, parameters):
        ...
        for k, v in parameters.items():
            if (
                ...
                and k
                in ["Match", "Supplied", "Optional", "DNOrder"] + self.fields2nid.keys()
            ):
        ...
        if "DNOrder" in parameters:
            ...
            if not any(
                [
                    any([f in parameters["DNOrder"] for f in self.nid2fields[n]])
                    for n in allFields
                ]
            ):
    ...
    def checkStatus(self, userDN):
        ...
        try:
            userNIDs = [
                self.fields2nid[f.split("=")[0]] for f in userDN.lstrip("/").split("/")
            ]
        ...
        for field, values in dnInfoDict.items():
            ...
            if nid in self.match and not self.dnInfoDictCA[field] == values:
                return S_ERROR(
                    "%s must be /%s=%s."
                    % (err, field, ("/%s=" % field).joing(self.dnInfoDictCA[field]))
                )
            if nid in self.maxDict:
                rangeMax = range(min(len(values), len(self.maxDict[nid])))
                if any(
                    [
                        True if len(values[i]) > self.maxDict[nid][i] else False
                        for i in rangeMax
                    ]
                ):
                    return S_ERROR(
                        "%s values must be less then %s."
                        % (err, ", ".join(self.maxDict[nid]))
                    )
            if nid in self.minDict:
                rangeMin = range(min(len(values), len(self.minDict[nid])))
                if any(
                    [
                        True if len(values[i]) < self.minDict[nid][i] else False
                        for i in rangeMin
                    ]
                ):
                    return S_ERROR(
                        "%s values must be more then %s."
                        % (err, ", ".join(self.minDict[nid]))
                    )
    ...
    def generateDN(self, **kwargs):
        ...
        for field in self.dnList:
            ...
            if not values and nid in self.supplied:
                ...
                if nid not in self.nid2defField:
                    return S_ERROR(
                        'No values set for "%s" DN' % min(self.nid2fields[nid], key=len)
                    )
        ...
        resDN = m2.x509_name_oneline(
            self.__X509Name.x509_name
    ...
    def __parseCACFG(self):
        ...
        with open(self.parameters["CAConfigFile"], "r") as caCFG:
            for line in caCFG:
                ...
                if re.findall(r"\[([A-Za-z0-9_]+)\]", line.replace(" ", "")):
                    block = "".join(
                        re.findall(r"\[([A-Za-z0-9_]+)\]", line.replace(" ", ""))
                    )
                ...
                if len(re.findall("=", line)) == 1:
                    ...
                    if "default_ca" in self.cfg.get("ca", {}):
                        if "policy" in self.cfg.get(self.cfg["ca"]["default_ca"], {}):
                            if (
                                block
                                == self.cfg[self.cfg["ca"]["default_ca"]]["policy"]
                            ):
                                self.dnList.append(field)
        ...
        self.algoritm = (
            self.cfg[self.cfg["ca"]["default_ca"]].get("default_md") or self.algoritm
        )
        if not self.parameters.get("CertFile"):
            self.parameters["CertFile"] = self.cfg[self.cfg["ca"]["default_ca"]][
                "certificate"
            ]
            self.parameters["KeyFile"] = self.cfg[self.cfg["ca"]["default_ca"]][
                "private_key"
            ]
        ...
        for k, v in self.cfg[self.cfg[self.cfg["ca"]["default_ca"]]["policy"]].items():
            ...
            for k in ["%s.%s" % (i, k) for i in range(0, 5)] + [k]:
                if k + "_default" in self.cfg["req"]["distinguished_name"]:
                    self.parameters[nid].append(
                        self.cfg["req"]["distinguished_name"][k + "_default"]
                    )
                if k + "_min" in self.cfg["req"]["distinguished_name"]:
                    self.minDict[nid].append(
                        self.cfg["req"]["distinguished_name"][k + "_min"]
                    )
                if k + "_max" in self.cfg["req"]["distinguished_name"]:
                    self.maxDict[nid].append(
                        self.cfg["req"]["distinguished_name"][k + "_max"]
                    )
            if v == "supplied":
        ...
        for value in values:
            if (
                ...
                and m2.x509_name_set_by_nid(
                    self.__X509Name.x509_name,  # pylint: disable=no-member
                    self.fields2nid[field],
                    value,
                )
                == 0
            ):
    ...
    def __createCertM2Crypto(self):
        ...
        pkey = EVP.PKey()
        pkey.assign_rsa(
            RSA.load_key_string(caKeyStr, callback=util.no_passphrase_callback)
        )
        # Sign
        ...
        userPubKeyStr = userPubKey.as_pem(
            cipher=None, callback=util.no_passphrase_callback
        )
    ...
    def _forceGenerateProxyForDN(self, dn, time, group=None):
        ...
        if result["OK"]:
            ...
            chain = X509Chain()
            if (
                chain.loadChainFromString(certStr)["OK"]
                and chain.loadKeyFromString(keyStr)["OK"]
            ):
