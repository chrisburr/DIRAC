...
class ARCComputingElement(ComputingElement):
    ...
    def __getARCJob(self, jobID):
        ...
        j.JobID = str(jobID)
        statURL = (
            "ldap://%s:2135/Mds-Vo-Name=local,o=grid??sub?(nordugrid-job-globalid=%s)"
            % (self.ceHost, jobID)
        )
    ...
    def __getXRSLExtraString(self, multiprocessor=False):
        ...
        self.log.debug(
            "Trying to get %s : first option %s" % (xtraVariable, firstOption)
        )
        ...
        if result != "":
            ...
            self.log.debug(
                "Trying to get %s : second option %s" % (xtraVariable, secondOption)
            )
            ...
            if result != "":
                ...
                self.log.debug(
                    "Trying to get %s : default option %s"
    ...
    def __writeXRSL(self, executableFile, processors=1):
        ...
        nProcessors = (
            processors
            if processors > 1
            else self.ceParameters.get("NumberOfProcessors", processors)
        )
...
%(xrslExtraString)s
    ...
    def submitJob(self, executableFile, proxy, numberOfJobs=1, processors=1):
        ...
        if not result["OK"]:
            self.log.error(
                "ARCComputingElement: failed to set up proxy", result["Message"]
            )
            return result
        ...
        for __i in range(numberOfJobs):
            ...
            if result == arc.SubmissionStatus.NONE:
                ...
                self.log.debug(
                    "Successfully submitted job %s to CE %s"
                    % (pilotJobReference, self.ceHost)
                )
            else:
                message = "Failed to submit job because "
                if result.isSet(
                    arc.SubmissionStatus.NOT_IMPLEMENTED
                ...
                if result.isSet(
                    arc.SubmissionStatus.NO_SERVICES
                ):  # pylint: disable=no-member
                    self.log.warn(
                ...
                if result.isSet(
                    arc.SubmissionStatus.ENDPOINT_NOT_QUERIED
                ):  # pylint: disable=no-member
                    self.log.warn(
                        "%s endpoint was not even queried. (network ..?)" % message
                    )
                if result.isSet(
                ...
                if result.isSet(
                    arc.SubmissionStatus.AUTHENTICATION_ERROR
                ):  # pylint: disable=no-member
                    self.log.warn(
                ...
                if result.isSet(
                    arc.SubmissionStatus.ERROR_FROM_ENDPOINT
                ):  # pylint: disable=no-member
                    self.log.warn(
                        "%s some error from the CE - possibly CE problems?" % message
                    )
    ...
    def killJob(self, jobIDList):
        ...
        for chunk in breakListIntoChunks(jobs, 100):
            ...
            if not job_supervisor.Cancel():
                ...
                return S_ERROR(
                    "Failed to kill at least one of these jobs: %s. CE(?) not reachable?"
                    % errorString
                )
    ...
    def getCEStatus(self):
        ...
        if not vo:
            ...
            self.log.debug(
                "Running jobs for CE %s : %s" % (self.ceHost, ceStats.RunningJobs)
            )
            self.log.debug(
                "Waiting jobs for CE %s : %s" % (self.ceHost, ceStats.WaitingJobs)
            )
            ...
            if not self.queue:
                ...
                res = S_ERROR(
                    "Unknown queue (%s) failure for site %s" % (self.queue, self.ceHost)
                )
                return res
            cmd1 = (
                "ldapsearch -x -o ldif-wrap=no -LLL -h %s:2135  -b 'o=glue' "
                % self.ceHost
            )
            cmd2 = (
                '"(&(objectClass=GLUE2MappingPolicy)(GLUE2PolicyRule=vo:%s))"'
                % vo.lower()
            )
            cmd3 = " | grep GLUE2MappingPolicyShareForeignKey | grep %s" % (
                self.queue.split("-")[-1]
            )
            cmd4 = " | sed 's/GLUE2MappingPolicyShareForeignKey: /GLUE2ShareID=/' "
            cmd5 = (
                " | xargs -L1 ldapsearch -x -o ldif-wrap=no -LLL -h %s:2135 -b 'o=glue' "
                % self.ceHost
            )
            ...
            try:
                ...
                running = [
                    lValue
                    for lValue in ldapValues
                    if "GLUE2ComputingShareRunningJobs" in lValue
                ]
                waiting = [
                    lValue
                    for lValue in ldapValues
                    if "GLUE2ComputingShareWaitingJobs" in lValue
                ]
    ...
    def getJobStatus(self, jobIDList):
        ...
        for chunk in breakListIntoChunks(jobsToRenew, 100):
            ...
            if not job_supervisor_renew.Renew():
                self.log.warn(
                    "At least one of the jobs failed to renew its credentials"
                )
