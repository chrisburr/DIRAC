...
def findFile(workingDir, fileName, pathToResult=None):
    ...
    res = Subprocess().systemCall(
        "find %s -name '%s'" % (workingDir, fileName), shell=True
    )
    if not res["OK"]:
        return res
    ...
    if not paths:
        return S_ERROR(
            errno.ENOENT, "Could not find %s in directory %s" % (fileName, workingDir)
        )
    return S_OK(paths)
...
def getCondorLogFile(pilotRef):
    ...
    workingDirectory = gConfig.getValue(
...
class HTCondorCEComputingElement(ComputingElement):
    ...
    def __writeSub(self, executable, nJobs, location, processors):
        ...
        fd, name = tempfile.mkstemp(
            suffix=".sub", prefix="HTCondorCE_", dir=self.workingDirectory
        )
        ...
        periodicRemove += (
            "(time() - EnteredCurrentStatus) > (%s * 24 * 3600)"
            % self.daysToKeepRemoteLogs
        )
...
""" % dict(
    ...
    def _reset(self):
        ...
        self.daysToKeepLogs = self.ceParameters.get(
            "DaysToKeepLogs", DEFAULT_DAYSTOKEEPLOGS
        )
        self.daysToKeepRemoteLogs = self.ceParameters.get(
            "DaysToKeepRemoteLogs", DEFAULT_DAYSTOKEEPREMOTELOGS
        )
        self.extraSubmitString = self.ceParameters.get("ExtraSubmitString", "").decode(
            "string_escape"
        )
        self.useLocalSchedd = self.ceParameters.get(
            "UseLocalSchedd", self.useLocalSchedd
        )
        ...
        self.remoteScheddOptions = (
            ""
            if self.useLocalSchedd
            else "-pool %s:9619 -name %s " % (self.ceName, self.ceName)
        )
    ...
    def submitJob(self, executableFile, proxy, numberOfJobs=1, processors=1):
        ...
        scheddOptions = (
            []
            if self.useLocalSchedd
            else ["-pool", "%s:9619" % self.ceName, "-remote", self.ceName]
        )
        ...
        if result["Value"][0]:
            ...
            errorString = (
                result["Value"][2] if result["Value"][2] else result["Value"][1]
            )
            return S_ERROR(
                "Pilot submission failed with error: %s " % errorString.strip()
            )
    ...
    def getCEStatus(self):
        ...
        if res["OK"]:
            ...
            self.log.warn(
                "Failure getting pilot count for %s: %s "
    ...
    def __getPilotReferences(self, jobString):
        ...
        if len(clusterIDs) != 2:
            return S_ERROR(
                "Something wrong with the condor_submit output: %s" % jobString
            )
        ...
        jobReferences = [
            "%s%s.%s" % (cePrefix, clusterID, i) for i in range(int(numJobs) + 1)
        ]
    ...
    def __cleanup(self):
        ...
        status, stdout = commands.getstatusoutput(
            'find -O3 %s -maxdepth 1 -mmin +120 -name "DIRAC_*" -delete '
