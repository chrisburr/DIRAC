...
class glexecComputingElement(ComputingElement):
    ...
    def submitJob(self, executableFile, proxy, dummy=None):
        ...
        if "X509_USER_PROXY" not in os.environ:
            self.log.error(
                "X509_USER_PROXY variable for pilot proxy not found in local environment"
            )
        ...
        if not result["OK"]:
            res = self.analyseExitCode(
                result["Value"]
            )  # take no action as we currently default to InProcess
            glexecLocation = None
            if (
                "RescheduleOnError" in self.ceParameters
                and self.ceParameters["RescheduleOnError"]
            ):
        ...
        if not glexecLocation:
            self.log.info(
                "glexec is not found, setting X509_USER_PROXY for payload proxy"
            )
        ...
        try:
            ...
            self.log.error(
                "Failed to change permissions of executable to 0755 with exception",
                "\n%s" % (x),
            )

        result = self.glexecExecute(os.path.abspath(executableFile), glexecLocation)
        if not result["OK"]:
            self.analyseExitCode(
                result["Value"]
            )  # take no action as we currently default to InProcess
    ...
    def recursivelyChangePermissions(self):
        ...
        try:
            self.log.info(
                "Trying to explicitly change permissions for parent directory %s"
                % currentDir
            )
            os.chmod(
                currentDir,
            ...
            self.log.error(
                "Problem changing directory permissions in parent directory", str(x)
            )
        ...
        if res["OK"] and res["Value"][0] == 0:
            self.log.info(
                "Contents of the working directory before permissions change:"
            )
            self.log.info(str(res["Value"][1]))
        else:
            self.log.error(
                "Failed to list the log directory contents", str(res["Value"][2])
            )
        ...
        if res["OK"] and res["Value"][0] == 0:
            ...
            self.log.error(
                "Failed to list the parent directory contents", str(res["Value"][2])
            )

        self.log.verbose(
            "Changing permissions to 0755 in current directory %s" % currentDir
        )
        for dirName, _, files in os.walk(currentDir):
            try:
                self.log.info(
                    "Changing file and directory permissions to 0755 for %s" % dirName
                )
        ...
        self.log.info(
            "Permissions in current directory %s updated successfully" % (currentDir)
        )
        res = shellCall(10, "ls -al")
        if res["OK"] and res["Value"][0] == 0:
            self.log.info(
                "Contents of the working directory after changing permissions:"
            )
            self.log.info(str(res["Value"][1]))
        else:
            self.log.error(
    ...
    def glexecTest(self, glexecLocation):
        ...
        try:
            ...
            self.log.error(
                "Failed to change permissions of test script to 0755 with exception",
                "\n%s" % (x),
            )
    ...
    def glexecLocate(self):
        ...
        if "OSG_GLEXEC_LOCATION" in os.environ:
            ...
            glexecPath = distutils.spawn.find_executable(
                "glexec"
