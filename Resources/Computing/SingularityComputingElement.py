...
class SingularityComputingElement(ComputingElement):
    ...
    def __init__(self, ceUniqueID):
        ...
        self.__installDIRACInContainer = self.ceParameters.get(
            "InstallDIRACInContainer", True
        )
        if isinstance(
            self.__installDIRACInContainer, six.string_types
        ) and self.__installDIRACInContainer.lower() in ("false", "no"):
    ...
    def __getInstallFlags(self, infoDict=None):
        ...
        pilotExtensionsList = str(
            infoDict["Setups"][setup].get("CommandExtensions")
        ).split(",")
        if not pilotExtensionsList:
            pilotExtensionsList = str(
                infoDict["Setups"]["Defaults"].get("CommandExtensions")
            ).split(",")
        if not pilotExtensionsList:
    ...
    def __createWorkArea(self, jobDesc=None, log=None, logLevel="INFO", proxy=None):
        ...
        try:
            ...
            if not os.path.isdir(self.__workdir):
                result = S_ERROR(
                    "Failed to create container base directory '%s'" % self.__workdir
                )
        ...
        try:
            baseDir = tempfile.mkdtemp(
                prefix="job%s_" % jobDesc.get("jobID", 0), dir=self.__workdir
            )
        except OSError:
            result = S_ERROR(
                "Failed to create container work directory in '%s'" % self.__workdir
            )
            result["ReschedulePayload"] = True
            return result
    ...
    def submitJob(self, executableFile, proxy=None, **kwargs):
        ...
        result = systemCall(
            0, cmd, callbackFunction=self.sendOutput, env=self.__getEnv()
        )
