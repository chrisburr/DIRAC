...
class ComputingElement(object):
    ...
    def __init__(self, ceName):
        ...
        self.minProxyTime = gConfig.getValue(
            "/Registry/MinProxyLifeTime", 10800
        )  # secs
        self.defaultProxyTime = gConfig.getValue(
            "/Registry/DefaultProxyLifeTime", 43200
        )  # secs
        self.proxyCheckPeriod = gConfig.getValue(
            "/Registry/ProxyCheckingPeriod", 3600
        )  # secs
    ...
    def _prepareProxy(self):
        ...
        if not self.proxy:
            ...
            result = gProxyManager.dumpProxyToFile(
                self.proxy, requiredTimeLeft=self.minProxyTime
            )
            if not result["OK"]:
                return result
        ...
        self.log.debug(
            "Set proxy variable X509_USER_PROXY to %s" % os.environ["X509_USER_PROXY"]
        )
        return S_OK()
    ...
    def _addCEConfigDefaults(self):
        ...
        self.ceParameters["MaxWaitingJobs"] = int(
            self.ceParameters.get("MaxWaitingJobs", MAX_WAITING_JOBS)
        )
        self.ceParameters["MaxTotalJobs"] = int(
            self.ceParameters.get("MaxTotalJobs", MAX_TOTAL_JOBS)
        )
    ...
    def loadBatchSystem(self):
        ...
        result = objectLoader.loadObject(
            "Resources.Computing.BatchSystems.%s" % self.batchSystem, self.batchSystem
        )
        if not result["OK"]:
    ...
    def available(self, jobIDList=None):
        ...
        maxWaitingJobs = int(
            max(
                int(self.ceParameters.get("MaxWaitingJobs", 0)),
        ...
        if totalJobs >= maxTotalJobs:
            ...
            result["Value"] = 0
            message = (
                "There are %s waiting jobs and total jobs %s >= %s max total jobs"
    ...
    def _monitorProxy(self, payloadProxy=None):
        ...
        pilotDN = (
            pilotProxyDict["chain"].getIssuerCert()["Value"].getSubjectDN()["Value"]
        )
        ...
        payloadDN = (
            payloadProxyDict["chain"].getIssuerCert()["Value"].getSubjectDN()["Value"]
        )
        ...
        retVal = VOMS().setVOMSAttributes(
            pilotProxyDict["chain"], attribute=attribute, vo=vo
        )
        if not retVal["OK"]:
            return retVal
    ...
    def getDescription(self):
        ...
        result = self.getCEStatus()
        if result["OK"]:
            ceDict["NumberOfProcessors"] = result.get(
                "AvailableProcessors", result.get("NumberOfProcessors", 1)
            )
        else:
            self.log.error(
                "Failure getting CE status",
    ...
    #############################################################################
    def submitJob(
        self, executableFile, proxy, dummy=None, processors=1
    ):  # pylint: disable=unused-argument
        """Method to submit job, should be overridden in sub-class."""
        name = "submitJob()"
        self.log.error("ComputingElement should be implemented in a subclass", name)
        return S_ERROR(
            "ComputingElement: %s should be implemented in a subclass" % (name)
        )
