...
from DIRAC.Resources.Computing.InProcessComputingElement import (
    InProcessComputingElement,
)
from DIRAC.Resources.Computing.SudoComputingElement import SudoComputingElement
from DIRAC.Resources.Computing.SingularityComputingElement import (
...
class PoolComputingElement(ComputingElement):
    ...
    def _reset(self):
        ...
        self.innerCESubmissionType = self.ceParameters.get(
    ...
    def submitJob(self, executableFile, proxy=None, **kwargs):
        ...
        if self.pPool is None:
            self.pPool = ProcessPool(
                minSize=self.processors,
    ...
    def _getProcessorsForJobs(self, kwargs):
        ...
        if "maxNumberOfProcessors" in kwargs and kwargs["maxNumberOfProcessors"]:
            requestedProcessors = min(
                int(kwargs["maxNumberOfProcessors"]), availableProcessors
            )
    ...
    def finalizeJob(self, taskID, result):
        ...
        if result["OK"]:
            self.log.info(
                "Task %d finished successfully, %d processor(s) freed" % (taskID, nProc)
            )
        else:
            self.log.error(
                "Task failed submission",
