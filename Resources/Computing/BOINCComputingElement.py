...
""" % {
            ...
            fd, name = tempfile.mkstemp(
                suffix="_pilotwrapper.py", prefix="DIRAC_", dir=os.getcwd()
            )
            os.close(fd)
            submitFile = name
        ...
        prefix = (
            os.path.splitext(os.path.basename(submitFile))[0]
            .replace("_pilotwrapper", "")
            .replace("DIRAC_", "")
        )
        batchIDList = []
        stampDict = {}
        for i in range(0, numberOfJobs):
            ...
            try:
                ...
                self.log.error(
                    "Could not submit the pilot to the BOINC CE",
            ...
            self.log.verbose(
                "Submit the pilot %s to the BOINC CE %s" % (jobID, self.wsdl)
            )
    ...
    def getCEStatus(self):
        ...
        try:
            ...
            self.log.error(
                "Could not get the BOINC CE dynamic jobs information", self.wsdl
            )
            return S_ERROR(
                "Could not get the BOINC CE %s dynamic jobs information, communication failed!"
                % self.wsdl
            )

        if not result["ok"]:
            self.log.warn(
                "Did not get the BOINC CE %s dynamic jobs information, the value returned is false!"
                % self.wsdl
            )
            return S_ERROR(
                "Did not get the BOINC CE %s dynamic jobs information, the value returned is false!"
    ...
    def getJobStatus(self, jobIDList):
        ...
        try:
            ...
            self.log.error(
                "Could not get the status about jobs in the list from the BOINC CE",
                self.wsdl,
            )
            return S_ERROR(
        ...
        self.log.debug(
            "Got the status about jobs in list from the BOINC CE %s." % self.wsdl
        )
        resultRe = {}
    ...
    def getJobOutput(self, jobID, localDir=None):
        ...
        try:
            ...
            self.log.error(
                "Could not get the outputs of job from the BOINC CE",
        ...
        self.log.debug(
            "Got the outputs of job %s from the BOINC CE %s." % (jobID, self.wsdl)
        )
...
if __name__ == "__main__":
    ...
    if test_parameter & test_submit:
        fd, fname = tempfile.mkstemp(
            suffix="_pilotwrapper.py", prefix="DIRAC_", dir="/home/client/dirac/data/"
        )
        os.close(fd)
        fd = open(fname, "w")
        fd.write(
    ...
    if test_parameter & test_getStatus:
        jobTestList = [
            "Uu0ghO_0@mardirac3.in2p3.fr",
            "1aDmIf_0@mardirac3.in2p3.fr",
            jobID,
        ]
    ...
    if test_parameter & test_getOutput:
        ...
        if not outstate["OK"]:
            ...
            print(
                "Please check the directory /tmp for the output and error files of job %s"
                % jobID
            )
