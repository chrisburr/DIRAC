...
def parseCondorStatus(lines, jobID):
    ...
    for line in lines:
        ...
        try:
            ...
            continue
        if l[0] == jobID:
            return {1: "Waiting", 2: "Running", 3: "Aborted", 4: "Done", 5: "HELD"}.get(
                status, "Unknown"
            )
    return "Unknown"
...
class Condor(object):
    ...
    def getJobStatus(self, **kwargs):
        ...
        status, stdout_q = commands.getstatusoutput(
            "condor_q -submitter %s -af:j JobStatus  " % user
        )
        ...
        condorHistCall = (
            "condor_history -af ClusterId ProcId JobStatus -submitter %s" % user
        )
