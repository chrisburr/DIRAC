...
from DIRAC.Resources.Computing.BatchSystems.TimeLeft.LSFResourceUsage import (
    LSFResourceUsage,
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            (S_OK(LSF_KEK_BQUEUES), S_OK(LSF_LSHOSTS)),
            "b66",
            (720 * 60 / 2.5),
            (1440 * 60 / 2.5),
        )
...
def test_init(
    mocker, runCommandResult, lsbHosts, cpuLimitExpected, wallClockLimitExpected
):
    mocker.patch(
...
@pytest.mark.parametrize(
    ...
    [
        (
            (
                S_OK(LSF_CERN_BQUEUES),
                S_ERROR(LSF_CERN_LSHOSTS_1),
                S_OK(LSF_CERN_LSINFO),
                S_OK(LSF_LSHOSTS),
            ),
            S_ERROR("no lsf.sh"),
            "b66",
...
def test_init_cern(
    ...
    mocker.patch(
        "DIRAC.Resources.Computing.BatchSystems.TimeLeft.LSFResourceUsage.sourceEnv",
