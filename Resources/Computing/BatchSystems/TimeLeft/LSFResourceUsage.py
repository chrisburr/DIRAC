...
class LSFResourceUsage(ResourceUsage):
    ...
    def __init__(self):
        ...
        self.log.debug(
            "From %s" % cmd,
            "\n".join(
                [line if len(line) <= 128 else line[:128] + " [...]" for line in lines]
            ),
        )
        for i in xrange(len(lines)):
            if re.search(".*CPULIMIT.*", lines[i]):
                ...
                if len(info) >= 1:
                    ...
                    self.log.warn(
                        'Problem parsing "%s" for wall clock limit' % lines[i + 1]
                    )
        ...
        if self.cpuRef:
            ...
            if not self.normRef:
                ...
                if not os.path.isfile("./lsf.sh"):
                    os.symlink(
                        os.path.join(os.environ["LSF_ENVDIR"], "lsf.conf"), "./lsf.sh"
                    )
                                ...
                                if line and line.split()[0] == self.cpuRef:
                                    try:
                                        ...
                                        self.log.info(
                                            ...
                                            "(%s) %s: %s"
                                            % (shared, self.cpuRef, self.normRef),
                                        )
                                    except ValueError as e:
                                        self.log.exception(
                                            "Exception reading LSF configuration", "", e
                                        )
                    else:
