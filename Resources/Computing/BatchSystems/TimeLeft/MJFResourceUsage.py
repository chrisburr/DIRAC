...
class MJFResourceUsage(ResourceUsage):
    ...
    def getResourceUsage(self):
        ...
        if jobFeaturesPath:
            try:
                wallClockLimit = int(
                    urlopen(jobFeaturesPath + "/wall_limit_secs").read()
                )
                ...
                self.log.exception(
                    "Issue with $JOBFEATURES/wall_limit_secs", lException=e
                )
                self.log.warn(
                    "Could not determine cpu limit from $JOBFEATURES/wall_limit_secs"
                )
            ...
            try:
                ...
                self.log.exception(
                    "Issue with $JOBFEATURES/jobstart_secs", lException=e
                )
            ...
            try:
                ...
                self.log.exception(
                    "Issue with $JOBFEATURES/cpu_limit_secs", lException=e
                )
                self.log.warn(
                    "Could not determine cpu limit from $JOBFEATURES/cpu_limit_secs"
                )
        ...
        if machineFeaturesPath and jobStartSecs:
            try:
                shutdownTime = int(
                    urlopen(machineFeaturesPath + "/shutdowntime").read()
                )
                ...
                self.log.warn(
                    "Could not determine a shutdowntime value from $MACHINEFEATURES/shutdowntime"
                )
