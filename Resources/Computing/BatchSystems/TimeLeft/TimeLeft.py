...
class TimeLeft(object):
    ...
    def __init__(self):
        ...
        if not self.scaleFactor:
            self.log.warn(
                "/LocalSite/CPUScalingFactor not defined for site %s" % DIRAC.siteName()
            )
    ...
    def getScaledCPU(self, processors=1):
        ...
        if "Value" in resourceDict:
            if resourceDict["Value"].get("CPU"):
                ...
                return (
                    resourceDict["Value"]["WallClock"] * self.scaleFactor * processors
                )
    ...
    def getTimeLeft(self, cpuConsumed=0.0, processors=1):
        ...
        if time and cpuConsumed > 3600.0 and self.normFactor:
    ...
    def __getBatchSystemPlugin(self):
        ...
        if (
            name is None
            and "MACHINEFEATURES" in os.environ
            and "JOBFEATURES" in os.environ
        ):
        ...
        try:
            ...
            msg = (
                "Could not import DIRAC.Resources.Computing.BatchSystems.TimeLeft.%s"
                % (batchSystemName)
            )
            self.log.warn(x)
            self.log.warn(msg)
