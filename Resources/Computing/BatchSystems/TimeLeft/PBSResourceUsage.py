        ...
        for line in lines:
            ...
            if re.search(".*resources_used.cput.*", line):
                if len(info) >= 3:
                    ...
                    newcpu = (float(cpuList[0]) * 60 + float(cpuList[1])) * 60 + float(
                        cpuList[2]
                    )
            ...
            if re.search(".*resources_used.walltime.*", line):
                if len(info) >= 3:
                    wcList = info[2].split(":")
                    wallClock = (float(wcList[0]) * 60 + float(wcList[1])) * 60 + float(
                        wcList[2]
                    )
                else:
                    self.log.warn(
                        'Problem parsing "%s" for elapsed wall clock time' % line
                    )
            if re.search(".*Resource_List.cput.*", line):
                if len(info) >= 3:
                    cpuList = info[2].split(":")
                    newcpuLimit = (
                        float(cpuList[0]) * 60 + float(cpuList[1])
                    ) * 60 + float(cpuList[2])
            ...
            if re.search(".*Resource_List.walltime.*", line):
                if len(info) >= 3:
                    wcList = info[2].split(":")
                    wallClockLimit = (
                        float(wcList[0]) * 60 + float(wcList[1])
                    ) * 60 + float(wcList[2])
                else:
