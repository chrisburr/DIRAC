...
class CREAMComputingElement(ComputingElement):
    ...
    def __writeJDL(self, executableFile, processors=1):
        ...
        fd, name = tempfile.mkstemp(
            suffix=".jdl", prefix="CREAM_", dir=workingDirectory
        )
        ...
        extraJDLParameterList = ";\n  ".join(
            [item.strip() for item in extraJDLParameters]
        )
...
[
  ...
  %(extraJDLParameters)s
        ...
        if numberOfJobs == 1:
            ...
            if result["OK"]:
                if result["Value"][0]:
                    ...
                    return S_ERROR(
                        "Pilot submission failed with error: %s " % errorString
                    )
            ...
            for _i in range(numberOfJobs):
                jdlName, diracStamp = self.__writeJDL(
                    executableFile, processors=processors
                )
                cmd = [
                    "glite-ce-job-submit",
                    "-n",
                ...
                if result["Value"][0] != 0:
                    self.log.error(
                        "Error in glite-ce-job-submit command",
    ...
    def getJobStatus(self, jobIDList):
        ...
        workingDirectory = self.ceParameters["WorkingDirectory"]
        fd, idFileName = tempfile.mkstemp(
            suffix=".ids", prefix="CREAM_", dir=workingDirectory
        )
    ...
    def getJobOutput(self, jobID, localDir=None):
        ...
        if result["OK"]:
            if not result["Value"][0]:
                ...
                result["Value"][0] == 1
                and "No such file or directory" in result["Value"][2]
            ):
