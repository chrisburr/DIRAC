...
class SudoComputingElement(ComputingElement):
    ...
    def submitJob(self, executableFile, proxy=None, **kwargs):
        ...
        if proxy:
            ...
            if payloadUsername:
                self.log.info(
                    "Payload username %s from PayloadUser in ceParameters"
                    % payloadUsername
                )
            else:
                ...
                try:
                    ...
                    self.log.info(
                        "Base username from BaseUsername in ceParameters : %s"
                        % baseUsername
                    )
                except Exception:
                    ...
                    if "USER" not in os.environ:
                        self.log.error(
                            'PayloadUser, BaseUsername and os.environ["USER"] are not properly defined'
                        )
                        return S_ERROR(
                            errno.EINVAL, "No correct payload username provided"
                        )

                    baseUsername = os.environ["USER"] + "00p00"
                    baseCounter = 0
                    self.log.info(
                        "Base username from $USER + 00p00 : %s" % baseUsername
                    )

                # Next one in the sequence
                payloadUsername = baseUsername[:-2] + (
                    "%02d" % (baseCounter + self.submittedJobs)
                )
                self.log.info(
                    "Payload username set to %s using jobs counter" % payloadUsername
                )
        ...
        # Submit job
        self.log.info(
            "Changing permissions of executable (%s) to 0755" % executableFile
        )
    ...
    #############################################################################
    def sudoExecute(
        self, executableFile, payloadProxy, payloadUsername, payloadUID, payloadGID
    ):
        ...
        cmd += "X509_USER_PROXY=/tmp/x509up_u%d sh -c '%s'" % (
            payloadUID,
            executableFile,
        )
    ...
    def monitorProxy(self, payloadProxy, payloadUsername, payloadUID, payloadGID):
        ...
        if not result["OK"]:
            self.log.error(
                "Could not recreate the copy of the proxy",
                "CMD: %s; %s" % (cmd, result["Message"]),
            )
