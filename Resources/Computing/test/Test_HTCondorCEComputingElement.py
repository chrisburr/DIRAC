...
class HTCondorCETests(unittest.TestCase):
    ...
    def setUp(self):
        self.ceParameters = {"Queue": "espresso", "GridEnv": "/dev/null"}
    ...
    @patch(
        MODNAME + ".commands.getstatusoutput",
        new=Mock(
            side_effect=(
                [(0, "\n".join(STATUS_LINES)), (0, "\n".join(HISTORY_LINES)), (0, 0)]
            )
        ),
    )
    @patch(
        MODNAME + ".HTCondorCEComputingElement._HTCondorCEComputingElement__cleanup",
        new=Mock(),
    )
    ...
    def test__writeSub_local(self):
        ...
        patchMkstemp = patch(
            MODNAME + ".tempfile.mkstemp", new=Mock(return_value=("os", "pilotName"))
        )
        patchMkdir = patch(MODNAME + ".mkDir", new=Mock())

        with patchFdopen, patchMkstemp, patchMkdir:
            htce._HTCondorCEComputingElement__writeSub(
                "dirac-install", 42, "", 1
    ...
    def test_reset_remote(self):
        ...
        htce._reset()
        self.assertEqual(
            htce.remoteScheddOptions, "-pool %s:9619 -name %s " % (ceName, ceName)
        )
    ...
    def test_submitJob_local(self):
        ...
        with patch(MODNAME + ".executeGridCommand", new=execMock), patch(
            MODNAME + ".os", new=Mock()
        ):
    ...
    def test_killJob(self, jobIDList, jobID, ret=0, success=True, local=True):
        ...
        if jobID:
            mock.assert_called_with(
                "condor_rm %s %s" % (htce.remoteScheddOptions, jobID)
            )
...
class BatchCondorTest(unittest.TestCase):
    ...
    def test_getJobStatus(self):
        ...
        with patch(MODNAME + ".commands.getstatusoutput", new=mock):
            ret = Condor.Condor().getJobStatus(
                JobIDList=["123.0", "123.1", "123.2", "333.3"]
            )
