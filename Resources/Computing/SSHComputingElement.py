...
class SSH(object):
    ...
    def __ssh_call(self, command, timeout):
        ...
        if expectFlag:
            ...
            try:
                ...
                i = child.expect(
                    [pexpect.TIMEOUT, ssh_newkey, pexpect.EOF, "assword: "]
                )
                if i == 0:  # Timeout
    ...
    def scpCall(
        self, timeout, localFile, remoteFile, postUploadCommand="", upload=True
    ):
        ...
        if upload:
            if self.sshTunnel:
                ...
                command = (
                    '/bin/sh -c \'cat %s | %s -q %s %s@%s "%s \\"cat > %s; %s\\""\' '
                    % (
                        localFile,
                        self.sshType,
                        self.options,
...
class SSHComputingElement(ComputingElement):
    ...
    def setProxy(self, proxy, valid=0):
        ...
        if self.ceParameters.get("SSHType", "ssh") == "gsissh":
            ...
            if not result["OK"]:
                gLogger.error(
                    "SSHComputingElement: failed to set up proxy", result["Message"]
                )
                return result
        return S_OK()
    ...
    def _prepareRemoteHost(self, host=None):
        ...
        cmd = "bash -c '%s'" % cmd
        self.log.verbose(
            "Creating working directories on %s" % self.ceParameters["SSHHost"]
        )
        result = ssh.sshCall(30, cmd)
        if not result["OK"]:
            self.log.error(
                "Failed creating working directories", "(%s)" % result["Message"][1]
            )
            return result
    ...
    def _generateControlScript(self):
        ...
        batchSystemDir = os.path.join(
            rootPath, "DIRAC", "Resources", "Computing", "BatchSystems"
        )
    ...
    def __executeHostCommand(self, command, options, ssh=None, host=None):
        ...
        cmd = "bash --login -c 'python %s/execute_batch %s'" % (
            self.sharedArea,
            options,
        )
        ...
        if not result["OK"]:
            self.log.error(
                "%s CE job submission failed" % self.ceType, result["Message"]
            )
            return result
        ...
        if sshStatus == 0:
            ...
            try:
                ...
                if isinstance(result, six.string_types) and result.startswith(
                    "Exception:"
                ):
    ...
    def _submitJobToHost(self, executableFile, numberOfJobs, host=None):
        ...
        result = ssh.scpCall(
            30, executableFile, submitFile, postUploadCommand="chmod +x %s" % submitFile
        )
        if not result["OK"]:
            return result
        ...
        resultCommand = self.__executeHostCommand(
            "submitJob", commandOptions, ssh=ssh, host=host
        )
        ...
        if result["Status"] != 0:
            ...
            if batchIDs:
                ...
                jobIDs = [
                    "%s%s://%s/%s"
                    % (self.ceType.lower(), self.batchSystem.lower(), ceHost, _id)
                    for _id in batchIDs
                ]
            else:
    ...
    def _getJobStatusOnHost(self, jobIDList, host=None):
        ...
        for jobList in breakListIntoChunks(stampList, 100):
            resultCommand = self.__executeHostCommand(
                "getJobStatus", {"JobIDList": jobList}, host=host
            )
            if not resultCommand["OK"]:
                return resultCommand
    ...
    def _getJobOutputFiles(self, jobID, host=None):
        ...
        if self.outputTemplate:
            ...
            resultCommand = self.__executeHostCommand(
                "getJobOutputFiles", {"JobIDList": [jobStamp]}, host=host
            )
            if not resultCommand["OK"]:
                return resultCommand
