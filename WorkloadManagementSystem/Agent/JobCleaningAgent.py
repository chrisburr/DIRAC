...
class JobCleaningAgent(AgentModule):
    ...
    def removeJobsByStatus(self, condDict, delay=False):
        ...
        if delay:
            self.log.verbose(
                "Removing jobs with %s and older than %s day(s)" % (condDict, delay)
            )
            result = self.jobDB.selectJobs(
                condDict, older=delay, limit=self.maxJobsAtOnce
            )
        else:
        ...
        if self.jobByJob:
            for jobID in jobList:
                ...
                if not resultJobDB["OK"]:
                    self.log.warn(
                        "Failed to remove job %d from JobDB" % jobID, result["Message"]
                    )
                    errorFlag = True
                if not resultTQ["OK"]:
                    self.log.warn(
            ...
            if not result["OK"]:
                self.log.error(
                    "Failed to delete %d jobs from JobLoggingDB" % len(jobList)
                )
            else:
        ...
        if count > 0 or error_count > 0:
            self.log.info(
                "Deleted %d jobs from JobDB, %d errors" % (count, error_count)
            )
        return S_OK()
    ...
    def removeHeartBeatLoggingInfo(self, status, delayDays):
        ...
        if not result["OK"]:
            self.log.error(
                "Failed to delete from HeartBeatLoggingInfo", result["Message"]
            )
        else:
