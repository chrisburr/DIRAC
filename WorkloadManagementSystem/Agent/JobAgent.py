...
class JobAgent(AgentModule):
    ...
    def __init__(self, agentName, loadName, baseAgentName=False, properties=None):
        ...
        self.computingElement = (
            None  # The ComputingElement object, e.g. SingularityComputingElement()
        )
    ...
    def initialize(self):
        ...
        if localCE != self.ceName:
            self.log.info(
                "Defining Inner CE from local configuration", "= %s" % localCE
            )
        ...
        self.ceName = localCE.split("/")[
            0
        ]  # It might be "Pool/Singularity", or simply "Pool"
        self.innerCESubmissionType = (
            localCE.split("/")[1]
            if len(localCE.split("/")) == 2
            else self.innerCESubmissionType
        )
        ...
        self.pilotReference = gConfig.getValue(
            "/LocalSite/PilotReference", self.pilotReference
        )
        ...
        # Agent options
        # This is the factor to convert raw CPU to Normalized units (based on the CPU Model)
        self.cpuFactor = gConfig.getValue(
            "/LocalSite/CPUNormalizationFactor", self.cpuFactor
        )
        self.jobSubmissionDelay = self.am_getOption(
            "SubmissionDelay", self.jobSubmissionDelay
        )
        self.fillingMode = self.am_getOption("FillingModeFlag", self.fillingMode)
        self.minimumTimeLeft = self.am_getOption(
            "MinimumTimeLeft", self.minimumTimeLeft
        )
    ...
    def execute(self):
        ...
        if not availableSlots:
            if runningJobs:
                ...
                return S_OK(
                    "Job Agent cycle complete with %d running jobs" % runningJobs
                )
        ...
        if not jobRequest["OK"]:
            ...
            self.am_setOption(
                "PollingTime", int(self.am_getOption("PollingTime") * 1.5)
            )

            if re.search("No match found", jobRequest["Message"]):
                self.log.notice(
                    "Job request OK, but no match found",
        ...
        for param in matcherParams:
            if param not in matcherInfo:
                ...
                self.log.verbose(
                    "Matcher returned", "%s = %s " % (param, matcherInfo[param])
                )
        ...
        processors = int(
            params.get(
                "NumberOfProcessors", int(params.get("MinNumberOfProcessors", 1))
            )
        )
        ...
        if self.extraOptions and "$DIRACROOT" in params.get("Executable", "").strip():
            params["Arguments"] = (
                params.get("Arguments", "") + " " + self.extraOptions
            ).strip()
        ...
        try:
            ...
            jobReport.setJobParameter(
                "MatcherServiceTime", str(matchTime), sendFlag=False
            )
            ...
            if "BOINC_JOB_ID" in os.environ:
                ...
                for thisp in (
                    ...
                    jobReport.setJobParameter(
                        thisp,
                        gConfig.getValue("/LocalSite/%s" % thisp, "Unknown"),
                        sendFlag=False,
                    )
            ...
            self.log.exception(
                "Exception in submission", "", lException=subExcept, lExcInfo=True
            )
            return self._rescheduleFailedJob(
                jobID,
    ...
    def _checkInstallSoftware(self, jobID, jobParams, resourceParams):
        ...
        self.log.verbose(
            "Found VO Software Distribution module", ": %s" % (softwareDist)
        )
    ...
    def _submitJob(
        ...
        jobParams.setdefault("ExecutionEnvironment", []).append(
            "DIRAC_JOB_PROCESSORS=%d" % processors
        )
        ...
        if submission["OK"]:
            ...
            self.log.info(
                "Job submitted", "(DIRAC JobID: %s; Batch ID: %s" % (jobID, batchID)
            )
            ...
            self.__setJobParam(
                jobID, "ErrorMessage", "%s CE Submission Error" % (self.ceName)
            )
            if "ReschedulePayload" in submission:
                self._rescheduleFailedJob(
                    jobID, submission["Message"], self.stopOnApplicationFailure
                )
        ...
        try:
            ...
            for param, value in paramsDict.items():
                if value.strip().startswith("{"):
                    ...
                    self.log.debug(
                        "Found standard parameter %s: %s" % (param, parameters[param])
                    )
    ...
    def __setJobParam(self, jobID, name, value):
        ...
        self.log.verbose(
            "Setting job parameter", "setJobParameter(%s,%s,%s)" % (jobID, name, value)
        )
        if not jobParam["OK"]:
    ...
    def __finish(self, message, stop=True):
        ...
        if stop:
            self.log.info(
                "JobAgent will stop", 'with message "%s", execution complete.' % message
            )
    ...
    def _rescheduleFailedJob(self, jobID, message, stop=True):
        ...
        jobReport.setJobStatus(
            status="Rescheduled", applicationStatus=message, sendFlag=True
        )
