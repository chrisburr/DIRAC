...
class MultiProcessorSiteDirector(SiteDirector):
    def getQueues(self, resourceDict):
        ...
        for queueName in self.queueDict:
            ...
            if maxProcessors:
                ...
                processorsTags = [
                    "%dProcessors" % processors for processors in maxProcessorsList
                ]
                if processorsTags:
    ...
    def submitPilots(self):
        ...
        for queue in queues:
            ...
            if failedCount != 0:
                self.log.warn(
                    "%s queue failed recently, skipping %d cycles"
            ...
            if not result["OK"]:
                self.log.error(
                    "Can not get the status of site %s: %s"
            ...
            if "CPUTime" in self.queueDict[queue]["ParametersDict"]:
                ...
                self.log.warn(
                    "CPU time limit is not specified for queue %s, skipping..." % queue
                )
                continue
            ...
            for tag in tqIDListByProcessors:
                ...
                self.log.verbose(
                    "Try to submit pilots for Tag=%s (TQs=%s)"
                ...
                if self.pilotWaitingFlag:
                    result = pilotAgentsDB.countPilots(
                        {"TaskQueueID": tagTqIDList, "Status": WAITING_PILOT_STATUS},
                        None,
                    )
                    if not result["OK"]:
                        self.log.error(
                ...
                self.log.verbose(
                    "Getting pilot proxy for %s/%s %d long"
                ...
                pilotsToSubmit = min(
                    self.maxPilotsToSubmit - queueSubmittedPilots, pilotsToSubmit
                )

                while pilotsToSubmit > 0:
                    self.log.info(
                        "Going to submit %d pilots to %s queue"
                    ...
                    result = ce.submitJob(
                        executable, "", pilotSubmissionChunk, processors=processors
                    )
                    ...
                    for tqID, pilotList in tqDict.items():
                        ...
                        for pilot in pilotList:
                            ...
                            if not result["OK"]:
                                self.log.error(
                                    "Failed to set pilot status: ", result["Message"]
                                )
                                continue
