...
# dirac install file
DIRAC_INSTALL = os.path.join(
    DIRAC.rootPath, "DIRAC", "Core", "scripts", "dirac-install.py"
)

# pilot2 python files (NOT needed for pilot 3)
DIRAC_PILOT = os.path.join(
    DIRAC.rootPath, "DIRAC", "WorkloadManagementSystem", "PilotAgent", "dirac-pilot.py"
)
DIRAC_MODULES = [
    os.path.join(
        DIRAC.rootPath,
...
class SiteDirector(AgentModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pilotFiles = (
            []
        )  # files whose content will be compressed and sent together with the pilot wrapper
    ...
    def beginExecution(self):
        ...
        result = findGenericPilotCredentials(
            vo=self.vo, pilotDN=self.pilotDN, pilotGroup=self.pilotGroup
        )
        if not result["OK"]:
            return result
        ...
        self.maxPilotsToSubmit = self.am_getOption(
            "MaxPilotsToSubmit", self.maxPilotsToSubmit
        )
        self.pilotWaitingFlag = self.am_getOption(
            "PilotWaitingFlag", self.pilotWaitingFlag
        )
        self.failedQueueCycleFactor = self.am_getOption(
            "FailedQueueCycleFactor", self.failedQueueCycleFactor
        )
        ...
        self.maxRetryGetPilotOutput = self.am_getOption(
            "MaxRetryGetPilotOutput", self.maxRetryGetPilotOutput
        )

        # Flags
        self.addPilotsToEmptySites = self.am_getOption(
            "AddPilotsToEmptySites", self.addPilotsToEmptySites
        )
        ...
        self.sendAccounting = self.am_getOption(
            "SendPilotAccounting", self.sendAccounting
        )
    ...
    def getQueues(self, resourceDict):
        ...
        self.log.debug(
            "The resources dictionary contains %d entries" % len(resourceDict)
        )
        ...
        for site in resourceDict:
            for ce in resourceDict[site]:
                ...
                for queue in qDict:
                    ...
                    self.queueDict[queueName]["ParametersDict"][
                        "Setup"
                    ] = gConfig.getValue("/DIRAC/Setup", "unknown")
                    ...
                    if (
                        ...
                        maxCPUTime = float(
                            self.queueDict[queueName]["ParametersDict"]["maxCPUTime"]
                        )
                        # For some sites there are crazy values in the CS
                        maxCPUTime = max(maxCPUTime, 0)
                        maxCPUTime = min(maxCPUTime, 86400 * 12.5)
                        si00 = float(
                            self.queueDict[queueName]["ParametersDict"]["SI00"]
                        )
                        queueCPUTime = 60.0 / 250.0 * maxCPUTime * si00
                        self.queueDict[queueName]["ParametersDict"]["CPUTime"] = int(
                            queueCPUTime
                        )
                    ...
                    for tagFieldName in ("Tag", "RequiredTag"):
                        ...
                        queueTags = self.queueDict[queueName]["ParametersDict"].get(
                            tagFieldName, []
                        )
                    ...
                    for parameter in ["MaxRAM", "NumberOfProcessors", "WholeNode"]:
                        queueParameter = self.queueDict[queueName][
                            "ParametersDict"
                        ].get(parameter)
                    ...
                    if (
                        queueName in self.queueCECache
                        and self.queueCECache[queueName]["Hash"] == queueHash
                    ):
                        queueCE = self.queueCECache[queueName]["CE"]
                    else:
                        result = ceFactory.getCE(
                            ceName=ce,
                    ...
                    self.queueDict[queueName]["ParametersDict"].update(
                        queueCE.ceParameters
                    )
                    ...
                    self.queueDict[queueName]["QueryCEFlag"] = ceDict.get(
                        "QueryCEFlag", "false"
                    )
                    ...
                    if self.checkPlatform:
                        ...
                        if (
                            "Platform"
                            not in self.queueDict[queueName]["ParametersDict"]
                            and platform
                        ):
                    ...
                    if "BundleProxy" in self.queueDict[queueName]["ParametersDict"]:
                        if self.queueDict[queueName]["ParametersDict"][
                            "BundleProxy"
                        ].lower() in ["true", "yes", "1"]:
    ...
    def execute(self):
        ...
        if self.rssFlag:
            ...
            if not result["OK"]:
                self.log.error(
                    "Can not get the status of computing elements",
                    " %s" % result["Message"],
                )
                return result
            self.ceMaskList = [
                ceName
                for ceName in result["Value"].iterkeys()
                if result["Value"][ceName]["all"] in ("Active", "Degraded")
            ]
    ...
    def submitPilots(self):
        ...
        for queueName, queueDictionary in queueDictItems:
            ...
            if "CPUTime" in queueDictionary["ParametersDict"]:
                ...
                self.log.warn(
                    "CPU time limit is not specified, skipping", "queue %s" % queueName
                )
                continue
            ...
            self.log.debug(
                "%d pilotsWeMayWantToSubmit are eligible for %s queue"
                % (pilotsWeMayWantToSubmit, queueName)
            )
            ...
            if self.pilotWaitingFlag:
                ...
                result = pilotAgentsDB.countPilots(
                    {"TaskQueueID": tqIDList, "Status": WAITING_PILOT_STATUS}, None
                )
                if not result["OK"]:
                    self.log.error(
                        "Failed to get Number of Waiting pilots", result["Message"]
                    )
                    totalWaitingPilots = 0
                else:
            ...
            if totalWaitingPilots >= pilotsWeMayWantToSubmit:
                self.log.verbose(
                    "Possibly enough pilots already waiting",
                    "(%d)" % totalWaitingPilots,
                )
            ...
            if manyWaitingPilotsFlag:
                ...
                pilotsToSubmit = max(
                    0, min(totalSlots, pilotsWeMayWantToSubmit - totalWaitingPilots)
                )
                self.log.info(
            ...
            while pilotsToSubmit:  # a cycle because pilots are submitted in chunks
                ...
                if not res["OK"]:
                    self.log.info(
                        "Won't try further because of failures", "Queue: %s" % queueName
                    )
                    pilotsToSubmit = 0
                    pilotList = []
                    stampDict = {}
                ...
                res = self._addPilotTQReference(
                    queueName, additionalInfo, pilotList, stampDict
                )

        self.log.info(
            "Total number of pilots submitted in this cycle",
            "%d" % self.totalSubmittedPilots,
        )
    ...
    def _allowedToSubmit(self, queue, anySite, jobSites, testSites):
        ...
        if (
            self.queueDict[queue]["Site"] not in self.siteMaskList
            and self.queueDict[queue]["Site"] not in testSites
        ):
            self.log.verbose(
                "Queue skipped (site not in mask)",
                "%s (%s)"
                % (self.queueDict[queue]["QueueName"], self.queueDict[queue]["Site"]),
            )
            return False
        ...
        if self.rssFlag:
            if self.queueDict[queue]["CEName"] not in self.ceMaskList:
                self.log.verbose(
                    ...
                    "%s at %s"
                    % (self.queueDict[queue]["CEName"], self.queueDict[queue]["Site"]),
                )
                return False
    ...
    def _getPilotsWeMayWantToSubmit(self, ceDict):
        ...
        if not result["OK"]:
            self.log.error(
                "Could not retrieve TaskQueues from TaskQueueDB", result["Message"]
            )
            return 0, {}
    ...
    def _submitPilotsToQueue(self, pilotsToSubmit, ce, queue):
        ...
        envVariables = self.queueDict[queue]["ParametersDict"].get(
        ...
        if not (
            ...
            or (
                self.queueDict[queue]["CEType"] == "Local"
                and ce.batchSystem == "Condor"
            )
        ):
            os.unlink(executable)
        if not submitResult["OK"]:
            self.log.error(
                "Failed submission to queue",
    ...
    def getQueueSlots(self, queue, manyWaitingPilotsFlag):
        ...
        queryCEFlag = self.queueDict[queue]["QueryCEFlag"].lower() in [
            "1",
            "yes",
            "true",
        ]
        ...
        availableSlotsCount = self.queueSlots[queue].setdefault(
        ...
        if totalSlots == 0:
            if availableSlotsCount % self.availableSlotsUpdateCycleFactor == 0:
                ...
                if queryCEFlag:
                    ...
                    if not result["OK"]:
                        self.log.warn(
                            "Failed to check the availability of queue",
                    ...
                    maxWaitingJobs = int(
                        self.queueDict[queue]["ParametersDict"].get(
                            "MaxWaitingJobs", 10
                        )
                    )
                    maxTotalJobs = int(
                        self.queueDict[queue]["ParametersDict"].get("MaxTotalJobs", 10)
                    )
                    waitingToRunningRatio = float(
                        self.queueDict[queue]["ParametersDict"].get(
                            "WaitingToRunningRatio", 0.0
                        )
                    )
                    waitingJobs = 0
                    totalJobs = 0
                    if jobIDList:
                        ...
                        if not result["OK"]:
                            ...
                            maxWaitingJobs = int(
                                max(maxWaitingJobs, runningJobs * waitingToRunningRatio)
                            )

                    totalSlots = min(
                        (maxTotalJobs - totalJobs), (maxWaitingJobs - waitingJobs)
                    )
    ...
    def getExecutable(
        ...
        **kwargs
        ...
        pilotOptions, pilotsSubmitted = self._getPilotOptions(
            queue, pilotsToSubmit, **kwargs
        )
        if not pilotOptions:
    ...
    def _getPilotOptions(self, queue, pilotsToSubmit, **kwargs):
        ...
        if (
            not self.pilot3
        ):  # in pilot 3 the version is taken from the JSON file exported from the CS
        ...
        if pilotExtensionsList:
            ...
            extensionsList = [
                ext for ext in CSGlobals.getCSExtensions() if "Web" not in ext
            ]
        if extensionsList:
        ...
        if "SharedArea" in queueDict:
            pilotOptions.append(
                "-o '/LocalSite/SharedArea=%s'" % queueDict["SharedArea"]
            )
    ...
    def _writePilotScript(
        ...
        try:
            pilotFilesCompressedEncodedDict = getPilotFilesCompressedEncodedDict(
                self.pilotFiles + [DIRAC_INSTALL], proxy
            )
        except BaseException as be:
            self.log.exception(
                "Exception during pilot modules files compression", lException=be
            )
        ...
        return _writePilotWrapperFile(
            workingDirectory=workingDirectory, localPilot=localPilot
        )
    ...
    def updatePilotStatus(self):
        ...
        result = gProxyManager.getPilotProxyFromDIRACGroup(
            self.pilotDN, self.pilotGroup, 23400
        )
        if not result["OK"]:
            return result
        proxy = result["Value"]
        ...
        for queue in self.queueDict:
            ...
            if self.sendAccounting:
                ...
                if not result["OK"]:
                    self.log.error(
                        "Failed to get pilots info from DB", result["Message"]
                    )
                    continue
                pilotDict = result["Value"]
    ...
    def _updatePilotStatusPerQueue(self, queue, proxy):
        ...
        if not result["OK"]:
            self.log.error(
                "Failed to get pilots status from CE",
                "%s: %s" % (ceName, result["Message"]),
            )
            return
        pilotCEDict = result["Value"]

        abortedPilots, getPilotOutput = self._updatePilotStatus(
            pilotRefs, pilotDict, pilotCEDict
        )
    ...
    def _updatePilotStatus(self, pilotRefs, pilotDict, pilotCEDict):
        ...
        for pRef in pilotRefs:
            ...
            if newStatus:
                self.log.info(
                    "Updating status", "to %s for pilot %s" % (newStatus, pRef)
                )
                result = pilotAgentsDB.setPilotStatus(
    ...
    def _getPilotOutput(self, pRef, pilotDict, ce, ceName):
        ...
        if not result["OK"]:
            ...
            self.log.error(
                "Failed to get pilot output", "%s: %s" % (ceName, result["Message"])
            )
            self.log.verbose(
                "Retries left: %d"
                % max(
                    0, self.maxRetryGetPilotOutput - self.failedPilotOutput[pRefStamp]
                )
            )
    ...
    def sendPilotAccounting(self, pilotDict):
        ...
        for pRef in pilotDict:
            ...
            if not retVal["OK"]:
                userName = "unknown"
                self.log.error(
                    "Can't determine username for dn", pilotDict[pRef]["OwnerDN"]
                )
            else:
            ...
            if result["OK"] and result["Value"]:
                pA.setValueByKey(
                    "Site", result["Value"][pilotDict[pRef]["DestinationSite"]].strip()
                )
            else:
            ...
            self.log.verbose(
                "Adding accounting record", "for pilot %s" % pilotDict[pRef]["PilotID"]
            )
    ...
    def sendPilotSubmissionAccounting(
        self, siteName, ceName, queueName, numTotal, numSucceeded, status
    ):
