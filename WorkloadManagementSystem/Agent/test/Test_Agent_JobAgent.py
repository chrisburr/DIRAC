...
def test__rescheduleFailedJob(mocker, mockJMInput, expected):
    ...
    mocker.patch("DIRAC.WorkloadManagementSystem.Agent.JobAgent.AgentModule.__init__")
    mocker.patch(
        "DIRAC.WorkloadManagementSystem.Agent.JobAgent.JobManagerClient.executeRPC",
        side_effect=mockJM,
    )
...
def test__setupProxy(mocker, mockGCReplyInput, mockPMReplyInput, expected):
    ...
    mocker.patch("DIRAC.WorkloadManagementSystem.Agent.JobAgent.AgentModule.__init__")
    mocker.patch(
        "DIRAC.WorkloadManagementSystem.Agent.JobAgent.AgentModule", side_effect=mockAM
    )
    mocker.patch(
...
def test__requestProxyFromProxyManager(
    mocker, mockGCReplyInput, mockPMReplyInput, expected
):
