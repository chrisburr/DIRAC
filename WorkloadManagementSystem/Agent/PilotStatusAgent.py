...
class PilotStatusAgent(AgentModule):
    ...
    def handleOldPilots(self, connection):
        ...
        timeLimitToConsider = Time.toString(
            Time.dateTime() - Time.day * self.pilotStalledDays
        )
    ...
    def accountPilots(self, pilotsToAccount, connection):
        ...
        if accountingFlag:
            retVal = self.pilotDB.getPilotInfo(
                list(pilotsToAccount.keys()), conn=connection
            )
            if not retVal["OK"]:
            ...
            for pref in dbData:
                if pref in pilotsToAccount:
                    if dbData[pref]["Status"] not in self.finalStateList:
                        ...
                        dbData[pref]["DestinationSite"] = pilotsToAccount[pref][
                            "DestinationSite"
                        ]
        ...
        if not accountingFlag or accountingSent:
            for pRef in pilotsToAccount:
                ...
                self.log.verbose(
                    "Setting Status for %s to %s" % (pRef, pDict["Status"])
                )
        ...
        for i in sorted(acc.keys()):
            ...
            if result["OK"] and i in result["Value"] and "Status" in result["Value"][i]:
                ...
                if ret["OK"]:
                    ...
                    self.log.error(
                        "Failed to delete pilot: ", "%s : %s" % (i, ret["Message"])
                    )
            else:
