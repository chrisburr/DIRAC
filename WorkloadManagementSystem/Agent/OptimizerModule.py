...
class OptimizerModule(AgentModule):
    ...
    def getOptimizerJobInfo(self, job, reportName):
        ...
        if result["OK"]:
            ...
            if not value:
                self.log.warn(
                    "JobDB returned null value", "for %s %s" % (job, reportName)
                )
                ...
                try:
                    ...
                    return S_ERROR(
                        "Could not evaluate optimizer parameters: %s" % repr(x)
                    )
    ...
    def setOptimizerJobInfo(self, job, reportName, value):
        ...
        """
        self.log.debug(
            "self.jobDB.setJobOptParameter(%s,'%s','%s')" % (job, reportName, value)
        )
    ...
    def setNextOptimizer(self, job):
        ...
        self.log.debug(
            "self.jobDB.setNextOptimizer(%s,'%s')"
    ...
    def setJobParam(self, job, reportName, value):
        ...
        self.log.debug(
            "self.jobDB.setJobParameter(%s,'%s','%s')" % (job, reportName, value)
        )
        if not self.am_Enabled():
            return S_OK()
    ...
    def setFailedJob(self, job, msg, classAdJob=None):
        ...
        self.log.debug(
            "self.updateJobStatus(%s,'%s','%s')" % (job, self.failedStatus, msg)
        )
        if not self.am_Enabled():
            return S_OK()
