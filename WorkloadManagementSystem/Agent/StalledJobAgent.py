...
class StalledJobAgent(AgentModule):
    ...
    def execute(self):
        ...
        wrapperSection = cfgPath(
            "Systems", "WorkloadManagement", wms_instance, "JobWrapper"
        )
        ...
        self.rescheduledTime = self.am_getOption(
            "RescheduledTime", self.rescheduledTime
        )
        ...
        watchdogCycle = max(
            watchdogCycle,
            gConfig.getValue(cfgPath(wrapperSection, "MinCheckingTime"), 20 * 60),
        )
    ...
    def _markStalledJobs(self, stalledTime):
        ...
        result = self.jobDB.selectJobs(
            {"Status": checkedStatuses}, older=checkTime, timeStamp="HeartBeatTime"
        )
        if not result["OK"]:
            return result
        if not result["Value"]:
        ...
        self.log.info(
            "Total jobs: %d, Stalled jobs: %d, %s jobs: %d"
        ...
        if jobs:
            ...
            for job in jobs:
                ...
                if pilotStatus != "Running":
                    ...
                    if not result["OK"]:
                        self.log.error(
                            "Failed to get job update time", result["Message"]
                        )
                        continue
    ...
    def __getJobPilotStatus(self, jobID):
        ...
        if not result["OK"]:
            if "No pilots found" in result["Message"]:
                ...
                return S_OK("NoPilot")
            self.log.error(
                "Failed to get pilot information",
    ...
    def __getLatestUpdateTime(self, job):
        ...
        latestUpdate = 0
        if (
            not result["Value"]["HeartBeatTime"]
            or result["Value"]["HeartBeatTime"] == "None"
        ):
        ...
        if (
            not result["Value"]["LastUpdateTime"]
            or result["Value"]["LastUpdateTime"] == "None"
        ):
            self.log.verbose("LastUpdateTime is null for job %s" % job)
        else:
            latestUpdate = max(
                latestUpdate, toEpoch(fromString(result["Value"]["LastUpdateTime"]))
            )
        ...
        if not latestUpdate:
            ...
            self.log.verbose(
                "Latest update time from epoch for job %s is %s" % (job, latestUpdate)
            )
    ...
    def __updateJobStatus(self, job, status, minorStatus=None):
        ...
        """
        self.log.verbose(
            "self.jobDB.setJobAttribute(%s,'Status','%s',update=True)" % (job, status)
        )
        ...
        if result["OK"]:
            if minorStatus:
                ...
                result = self.jobDB.setJobAttribute(
                    job, "MinorStatus", minorStatus, update=True
                )
        ...
        result = self.logDB.addLoggingRecord(
            job, status=logStatus, minorStatus=minorStatus, source="StalledJobAgent"
        )
        if not result["OK"]:
    ...
    def _kickStuckJobs(self):
        ...
        if jobIDs:
            self.log.info(
                "Rescheduling %d jobs stuck in Rescheduled status" % len(jobIDs)
            )
            result = self.jobDB.rescheduleJobs(jobIDs)
            if "FailedJobs" in result:
                if message:
                    message += "\n"
                message += (
                    "Failed to reschedule %d jobs stuck in Rescheduled status"
                    % len(result["FailedJobs"])
                )
    ...
    def _failSubmittingJobs(self):
        ...
        result = self.jobDB.selectJobs(
            {"Status": JobStatus.SUBMITTING}, older=checkTime
        )
        if not result["OK"]:
            self.log.error("Failed to select jobs", result["Message"])
            return result
    ...
    def __sendKillCommand(self, job):
        ...
        if ownerDN["OK"] and ownerGroup["OK"]:
            ...
            if not resKill["OK"]:
                self.log.error(
                    "Failed to send kill command to job",
