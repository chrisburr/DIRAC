        ...
        if not result["OK"]:
            ...
            for record in values:
                ...
                for iP in range(len(self.__summaryKeyFieldsMapping)):
                    ...
                    rD[self.__renameFieldsMapping.get(fieldName, fieldName)] = record[
                        iP
                    ]
            ...
            if self.retryOnce and self.retryValues:
                for acWMSCumulated in self.retryValues:
                    ...
                    if not retVal["OK"]:
                        self.log.error(
                            "Invalid accounting record ", "%s" % (retVal["Message"])
                        )
                    else:
            ...
            for setup in self.dsClients:
                ...
                if not result["OK"]:
                    ...
                    self.log.info(
                        "Sent %s records for setup %s" % (result["Value"], setup)
                    )
                    self.retryOnce = False
        return S_OK()
