...
class CachedJobState(object):
    ...
    def deserialize(stub):
        ...
        if dataTuple[3] is not None and (
            not isinstance(dataTuple[3], tuple) and len(dataTuple[3]) != 2
        ):
    ...
    def __cacheDict(self, prefix, functor, keyList=None):
        if not keyList or not self.__cacheExists(
            ...
            return S_OK(data)
        return S_OK(
            dict([(key, self.__cache["%s.%s" % (prefix, key)]) for key in keyList])
        )
    ...
    def __addLogRecord(
        self, majorStatus=None, minorStatus=None, appStatus=None, source=None
    ):
        record = {}
        if majorStatus:
    ...
    def getStatus(self):
        return self.__cacheResult(
            ("att.Status", "att.MinorStatus"), self.__jobState.getStatus
        )
    ...
    def setAttributes(self, attDict):
        if not isinstance(attDict, dict):
            return S_ERROR(
                "Attributes has to be a dictionary and it's %s" % str(type(attDict))
            )
        for key in attDict:
    ...
    def getAttribute(self, name):
        return self.__cacheResult(
            "att.%s" % name, self.__jobState.getAttribute, (name,)
        )
    ...
    def getOptParameter(self, name):
        return self.__cacheResult(
            "optp.%s" % name, self.__jobState.getOptParameter, (name,)
        )
