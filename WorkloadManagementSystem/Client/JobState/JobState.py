...
class JobState(object):
    ...
    def setManifest(self, manifest):
        ...
        return self.__retryFunction(
            5, JobState.__db.jobDB.setJobJDL, (self.__jid, manifestJDL)
        )
    ...
    def commitCache(self, initialState, cache, jobLog):
        ...
        if not result["Value"] == initialState:
            return S_OK(False)
        gLogger.verbose(
            "Job %s: About to execute trace. Current state %s"
        ...
        if data["att"]:
            ...
            result = self.__retryFunction(
                5,
                JobState.__db.jobDB.setJobAttributes,
        ...
        if data["jobp"]:
            result = self.__retryFunction(
                5, JobState.__db.jobDB.setJobParameters, (self.__jid, data["jobp"])
            )
            if not result["OK"]:
                return result
        ...
        for k, v in data["optp"]:
            result = self.__retryFunction(
                5, JobState.__db.jobDB.setJobOptParameter, (self.__jid, k, v)
            )
            if not result["OK"]:
                return result
        ...
        if "inputData" in cache:
            result = self.__retryFunction(
                5, JobState.__db.jobDB.setInputData, (self.__jid, cache["inputData"])
            )
            if not result["OK"]:
                return result
        ...
        for record, updateTime, source in jobLog:
            gLogger.verbose(
                "Logging records for %s: %s %s %s"
                % (self.__jid, record, updateTime, source)
            )
            record["date"] = updateTime
            record["source"] = source
            result = self.__retryFunction(
                5, JobState.__db.logDB.addLoggingRecord, (self.__jid,), record
            )
            if not result["OK"]:
                return result
    ...
    def setMinorStatus(self, minorStatus, source=None, updateTime=None):
        ...
        if not source:
            source = self.__source
        return JobState.__db.logDB.addLoggingRecord(
            self.__jid, minorStatus=minorStatus, date=updateTime, source=source
        )

    def getStatus(self):
        result = JobState.__db.jobDB.getJobAttributes(
    ...
    def setOptParameters(self, pDict):
        try:
            ...
            return S_ERROR(str(excp))
        for name in pDict:
            result = JobState.__db.jobDB.setJobOptParameter(
                self.__jid, name, pDict[name]
            )
            if not result["OK"]:
                return result
        return S_OK()
    ...
    def rescheduleJob(self, source=""):
        ...
        if not result["OK"]:
            return S_ERROR(
                "Cannot delete from TQ job %s: %s" % (self.__jid, result["Message"])
            )
        result = JobState.__db.jobDB.rescheduleJob(self.__jid)
        if not result["OK"]:
            return S_ERROR(
                "Cannot reschedule in JobDB job %s: %s"
                % (self.__jid, result["Message"])
            )
    ...
    def resetJob(self, source=""):
        result = JobState.__db.jobDB.setJobAttribute(
            self.__jid, "RescheduleCounter", -1
        )
        if not result["OK"]:
            return S_ERROR(
                "Cannot set the RescheduleCounter for job %s: %s"
                % (self.__jid, result["Message"])
            )
        result = JobState.__db.tqDB.deleteJob(self.__jid)
        if not result["OK"]:
            ...
            if "Replicas" not in pDict[lfn]:
                ...
                for seName in replicas:
                    if "SURL" not in replicas or "Disk" not in replicas:
                        return S_ERROR(
                            "Missing SURL or Disk for %s:%s replica" % (seName, lfn)
                        )
        return S_OK()
    ...
    def insertIntoTQ(self, manifest=None):
        ...
        result = self.__retryFunction(
            2, JobState.__db.tqDB.insertJob, (self.__jid, jobReqDict, jobPriority)
        )
        if not result["OK"]:
