...
class DownloadInputData(object):
    ...
    def execute(self, dataToResolve=None):
        ...
        for lfn, reps in downloadReplicas.iteritems():
            ...
            if not reps["SE"]:
                self.log.info(
                    "Failed to find data at local SEs, will try to download from anywhere",
                    lfn,
                )
                reps["SE"] = ""
            else:
        ...
        for lfn in failedReplicas:
            self.log.warn(
                "Not all file metadata (SE,PFN,Size,GUID) was available for LFN", lfn
            )
        ...
        if not result["Value"]:
            self.log.warn(
                "Not enough disk space available for download",
                "%s / %s bytes" % (result["Value"], totalSize),
            )
            self.__setJobParam(
                COMPONENT_NAME,
                "Not enough disk space available for download: %s / %s bytes"
        ...
        for lfn, info in downloadReplicas.iteritems():
            ...
            if seName:
                ...
                if error:
                    ...
                    self.log.info(
                        "Preliminary checks OK", "download %s from %s:" % (lfn, seName)
                    )
                    result = self._downloadFromSE(lfn, seName, reps, guid)
                    if not result["OK"]:
                        self.log.error(
                            "Download failed",
                            "Tried downloading from SE %s: %s"
            ...
            if not result["OK"]:
                ...
                if reps:
                    ...
                    if not result["OK"]:
                        self.log.error(
                            "Download from best SE failed",
                            "Tried downloading %s: %s" % (lfn, result["Message"]),
                        )
                        failedReplicas.add(lfn)
                else:
                    failedReplicas.add(lfn)
        ...
        if failedReplicas:
            self.log.warn(
                "The following LFN(s) could not be downloaded to the WN:\n%s"
    ...
    def __getDownloadDir(self, incrementCounter=True):
        if self.inputDataDirectory == "PerFile":
            ...
            return tempfile.mkdtemp(
                prefix="InputData_%s" % (self.counter), dir=os.getcwd()
            )
    ...
    def _downloadFromBestSE(self, lfn, reps, guid):
        ...
        """
        self.log.verbose(
            "Attempting to download file from all SEs",
            "(%s): %s" % (",".join(reps), lfn),
        )
        diskSEs = set()
        tapeSEs = set()
    ...
    def _downloadFromSE(self, lfn, seName, reps, guid):
        ...
        for localFile in (
            ...
            if os.path.exists(localFile):
                self.log.info(
                    "File already exists locally", "%s as %s" % (fileName, localFile)
                )
                fileDict = {
                    "turl": "LocalData",
                    "protocol": "LocalData",
        ...
        if os.path.exists(localFile):
            self.log.verbose(
                "File successfully downloaded locally", "(%s to %s)" % (lfn, localFile)
            )
            fileDict = {
                "turl": "Downloaded",
                "protocol": "Downloaded",
    ...
    def __cleanFailedFile(self, lfn, downloadDir):
        ...
        self.log.info(
            "Trying to remove file after failed download", "Local path: %s " % filePath
        )
        if os.path.exists(filePath):
            try:
                os.remove(filePath)
                self.log.info(
                    "Removed file remnant after failed download",
                    "Local path: %s " % filePath,
                )
            except OSError as e:
