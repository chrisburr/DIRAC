...
class SandboxStoreClient(object):
    ...
    def __init__(self, rpcClient=None, transferClient=None, smdb=False, **kwargs):
        ...
        if SandboxStoreClient.__smdb is True:
            try:
                from DIRAC.WorkloadManagementSystem.DB.SandboxMetadataDB import (
                    SandboxMetadataDB,
                )

                SandboxStoreClient.__smdb = SandboxMetadataDB()
                result = (
                    SandboxStoreClient.__smdb._getConnection()
    ...
    def uploadFilesAsSandboxForJob(self, fileList, jobId, sbType, sizeLimit=0):
        ...
        return self.uploadFilesAsSandbox(
            fileList, sizeLimit, assignTo={"Job:%s" % jobId: sbType}
        )
    ...
    def uploadFilesAsSandboxForPilot(self, fileList, jobId, sbType, sizeLimit=0):
        ...
        if sbType not in self.__validSandboxTypes:
            return S_ERROR("Invalid Sandbox type %s" % sbType)
        return self.uploadFilesAsSandbox(
            fileList, sizeLimit, assignTo={"Pilot:%s" % jobId: sbType}
        )
    ...
    def uploadFilesAsSandbox(self, fileList, sizeLimit=0, assignTo=None):
        ...
        for sFile in fileList:
            if isinstance(sFile, six.string_types):
                ...
                return S_ERROR(
                    "Objects of type %s can't be part of InputSandbox" % type(sFile)
                )
        ...
        with tarfile.open(name=tmpFilePath, mode="w|bz2") as tf:
            for sFile in files2Upload:
                if isinstance(sFile, six.string_types):
                    tf.add(
                        os.path.realpath(sFile), os.path.basename(sFile), recursive=True
                    )
                elif isinstance(sFile, StringIO.StringIO):
    ...
    def downloadSandbox(
        self, sbLocation, destinationDir="", inMemory=False, unpack=True
    ):
        """
    ...
    def assignSandboxesToJob(
        self, jobId, sbList, ownerName="", ownerGroup="", eSetup=""
    ):
        """Assign SB to a job"""
        return self.__assignSandboxesToEntity(
            "Job:%s" % jobId, sbList, ownerName, ownerGroup, eSetup
        )

    def assignSandboxToJob(
        self, jobId, sbLocation, sbType, ownerName="", ownerGroup="", eSetup=""
    ):
        """Assign SB to a job"""
        return self.__assignSandboxToEntity(
            "Job:%s" % jobId, sbLocation, sbType, ownerName, ownerGroup, eSetup
        )
    ...
    def assignSandboxesToPilot(
        self, pilotId, sbList, ownerName="", ownerGroup="", eSetup=""
    ):
        """Assign SB to a pilot"""
        return self.__assignSandboxesToEntity(
            "Pilot:%s" % pilotId, sbList, ownerName, ownerGroup, eSetup
        )

    def assignSandboxToPilot(
        self, pilotId, sbLocation, sbType, ownerName="", ownerGroup="", eSetup=""
    ):
        """Assign SB to a pilot"""
        return self.__assignSandboxToEntity(
            "Pilot:%s" % pilotId, sbLocation, sbType, ownerName, ownerGroup, eSetup
        )
    ...
    def __assignSandboxesToEntity(
        self, eId, sbList, ownerName="", ownerGroup="", eSetup=""
    ):
        """
        ...
        if SandboxStoreClient.__smdb and ownerName and ownerGroup:
            ...
            return SandboxStoreClient.__smdb.assignSandboxesToEntities(
                {eId: sbList}, ownerName, ownerGroup, eSetup
            )
        rpcClient = self.__getRPCClient()
        return rpcClient.assignSandboxesToEntities(
            {eId: sbList}, ownerName, ownerGroup, eSetup
        )

    def __assignSandboxToEntity(
        self, eId, sbLocation, sbType, ownerName="", ownerGroup="", eSetup=""
    ):
        """
        ...
        """
        return self.__assignSandboxesToEntity(
            eId, [(sbLocation, sbType)], ownerName, ownerGroup, eSetup
        )
