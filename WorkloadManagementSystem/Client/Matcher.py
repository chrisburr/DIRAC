...
class Matcher(object):
    ...
    def __init__(
        ...
        self.limiter = Limiter(
            jobDB=self.jobDB, opsHelper=self.opsHelper, pilotRef=pilotRef
        )
    ...
    def _reportStatus(self, resourceDict, jobID):
        ...
        result = self.jlDB.addLoggingRecord(
            jobID, status="Matched", minorStatus="Assigned", source="Matcher"
        )
        if not result["OK"]:
            self.log.error(
                "Problem reporting job status",
    ...
    def _checkMask(self, resourceDict):
        ...
        if not result["OK"]:
            self.log.error(
                "Internal error", "siteClient.getUsableSites: %s" % result["Message"]
            )
    ...
    def _updatePilotJobMapping(self, resourceDict, jobID):
        ...
        if pilotReference and pilotReference != "Unknown":
            ...
            result = self.pilotAgentsDB.setJobForPilot(
                jobID, pilotReference, updateStatus=False
            )
            if not result["OK"]:
                self.log.error(
                    "Problem updating pilot information",
    ...
    def _checkCredentials(self, resourceDict, credDict):
        ...
        if Properties.GENERIC_PILOT in credDict["properties"]:
            ...
            # belongs to the same group
                ...
                if (
                    "OwnerDN" in resourceDict
                    and resourceDict["OwnerDN"] != credDict["DN"]
                ):
