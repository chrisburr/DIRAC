...
NORMALIZATIONCONSTANT = (
    60.0 / 250.0
)  # from minutes to seconds and from SI00 to HS06 (ie min * SI00 -> sec * HS06 )
...
def getMachineFeatures():
    ...
    return __getFeatures(
        "MACHINEFEATURES", ("hs06", "jobslots", "log_cores", "phys_cores")
    )
...
def getPowerFromMJF():
    ...
    try:
        ...
        denom = (
            min(max(logCores, physCores), jobSlots)
            if (logCores or physCores) and jobSlots
            else None
        )
...
def queueNormalizedCPU(ceUniqueID):
    ...
    if maxCPUTime and benchmarkSI00:
        ...
        if not benchmarkSI00:
            ...
            return S_ERROR(
                "benchmarkSI00 info not available for %s" % subClusterUniqueID
            )
        if not maxCPUTime:
...
def getCPUTime(cpuNormalizationFactor):
    ...
    if cpuWorkLeft > 0:
        ...
        if (
            not cpuNormalizationFactor
        ):  # if cpuNormalizationFactor passed in is 0, try get it from the local cfg
    ...
    if not cpuTimeLeft:
        ...
        if not ceQueue:
            ...
            queues = res["Value"]
            cpuTimes = [
                gConfig.getValue(queueSection + "/" + queue + "/maxCPUTime", 9999999.0)
                for queue in queues
            ]
            ...
            if not queueInfo["OK"] or not queueInfo["Value"]:
                gLogger.warn(
                    "Can't find a CE/queue, defaulting CPUTime to %d" % cpuTimeLeft
                )
            else:
                queueCSSection = queueInfo["Value"]["QueueCSSection"]
                # These are (real, wall clock) minutes - damn BDII!
                cpuTimeInMinutes = gConfig.getValue(
                    "%s/maxCPUTime" % queueCSSection, 0.0
                )
                if cpuTimeInMinutes:
...
def getQueueInfo(ceUniqueID, diracSiteName=""):
    ...
    siteCSSEction = "/Resources/Sites/%s/%s/CEs/%s" % (
        gridType,
        diracSiteName,
        subClusterUniqueID,
    )
