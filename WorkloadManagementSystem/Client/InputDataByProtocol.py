...
class InputDataByProtocol(object):
    ...
    def execute(self, dataToResolve=None):
        ...
        if dataToResolve:
            self.log.verbose(
                "Data to resolve passed directly to InputDataByProtocol module"
            )
        ...
        self.log.verbose(
            "InputData requirement to be resolved by protocol is:\n%s"
            % "\n".join(self.inputData)
        )
        ...
        result = self.__resolveReplicas(
            localSEList, replicas, requestedProtocol=requestedProtocol
        )
        if not result["OK"]:
            return result
        ...
        if not allReplicas:
            ...
            return S_OK(
                {"Successful": bestReplica, "Failed": result["Value"]["Failed"]}
            )
        ...
        if seList:
            ...
            result = self.__resolveReplicas(
                seList, replicas, ignoreTape=True, requestedProtocol=requestedProtocol
            )
            if not result["OK"]:
                return result
    ...
    def __resolveReplicas(
        self, seList, replicas, ignoreTape=False, requestedProtocol=""
    ):
        diskSEs = set()
        tapeSEs = set()
        if not seList:
        ...
        sortedSEs = sorted(
            ((len(lfns), seName) for seName, lfns in seFilesDict.iteritems()),
            reverse=True,
        )
        ...
        for seName, lfns in seFilesDict.iteritems():
            ...
            for lfn, metadata in result["Value"]["Successful"].iteritems():
                if metadata.get("Lost", False):
                    ...
                    error = (
                        "File is declared Unavailable by the StorageElement %s" % seName
                    )
                elif seName in tapeSEs and not metadata.get(
                    "Cached", metadata["Accessible"]
                ):
            ...
            if not failedReps:
                self.log.info(
                    "Preliminary checks OK, getting TURLS at %s for:\n%s"
                    % (seName, "\n".join(lfns))
                )
            else:
                self.log.warn(
                    "Errors during preliminary checks for %d files" % len(failedReps)
                )
            ...
            if badTURLCount:
                self.log.warn(
                    "Found %s problematic TURL(s) for job %s"
                    % (badTURLCount, self.jobID)
                )
                param = "\n".join(badTURLs)
                self.log.info(param)
        ...
        for (
            lfn,
            mdataList,
        ) in trackLFNs.items():  # There is a pop below, can't use iteritems()
    ...
    def __setJobParam(self, name, value):
        ...
        return JobStateUpdateClient().setJobParameter(
