...
class Limiter(object):
    ...
    def __getRunningCondition(self, siteName):
        ...
        for attName in limitsDict:
            if attName not in self.jobDB.jobAttributeNames:
                self.log.error(
                    "Attribute does not exist", "(%s). Check the job limits" % attName
                )
                continue
            ...
            if not data:
                result = self.jobDB.getCounters(
                    "Jobs",
                    [attName],
                    {"Site": siteName, "Status": ["Running", "Matched", "Stalled"]},
                )
                if not result["OK"]:
                    return result
                data = result["Value"]
            ...
            for attValue in limitsDict[attName]:
                ...
                if running >= limit:
                    self.log.verbose(
                        "Job Limit imposed",
                        "at %s on %s/%s=%d, %d jobs already deployed"
                        % (siteName, attName, attValue, limit, running),
                    )
    ...
    def updateDelayCounters(self, siteName, jid):
        ...
        for attName in delayDict:
            if attName not in self.jobDB.jobAttributeNames:
                self.log.error(
                    "Attribute does not exist in the JobDB. Please fix it!",
                    "(%s)" % attName,
                )
            else:
        ...
        for attName in atts:
            ...
            if attValue in delayDict[attName]:
                ...
                self.log.notice(
                    "Adding delay for %s/%s=%s of %s secs"
                    % (siteName, attName, attValue, delayTime)
                )
