    ...
    def getNodeInformation(self):
        ...
        try:
            ...
            with open("/proc/cpuinfo", "r") as cpuInfo:
                ...
                result["CPU(MHz)"] = (
                    info[7].split(":")[1].replace(" ", "").replace("\n", "")
                )
                result["ModelName"] = (
                    info[4].split(":")[1].replace(" ", "").replace("\n", "")
                )
                result["CacheSize(kB)"] = (
                    info[8].split(":")[1].replace(" ", "").replace("\n", "")
                )
            with open("/proc/meminfo", "r") as memInfo:
                info = memInfo.readlines()
                result["Memory(kB)"] = (
                    info[3].split(":")[1].replace(" ", "").replace("\n", "")
                )
            ...
            result = S_ERROR()
            result["Message"] = (
                "Failed to obtain system information for " + self.systemFlag
            )
            return result
