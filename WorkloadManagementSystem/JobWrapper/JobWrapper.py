...
class JobWrapper(object):
    ...
    def __init__(self, jobID=None, jobReport=None):
        ...
        self.section = os.path.join(
            getSystemSection("WorkloadManagement/JobWrapper"), "JobWrapper"
        )
        ...
        self.defaultOutputFile = gConfig.getValue(
            self.section + "/DefaultOutputFile", "std.out"
        )
        self.defaultErrorFile = gConfig.getValue(
            self.section + "/DefaultErrorFile", "std.err"
        )
        self.diskSE = gConfig.getValue(
            self.section + "/DiskSE", ["-disk", "-DST", "-USER"]
        )
        self.tapeSE = gConfig.getValue(
            self.section + "/TapeSE", ["-tape", "-RDST", "-RAW"]
        )
        self.sandboxSizeLimit = gConfig.getValue(
            self.section + "/OutputSandboxLimit", 1024 * 1024 * 10
        )
        ...
        self.cpuNormalizationFactor = gConfig.getValue(
        ...
        self.defaultOutputSE = resolveSEGroup(
            gConfig.getValue("/Resources/StorageElementGroups/SE-USER", [])
        )
        self.defaultCatalog = gConfig.getValue(self.section + "/DefaultCatalog", [])
        self.masterCatalogOnlyFlag = gConfig.getValue(
            self.section + "/MasterCatalogOnlyFlag", True
        )
        self.defaultFailoverSE = resolveSEGroup(
    ...
    def initialize(self, arguments):
        ...
        self.__report(
            status=JobStatus.RUNNING, minorStatus=JobMinorStatus.JOB_INITIALIZATION
        )
        ...
        if self.jobID:
            ...
            if (
                extraOpts
                and "dirac-jobexec" in self.jobArgs.get("Executable", "").strip()
            ):
    ...
    def __setInitialJobParameters(self):
        ...
        if "CPUNormalizationFactor" in self.ceArgs:
            parameters.append(
                ("CPUNormalizationFactor", self.ceArgs["CPUNormalizationFactor"])
            )
        if self.boincUserID:
    ...
    def __dictAsInfoString(self, dData, infoString="", currentBase=""):
        for key in dData:
            ...
            if isinstance(value, dict):
                infoString = self.__dictAsInfoString(
                    value, infoString, "%s/%s" % (currentBase, key)
                )
    ...
    def execute(self):
        ...
        self.log.info(
            "Job Wrapper is starting execution phase for job %s" % (self.jobID)
        )
        ...
        if re.search("DIRACROOT", executable):
            ...
            self.log.verbose(
                "Replaced $DIRACROOT for executable as %s" % (self.localSiteRoot)
            )
        ...
        if not os.access(executable, os.X_OK):
            try:
                ...
                self.log.warn(
                    "Failed to change mode to 775 for the executable", executable
                )
        ...
        if os.path.exists(executable):
            ...
            self.__report(
                ...
                minorStatus=JobMinorStatus.APPLICATION,
                sendFlag=True,
            )
            ...
            exeThread = ExecutionThread(
                spObject, command, maxPeekLines, outputFile, errorFile, exeEnv
            )
            exeThread.start()
            time.sleep(10)
            ...
            self.__report(
                ...
                minorStatus=JobMinorStatus.APP_NOT_FOUND,
                sendFlag=True,
            )
        ...
        watchdogInstance = WatchdogFactory().getWatchdog(
            pid=self.currentPID,
            exeThread=exeThread,
            spObject=spObject,
        ...
        if not watchdogInstance["OK"]:
            self.log.error(
                "Could not create Watchdog instance", watchdogInstance["Message"]
            )
        ...
        if "Thread" in EXECUTION_RESULT:
            ...
            if not threadResult["OK"]:
                ...
                self.__report(
                    status=JobStatus.FAILED,
                    minorStatus=JobMinorStatus.APP_THREAD_FAILED,
                    sendFlag=True,
                )
                ...
                if outs:
                    ...
                    self.__setJobParam(
                        "ApplicationError", "None reported", sendFlag=True
                    )
            else:
            ...
            self.__report(
                status=JobStatus.FAILED,
                minorStatus=JobMinorStatus.APP_THREAD_NOT_COMPLETE,
                sendFlag=True,
            )
            self.__setJobParam(
                "ApplicationError",
                JobMinorStatus.APP_THREAD_NOT_COMPLETE,
                sendFlag=True,
            )
        ...
        if watchdog.checkError:
            ...
            self.__report(
                status=JobStatus.FAILED, minorStatus=watchdog.checkError, sendFlag=True
            )
            if "CPU" in EXECUTION_RESULT:
                if "LastUpdateCPU(s)" in watchdog.currentStats:
                    ...
                    EXECUTION_RESULT["CPU"][0] = 0
                    EXECUTION_RESULT["CPU"][0] = 0
                    EXECUTION_RESULT["CPU"][0] = watchdog.currentStats[
                        "LastUpdateCPU(s)"
                    ]
        ...
        if watchdog.currentStats:
            self.log.info(
                ...
                "\n  ".join(
                    ["%s: %s" % items for items in watchdog.currentStats.items()]
                ),
            )  # can be an iterator
        if outputs:
            ...
            if not watchdog.checkError and not status:
                ...
                self.__report(
                    ...
                    minorStatus=JobMinorStatus.APP_SUCCESS,
                    sendFlag=True,
                )
            elif not watchdog.checkError:
                self.__report(
                    status=JobStatus.COMPLETING,
                    minorStatus=JobMinorStatus.APP_ERRORS,
                    sendFlag=True,
                )
                if status in (
                    DErrno.EWMSRESC,
                    ...
                    self.__report(
                        status=JobStatus.COMPLETING,
                        minorStatus=JobMinorStatus.GOING_RESCHEDULE,
                        sendFlag=True,
                    )
    ...
    def __sendFinalStdOut(self, exeThread):
        ...
        if self.jobID:
            result = JobStateUpdateClient().sendHeartBeat(
                self.jobID, heartBeatDict, staticParamDict
            )
            if not result["OK"]:
                self.log.error(
                    "Problem sending final heartbeat from JobWrapper", result["Message"]
                )
    ...
    def resolveInputData(self):
        ...
        if not localSEList:
            ...
            self.log.info(
                "Site has the following local SEs: %s" % ", ".join(localSEList)
            )

        # How to get this data?
        if "InputDataModule" not in self.jobArgs:
            self.log.warn(
                "Job has no input data resolution module specified, using the default one"
            )
            inputDataPolicy = (
        ...
        self.log.verbose(
            "Job input data resolution policy module is %s" % (inputDataPolicy)
        )
        ...
        if self.optArgs:
            try:
                ...
                self.log.warn(str(x))
                self.log.warn(
                    "Optimizer information could not be converted to a dictionary will call catalog directly"
                )

        result = self.__checkFileCatalog(lfns, optReplicas)
        if not result["OK"]:
            self.log.info(
                "Could not obtain replica information from Optimizer File Catalog information"
            )
            self.log.warn(result)
            result = self.__checkFileCatalog(lfns)
            if not result["OK"]:
                self.log.warn(
                    "Could not obtain replica information from File Catalog directly"
                )
                self.log.warn(result)
        ...
        for lfn, mdata in resolvedData["Value"][
            "Successful"
            ...
            if "Size" in mdata:
                ...
                if not isinstance(lfnSize, six.integer_types):
                    try:
                        ...
                        lfnSize = 0
                        self.log.info(
                            "File size for LFN was not an integer, setting size to 0",
                            lfn,
                        )
    ...
    def __checkFileCatalog(self, lfns, optReplicaInfo=None):
        ...
        if failedGUIDs:
            self.log.info(
                "The following file(s) were found not to have a GUID:\n%s"
                % ",\n".join(failedGUIDs)
            )
    ...
    def __getReplicaMetadata(self, lfns):
        ...
        for lfn, replicas in catalogResult.get(
            "Successful", {}
        ).items():  # can be an iterator
            if not replicas:
                badLFNCount += 1
        ...
        if badLFNCount:
            self.log.warn(
                "Job Wrapper found %s problematic LFN(s) for job %s"
                % (badLFNCount, self.jobID)
            )
            param = "\n".join(badLFNs)
            self.log.info(param)
        ...
        if failed:
            self.log.warn(
                "Could not retrieve GUIDs from catalog for the following files"
            )
            self.log.warn(failed)
            return S_ERROR("Missing GUIDs")

        for lfn, reps in repsResult["Value"][
            "Successful"
        ].items():  # can be an iterator
    ...
    def processJobOutputs(self):
        ...
        if not resolvedSandbox["OK"]:
            ...
            self.__report(
                status=JobStatus.FAILED,
                minorStatus=JobMinorStatus.RESOLVING_OUTPUT_SANDBOX,
            )
        ...
        if missingFiles:
            self.jobReport.setJobParameter(
                "OutputSandboxMissingFiles", ", ".join(missingFiles), sendFlag=False
            )
        ...
        if not self.failedFlag:
            self.__report(
                status=JobStatus.COMPLETING,
                minorStatus=JobMinorStatus.UPLOADING_OUTPUT_SANDBOX,
            )
        ...
        if fileList and self.jobID:
            ...
            self.log.info(
                "Attempting to upload Sandbox with limit:", self.sandboxSizeLimit
            )
            ...
            if not result_sbUpload["OK"]:
                self.log.error(
                    "Output sandbox upload failed with message",
                    result_sbUpload["Message"],
                )
                outputSandboxData = result_sbUpload.get("SandboxFileName")
                if outputSandboxData:

                    self.log.info(
                        "Attempting to upload %s as output data" % (outputSandboxData)
                    )
                    if self.failedFlag:
                    ...
                    self.jobReport.setJobParameter(
                        "OutputSandbox",
                        "Sandbox uploaded to grid storage",
                        sendFlag=False,
                    )
                    self.jobReport.setJobParameter(
                        "OutputSandboxLFN",
                        self.__getLFNfromOutputFile(outputSandboxData)[0],
                        sendFlag=False,
                    )
                else:
                    self.log.info(
                        "Could not get SandboxFileName to attempt upload to Grid storage"
                    )
                    return S_ERROR(
                ...
                if not self.failedFlag:
                    self.__report(
                        status=JobStatus.COMPLETING,
                        minorStatus=JobMinorStatus.OUTPUT_SANDBOX_UPLOADED,
                    )
        ...
        if (outputData and not self.failedFlag) or uploadOutputDataInAnyCase:
            ...
            result_transferODF = self.__transferOutputDataFiles(
                outputData, outputSE, outputPath
            )
        ...
        for i in outputSandbox:
            ...
            for check in globList:
                if os.path.isfile(check):
                    self.log.verbose(
                        "Found locally existing OutputSandbox file: %s" % check
                    )
                    okFiles.append(check)
                if os.path.isdir(check):
                    self.log.verbose(
                        "Found locally existing OutputSandbox directory: %s" % check
                    )
                    cmd = ["tar", "cf", "%s.tar" % check, check]
                    result = systemCall(60, cmd)
                    if not result["OK"]:
                        self.log.error(
                            "Failed to create OutputSandbox tar", result["Message"]
                        )
                    elif result["Value"][0]:
                        self.log.error(
                            "Failed to create OutputSandbox tar", result["Value"][2]
                        )
                    if os.path.isfile("%s.tar" % (check)):
                        ...
                        self.log.warn(
                            "Could not tar OutputSandbox directory: %s" % check
                        )
    ...
    def __transferOutputDataFiles(self, outputData, outputSE, outputPath):
        ...
        self.__report(
            status=JobStatus.COMPLETING,
            minorStatus=JobMinorStatus.UPLOADING_OUTPUT_DATA,
        )
        self.log.info(
            "Output data files %s to be uploaded to %s SE"
            % (", ".join(outputData), outputSE)
        )
        missing = []
        uploaded = []
        ...
        if globbedOutputList != nonlfnList and globbedOutputList:
            self.log.info(
                "Found a pattern in the output data file list, files to upload are:",
        ...
        if not result["OK"]:
            self.log.warn(
                "Failed to determine POOL GUID(s) for output file list (OK if not POOL files)",
                result["Message"],
            )
        else:
        ...
        for outputFile in outputData:
            ...
            if fileGUID:
                self.log.verbose(
                    "Found GUID for file from POOL XML catalogue %s" % localfile
                )
            ...
            if upload["OK"]:
                self.log.info(
                    '"%s" successfully uploaded to "%s" as "LFN:%s"'
            ...
            if not result["OK"]:
                self.log.error(
                    "Completely failed to upload file to failover SEs",
                    result["Message"],
                )
                missing.append(outputFile)
            else:
                self.log.info(
                    "File %s successfully uploaded to failover storage element" % lfn
                )
                uploaded.append(lfn)
        ...
        if uploaded:
            ...
            result = JobMonitoringClient().getJobParameter(
                int(self.jobID), "UploadedOutputData"
            )
            if result["OK"]:
        ...
        self.__report(
            status=JobStatus.COMPLETING, minorStatus=JobMinorStatus.OUTPUT_DATA_UPLOADED
        )
    ...
    def transferInputSandbox(self, inputSandbox):
        ...
        self.log.info(
            "Downloading InputSandbox for job %s: %s"
            % (self.jobID, ", ".join(sandboxFiles))
        )
        if os.path.exists("%s/inputsandbox" % (self.root)):
            ...
            for inputFile in sandboxFiles:
                if os.path.exists("%s/inputsandbox/%s" % (self.root, inputFile)):
                    self.log.info(
                        "Getting InputSandbox file %s from local directory for testing"
                        % (inputFile)
                    )
            ...
            if registeredISB:
                for isb in registeredISB:
                    ...
                    if not result["OK"]:
                        self.__report(
                            status=JobStatus.RUNNING,
                            minorStatus=JobMinorStatus.FAILED_DOWNLOADING_INPUT_SANDBOX,
                        )
                        return S_ERROR(
                            "Cannot download Input sandbox %s: %s"
                            % (isb, result["Message"])
                        )
                    else:
        ...
        if lfns:
            self.log.info(
                "Downloading Input SandBox LFNs, number of files to get", len(lfns)
            )
            self.__report(
                status=JobStatus.RUNNING,
                minorStatus=JobMinorStatus.DOWNLOADING_INPUT_SANDBOX_LFN,
            )
            ...
            if not download["OK"]:
                ...
                self.__report(
                    status=JobStatus.RUNNING,
                    minorStatus=JobMinorStatus.FAILED_DOWNLOADING_INPUT_SANDBOX_LFN,
                )
            ...
            for lfn in lfns:
                if os.path.exists(
                    ...
                    % (
                        self.root,
                        os.path.basename(download["Value"]["Successful"][lfn]),
                    )
                ):
                    sandboxFiles.append(
                        os.path.basename(download["Value"]["Successful"][lfn])
        ...
        for possibleTarFile in userFiles:
            if not os.path.exists(possibleTarFile):
                continue
            try:
                if os.path.isfile(possibleTarFile) and tarfile.is_tarfile(
                    possibleTarFile
                ):
                ...
                return S_ERROR(
                    "Could not untar %s with exception %s" % (possibleTarFile, str(x))
                )
        ...
        if userFiles:
            ...
            self.log.info(
                ...
                "%0.2f MiB (%s bytes)"
                % (self.inputSandboxSize / 1048576.0, self.inputSandboxSize),
            )
    ...
    def finalize(self):
        ...
        if not res[
            "OK"
        ]:  # This means that the request could not be set (this should "almost never" happen)
            finalStatus = JobStatus.FAILED
        ...
        self.log.info(
            prString, "with%s pending requests" % ("" if requestFlag else " no")
        )
    ...
    def sendJobAccounting(self, status="", minorStatus=""):
        ...
        cpuTime = utime + stime + cutime + cstime
        execTime = elapsed
        diskSpaceConsumed = getGlobbedTotalSize(
            os.path.join(self.root, str(self.jobID))
        )
        # Fill the data
        acData = {
            "User": self.owner,
    ...
    def sendFailoverRequest(self):
        ...
        if result["OK"]:
            if isinstance(result["Value"], Operation):
                self.log.info(
                    "Adding a job state update DISET operation to the request"
                )
                request.addOperation(result["Value"])
        else:
            self.log.warn(
                "JobReportFailure",
                "Could not generate a forwardDISET operation: %s" % result["Message"],
            )
            self.log.warn(
                "JobReportFailure",
                "The job won't fail, but the jobLogging info might be incomplete",
            )
        ...
        if len(request):
            ...
            if not isValid["OK"]:
                ...
                self.log.error(
                    "Job will fail, first trying to print out the content of the request"
                )
                ...
                for counter in range(10):
                    ...
                    if result["OK"]:
                        ...
                        self.log.error(
                            "Failed to set failover request",
    ...
    def __report(self, status="", minorStatus="", sendFlag=False):
        ...
        self.log.verbose(
            "setJobStatus",
            "(%s,%s,%s,%s)" % (self.jobID, status, minorStatus, "JobWrapper"),
        )
        ...
        jobStatus = self.jobReport.setJobStatus(
            status=status, minorStatus=minorStatus, sendFlag=sendFlag
        )
        if not jobStatus["OK"]:
...
class ExecutionThread(threading.Thread):
    ...
    def run(self):
        ...
        output = spObject.systemCall(
            cmd, env=self.exeEnv, callbackFunction=self.sendOutput, shell=True
        )
        ...
        cpuString = " ".join(["%.2f" % x for x in EXECUTION_RESULT["CPU"]])
        log.info(
            "EXECUTION_RESULT[CPU] after Execution of spObject.systemCall", cpuString
        )
        log.info(
            "EXECUTION_RESULT[Thread] after Execution of spObject.systemCall",
            str(EXECUTION_RESULT["Thread"]),
        )
...
def rescheduleFailedJob(jobID, minorStatus, jobReport=None):
    ...
    try:
        ...
        jobReport.setJobStatus(
            status=JobStatus.RESCHEDULED, minorStatus=minorStatus, sendFlag=False
        )
