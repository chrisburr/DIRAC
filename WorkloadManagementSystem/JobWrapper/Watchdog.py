...
class Watchdog(object):
    ...
    def __init__(
        ...
        self.stopSigStartSeconds = int(
            jobArgs.get("StopSigStartSeconds", 1800)
        )  # 30 minutes
        self.stopSigFinishSeconds = int(
            jobArgs.get("StopSigFinishSeconds", 1800)
        )  # 30 minutes
    ...
    def initialize(self):
        ...
        # Other parameters
        self.pollingTime = gConfig.getValue(
            self.section + "/PollingTime", 10
        )  # 10 seconds
        self.checkingTime = gConfig.getValue(
            self.section + "/CheckingTime", 30 * 60
        )  # 30 minute period
        self.minCheckingTime = gConfig.getValue(
            self.section + "/MinCheckingTime", 20 * 60
        )  # 20 mins
        self.maxWallClockTime = gConfig.getValue(
            self.section + "/MaxWallClockTime", 3 * 24 * 60 * 60
        )  # e.g. 4 days
        self.jobPeekFlag = gConfig.getValue(
            self.section + "/JobPeekFlag", 1
        )  # on / off
        self.minDiskSpace = gConfig.getValue(self.section + "/MinDiskSpace", 10)  # MB
        self.loadAvgLimit = gConfig.getValue(
            self.section + "/LoadAverageLimit", 1000
        )  # > 1000 and jobs killed
        self.sampleCPUTime = gConfig.getValue(
            self.section + "/CPUSampleTime", 30 * 60
        )  # e.g. up to 20mins sample
        self.jobCPUMargin = gConfig.getValue(
            self.section + "/JobCPULimitMargin", 20
        )  # %age buffer before killing job
        self.minCPUWallClockRatio = gConfig.getValue(
            self.section + "/MinCPUWallClockRatio", 5
        )  # ratio %age
    ...
    def execute(self):
        ...
        if (
            ...
            and (time.time() - self.initialValues["StartTime"])
    ...
    def _performWallClockChecks(self):
        ...
        if (int(time.time()) > jobstartSeconds + self.stopSigStartSeconds) and (
            wallClockSecondsLeft
            < self.stopSigFinishSeconds + self.wallClockCheckSeconds
        ):
            # Need to send the signal! Assume it works to avoid sending the signal more than once
            self.log.info(
                "Sending signal %d to JobWrapper children" % self.stopSigNumber
            )
            ...
            try:
                for childPid in getChildrenPIDs(self.wrapperPID):
                    try:
                        cmdline = (
                            open("/proc/%d/cmdline" % childPid, "r")
                            .read()
                            .replace("\0", " ")
                            .strip()
                        )
                ...
                self.log.error(
                    "Failed to send signals to JobWrapper children! (%s)" % str(e)
                )
    ...
    def _performChecks(self):
        ...
        if not result["OK"]:
            ...
            if self.jobPeekFlag:
                ...
                if result["OK"]:
                    ...
                    size = len(outputList)
                    self.log.info(
                        "Last %s lines of available application output:" % (size)
                    )
                    self.log.info("================START================")
        ...
        if self.jobPeekFlag:
            ...
            if result["OK"]:
                ...
                size = len(outputList)
                recentStdOut = (
                    "Last %s lines of application output from Watchdog on %s [UTC]:"
                    % (size, Time.dateTime())
                )
    ...
    def __getCPU(self):
        ...
        """
        result = self.profiler.cpuUsageUser(
            withChildren=True, withTerminatedChildren=True
        )
        if not result["OK"]:
            self.log.warn(
                "Issue while checking consumed CPU for user", result["Message"]
            )
        ...
        result = self.profiler.cpuUsageSystem(
            withChildren=True, withTerminatedChildren=True
        )
        if not result["OK"]:
            self.log.warn(
                "Issue while checking consumed CPU for system", result["Message"]
            )
            if result["Errno"] == errno.ESRCH:
                self.log.warn(
                    "The main process does not exist (anymore). This might be correct."
    ...
    def __checkCPUConsumed(self):
        ...
        wallClockTime = (
            self.parameters["WallClockTime"][-1]
            - self.parameters["WallClockTime"][-1 - intervals]
        )
        try:
            ...
            cpuTime -= self.__convertCPUTime(
                self.parameters["CPUConsumed"][-1 - intervals]
            )["Value"]
            if cpuTime < 0:
                self.log.warn(
    ...
    def __timeLeft(self):
        ...
        if not self.littleTimeLeft:
            if timeLeft and timeLeft < self.grossTimeLeftLimit:
                self.log.info(
                    "TimeLeft bellow %s, now checking with higher frequency" % timeLeft
                )
        ...
        if "MemoryUsed" in self.parameters:
            ...
            if memory:
                summary["MemoryUsed(kb)"] = abs(
                    float(memory[-1]) - float(self.initialValues["MemoryUsed"])
                )
            else:
    ...
    def __killRunningThread(self):
        ...
        self.log.info(
            "Sending kill signal to application PID %s" % (self.spObject.getChildPID())
        )
    ...
    def __sendSignOfLife(self, jobID, heartBeatDict, staticParamDict):
        ...
        result = JobStateUpdateClient().sendHeartBeat(
            jobID, heartBeatDict, staticParamDict
        )
        if not result["OK"]:
    ...
    def getNodeInformation(self):
        ...
        self.log.warn(
            "Watchdog: " + methodName + " method should be implemented in a subclass"
        )
        return S_ERROR(
            "Watchdog: " + methodName + " method should be implemented in a subclass"
        )
