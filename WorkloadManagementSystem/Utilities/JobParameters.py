...
def getProcessorFromMJF():
    ...
    if jobFeatures:
        try:
            ...
            gLogger.error(
                "MJF is available but allocated_cpu is not an integer",
...
def getMemoryFromMJF():
    ...
    if jobFeatures:
        try:
            ...
            gLogger.error(
                "MJF is available but max_rss_bytes is not an integer",
...
def getNumberOfProcessors(siteName=None, gridCE=None, queue=None):
    ...
    numberOfProcessors = gConfig.getValue(
        "/Resources/Computing/CEDefaults/NumberOfProcessors", 0
    )
    ...
    if not (siteName and gridCE and queue):
        gLogger.error(
            "Could not find NumberOfProcessors: missing siteName or gridCE or queue. Returning '1'"
        )
        return 1
...
def getNumberOfJobProcessors(jobID):
    ...
    numberOfProcessors = gConfig.getValue(
        "Resources/Computing/JobLimits/%s/NumberOfProcessors" % jobID
    )
    if numberOfProcessors:
        return numberOfProcessors
