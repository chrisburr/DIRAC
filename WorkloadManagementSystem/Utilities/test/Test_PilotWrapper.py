...
def test_scriptReal():
    ...
    diracPilot = os.path.join(
        os.getcwd(), "WorkloadManagementSystem/PilotAgent/dirac-pilot.py"
    )
    with open(diracPilot, "r") as fd:
        diracPilot = fd.read()
    diracPilotEncoded = base64.b64encode(bz2.compress(diracPilot, 9))

    diracPilotTools = os.path.join(
        os.getcwd(), "WorkloadManagementSystem/PilotAgent/pilotTools.py"
    )
    ...
    diracPilotCommands = os.path.join(
        os.getcwd(), "WorkloadManagementSystem/PilotAgent/pilotCommands.py"
    )
