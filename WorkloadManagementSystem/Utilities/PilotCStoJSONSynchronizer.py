...
class PilotCStoJSONSynchronizer(object):
    ...
    def __init__(self):
        ...
        self.pilotRepo = (
            "https://github.com/DIRACGrid/Pilot.git"  # repository of the pilot
        )
        ...
        self.pilotScriptPath = (
            "Pilot"  # where the find the pilot scripts in the pilot repository
        )
    ...
    def sync(self):
        ...
        self.pilotFileServer = ops.getValue(
            "Pilot/pilotFileServer", self.pilotFileServer
        )
        ...
        self.pilotScriptPath = ops.getValue(
            "Pilot/pilotScriptsPath", self.pilotScriptPath
        )
        self.pilotVOScriptPath = ops.getValue(
            "Pilot/pilotVOScriptsPath", self.pilotVOScriptPath
        )
        self.pilotRepoBranch = ops.getValue(
            "Pilot/pilotRepoBranch", self.pilotRepoBranch
        )
        self.pilotVORepoBranch = ops.getValue(
            "Pilot/pilotVORepoBranch", self.pilotVORepoBranch
        )
        ...
        if self.pilotVORepo:
            self.log.notice(
                "-- Synchronizing the VO extended pilot scripts with the content of the repository --",
                "(%s)" % self.pilotVORepo,
            )
        self._syncScripts()
        self._syncChecksum()
    ...
    def _getCSDict(self):
        ...
        if not setupsInDIRACRes["OK"]:
            self.log.error(
                "Can't get sections from DIRAC/Setups", setupsInDIRACRes["Message"]
            )
        ...
        for grid in sitesSection["Value"]:
            ...
            if not gridSection["OK"]:
                self.log.error(
                    "Can't get sections from Resources", gridSection["Message"]
                )
                return gridSection
            ...
            for site in gridSection["Value"]:
                ...
                for ce in ceList["Value"]:
                    ...
                    if not res["OK"]:
                        # Skip but log it
                        self.log.error(
                            "No queues found for CE", ce + ": " + res["Message"]
                        )
                        continue
                    queueList = res["Value"]
    ...
    def _getPilotOptionsPerSetup(self, setup, pilotDict):
        ...
        if not options["OK"]:
            self.log.warn(
                "Section does not exist: skipping", "/Operations/%s/Pilot " % setup
            )
            return
        ...
        try:
            pilotDict["GenericPilotDNs"].append(
                pilotDict["Setups"][setup]["GenericPilotDN"]
            )
        except KeyError:
            pass
        ceTypesCommands = gConfig.getOptionsDict(
            "/Operations/%s/Pilot/Commands" % setup
        )
        if ceTypesCommands["OK"]:
            ...
            for ceType in ceTypesCommands["Value"]:
                ...
                pilotDict["Setups"][setup]["Commands"][ceType] = ceTypesCommands[
                    "Value"
                ][ceType].split(", ")
        ...
        if "CommandExtensions" in pilotDict["Setups"][setup]:
            ...
            pilotDict["Setups"][setup]["CommandExtensions"] = pilotDict["Setups"][
                setup
        ...
        if "LoggingMQService" in pilotDict["Setups"][setup]:
            ...
            pilotDict["Setups"][setup]["Logging"]["Host"] = loggingMQService["Value"][
                "Host"
            ]
            pilotDict["Setups"][setup]["Logging"]["Port"] = loggingMQService["Value"][
                "Port"
            ]

            loggingMQServiceQueuesSections = gConfig.getSections(
                "/Resources/MQServices/%s/Queues"
                % pilotDict["Setups"][setup]["LoggingMQService"]
            )
            ...
            for queue in loggingMQServiceQueuesSections["Value"]:
                loggingMQServiceQueue = gConfig.getOptionsDict(
                    "/Resources/MQServices/%s/Queues/%s"
    ...
    def _syncScripts(self):
        ...
        if self.pilotVORepo:
            ...
            if repo_VO.tags:
                repo_VO.git.checkout(
                    repo_VO.tags[self.pilotVOVersion], b="pilotVOScripts"
                )
            else:
                repo_VO.git.checkout(
                    "upstream/%s" % self.pilotVORepoBranch, b="pilotVOScripts"
                )
            scriptDir = os.path.join(
                pilotVOLocalRepo, self.projectDir, self.pilotVOScriptPath, "*.py"
            )
            ...
            self.log.warn(
                "The /Operations/<Setup>/Pilot/pilotVORepo option is not defined"
            )
        ...
        if repo.tags:
            ...
            with open(
                os.path.join(localRepo, self.projectDir, "releases.cfg"), "r"
            ) as releasesFile:
                lines = [line.rstrip("\n") for line in releasesFile]
                lines = [s.strip() for s in lines]
                if self.pilotVOVersion in lines:
                    self.pilotVersion = lines[
                        (lines.index(self.pilotVOVersion)) + 3
                    ].split(":")[1]
        ...
        try:
            ...
            if not os.path.isfile(
                os.path.join(pilotLocalRepo, self.pilotScriptPath, "dirac-install.py")
            ):
                self._upload(
                    filename="dirac-install.py",
                    pilotScript=os.path.join(
                        pilotLocalRepo, "Core/scripts/dirac-install.py"
                    ),
                )
                tarFiles.append(
                    os.path.join(pilotLocalRepo, "Core/scripts/dirac-install.py")
                )
            ...
            with tarfile.TarFile(name=tarPath, mode="w") as tf:
                for ptf in tarFiles:
                    ...
                    shutil.copyfile(
                        ptf, os.path.join(self.workDir, os.path.basename(ptf))
                    )
    ...
    def _upload(self, pilotDict=None, filename="", pilotScript=""):
        ...
        if not self.pilotFileServer:
            ...
            if pilotDict:
                print(
                    json.dumps(pilotDict, indent=4, sort_keys=True)
                )  # just print here as formatting is important
            ...
            for pfServer in self.pilotFileServer.replace(" ", "").split(","):
                try:
                    ...
                    if resp.status_code != 200:
                        self.log.error(
                            "Status code != 200",
                            "%s returned when POSTing on %s" % (resp.text, pfServer),
                        )
    ...
    def _checksumFile(self, filePath):
        ...
        self._checksumDict[filename] = hashlib.sha512(
