...
class JobDB(DB):
    ...
    def __init__(self):
        ...
        res = ObjectLoader().loadObject(
            "ConfigurationSystem.Client.Helpers.Resources", "getDIRACPlatform"
        )
        if not res["OK"]:
    ...
    def getAttributesForJobList(self, jobIDList, attrList=None):
        ...
        if attrList:
            ...
            if missingAttr:
                return S_ERROR(
                    "JobDB.getAttributesForJobList: Unknown Attribute(s): %s"
                    % ", ".join(missingAttr)
                )
            attrNames = ",".join(
                str(x) for x in attrList if x in self.jobAttributeNames
            )
    ...
    def traceJobParameters(
        self, site, localIDs, paramList=None, attributeList=None, date=None, until=None
    ):
        import datetime
            ...
            if jobID:
                ...
                if localID and int(localID) in localIDs:
                    ...
                    if exactTime:
                        ...
                        for att in ("EndExecTime", "HeartBeatTime"):
                            ...
                            if lastTime:
                                break
                        lastTime = (
                            datetime.datetime.strptime(lastTime, "%Y-%m-%d %H:%M:%S")
                            if lastTime
                            else now
                        )
                    ...
                    if okTime:
                        ...
                        resultDict["Successful"].setdefault(int(localID), {})[
                            int(jobID)
                        ] = attributes
    ...
    def getJobParameters(self, jobID, paramList=None):
        ...
        if paramList:
            ...
            cmd = (
                "SELECT JobID, Name, Value FROM JobParameters WHERE JobID IN (%s) AND Name IN (%s)"
            ...
            if result["OK"]:
                ...
                return S_OK(
                    resultDict
                )  # there's a slim chance that this is an empty dictionary
            else:
                return S_ERROR("JobDB.getJobParameters: failed to retrieve parameters")

        else:
            result = self.getFields(
                "JobParameters", ["JobID", "Name", "Value"], {"JobID": jobID}
            )
            if not result["OK"]:
                return result
    ...
    def getJobOptParameter(self, jobID, parameter):
        ...
        result = self.getFields(
            "OptimizerParameters", ["Value"], {"JobID": jobID, "Name": parameter}
        )
        if result["OK"]:
            if result["Value"]:
    ...
    def getJobOptParameters(self, jobID, paramList=None):
        ...
        if paramList:
            ...
            paramNames = ",".join(paramNameList)
            cmd = (
                "SELECT Name, Value from OptimizerParameters WHERE JobID=%s and Name in (%s)"
                % (jobID, paramNames)
            )
        else:
    ...
    def setNextOptimizer(self, jobID, currentOptimizer):
        ...
        result = self.setJobStatus(
            jobID, status=JobStatus.CHECKING, minorStatus=nextOptimizer
        )
        if not result["OK"]:
            return result
    ...
    def setJobAttribute(self, jobID, attrName, attrValue, update=False, myDate=None):
        ...
        if update:
            cmd = (
                "UPDATE Jobs SET %s=%s,LastUpdateTime=UTC_TIMESTAMP() WHERE JobID=%s"
                % (attrName, value, jobID)
            )
        else:
    ...
    def setJobAttributes(self, jobID, attrNames, attrValues, update=False, myDate=None):
        ...
        cmd = "UPDATE Jobs SET %s WHERE JobID in ( %s )" % (
            ", ".join(attr),
            ", ".join(jIDList),
        )
    ...
    def setEndExecTime(self, jobID, endDate=None):
        ...
        req = (
            "UPDATE Jobs SET EndExecTime=%s WHERE JobID=%s AND EndExecTime IS NULL"
            % (endDate, jobID)
        )
    ...
    def setStartExecTime(self, jobID, startDate=None):
        ...
        req = (
            "UPDATE Jobs SET HeartBeatTime=%s WHERE JobID=%s AND HeartBeatTime IS NULL"
            % (startDate, jobID)
        )
        ...
        if not ret["OK"]:
            return ret
        req = (
            "UPDATE Jobs SET StartExecTime=%s WHERE JobID=%s AND StartExecTime IS NULL"
            % (startDate, jobID)
        )
        return self._update(req)
    ...
    def setJobParameter(self, jobID, key, value):
        ...
        cmd = "REPLACE JobParameters (JobID,Name,Value) VALUES (%d,%s,%s)" % (
            int(jobID),
            e_key,
            e_value,
        )
    ...
    def setJobParameters(self, jobID, parameters):
        ...
        cmd = "REPLACE JobParameters (JobID,Name,Value) VALUES %s" % ", ".join(
            insertValueList
        )
        return self._update(cmd)
    ...
    def setJobOptParameter(self, jobID, name, value):
        ...
        return self.insertFields(
            "OptimizerParameters", ["JobID", "Name", "Value"], [jobID, name, value]
        )
    ...
    def setAtticJobParameter(self, jobID, key, value, rescheduleCounter):
        ...
        cmd = (
            "INSERT INTO AtticJobParameters (JobID,RescheduleCycle,Name,Value) VALUES(%s,%s,%s,%s)"
    ...
    def __insertNewJDL(self, jdl):
        ...
        result = self.insertFields(
            "JobJDLs", ["JDL", "JobRequirements", "OriginalJDL"], ["", "", jdl]
        )
        if not result["OK"]:
    ...
    def removeJobFromDB(self, jobIDs):
        ...
        if failedTablesList:
            result = S_ERROR(
                "Errors while job removal (tables %s)" % ",".join(failedTablesList)
            )
    ...
    def rescheduleJob(self, jobID):
        ...
        if rescheduleCounter > self.maxRescheduling:
            self.log.warn(
                "Maximum number of reschedulings is reached", "Job %s" % jobID
            )
            res = self.setJobStatus(
                jobID, status="Failed", minorStatus="Maximum of reschedulings reached"
            )
            if not res["OK"]:
                return res
            return S_ERROR(
        ...
        if result["OK"]:
            ...
            for key, value in parDict.get(jobID, {}).iteritems():
                result = self.setAtticJobParameter(
                    jobID, key, value, rescheduleCounter - 1
                )
                if not result["OK"]:
                    break
    ...
    #############################################################################
    def __setSiteStatusInMask(
        self, site, status, authorDN="Unknown", comment="No comment"
    ):
        ...
        if result["OK"]:
            if result["Value"]:
                ...
                req = "INSERT INTO SiteMask VALUES (%s,%s,UTC_TIMESTAMP(),%s,%s)" % (
                    site,
                    status,
                    authorDN,
                    comment,
            ...
            req = "INSERT INTO SiteMaskLogging VALUES (%s,%s,UTC_TIMESTAMP(),%s,%s)" % (
                site,
                status,
                authorDN,
                comment,
    ...
    def getSiteMaskLogging(self, siteList):
        ...
        if siteList:
            ...
            req = (
                "SELECT Site,Status,UpdateTime,Author,Comment FROM SiteMaskLogging WHERE Site in (%s)"
                % siteString
            )
        else:
        ...
        for site in siteList:
            if not result["Value"] or site not in availableSiteList:
                ...
                e_site = ret["Value"]
                req = (
                    "SELECT Status Site,Status,LastUpdateTime,Author,Comment FROM SiteMask WHERE Site=%s"
                    % e_site
                )
    ...
    def getSiteSummary(self):
        ...
        for site in siteList:
            ...
            req = "SELECT COUNT(JobID) FROM Jobs WHERE Status IN (%s) AND Site=%s" % (
                waitingString,
                e_site,
            )
            result = self._query(req)
            ...
            for status in [
                ...
                req = "SELECT COUNT(JobID) FROM Jobs WHERE Status=%s AND Site=%s" % (
                    status,
                    e_site,
                )
                result = self._query(req)
    ...
    def getSiteSummaryWeb(self, selectDict, sortList, startItem, maxItems):
        ...
        resultDay = self.getCounters(
            "Jobs", ["Site", "Status"], {}, newer=last_day, timeStamp="EndExecTime"
        )
        ...
        for siteFullName in resultDict:
            ...
            if total_finished > 0:
                efficiency = float(
                    siteDict[JobStatus.DONE] + siteDict[JobStatus.COMPLETED]
    ...
    def setHeartBeatData(self, jobID, dynamicDataDict):
        ...
        req = (
            "UPDATE Jobs SET HeartBeatTime=UTC_TIMESTAMP(), Status='%s' WHERE JobID=%s"
    ...
    def getHeartBeatData(self, jobID):
        ...
        cmd = (
            "SELECT Name,Value,HeartBeatTime from HeartBeatLoggingInfo WHERE JobID=%s"
            % jobID
        )
        res = self._query(cmd)
        if not res["OK"]:
        ...
        for row in values:
            result.append(
                (str(row[0]), "%.01f" % (float(row[1].replace('"', ""))), str(row[2]))
            )
    ...
    def getJobCommand(self, jobID, status="Received"):
        ...
        req = (
            "SELECT Command, Arguments FROM JobCommands WHERE JobID=%s AND Status=%s"
            % (jobID, status)
        )
        result = self._query(req)
        if not result["OK"]:
    ...
    def getSummarySnapshot(self, requestedFields=False):
        ...
        sqlCmd = "SELECT %s, %s From Jobs GROUP BY %s" % (
            defString,
            valueString,
            defString,
        )
    ...
    def removeInfoFromHeartBeatLogging(self, status, delTime, maxLines):
        ...
        self.log.verbose(
            "Removing HeartBeatLogginInfo for", "%r %r %r" % (status, delTime, maxLines)
        )
