...
class PilotAgentsDB(DB):
    ...
    def setPilotStatus(
        ...
        req = (
            "UPDATE PilotAgents SET "
            + set_string
            + " WHERE PilotJobReference='%s'" % pilotRef
        )
    ...
    def getPilotGroups(
        self, groupList=["Status", "OwnerDN", "OwnerGroup", "GridType"], condDict={}
    ):
        """
        ...
        for cond in condDict:
            condList.append(
                '%s in ( "%s" )' % (cond, '", "'.join([str(y) for y in condDict[cond]]))
            )
...
AND SubmissionTime < DATE_SUB(UTC_TIMESTAMP(),INTERVAL %d DAY)"
    ...
    ##########################################################################################
    def getPilotInfo(
        self, pilotRef=False, parentId=False, conn=False, paramNames=[], pilotID=False
    ):
        ...
        if pilotID:
            if isinstance(pilotID, list):
                condSQL.append(
                    "PilotID IN (%s)" % ",".join(["%s" % x for x in pilotID])
                )
            else:
        ...
        if parentId:
            if isinstance(parentId, list):
                condSQL.append(
                    "ParentID IN (%s)" % ",".join(["%s" % x for x in parentId])
                )
            else:
    ...
    def setPilotBenchmark(self, pilotRef, mark):
        ...
        req = "UPDATE PilotAgents SET BenchMark='%f' WHERE PilotJobReference='%s'" % (
            mark,
            pilotRef,
        )
        result = self._update(req)
    ...
    def setAccountingFlag(self, pilotRef, mark="True"):
        ...
        req = (
            "UPDATE PilotAgents SET AccountingSent='%s' WHERE PilotJobReference='%s'"
            % (mark, pilotRef)
        )
        result = self._update(req)
        return result
    ...
    def storePilotOutput(self, pilotRef, output, error):
        ...
        req = (
            "INSERT INTO PilotOutput (PilotID,StdOutput,StdError) VALUES (%d,%s,%s)"
            % (pilotID, e_output, e_error)
        )
        result = self._update(req)
    ...
    def getPilotOutput(self, pilotRef):
        ...
        req += (
            "PilotOutput.PilotID = PilotAgents.PilotID AND PilotAgents.PilotJobReference='%s'"
            % pilotRef
        )
        result = self._query(req)
        if not result["OK"]:
    ...
    def __getPilotID(self, pilotRef):
        ...
        if isinstance(pilotRef, six.string_types):
            req = (
                "SELECT PilotID from PilotAgents WHERE PilotJobReference='%s'"
                % pilotRef
            )
            result = self._query(req)
            if not result["OK"]:
            ...
            req = (
                "SELECT PilotID from PilotAgents WHERE PilotJobReference in ( %s )"
                % refString
            )
            result = self._query(req)
            if not result["OK"]:
    ...
    def setJobForPilot(self, jobID, pilotRef, site=None, updateStatus=True):
        ...
        if pilotID:
            if updateStatus:
                ...
                result = self.setPilotStatus(
                    pilotRef, status="Running", statusReason=reason, gridSite=site
                )
                if not result["OK"]:
                    return result
            req = (
                "INSERT INTO JobToPilotMapping (PilotID,JobID,StartTime) VALUES (%d,%d,UTC_TIMESTAMP())"
                % (pilotID, jobID)
            )
            result = self._update(req)
            return result
    ...
    def setCurrentJobID(self, pilotRef, jobID):
        ...
        req = "UPDATE PilotAgents SET CurrentJobID=%d WHERE PilotJobReference='%s'" % (
            jobID,
            pilotRef,
        )
        result = self._update(req)
    ...
    def getPilotsForTaskQueue(self, taskQueueID, gridType=None, limit=None):
        ...
        if gridType:
            req = (
                "SELECT PilotID FROM PilotAgents WHERE TaskQueueID=%s AND GridType='%s' "
                % (taskQueueID, gridType)
            )
        else:
        ...
        if not result["OK"]:
            ...
            if result["Value"]:
                ...
                return S_OK(pilotList)
            return S_ERROR(
                "PilotJobReferences for TaskQueueID %s not found" % taskQueueID
            )
    ...
    def getPilotsForJobID(self, jobID):
        ...
        result = self._query(
            "SELECT PilotID FROM JobToPilotMapping WHERE JobID=%s" % jobID
        )
    ...
    def getPilotCurrentJob(self, pilotRef):
        ...
        req = (
            "SELECT CurrentJobID FROM PilotAgents WHERE PilotJobReference='%s' "
            % pilotRef
        )
        ...
        if result["Value"]:
            ...
            return S_OK(jobID)
        self.log.warn(
            "Current job ID for pilot %s is not known: pilot did not match jobs yet?"
            % pilotRef
        )
        return S_OK()
    ...
    def getPilotSummary(self, startdate="", enddate=""):
        ...
        for st in st_list:
            ...
            req = (
                "SELECT DestinationSite,count(DestinationSite) FROM PilotAgents "
                + "WHERE Status='%s' " % st
            )
            if startdate:
        ...
        reqDict = {}
        reqDict["Aborted_Hour"] = (
            req + " LastUpdateTime >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 HOUR)"
        )
        reqDict["Aborted_Day"] = (
            req + " LastUpdateTime >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 DAY)"
        )
    ...
    def getPilotSummaryWeb(self, selectDict, sortList, startItem, maxItems):
        ...
        for site in resultDict:
            ...
            for ce in resultDict[site]:
                ...
                if total > 0:
                    eff = float(total - aborted) / float(total) * 100.0
                else:
                    eff = 100.0
                itemList.append("%.2f" % eff)
