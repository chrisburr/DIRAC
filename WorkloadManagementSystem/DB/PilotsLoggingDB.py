...
class PilotsLoggingDB(object):
    def __init__(self):
        ...
        self.tablesList = getattr(
            Utils.voimport("DIRAC.WorkloadManagementSystem.DB.PilotsLoggingDB"),
            "TABLESLIST",
        )
    ...
    ##########################################################################################
    def addPilotsLogging(
        self, pilotUUID, timestamp, source, phase, status, messageContent
    ):
        """Add new pilot logging entry"""

        session = self.sqlalchemySession()
        logging = PilotsLogging(
    ...
    def deletePilotsLogging(self, pilotUUID):
        ...
        session.query(PilotsLogging).filter(
            PilotsLogging.pilotUUID.in_(pilotUUID)
