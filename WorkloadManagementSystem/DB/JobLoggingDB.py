...
class JobLoggingDB(DB):
    ...
    def addLoggingRecord(
        ...
        self.log.info(
            "Adding record for job ", str(jobID) + ": '" + event + "' from " + source
        )
    ...
    def getJobLoggingInfo(self, jobID):
        ...
        cmd = (
            ...
            " LoggingInfo WHERE JobId=%d ORDER BY StatusTimeOrder,StatusTime"
            % int(jobID)
        )
    ...
    def getWMSTimeStamps(self, jobID):
        ...
        result = {}
        cmd = "SELECT Status,StatusTimeOrder FROM LoggingInfo WHERE JobID=%d" % int(
            jobID
        )
