...
class TaskQueueDB(DB):
    ...
    def findOrphanJobs(self):
        ...
        result = self._query(
            "select JobID from tq_Jobs WHERE TQId not in (SELECT TQId from tq_TaskQueues)"
        )
        if not result["OK"]:
            return result
    ...
    def fitCPUTimeToSegments(self, cpuTime):
        ...
        """
        maxCPUSegments = self.__getCSOption(
            "taskQueueCPUTimeIntervals", self.__defaultCPUSegments
        )
        try:
    ...
    def _checkTaskQueueDefinition(self, tqDefDict):
        ...
        for field in singleValueDefFields:
            if field not in tqDefDict:
                return S_ERROR(
                    "Missing mandatory field '%s' in task queue definition" % field
                )
            if field in ["CPUTime"]:
                if not isinstance(tqDefDict[field], six.integer_types):
                    return S_ERROR(
                        "Mandatory field %s value type is not valid: %s"
                        % (field, type(tqDefDict[field]))
                    )
            else:
        ...
        for field in multiValueDefFields:
            ...
            if not isinstance(tqDefDict[field], (list, tuple)):
                return S_ERROR(
                    "Multi value field %s value type is not valid: %s"
                    % (field, type(tqDefDict[field]))
                )
            ...
            if isinstance(value, (list, tuple)):
                for subValue in value:
                    if not isinstance(subValue, validTypes):
                        return S_ERROR(
                            "List contained type %s is not valid -> %s"
                            % (type(subValue), validTypes)
                        )
                if escapeValues:
                ...
                if not isinstance(value, validTypes):
                    return S_ERROR(
                        "Type %s is not valid -> %s" % (type(value), validTypes)
                    )
                if escapeValues:
        ...
        for field in singleValueDefFields:
            if field not in tqMatchDict:
                if field in mandatoryMatchFields:
                    return S_ERROR(
                        "Missing mandatory field '%s' in match request definition"
                        % field
                    )
                continue
            fieldValue = tqMatchDict[field]
            if field in ["CPUTime"]:
                result = travelAndCheckType(
                    fieldValue, six.integer_types, escapeValues=False
                )
            else:
                result = travelAndCheckType(fieldValue, basestring)
            if not result["OK"]:
                return S_ERROR(
                    "Match definition field %s failed : %s" % (field, result["Message"])
                )
        ...
        for multiField in multiValueMatchFields:
            for field in (
                ...
                if field in tqMatchDict:
                    ...
                    if not result["OK"]:
                        return S_ERROR(
                            "Match definition field %s failed : %s"
    ...
    def __createTaskQueue(self, tqDefDict, priority=1, connObj=False):
        ...
        for field in multiValueDefFields:
            if field not in tqDefDict:
                continue
            values = List.uniqueElements(
                [value for value in tqDefDict[field] if value.strip()]
            )
            if not values:
                continue
            ...
            if not result["OK"]:
                self.log.error(
                    "Failed to insert condition", "%s : %s" % field, result["Message"]
                )
                self.cleanOrphanedTaskQueues(connObj=connObj)
                return S_ERROR(
                    "Can't insert values %s for field %s: %s"
                    % (str(values), field, result["Message"])
                )
    ...
    def cleanOrphanedTaskQueues(self, connObj=False):
        ...
        for mvField in multiValueDefFields:
            result = self._update(
                "DELETE FROM `tq_TQTo%s` WHERE TQId in ( %s )"
                % (mvField, ",".join(orphanedTQs)),
                conn=connObj,
            )
            if not result["OK"]:
                return result
        ...
        result = self._update(
            "DELETE FROM `tq_TaskQueues` WHERE TQId in ( %s )" % ",".join(orphanedTQs),
            conn=connObj,
        )
        if not result["OK"]:
            return result
    ...
    def __setTaskQueueEnabled(self, tqId, enabled=True, connObj=False):
        if enabled:
            ...
            enabled = "- 1"
        upSQL = "UPDATE `tq_TaskQueues` SET Enabled = Enabled %s WHERE TQId=%d" % (
            enabled,
            tqId,
        )
        ...
        if not result["OK"]:
            self.log.error(
                "Error setting TQ state",
                "TQ %s State %s: %s" % (tqId, enabled, result["Message"]),
            )
            return result
    ...
    def __hackJobPriority(self, jobPriority):
        jobPriority = min(
            max(int(jobPriority), self.__jobPriorityBoundaries[0]),
            self.__jobPriorityBoundaries[1],
        )
    ...
    def insertJob(self, jobId, tqDefDict, jobPriority, skipTQDefCheck=False):
        ...
        tqDefDict["CPUTime"] = self.fitCPUTimeToSegments(tqDefDict["CPUTime"])
        self.log.info(
            "Inserting job with requirements",
            "(%s : %s)" % (jobId, printDict(tqDefDict)),
        )
        retVal = self.__findAndDisableTaskQueue(
            tqDefDict, skipDefinitionCheck=True, connObj=connObj
        )
        if not retVal["OK"]:
            return retVal
        tqInfo = retVal["Value"]
        ...
        try:
            result = self.__insertJobInTaskQueue(
                jobId, tqId, int(jobPriority), checkTQExists=False, connObj=connObj
            )
            if not result["OK"]:
                self.log.error(
                    "Error inserting job in TQ",
                    "Job %s TQ %s: %s" % (jobId, tqId, result["Message"]),
                )
                return result
            if newTQ:
                self.recalculateTQSharesForEntity(
                    tqDefDict["OwnerDN"], tqDefDict["OwnerGroup"], connObj=connObj
                )
        finally:
            self.__setTaskQueueEnabled(tqId, True)
        return S_OK()

    def __insertJobInTaskQueue(
        self, jobId, tqId, jobPriority, checkTQExists=True, connObj=False
    ):
        ...
        """
        self.log.info(
            "Inserting job in TQ with priority",
        ...
        if checkTQExists:
            result = self._query(
                "SELECT tqId FROM `tq_TaskQueues` WHERE TQId = %s" % tqId, conn=connObj
            )
            if not result["OK"] or not result["Value"]:
                return S_OK(
                    "Can't find task queue with id %s: %s" % (tqId, result["Message"])
                )
    ...
    def __generateTQFindSQL(self, tqDefDict, skipDefinitionCheck=False):
        ...
        for field in multiValueDefFields:
            ...
            if field in tqDefDict and tqDefDict[field]:
                ...
                valuesList = List.uniqueElements(
                    [value.strip() for value in tqDefDict[field] if value.strip()]
                )
                ...
                sqlCondList.append(
                    "`tq_TaskQueues`.TQId not in ( SELECT DISTINCT %s.TQId from %s )"
                    % (tableName, tableName)
                )
        # END MAGIC: That was easy ;)
        return S_OK(" AND ".join(sqlCondList))

    def __findAndDisableTaskQueue(
        self, tqDefDict, skipDefinitionCheck=False, retries=10, connObj=False
    ):
        """Disable and find TQ
        ...
        for _ in xrange(retries):
            result = self.__findSmallestTaskQueue(
                tqDefDict, skipDefinitionCheck=skipDefinitionCheck, connObj=connObj
            )
            if not result["OK"]:
                return result
            data = result["Value"]
            ...
            if data["enabled"] < 1:
                self.log.debug(
                    "TaskQueue {tqId} seems to be already disabled ({enabled})".format(
                        **data
                    )
                )
    ...
    def __findSmallestTaskQueue(
        self, tqDefDict, skipDefinitionCheck=False, connObj=False
    ):
        """
        ...
        :returns: S_OK() / S_ERROR
        """
        result = self.__generateTQFindSQL(
            tqDefDict, skipDefinitionCheck=skipDefinitionCheck
        )
        if not result["OK"]:
            return result
...
ORDER BY COUNT( `tq_Jobs`.JobID ) ASC"
    ...
    def matchAndGetJob(
        self, tqMatchDict, numJobsPerTry=50, numQueuesPerTry=10, negativeCond=None
    ):
...
WHERE `tq_Jobs`.TQId = %s ORDER BY RAND() / `tq_Jobs`.RealPriority ASC LIMIT 1"
        ...
        for _ in xrange(self.__maxMatchRetry):
            ...
            if "JobID" in tqMatchDict:
                ...
                preJobSQL = "%s AND `tq_Jobs`.JobId = %s " % (
                    preJobSQL,
                    tqMatchDict["JobID"],
                )
            else:
            ...
            for tqId, tqOwnerDN, tqOwnerGroup in tqList:
                ...
                if not retVal["OK"]:
                    return S_ERROR(
                        "Can't retrieve winning priority for matching job: %s"
                        % retVal["Message"]
                    )
                if not retVal["Value"]:
                    noJobsFound = True
                ...
                retVal = self._query(
                    "%s %s" % (preJobSQL % (tqId, prio), postJobSQL), conn=connObj
                )
                if not retVal["OK"]:
                    return S_ERROR(
                        "Can't begin transaction for matching job: %s"
                        % retVal["Message"]
                    )
                jobTQList = [(row[0], row[1]) for row in retVal["Value"]]
                if not jobTQList:
                    self.log.info(
                        "Task queue seems to be empty, triggering a cleaning of", tqId
                    )
                    self.__deleteTQWithDelay.add(
                        tqId, 300, (tqId, tqOwnerDN, tqOwnerGroup)
                    )
                while jobTQList:
                    jobId, tqId = jobTQList.pop(random.randint(0, len(jobTQList) - 1))
                    self.log.verbose(
                        "Trying to extract job from TQ", "%s : %s" % (jobId, tqId)
                    )
                    retVal = self.deleteJob(jobId, connObj=connObj)
                    if not retVal["OK"]:
                        msgFix = "Could not take job"
                        msgVar = " %s out from the TQ %s: %s" % (
                            jobId,
                            tqId,
                            retVal["Message"],
                        )
                    ...
                    if retVal["Value"]:
                        self.log.info(
                            "Extracted job with prio from TQ",
        ...
        self.log.info(
            "Could not find a match after %s match retries" % self.__maxMatchRetry
        )
        return S_ERROR(
            "Could not find a match after %s match retries" % self.__maxMatchRetry
        )
    ...
    def matchAndGetTaskQueue(
        ...
        if not skipMatchDictDef:
            ...
            if not retVal["OK"]:
                return retVal
        retVal = self.__generateTQMatchSQL(
    ...
    def __generateNotSQL(self, negativeCond):
        ...
        raise RuntimeError(
            "negativeCond has to be either a list or a dict or a tuple, and it's %s"
            % type(negativeCond)
        )
            ...
            if field in multiValueMatchFields:
                ...
                for value in valList:
                    ...
                    sql = (
                        ...
                        % (value, fullTableN, fullTableN, fullTableN)
                    )
                    subList.append(sql)
    ...
    def __generateTQMatchSQL(self, tqMatchDict, numQueuesToGet=1, negativeCond=None):
        ...
        if "OwnerDN" in tqMatchDict and "OwnerGroup" in tqMatchDict:
            ...
            ownerConds = []
            for group in groups:
                if Properties.JOB_SHARING in Registry.getPropertiesForGroup(
                    group.replace('"', "")
                ):
                    ...
                    for dn in dns:
                        ownerConds.append(
                            "( tq.OwnerDN = %s AND tq.OwnerGroup = %s )" % (dn, group)
                        )
                ...
                if field in tqMatchDict:
                    sqlCondList.append(
                        self.__generateSQLSubCond(
                            "tq.%s = %%s" % field, tqMatchDict[field]
                        )
                    )
        ...
        for field in multiValueMatchFields:
            ...
            if field in tqMatchDict:
                self.log.debug(
                    "Evaluating %s with value %s" % (field, tqMatchDict[field])
                )
                ...
                if field == "Tag":
                    ...
                    self.log.debug(
                        "Evaluating tag %s of type %s" % (tag_fv, type(tag_fv))
                    )
                    ...
                    if any(
                        fvx.lower().translate(None, string.punctuation) == "any"
                        for fvx in tag_fv
                    ):
                        continue
                    else:
                        sqlMultiCondList.append(
                            self.__generateTagSQLSubCond(fullTableN, tag_fv)
                        )
        ...
        # Is there something to consider?
        if not rtag_fv or any(
            fv.lower().translate(None, string.punctuation) == "any" for fv in rtag_fv
        ):
            pass
            ...
            sqlCondList.append(
                self.__generateRequiredTagSQLSubCond("`tq_TQToTags`", rtag_fv)
            )
        ...
        tqSqlCmd = (
            "SELECT tq.TQId, tq.OwnerDN, tq.OwnerGroup FROM `tq_TaskQueues` tq WHERE %s"
    ...
    def __generateTagSQLSubCond(tableName, tagMatchList):
        ...
        """
        sql1 = "SELECT COUNT(%s.Value) FROM %s WHERE %s.TQId=tq.TQId" % (
            tableName,
            tableName,
            tableName,
        )
        if not tagMatchList:
            ...
            if isinstance(tagMatchList, (list, tuple)):
                sql2 = sql1 + " AND %s.Value in ( %s )" % (
                    tableName,
                    ",".join(["%s" % v for v in tagMatchList]),
                )
            else:
    ...
    def __generateRequiredTagSQLSubCond(tableName, tagMatchList):
        ...
        of the resource
        """
        sql = "SELECT COUNT(%s.Value) FROM %s WHERE %s.TQId=tq.TQId" % (
            tableName,
            tableName,
            tableName,
        )
        if isinstance(tagMatchList, (list, tuple)):
            sql = sql + " AND %s.Value in ( %s )" % (
                tableName,
                ",".join(["%s" % v for v in tagMatchList]),
            )
...
WHERE j.JobId = %s AND t.TQId = j.TQId"
        ...
        if not retVal["OK"]:
            return S_ERROR(
                "Could not get job from task queue %s: %s" % (jobId, retVal["Message"])
            )
        data = retVal["Value"]
        if not data:
            return S_OK(False)
        ...
        retVal = self._update(
            "DELETE FROM `tq_Jobs` WHERE JobId = %s" % jobId, conn=connObj
        )
        if not retVal["OK"]:
            return S_ERROR(
                "Could not delete job from task queue %s: %s"
                % (jobId, retVal["Message"])
            )
        if retVal["Value"] == 0:
            # No job deleted
            return S_OK(False)
    ...
    def getTaskQueueForJob(self, jobId, connObj=False):
        ...
        retVal = self._query(
            "SELECT TQId FROM `tq_Jobs` WHERE JobId = %s " % jobId, conn=connObj
        )
    ...
    def getTaskQueueForJobs(self, jobIDs, connObj=False):
        ...
        retVal = self._query(
            "SELECT JobId,TQId FROM `tq_Jobs` WHERE JobId in (%s) " % jobString,
            conn=connObj,
        )
    ...
    def __getOwnerForTaskQueue(self, tqId, connObj=False):
        retVal = self._query(
            "SELECT OwnerDN, OwnerGroup from `tq_TaskQueues` WHERE TQId=%s" % tqId,
            conn=connObj,
        )
        if not retVal["OK"]:
            return retVal
    ...
    def deleteTaskQueueIfEmpty(
        self, tqId, tqOwnerDN=False, tqOwnerGroup=False, connObj=False
    ):
        """
        ...
        sqlCmd = (
            "SELECT TQId FROM `tq_TaskQueues` WHERE Enabled >= 1 AND `tq_TaskQueues`.TQId = %s "
            % tqId
        )
        sqlCmd += (
            "AND `tq_TaskQueues`.TQId not in ( SELECT DISTINCT TQId from `tq_Jobs` )"
        )
        retVal = self._query(sqlCmd, conn=connObj)
        if not retVal["OK"]:
            self.log.error(
                "Could not select task queue", "%s : %s" % tqId, retVal["Message"]
            )
            return retVal
        ...
        if tqToDel:
            for mvField in multiValueDefFields:
                retVal = self._update(
                    "DELETE FROM `tq_TQTo%s` WHERE TQId = %s" % (mvField, tqId),
                    conn=connObj,
                )
                if not retVal["OK"]:
                    return retVal
            retVal = self._update(
                "DELETE FROM `tq_TaskQueues` WHERE TQId = %s" % tqId, conn=connObj
            )
            if not retVal["OK"]:
                return retVal
    ...
    def deleteTaskQueue(self, tqId, tqOwnerDN=False, tqOwnerGroup=False, connObj=False):
        ...
        if not retVal["OK"]:
            return S_ERROR(
                "Could not delete task queue %s: %s" % (tqId, retVal["Message"])
            )
        delTQ = retVal["Value"]
        ...
        for field in multiValueDefFields:
            retVal = self._update(
                "DELETE FROM `tq_TQTo%s` WHERE TQId = %s" % (field, tqId), conn=connObj
            )
            if not retVal["OK"]:
                return retVal
        if delTQ > 0:
    ...
    def getMatchingTaskQueues(self, tqMatchDict, negativeCond=False):
        ...
        result = self.matchAndGetTaskQueue(
            tqMatchDict, numQueuesToGet=0, negativeCond=negativeCond
        )
        if not result["OK"]:
            return result
    ...
    def retrieveTaskQueues(self, tqIdList=None):
        ...
        sqlCmd = "SELECT %s FROM `tq_TaskQueues`, `tq_Jobs`" % ", ".join(
            sqlSelectEntries
        )
        sqlTQCond = ""
        if tqIdList is not None:
            if not tqIdList:
                ...
                sqlTQCond += " AND `tq_TaskQueues`.TQId in ( %s )" % ", ".join(
                    [str(id_) for id_ in tqIdList]
                )
        ...
        for field in multiValueDefFields:
            ...
            if not retVal["OK"]:
                self.log.error(
                    "Can't retrieve task queues field",
    ...
    def recalculateTQSharesForEntity(self, userDN, userGroup, connObj=False):
        ...
        if Properties.JOB_SHARING in Registry.getPropertiesForGroup(userGroup):
            ...
            return self.__setPrioritiesForEntity(
                userDN, userGroup, share, connObj=connObj
            )

        selSQL = (
            "SELECT OwnerDN, COUNT(OwnerDN) FROM `tq_TaskQueues` WHERE OwnerGroup='%s' GROUP BY OwnerDN"
            % (userGroup)
        )
        ...
        if self.isSharesCorrectionEnabled():
            entitiesShares = self.__sharesCorrector.correctShares(
                entitiesShares, group=userGroup
            )
        # Keep updating
        owners = dict(data)
        ...
        if userDN in owners and owners[userDN] > 1:
            return self.__setPrioritiesForEntity(
                userDN, userGroup, entitiesShares[userDN], connObj=connObj
            )
        # Oops the number of owners may have changed so we recalculate the prio for all owners in the group
        for userDN in owners:
            self.__setPrioritiesForEntity(
                userDN, userGroup, entitiesShares[userDN], connObj=connObj
            )
        return S_OK()

    def __setPrioritiesForEntity(
        self, userDN, userGroup, share, connObj=False, consolidationFunc="AVG"
    ):
        """
        ...
        allowBgTQs = gConfig.getValue(
            "/Registry/Groups/%s/AllowBackgroundTQs" % userGroup, False
        )
...
FROM `tq_TaskQueues` t, `tq_Jobs` j WHERE "
            ...
            return S_ERROR(
                "Unknown consolidation func %s for setting priorities"
                % consolidationFunc
            )
        ...
        for prio in prioDict:
            ...
            updateSQL = (
                "UPDATE `tq_TaskQueues` SET Priority=%.4f WHERE TQId in ( %s )"
                % (prio, tqList)
            )
    ...
    def getGroupShares():
        ...
        shares = {}
        for group in groups:
            shares[group] = gConfig.getValue(
                "/Registry/Groups/%s/JobShare" % group, DEFAULT_GROUP_SHARE
            )
        return shares
