...
class SandboxMetadataDB(DB):
    ...
    def registerAndGetOwnerId(self, owner, ownerDN, ownerGroup):
        ...
        sqlCmd = (
            "SELECT OwnerId FROM `sb_Owners` WHERE Owner = %s AND OwnerDN = %s AND OwnerGroup = %s"
        ...
        sqlCmd = (
            "INSERT INTO `sb_Owners` ( OwnerId, Owner, OwnerDN, OwnerGroup ) VALUES ( 0, %s, %s, %s )"
            % (ownerEscaped, ownerDNEscaped, ownerGroupEscaped)
        )
    ...
    def registerAndGetSandbox(self, owner, ownerDN, ownerGroup, sbSE, sbPFN, size=0):
        ...
        sqlCmd = (
            "%s VALUES ( 0, '%s', '%s', '%s', %d, UTC_TIMESTAMP(), UTC_TIMESTAMP() )"
            % (sqlCmd, ownerId, sbSE, sbPFN, size)
        )
        result = self._update(sqlCmd)
        if not result["OK"]:
            ...
            if not result["Value"]:
                return S_ERROR(
                    "Location %s already exists but doesn't belong to the user or setup"
                )
    ...
    def __accessedSandboxByCond(self, condDict):
        ...
        return self._update(
            "UPDATE `sb_SandBoxes` SET LastAccessTime=UTC_TIMESTAMP() WHERE %s"
            % " AND ".join(sqlCond)
        )

    def assignSandboxesToEntities(
        self,
        enDict,
        requesterName,
        requesterGroup,
        ...
        if ownerName or ownerGroup:
            requesterProps = Registry.getPropertiesForEntity(
                requesterGroup, name=requesterName
            )
        ...
        for entityId in enDict:
            for sbTuple in enDict[entityId]:
                ...
                entitiesToSandboxList.append(
                    (entityId, enSetup, sbTuple[1], SEName, SEPFN)
                )
        ...
        for entityId, entitySetup, SBType, SEName, SEPFN in entitiesToSandboxList:
            ...
            if not result["OK"]:
                self.log.warn(
                    "Cannot find id for %s:%s with requester %s@%s"
                    % (SEName, SEPFN, requesterName, requesterGroup)
                )
            else:
                sbId = result["Value"]
            ...
            sqlCmd = (
                "INSERT INTO `sb_EntityMapping` ( entityId, entitySetup, Type, SBId ) VALUES %s"
                % ", ".join(insertValues)
            )
            result = self._update(sqlCmd)
            if not result["OK"]:
            ...
            assigned += 1
        sqlCmd = (
            "UPDATE `sb_SandBoxes` SET Assigned=1 WHERE SBId in ( %s )"
            % ", ".join(sbIds)
        )
        result = self._update(sqlCmd)
        if not result["OK"]:
    ...
    def __filterEntitiesByRequester(
        self, entitiesList, entitiesSetup, requesterName, requesterGroup
    ):
        """
    ...
    def unassignEntities(self, entitiesDict, requesterName, requesterGroup):
        ...
        for entitySetup in entitiesDict:
            ...
            result = self.__filterEntitiesByRequester(
                entitiesIds, escapedSetup, requesterName, requesterGroup
            )
            if not result["OK"]:
            ...
            sqlCond.append(
                "EntityId in ( %s )" % ", ".join(["'%s'" % str(eid) for eid in ids])
            )
    ...
    def getSandboxesAssignedToEntity(
        self, entityId, entitySetup, requesterName, requesterGroup
    ):
        """
        ...
        if (
            Properties.JOB_ADMINISTRATOR in requesterProps
            or Properties.JOB_MONITOR in requesterProps
        ):
            # Do nothing, just ensure it doesn't fit in the other cases
            pass
        elif Properties.JOB_SHARING in requesterProps:
    ...
    def getSandboxId(
        ...
        requesterProps = Registry.getPropertiesForEntity(
            requesterGroup, name=requesterName, dn=requesterDN
        )
        if (
            Properties.JOB_ADMINISTRATOR in requesterProps
            or Properties.JOB_MONITOR in requesterProps
    ...
    def getSandboxOwner(self, SEName, SEPFN, requesterDN, requesterGroup):
        ...
        """
        res = self.getSandboxId(
            SEName, SEPFN, None, requesterGroup, "OwnerId", requesterDN=requesterDN
        )
        if not res["OK"]:
            return res

        sqlCmd = (
            "SELECT `Owner`, `OwnerDN`, `OwnerGroup` FROM `sb_Owners` WHERE `OwnerId` = %d"
            % res["Value"]
        )
