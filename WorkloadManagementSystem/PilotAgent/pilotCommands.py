...
class GetPilotVersion(CommandBase):
    ...
    def execute(self):
        ...
        if self.pp.releaseVersion:
            ...
            result = retrieveUrlTimeout(
                self.pp.pilotCFGFileLocation + "/" + self.pp.pilotCFGFile,
...
class CheckCECapabilities(CommandBase):
    ...
    def execute(self):
        ...
        checkCmd = "dirac-resource-get-parameters -S %s -N %s -Q %s %s" % (
            self.pp.site,
            self.pp.ceName,
            self.pp.queueName,
            " ".join(self.cfg),
        ...
        if self.pp.maxNumberOfProcessors == 0:
            self.pp.maxNumberOfProcessors = int(
                resourceDict.get(
                    "MaxNumberOfProcessors", resourceDict.get("NumberOfProcessors", 0)
                )
            )
        ...
        if resourceDict.get("ContainerBin"):
            self.cfg.append(
                '-o "/LocalSite/ContainerBin=%s"' % resourceDict["ContainerBin"]
            )
        if resourceDict.get("ContainerRoot"):
            self.cfg.append(
                '-o "/LocalSite/ContainerRoot=%s"' % resourceDict["ContainerRoot"]
            )
        if resourceDict.get("ContainerBind"):
            self.cfg.append(
                '-o "/LocalSite/ContainerBind=%s"' % resourceDict["ContainerBind"]
            )
        if resourceDict.get("ContainerOptions"):
            self.cfg.append(
                '-o "/LocalSite/ContainerOptions=%s"' % resourceDict["ContainerOptions"]
            )
        if resourceDict.get("ContainerExtraOpts"):
            self.cfg.append(
                '-o "/LocalSite/ContainerExtraOpts=%s"'
                % resourceDict["ContainerExtraOpts"]
            )
        ...
        if self.cfg:
            ...
            if self.pp.localConfigFile:
                self.cfg.append(
                    "-O %s" % self.pp.localConfigFile
                )  # this file is as output
                self.cfg.append(self.pp.localConfigFile)  # this file is as input
...
class CheckWNCapabilities(CommandBase):
    ...
    def execute(self):
        ...
        checkCmd = "dirac-wms-get-wn-parameters -S %s -N %s -Q %s %s" % (
            self.pp.site,
            self.pp.ceName,
            self.pp.queueName,
            " ".join(self.cfg),
        ...
        if self.pp.maxNumberOfProcessors > 0:
            numberOfProcessors = min(
                numberOfProcessorsOnWN, self.pp.maxNumberOfProcessors
            )
...
class ConfigureSite(CommandBase):
    ...
    def execute(self):
        ...
        if self.boincHostPlatform:
            self.cfg.append(
                "-o /LocalSite/BoincHostPlatform=%s" % self.boincHostPlatform
            )
    ...
    def __getCEName(self):
        ...
        if self.pp.flavour in ["LCG", "OSG"]:
            retCode, CEName = self.executeAndGetOutput(
                "glite-brokerinfo getCE", self.pp.installEnv
            )
            if retCode:
                self.log.warn(
                    "Could not get CE name with 'glite-brokerinfo getCE' command [ERROR %d]"
                    % retCode
                )
...
class ConfigureCPURequirements(CommandBase):
    ...
    def execute(self):
        ...
        cpuNormalizationFactor = float(
            cpuNormalizationFactorOutput.split("\n")[0]
            .replace("Estimated CPU power is ", "")
            .replace(" HS06", "")
        )
        self.log.info(
        ...
        retCode, cpuTimeOutput = self.executeAndGetOutput(
            "dirac-wms-get-queue-cpu-time %s %s"
            % (configFileArg, self.pp.localConfigFile),
            self.pp.installEnv,
        )
...
class LaunchAgent(CommandBase):
    ...
    def __setInProcessOpts(self):
        ...
        self.inProcessOpts.append(
            "-o /LocalSite/MaxCPUTime=%s" % (int(self.pp.jobCPUReq))
        )
    ...
    def __startJobAgent(self):
        ...
        if self.pp.executeCmd:
            ...
            self.exitWithError(
                os.system("source bashrc; %s" % self.pp.executeCmd) / 256
            )
