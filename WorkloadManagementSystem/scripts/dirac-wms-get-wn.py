...
if __name__ == "__main__":
    ...
    for switch in switches:
        if switch[0] == "Site":
            ...
            if isinstance(since, int):
                since = str(
                    datetime.datetime.now() - datetime.timedelta(days=since)
                ).split()[0]
    ...
    for stat in status:
        res = dirac.selectJobs(
            site=site, date=since, status=stat, minorStatus=minorStatus
        )
        if not res["OK"]:
        ...
        if not res["OK"]:
            ...
            for job in allJobs:
                stat = (
                    ...
                    + "; "
                    + jobApplicationStatus.get(job, {}).get(
    ...
    if workerNodes or batchIDs:
        gLogger.always(
            "Found %d jobs at %s, WN %s (since %s):"
