...
class OptimizerExecutor(ExecutorModule):
            ...
            def __call__(self, msg, varMsg=""):
                try:
                    ...
                    raise AttributeError(
                        "Logger does not have %s method" % self.__funcName
                    )
                msg = "\n".join(
                    "[JID %s] %s" % (self.__jid, line) for line in msg.split("\n")
                )
                funcObj(msg, varMsg)
    ...
    def setNextOptimizer(self, jobState=None):
        ...
        if chainLength - 1 == opIndex:
            ...
            result = jobState.setStatus(
                ...
                minorStatus=self.ex_getOption(
                    "WaitingMinorStatus", "Pilot Agent Submission"
                ),
                appStatus="Unknown",
                source=opName,
            )
    ...
    def retrieveOptimizerParam(self, name):
        ...
        try:
            ...
            self.jobLog.warn(
                "Opt param %s doesn't seem to be dencoded %s" % (name, valenc)
            )
    ...
    def jobLog(self):
        if not self.__jobData.jobLog:
            raise RuntimeError(
                "jobLog can only be invoked inside the optimizeJob function"
            )
    ...
    def fastTrackDispatch(self, jid, jobState):
        ...
        if not result["OK"]:
            return S_ERROR(
                "Could not retrieve job status for %s: %s" % (jid, result["Message"])
            )
        ...
        if not result["OK"]:
            return S_ERROR(
                "Could not retrieve OptimizerChain for job %s: %s"
                % (jid, result["Message"])
            )
