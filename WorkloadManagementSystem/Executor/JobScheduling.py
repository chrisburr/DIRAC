...
from DIRAC.StorageManagementSystem.Client.StorageManagerClient import (
    StorageManagerClient,
    getFilesToStage,
)
from DIRAC.WorkloadManagementSystem.Executor.Base.OptimizerExecutor import (
    OptimizerExecutor,
)
...
class JobScheduling(OptimizerExecutor):
    ...
    def optimizeJob(self, jid, jobState):
        ...
        if reschedules != 0:
            ...
            if waited < delay:
                return self.__holdJob(
                    jobState, "On Hold: after rescheduling %s" % reschedules, delay
                )
        ...
        if userSites:
            if jobType not in self.ex_getOption("ExcludedOnHoldJobTypes", []):
                ...
                if invalidSites:
                    self.jobLog.debug(
                        "Invalid site(s) requested: %s" % ",".join(invalidSites)
                    )
                    if not self.ex_getOption("AllowInvalidSites", True):
                        return self.__holdJob(
                            jobState,
                            "Requested site(s) %s are invalid" % ",".join(invalidSites),
                        )
                if bannedSites:
                    self.jobLog.debug(
                        "Banned site(s) %s ignored" % ",".join(bannedSites)
                    )
                    if not usableSites:
                        return self.__holdJob(
                            jobState,
                            "Requested site(s) %s are inactive" % ",".join(bannedSites),
                        )
        ...
        if checkPlatform and jobPlatform:
            ...
            if not result["OK"]:
                self.jobLog.error(
                    "Unable to get OSCompatibility list", result["Message"]
                )
                return result
        ...
        if checkPlatform and userSites:
            if jobPlatform:
                ...
                if not result["OK"]:
                    self.jobLog.error(
                        "Failed to filter job sites by platform", result["Message"]
                    )
                    return result
                ...
                if not userSites:
                    ...
                    return S_ERROR(
                        "No selected sites match platform '%s'" % jobPlatform
                    )
        ...
        for site in idSites:
            if numData != idSites[site]["disk"] + idSites[site]["tape"]:
                self.jobLog.error(
                    "Site candidate does not have all the input data", "(%s)" % site
                )
                errorSites.add(site)
        ...
        if not stageSites:
            return self.__holdJob(
                jobState, "Sites %s are inactive or banned" % ", ".join(siteCandidates)
            )
        ...
        if not stageRequired:
            ...
            return self.__sendToTQ(
                jobState, jobManifest, siteCandidates, userBannedSites
            )
        ...
        self.jobLog.verbose(
            "Updating Optimizer Info", ": %s for %s" % (idAgent, opData)
        )
    ...
    def _getTagsFromManifest(self, jobManifest):
        ...
        if "WholeNode" in jobManifest:
            if jobManifest.getOption("WholeNode", "").lower() in [
                "1",
                "yes",
                "true",
                "y",
    ...
    def __sendToTQ(self, jobState, jobManifest, sites, bannedSites, onlineSites=None):
        ...
        if not result["OK"]:
            self.jobLog.error(
                "Cannot create jobManifest section",
                "(%s: %s)" % reqSection,
                result["Message"],
            )
            return result
    ...
    def __preRequestStaging(self, jobManifest, stageSite, opData):
        ...
        connectionLevel = (
            "DOWNLOAD" if "download" in inputDataPolicy.lower() else "PROTOCOL"
        )
        ...
        sortedSEs = reversed(
            sorted([(len(stageLFNs[seName]), seName) for seName in stageLFNs])
        )
    ...
    def __requestStaging(self, jobState, stageLFNs):
        ...
        if not result["OK"]:
            self.jobLog.error(
                "Could not send stage request", ": %s" % result["Message"]
            )
            return result
    ...
    def __setJobSite(self, jobState, siteList, onlineSites=None):
        ...
        if len(onlineSites) == 1:
            ...
            self.jobLog.info(
                "Several input sites are candidate", ": %s" % ",".join(onlineSites)
            )
        else:
    ...
    def __checkStageAllowed(self, jobState):
        ...
        if not result["OK"]:
            self.jobLog.error(
                "Cannot retrieve OwnerGroup from DB", ": %s" % result["Message"]
            )
            return result
        group = result["Value"]
