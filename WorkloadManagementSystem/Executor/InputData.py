...
class InputData(OptimizerExecutor):
    ...
    def initializeOptimizer(cls):
        ...
        cls.failedMinorStatus = cls.ex_getOption(
            "/FailedJobStatus", "Input Data Not Available"
        )
    ...
    def optimizeJob(self, jid, jobState):
        ...
        if result["OK"] and result["Value"]:
            ...
            if self.checkWithUserProxy:
                ...
                if not result["OK"]:
                    self.jobLog.error(
                        "Could not retrieve job owner group", result["Message"]
                    )
                    return result
                ...
                if inputSandbox:
                    ...
                    if not result["OK"]:
                        self.jobLog.error(
                            "Could not resolve input sandbox", result["Message"]
                        )
                        return result
    ...
    def _resolveInputData(self, jobState, inputData):
        ...
        self.jobLog.verbose(
            "Catalog replicas lookup time", "%.2f seconds " % (time.time() - startTime)
        )
        if not result["OK"]:
        ...
        result = self.storeOptimizerParam(
            self.ex_getProperty("optimizerName"), resolvedData
        )
        if not result["OK"]:
            self.log.warn(result["Message"])
            return result
    ...
    def __checkReplicas(self, replicaDict, vo):
        ...
        if badLFNs:
            errorMsg = "\n".join(badLFNs)
            self.jobLog.info(
                "Found a number of problematic LFN(s)",
                "%d\n: %s" % (len(badLFNs), errorMsg),
            )
            result = self.storeOptimizerParam(
                self.ex_getProperty("optimizerName"), errorMsg
            )
            if not result["OK"]:
