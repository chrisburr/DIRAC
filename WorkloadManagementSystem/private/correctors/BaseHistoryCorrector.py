...
from DIRAC.WorkloadManagementSystem.private.correctors.BaseCorrector import (
    BaseCorrector,
)
...
class BaseHistoryCorrector(BaseCorrector):
    ...
    def updateHistoryKnowledge(self):
        ...
        for timeSlice in self.__slices:
            result = self._getUsageHistoryForTimeSpan(
                self.__slices[timeSlice][self._SLICE_TIME_SPAN], self.getGroup()
            )
            if not result["OK"]:
                self.__usageHistory = {}
                self.log.warn(
                    "Could not get history for slice",
                    "%s: %s" % (timeSlice, result["Message"]),
                )
                return
            self.__usageHistory[timeSlice] = result["Value"]
            self.log.verbose(
                "Got history for slice %s (%s entities in slice)"
    ...
    def _getUsageHistoryForTimeSpan(self, timeSpan, groupToUse=""):
        ...
        if groupToUse:
            ...
            for userName in data:
                ...
                if not result["OK"]:
                    self.log.error(
                        "User does not have any DN assigned",
    ...
    def __normalizeShares(self, entityShares):
        ...
        for entity in entityShares:
            ...
            self.log.verbose(
                "Normalized share for %s: %.3f" % (entity, normalizedShare)
            )
    ...
    def applyCorrection(self, entitiesExpectedShare):
        ...
        entitiesSliceCorrections = dict(
            [(entity, []) for entity in entitiesExpectedShare]
        )
        for timeSlice in self.__usageHistory:
            ...
            for entity in entitiesExpectedShare:
                if entity in sliceHistory:
                    ...
                    self.log.verbose(
                        "Usage for %s: %.3f" % (entity, sliceHistory[entity])
                    )
            ...
            for entity in entitiesExpectedShare:
                if entity in sliceHistory:
                    ...
                    sliceCorrectionFactor = (
                        normalizedShares[entity] / normalizedSliceUsage
                    )
                    sliceCorrectionFactor = min(
                        sliceCorrectionFactor, maxSliceCorrection
                    )
                    sliceCorrectionFactor = max(
                        sliceCorrectionFactor, minSliceCorrection
                    )
                    ...
                    self.log.verbose(
                        "Entity %s is not present in slice %s" % (entity, timeSlice)
                    )
                    sliceCorrectionFactor = maxSliceCorrection
                self.log.verbose(
        ...
        for entity in entitiesSliceCorrections:
            ...
            if not slicesCorrections:
                ...
                entityCorrectionFactor = min(
                    entityCorrectionFactor, maxGlobalCorrectionFactor
                )
                entityCorrectionFactor = max(
                    entityCorrectionFactor, minGlobalCorrectionFactor
                )
