...
class SharesCorrector(object):
    ...
    def __getCorrectorClass(self, correctorName):
        ...
        result = self.__objLoader.getObjects(
            baseImport, ".*Corrector", parentClass=BaseCorrector
        )
        if not result["OK"]:
            return result
        data = result["Value"]
    ...
    def instantiateRequiredCorrectors(self):
        ...
        for corrector in correctorsToStart:
            if corrector not in self.__shareCorrectors:
                ...
                result = self.__opsHelper.getSections(
                    "%s/%s" % (self.__baseCS, corrector)
                )
                if not result["OK"]:
                    self.__log.error(
                ...
                if not result["OK"]:
                    self.__log.error(
                        "Cannot instantiate corrector",
                        "%s %s" % (corrector, result["Message"]),
                    )
                    continue
                ...
                for groupCor in groupCorrectors:
                    ...
                    if groupToCorrect:
                        ...
                        groupKey = "global"
                    self.__log.info(
                        "Instantiating group corrector %s (%s) of type %s"
                        % (groupCor, groupToCorrect, corrector)
                    )
                    if groupKey in self.__shareCorrectors[corrector]:
                        ...
                        correctorObj = correctorClass(
                            self.__opsHelper, groupCorPath, groupToCorrect
                        )
    ...
    def correctShares(self, shareDict, group=""):
        ...
        for corrector in self.__shareCorrectors:
            if groupKey in self.__shareCorrectors[corrector]:
                shareDict = self.__shareCorrectors[corrector][groupKey].applyCorrection(
                    shareDict
                )
        return shareDict
