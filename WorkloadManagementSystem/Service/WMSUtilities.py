...
def getPilotLoggingInfo(grid, pilotRef):
    ...
    if grid == "CREAM":
        ...
        from DIRAC.Resources.Computing.HTCondorCEComputingElement import (
            getCondorLogFile,
        )
...
def getGridJobOutput(pilotReference):
    ...
    result = getQueue(
        pilotDict["GridSite"], pilotDict["DestinationSite"], pilotDict["Queue"]
    )
    if not result["OK"]:
        return result
    ...
    result = ceFactory.getCE(
        pilotDict["GridType"], pilotDict["DestinationSite"], queueDict
    )
    if not result["OK"]:
