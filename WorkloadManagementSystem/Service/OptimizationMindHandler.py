...
def cleanTaskQueues():
    ...
    for jid in result["Value"]:
        ...
        if not result["OK"]:
            ...
            continue
        result = logDB.addLoggingRecord(
            jid, JobStatus.RECEIVED, "", "", source="JobState"
        )
        if not result["OK"]:
            gLogger.error(
                "Cannot add logging record in JobLoggingDB %s" % jid, result["Message"]
            )
            continue
    return S_OK()
...
class OptimizationMindHandler(ExecutorMindHandler):
    ...
    def msg_OptimizeJobs(self, msgObj):
        ...
        for jid in jids:
            ...
            if not result["OK"]:
                self.log.error(
                    "Could not add job %s to optimization: %s" % (jid, result["Value"])
                )
            else:
    ...
    def __loadJobs(cls, eTypes=None):
        ...
        for opState in cls.__optimizationStates:
            ...
            result = cls.__jobDB.selectJobs(
                jobCond, limit=cls.srv_getCSOption("JobQueryLimit", 10000)
            )
            if not result["OK"]:
                return result
    ...
    def exec_taskProcessed(cls, jid, jobState, eType):
        ...
        if not result["OK"]:
            cls.log.error(
                "Could not save changes for job", "%s: %s" % (jid, result["Message"])
            )
        return result
    ...
    def exec_dispatch(cls, jid, jobState, pathExecuted):
        ...
        if not result["OK"]:
            cls.log.error(
                "Could not get status for job", "%s: %s" % (jid, result["Message"])
            )
        ...
        if not result["OK"]:
            cls.log.error(
                "Could not get optimizer chain for job, auto resetting job",
                "%s: %s" % (jid, result["Message"]),
            )
            result = jobState.resetJob()
            if not result["OK"]:
                cls.log.error(
                    "Could not reset job", "%s: %s" % (jid, result["Message"])
                )
        ...
        if minorStatus not in optChain:
            cls.log.error(
                "Next optimizer is not in the chain for job",
                "%s: %s not in %s" % (jid, minorStatus, optChain),
            )
            return S_ERROR(
                "Next optimizer %s not in chain %s" % (minorStatus, optChain)
            )
    ...
    def exec_taskError(cls, jid, cachedJobState, errorMsg):
        ...
        if not result["OK"]:
            cls.log.error(
                "Cannot write changes to job %s: %s" % (jid, result["Message"])
            )
        ...
        if result["OK"]:
            ...
            cls.log.error(
                "Could not get status of job %s: %s" % (jid, result["Message "])
            )
