...
class JobMonitoringHandler(RequestHandler):
    ...
    def parseSelectors(cls, selectDict=None):
        ...
        if pilotJobRefs:
            ...
            if not selectDict.get("JobID"):
                for pilotJobRef in (
                    [pilotJobRefs]
                    if isinstance(pilotJobRefs, six.string_types)
                    else pilotJobRefs
                ):
    ...
    def export_getApplicationStates(cls, condDict=None, older=None, newer=None):
        ...
        return cls.gJobDB.getDistinctJobAttributes(
            "ApplicationStatus", condDict, older, newer
        )
    ...
    def export_getJobGroups(cls, condDict=None, older=None, cutDate=None):
        ...
        """
        return cls.gJobDB.getDistinctJobAttributes(
            "JobGroup", condDict, older, newer=cutDate
        )
    ...
    def export_getMinorStates(cls, condDict=None, older=None, newer=None):
        ...
        """
        return cls.gJobDB.getDistinctJobAttributes(
            "MinorStatus", condDict, older, newer
        )
    ...
    def export_getCounters(cls, attrList, attrDict=None, cutDate=""):
        ...
        return cls.gJobDB.getCounters(
            "Jobs", attrList, attrDict, newer=str(cutDate), timeStamp="LastUpdateTime"
        )
    ...
    def export_getCurrentJobCounters(cls, attrDict=None):
        ...
        """
        _, _, attrDict = cls.parseSelectors(attrDict)
        result = cls.gJobDB.getCounters(
            "Jobs", ["Status"], attrDict, timeStamp="LastUpdateTime"
        )
        if not result["OK"]:
            return result
        last_update = Time.dateTime() - Time.day
        resultDay = cls.gJobDB.getCounters(
            "Jobs", ["Status"], attrDict, newer=last_update, timeStamp="LastUpdateTime"
        )
        if not resultDay["OK"]:
            return resultDay
    ...
    def export_getJobPageSummaryWeb(
        self, selectDict, sortList, startItem, maxItems, selectJobs=True
    ):
        ...
        statusDict = {}
        result = self.gJobDB.getCounters(
            "Jobs",
            ["Status"],
            selectDict,
            newer=startDate,
        ...
        if selectJobs:
            ...
            if not self.globalJobsInfo:
                (
                    ...
                    _nonauthJobs,
                    _ownJobs,
                ) = self.jobPolicy.evaluateJobRights(summaryJobList, RIGHT_GET_INFO)
    ...
    @classmethod
    def export_traceJobParameters(
        cls, site, localID, parameterList, attributeList, date, until
    ):
        return cls.gJobDB.traceJobParameters(
