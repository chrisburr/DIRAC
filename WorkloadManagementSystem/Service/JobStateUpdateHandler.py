...
class JobStateUpdateHandler(RequestHandler):
    ...
    @classmethod
    def export_setJobStatus(
        cls, jobID, status="", minorStatus="", source="Unknown", datetime=None
    ):
    ...
    @classmethod
    @deprecated("unused")
    def export_setJobsStatus(
        cls, jobIDs, status="", minorStatus="", source="Unknown", datetime=None
    ):
        """Set the major and minor status for job specified by its JobId.
        Set optionally the status date and source component which sends the
        status information.
    ...
    def __setJobStatusBulk(cls, jobID, statusDict):
        ...
        if dates[-1] >= lastTime:
            ...
            result = cls.jobDB.setJobAttributes(
                jobID, attrNames, attrValues, update=True
            )
            if not result["OK"]:
                return result
    ...
    def export_setJobParameter(cls, jobID, name, value):
        ...
        if cls.elasticJobParametersDB:
            return cls.elasticJobParametersDB.setJobParameter(
                int(jobID), name, value
    ...
    def export_setJobsParameter(cls, jobsParameterDict):
        ...
        for jobID in jobsParameterDict:
            ...
            if cls.elasticJobParametersDB:
                ...
                if not res["OK"]:
                    gLogger.error(
                        "Failed to add Job Parameter to cls.elasticJobParametersDB",
                        res["Message"],
                    )
                    failed = True
                    message = res["Message"]
                ...
                if not res["OK"]:
                    gLogger.error(
                        "Failed to add Job Parameter to MySQL", res["Message"]
                    )
                    failed = True
                    message = res["Message"]
    ...
    def export_setJobParameters(cls, jobID, parameters):
        ...
        if cls.elasticJobParametersDB:
            for key, value in parameters:  # FIXME: should use a bulk method
                ...
                if not res["OK"]:
                    gLogger.error(
                        "Failed to add Job Parameters to cls.elasticJobParametersDB",
                        res["Message"],
                    )
                    failed = True
                    message = res["Message"]
            ...
            if not result["OK"]:
                gLogger.error(
                    "Failed to add Job Parameters to MySQL", result["Message"]
                )
                failed = True
    ...
    def export_sendHeartBeat(cls, jobID, dynamicData, staticData):
        ...
        if not result["OK"]:
            gLogger.warn(
                "Failed to set the heart beat data", "for job %d " % int(jobID)
            )

        if cls.elasticJobParametersDB:
            for key, value in staticData.items():
                result = cls.elasticJobParametersDB.setJobParameter(
                    int(jobID), key, value
                )
                if not result["OK"]:
                    gLogger.error(
                        "Failed to add Job Parameters to ElasticSearch",
                        result["Message"],
                    )
        else:
