...
def initializeJobManagerHandler(serviceInfo):
    ...
    enablePilotsLogging = gConfig.getValue(
        serviceInfo["serviceSectionPath"].replace("JobManager", "PilotsLogging")
        + "/Enable",
        "False",
    ).lower() in ("yes", "true")
...
class JobManagerHandler(RequestHandler):
    ...
    def initialize(self):
        ...
        self.maxParametricJobs = self.srv_getCSOption(
            "MaxParametricJobs", MAX_PARAMETRIC_JOBS
        )
    ...
    def export_submitJob(self, jobDesc):
        ...
        for (
            ...
            jobID = result["JobID"]
            self.log.info(
                'Job added to the JobDB", "%s for %s/%s'
    ...
    def export_confirmBulkSubmission(self, jobIDs):
        ...
        (
            ...
            _nonauthJobList,
            _ownerJobList,
        ) = self.jobPolicy.evaluateJobRights(jobList, RIGHT_SUBMIT)
        ...
        result = gJobDB.setJobAttributes(
            jobUpdateStatusList,
            ["Status", "MinorStatus"],
    ...
    def __checkIfProxyUploadIsRequired(self):
        ...
        :return: bool
        """
        result = gProxyManager.userHasProxy(
            self.ownerDN, self.ownerGroup, validSeconds=18000
        )
        if not result["OK"]:
            self.log.error(
                "Can't check if the user has proxy uploaded", result["Message"]
            )
            return True
        # Check if an upload is required
    ...
    def export_rescheduleJob(self, jobIDs):
        ...
        (
            ...
            nonauthJobList,
            ownerJobList,
        ) = self.jobPolicy.evaluateJobRights(jobList, RIGHT_RESCHEDULE)
        ...
        result["requireProxyUpload"] = (
            len(ownerJobList) > 0 and self.__checkIfProxyUploadIsRequired()
        )
    ...
    def __deleteJob(self, jobID):
        ...
        for pilot in result["Value"]:
            ...
            if not res[
                "Value"
            ]:  # if list of jobs for pilot is empty, delete pilot and pilotslogging
                ...
                if not ret["OK"]:
                    self.log.error(
                        "Failed to delete pilot from PilotAgentsDB", ret["Message"]
                    )
                    return ret
                if enablePilotsLogging:
    ...
    def __kill_delete_jobs(self, jobIDList, right):
        ...
        (
            ...
            nonauthJobList,
            ownerJobList,
        ) = self.jobPolicy.evaluateJobRights(jobList, right)
        ...
        if nonauthJobList or badIDs:
            ...
            if nonauthJobList:
                self.log.warn(
                    "Non-authorized JobIDs won't be deleted", str(nonauthJobList)
                )
                result["NonauthorizedJobIDs"] = nonauthJobList
            if badIDs:
    ...
    def export_resetJob(self, jobIDs):
        ...
        (
            ...
            nonauthJobList,
            ownerJobList,
        ) = self.jobPolicy.evaluateJobRights(jobList, RIGHT_RESET)
