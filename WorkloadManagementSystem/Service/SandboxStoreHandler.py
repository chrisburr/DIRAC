...
class SandboxStoreHandler(RequestHandler):
    ...
    def initialize(self):
        ...
        if SandboxStoreHandler.__purgeCount > self.getCSOption(
            "QueriesBeforePurge", 1000
        ):
    ...
    def transfer_fromClient(self, fileId, token, fileSize, fileHelper):
        ...
        if not result["OK"]:
            gLogger.error(
                "Error while receiving sandbox file", "%s" % result["Message"]
            )
            return result
        hdPath = result["Value"]
        ...
        # Register!
        gLogger.info(
            "Registering sandbox in the DB with",
            "SB:%s|%s" % (self.__seNameToUse, sbPath),
        )
        result = sandboxDB.registerAndGetSandbox(
            credDict["username"],
            credDict["DN"],
            credDict["group"],
            self.__seNameToUse,
    ...
    def transfer_bulkFromClient(self, fileId, token, _fileSize, fileHelper):
        ...
        if not result["OK"]:
            gLogger.error(
                "Could not move sandbox to final destination", result["Message"]
            )
            return result
    ...
    def __networkToFile(self, fileHelper, destFileName=False):
        ...
        try:
            ...
            gLogger.error(
                "Cannot open to write destination file",
                "%s: %s" % (destFileName, repr(e).replace(",)", ")")),
            )
    ...
    def __secureUnlinkFile(self, filePath):
        try:
            ...
            gLogger.warn(
                "Could not unlink file %s: %s" % (filePath, repr(e).replace(",)", ")"))
            )
            return False
        return True
    ...
    def __copyToExternalSE(self, localFilePath, sbPath):
        ...
        try:
            ...
            if sbPath not in okTrans:
                gLogger.verbose(
                    "Ooops, SB transfer wasn't in the successful ones", str(result)
                )
                return S_ERROR(
            ...
            gLogger.error(
                "Error while moving sandbox to SE", "%s" % repr(e).replace(",)", ")")
            )
    ...
    def export_assignSandboxesToEntities(
        self, enDict, ownerName="", ownerGroup="", entitySetup=False
    ):
        """
        ...
        credDict = self.getRemoteCredentials()
        return sandboxDB.assignSandboxesToEntities(
            enDict,
            credDict["username"],
            credDict["group"],
            entitySetup,
    ...
    def export_unassignEntities(self, entitiesList, entitiesSetup=False):
        ...
        credDict = self.getRemoteCredentials()
        return sandboxDB.unassignEntities(
    ...
    def export_getSandboxesAssignedToEntity(self, entityId, entitySetup=False):
        ...
        if not entitySetup:
            entitySetup = self.serviceInfoDict["clientSetup"]
        credDict = self.getRemoteCredentials()
        result = sandboxDB.getSandboxesAssignedToEntity(
    ...
    def transfer_toClient(self, fileID, token, fileHelper):
        ...
        result = sandboxDB.getSandboxId(
            self.__localSEName, filePath, credDict["username"], credDict["group"]
        )
        if not result["OK"]:
            return result
        sbId = result["Value"]
    ...
    def purgeUnusedSandboxes(self):
        ...
        if not result["OK"]:
            gLogger.error(
                "Error while retrieving sandboxes to purge", result["Message"]
            )
    ...
    def __deleteSandboxFromBackend(self, SEName, SEPFN):
        ...
        if SEName != self.__localSEName:
            ...
            try:
                ...
                gLogger.error(
                    "Cannot perform isfile",
                    "%s : %s" % (hdPath, repr(e).replace(",)", ")")),
                )
