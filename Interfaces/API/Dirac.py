...
class Dirac(API):
    ...
    def _checkFileArgument(self, fnList, prefix=None, single=False):
        ...
        if isinstance(fnList, six.string_types):
            ...
            if otherPrefix in fnList:
                return self._errorReport("Expected %s string, not %s") % (
                    prefix,
                    otherPrefix,
                )
            ...
            try:
                ...
                return self._errorReport(
                    str(x), "Expected strings in list of %ss" % prefix
                )
        else:
            return self._errorReport(
                "Expected single string or list of strings for %s(s)" % prefix
            )
    ...
    def _checkJobArgument(self, jobID, multiple=False):
        try:
            ...
            return self._errorReport(
                str(x),
                "Expected %sinteger or string for existing jobID" % "(list of) "
                if multiple
                else "",
            )
    ...
    def monitorRepository(self, printOutput=False):
        ...
        if not res["OK"]:
            return self._errorReport(
                res["Message"], "Failed to get status of jobs from WMS"
            )
    ...
    def retrieveRepositorySandboxes(
        self, requestedStates=None, destinationDirectory=""
    ):
    ...
    def submitJob(self, job, mode="wms"):
        ...
        if isinstance(job, six.string_types):
            ...
            if formulationErrors:
                for (
                    method,
                    errorList,
                ) in formulationErrors.items():  # can be an iterator
                    self.log.error(
                        ">>>> Error in %s() <<<<\n%s" % (method, "\n".join(errorList))
                    )
            ...
            try:
                ...
                if not result["OK"]:
                    self.log.error(
                        'Pre-submission checks failed for job with message: "%s"'
                        % (result["Message"])
                    )
                    return result
            ...
            jobDescriptionObject = StringIO.StringIO(
                job._toXML()
            )  # pylint: disable=protected-access
            jdlAsString = job._toJDL(
                jobDescriptionObject=jobDescriptionObject
            )  # pylint: disable=protected-access
        ...
        if mode.lower() == "local":
            ...
            self.log.info(
                "Executing workflow locally with full WMS submission and DIRAC Job Agent"
            )
            result = self.runLocalAgent(jdlAsString, jobDescriptionObject)

        elif mode.lower() == "wms":
            self.log.verbose(
                "Will submit job to WMS"
    ...
    def runLocalAgent(self, jdl, jobDescriptionObject):
        ...
        self.log.info(
            "The job has been submitted to the WMS with jobID = %s, monitoring starts."
            % jobID
        )
        ...
        self.log.info(
            "Job %s is now eligible to be picked up from the WMS by a local job agent"
            % jobID
        )
    ...
    def __runJobAgent(self, jobID):
        ...
        self.log.verbose(
            "In case being booted from a DIRAC script,"
        ...
        localCfg.addDefaultEntry(
            "/Resources/Computing/%s/PilotType" % ceType, "private"
        )
        localCfg.addDefaultEntry("/Resources/Computing/%s/OwnerDN" % ceType, ownerDN)
        localCfg.addDefaultEntry(
            "/Resources/Computing/%s/OwnerGroup" % ceType, ownerGroup
        )
        ...
        if not result["OK"]:
            self.log.error(
                "There were errors when loading configuration", result["Message"]
            )
        ...
        if not result["OK"]:
            self.log.error(
                "Job Agent execution completed with errors", result["Message"]
            )
    ...
    def __monitorSubmittedJob(self, jobID):
        ...
        while not finalState:
            ...
            if not jobStatus["OK"]:
                self.log.error(
                    "Could not monitor job status, will retry in %s seconds"
                    % pollingTime,
                    jobStatus["Message"],
                )
            else:
                ...
                if jobStatus.lower() == "failed":
                    finalState = True
                    return S_ERROR(
                        "Problem with job %s definition, WMS status is Failed" % jobID
                    )
                self.log.info(
                    "Current status for job %s is %s will retry in %s seconds"
                    % (jobID, jobStatus, pollingTime)
                )
            current = time.time()
            if current - start > maxWaitingTime:
                finalState = True
                return S_ERROR(
                    "Exceeded max waiting time of %s seconds for job %s to enter Waiting state,"
    ...
    def __getVOPolicyModule(module):
        ...
        if setup and vo:
            moduleName = gConfig.getValue(
                "DIRAC/VOPolicy/%s/%s/%s" % (vo, setup, module), ""
            )
            if not moduleName:
    ...
    def getInputDataCatalog(
        self, lfns, siteName="", fileName="pool_xml_catalog.xml", ignoreMissing=False
    ):
        ...
        if not inputDataPolicy:
            return self._errorReport(
                "Could not retrieve DIRAC/VOPolicy/InputDataModule for VO"
            )
        ...
        if not result["OK"]:
            if "Failed" in result:
                self.log.error(
                    "Input data resolution failed for the following files:\n",
                    "\n".join(result["Failed"]),
                )
    ...
    def runLocal(self, job):
        ...
        if inputData:
            ...
            if not localSEList:
                return self._errorReport(
                    "LocalSite/LocalSE should be defined in your config file"
                )
            ...
            self.log.info(
                "Job has input data requirement, will attempt to resolve data for %s"
                % DIRAC.siteName()
            )
            self.log.verbose(
                "\n".join(
                    inputData if isinstance(inputData, (list, tuple)) else [inputData]
                )
            )
        ...
        if softwarePolicy:
            moduleFactory = ModuleFactory()
            moduleInstance = moduleFactory.getModule(
                softwarePolicy, {"Job": parameters}
            )
            if not moduleInstance["OK"]:
            ...
            if not result["OK"]:
                self.log.warn(
                    "Software installation failed with result:\n%s" % (result)
                )
                return result
        else:
            self.log.verbose(
                "Could not retrieve DIRAC/VOPolicy/SoftwareDistModule for VO"
            )
        ...
        if sandbox:
            ...
            for isFile in sandbox:
                ...
                if os.path.exists(os.path.join(os.getcwd(), os.path.basename(isFile))):
                    self.log.debug(
                        "Input Sandbox %s found in the job directory, no need to copy it"
                        % isFile
                    )
                else:
                    if os.path.isabs(isFile) and os.path.exists(isFile):
                        self.log.debug(
                            "Input Sandbox %s is a file with absolute path, copying it"
                            % isFile
                        )
                        shutil.copy(isFile, os.getcwd())
                    elif os.path.isdir(isFile):
                        self.log.debug(
                            "Input Sandbox %s is a directory, found in the user working directory, copying it"
                            % isFile
                        )
                        ...
                        self.log.debug(
                            "Input Sandbox %s found in the submission directory, copying it"
                            % isFile
                        )
                        shutil.copy(
                            os.path.join(curDir, os.path.basename(isFile)), os.getcwd()
                        )
                    elif os.path.exists(
                        os.path.join(tmpdir, isFile)
                    ):  # if it is in the tmp dir
                        self.log.debug(
                            "Input Sandbox %s is a file, found in the tmp directory, copying it"
                            % isFile
                        )
                        shutil.copy(os.path.join(tmpdir, isFile), os.getcwd())
                    else:
                        self.log.verbose(
                            "perhaps the file %s is in an LFN, so we attempt to download it."
                            % isFile
                        )
                        getFile = self.getFile(isFile)
                        if not getFile["OK"]:
                            self.log.warn(
                                "Failed to download %s with error: %s"
                                % (isFile, getFile["Message"])
                            )
                ...
                if tarfile.is_tarfile(basefname):
                    try:
                        ...
                        return S_ERROR(
                            "Could not untar or extract %s with exception %s"
                            % (basefname, repr(x))
                        )
        ...
        if variableList:
            ...
            for var in variableList:
                nameEnv = var.split("=")[0]
                valEnv = urllib.unquote(
                    var.split("=")[1]
                )  # this is needed to make the value contain strange things
        ...
        result = systemCall(
            ...
            env=executionEnv,
            callbackFunction=self.__printOutput,
        )
        if not result["OK"]:
            return result
    ...
    def getReplicas(
        self, lfns, active=True, preferDisk=False, diskOnly=False, printOutput=False
    ):
        ...
        repsResult = dm.getReplicas(
            lfns, active=active, preferDisk=preferDisk, diskOnly=diskOnly
        )
        ...
        if printOutput:
            ...
            for lfn in repsResult["Value"]["Successful"]:
                lfnPrint = lfn
                for se, url in repsResult["Value"]["Successful"][
                    lfn
                ].items():  # can be an iterator
            ...
            for lfn in repsResult["Value"]["Failed"]:
                records.append(
                    (lfn, "Unknown", str(repsResult["Value"]["Failed"][lfn]))
                )
    ...
    def splitInputData(self, lfns, maxFilesPerJob=20, printOutput=False):
        ...
        if not replicaDict["Value"]["Successful"]:
            return self._errorReport(
                list(replicaDict["Value"]["Failed"].iteritems())[0],
    ...
    def getLfnMetadata(self, lfns, printOutput=False):
        ...
        if not fileResult["OK"]:
            self.log.warn(
                "Failed to retrieve file metadata from the catalogue",
                fileResult["Message"],
            )
            return fileResult
        ...
        if repsResult["Failed"]:
            ...
            if not dirResult["OK"]:
                self.log.warn(
                    "Failed to retrieve directory metadata from the catalogue"
                )
                self.log.warn(dirResult["Message"])
                return dirResult
            for directory in dirResult["Value"]["Successful"]:
                repsResult["Successful"][directory] = dirResult["Value"]["Successful"][
                    directory
                ]
    ...
    def addFile(self, lfn, fullPath, diracSE, fileGuid=None, printOutput=False):
        ...
        if not result["OK"]:
            return self._errorReport(
                "Problem during putAndRegister call", result["Message"]
            )
        if printOutput:
    ...
    #############################################################################
    def replicateFile(
        self, lfn, destinationSE, sourceSE="", localCache="", printOutput=False
    ):
        ...
        if not result["OK"]:
            return self._errorReport(
                "Problem during replicateFile call", result["Message"]
            )
        if printOutput:
            print(self.pPrint.pformat(result["Value"]))
        return result
    ...
    def getAccessURL(self, lfn, storageElement, printOutput=False, protocol=False):
        ...
        if not result["OK"]:
            return self._errorReport(
                "Problem during getAccessURL call", result["Message"]
            )
        if printOutput:
            print(self.pPrint.pformat(result["Value"]))
        return result
    ...
    def getPhysicalFileMetadata(self, pfn, storageElement, printOutput=False):
        ...
        if not result["OK"]:
            return self._errorReport(
                "Problem during getStorageFileMetadata call", result["Message"]
            )
        if printOutput:
            print(self.pPrint.pformat(result["Value"]))
        return result
    ...
    def getInputSandbox(self, jobID, outputDir=None):
        ...
        if outputDir:
            ...
            if os.path.exists(dirPath):
                return self._errorReport(
                    "Job input sandbox directory %s already exists" % (dirPath)
                )
        else:
        ...
        try:
            ...
            return self._errorReport(
                repr(x), "Could not create directory in %s" % (dirPath)
            )

        result = SandboxStoreClient(
            useCertificates=self.useCertificates
        ).downloadSandboxForJob(jobID, "Input", dirPath)
        if not result["OK"]:
    ...
    def getOutputSandbox(
        self, jobID, outputDir=None, oversized=True, noJobDir=False, unpack=True
    ):
        ...
        if outputDir:
            ...
            if os.path.exists(dirPath):
                return self._errorReport(
                    "Job output directory %s already exists" % (dirPath)
                )
        mkDir(dirPath)
        ...
        result = SandboxStoreClient(
            useCertificates=self.useCertificates
        ).downloadSandboxForJob(jobID, "Output", dirPath, inMemory=False, unpack=unpack)
        if result["OK"]:
            self.log.info("Files retrieved and extracted in %s" % (dirPath))
            if self.jobRepo:
                self.jobRepo.updateJob(
                    jobID, {"Retrieved": 1, "Sandbox": os.path.realpath(dirPath)}
                )
            return result
        self.log.warn(result["Message"])
        ...
        if not params["OK"]:
            self.log.verbose(
                "Could not retrieve job parameters to check for oversized sandbox"
            )
            return params

        if not params["Value"].get("OutputSandboxLFN"):
            self.log.verbose(
                "No oversized output sandbox for job %s:\n%s" % (jobID, params)
            )
            return result
        ...
        if not getFile["OK"]:
            self.log.warn(
                "Failed to download %s with error:%s"
                % (oversizedSandbox, getFile["Message"])
            )
            os.chdir(start)
            return getFile
    ...
    def getJobInputData(self, jobID):
        ...
        for job in jobID:
            ...
            if result["OK"]:
                ...
                self.log.warn(
                    "Getting input data for job %s failed with message:\n%s"
                    % (job, result["Message"])
                )
                summary[job] = []
    ...
    def getJobOutputLFNs(self, jobID):
        ...
        if not result["Value"].get("UploadedOutputData"):
            self.log.info(
                "Parameters for job %s do not contain uploaded output data:\n%s"
                % (jobID, result)
            )
        ...
        self.log.verbose(
            "Found the following output data LFNs:\n", "\n".join(outputData)
        )
    ...
    def getJobOutputData(self, jobID, outputFiles="", destinationDir=""):
        ...
        if outputFiles:
            if isinstance(outputFiles, six.string_types):
                ...
                try:
                    ...
                    return self._errorReport(
                        str(x), "Expected strings for output file names"
                    )
            else:
                return self._errorReport("Expected strings for output file names")
            self.log.info(
                "Found specific outputFiles to download:", ", ".join(outputFiles)
            )
            newOutputData = []
        ...
        if self.jobRepo:
            self.jobRepo.updateJob(
                jobID, {"OutputData": 1, "OutputFiles": obtainedFiles}
            )
        return S_OK(outputData)
    ...
    def selectJobs(
        ...
        conditions = dict(
            (key, str(value)) for key, value in options.iteritems() if value
        )
        ...
        self.log.verbose(
            "Will select jobs with last update %s and following conditions" % date
        )
        ...
        if not jobIDs:
            self.log.error(
                "No jobs selected",
                "with date '%s' for conditions: %s" % (str(date), conditions),
            )
        ...
        if outputFile:
            ...
            if re.search("/", dirPath) and not os.path.exists(dirPath):
                try:
                    ...
                    return self._errorReport(
                        str(x), "Could not create directory %s" % (dirPath)
                    )
    ...
    def getJobDebugOutput(self, jobID):
        ...
        if not result["OK"]:
            self.log.info(
                "Could not obtain status information for jobID %s, please check this is valid."
                % jobID
            )
        ...
        try:
            ...
            return self._errorReport(
                str(x), "Could not create directory in %s" % (debugDir)
            )
    ...
    def getJobCPUTime(self, jobID, printOutput=False):
        ...
        for job in jobID:
            ...
            if not result["OK"]:
                return self._errorReport(
                    result["Message"], "Could not get heartbeat data for job %s" % job
                )
            if result["Value"]:
    ...
    def pingService(self, system, service, printOutput=False, url=None):
        ...
        if (
            not isinstance(system, basestring)
            and isinstance(service, basestring)
            and not isinstance(url, six.string_types)
        ):
            return self._errorReport(
                "Expected string for system and service or a url to ping()"
            )
        result = S_ERROR()
        try:
            if not url:
                ...
                self.log.verbose(
                    "Requested service should have CS path: %s" % (section)
                )
            ...
            self.log.warn(
                "ping for %s/%s failed with exception:\n%s" % (system, service, str(x))
            )
    ...
    def __printInfo(self):
        ...
        self.log.verbose(
            "DIRAC is running at %s in setup %s" % (DIRAC.siteName(), self.setup)
        )
