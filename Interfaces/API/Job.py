...
class Job(API):
    ...
    def __init__(self, script=None, stdout="std.out", stderr="std.err"):
        ...
        res = ObjectLoader().loadObject(
            "ConfigurationSystem.Client.Helpers.Resources", "getDIRACPlatforms"
        )
        if not res["OK"]:
    ...
    def setExecutable(
        ...
        if (
            not isinstance(executable, basestring)
        ...
        step = getStepDefinition(
            "ScriptStep%s" % (self.stepCount), modulesList, parametersList=parameters
        )
    ...
    def setName(self, jobName):
        ...
        if not isinstance(jobName, six.string_types):
            ...
            self._addParameter(
                self.workflow, "JobName", "JDL", jobName, "User specified name"
            )
    ...
    def setInputSandbox(self, files):
        ...
        if isinstance(files, list) and files:
            ...
            self._addParameter(
                self.workflow, "InputSandbox", "JDL", fileList, description
            )
            ...
            kwargs = {"files": files}
            return self._reportError(
                "Expected file string or list of files for input sandbox contents",
                **kwargs
            )
    ...
    def setOutputSandbox(self, files):
        ...
        if isinstance(files, list) and files:
            ...
            self._addParameter(
                self.workflow, "OutputSandbox", "JDL", fileList, description
            )
        elif isinstance(files, six.string_types):
            description = "Output sandbox file"
            self._addParameter(
                self.workflow, "OutputSandbox", "JDL", files, description
            )
        else:
            kwargs = {"files": files}
            return self._reportError(
    ...
    def setInputData(self, lfns):
        ...
        if isinstance(lfns, list) and lfns:
            ...
            self._addParameter(
                self.workflow, "InputData", "JDL", inputDataStr, description
            )
    ...
    def setParameterSequence(self, name, parameterList, addToWorkflow=False):
        ...
        if self.numberOfParameters == 0:
            ...
            return self._reportError(
                "Parameter sequences of different length", name="setParameterSequence"
            )
    ...
    def setInputDataPolicy(self, policy, dataScheduling=True):
        ...
        if not finalPolicy:
            return self._reportError(
                "Expected one of %s for input data policy" % (", ".join(possible)),
                __name__,
                **kwargs
            )
        ...
        if not jobPolicy:
            return self._reportError(
                "Could not get value for Operations option %s/%s"
                % (csSection, finalPolicy),
                __name__,
                **kwargs
            )
        ...
        self._addParameter(
            self.workflow, "InputDataPolicy", "JDL", jobPolicy, description
        )

        if not dataScheduling and policy.lower() == "download":
            self.log.verbose(
                "Scheduling by input data is disabled, jobs will run anywhere and download input data"
            )
            self._addParameter(
                self.workflow,
                "DisableDataScheduling",
                "JDL",
                "True",
                "Disable scheduling by input data",
            )

        if not dataScheduling and policy.lower() != "download":
            self.log.error(
                'Expected policy to be "download" for bypassing data scheduling'
            )
            return self._reportError(
                'Expected policy to be "download" for bypassing data scheduling',
                __name__,
    ...
    def setOutputData(self, lfns, outputSE=None, outputPath=""):
        ...
        if isinstance(lfns, list) and lfns:
            ...
            self._addParameter(
                self.workflow, "OutputData", "JDL", outputDataStr, description
            )
        ...
        if outputPath:
            ...
            self._addParameter(
                self.workflow, "OutputPath", "JDL", outputPath, description
            )
    ...
    def setSubmitPool(self, backend):
        ...
        if not backend.lower() == "any":
            self._addParameter(
                self.workflow, "SubmitPools", "JDL", backend, "Submit Pool"
            )
        ...
        if not isinstance(timeInSecs, int):
            try:
                ...
                if not re.search("{{", timeInSecs):
                    return self._reportError(
                        "Expected numerical string or int for CPU time in seconds",
                        **kwargs
                    )
    ...
    def setDestination(self, destination):
        ...
        if isinstance(destination, list):
            ...
            if sites:
                ...
                if not result["OK"]:
                    return self._reportError(
                        "%s is not a valid destination site" % (destination), **kwargs
                    )
            destSites = ";".join(destination)
            self._addParameter(self.workflow, "Site", "JDL", destSites, description)
        else:
            return self._reportError(
                "Invalid destination site, expected string or list", **kwargs
            )
        return S_OK()

    #############################################################################
    def setNumberOfProcessors(
        self,
        numberOfProcessors=None,
        minNumberOfProcessors=None,
        maxNumberOfProcessors=None,
    ):
        """Helper function.
    ...
    def __checkSiteIsValid(self, site):
        ...
        if isinstance(site, (list, set, dict)):
            ...
            return S_OK()
        return self._reportError(
            "Specified site %s is not in list of defined sites" % str(site)
        )
    ...
    def setDestinationCE(self, ceName, diracSite=None):
        ...
        if not diracSite:
            ...
            if not res["Value"]:
                return self._reportError(
                    "No DIRAC site name found for CE %s" % (ceName), **kwargs
                )
    ...
    def setBannedSites(self, sites):
        ...
        if isinstance(sites, list) and sites:
            ...
            self._addParameter(
                self.workflow, "BannedSites", "JDL", bannedSites, description
            )
    ...
    def setOwner(self, ownerProvided):
        ...
        if not isinstance(ownerProvided, six.string_types):
            return self._reportError(
                "Expected string for owner", **{"ownerProvided": ownerProvided}
            )

        self._addParameter(
            self.workflow, "Owner", "JDL", ownerProvided, "User specified ID"
        )
        return S_OK()
    ...
    def setOwnerGroup(self, ownerGroup):
        ...
        if not isinstance(ownerGroup, six.string_types):
            return self._reportError(
                "Expected string for job owner group", **{"ownerGroup": ownerGroup}
            )
    ...
    def setOwnerDN(self, ownerDN):
        ...
        if not isinstance(ownerDN, six.string_types):
            return self._reportError(
                "Expected string for job owner DN", **{"ownerGroup": ownerDN}
            )

        self._addParameter(
            self.workflow, "OwnerDN", "JDL", ownerDN, "User specified owner DN."
        )
        return S_OK()
    ...
    def setType(self, jobType):
        ...
        if not isinstance(jobType, six.string_types):
            return self._reportError(
                "Expected string for job type", **{"jobType": jobType}
            )

        self._addParameter(
            self.workflow, "JobType", "JDL", jobType, "User specified type"
        )
        self.type = jobType
        return S_OK()
    ...
    def setTag(self, tags):
        ...
        self._addParameter(
            self.workflow, "Tags", "JDL", tagValue, "User specified job tags"
        )
        return S_OK()
    ...
    def setJobGroup(self, jobGroup):
        ...
        if not isinstance(jobGroup, six.string_types):
            return self._reportError(
                "Expected string for job group name", **{"jobGroup": jobGroup}
            )
    ...
    def setLogLevel(self, logLevel):
        ...
        if isinstance(logLevel, six.string_types):
            if logLevel.upper() in gLogger.getAllPossibleLevels():
                ...
                self._addParameter(
                    self.workflow, "LogLevel", "JDL", logLevel, description
                )
            else:
                return self._reportError(
                    'Error Level "%s" not valid' % logLevel, **kwargs
                )
        else:
    ...
    def setConfigArgs(self, cfgString):
        ...
        if not isinstance(cfgString, six.string_types):
            return self._reportError(
                "Expected string for DIRAC Job Config Args", **{"cfgString": cfgString}
            )

        description = "User specified cfg settings"
        self._addParameter(
            self.workflow, "JobConfigArgs", "JDL", cfgString, description
        )
        return S_OK()
    ...
    def setExecutionEnv(self, environmentDict):
        ...
        if not isinstance(environmentDict, dict):
            return self._reportError(
                "Expected dictionary of environment variables", **kwargs
            )
        ...
        if environmentDict:
            ...
            for var, val in environmentDict.iteritems():
                try:
                    ...
                    return self._reportError(
                        "Expected string for environment variable key value pairs",
                        **kwargs
                    )
            ...
            self._addParameter(
                self.workflow, "ExecutionEnvironment", "JDL", envStr, description
            )
        return S_OK()
    ...
    def __setJobDefaults(self):
        ...
        self._addParameter(
            self.workflow, "Priority", "JDL", self.priority, "User Job Priority"
        )
        self._addParameter(
            self.workflow, "JobGroup", "JDL", self.group, "Name of the JobGroup"
        )
        ...
        self._addParameter(
            self.workflow, "Origin", "JDL", self.origin, "Origin of client"
        )
        self._addParameter(
            self.workflow, "StdOutput", "JDL", self.stdout, "Standard output file"
        )
        self._addParameter(
            self.workflow, "StdError", "JDL", self.stderr, "Standard error file"
        )
        self._addParameter(
            self.workflow, "InputData", "JDL", "", "Default null input data value"
        )
        self._addParameter(
            self.workflow, "LogLevel", "JDL", self.logLevel, "Job Logging Level"
        )
        self._addParameter(
            self.workflow, "arguments", "string", "", "Arguments to executable Step"
        )
                ...
                if not output["OK"]:
                    ...
                    for check in files:
                        if os.path.isfile(check):
                            self.log.verbose(
                                "Found file " + check + " appending to Input Sandbox"
                            )
                            resolvedIS.append(check)
                        if os.path.isdir(check):
                            if re.search(
                                "/$", check
                            ):  # users can specify e.g. /my/dir/lib/
                                check = check[:-1]
                            tarName = os.path.basename(check)
                            directory = os.path.dirname(
                                check
                            )  # if just the directory this is null
                            if directory:
                                cmd = (
                                    "tar cfz "
                                    + tarName
    ...
    def _toJDL(self, xmlFile="", jobDescriptionObject=None):
        ...
        if "DIRACSetup" in paramsDict:
            if paramsDict["DIRACSetup"]["value"]:
                arguments.append(
                    "-o DIRAC/Setup=%s" % (paramsDict["DIRACSetup"]["value"])
                )
            else:
        ...
        if self.addToInputSandbox:
            ...
            if "InputSandbox" in paramsDict:
                ...
                self.log.verbose(
                    "Final unique Input Sandbox %s" % (";".join(uniqueInputSandbox))
                )
            else:
        ...
        if self.addToOutputSandbox:
            ...
            if "OutputSandbox" in paramsDict:
                ...
                self.log.verbose(
                    "Final unique Output Sandbox %s" % (";".join(uniqueOutputSandbox))
                )
            else:
        ...
        if self.addToInputData:
            ...
            if "InputData" in paramsDict:
                ...
                self.log.verbose(
                    "Final unique Input Data %s" % (";".join(uniqueInputData))
                )
            else:
        ...
        if self.numberOfParameters > 0:
            paramsDict, arguments = self._handleParameterSequences(
                paramsDict, arguments
            )
    ...
    def _addJDLParameter(self, name, value):
        ...
        self._addParameter(
            self.workflow, name, "JDL", value, "Optional JDL parameter added"
        )
        return self.workflow.setValue(name, value)
