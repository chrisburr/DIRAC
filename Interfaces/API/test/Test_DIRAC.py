...
def test_runLocal(dirac, job, mocker, osmock, confMock):
    ...
    mocker.patch(
        "DIRAC.Interfaces.API.Dirac.tarfile", new=MagicMock(return_value=False)
    )
    ...
    shMock = mocker.patch(
        "DIRAC.Interfaces.API.Dirac.shutil", new=MagicMock(name="Shutil")
    )
    ...
    assert ret["OK"]
    assert (
        call("/abspath/absfile.xml", "/pwd/tempFolder/") in shMock.copy.call_args_list
    )
    assert (
        call("/pwd/file_in_pwd.xml", "/pwd/tempFolder/") in shMock.copy.call_args_list
    )
