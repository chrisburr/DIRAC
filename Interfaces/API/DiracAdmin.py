...
class DiracAdmin(API):
    ...
    def getSiteSection(self, site, printOutput=False):
        ...
        if not gConfig.getSections("/Resources/Sites/%s" % (gridType))["OK"]:
            return S_ERROR(
                "/Resources/Sites/%s is not a valid site section" % (gridType)
            )
    ...
    def getSiteMaskLogging(self, site=None, printOutput=False):
        ...
        if self.rssFlag:
            result = ResourceStatusClient().selectStatusElement(
                "Site", "History", name=site
            )
        else:
    ...
    def getServicePorts(self, setup="", printOutput=False):
        ...
        if setup not in setupList:
            return S_ERROR(
                "Setup %s is not in allowed list: %s" % (setup, ", ".join(setupList))
            )
    ...
    def getProxy(self, userDN, userGroup, validity=43200, limited=False):
        ...
        return gProxyManager.downloadProxy(
            userDN, userGroup, limited=limited, requiredTimeLeft=validity
        )

    #############################################################################
    def getVOMSProxy(
        self, userDN, userGroup, vomsAttr=False, validity=43200, limited=False
    ):
        ...
        if isinstance(jobID, six.string_types):
            try:
                ...
                return self._errorReport(
                    str(x), "Expected integer or convertible integer for existing jobID"
                )
    ...
    def setSiteProtocols(self, site, protocolsList, printOutput=False):
        ...
        if not siteSEs:
            return S_ERROR(
                "No SEs found for site %s in section %s" % (site, siteSection)
            )
        ...
        for se in siteSEs:
            ...
            for section in sections["Value"]:
                if (
                    gConfig.getValue(
                        "/Resources/StorageElements/%s/%s/ProtocolName" % (se, section),
                        "",
                    )
                    == "SRM2"
                ):
                    ...
                    self.log.verbose(
                        "Setting %s to %s" % (path, ", ".join(protocolsList))
                    )
        ...
        if modifiedCS:
            ...
            if not result["OK"]:
                return S_ERROR(
                    "CS Commit failed with message = %s" % (result["Message"])
                )
            else:
                if printOutput:
    ...
    #############################################################################
    def sendMail(
        self, address, subject, body, fromAddress=None, localAttempt=True, html=False
    ):
        """
        ...
        return notification.sendMail(
            address, subject, body, fromAddress, localAttempt, html
        )
