...
Script.registerSwitch(
    "D:", "HostDN:", "DN of the Host Certificate (Mandatory)", setHostDN
)
Script.registerSwitch(
    "P:",
    "Property:",
...
for prop in args:
    ...
    if len(pl) < 2:
        errorList.append(
            (
                "in arguments",
                "Property %s has to include a '=' to separate name from value" % prop,
            )
        )
        exitCode = 255
    else:
...
if exitCode == 0:
    ...
    if not ret["OK"]:
        Script.gLogger.error(
            "Cannot check if host is registered in ComponentMonitoring", ret["Message"]
        )
    elif ret["Value"]:
        ...
        if not ret["OK"]:
            Script.gLogger.error(
                "Failed to add Host to ComponentMonitoring", ret["Message"]
            )
