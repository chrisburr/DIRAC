...
class MonitoringDB(ElasticDB):
    ...
    def __init__(self, name="Monitoring/MonitoringDB", readOnly=False):
        ...
        indexPrefix = gConfig.getValue(
            "%s/IndexPrefix" % section, CSGlobals.getSetup()
        ).lower()
        ...
        for pythonClassName in sorted(objectsLoaded):
            ...
            if self.__readonly:
                ...
                if self.exists("%s-*" % indexName):
                    ...
                    result = self.createIndex(
                        indexName, self.documentTypes[monitoringType]["mapping"], period
                    )
                    if not result["OK"]:
                        self.log.error(result["Message"])
                        raise RuntimeError(result["Message"])
    ...
    def getKeyValues(self, monitoringType):
        ...
        try:
            properties = docs[monitoringType][
                "properties"
            ]  # "old" way, with ES types == Monitoring types
        except KeyError:
            try:
                properties = docs["_doc"][
                    "properties"
                ]  # "ES6" way, with ES types == '_doc'
            except KeyError:
    ...
    def retrieveBucketedData(
        ...
        q = [
            self._Q("range", timestamp={"lte": endTime * 1000, "gte": startTime * 1000})
        ]
            ...
            if isAvgAgg:
                ...
                for k in bucket.end_data.buckets:
                    if (k.key / 1000) not in result[bucket.key]:
                        result[bucket.key][
                            k.key / 1000
                        ] = k.timeAggregation_avg_bucket.value
                    else:
                        result[bucket.key][k.key / 1000].append(
                            k.timeAggregation_avg_bucket.value
                        )
    ...
    def retrieveAggregatedData(
        ...
        s.aggs.bucket(
            "end_data", "date_histogram", field="timestamp", interval=interval
        ).metric("tt", a1)
    ...
    def put(self, records, monitoringType):
        ...
        return self.bulk_index(
            indexPrefix=indexName, data=records, mapping=mapping, period=period
        )
        ...
        if hits and "hits" in hits and hits["hits"]:
            ...
            for resObj in hits["hits"]:
                records.append(
                    dict(
                        [
                            (paramName, getattr(resObj["_source"], paramName))
    ...
    def getDataForAGivenPeriod(self, typeName, condDict, initialDate="", endDate=""):
        ...
        if initialDate:
            condDict["startTime"] = calendar.timegm(
                time.strptime(initialDate, "%d/%m/%Y %H:%M")
            )
        else:
            condDict["startTime"] = 0000000000
        if endDate:
            condDict["endTime"] = calendar.timegm(
                time.strptime(endDate, "%d/%m/%Y %H:%M")
            )
        else:
