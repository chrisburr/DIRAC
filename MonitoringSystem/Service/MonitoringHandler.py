...
class MonitoringHandler(RequestHandler):
    ...
    def initializeHandler(cls, serviceInfo):
        ...
        dataPath = gConfig.getValue(
            "%s/DataLocation" % reportSection, "data/monitoringPlots"
        )
    ...
    def __generatePlotFromFileId(self, fileId):
        ...
        gLogger.info(
            "Returning %s file: %s " % (fileToReturn, result["Value"][fileToReturn])
        )
    ...
    def __checkPlotRequest(self, reportRequest):
        ...
        if not isinstance(
            reportRequest["extraArgs"], self.__reportRequestDict["extraArgs"]
        ):
            return S_ERROR(
                "Extra args has to be of type %s"
    ...
    def export_getDataForAGivenPeriod(
        self, typeName, condDict, initialDate="", endDate=""
    ):
        """
        ...
        """
        return self.__db.getDataForAGivenPeriod(
            typeName, condDict, initialDate, endDate
        )
