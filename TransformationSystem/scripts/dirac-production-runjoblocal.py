...
Script.registerSwitch(
    "D:", "Download=", "Defines data acquisition as DownloadInputData"
)
Script.registerSwitch(
    "P:", "Protocol=", "Defines data acquisition as InputDataByProtocol"
)
...
def __modifyJobDescription(jobID, basepath, downloadinputdata):
    ...
    if not downloadinputdata:
        ...
        archive = et.parse(
            basepath + "InputSandbox" + str(jobID) + os.path.sep + "jobDescription.xml"
        )
        for element in archive.getiterator():
            if (
                element.text
                == "DIRAC.WorkloadManagementSystem.Client.DownloadInputData"
            ):
                element.text = (
                    "DIRAC.WorkloadManagementSystem.Client.InputDataByProtocol"
                )
                archive.write(
                    basepath
                    + "InputSandbox"
                ...
                S_OK(
                    "Job parameter changed from DownloadInputData to InputDataByProtocol."
                )
...
def __runJobLocally(jobID, basepath, vo):
    ...
    """
    ipr = __import__(
        str(vo) + "DIRAC.Interfaces.API." + str(vo) + "Job",
        globals(),
        locals(),
        [str(vo) + "Job"],
        -1,
