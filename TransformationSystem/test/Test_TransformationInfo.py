...
def userProxyFixture(mocker):
    ...
    mocker.patch(
        "DIRAC.TransformationSystem.Utilities.TransformationInfo.UserProxy",
        new=_mockedCM,
    )
...
def tiFixture():
    ...
    fcMock = Mock(
        name="fcMock", spec=DIRAC.Resources.Catalog.FileCatalogClient.FileCatalogClient
    )
    jmMock = Mock(
        name="jobMonMock",
    ...
    jsucMock = Mock(
        name="jsuc",
        spec=DIRAC.WorkloadManagementSystem.Client.JobStateUpdateClient.JobStateUpdateClient,
    )
    transInfoDict = dict(
...
def test_setJob_Status(tiFixture):
    ...
    tiFixture.setJobDone(job)
    tiFixture._TransformationInfo__setTaskStatus.assert_called_once_with(job, "Done")
    tiFixture._TransformationInfo__updateJobStatus.assert_called_once_with(
        job.jobID, "Done", "Job forced to Done"
    )
    ...
    tiFixture._TransformationInfo__setTaskStatus.assert_called_once_with(job, "Failed")
    tiFixture._TransformationInfo__updateJobStatus.assert_called_once_with(
        job.jobID, "Failed", "Job forced to Failed"
    )
...
def test_setInputStatusFuncs(tiFixture):
    ...
    tiFixture._TransformationInfo__setInputStatus.assert_called_once_with(
        job, "Processed"
    )
...
def test_setInputStatus(tiFixture):
    ...
    tiFixture.enabled = True
    tiFixture.tClient.setFileStatusForTransformation.return_value = S_ERROR(
        "Failed to set"
    )
    with pytest.raises(RuntimeError) as re:
        tiFixture._TransformationInfo__setInputStatus(job, status)
...
def test_setTaskStatus(tiFixture):
    ...
    tiFixture.tClient.setTaskStatus.assert_called_once_with(
        tiFixture.transName, 1234, "Processed"
    )
...
def test_updateJobStatus(tiFixture):
    ...
    tiFixture.enabled = False
    res = tiFixture._TransformationInfo__updateJobStatus(
        1234, "Failed", minorstatus=None
    )
    assert res["OK"]
    ...
    tiFixture.jobStateClient.setJobStatus.assert_called_once_with(
        1234, "Failed", None, "DataRecoveryAgent"
    )

    tiFixture.jobStateClient.setJobStatus.return_value = S_ERROR(
        "Error setting job status"
    )
    tiFixture.enabled = True
    with pytest.raises(RuntimeError) as re:
...
def test_findAllDescendants(tiFixture):
    ...
    tiFixture.fcClient.getFileDescendents = Mock(
        return_value=S_ERROR("Cannot get descendants")
    )
    descList = tiFixture._TransformationInfo__findAllDescendants(lfnList=[])
    assert descList == []
...
def test_cleanOutputs(tiFixture, userProxyFixture):
    ...
    tiFixture.cleanOutputs(jobInfo)
    tiFixture.log.notice.assert_any_call(
        MatchStringWith("Would have removed these files")
    )
    ...
    tiFixture._TransformationInfo__findAllDescendants = Mock(return_value=descList)
    with patch(
        "DIRAC.TransformationSystem.Utilities.TransformationInfo.DataManager",
    ...
    with patch(
        ...
        tiFixture.cleanOutputs(jobInfo)
        tiFixture.log.notice.assert_any_call(
            MatchStringWith("Successfully removed 2 files")
        )
    ...
    tiFixture.cleanOutputs(jobInfo)
    tiFixture._TransformationInfo__findAllDescendants.assert_called_once_with(
        jobInfo.outputFiles
    )
...
def test_getJobs(tiFixture):
    ...
    tiFixture.jobMon.getJobs.side_effect = (
        S_OK([123, 456, 789]),
        S_OK([1123, 1456, 1789]),
    )
