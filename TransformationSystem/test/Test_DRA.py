...
class TestDRA(unittest.TestCase):
    ...
    def setUp(self):
        self.dra = DataRecoveryAgent(
            agentName="ILCTransformationSystem/DataRecoveryAgent", loadName="TestDRA"
        )
        self.dra.transNoInput = ["MCGeneration"]
        self.dra.transWithInput = ["MCSimulation", "MCReconstruction"]
        self.dra.transformationTypes = [
            "MCGeneration",
            "MCSimulation",
            "MCReconstruction",
        ]
        ...
        self.dra.reqClient = Mock(
            name="reqMock",
            spec=DIRAC.RequestManagementSystem.Client.ReqClient.ReqClient,
        )
    ...
    def test_beginExecution(self):
        ...
        with patch(
            "DIRAC.TransformationSystem.Agent.DataRecoveryAgent.Operations",
            return_value=theOps,
        ):
            res = self.dra.beginExecution()
        assert isinstance(self.dra.transformationTypes, list)
        assert set(["MCGeneration", "MCReconstruction", "Merge"]) == set(
            self.dra.transformationTypes
        )
    ...
    def test_getEligibleTransformations_success(self):
        ...
        res = self.dra.getEligibleTransformations(
            status="Active", typeList=["TestProds"]
        )
        self.assertTrue(res["OK"])
    ...
    def test_treatTransformation3(self):
        ...
        with patch(
            "%s.TransformationInfo" % MODULE_NAME,
            autospec=True,
            return_value=getJobMock,
        ):
            self.dra.treatTransformation(
                transID=1234, transInfoDict=transInfoDict
            )  # returns None
        self.dra.log.notice.assert_called_with(
            MatchStringWith("Skipping transformation 1234")
        )

    def test_checkJob(self):
        """test for DataRecoveryAgent checkJob No inputFiles............................................."""

        from DIRAC.TransformationSystem.Utilities.TransformationInfo import (
            TransformationInfo,
        )
    ...
    @parameterized.expand(
        [
            ...
            param(
                3,
                ["cleanOutputs", "setJobFailed", "setInputDeleted"],
                ifStat=["Missing"],
            ),
            param(
                4,
                ["cleanOutputs", "setJobFailed"],
                tFiStat=["Deleted"],
                ifStat=["Missing"],
            ),
            param(
    ...
    def test_failHard(
        self, infoCalls, jID=666, jStat="Done", inFiles=None, ofStat=["Exists"]
    ):
        ...
        self.dra._DataRecoveryAgent__failJobHard(
            testJob, tInfoMock
    ...
    def test_notOnlyKeepers(self):
        ...
        funcToTest = (
            self.dra._DataRecoveryAgent__notOnlyKeepers
        )  # pylint: disable=protected-access, no-member
    ...
    def test_checkAllJob(self):
        ...
        self.dra.log.error.assert_any_call(
            MatchStringWith("+++++ Exception"), "ARGJob1"
        )
        self.dra.log.error.assert_any_call(
    ...
    def test_checkAllJob_2(self):
        ...
        self.dra._DataRecoveryAgent__failJobHard = Mock(
            side_effect=(RuntimeError("ARGJob4"), None), name="FJH"
        )
        ...
        self.dra.log.error.assert_any_call(
            MatchStringWith("+++++ Exception"), "ARGJob4"
        )
        self.dra.log.reset_mock()
    ...
    def test_execute(self):
        ...
        self.dra.getEligibleTransformations = Mock(
            return_value=S_OK({123: d123, 124: d124, 125: d125})
        )
        res = self.dra.execute()
        self.assertTrue(res["OK"])
    ...
    def test_setPendingRequests_Fail(self):
        ...
        reqClient.readRequestsForJobs.side_effect = (
            S_ERROR("Failure"),
            S_OK({"Successful": {}}),
        )
        self.dra.reqClient = reqClient
