...
class JobInfo(object):
    ...
    def __init__(self, jobID, status, tID, tType=None):
        ...
        self.maxResetCounter = Operations().getValue(
    ...
    def __str__(self):
        ...
        if self.outputFiles:
            ...
            efInfo = [
                "%s (%s)" % _
                for _ in izip_longest(self.outputFiles, self.outputFileStatus)
            ]
            info += ", ".join(efInfo)
    ...
    def allFilesExist(self):
        ...
        return bool(self.outputFileStatus) and all(
            "Exists" in status for status in self.outputFileStatus
        )

    def allFilesMissing(self):
        """Check if all files are missing, if not output files, return False"""
        return bool(self.outputFileStatus) and all(
            "Missing" in status for status in self.outputFileStatus
        )

    def someFilesMissing(self):
        """check if some files are missing and therefore some files exist"""
        return bool(self.outputFileStatus) and not (
            self.allFilesExist() or self.allFilesMissing()
        )
    ...
    def allFilesProcessed(self):
        ...
        return bool(self.transFileStatus) and all(
            "Processed" in status for status in self.transFileStatus
        )

    def allFilesAssigned(self):
        """Check if all input files are assigned."""
        return bool(self.transFileStatus) and all(
            status in ASSIGNEDSTATES for status in self.transFileStatus
        )

    def allTransFilesDeleted(self):
        """Check if all input files are deleted in the Transformation System."""
        return bool(self.transFileStatus) and all(
            status == "Deleted" for status in self.transFileStatus
        )
    ...
    def getJobInformation(self, diracAPI, jobMon, jdlOnly=False):
        ...
        * ProductionOutputData: from jobMonitor getJobParameter ProductionOutputData
        ...
        if not jdlOnly:
            ...
            if resOutput["OK"]:
                self.outputFiles = fromChar(
                    resOutput["Value"].get("ProductionOutputData", "")
                )
    ...
    def getTaskInfo(self, tasksDict, lfnTaskDict, withInputTypes):
        ...
        for inputLFN in self.inputFiles:
            try:
                ...
                LOG.error(
                    "Failed to get taskDict",
                    "%s, %s: %s" % (self.taskID, self.inputFiles, pformat(lfnTaskDict)),
                )
                raise
