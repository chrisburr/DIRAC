...
class TransformationInfo(object):
    ...
    def __init__(
        self, transformationID, transInfoDict, enabled, tClient, fcClient, jobMon
    ):
        """Store clients etc."""
    ...
    def checkTasksStatus(self):
        ...
        if not res["OK"]:
            raise RuntimeError(
                "Failed to get transformation tasks: %s" % res["Message"]
            )
        ...
        for task in res["Value"]:
            ...
            tasksDict[taskID].append(
                dict(FileID=fileID, LFN=lfn, Status=status, ErrorCount=errorCount)
            )
    ...
    def __updateJobStatus(self, jobID, status, minorstatus=""):
        ...
        if self.enabled:
            ...
            result = self.jobStateClient.setJobStatus(
                jobID, status, minorstatus, source
            )
        else:
    ...
    def cleanOutputs(self, jobInfo):
        ...
        existingOutputFiles = [
            lfn
            for lfn, status in izip_longest(
                jobInfo.outputFiles, jobInfo.outputFileStatus
            )
            if status == "Exists"
        ]
        ...
        if not self.enabled:
            self.log.notice(
                "Would have removed these files: \n +++ %s "
                % "\n +++ ".join(filesToDelete)
            )
            return
        self.log.notice(
        ...
        for lfnList in breakListIntoChunks(filesToDelete, 200):
            with UserProxy(
                proxyUserDN=self.authorDN, proxyUserGroup=self.authorGroup
            ) as proxyResult:
                if not proxyResult["OK"]:
                    raise RuntimeError(
                        "Failed to get a proxy: %s" % proxyResult["Message"]
                    )
        ...
        for reason, lfns in errorReasons.items():
            self.log.error(
                "Failed to remove %d files with error: %s" % (len(lfns), reason)
            )
