...
class Params(object):
    ...
    def registerSwitches(self, script):
        ...
        script.registerSwitch(
            "G:",
            "GroupSize=",
            "Number of Files per transformation task",
            self.setGroupSize,
        )
        script.registerSwitch(
            "R:", "GroupName=", "TransformationGroup Name", self.setGroupName
        )
        script.registerSwitch(
            "S:",
            "SourceSEs=",
            "SourceSE(s) to use, comma separated list",
            self.setSourceSE,
        )
        script.registerSwitch(
            "N:",
            "Extraname=",
            "String to append to transformation name",
            self.setExtraname,
        )
        script.registerSwitch(
            "P:", "Plugin=", "Plugin to use for transformation", self.setPlugin
        )
        script.registerSwitch(
            "T:",
            "Flavour=",
            "Flavour to create: Replication or Moving",
            self.setTransFlavour,
        )
        script.registerSwitch(
            "K:", "MetaKey=", "Meta Key to use: TransformationID", self.setMetaKey
        )
        script.registerSwitch(
            "M:",
            "MetaData=",
            "MetaData to use Key/Value Pairs: 'DataType:REC,'",
            self.setMetadata,
        )
        script.registerSwitch(
            "x",
            "Enable",
            "Enable the transformation creation, otherwise dry-run",
            self.setEnable,
        )
    ...
    def checkSettings(self, script, checkArguments=True):
        ...
        if self.metaKey is None:
            self.metaKey = Operations().getValue(
                "Transformations/TransfIDMeta", "TransformationID"
            )
