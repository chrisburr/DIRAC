...
class TransformationAgent(AgentModule, TransformationAgentsUtilities):
    ...
    def initialize(self):
        ...
        self.maxFiles = self.am_getOption(
            "MaxFilesToProcess", self.am_getOption("MaxFiles", 5000)
        )
        ...
        self.log.info(
            "Will treat the following transformation types: %s"
    ...
    def execute(self):
                ...
                if not res["OK"]:
                    ...
                    if movedFiles:
                        ...
                        for status, val in movedFiles.iteritems():
                            self._logInfo(
                                "\t%d files to status %s" % (val, status),
                                transID=transID,
                            )
        ...
        self._logInfo(
            "Out of %d transformations, %d put in thread queue"
            % (len(res["Value"]), count)
        )
        return S_OK()
    ...
    def getTransformations(self):
        ...
        if transName == "All":
            ...
            self._logInfo(
                "Obtained %d transformations to process" % len(transformations),
                method=method,
            )
        else:
            ...
            if not res["OK"]:
                self._logError(
                    "Failed to get transformation:", res["Message"], method=method
                )
                return res
            transformations = [res["Value"]]
    ...
    def _execute(self, threadID):
        ...
        while True:
            ...
            try:
                ...
                self._logInfo(
                    "Processing transformation %s." % transID, transID=transID
                )
                startTime = time.time()
                ...
                if not transID:
                    transID = "None"
                self._logInfo(
                    "Processed transformation in %.1f seconds"
                    % (time.time() - startTime),
                    transID=transID,
                )
                ...
                self._logVerbose(
                    "%d transformations still in queue" % len(self.transInQueue)
                )
        return S_OK()
    ...
    def processTransformation(self, transDict, clients):
        ...
        if not forJobs:
            ...
            if len(lfnsToProcess) != totLfns:
                self._logInfo(
                    "Reduced number of files from %d to %d"
        ...
        self._logInfo(
            "Processing transformation with '%s' plug-in." % plugin,
            method=method,
            transID=transID,
        )
        ...
        if not res["OK"]:
            self._logError(
                "Failed to generate tasks for transformation:",
                res["Message"],
                method=method,
                transID=transID,
            )
        ...
        for se, lfns in tasks:
            ...
            if not res["OK"]:
                self._logError(
                    "Failed to add task generated by plug-in:",
                    res["Message"],
                    method=method,
                    transID=transID,
                )
        ...
        if created:
            self._logInfo(
                "Successfully created %d tasks for transformation." % created,
                method=method,
                transID=transID,
            )
        else:
            self._logInfo(
                "No new tasks created for transformation.",
                method=method,
                transID=transID,
            )
        ...
        if transDict["Status"] == "Flush" and allCreated:
            ...
            if not res["OK"]:
                ...
                self._logInfo(
                    "Updated transformation status to 'Active'.",
                    method=method,
                    transID=transID,
                )
        return S_OK()
    ...
    def _getTransformationFiles(
        self, transDict, clients, statusList=None, replicateOrRemove=False
    ):
        ...
        noUnusedDelay = (
            ...
            else operations.getValue(
                "TransformationPlugins/%s/NoUnusedDelay" % plugin, self.noUnusedDelay
            )
        )
        ...
        if not transFiles:
            self._logInfo(
                "No '%s' files found for transformation." % ",".join(statusList),
                method=method,
                transID=transID,
            )
        ...
        kickFile = os.path.join(
            self.controlDirectory, "KickTransformation_%s" % str(transID)
        )
        try:
        ...
        if not kickTrans and skipIfNoNewUnused and noUnusedDelay:
            nextStamp = self.unusedTimeStamp.setdefault(
                transID, now
            ) + datetime.timedelta(hours=noUnusedDelay)
            skip = now < nextStamp
            if (
                len(transFiles) == self.unusedFiles.get(transID, 0)
                and transDict["Status"] != "Flush"
                and skip
            ):
                self._logInfo(
                    "No new '%s' files found for transformation."
                    % ",".join(statusList),
                    method=method,
                    transID=transID,
                )
        ...
        notUnused = [
            trFile["LFN"] for trFile in transFiles if trFile["Status"] != "Unused"
        ]
        ...
        if notUnused:
            res = transClient.setFileStatusForTransformation(
                transID, "Unused", notUnused, force=True
            )
            if not res["OK"]:
                self._logError(
                    "Error setting %d files Unused:" % len(notUnused),
                    res["Message"],
                    method=method,
                    transID=transID,
                )
    ...
    def __getDataReplicas(self, transDict, lfns, clients, forJobs=True):
        ...
        clearCacheFile = os.path.join(
            self.controlDirectory, "ClearCache_%s" % str(transID)
        )
        try:
        ...
        nLfns = len(lfns)
        self._logVerbose(
            "Getting replicas for %d files" % nLfns, method=method, transID=transID
        )
        ...
        self._logInfo(
            "Number of cached replicas: %d" % len(cachedReplicas),
            method=method,
            transID=transID,
        )
        ...
        self._logInfo(
            "ReplicaCache hit for %d out of %d LFNs" % (len(dataReplicas), nLfns),
            method=method,
            transID=transID,
        )
        if newLFNs:
            startTime = time.time()
            self._logInfo(
                "Getting replicas for %d files from catalog" % len(newLFNs),
                method=method,
                transID=transID,
            )
            newReplicas = {}
            for chunk in breakListIntoChunks(newLFNs, 10000):
                ...
                if res["OK"]:
                    reps = dict(
                        (lfn, ses) for lfn, ses in res["Value"].iteritems() if ses
                    )
            ...
            self._logInfo(
                "Obtained %d replicas from catalog in %.1f seconds"
    ...
    def _getDataReplicasDM(
        self, transID, lfns, clients, forJobs=True, ignoreMissing=False
    ):
        ...
        startTime = time.time()
        self._logVerbose(
            "Getting replicas%s from catalog for %d files"
        ...
        problematicLfns = [
            lfn
            for lfn in lfns
            if lfn not in replicas["Successful"] and lfn not in replicas["Failed"]
        ]
        if problematicLfns:
            self._logInfo(
            ...
            if not res["OK"]:
                self._logError(
                    "Failed to update status of problematic files:",
                    res["Message"],
                    method=method,
                    transID=transID,
                )
        ...
        for lfn, replicaDict in replicas["Successful"].iteritems():
            for se in replicaDict:
                ...
                if forJobs and "failover" in se.lower():
                    self._logVerbose(
                        "Ignoring failover replica for %s." % lfn,
                        method=method,
                        transID=transID,
                    )
                else:
        ...
        for lfn, reason in replicas["Failed"].iteritems():
            if "No such file or directory" in reason:
                self._logVerbose(
                    "%s not found in the catalog." % lfn, method=method, transID=transID
                )
        ...
        if missingLfns:
            ...
            if ignoreMissing:
                ...
                if not res["OK"]:
                    self._logError(
                        "Failed to update status of missing files:",
                        res["Message"],
                        method=method,
                        transID=transID,
                    )
    ...
    def __updateCache(self, transID, newReplicas):
        ...
        self.replicaCache.setdefault(transID, {})[
    ...
    def __cleanReplicas(self, transID, lfns):
        ...
        if toRemove:
            self._logInfo(
                "Remove %d files from cache" % len(toRemove),
    ...
    def __removeFilesFromCache(self, transID, lfns):
        ...
        if removed:
            self._logInfo(
                "Removed %d replicas from cache" % removed,
    ...
    def __readCache(self, transID):
        ...
        try:
            ...
            self._logException(
                "Failed to load replica cache from file %s" % fileName,
                lException=x,
                method=method,
                transID=transID,
            )
    ...
    def __writeCache(self, transID=None):
        ...
        try:
            ...
            self._logException(
                "Could not write replica cache file %s" % cacheFile,
                lException=x,
                method=method,
                transID=t_id,
            )
    ...
    def __generatePluginObject(self, plugin, clients):
        ...
        try:
            plugModule = __import__(
                self.pluginLocation, globals(), locals(), ["TransformationPlugin"]
            )
        except ImportError as e:
            self._logException(
                "Failed to import 'TransformationPlugin' %s" % plugin,
                lException=e,
    ...
    def pluginCallback(self, transID, invalidateCache=False):
        ...
        if invalidateCache:
            try:
                if transID in self.replicaCache:
                    self._logInfo(
                        "Removed cached replicas for transformation",
