...
class MCExtensionAgent(AgentModule):
    ...
    def initialize(self):
        ...
        gLogger.info(
            "Will create a maximum of %s tasks per iteration" % self.maxIterationTasks
        )
        gLogger.info(
    ...
    def extendTransformation(self, transID, maxTasks):
        ...
        if not res["OK"]:
            if res["Message"] != "No records found":
                gLogger.error(
                    "Failed to get task statistics", "%s %s" % (transID, res["Message"])
                )
                return res
            else:
                statusDict = {}
        ...
        if not res["OK"]:
            gLogger.error(
                "Failed to extend transformation", "%s %s" % (transID, res["Message"])
            )
            return res
        gLogger.info(
            "Successfully extended transformation %d by %d tasks"
