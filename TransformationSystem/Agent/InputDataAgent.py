...
class InputDataAgent(AgentModule):
    ...
    def initialize(self):
        ...
        gMonitor.registerActivity(
            "Iteration", "Agent Loops", AGENT_NAME, "Loops/min", gMonitor.OP_SUM
        )
    ...
    def execute(self):
        ...
        for transDict in result["Value"]:
            ...
            if self.refreshonly:
                ...
                if transID in self.timeLog:
                    if transID in self.fullTimeLog:
                        ...
                        if (
                            datetime.datetime.utcnow() - self.fullTimeLog[transID]
                        ) < datetime.timedelta(seconds=self.fullUpdatePeriod):
            ...
            self.log.verbose(
                "Using input data query for transformation",
            ...
            if lfnList:
                self.log.verbose(
                    "Processing lfns for transformation:",
                    "%d -> %d" % (transID, len(lfnList)),
                )
                # Add the files to the transformation
                self.log.verbose(
                    "Adding lfns for transformation:",
                    "%d -> %d" % (transID, len(lfnList)),
                )
