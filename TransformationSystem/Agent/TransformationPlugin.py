...
class TransformationPlugin(PluginBase):
    ...
    def __init__(self, plugin, transClient=None, dataManager=None, fc=None):
        ...
        self.util = PluginUtilities(
            plugin, transClient=transClient, dataManager=dataManager, fc=fc
        )

    def __del__(self):
        """Destructor: print the elapsed time"""
        self.util.logInfo(
            "Execution finished, timing: %.3f seconds" % (time.time() - self.startTime)
        )
    ...
    def _ByShare(self, shareType="CPU"):
        ...
        if existingCount:
            ...
            for se in sorted(normalisedExistingCount):
                self.util.logInfo(
                    "%s: %.1f" % (se.ljust(15), normalisedExistingCount[se])
                )
        ...
        for replicaSE, lfns in replicaGroups:
            ...
            res = self._getNextSite(
                existingCount, cpuShares, candidates=self._getSitesForSEs(possibleSEs)
            )
            if not res["OK"]:
    ...
    def _getShares(self, shareType, normalise=False):
        ...
        if normalise:
            shares = self.util._normaliseShares(
                shares
            )  # pylint: disable=protected-access
        if not shares:
    ...
    def _getNextSite(self, existingCount, targetShares, candidates=None):
        ...
        existingShares = self.util._normaliseShares(
            existingCount
        )  # pylint: disable=protected-access
