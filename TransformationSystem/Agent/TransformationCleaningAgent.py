...
class TransformationCleaningAgent(AgentModule):
    ...
    def initialize(self):
        ...
        if agentTSTypes:
            ...
            self.transformationTypes = sorted(
                self.dataProcTTypes + self.dataManipTTypes
            )
        self.log.info(
            "Will consider the following transformation types: %s"
        ...
        self.log.info(
            "Will use %s as metadata tag name for TransformationID" % self.transfidmeta
        )
        # # archive periof in days
        self.archiveAfter = self.am_getOption("ArchiveAfter", self.archiveAfter)  # days
        self.log.info(
            "Will archive Completed transformations after %d days" % self.archiveAfter
        )
        # # transformation log SEs
    ...
    def execute(self):
        ...
        res = self.transClient.getTransformations(
            {"Status": "Cleaning", "Type": self.transformationTypes}
        )
        if res["OK"]:
        ...
        res = self.transClient.getTransformations(
            {"Status": "Completed", "Type": self.transformationTypes},
            older=olderThanTime,
    ...
    def finalize(self):
        ...
        res = self.jobMonitoringClient.getJobGroups(
            None, datetime.utcnow() - timedelta(days=365)
        )
        if not res["OK"]:
    ...
    def _executeClean(self, transDict):
        ...
        if transDict["Type"] in self.dataManipTTypes:
            ...
            if not res["OK"]:
                self.log.error(
                    "Problems archiving transformation %s: %s"
            ...
            if not res["OK"]:
                self.log.error(
                    "Problems cleaning transformation %s: %s"
                    % (transDict["TransformationID"], res["Message"])
                )
    ...
    def _executeRemoval(self, transDict):
        ...
        if not res["OK"]:
            self.log.error(
                "Problems removing transformation %s: %s"
                % (transDict["TransformationID"], res["Message"])
            )
    ...
    def getTransformationDirectories(self, transID):
        ...
        """
        self.log.verbose(
            "Cleaning Transformation directories of transformation %d" % transID
        )
        directories = []
        if "TransformationDB" in self.directoryLocations:
            ...
            if not res["OK"]:
                self.log.error(
                    "Failed to obtain transformation directories", res["Message"]
                )
                return res
            transDirectories = []
            if res["Value"]:
        ...
        if "MetadataCatalog" in self.directoryLocations:
            ...
            if not res["OK"]:
                self.log.error(
                    "Failed to obtain metadata catalog directories", res["Message"]
                )
                return res
            transDirectories = res["Value"]
            directories = self._addDirs(transID, transDirectories, directories)
    ...
    def cleanContent(self, directory):
        ...
        if not filesFound:
            self.log.info(
                "No files are registered in the catalog directory %s" % directory
            )
            return S_OK()
        self.log.info(
        ...
        for lfn, reason in res["Value"]["Failed"].items():
            if "File does not exist" in str(reason):
                ...
                self.log.error(
                    "Failed to remove file found in the catalog",
                    "%s %s" % (lfn, reason),
                )
                realFailure = True
        if realFailure:
    ...
    def __getCatalogDirectoryContents(self, directories):
        ...
        while activeDirs:
            ...
            if (
                not res["OK"] and "Directory does not exist" in res["Message"]
            ):  # FIXME: DFC should return errno
                ...
                if "No such file or directory" in res["Message"]:
                    ...
                    self.log.error(
                        "Failed to get directory %s content: %s"
    ...
    def cleanTransformationLogFiles(self, directory):
        ...
        res = returnSingleResult(
            StorageElement(self.logSE).removeDirectory(directory, recursive=True)
        )
        if not res["OK"]:
    ...
    def archiveTransformation(self, transID):
        ...
        if not res["OK"]:
            self.log.error(
                "Failed to update status of transformation %s to Archived" % (transID),
                res["Message"],
            )
            return res
    ...
    def cleanTransformation(self, transID):
        ...
        if not res["OK"]:
            self.log.error(
                "Failed to update status of transformation %s to Cleaned" % (transID),
                res["Message"],
            )
            return res
    ...
    def cleanMetadataCatalogFiles(self, transID):
        ...
        for lfn, reason in res["Value"]["Failed"].items():
            self.log.error(
                "Failed to remove file found in metadata catalog",
                "%s %s" % (lfn, reason),
            )
        if res["Value"]["Failed"]:
    ...
    def __removeWMSTasks(self, transJobIDs):
        ...
        for jobList in breakListIntoChunks(jobIDs, 500):
            ...
            if res["OK"]:
                ...
                ("InvalidJobIDs" in res)
                and ("NonauthorizedJobIDs" not in res)
                and ("FailedJobIDs" not in res)
            ):
                self.log.info(
                    "Found %s jobs which did not exist in the WMS"
        ...
        for jobID, requestID in res["Value"]["Successful"].items():
            ...
            if not res["OK"]:
                self.log.error(
                    "Failed to remove request from RequestDB", res["Message"]
                )
                failed += 1
            else:
                self.log.verbose(
                    "Removed request %s associated to job %d." % (requestID, jobID)
                )

        if failed:
            self.log.info(
                "Successfully removed %s requests" % (len(failoverRequests) - failed)
            )
