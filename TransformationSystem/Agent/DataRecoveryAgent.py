...
class DataRecoveryAgent(AgentModule):
        ...
        self.todo = {
            ...
            "InputFiles": [  # must always be first!
                ...
                dict(
                    ...
                    and not job.allTransFilesDeleted(),
                    Actions=lambda job, tInfo: [
                        job.cleanOutputs(tInfo),
                        job.setJobFailed(tInfo),
    ...
    def __getCSOptions(self):
        ...
        extendableTTypes = set(
            ops.getValue("Transformations/ExtendableTransfTypes", ["MCSimulation"])
        )
        dataProcessing = set(ops.getValue("Transformations/DataProcessing", []))
        self.transNoInput = self.am_getOption(
            "TransformationsNoInput", list(extendableTTypes)
        )
        self.transWithInput = self.am_getOption(
            "TransformationsWithInput", list(dataProcessing - extendableTTypes)
        )
        self.transformationTypes = self.transWithInput + self.transNoInput
        self.log.notice(
            "Will treat transformations without input files", self.transNoInput
        )
        self.log.notice(
            "Will treat transformations with input files", self.transWithInput
        )
    ...
    def execute(self):
        ...
        self.log.notice(
            "Will ignore the following transformations: %s"
    ...
    def getEligibleTransformations(self, status, typeList):
        ...
        res = self.tClient.getTransformations(
            condDict={"Status": status, "Type": typeList}
        )
        if not res["OK"]:
            return res
        transformations = {}
    ...
    def checkJob(self, job, tInfo):
        ...
        checks = (
            ...
            if job.tType in self.transNoInput
            else self.todo["InputFiles"]
        )
        for do in checks:
    ...
    def getLFNStatus(self, jobs):
        ...
        for counter, job in enumerate(jobs.values()):
            ...
            while True:
                try:
                    job.getJobInformation(
                        self.diracAPI, self.jobMon, jdlOnly=self.getJobInfoFromJDLOnly
                    )
        ...
        self.log.notice(
            "Getting JobInfo Done: %3.1fs (%3.3fs per job)"
        ...
        for lfnChunk in breakListIntoChunks(list(lfnCache), 200):
            ...
            while True:
                try:
                    ...
                    pass
        self.log.notice(
            "Getting FileInfo Done: %3.1fs" % (float(time.time() - fileInfoStart))
        )
                ...
                for job in jobChunk:
                    if job.jobID == jobID:
                        ...
                        self.log.notice(
                            ...
                            % ("pending" if job.pendingRequest else "finished", jobID)
                        )
                        break
