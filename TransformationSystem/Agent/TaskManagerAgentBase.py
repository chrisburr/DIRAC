...
class TaskManagerAgentBase(AgentModule, TransformationAgentsUtilities):
    ...
    def initialize(self):
        ...
        self.bulkSubmissionFlag = self.am_getOption(
            "BulkSubmission", self.bulkSubmissionFlag
        )
    ...
    def execute(self):
        ...
        if not enableTaskMonitor:
            ...
            if not transformations["OK"]:
                self.log.warn(
                    "Could not select transformations:", transformations["Message"]
                )
            else:
    ...
    def _execute(self, threadID):
        ...
        clients = (
            ...
            else self._getClients(
                ownerGroup=self.credTuple[1], ownerDN=self.credTuple[2]
            )
            if self.credentials
            else None
        )
        ...
        while True:
            ...
            try:
                ...
                if transID not in self.transInQueue:
                    self._logWarn(
                        "Got a transf not in transInQueue...?",
                        method=method,
                        transID=transID,
                    )
                    break
                if not (self.credentials or self.shifterProxy):
                    ownerDN, group = (
                        transIDOPBody[transID]["OwnerDN"],
                        transIDOPBody[transID]["OwnerGroup"],
                    )
                ...
                self._logInfo(
                    "Start processing transformation", method=method, transID=transID
                )
                clients["TaskManager"].transInThread = self.transInThread
                for operation in operations:
                    self._logInfo(
                        "Executing %s" % operation, method=method, transID=transID
                    )
                    startOperation = time.time()
                    res = getattr(self, operation)(transIDOPBody, clients)
                    if not res["OK"]:
                        self._logError(
                            "Failed to %s: %s" % (operation, res["Message"]),
                            method=method,
                            transID=transID,
                        )
                    self._logInfo(
                        "Executed %s in %.1f seconds"
                ...
                self._logException(
                    "Exception executing operation %s" % operation,
                    lException=x,
                    method=method,
                    transID=transID,
                )
                ...
                self._logVerbose(
                    "%d transformations still in queue" % (len(self.transInThread)),
                    method=method,
                    transID=transID,
                )
                ...
                self._logDebug(
                    "transInQueue = ", self.transInQueue, method=method, transID=transID
                )
    ...
    def updateTaskStatus(self, transIDOPBody, clients):
        ...
        if chunkSize:
            self._logVerbose(
                "Getting %d tasks status (chunks of %d)"
            ...
            self._logVerbose(
                "Getting %d tasks status" % len(transformationTasks["Value"]),
                method=method,
                transID=transID,
            )
        updated = {}
        for nb, taskChunk in enumerate(
            ...
            submittedTaskStatus = clients["TaskManager"].getSubmittedTaskStatus(
                taskChunk
            )
            if not submittedTaskStatus["OK"]:
                self._logError(
                    "Failed to get updated task states:",
            ...
            if not statusDict:
                self._logVerbose(
                    "%4d: No tasks to update" % nb, method=method, transID=transID
                )

            # Set status for tasks that changes
            for status, taskIDs in statusDict.iteritems():
                self._logVerbose(
                    "%4d: Updating %d task(s) to %s" % (nb, len(taskIDs), status),
                    method=method,
                    transID=transID,
                )
    ...
    def updateFileStatus(self, transIDOPBody, clients):
        ...
        chunkSize = 100
        self._logVerbose(
            "Getting file status for %d tasks (chunks of %d)"
        ...
        for nb, taskIDs in enumerate(breakListIntoChunks(taskFiles, chunkSize)):
            ...
            submittedFileStatus = clients["TaskManager"].getSubmittedFileStatus(
                fileChunk
            )
            ...
            if not statusDict:
                self._logVerbose(
                    "%4d: No file states to be updated" % nb,
                    method=method,
                    transID=transID,
                )
                continue
            ...
            if not commit["OK"]:
                ...
                self._logVerbose(
                    "%4d: Updated the states of %d files" % (nb, len(commit["Value"])),
                    method=method,
                    transID=transID,
                )
        ...
        for status, nb in updated.iteritems():
            self._logInfo(
                "Updated %d files to status %s" % (nb, status),
                method=method,
                transID=transID,
            )
        return S_OK()
    ...
    def checkReservedTasks(self, transIDOPBody, clients):
        ...
        self._logDebug(
            "getTransformationTasks(%s) return value:" % condDict,
            res,
            method=method,
            transID=transID,
        )
        ...
        self._logDebug(
            "updateTransformationReservedTasks(%s) return value:" % reservedTasks,
            res,
            method=method,
            transID=transID,
        )
        if not res["OK"]:
            self._logError(
                "Failed to update transformation reserved tasks:",
                res["Message"],
                method=method,
                transID=transID,
            )
        ...
        for taskName, extTaskID in taskNameIDs.items():
            transID, taskID = self._parseTaskName(taskName)
            self._logInfo(
                "Setting status of %s to Submitted with ID %s" % (taskName, extTaskID),
                method=method,
                transID=transID,
            )
            setTaskStatusAndWmsID = clients[
                "TransformationClient"
            ].setTaskStatusAndWmsID(transID, taskID, "Submitted", str(extTaskID))
    ...
    def submitTasks(self, transIDOPBody, clients):
        ...
        if not tasks:
            self._logVerbose(
                "No tasks found for submission", method=method, transID=transID
            )
            return tasksToSubmit
        self._logInfo(
            "Obtained %d tasks for submission" % len(tasks),
            method=method,
            transID=transID,
        )
        ...
        chunkSize = (
            self.maxParametricJobs if self.bulkSubmissionFlag else self.tasksPerLoop
        )
        for taskDictChunk in breakDictionaryIntoChunks(tasks, chunkSize):
            ...
            if not res["OK"]:
                return res
            self._logVerbose(
                "Submitted %d jobs, bulkSubmissionFlag = %s"
                % (len(taskDictChunk), self.bulkSubmissionFlag)
            )

        return S_OK()

    def _prepareAndSubmitAndUpdateTasks(
        self, transID, transBody, tasks, owner, ownerDN, ownerGroup, clients
    ):
        ...
        self._logDebug(
            "prepareTransformationTasks return value:",
        ...
        self._logDebug(
            "submitTransformationTasks return value:",
            res,
            method=method,
            transID=transID,
        )
        if not res["OK"]:
            self._logError(
                "Failed to submit prepared tasks:",
                res["Message"],
                method=method,
                transID=transID,
            )
        ...
        self._logDebug(
            "updateDBAfterTaskSubmission return value:",
            res,
            method=method,
            transID=transID,
        )
        if not res["OK"]:
            self._logError(
                "Failed to update DB after task submission:",
                res["Message"],
                method=method,
                transID=transID,
            )
