...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            tasks,
            {"OK": True, "Value": {"Running": [1, 2], "Done": [3]}},
            True,
        ),  # tasks, to update, no errors
    ],
)
def test_updateTaskStatusSuccess(
    mocker, tcMockReturnValue, tmMockGetSubmittedTaskStatusReturnvalue, expected
):
    mocker.patch(
        "DIRAC.TransformationSystem.Agent.TaskManagerAgentBase.AgentModule",
        side_effect=mockAM,
    ...
    tm_mock.getSubmittedTaskStatus.return_value = (
...
def test_updateFileStatusSuccess(
    ...
    tmab = TaskManagerAgentBase()
    tc_mock.getTransformationFiles.return_value = (
        tcMockGetTransformationFilesReturnValue
    )
    tm_mock.getSubmittedFileStatus.return_value = (
        tmMockGetSubmittedFileStatusReturnValue
    )
...
def test_checkReservedTasks(
    ...
    tmab = TaskManagerAgentBase()
    tc_mock.getTransformationTasks.return_value = (
        tcMockGetTransformationTasksReturnValue
    )
    tm_mock.updateTransformationReservedTasks.return_value = (
        tmMockUpdateTransformationReservedTasksReturnValue
    )
...
def test_submitTasks(
    ...
    tm_mock.prepareTransformationTasks.return_value = (
        tmMockPrepareTransformationTasksReturnValue
    )
    tm_mock.submitTransformationTasks.return_value = (
        tmMockSubmitTransformationTasksReturnValue
    )
    tm_mock.updateDBAfterTaskSubmission.return_value = (
        tmMockUpdateDBAfterTaskSubmissionReturnValue
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"TransformationID": 123, "Status": "Stopped", "Type": "Replication"},
            goodFiles,
            True,
        ),
        (
            {"TransformationID": 123, "Status": "Stopped", "Type": "Removal"},
            noFiles,
            True,
        ),
    ],
