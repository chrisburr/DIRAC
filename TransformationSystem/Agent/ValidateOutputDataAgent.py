...
class ValidateOutputDataAgent(AgentModule):
    ...
    def execute(self):
        ...
        res = self.transClient.getTransformations(
            {"Status": "ValidatingOutput", "Type": self.transformationTypes}
        )
        if not res["OK"]:
            gLogger.error(
                "Failed to get ValidatingOutput transformations", res["Message"]
            )
            return res
        ...
        if not transDicts:
            ...
            return S_OK()
        gLogger.info(
            "Found %s transformations in ValidatingOutput status" % len(transDicts)
        )
    ...
    def updateWaitingIntegrity(self):
        ...
        if not res["OK"]:
            gLogger.error(
                "Failed to get WaitingIntegrity transformations", res["Message"]
            )
            return res
        transDicts = res["Value"]
        if not transDicts:
    ...
    def finalizeCheck(self, transID):
        ...
        if newStatus:
            ...
            if not res["OK"]:
                ...
                gLogger.info(
                    "Updated status of transformation %s to %s" % (transID, newStatus)
                )
        gLogger.info("-" * 40)
        return S_OK()
