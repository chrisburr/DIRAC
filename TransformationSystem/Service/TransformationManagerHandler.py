...
class TransformationManagerHandler(RequestHandler):
    ...
    @staticmethod
    def export_getCounters(
        table, attrList, condDict, older=None, newer=None, timeStamp=None
    ):
        return database.getCounters(
    ...
    def export_addTransformation(
        ...
        res = database.addTransformation(
            transName,
            description,
            longDescription,
            authorDN,
    ...
    def export_setTransformationParameter(self, transName, paramName, paramValue):
        ...
        authorDN = credDict.get("DN", credDict.get("CN"))
        # authorDN = self._clientTransport.peerCredentials['DN']
        return database.setTransformationParameter(
            transName, paramName, paramValue, author=authorDN
        )
    ...
    def export_getTransformations(
        ...
        if not condDict:
            condDict = {}
        return database.getTransformations(
            condDict=condDict,
            older=older,
            newer=newer,
            timeStamp=timeStamp,
    ...
    def export_setFileStatusForTransformation(transName, dictOfNewFilesStatus):
        ...
        return database.setFileStatusForTransformation(
            transID, newStatusForFileIDs, connection=connection
        )
    ...
    def export_getTransformationFilesCount(transName, field, selection={}):
        return database.getTransformationFilesCount(
            transName, field, selection=selection
        )
    ...
    def export_getTransformationFiles(
        ...
        if not condDict:
            condDict = {}
        return database.getTransformationFiles(
            condDict=condDict,
            older=older,
            newer=newer,
            timeStamp=timeStamp,
    ...
    def export_getTransformationTasks(
        ...
        if not condDict:
            condDict = {}
        return database.getTransformationTasks(
            condDict=condDict,
            older=older,
            newer=newer,
            timeStamp=timeStamp,
    ...
    def export_getTasksToSubmit(self, transName, numTasks, site=""):
        ...
        transDict = res["Value"]
        submitDict = {}
        res = database.getTasksForSubmission(
    ...
    def export_createTransformationMetaQuery(self, transName, queryDict, queryType):
        credDict = self.getRemoteCredentials()
        authorDN = credDict.get("DN", credDict.get("CN"))
        return database.createTransformationMetaQuery(
            transName, queryDict, queryType, author=authorDN
        )
    ...
    def export_deleteTransformationMetaQuery(self, transName, queryType):
        credDict = self.getRemoteCredentials()
        authorDN = credDict.get("DN", credDict.get("CN"))
        return database.deleteTransformationMetaQuery(
            transName, queryType, author=authorDN
        )
    ...
    def export_getDistinctAttributeValues(self, attribute, selectDict):
        res = database.getTableDistinctAttributeValues(
            "Transformations", [attribute], selectDict
        )
        if not res["OK"]:
            return res
    ...
    def export_getTransformationSummary(self):
        ...
        for transDict in transList:
            ...
            if not res["OK"]:
                self.log.warn(
                    "Failed to get job statistics for transformation", transID
                )
                continue
    ...
    def export_getTabbedSummaryWeb(
        self, table, requestedTables, selectDict, sortList, startItem, maxItems
    ):
        tableDestinations = {
            "Transformations": {
        ...
        if not res["OK"]:
            self.log.error(
                "Failed to get Summary for table", "%s %s" % (table, res["Message"])
            )
            return res
    ...
    def export_getTransformationsSummaryWeb(
        self, selectDict, sortList, startItem, maxItems
    ):
    ...
    def export_getTransformationTasksSummaryWeb(
        self, selectDict, sortList, startItem, maxItems
    ):
        return self.__getTableSummaryWeb(
            "TransformationTasks",
    ...
    def export_getTransformationFilesSummaryWeb(
        self, selectDict, sortList, startItem, maxItems
    ):
        return self.__getTableSummaryWeb(
            "TransformationFiles",
    ...
    def __getTableSummaryWeb(
        ...
        if not fcn:
            return S_ERROR(
                "Unable to invoke database.%s, it isn't a member function of database"
                % fcnName
            )
        res = fcn(
            condDict=selectDict,
    ...
    def export_getTransformationSummaryWeb(
        self, selectDict, sortList, startItem, maxItems
    ):
        ...
        taskStateNames = TASKS_STATE_NAMES + ops.getValue(
            "Transformations/AdditionalTaskStates", []
        )
        resultDict["ParameterNames"] += ["Jobs_" + x for x in taskStateNames]
        # Add the file states to the ParameterNames entry
        fileStateNames = FILES_STATE_NAMES + ops.getValue(
            "Transformations/AdditionalFileStates", []
        )
        ...
        statusDict = {}
        extendableTranfs = ops.getValue(
            "Transformations/ExtendableTransfTypes", ["Simulation", "MCsimulation"]
        )
        givenUpFileStatus = ops.getValue(
            "Transformations/GivenUpFileStatus", ["MissingInFC"]
        )
        problematicStatuses = ops.getValue(
            "Transformations/ProblematicStatuses", ["Problematic"]
        )
        ...
        for trans in transList:
            ...
            if transType.lower() in extendableTranfs:
                ...
                if res["OK"]:
                    ...
                    fileDict["PercentProcessed"] = (
                        "%.1f" % (int(processed * 1000.0 / total) / 10.0)
                        if total
                        else 0.0
                    )
            problematic = 0
