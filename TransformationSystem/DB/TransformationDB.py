...
class TransformationDB(DB):
    ...
    def __init__(self, dbname=None, dbconfig=None, dbIn=None):
        ...
        res = self._query(
            "SELECT Engine FROM INFORMATION_SCHEMA.TABLES WHERE table_name = 'TransformationTasks'"
        )
        if not res["OK"]:
    ...
    def addTransformation(
        ...
        if res["OK"]:
            return S_ERROR(
                "Transformation with name %s already exists with TransformationID = %d"
                % (transName, res["Value"])
            )
        ...
        if inputMetaQuery:
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to add input meta query to the transformation",
                    res["Message"],
                )
        ...
        if outputMetaQuery:
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to add output meta query to the transformation",
                    res["Message"],
                )
                return self.deleteTransformation(transID, connection=connection)
        ...
        if inheritedFrom:
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to get ID for parent transformation, now deleting",
                    res["Message"],
                )
                return self.deleteTransformation(transID, connection=connection)
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to update parent transformation status: now deleting",
                    res["Message"],
                )
                return self.deleteTransformation(transID, connection=connection)
            res = self.setTransformationParameter(
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to update parent transformation agent type, now deleting",
                    res["Message"],
                )
                return self.deleteTransformation(transID, connection=connection)
            message = "Creation of the derived transformation (%d)" % transID
            self.__updateTransformationLogging(
                originalID, message, authorDN, connection=connection
            )
            res = self.getTransformationFiles(
                condDict={"TransformationID": originalID}, connection=connection
            )
            if not res["OK"]:
                gLogger.error(
                    "Could not get transformation files, now deleting", res["Message"]
                )
                return self.deleteTransformation(transID, connection=connection)
            if res["Records"]:
                res = self.__insertExistingTransformationFiles(
                    transID, res["Records"], connection=connection
                )
                if not res["OK"]:
                    gLogger.error(
                        "Could not insert files, now deleting", res["Message"]
                    )
                    return self.deleteTransformation(transID, connection=connection)
        ...
        if addFiles and inputMetaQuery:
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to find files to be added to the transformation",
                    res["Message"],
                )
                return res
            ...
            if filesToAdd:
                ...
                res = self.__addFilesToTransformation(
                    transID, fileIDs, connection=connection
                )
                if not res["OK"]:
                    gLogger.error(
                        "Failed to add files to transformation",
                        "%s %s" % (transID, res["Message"]),
                    )
        message = "Created transformation %d" % transID

        self.__updateTransformationLogging(
            transID, message, authorDN, connection=connection
        )
        return S_OK(transID)
    ...
    def getTransformations(
        ...
        req = "SELECT %s FROM Transformations %s" % (
            ...
            self.buildCondition(
                condDict, older, newer, timeStamp, orderAttribute, limit, offset=offset
            ),
        )
        ...
        for row in res["Value"]:
            ...
            rList = [
                str(item) if not isinstance(item, six.integer_types) else item
                for item in row
            ]
            transDict = dict(zip(self.TRANSPARAMS, row))
            webList.append(rList)
            if extraParams:
                res = self.__getAdditionalParameters(
                    transDict["TransformationID"], connection=connection
                )
                if not res["OK"]:
                    return res
    ...
    def getTransformationParameters(self, transName, parameters, connection=False):
        ...
        res = self.getTransformation(
            transName, extraParams=extraParams, connection=connection
        )
        if not res["OK"]:
            return res
        ...
        for reqParam in parameters:
            if reqParam not in transParams:
                return S_ERROR(
                    "Parameter %s not defined for transformation %s"
                    % (reqParam, transName)
                )
    ...
    def getTransformationWithStatus(self, status, connection=False):
        ...
        req = (
            "SELECT TransformationID FROM Transformations WHERE Status = '%s';" % status
        )
    ...
    def __getTableDistinctAttributeValues(
        ...
        for attribute in attributes:
            if possible and (attribute not in possible):
                return S_ERROR(
                    "Requested attribute (%s) does not exist in table %s"
                    % (attribute, table)
                )
            ...
            if not res["OK"]:
                return S_ERROR(
                    "Failed to serve values for attribute %s in table %s"
                    % (attribute, table)
                )
            attributeValues[attribute] = res["Value"]
        return S_OK(attributeValues)

    def __updateTransformationParameter(
        self, transID, paramName, paramValue, connection=False
    ):
        if paramName not in self.mutable:
            return S_ERROR(
                "Can not update the '%s' transformation parameter" % paramName
            )
        if paramName == "Body":
            ...
            req = (
                "UPDATE Transformations SET %s=%s, LastUpdate=UTC_TIMESTAMP() WHERE TransformationID=%d"
                % (paramName, paramValue, transID)
            )
    ...
    def _getTransformationID(self, transName, connection=False):
        ...
        try:
            ...
            cmd = (
                "SELECT TransformationID from Transformations WHERE TransformationID=%d;"
                % transName
            )
        except ValueError:
            ...
            cmd = (
                "SELECT TransformationID from Transformations WHERE TransformationName='%s';"
                % transName
            )
        res = self._query(cmd, connection)
        if not res["OK"]:
            gLogger.error(
                "Failed to obtain transformation ID for transformation",
                "%s: %s" % (transName, res["Message"]),
            )
            return res
        elif not res["Value"]:
    ...
    def __updateFilterQueries(self, connection=False):
        ...
        resultList = []
        res = self.getTransformations(
            condDict={"Status": ["New", "Active", "Stopped", "Flush", "Completing"]},
            connection=connection,
        )
        if not res["OK"]:
            return res
    ...
    #
    def setTransformationParameter(
        self, transName, paramName, paramValue, author="", connection=False
    ):
        ...
        if paramName in self.TRANSPARAMS:
            res = self.__updateTransformationParameter(
                transID, paramName, paramValue, connection=connection
            )
            if res["OK"]:
        ...
        if message:
            self.__updateTransformationLogging(
                transID, message, author, connection=connection
            )
        return res
    ...
    def deleteTransformationParameter(
        self, transName, paramName, author="", connection=False
    ):
        ...
        res = self.__deleteTransformationParameters(
            transID, parameters=[paramName], connection=connection
        )
        if not res["OK"]:
            return res
        self.__updateTransformationLogging(
            transID,
            "Removed additional parameter %s" % paramName,
            author,
            connection=connection,
        )
        return res

    def __addAdditionalTransformationParameter(
        self, transID, paramName, paramValue, connection=False
    ):
        req = (
            "DELETE FROM AdditionalParameters WHERE TransformationID=%d AND ParameterName='%s'"
            % (transID, paramName)
        )
        res = self._update(req, connection)
        if not res["OK"]:
    ...
    def __deleteTransformationParameters(
        self, transID, parameters=None, connection=False
    ):
    ...
    def addFilesToTransformation(self, transName, lfns, connection=False):
        ...
        gLogger.info(
            ...
            " Attempting to add %s files to transformations: %s"
            % (len(lfns), transName)
        )
        if not lfns:
        ...
        if fileIDs:
            res = self.__addFilesToTransformation(
                transID, list(fileIDs), connection=connection
            )
            if not res["OK"]:
                return res
    ...
    def getTransformationFiles(
        ...
        connection = self.__getConnection(connection)
        req = "SELECT %s FROM TransformationFiles" % (
            intListToString(self.TRANSFILEPARAMS)
        )
        originalFileIDs = {}
        if condDict is None:
            condDict = {}
    ...
    def getFileSummary(self, lfns, connection=False):
        ...
        for fileDict in res["Value"]:
            resDict.setdefault(fileDict["LFN"], {})[
                fileDict["TransformationID"]
            ] = fileDict
        failedDict = dict.fromkeys(
            set(lfns) - set(resDict), "Did not exist in the Transformation database"
        )
        return S_OK({"Successful": resDict, "Failed": failedDict})

    def setFileStatusForTransformation(
        self, transID, fileStatusDict=None, connection=False
    ):
        ...
        for error, fileIDStatusList in statusFileDict.iteritems():
            req = reqBase + ",".join(
                "(%d, %d, '%s', 0, UTC_TIMESTAMP())" % (transID, fileID, status)
    ...
    def getTransformationStats(self, transName, connection=False):
        ...
        if not res["OK"]:
            return res
        statusDict = dict(
            (attrDict["Status"], count)
            for attrDict, count in res["Value"]
            if "-" not in attrDict["Status"]
        )
        statusDict["Total"] = sum(statusDict.values())
        return S_OK(statusDict)

    def getTransformationFilesCount(
        self, transName, field, selection=None, connection=False
    ):
        ...
        res = self.getCounters(
            "TransformationFiles", ["TransformationID", field], selection
        )
        if not res["OK"]:
            return res
    ...
    def __insertExistingTransformationFiles(
        self, transID, fileTuplesList, connection=False
    ):
    ...
    def __assignTransformationFile(
        self, transID, taskID, se, fileIDs, connection=False
    ):
        ...
        req = (
            "INSERT INTO TransformationFileTasks (TransformationID,FileID,TaskID) VALUES %s"
            % ",".join(fileTuples)
        )
        res = self._update(req, connection)
        if not res["OK"]:
    ...
    def __setTransformationFileStatus(self, fileIDs, status, connection=False):
        req = "UPDATE TransformationFiles SET Status = '%s' WHERE FileID IN (%s);" % (
            status,
            intListToString(fileIDs),
        )
        res = self._update(req, connection)
    ...
    def __setTransformationFileUsedSE(self, fileIDs, usedSE, connection=False):
        req = "UPDATE TransformationFiles SET UsedSE = '%s' WHERE FileID IN (%s);" % (
            usedSE,
            intListToString(fileIDs),
        )
        res = self._update(req, connection)
    ...
    def __deleteTransformationFileTask(self, transID, taskID, connection=False):
        ...
        """
        req = (
            "DELETE FROM TransformationFileTasks WHERE TransformationID=%d AND TaskID=%d"
            % (transID, taskID)
        )
        return self._update(req, connection)
    ...
    def __deleteTransformationFileTasks(self, transID, connection=False):
        ...
        req = (
            "DELETE FROM TransformationFileTasks WHERE TransformationID = %d;" % transID
        )
        res = self._update(req, connection)
        if not res["OK"]:
            gLogger.error(
                "Failed to delete transformation files/task history", res["Message"]
            )
        return res
    ...
    def deleteTasks(
        self, transName, taskIDbottom, taskIDtop, author="", connection=False
    ):
        ...
        for taskID in range(taskIDbottom, taskIDtop + 1):
            res = self.__removeTransformationTask(
                transID, taskID, connection=connection
            )
            if not res["OK"]:
                return res
    ...
    def reserveTask(self, transName, taskID, connection=False):
        ...
        if not res["Value"]:
            return S_ERROR(
                "Failed to set Reserved status for job %d - already Reserved"
                % int(taskID)
            )
        ...
        if not res["OK"]:
            return S_ERROR(
                "Failed to set Reserved status for job %d - failed to update the time stamp"
                % int(taskID)
            )
        return S_OK()

    def setTaskStatusAndWmsID(
        self, transName, taskID, status, taskWmsID, connection=False
    ):
        ...
        res = self.__setTaskParameterValue(
            transID, taskID, "ExternalID", taskWmsID, connection=connection
        )
        if not res["OK"]:
            return res
        return self.__setTaskParameterValue(
            transID, taskID, "ExternalStatus", status, connection=connection
        )
    ...
    def getTransformationTaskStats(self, transName="", connection=False):
        ...
        if transName:
            ...
            res = self.getCounters(
                "TransformationTasks",
                ["ExternalStatus"],
                {"TransformationID": res["Value"]},
                connection=connection,
            )
        else:
            res = self.getCounters(
                "TransformationTasks",
                ["ExternalStatus", "TransformationID"],
                {},
                connection=connection,
            )
        if not res["OK"]:
    ...
    def __setTaskParameterValue(
        self, transID, taskID, paramName, paramValue, connection=False
    ):
        req = (
            "UPDATE TransformationTasks SET %s='%s', LastUpdateTime=UTC_TIMESTAMP()"
    ...
    def __deleteTransformationTask(self, transID, taskID, connection=False):
        ...
        req = (
            "DELETE FROM TransformationTasks WHERE TransformationID=%d AND TaskID=%d"
            % (transID, taskID)
        )
        return self._update(req, connection)
    ...
    def __deleteTransformationMetaQueries(self, transID, connection=False):
        ...
        req = (
            "DELETE FROM TransformationMetaQueries WHERE TransformationID=%d" % transID
        )
        return self._update(req, connection)
    ...
    def createTransformationMetaQuery(
        self, transName, queryDict, queryType, author="", connection=False
    ):
        ...
        connection = res["Value"]["Connection"]
        transID = res["Value"]["TransformationID"]
        return self.__addMetaQuery(
            transID, queryDict, queryType, author=author, connection=connection
        )

    def __addMetaQuery(
        self, transID, queryDict, queryType, author="", connection=False
    ):
        ...
        for parameterName in sorted(queryDict):
            ...
            if not res["OK"]:
                ...
                self.deleteTransformationMetaQuery(
                    transID, queryType, connection=connection
                )
                break
            else:
    ...
    def deleteTransformationMetaQuery(
        self, transName, queryType, author="", connection=False
    ):
        ...
        queryType = res["Value"]
        req = (
            "DELETE FROM TransformationMetaQueries WHERE TransformationID=%d AND QueryType=%s;"
            % (transID, queryType)
        )
        res = self._update(req, connection)
        if not res["OK"]:
    ...
    def getTransformationMetaQuery(self, transName, queryType, connection=False):
        ...
        req = req + " WHERE TransformationID=%d AND QueryType=%s;" % (
            transID,
            queryType,
        )
        res = self._query(req, connection)
    ...
    def getTaskInputVector(self, transName, taskID, connection=False):
        ...
        req = (
            "SELECT TaskID,InputVector FROM TaskInputs WHERE TaskID in (%s) AND TransformationID='%d';"
            % (taskString, transID)
        )
        res = self._query(req)
        inputVectorDict = {}
    ...
    def __updateTransformationLogging(
        self, transName, message, authorDN, connection=False
    ):
        ...
        req = req + " VALUES (%s,'%s','%s',UTC_TIMESTAMP());" % (
            transID,
            message,
            authorDN,
        )
    ...
    def __getFileIDsForLfns(self, lfns, connection=False):
        ...
        """
        req = "SELECT LFN,FileID FROM DataFiles WHERE LFN in (%s);" % (
    ...
    def __getLfnsForFileIDs(self, fileIDs, connection=False):
        ...
        req = (
            "SELECT LFN,FileID FROM DataFiles WHERE FileID in (%s);"
            % stringListToString(fileIDs)
        )
        res = self._query(req, connection)
        if not res["OK"]:
            return res
    ...
    def __setDataFileStatus(self, fileIDs, status, connection=False):
        ...
        req = "UPDATE DataFiles SET Status = '%s' WHERE FileID IN (%s);" % (
            status,
            intListToString(fileIDs),
        )
        return self._update(req, connection)
    ...
    def addTaskForTransformation(
        self, transID, lfns=None, se="Unknown", connection=False
    ):
        ...
        if lfns:
            res = self.getTransformationFiles(
                condDict={"TransformationID": transID, "LFN": lfns},
                connection=connection,
            )
            if not res["OK"]:
                return res
            ...
            for fileDict in res["Value"]:
                ...
                if fileDict["Status"] in self.allowedStatusForTasks:
                    ...
                    gLogger.error(
                        "Supplied file not in %s status but %s"
                        % (self.allowedStatusForTasks, fileDict["Status"]),
                        lfn,
                    )
            unavailableLfns = set(lfns) - foundLfns
            if unavailableLfns:
                gLogger.error(
                    "Supplied files not found for transformation",
                    sorted(unavailableLfns),
                )
                return S_ERROR(
                    "Not all supplied files available in the transformation database"
                )
        ...
        req = req + " VALUES (%s,'%s','%d','%s', UTC_TIMESTAMP(), UTC_TIMESTAMP());" % (
            transID,
            "Created",
            0,
            se,
        ...
        if lfns:
            ...
            res = self.__assignTransformationFile(
                transID, taskID, se, fileIDs, connection=connection
            )
            if not res["OK"]:
                self.__removeTransformationTask(transID, taskID, connection=connection)
                return res
    ...
    def extendTransformation(self, transName, nTasks, author="", connection=False):
        ...
        transID = res["Value"]["TransformationID"]
        extendableProds = Operations().getValue(
            "Transformations/ExtendableTransfTypes", ["Simulation", "MCSimulation"]
        )
        ...
        self.__updateTransformationLogging(
            transName, message, author, connection=connection
        )
        return S_OK(taskIDs)
    ...
    def cleanTransformation(self, transName, author="", connection=False):
        ...
        self.__updateTransformationLogging(
            transID, "Transformation Cleaned", author, connection=connection
        )
    ...
    def __removeTransformationTask(self, transID, taskID, connection=False):
        ...
        if not res["OK"]:
            return res
        res = self.__deleteTransformationFileTask(
            transID, taskID, connection=connection
        )
        if not res["OK"]:
            return res
    ...
    def __checkUpdate(
        self, table, param, paramValue, selectDict=None, connection=False
    ):
    ...
    def exists(self, lfns, connection=False):
        ...
        gLogger.info(
            "TransformationDB.exists: Attempting to determine existence of %s files."
            % len(lfns)
        )
        res = self.__getFileIDsForLfns(lfns, connection=connection)
        if not res["OK"]:
    ...
    def addFile(self, fileDicts, force=False, connection=False):
        ...
        gLogger.info(
            "TransformationDB.addFile: Attempting to add %s files." % len(fileDicts)
        )
        successful = {}
        failed = {}
        ...
        for lfn in fileDicts:
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to getFileUserMetadata for file",
                    "%s: %s" % (lfn, res["Message"]),
                )
            ...
            if not (transIDs or force):  # not clear how force should be used for
                successful[
                    lfn
                ] = False  # True -> False bug fix: otherwise it is set to True even if transIDs is empty.
            else:
    ...
    def removeFile(self, lfns, connection=False):
        ...
        gLogger.info(
            "TransformationDB.removeFile: Attempting to remove %s files." % len(lfns)
        )
        failed = {}
        successful = {}
        connection = self.__getConnection(connection)
        ...
        if fileIDs:
            res = self.__setTransformationFileStatus(
                list(fileIDs), "Deleted", connection=connection
            )
            if not res["OK"]:
                return res
    ...
    def __addDirectory(self, path, force):
        ...
        if not res["OK"]:
            gLogger.error(
                "TransformationDB.addDirectory: Failed to get files. %s"
                % res["Message"]
            )
            return res
        if path not in res["Value"]["Successful"]:
            ...
            return res
        gLogger.info(
            "TransformationDB.addDirectory: Obtained %s files in %s seconds."
        ...
        return {
            "OK": True,
            "Value": len(res["Value"]["Successful"]),
    ...
    def setMetadata(self, path, usermetadatadict):
        ...
        """
        gLogger.info(
            "setMetadata: Attempting to set metadata %s to: %s"
            % (usermetadatadict, path)
        )
        transFiles = {}
        filesToAdd = []
        ...
        if not transIDs:
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to findFilesByMetadata %s: %s" % (path, res["Message"])
                )
                return res
    ...
    def _filterFileByMetadata(self, metadatadict):
        ...
        for transID, query in queries:
            ...
            gLogger.info(
                "Apply query %s to metadata %s" % (mq.getMetaQuery(), metadatadict)
            )
