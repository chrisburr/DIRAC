...
class TransformationClient(Client):
    ...
    def getCounters(
        self, table, attrList, condDict, older=None, newer=None, timeStamp=None
    ):
        ...
        while True:
            if lfnList:
                ...
                res = rpcClient.getTransformationFiles(
                    condDict, older, newer, timeStamp, orderAttribute, None, None
                )
            else:
                res = rpcClient.getTransformationFiles(
                    condDict,
            ...
            if not res["OK"]:
                gLogger.error(
                    ...
                    % (
                        str(transID),
                        offsetToApply,
                        ("retry %d times" % retries) if retries else "give up",
                    ),
                    res["Message"],
                )
                retries -= 1
    ...
    def cleanTransformation(self, transID):
        ...
        if not res["OK"]:
            return res
        # Setting the status
        return self.setTransformationParameter(
            transID, "Status", "TransformationCleaned"
        )
    ...
    def startTransformation(self, transID):
        ...
        if not res["OK"]:
            gLogger.error(
                "Failed to start transformation %s: %s" % (transID, res["Message"])
            )
            return res
        else:
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to set AgentType to transformation %s: %s"
                    % (transID, res["Message"])
                )
    ...
    def stopTransformation(self, transID):
        ...
        if not res["OK"]:
            gLogger.error(
                "Failed to stop transformation %s: %s" % (transID, res["Message"])
            )
            return res
        else:
    ...
    def moveFilesToDerivedTransformation(self, transDict, resetUnused=True):
        ...
        res = self.getTransformationFiles(
            condDict={"TransformationID": parentProd, "Status": ["Unused", "MaxReset"]}
        )
        if not res["OK"]:
            log.error(
                " Error getting Unused files from transformation",
        ...
        if not lfns:
            log.info(
                " No files found to be moved from transformation", "%d" % parentProd
            )
            return S_OK((parentProd, movedFiles))
        # get the lfns of the derived production that were Unused/MaxReset in the parent one
        res = self.getTransformationFiles(
            condDict={"TransformationID": prod, "LFN": lfns}
        )
        if not res["OK"]:
            log.error(
                " Error getting files from derived transformation:", res["Message"]
            )
            return res
        derivedFiles = res["Value"]
        derivedStatusDict = dict(
            (derivedDict["LFN"], derivedDict["Status"]) for derivedDict in derivedFiles
        )
        newStatusFiles = {}
        parentStatusFiles = {}
        badStatusFiles = {}
        for parentDict in parentFiles:
            ...
            if derivedStatus:
                ...
                if derivedStatus.endswith("-inherited"):
                    ...
                    badStatusFiles[derivedStatus] = (
                        badStatusFiles.setdefault(derivedStatus, 0) + 1
                    )
            ...
            for lfnChunk in breakListIntoChunks(lfnList, 5000):
                ...
                if not res["OK"]:
                    ...
                    log.info(
                        ...
                        ": %d files from %s to %s" % (len(lfnChunk), oldStatus, status),
                    )
        ...
        if res["OK"]:
            ...
            if assignedFiles:
                ...
                res = self.getTransformationFiles(
                    condDict={"TransformationID": parentProd, "LFN": lfns}
                )
                if res["OK"]:
                    parentFiles = res["Value"]
                    processedLfns = [
    ...
    def setFileStatusForTransformation(
        self, transName, newLFNsStatus=None, lfns=None, force=False
    ):
        ...
        tsFiles = self.getTransformationFiles(
            {"TransformationID": transName, "LFN": list(newLFNsStatus)}
        )
        if not tsFiles["OK"]:
            return tsFiles
        ...
        if tsFiles:
            ...
            tsFilesAsDict = dict(
                (
                    tsFile["LFN"],
                    [tsFile["Status"], tsFile["ErrorCount"], tsFile["FileID"]],
                )
                for tsFile in tsFiles
            )

            # applying the state machine to the proposed status
            newStatuses = self._applyTransformationFilesStateMachine(
                tsFilesAsDict, newLFNsStatus, force
            )
            ...
            if newStatuses:  # if there's something to update
                ...
                newStatusForFileIDs = dict(
                    (
                        ...
                        [
                            newStatuses[lfn],
                            self._wasFileInError(
                                newStatuses[lfn], tsFilesAsDict[lfn][0]
                            ),
                        ],
                    )
                    for lfn in newStatuses
                )
                res = rpcClient.setFileStatusForTransformation(
                    transName, newStatusForFileIDs
                )
                if not res["OK"]:
                    return res
    ...
    def _applyTransformationFilesStateMachine(
        self, tsFilesAsDict, dictOfProposedLFNsStatus, force
    ):
        ...
        for lfn, newStatus in dictOfProposedLFNsStatus.items():  # can be an iterator
            if lfn in tsFilesAsDict:
                ...
                if currentStatus == "processed" and newStatus.lower() != "processed":
                    ...
                    if (
                        errorCount
                        and ((errorCount % self.maxResetCounter) == 0)
                        and not force
                    ):
                        newStatus = "MaxReset"
    ...
    def setTransformationParameter(self, transID, paramName, paramValue, force=False):
        ...
        if paramName.lower() == "status":
            ...
            # applying the state machine to the proposed status
            value = self._applyTransformationStatusStateMachine(
                transIDAsDict, dictOfProposedstatus, force
            )
        else:
            value = paramValue
    ...
    def _applyTransformationStatusStateMachine(
        self, transIDAsDict, dictOfProposedstatus, force
    ):
