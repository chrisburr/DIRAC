...
class TaskManagerPlugin(PluginBase):
    ...
    def _ByJobType(self):
        ...
        gLogger.debug(
            "Explicitly excluded sites for %s task: %s"
        ...
        if not res["OK"]:
            ...
            allowed = dict(
                (site, set(fromChar(fromSites)))
                for site, fromSites in res["Value"].iteritems()
            )

        autoAddedSites = set(
            self.opsH.getValue(
                "JobTypeMapping/%s/AutoAddedSites" % jobType, autoAddedSites
            )
        )
        gLogger.debug(
            "Auto-added sites for %s task: %s" % (jobType, ",".join(autoAddedSites))
        )
