...
class Transformation(API):
    ...
    def setBody(self, body):
        ...
        if not isinstance(body, (list, tuple)):
            raise TypeError(
                "Expected list or string, but %r is %s" % (body, type(body))
            )

        for tup in body:
            if not isinstance(tup, (tuple, list)):
                raise TypeError(
                    "Expected tuple or list, but %r is %s" % (tup, type(tup))
                )
            if len(tup) != 2:
                raise TypeError(
                    "Expected 2-tuple, but %r is length %d" % (tup, len(tup))
                )
            ...
            for par, val in tup[1].iteritems():
                ...
                if not isinstance(
                    val, (basestring, int, long, float, list, tuple, dict)
                ):
    ...
    def __getParam(self):
        ...
        raise AttributeError(
            "Unknown parameter for transformation: %s" % self.item_called
        )
    ...
    def extendTransformation(self, nTasks, printOutput=False):
        return self.__executeOperation(
            "extendTransformation", nTasks, printOutput=printOutput
        )
    ...
    def addFilesToTransformation(self, lfns, printOutput=False):
        return self.__executeOperation(
            "addFilesToTransformation", lfns, printOutput=printOutput
        )

    def setFileStatusForTransformation(self, status, lfns, printOutput=False):
        return self.__executeOperation(
            "setFileStatusForTransformation", status, lfns, printOutput=printOutput
        )

    def getTransformationTaskStats(self, printOutput=False):
        return self.__executeOperation(
            "getTransformationTaskStats", printOutput=printOutput
        )

    def getTransformationStats(self, printOutput=False):
        return self.__executeOperation(
            "getTransformationStats", printOutput=printOutput
        )

    def deleteTasks(self, taskMin, taskMax, printOutput=False):
        return self.__executeOperation(
            "deleteTasks", taskMin, taskMax, printOutput=printOutput
        )

    def addTaskForTransformation(self, lfns=[], se="Unknown", printOutput=False):
        return self.__executeOperation(
            "addTaskForTransformation", lfns, se, printOutput=printOutput
        )

    def setTaskStatus(self, taskID, status, printOutput=False):
        return self.__executeOperation(
            "setTaskStatus", taskID, status, printOutput=printOutput
        )
    ...
    def getTransformationFiles(
        ...
        if printOutput:
            if not outputFields:
                gLogger.info(
                    "Available fields are: %s" % res["ParameterNames"].join(" ")
                )
            elif not res["Value"]:
                gLogger.info("No tasks found for selection")
            else:
                self._printFormattedDictList(
                    res["Value"], outputFields, "FileID", orderBy
                )
        return res
    ...
    def getTransformationTasks(
        ...
        if printOutput:
            if not outputFields:
                ...
                self._printFormattedDictList(
                    res["Value"], outputFields, "TaskID", orderBy
                )
        return res
    ...
    def getTransformations(
        ...
        if printOutput:
            if not outputFields:
                ...
                self._printFormattedDictList(
                    res["Value"], outputFields, "TransformationID", orderBy
                )
        return res
    ...
    def addTransformation(self, addFiles=True, printOutput=False):
        ...
        for paramName, paramValue in self.paramValues.items():
            if paramName not in self.paramTypes:
                ...
                if not res["OK"]:
                    gLogger.error(
                        "Failed to add parameter", "%s %s" % (paramName, res["Message"])
                    )
                    gLogger.notice(
        ...
        if plugin:
            if plugin not in self.supportedPlugins:
                ...
                res = self.__promptForParameter(
                    "Plugin", choices=self.supportedPlugins, default="Standard"
                )
                if not res["OK"]:
                    return res
    ...
    def __promptForParameter(self, parameter, choices=[], default="", insert=True):
        res = promptUser(
            "Please enter %s" % parameter, choices=choices, default=default
        )
        if not res["OK"]:
