...
class TaskBase(TransformationAgentsUtilities):
        ...
        for taskID, task in taskDict.iteritems():
            ...
            if task["Success"]:
                res = self.transClient.setTaskStatusAndWmsID(
                    transID, taskID, "Submitted", str(task["ExternalID"])
                )
                if not res["OK"]:
                    self._logWarn(
    ...
    def updateTransformationReservedTasks(
        self, _taskDicts
...
class RequestTasks(TaskBase):
    ...
    def _multiOperationsBody(self, transJson, taskDict, ownerDN, ownerGroup):
        ...
        for taskID, task in taskDict.items():
            ...
            if not task.get("InputData"):
                self._logError(
                    "Error creating request for task",
            ...
            result = self._assignRequestToTask(
                oRequest, taskDict, transID, taskID, ownerDN, ownerGroup
            )
            if not result["OK"]:
    ...
    def _assignRequestToTask(
        self, oRequest, taskDict, transID, taskID, ownerDN, ownerGroup
    ):
    ...
    def submitTransformationTasks(self, taskDict):
        ...
        for task in taskDict.itervalues():
            ...
            if res["OK"]:
                ...
                self._logError(
                    "Failed to submit task to RMS", res["Message"], transID=transID
                )
                task["Success"] = False
                failed += 1
        if submitted:
        ...
        if failed:
            self._logWarn(
                "Failed to submit %d tasks to RMS." % (failed),
                transID=transID,
                method=method,
            )
        return S_OK(taskDict)
    ...
    def submitTaskToExternal(self, oRequest):
        ...
        if isinstance(oRequest, self.requestClass):
            return self.requestClient.putRequest(
                oRequest, useFailoverProxy=False, retryMainService=2
            )
    ...
    def updateTransformationReservedTasks(self, taskDicts):
        ...
        for taskDict in taskDicts:
            requestName = self._transTaskName(
                taskDict["TransformationID"], taskDict["TaskID"]
            )
        ...
        for taskDict in taskDicts:
            ...
            if taskDict["ExternalID"] and int(taskDict["ExternalID"]):
                ...
                if not newStatus["OK"]:
                    log = (
                        self._logVerbose
                        if "not exist" in newStatus["Message"]
                        else self._logWarn
                    )
                    log(
    ...
    def getSubmittedFileStatus(self, fileDicts):
        ...
        res = self.transClient.getTransformationTasks(
            {"TransformationID": transID, "TaskID": list(taskFiles)}
        )
        if not res["OK"]:
            return res
        requestFiles = {}
        for taskDict in res["Value"]:
            ...
            if (
                taskDict["ExternalStatus"] != "Created"
                and externalID
                and int(externalID)
            ):
        ...
        for requestID, lfnList in requestFiles.iteritems():
            ...
            if not statusDict["OK"]:
                log = (
                    self._logVerbose
                    if "not exist" in statusDict["Message"]
                    else self._logWarn
                )
                log(
...
class WorkflowTasks(TaskBase):
    ...
    def prepareTransformationTasks(
        ...
        if bulkSubmissionFlag:
            return self.__prepareTasksBulk(
                transBody, taskDict, owner, ownerGroup, ownerDN
            )
    ...
    def __prepareTasksBulk(self, transBody, taskDict, owner, ownerGroup, ownerDN):
        ...
        self._logVerbose(
            "Setting job owner:group to %s:%s" % (owner, ownerGroup),
            transID=transID,
            method=method,
        )
        oJob.setOwner(owner)
        ...
        if not oJob.workflow.findParameter("JOB_ID"):
            oJob._addParameter(
                oJob.workflow, "JOB_ID", "string", "00000000", "Initial JOB_ID"
            )

        if oJob.workflow.findParameter("PRODUCTION_ID"):
            oJob._setParamValue(
                "PRODUCTION_ID", str(transID).zfill(8)
            )  # pylint: disable=protected-access
        else:
            oJob._addParameter(
        ...
        self._logVerbose(
            "Adding default transformation group of %s" % (transGroup),
            transID=transID,
            method=method,
        )
        ...
        for taskID in sorted(taskDict):
            ...
            if not sites:
                self._logError(
                    "Could not get a list a sites", transID=transID, method=method
                )
                return S_ERROR(ETSUKN, "Can not evaluate destination site")
            else:
                self._logVerbose(
                    "Setting Site: ", str(sites), transID=transID, method=method
                )
                seqDict["Site"] = sites
            ...
            if inputData:
                ...
                self._logVerbose(
                    "Setting input data to %s" % inputData,
                    transID=transID,
                    method=method,
                )
                ...
                return S_ERROR(
                    ETSDATA, "Invalid mixture of jobs with and without input data"
                )
            ...
            for paramName, paramValue in paramsDict.iteritems():
                if paramName not in ("InputData", "Site", "TargetSE"):
                    if paramValue:
                        self._logVerbose(
                            "Setting %s to %s" % (paramName, paramValue),
                            transID=transID,
                            method=method,
                        )
        ...
        for paramName, paramSeq in paramSeqDict.iteritems():
            if (
                paramName
                in ["JOB_ID", "PRODUCTION_ID", "InputData"] + outputParameterList
            ):
                res = oJob.setParameterSequence(
                    paramName, paramSeq, addToWorkflow=paramName
                )
            else:
    ...
    def __prepareTasks(self, transBody, taskDict, owner, ownerGroup, ownerDN):
        ...
        for taskID, paramsDict in taskDict.iteritems():
            ...
            if not templateOK:
                ...
                self._logVerbose(
                    "Job owner:group to %s:%s" % (owner, ownerGroup),
                    transID=transID,
                    method=method,
                )
                transGroup = str(transID).zfill(8)
            ...
            self._logVerbose(
                "Setting task name to %s" % constructedName,
                transID=transID,
                method=method,
            )
            ...
            if not sites:
                ...
                if not res["OK"]:
                    self._logError(
                        "Could not set the site: %s" % res["Message"],
                        transID=transID,
                        method=method,
                    )
                    paramsDict["TaskObject"] = ""
    ...
    def _handleInputs(self, oJob, paramsDict):
        ...
        if inputData:
            ...
            if not res["OK"]:
                self._logError(
                    "Could not set the inputs: %s" % res["Message"],
                    transID=transID,
                    method="_handleInputs",
                )
    ...
    def _checkSickTransformations(self, transID):
        ...
        if transID in set(
            int(x)
            for x in self.opsH.getValue(os.path.join(clinicPath, "Transformations"), [])
        ):
            return clinicPath
    ...
    def _handleHospital(self, oJob, clinicPath):
        ...
        if not hospitalSite:
            hospitalSite = self.opsH.getValue(
                "Hospital/HospitalSite", "DIRAC.JobDebugger.ch"
            )
        if not hospitalCEs:
    ...
    def __generatePluginObject(self, plugin):
        ...
        try:
            plugModule = __import__(
                self.pluginLocation, globals(), locals(), ["TaskManagerPlugin"]
            )
        except ImportError as e:
            self._logException(
                "Failed to import 'TaskManagerPlugin' %s: %s" % (plugin, e),
                method=method,
            )
            return S_ERROR()
        try:
            plugin_o = getattr(plugModule, "TaskManagerPlugin")(
                "%s" % plugin, operationsHelper=self.opsH
            )
            return S_OK(plugin_o)
        except AttributeError as e:
            self._logException(
                "Failed to create %s(): %s." % (plugin, e), method=method
            )
            return S_ERROR()
    ...
    def __submitTransformationTasksBulk(self, taskDict):
        ...
        if not result["OK"]:
            self._logError(
                "Failed to submit tasks to external", transID=transID, method=method
            )
            return result
    ...
    def __submitTransformationTasks(self, taskDict):
        ...
        if failed:
            self._logError(
                "Failed to submit %d tasks to WMS." % (failed),
                transID=transID,
                method=method,
            )
        return S_OK(taskDict)
    ...
    def updateTransformationReservedTasks(self, taskDicts):
        transID = None
        jobNames = [
            self._transTaskName(taskDict["TransformationID"], taskDict["TaskID"])
            for taskDict in taskDicts
        ]
    ...
    def getSubmittedTaskStatus(self, taskDicts):
        ...
        if not res["OK"]:
            self._logWarn(
                "Failed to get job status from the WMS system",
                transID=transID,
                method=method,
            )
            return res
    ...
    def getSubmittedFileStatus(self, fileDicts):
        ...
        if not res["OK"]:
            self._logWarn(
                "Failed to obtain taskIDs for files", transID=transID, method=method
            )
            return res
