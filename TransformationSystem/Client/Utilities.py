...
class PluginUtilities(object):
                ...
                if lfns:
                    ...
                    if not groupSE:
                        ...
                        for se in [se for se in seFiles if se != replicaSE]:
                            seFiles[se] = [
                                lfn for lfn in seFiles[se] if lfn not in lfnsInTasks
                            ]
            self.logVerbose(
    ...
    def createTasksBySize(self, lfns, replicaSE, fileSizes=None, flush=False):
        ...
        if not self.groupSize:
            ...
            self.groupSize = (
                float(self.getPluginParam("GroupSize", 1.0)) * 1000 * 1000 * 1000
            )
        if not self.maxFiles:
            # FIXME: prepare for chaging the name of the ambiguoug  CS option
            self.maxFiles = self.getPluginParam(
                "MaxFilesPerTask", self.getPluginParam("MaxFiles", 100)
            )
    ...
    def groupBySize(self, files, status):
        ...
        if not self.groupSize:
            # input size in GB converted to bytes
            self.groupSize = (
                float(self.getPluginParam("GroupSize", 1)) * 1000 * 1000 * 1000
            )
        flush = status == "Flush"
        self.logVerbose(
        ...
        for groupSE in (True, False):
            ...
            for replicaSE in sorted(seFiles) if groupSE else sortSEs(seFiles):
                lfns = seFiles[replicaSE]
                newTasks = self.createTasksBySize(
                    lfns, replicaSE, fileSizes=fileSizes, flush=flush
                )
                lfnsInTasks = []
            ...
            self.logVerbose(
                "groupBySize: %d files have not been included in tasks" % len(files)
            )
            nTasks = len(tasks)
    ...
    def _getFileSizeFromCatalog(self, lfns, fileSizes):
        ...
        if res["Value"]["Failed"]:
            ...
            self.logWarn(
                "Failed to get sizes for %d files:" % len(res["Value"]["Failed"]),
                errorReason,
            )
    ...
    def _normaliseShares(originalShares):
        ...
        return dict(
            [
                (site, 100.0 * float(share) / total if total else 0.0)
    ...
    def closerSEs(self, existingSEs, targetSEs, local=False):
        ...
        sameSEs = set(
            [se1 for se1 in setTarget for se2 in existingSEs if self.isSameSE(se1, se2)]
        )
        ...
        if targetSEs:
            ...
            closeSEs = set(
                [
                    se
                    for se in targetSEs
                    if self.dmsHelper.getLocalSiteForSE(se).get("Value")
                    in existingSites
                ]
            )
