...
class TransformationCLI(CLI, API):
    ...
    def check_params(self, args, num):
        ...
        if length < num:
            print(
                "Error: Number of arguments provided %d less that required %d, please correct."
                % (length, num)
            )
            return (False, length)
        return (argss, length)
    ...
    def do_getAllByUser(self, args):
        ...
        oTrans.getTransformationsByUser(
    ...
    def do_start(self, args):
        ...
        for transName in argss:
            ...
            if not res["OK"]:
                ...
                if not res["OK"]:
                    print(
                        "Setting AgentType of %s failed: %s"
    ...
    def do_modMask(self, args):
        ...
        for transName in transNames:
            ...
            if not res["OK"]:
                print(
                    "Failed to modify input file mask for %s: %s"
                    % (transName, res["Message"])
                )
            else:
    ...
    def do_resetProcessedFile(self, args):
        ...
        res = self.transClient.setFileStatusForTransformation(
            transName, "Unused", lfns, force=True
        )
        if not res["OK"]:
            print("Failed to reset file status: %s" % res["Message"])
        else:
