...
class ProdValidator(object):
    ...
    def checkTransStatus(self, transID):
        ...
        if status != "New":
            return S_ERROR(
                "checkTransStatus failed : Invalid transformation status: %s" % status
            )
    ...
    def compareValues(self, value, parentValue):
        ...
        """
        return set(value.values()[0]).issubset(set(parentValue.values()[0])) or set(
            parentValue.values()[0]
        ).issubset(set(value.values()[0]))
