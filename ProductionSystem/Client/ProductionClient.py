...
class ProductionClient(Client):
    ...
    def _applyProductionStatusStateMachine(self, prodID, status, force=False):
        ...
        if force:
            ...
            stateChange = ProductionsStateMachine(originalStatus).setState(
                proposedStatus
            )
    ...
    def addProductionStep(self, prodStep):
        ...
        if not res["OK"]:
            return res
        stepID = res["Value"]
        self.prodDescription[prodStep.Name] = {
            "stepID": stepID,
            "parentStep": prodStepDict["parentStep"],
        }
        return S_OK()
