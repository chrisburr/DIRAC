...
class ProductionDB(DB):
    ...
    def addProduction(
        self, prodName, prodDescription, authorDN, authorGroup, connection=False
    ):
        ...
        if res["OK"]:
            return S_ERROR(
                "Production with name %s already exists with ProductionID = %d"
                % (prodName, res["Value"])
            )
    ...
    def getProduction(self, prodName, connection=False):
        ...
        res = self.getProductions(
            condDict={"ProductionID": prodID}, connection=connection
        )
        if not res["OK"]:
            return res
        if not res["Value"]:
    ...
    def getProductionStep(self, stepID, connection=False):
        ...
        """
        connection = self.__getConnection(connection)
        req = "SELECT %s FROM ProductionSteps WHERE StepID = %s" % (
            intListToString(self.PRODSTEPSPARAMS),
            str(stepID),
        )
        res = self._query(req, connection)
        if not res["OK"]:
    ...
    def __setProductionStatus(self, prodID, status, connection=False):
        ...
        """
        req = (
            "UPDATE Productions SET Status='%s', LastUpdate=UTC_TIMESTAMP() WHERE ProductionID=%d"
            % (status, prodID)
        )
    ...
    def setProductionStatus(self, prodName, status, connection=False):
        ...
        res = self.ProdTransManager.executeActionOnTransformations(
            prodID, self.statusActionDict[status]
        )
        if not res["OK"]:
    ...
    def startProduction(self, prodName, connection=False):
        ...
        for step in sorted(prodDescription):
            res = self.ProdTransManager.addTransformationStep(
                prodDescription[step]["stepID"], prodID
            )
        ...
        for step in prodDescription:
            ...
            res = self.addTransformationsToProduction(
                prodID, transID, parentTransIDs=parentTransIDs
            )
        ...
        res = self.ProdTransManager.executeActionOnTransformations(
            prodID, self.statusActionDict["Active"]
        )
        if not res["OK"]:
            gLogger.error(res["Message"])
    ...
    def __deleteProductionTransformations(self, prodID, connection=False):
        ...
        if not res["OK"]:
            gLogger.error(
                "Failed to delete production transformations from the TS",
                res["Message"],
            )

        # Remove transformations from the PS
        req = (
            "DELETE FROM ProductionTransformationLinks WHERE ProductionID = %d;"
            % prodID
        )
        res = self._update(req, connection)
        if not res["OK"]:
            gLogger.error(
                "Failed to delete production transformation links from the PS",
                res["Message"],
            )
        ...
        if not res["OK"]:
            gLogger.error(
                "Failed to delete production transformations from the PS",
                res["Message"],
            )
    ...
    def addTransformationsToProduction(
        self, prodName, transIDs, parentTransIDs=None, connection=False
    ):
        ...
        if parentTransIDs:
            for transID in transIDs:
                for parentTransID in parentTransIDs:
                    ...
                    res = self.prodValidator.checkTransDependency(
                        transID, parentTransID
                    )
                    if not res["OK"]:
                        gLogger.error("Production is not valid:", res["Message"])
        ...
        if not res["OK"]:
            msg = "Failed to add transformations %s to production %s: %s" % (
                transIDs,
                prodID,
                res["Message"],
            )
        ...
        res = self.__addTransformationLinks(
            prodID, transIDs, parentTransIDs=parentTransIDs, connection=connection
        )
        if not res["OK"]:
            msg = (
                "Failed to add production transformations links to transformations %s: %s"
                % (transIDs, res["Message"])
            )
            return S_ERROR(msg)
        ...
        res = self.ProdTransManager.executeActionOnTransformations(
            prodID, self.statusActionDict[prodStatus]
        )
        if not res["OK"]:
            gLogger.error(res["Message"])
    ...
    def __addTransformationLinks(
        self, prodID, transIDs, parentTransIDs=None, connection=False
    ):
    ...
    def _getProductionID(self, prodName, connection=False):
        ...
        try:
            ...
            cmd = (
                "SELECT ProductionID from Productions WHERE ProductionID=%d;" % prodName
            )
        except BaseException:
            if not isinstance(prodName, six.string_types):
                return S_ERROR("Production should be ID or name")
            cmd = (
                "SELECT ProductionID from Productions WHERE ProductionName='%s';"
                % prodName
            )
        res = self._query(cmd, connection)
        if not res["OK"]:
            gLogger.error(
                "Failed to obtain production ID for production",
                "%s: %s" % (prodName, res["Message"]),
            )
            return res
        elif not res["Value"]:
    ...
    def _getConnectionProdID(self, connection, prodName):
        ...
        if not res["OK"]:
            gLogger.error(
                "Failed to get ID for production %s: %s" % (prodName, res["Message"])
            )
            return res
        prodID = res["Value"]
