...
class RequestTask(object):
    ...
    def __setupManagerProxies(self):
        ...
        for shifter in shifters:
            ...
            if not shifterDict["OK"]:
                self.log.error(
                    "Cannot get options dict for shifter",
            ...
            if vomsAttr:
                ...
                self.log.debug(
                    "getting proxy for shifter %s@%s (%s)"
            ...
            if not getProxy["OK"]:
                return S_ERROR(
                    "unable to setup shifter proxy for %s: %s"
    ...
    def setupProxy(self):
        ...
        if not ownerProxyFile["OK"] or not ownerProxyFile["Value"]:
            ...
            return S_ERROR(
                "Change proxy error for '%s'@'%s': %s" % (ownerDN, ownerGroup, reason)
            )
    ...
    def getHandler(self, operation):
        ...
        if not handler:
            try:
                ...
                self.log.exception(
                    "Error getting Handler", "%s" % error, lException=error
                )
    ...
    def __call__(self):
        ...
        if not setupProxy["OK"]:
            ...
            if "has no proxy registered" in setupProxy["Message"]:
                self.log.error(
                    "Error setting proxy. Request set to Failed:", setupProxy["Message"]
                )
