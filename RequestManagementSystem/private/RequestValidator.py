...
class RequestValidator(object):
    ...
    def addReqAttrsCheck(cls, operationType, operationAttrs=None, filesAttrs=None):
        ...
        """
        toUpdate = {
            "Operation": operationAttrs if operationAttrs else [],
    ...
    def _hasOwner(request):
        ...
        if not request.OwnerDN:
            return S_ERROR(
                "Request '%s' is missing OwnerDN value" % request.RequestName
            )
        if not request.OwnerGroup:
            return S_ERROR(
                "Request '%s' is missing OwnerGroup value" % request.RequestName
            )
        return S_OK()
    ...
    def _hasOperations(request):
        ...
        if not len(request):
            return S_ERROR(
                "Operations not present in request '%s'" % request.RequestName
            )
        return S_OK()
