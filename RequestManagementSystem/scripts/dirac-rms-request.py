...
if __name__ == "__main__":
    ...
    if status and not requests:
        ...
        gLogger.notice(
            "Obtained %d requests %s between %s and %s"
    ...
    for reqID in requests:
        ...
        if cancel:
            if request.Status not in ("Done", "Failed"):
                ...
                gLogger.notice(
                    "Request %s is in status %s, not cancelled"
            ...
            if reset:
                ...
                if not ret["OK"]:
                    gLogger.error(
                        "Error resetting request %s" % requestID, ret["Message"]
                    )
            else:
                if len(requests) > 1:
                    gLogger.notice("\n===================================")
                ...
                printRequest(
                    request, status=dbStatus, full=full, verbose=verbose, terse=terse
                )
