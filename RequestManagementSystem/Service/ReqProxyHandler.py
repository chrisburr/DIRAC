...
class ReqProxyHandler(RequestHandler):
    ...
    def export_putRequest(self, requestJSON):
        ...
        if not setRequest["OK"]:
            ...
            if not save["OK"]:
                ...
                return save
            gLogger.info(
                "setRequest: %s is saved to %s file" % (requestName, save["Value"])
            )
            return S_OK({"set": False, "saved": True})

        gLogger.info(
            "setRequest: request '%s' has been set to the ReqManager" % (requestName)
        )
    ...
    def export_showCachedRequest(self, filename):
        ...
        try:
            ...
            return S_ERROR(
                DErrno.ERMSUKN,
                "Error showing cached request %s: %s" % (fullPath, repr(e)),
            )
