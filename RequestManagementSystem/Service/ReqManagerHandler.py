...
class ReqManagerHandler(RequestHandler):
    ...
    def initializeHandler(cls, serviceInfoDict):
        ...
        cls.constantRequestDelay = getServiceOption(
            serviceInfoDict, "ConstantRequestDelay", 0
        )
    ...
    def export_putRequest(self, requestJSON):
        ...
        if not isAuthorized:
            return S_ERROR(
                DErrno.ENOAUTH, "Credentials in the requests are not allowed"
            )
        ...
        if not valid["OK"]:
            gLogger.error(
                "putRequest: request %s not valid: %s" % (requestName, valid["Message"])
            )
            return valid
    ...
    def export_getRequestSummaryWeb(cls, selectDict, sortList, startItem, maxItems):
        ...
        """
        return cls.__requestDB.getRequestSummaryWeb(
    ...
    def export_getRequestIDsList(
        cls, statusList=None, limit=None, since=None, until=None, getJobID=False
    ):
