...
class RequestExecutingAgent(AgentModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__requestsPerCycle = self.am_getOption(
            "RequestsPerCycle", self.__requestsPerCycle
        )
        ...
        self.__poolTimeout = int(
            self.am_getOption("ProcessPoolTimeout", self.__poolTimeout)
        )
        ...
        if not opHandlers["OK"]:
            ...
            raise AgentConfigError(
                "OperationHandlers section not found in CS under %s" % self.__configPath
            )
        ...
        for opHandler in opHandlers:
            ...
            if not opLocation:
                self.log.error(
                    "%s not set for %s operation handler" % (opHandlerPath, opHandler)
                )
                continue
            self.timeOuts[opHandler] = {
                "PerFile": self.__fileTimeout,
                "PerOperation": self.__operationTimeout,
            }

            opTimeout = gConfig.getValue(
                "%s/%s/TimeOut" % (opHandlersPath, opHandler), 0
            )
            if opTimeout:
                self.timeOuts[opHandler]["PerOperation"] = opTimeout
            fileTimeout = gConfig.getValue(
                "%s/%s/TimeOutPerFile" % (opHandlersPath, opHandler), 0
            )
            if fileTimeout:
    ...
    def processPool(self):
        ...
        if not self.__processPool:
            ...
            self.log.info(
                ...
                "minProcess = %d maxProcess = %d queueSize = %d"
    ...
    def cacheRequest(self, request):
        ...
        if len(self.__requestCache) > maxProcess + 50:
            ...
            self.log.warn(
                "Too many requests in cache", ": %d" % len(self.__requestCache)
            )
        ...
        if request.RequestID in self.__requestCache:
            ...
            self.log.warn(
                "Duplicate request, keep it but don't execute",
    ...
    def putRequest(self, requestID, taskResult=None):
        ...
        if requestID in self.__requestCache:
            ...
            reset = self.requestClient().putRequest(
                request, useFailoverProxy=False, retryMainService=2
            )
            if not reset["OK"]:
                return S_ERROR(
                    "putRequest: unable to reset request %s: %s"
                    % (requestID, reset["Message"])
                )
        else:
    ...
    def putAllRequests(self):
        ...
        """
        self.log.info(
            "putAllRequests: will put back requests", "%s" % len(self.__requestCache)
        )
        for requestID in self.__requestCache.keys():
            ...
            if not reset["OK"]:
                ...
                self.log.debug(
                    "putAllRequests: request %s has been put back with its initial state"
                    % requestID
                )
        return S_OK()
    ...
    def execute(self):
        ...
        while taskCounter < self.__requestsPerCycle:
            ...
            if not self.__bulkRequest:
                ...
                numberOfRequest = min(
                    self.__bulkRequest, self.__requestsPerCycle - taskCounter
                )
                ...
                for rId in getRequests["Value"]["Failed"]:
                    self.log.error(
                        "execute:", "%s" % getRequests["Value"]["Failed"][rId]
                    )

                requestsToExecute = getRequests["Value"]["Successful"].values()

            self.log.info(
                "execute: will execute requests ", "%s" % len(requestsToExecute)
            )
            ...
            for request in requestsToExecute:
                ...
                self.log.info(
                    ...
                    % (
                        self.processPool().getNumIdleProcesses(),
                ...
                while True:
                    if not self.processPool().getFreeSlots():
                        ...
                        if looping:
                            self.log.info(
                                "Free slot found",
                                "after %d seconds" % looping * self.__poolSleep,
                            )
                        looping = 0
                        ...
                        self.log.info(
                            "spawning task for request",
                            "'%s/%s'" % (request.RequestID, request.RequestName),
                        )
                        ...
                        if not enqueue["OK"]:
                            ...
                            self.log.debug(
                                "successfully enqueued task", "'%s'" % taskID
                            )
                            # # update monitor
        ...
        self.log.info(
            "Flushing callbacks",
            "(%d requests still in cache)" % len(self.__requestCache),
        )
    ...
    def getTimeout(self, request):
        ...
        for op in request:
            ...
            if op.Type not in self.timeOuts:
                ...
                perOp = self.timeOuts[op.Type].get(
                    "PerOperation", self.__operationTimeout
                )
                perFiles = self.timeOuts[op.Type].get(
                    "PerFile", self.__fileTimeout
                ) * len(op)
                timeout += perOp + perFiles
        self.log.info(
            "estimated timeOut for request",
            "(%s/%s) is %s" % (request.RequestID, request.RequestName, timeout),
        )
        return timeout
    ...
    def exceptionCallback(self, taskID, taskException):
        ...
        """
        self.log.error(
            "exceptionCallback:", "%s was hit by exception %s" % (taskID, taskException)
        )
