...
class CleanReqDBAgent(AgentModule):
    ...
    def initialize(self):
        ...
        self.log.info(
            "Delete failed requests: %s" % {True: "yes", False: "no"}[self.DEL_FAILED]
        )

        self.KICK_GRACE_HOURS = self.am_getOption(
            "KickGraceHours", self.KICK_GRACE_HOURS
        )
        self.log.info(
            "Kick assigned requests period = %s hours" % self.KICK_GRACE_HOURS
        )
    ...
    def execute(self):
        ...
        for requestID, status, lastUpdate in requestIDsList:
            ...
            if not reqStatus["OK"]:
                self.log.error(
                    ("execute: unable to get request status", reqStatus["Message"])
                )
                continue
            ...
            if lastUpdate < kickTime and status == "Assigned":
                ...
                if not getRequest["OK"]:
                    self.log.error(
                        "execute: unable to read request '%s': %s"
        ...
        for requestID, status, lastUpdate in requestIDsList:
            if lastUpdate < rmTime:
                self.log.info(
                    "execute: deleting request '%s' with status %s"
                    % (requestID, status)
                )
