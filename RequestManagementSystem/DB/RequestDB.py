...
operationTable = Table(
    ...
    Column(
        "Status",
        Enum(
            "Waiting", "Assigned", "Queued", "Done", "Failed", "Canceled", "Scheduled"
        ),
...
class RequestDB(object):
                ...
                .values(
                    {
                        ...
                        Request._LastUpdate: datetime.datetime.utcnow().strftime(
    ...
    def getRequestIDsList(
        ...
        try:
            if getJobID:
                ...
                reqQuery = session.query(
                    Request.RequestID, Request._Status, Request._LastUpdate
                ).filter(Request._Status.in_(statusList))
            if since:
    ...
    def getDBSummary(self):
        ...
        try:
            requestQuery = (
                session.query(Request._Status, func.count(Request.RequestID))
            ...
            operationQuery = (
                session.query(
                    Operation.Type, Operation._Status, func.count(Operation.OperationID)
                )
            ...
            fileQuery = (
                session.query(File._Status, func.count(File.FileID))
    ...
    def getRequestSummaryWeb(self, selectDict, sortList, startItem, maxItems):
        ...
        try:
            ...
            for key, value in selectDict.items():
                ...
                if key == "ToDate":
                    ...
                    if key == "Type":
                        summaryQuery = summaryQuery.join(
            ...
            if sortList:
                summaryQuery = summaryQuery.order_by(
                    eval("Request.%s.%s()" % (sortList[0][0], sortList[0][1].lower()))
                )
        ...
        #
            ...
            self.log.exception(
                "getRequestSummaryWeb: unexpected exception", lException=e
            )
    ...
    def getRequestCountersWeb(self, groupingAttribute, selectDict):
        ...
        try:
            summaryQuery = session.query(
                eval(groupingAttribute), func.count(Request.RequestID)
            )
            ...
            for key, value in selectDict.items():
                if key == "ToDate":
                    ...
                    if isinstance(value, list):
                        summaryQuery = summaryQuery.filter(
                            eval("%s.%s.in_(%s)" % (objectType, key, value))
                        )
                    else:
                        summaryQuery = summaryQuery.filter(
                            eval("%s.%s" % (objectType, key)) == value
                        )
    ...
    def getDistinctValues(self, tableName, columnName):
        ...
        try:
            result = session.query(
                distinct(eval("%s.%s" % (tableName, columnName)))
            ).all()
    ...
    def getRequestIDsForJobs(self, jobIDs):
        ...
        try:
            ret = (
                session.query(Request.JobID, Request.RequestID)
            ...
            reqDict["Failed"] = dict(
                (jobid, "Request not found")
                for jobid in jobIDs - set(reqDict["Successful"])
            )
        except Exception as e:
            self.log.exception(
                "getRequestIDsForJobs: unexpected exception", lException=e
            )
    ...
    def readRequestsForJobs(self, jobIDs=None):
        ...
        try:
            ...
            self.log.exception(
                "readRequestsForJobs: unexpected exception", lException=e
            )
    ...
    def getRequestFileStatus(self, requestID, lfnList):
        ...
        try:
            ...
            self.log.exception(
                "getRequestFileStatus: unexpected exception", lException=e
            )
    ...
    def getRequestIDForName(self, requestName):
        ...
        try:
            ...
            if not ret:
                ...
                return S_ERROR(
                    "RequestName %s not unique (%s matches)" % (requestName, len(ret))
                )
            ...
            self.log.exception(
                "getRequestIDsForName: unexpected exception", lException=e
            )
