...
class Request(object):
    ...
    def CreationTime(self, value=None):
        ...
        if isinstance(value, six.string_types):
            value = datetime.datetime.strptime(
                value.split(".")[0], self._datetimeFormat
            )
    ...
    def Order(self):
        ...
        return (
            opStatuses.index("Waiting") if "Waiting" in opStatuses else len(opStatuses)
        )
    ...
    def _getJSONData(self):
        ...
        for attrName in attrNames:
            ...
            if isinstance(value, datetime.datetime):
                ...
                jsonData[attrName] = value.strftime(
    ...
    def getDigest(self):
        ...
        for op in self:
            ...
            if len(op):
                opFile = op[0]
                extraFilesStr = (
                    "...+<%d files>" % (len(op) - 1) if (len(op) > 1) else ""
                )
        ...
        * Catalog
        ...
        while i < len(self.__operations__):
            ...
            if i < len(self.__operations__) - 1:
                ...
                if (
                    getattr(op1, "Type") == "ReplicateAndRegister"
                    and getattr(op2, "Type") == "RemoveReplica"
                ):
            ...
            if insertNow:
                if repAndRegList:
                    ...
                    optimized = True
                    insertBefore = (
                        self.__operations__[i] if i < len(self.__operations__) else None
                    )
                    # print 'Insert new operations before', insertBefore
                    for op in [op for _targetSE, op in sorted(repAndRegList)] + [
                        op for _targetSE, op in sorted(removeRepList)
                    ]:
                        _res = (
                            self.insertBefore(op, insertBefore)
                            if insertBefore
                            else self.addOperation(op)
                        )
            ...
            while i < len(self.__operations__) - 1:
                ...
                try:
                    while len(self.__operations__[i + 1]):
                        ...
                        if len(self.__operations__[i + 1]) and (
                            self.__operations__[i + 1][0] == fileToMove
                        ):
