...
class RequestTests(unittest.TestCase):
    ...
    def test_05FTS(self):
        ...
        self.assertEqual(
            req.Status, "Waiting", "1. wrong request status: %s" % req.Status
        )
        self.assertEqual(
            ftsTransfer.Status,
            "Waiting",
        ...
        self.assertEqual(
            req.Status, "Scheduled", "2. wrong status for request: %s" % req.Status
        )
        ...
        self.assertEqual(
            req.Status, "Waiting", "3. wrong status for request: %s" % req.Status
        )
        ...
        self.assertEqual(
            req.Status, "Scheduled", "4. wrong status for request: %s" % req.Status
        )
        ...
        self.assertEqual(
            req.Status, "Waiting", "5. wrong status for request: %s" % req.Status
        )
    ...
    def test_08Optimize(self):
        ...
        for reqType in title:
            ...
            res = optimizeRequest(
                r,
                printOutput=title[reqType]
                if (debug == reqType and debug is not False)
                else False,
            )
