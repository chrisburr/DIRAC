...
class ReqClient(Client):
    ...
    def __init__(self, url=None, **kwargs):
        ...
        self.log = gLogger.getSubLogger(
            "RequestManagement/ReqClient/pid_%s" % (os.getpid())
        )
    ...
    def putRequest(self, request, useFailoverProxy=True, retryMainService=0):
        ...
        self.log.error(
            "putRequest: unable to set request", "'%s'" % request.RequestName
        )
        errorsDict["Message"] = (
            "ReqClient.putRequest: unable to set request '%s'" % request.RequestName
        )
        return errorsDict
    ...
    def getRequest(self, requestID=0):
        ...
        if not getRequest["OK"]:
            self.log.error(
                "getRequest: unable to get request",
                "'%s' %s" % (requestID, getRequest["Message"]),
            )
            return getRequest
    ...
    def getBulkRequests(self, numberOfRequest=10, assigned=True):
        ...
        if not getRequests["OK"]:
            self.log.error(
                "getRequests: unable to get '%s' requests: %s"
    ...
    def peekRequest(self, requestID):
        ...
        if not peekRequest["OK"]:
            self.log.error(
                "peekRequest: unable to peek request",
                "request: '%s' %s" % (requestID, peekRequest["Message"]),
            )
            return peekRequest
    ...
    def getRequestIDsList(
        self, statusList=None, limit=None, since=None, until=None, getJobID=False
    ):
        ...
        return self._getRPC().getRequestIDsList(
            statusList, limit, since, until, getJobID
        )
    ...
    def getDBSummary(self):
        ...
        if not dbSummary["OK"]:
            self.log.error(
                "getDBSummary: unable to get RequestDB summary", dbSummary["Message"]
            )
        return dbSummary
    ...
    def getDigest(self, requestID):
        ...
        """
        self.log.debug(
            "getDigest: attempting to get digest for '%s' request." % requestID
        )
        digest = self._getRPC().getDigest(int(requestID))
        if not digest["OK"]:
            self.log.error(
                "getDigest: unable to get digest for request",
    ...
    def getRequestStatus(self, requestID):
        ...
        if isinstance(requestID, six.string_types):
            requestID = int(requestID)
        self.log.debug(
            "getRequestStatus: attempting to get status for '%d' request." % requestID
        )
    ...
    def getRequestInfo(self, requestID):
        ...
        """
        self.log.debug(
            "getRequestInfo: attempting to get info for '%s' request." % requestID
        )
    ...
    def getRequestFileStatus(self, requestID, lfns):
        ...
        """
        self.log.debug(
            "getRequestFileStatus: attempting to get file statuses for '%s' request."
            % requestID
        )
    ...
    def finalizeRequest(self, requestID, jobID, useCertificates=True):
        ...
        stateServer = RPCClient(
            "WorkloadManagement/JobStateUpdate", useCertificates=useCertificates
        )
        ...
        monitorServer = RPCClient(
            "WorkloadManagement/JobMonitoring", useCertificates=useCertificates
        )
        res = monitorServer.getJobSummary(int(jobID))
        if not res["OK"]:
            self.log.error(
                "finalizeRequest: Failed to get job status", "JobID: %d" % jobID
            )
            return res
        elif not res["Value"]:
            self.log.info(
                "finalizeRequest: job %d does not exist (anymore): finalizing" % jobID
            )
            return S_OK()
        else:
            ...
            stateUpdate = stateServer.setJobStatus(
                jobID, newJobStatus, "Requests done", "RMS"
            )
    ...
    def getRequestIDsForJobs(self, jobIDs):
        ...
        """
        self.log.verbose(
            "getRequestIDsForJobs: attempt to get request(s) for job %s" % jobIDs
        )
        res = self._getRPC().getRequestIDsForJobs(jobIDs)
        if not res["OK"]:
            self.log.error(
                "getRequestIDsForJobs: unable to get request(s) for jobs",
    ...
    def readRequestsForJobs(self, jobIDs):
        ...
        # Do not forget to cast back str keys to int
        successful = {
            int(jobID): Request(jsonReq)
            for jobID, jsonReq in ret["Successful"].iteritems()
        }
...
def printRequest(request, status=None, full=False, verbose=True, terse=False):
    ...
    if full:
        ...
        if request.OwnerDN:
            gLogger.always(
                "Owner: '%s', Group: %s" % (request.OwnerDN, request.OwnerGroup)
            )
...
def printFile(indexFile):
    ...
    gLogger.always(
        ...
        % (
            ...
            (" Checksum='%s'" % fi.Checksum)
            if fi.Checksum or (fi.Error and "checksum" in fi.Error.lower())
            else "",
...
def recoverableRequest(request):
    ...
    for op in request:
        if op.Status == "Failed" and (
            not op.Error
            or not [errStr for errStr in operationErrorsOK if errStr in op.Error]
        ):
            for fi in op:
                if fi.Status == "Failed":
