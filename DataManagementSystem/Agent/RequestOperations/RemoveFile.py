    ...
    def __call__(self):
        ...
        if targetSEs:
            ...
            bannedTargets = self.checkSEsRSS(
                targetSEs, access="RemoveAccess", failIfBanned=False
            )
        ...
        if bannedTargets and "always banned" in self.operation.Error:
            ...
            if not toRemoveDict:
                ...
                return S_OK(
                    "%s targets are always banned for removal"
        ...
        if toRemoveDict:
            ...
            if not bulkRemoval["OK"]:
                ...
                gMonitor.addMark(
                    "RemoveFileOK", len(toRemoveDict) - len(bulkRemoval["Value"])
                )
            ...
            for lfn, opFile in toRemoveDict.iteritems():
                ...
                if not singleRemoval["OK"]:
                    self.log.error(
                        "Error removing single file", singleRemoval["Message"]
                    )
        ...
        if bannedTargets:
            return S_OK(
                "%s targets are banned for removal" % ",".join(sorted(bannedTargets))
            )
        return S_OK()
        ...
        if "Write access not permitted for this credential" in opFile.Error:
            if "DataManager" in self.shifter:
                ...
                try:
                    ...
                    if not fileProxy["OK"]:
                        opFile.Error = (
                            "Error getting owner's proxy : %s" % fileProxy["Message"]
                        )
                    else:
