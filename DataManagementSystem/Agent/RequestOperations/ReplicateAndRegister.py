...
def filterReplicas(opFile, logger=None, dataManager=None):
    ...
    for repSEName in replicas:
        ...
        if error:
            log.warn(
                "unable to get metadata at %s for %s" % (repSEName, opFile.LFN),
                error.replace("\n", ""),
            )
...
class ReplicateAndRegister(DMSRequestOperationsBase):
    ...
    def __call__(self):
        ...
        if hasattr(self, "FTSMode") and getattr(self, "FTSMode"):
            bannedGroups = (
                getattr(self, "FTSBannedGroups")
                if hasattr(self, "FTSBannedGroups")
                else ()
            )
    ...
    def _addMetadataToFiles(self, toSchedule):
        ...
        if toSchedule:
            self.log.info(
                "found %s files to schedule, getting metadata from FC" % len(toSchedule)
            )
        else:
    ...
    def _checkExistingFTS3Operations(self):
        ...
        unfinishedFTSOperations = [
            ops
            for ops in existingFTSOperations
        ...
        rmsFileIDsToSetScheduled = set(
            [
                ftsFile.rmsFileID
                for ftsOp in unfinishedFTSOperations
    ...
    def fts3Transfer(self):
        ...
        for opFile in self.getWaitingFilesList():
            ...
            if validReplicas:
                ...
                if not validTargets:
                    self.log.info(
                        "file %s is already present at all targets" % opFile.LFN
                    )
                    opFile.Status = "Done"
                else:
        ...
        if fts3Files:
            ...
            if not ftsSchedule["OK"]:
                self.log.error(
                    "Completely failed to schedule to FTS3:", ftsSchedule["Message"]
                )
                return ftsSchedule
    ...
    def dmTransfer(self, fromFTS=False):
        ...
        for opFile in waitingFiles:
            ...
            for targetSE in self.operation.targetSEList:
                ...
                if targetSE in validReplicas:
                    ...
                    continue
                res = self.dm.replicateAndRegister(
                    lfn, targetSE, sourceSE=sourceSE, catalog=catalogs
                )
                if res["OK"]:
                    ...
                    if lfn in res["Value"]["Successful"]:
                        ...
                        if "replicate" in res["Value"]["Successful"][lfn]:
                            ...
                            self.log.error(
                                "Failed to replicate", "%s to %s" % (lfn, targetSE)
                            )
        ...
        if delayExecution:
            self.log.info(
                "Delay execution of the request by %d minutes" % delayExecution
            )
