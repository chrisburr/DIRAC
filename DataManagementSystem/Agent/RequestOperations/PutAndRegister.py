    ...
    def __call__(self):
        ...
        if len(targetSEs) != 1:
            ...
            self.operation.Error = (
                "Wrong parameters: TargetSE should contain only one targetSE"
            )
            ...
            return S_ERROR(
                "TargetSE should contain only one target, got %s" % targetSEs
            )
        ...
        for opFile in waitingFiles:
            ...
            if not putAndRegister["OK"]:
                ...
                self.log.error(
                    "Completely failed to put and register file",
            ...
            if lfn in putAndRegister["Failed"]:
                ...
                self.log.error(
                    "Failed to put and register file",
            ...
            if lfn in putAndRegister:
                ...
                if "register" not in putAndRegister[lfn]:
                    ...
                    self.log.info(
                        "put of %s to %s took %s seconds"
                        % (lfn, targetSE, putAndRegister[lfn]["put"])
                    )
                    self.log.error(
                        "Register of lfn to SE failed", "%s to %s" % (lfn, targetSE)
                    )
