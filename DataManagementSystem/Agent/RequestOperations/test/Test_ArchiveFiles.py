...
logging.basicConfig(
    level=logging.WARNING, format="%(levelname)-5s - %(name)-8s: %(message)s"
)
...
def archiveFiles(mocker, archiveRequestAndOp, multiRetValOK):
    ...
    af.fc.getReplicas.side_effect = partial(
        multiRetVal, Success={"SOURCE-SE": "PFN"}, Index=11
    )
    af.fc.isFile = mocker.MagicMock()
    archiveLFN = "/vo/tars/myTar.tar"
    af.fc.isFile.return_value = S_OK(
        {"Failed": {archiveLFN: "no file"}, "Successful": {}}
    )
...
def test_run_IgnoreMissingFiles(archiveFiles, _myMocker, listOfLFNs):
    archiveFiles.dm.getFile.side_effect = partial(
        multiRetVal, Index=5, Error="No such file or directory"
    )
    archiveFiles._run()
    archiveFiles.dm.getFile.assert_called_with(
        listOfLFNs[9],
...
def test_checkFilePermissions(archiveFiles, _myMocker):
    ...
    archiveFiles.fc.hasAccess.side_effect = partial(
        multiRetVal, Index=3, Error="Permission denied"
    )
...
def test_uploadTarBall_breaks(archiveFiles, _myMocker, listOfLFNs):
    ...
    archiveFiles.dm.putAndRegister.assert_called_with(
        "/vo/tars/myTar.tar", "myTar.tar", "TARBALL-SE"
    )
...
def test_cleanup(archiveFiles, mocker):
    osMocker = mocker.patch(
        MODULE + ".os.remove", side_effect=OSError("No such file or directory")
    )
    rmTreeMock = mocker.patch(
        MODULE + ".shutil.rmtree", side_effect=OSError("No such file or directory")
    )
...
def test_checkReplicas_success(archiveFiles):
    ...
    archiveFiles.fc.getReplicas.side_effect = partial(
        multiRetVal, Index=11, Success={"SOURCE-SE": "PFN"}
    )
...
def test_checkReplicas_notAt(archiveFiles):
    ...
    archiveFiles.lfns = [opFile.LFN for opFile in archiveFiles.waitingFiles]
    archiveFiles.fc.getReplicas.side_effect = partial(
        multiRetVal, Index=11, Success={"Not-SOURCE-SE": "PFN"}
    )
...
def test_registerDescendent_success(archiveFiles, listOfLFNs):
    ...
    archiveFiles.fc.addFileAncestors = Mock(
        name="AFA",
        return_value=S_OK({"Failed": {}, "Successful": {archiveLFN: "Done"}}),
    )
    assert archiveFiles._registerDescendent() is None
    archiveFiles.fc.addFileAncestors.assert_called_with(
        {archiveLFN: {"Ancestors": archiveFiles.lfns}}
    )
...
def test_registerDescendent_PartialFailure(archiveFiles, listOfLFNs):
    ...
    archiveFiles.fc.addFileAncestors = Mock(
        ...
        side_effect=(
            S_ERROR("Failure"),
            S_OK({"Failed": {}, "Successful": {archiveLFN: "Done"}}),
        ),
    )
