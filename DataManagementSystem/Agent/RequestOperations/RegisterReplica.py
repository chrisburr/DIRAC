    ...
    def __call__(self):
        ...
        for opFile in waitingFiles:
            ...
            if not registerReplica["OK"] or lfn in registerReplica["Value"]["Failed"]:
                ...
                if lfn in registerReplica.get("Value", {}).get(
                    "Successful", {}
                    ...
                    if isinstance(reason, dict):
                        ...
                        for failedCatalog in reason:
                            catMaster = (
                                catMaster
                                and FileCatalog()
                                ._getCatalogConfigDetails(failedCatalog)
                                .get("Value", {})
                                .get("Master", False)
                            )
                    ...
                    if (catalogs or catMaster) and (
                        ...
                        notExist = bool(
                            "No such file"
                            in res.get("Value", {}).get("Failed", {}).get(lfn, "")
                        )
                        if not notExist:
        ...
        if registerOperations:
            self.log.info(
                "adding %d operations to the request" % len(registerOperations)
            )
