...
class RemoveReplica(DMSRequestOperationsBase):
    ...
    def __call__(self):
        ...
        self.log.info(
            "Todo: %s replicas to delete from %s SEs"
        ...
        for targetSE in targetSEs:
            ...
            gMonitor.addMark(
                "RemoveReplicaOK",
                len(
                    [opFile for opFile in toRemoveDict.itervalues() if not opFile.Error]
                ),
            )
        ...
        for opFile in self.operation:
            if opFile.Status == "Waiting":
                errors = list(
                    set(
                        error
                        for error in removalStatus[opFile.LFN].itervalues()
                        if error
                    )
                )
                if errors:
                ...
                try:
                    ...
                    if not fileProxy["OK"]:
                        ...
                        if not removeReplica["OK"]:
                            ...
                            opFile.Error = removeReplica["Value"]["Failed"].get(
                                opFile.LFN, ""
                            )
                finally:
                    if proxyFile:
