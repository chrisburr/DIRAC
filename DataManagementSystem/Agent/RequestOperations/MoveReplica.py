...
class MoveReplica(DMSRequestOperationsBase):
    ...
    def __call__(self):
        ...
        if bannedTargets["Value"]:
            return S_OK(
                "%s targets are banned for removal" % ",".join(bannedTargets["Value"])
            )
        ...
        for opFile in waitingFiles:
            ...
            if not res["OK"]:
                ...
                self.log.info(
                    "todo: %s replicas to delete from %s sites"
    ...
    def dmRemoval(self, toRemoveDict, targetSEs):
        ...
        for targetSE in targetSEs:
            ...
            toRetry = dict(
                [(lfn, opFile) for lfn, opFile in bulkRemoval.items() if opFile.Error]
            )
        ...
        for opFile in self.operation:
            if opFile.Status == "Waiting":
                errors = list(
                    set([error for error in removalStatus[lfn].values() if error])
                )
                if errors:
    ...
    def dmTransfer(self, opFile):
        ...
        if noReplicas:
            ...
            gMonitor.addMark("ReplicateFail")
            self.log.error(
                "Unable to replicate", "%s, all replicas are missing" % (lfn)
            )
            ...
            gMonitor.addMark("ReplicateFail")
            self.log.error(
                "Unable to replicate", "%s, all replicas are corrupted" % (lfn)
            )
            ...
            self.log.error(
                "Unable to replicate",
                "%s, replicas corrupted at %s" % (lfn, someReplicasCorrupted[lfn]),
            )
