...
from DIRAC.RequestManagementSystem.private.OperationHandlerBase import (
...
class ArchiveFiles(OperationHandlerBase):
    ...
    def _run(self):
        ...
        self.parameterDict = DEncode.decode(self.operation.Arguments)[
            0
        ]  # tuple: dict, number of characters
    ...
    def _checkReplicas(self):
        ...
        for lfn, replInfo in resReplica["Value"]["Successful"].iteritems():
            if sourceSE in replInfo:
                ...
                self.log.warn(
                    "LFN %r not found at source, only at: %s"
    ...
    def _downloadFiles(self):
        ...
        for index, opFile in enumerate(self.waitingFiles):
            lfn = opFile.LFN
            self.log.info(
                "Processing file (%d/%d) %r" % (index, len(self.waitingFiles), lfn)
            )
            sourceSE = self.parameterDict["SourceSE"]
            ...
            while True:
                attempts += 1
                download = returnSingleResult(
                    self.dm.getFile(lfn, destinationDir=destFolder, sourceSE=sourceSE)
                )
                if download["OK"]:
                ...
                if attempts > 10:
                    ...
                    raise RuntimeError(
                        "Completely failed to download file: %s" % errorString
                    )
        return
    ...
    def _tarFiles(self):
        """Tar the files."""
        tarFileName = os.path.splitext(
            os.path.basename(self.parameterDict["ArchiveLFN"])
        )[0]
        baseDir = self.parameterDict["ArchiveLFN"].strip("/").split("/")[0]
        shutil.make_archive(
            tarFileName,
            format="tar",
            root_dir=self.cacheFolder,
            base_dir=baseDir,
            dry_run=False,
            logger=self.log,
        )
    ...
    def _registerDescendent(self):
        ...
        if not registerDescendents:
            self.log.verbose(
                "Will not register tarball as descendent to the Archived LFNs."
            )
            return
