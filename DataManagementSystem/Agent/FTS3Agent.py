...
class FTS3Agent(AgentModule):
    ...
    def __readConf(self):
        ...
        srvDict = res["Value"]
        serverPolicyType = opHelper().getValue(
            "DataManagement/FTSPlacement/FTS3/ServerPolicy", "Random"
        )
        self._serverPolicy = FTS3Utilities.FTS3ServerPolicy(
            srvDict, serverPolicy=serverPolicyType
        )
    ...
    def getFTS3Context(self, username, group, ftsServer, threadID):
        ...
        if not contextes.exists(idTuple, 15 * 60):
            ...
            res = FTS3Job.generateContext(
                ftsServer, proxyFile, lifetime=self.proxyLifetime
            )
    ...
    def _monitorJob(self, ftsJob):
        ...
        * update the FTSJob status
        ...
        try:
            ...
            if not res["OK"]:
                log.error(
                    "Error updating file fts status", "%s, %s" % (ftsJob.ftsGUID, res)
                )
                return ftsJob, res
    ...
    def monitorJobsLoop(self):
        ...
        * spawn a thread to monitor each of them
        ...
        res = self.fts3db.getActiveJobs(
            limit=self.jobBulkSize, jobAssignmentTag=self.assignmentTag
        )
    ...
    def _treatOperationCallback(returnedValue):
        ...
        log = gLogger.getSubLogger(
            "_treatOperationCallback/%s" % operation.operationID, child=True
        )
        if not res["OK"]:
    ...
    def _treatOperation(self, operation):
        ...
        try:
            threadID = current_process().name
            log = gLogger.getSubLogger(
                "treatOperation/%s" % operation.operationID, child=True
            )
            ...
            if operation.isTotallyProcessed():
                log.debug(
                    "FTS3Operation %s is totally processed" % operation.operationID
                )
                ...
                log.debug(
                    "FTS3Operation %s is not totally processed yet"
                ...
                if continueOperationProcessing:
                    ...
                    log.debug(
                        "FTS3Operation %s: %s new jobs to be submitted"
                    ...
                    for ftsJob in newJobs:
                        ...
                        try:
                            tpcProtocols = operation.fts3Plugin.selectTPCProtocols(
                                ftsJob=ftsJob
                            )
    ...
    def kickOperations(self):
        ...
        res = self.fts3db.kickStuckOperations(
            limit=self.maxKick, kickDelay=self.kickDelay
        )
        if not res["OK"]:
            return res
    ...
    def deleteOperations(self):
        ...
        res = self.fts3db.deleteFinalOperations(
            limit=self.maxDelete, deleteDelay=self.deleteDelay
        )
        if not res["OK"]:
            return res
