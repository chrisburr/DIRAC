...
class FTS3Job(JSerializable):
    ...
    def _constructTransferJob(
        self, pinTime, allLFNs, target_spacetoken, protocols=None
    ):
        ...
        if sourceIsTape:
            ...
            if srcProto not in srcSE.localStageProtocolList:
                ...
                if len(allLFNs) != 1:
                    log.debug(
                        "Multihop job has %s files while only 1 allowed" % len(allLFNs)
                    )
                    return S_ERROR(
                        errno.E2BIG, "Trying multihop job with more than one file !"
                    )

                res = srcSE.getURL(
                    allSrcDstSURLs, protocol=srcSE.localStageProtocolList
                )
    ...
    def _constructStagingJob(self, pinTime, allLFNs, target_spacetoken):
        ...
        log = gLogger.getSubLogger(
            "constructStagingJob/%s/%s" % (self.operationID, self.targetSE), True
        )
    ...
    def submit(
        self, context=None, ftsServer=None, ucert=None, pinTime=36000, protocols=None
    ):
        ...
        log = gLogger.getSubLogger(
            "submit/%s/%s_%s" % (self.operationID, self.sourceSE, self.targetSE), True
        )
        ...
        if self.type == "Transfer":
            res = self._constructTransferJob(
                pinTime, allLFNs, target_spacetoken, protocols=protocols
            )
            ...
            for ftsFile in self.filesToSubmit:
                ...
                if ftsFile.ftsGUID is not None:
                    log.warn(
                        ...
                        % (ftsFile.fileID, ftsFile.ftsGUID),
                    )
    ...
    def _fillAccountingDict(self, jobStatusDict):
        ...
        accountingDict["TransferTime"] = sum(
            int(fileDict["tx_duration"])
            for fileDict in successfulFiles
            if fileDict["tx_duration"]
        )
