...
class DirectoryListing(object):
    ...
    def addFile(self, name, fileDict, repDict, numericid):
        ...
        if "OwnerGroup" in fileDict:
            ...
            if groups:
                if len(groups) > 1:
                    gname = groups[0]
                    default_group = gConfig.getValue(
                        "/Registry/DefaultGroup", "unknown"
                    )
    ...
    def addDirectory(self, name, dirDict, numericid):
        ...
        self.entries.append(
            ("d" + self.__getModeString(perm), nlinks, uname, gname, size, date, name)
        )
