...
class DataManager(object):
    ...
    def __init__(self, catalogs=None, masterCatalogOnly=False, vo=False):
        ...
        if catalogs is None:
            catalogs = []
        catalogsToUse = (
            FileCatalog(vo=self.voName).getMasterCatalogNames()["Value"]
    ...
    def cleanLogicalDirectory(self, lfnDir):
        ...
        for folder in lfnDir:
            ...
            if not res["OK"]:
                log.debug(
                    "Failed to clean directory.", "%s %s" % (folder, res["Message"])
                )
    ...
    def __cleanDirectory(self, folder):
        ...
        listOfFolders = [
            aDir
            for aDir in areDirs["Value"]["Successful"]
            if areDirs["Value"]["Successful"][aDir]
        ]
        ...
        for lfn, reason in res["Value"]["Failed"].items():  # can be an iterator
            log.error(
                "Failed to remove file found in the catalog", "%s %s" % (lfn, reason)
            )
        ...
        for aFolder in sorted(listOfFolders, reverse=True):
            res = returnSingleResult(
                self.fileCatalog.removeDirectory(aFolder, recursive=True)
            )
    ...
    def __removeStorageDirectory(self, directory, storageElement):
        ...
        if not exists:
            log.debug(
                "The directory %s does not exist at %s " % (directory, storageElement)
            )
            return S_OK()
    ...
    def __getCatalogDirectoryContents(self, directories, includeDirectories=False):
        ...
        log.debug(
            "Obtaining the catalog contents for %d directories:" % len(directories)
        )
        ...
        while len(activeDirs) > 0:
            ...
            if not res["OK"]:
                log.debug(
                    "Problem getting the %s directory content" % currentDir,
                    res["Message"],
                )
            else:
    ...
    def getReplicasFromDirectory(self, directory):
        ...
        if not res["OK"]:
            return res
        allReplicas = dict(
            (lfn, metadata["Replicas"]) for lfn, metadata in res["Value"].items()
        )  # can be an iterator
    ...
    def getFilesFromDirectory(self, directory, days=0, wildcard="*"):
        ...
        log.debug(
            "Obtaining the files older than %d days in %d directories:"
        ...
        while len(activeDirs) > 0:
            ...
            if not res["OK"]:
                ...
                for subdir in subdirs:
                    if (not days) or _isOlderThan(
                        subdirs[subdir]["CreationDate"], days
                    ):
                        if subdir[0] != "/":
                ...
                for fileName in files:
                    ...
                    if (
                        (not days)
                        or not fileInfo.get("CreationDate")
                        or _isOlderThan(fileInfo["CreationDate"], days)
                    ):
    ...
    def __getFile(self, lfn, replicas, metadata, destinationDir, sourceSE=None):
        ...
        for storageElementName in sortedSEs:
            ...
            res = returnSingleResult(
                se.getFile(lfn, localPath=os.path.realpath(destinationDir))
            )
            ...
            if not res["OK"]:
                ...
                localFile = os.path.realpath(
                    os.path.join(destinationDir, os.path.basename(lfn))
                )
                ...
                if metadata["Size"] != res["Value"]:
                    errTuple = (
                        "Mismatch of sizes:",
                        "downloaded = %d, catalog = %d"
                        % (res["Value"], metadata["Size"]),
                    )
                    errToReturn = S_ERROR(DErrno.EFILESIZE, errTuple[1])

                elif (metadata["Checksum"]) and (
                    not compareAdler(metadata["Checksum"], localAdler)
                ):
                    errTuple = (
                        "Mismatch of checksums:",
                        "downloaded = %s, catalog = %s"
    ...
    def _getSEProximity(self, replicas):
        ...
        localSEs = [
            se
            for se in self.dmsHelper.getSEsAtSite(siteName).get("Value", [])
            if se in replicas
        ]
        countrySEs = []
        ...
        if res["OK"]:
            countrySEs = [
                se for se in res["Value"] if se in replicas and se not in localSEs
            ]
    ...
    def putAndRegister(
        ...
        if not res["OK"]:
            ...
            if not DErrno.cmpError(res, errno.EACCES):
                ...
                startTime = time.time()
                log.debug(
                    "putAndRegister: Sending accounting took %.1f seconds"
                    % (time.time() - startTime)
                )
        ...
        res = returnSingleResult(
            storageElement.getURL(lfn, protocol=self.registrationProtocol)
        )
        if not res["OK"]:
    ...
    def replicateAndRegister(
        self, lfn, destSE, sourceSE="", destPath="", localCache="", catalog=""
    ):
        ...
        if not res["OK"]:
            ...
            log.debug(errStr, res["Message"])
            failed[lfn] = {
                "Registration": {"LFN": lfn, "TargetSE": destSE, "PFN": destPfn}
            }
        else:
    ...
    def __replicate(self, lfn, destSEName, sourceSEName="", destPath="", localCache=""):
        ...
        possibleSourceSEs = sorted(
            possibleSourceSEs,
            key=lambda x: self.dmsHelper.isSameSiteSE(x, destSEName).get(
                "Value", False
            ),
            reverse=True,
        )
        ...
        for candidateSEName in possibleSourceSEs:
            ...
            if not res["OK"]:
                log.verbose(
                    "The storage element is not currently valid.",
            ...
            if not res["OK"]:
                log.debug(
                    "could not get fileSize on %s" % candidateSEName, res["Message"]
                )
                continue
            ...
            if seFileSize != catalogSize:
                log.debug(
                    "Catalog size and physical file size mismatch.",
            ...
            for compatibleProtocol in replicationProtocols:
                ...
                res = returnSingleResult(
                    candidateSE.getURL(lfn, protocol=compatibleProtocol)
                )
                if not res["OK"]:
                ...
                destURL = ""
                res = returnSingleResult(
                    destStorageElement.getURL(destPath, protocol=compatibleProtocol)
                )
                if not res["OK"]:
                ...
                if not res["OK"]:
                    log.debug(
                        "Replication failed",
                        "%s from %s to %s." % (lfn, candidateSEName, destSEName),
                    )
                    continue
        ...
        log.debug(
            "Will try intermediate transfer from %s sources"
        ...
        for candidateSE in possibleIntermediateSEs:
            ...
            if not res["OK"]:
                log.debug(
                    "Error getting the file from %s" % candidateSE.name, res["Message"]
                )
                continue
            ...
            if not res["OK"]:
                log.debug(
                    "Error putting file coming from %s" % candidateSE.name,
                    res["Message"],
                )
            ...
            res = returnSingleResult(
                destStorageElement.getURL(destPath, protocol=self.registrationProtocol)
            )
            if not res["OK"]:
                log.debug("Error getting the registration URL", res["Message"])
                # it's maybe pointless to try the other candidateSEs...
    ...
    def __registerReplica(self, replicaTuples, catalog):
        ...
        log.debug(
            "Successfully resolved %s replicas for registration." % len(replicaTuples)
        )
        # HACK!
        replicaDict = {}
    ...
    def removeFile(self, lfn, force=None):
        ...
        if force:
            ...
            failed = dict.fromkeys(
                (lfn for lfn in success if not success[lfn]),
        ...
        if lfns:
            ...
            if lfns:
                log.debug(
                    "Attempting to remove %d files from Storage and Catalogue. Get replicas first"
                    % len(lfns)
                )
        ...
        if not lfns:
            ...
            for lfn, repDict in replicaDict.items():  # can be an iterator
                if storageElementName not in repDict:
                    ...
                    log.debug(
                        "The replica you are trying to remove is the only one.",
    ...
    def __removeReplica(self, storageElementName, lfns, replicaDict=None):
        ...
        replicaTuples = [
            (lfn, replicaDict[lfn][storageElementName], storageElementName)
    ...
    def removeReplicaFromCatalog(self, storageElementName, lfn):
        ...
        if not lfns:
            return S_OK({"Successful": successful, "Failed": failed})
        log.debug(
            "Will remove catalogue entry for %s lfns at %s."
        ...
        log.debug(
            "Resolved %s pfns for catalog removal at %s."
            % (len(replicaTuples), storageElementName)
        )
        res = self.__removeCatalogReplica(replicaTuples)
        failed.update(res["Value"]["Failed"])
        successful.update(res["Value"]["Successful"])
    ...
    def __removePhysicalReplica(
        self, storageElementName, lfnsToRemove, replicaDict=None
    ):
        ...
        log.debug(
            "Attempting to remove %s pfns at %s."
        ...
        if not res["OK"]:
            ...
            return S_ERROR("%s %s" % (errStr, res["Message"]))
        oDataOperation = _initialiseAccountingObject(
            "removePhysicalReplica", storageElementName, len(lfnsToRemove)
        )
        oDataOperation.setStartTime()
        start = time.time()
        ...
        if not res["OK"]:
            ...
            deletedSize = sum(
                deletedSizes.get(lfn, 0) for lfn in res["Value"]["Successful"]
            )
        ...
        seStatus = dict(
            (
                ...
                (
                    ...
                    self.__checkSEStatus(se, status="TapeSE"),
                ),
            )
            for se in seList
        )
    ...
    def __filterReplicasForJobs(self, replicaDict):
        ...
        # Get a cache of SE statuses for long list of replicas
        seStatus = dict(
            (se, (self.dmsHelper.isSEForJobs(se), self.dmsHelper.isSEArchive(se)))
            for se in seList
        )
        # Beware, there is a del below
        for lfn, replicas in list(replicaDict["Successful"].items()):
    ...
    def checkActiveReplicas(self, replicaDict):
        ...
        if not isinstance(replicaDict, dict):
            return S_ERROR(
                "Wrong argument type %s, expected a dictionary" % type(replicaDict)
            )
        ...
        for key in ["Successful", "Failed"]:
            ...
            if not isinstance(replicaDict[key], dict):
                return S_ERROR(
                    "Wrong argument type %s, expected a dictionary"
                    % type(replicaDict[key])
                )
    ...
    def getReplicas(
        ...
        if not getUrl:
            ...
            for se in se_lfn:
                ...
                succPfn = (
                    seObj.getURL(se_lfn[se], protocol=self.registrationProtocol)
                    .get("Value", {})
                    .get("Successful", {})
                )
                for lfn in succPfn:
    ...
    def __executeIfReplicaExists(self, storageElementName, lfn, method, **kwargs):
        ...
        for lfn, reason in retDict["Failed"].items():  # can be an iterator
            log.error(
                "_callReplicaSEFcn: Failed to get replicas for file.",
                "%s %s" % (lfn, reason),
            )
        # # good replicas
        lfnReplicas = res["Value"]["Successful"]
    ...
    def getReplicaAccessUrl(self, lfn, storageElementName, protocol=False):
        ...
        :param bool singleFile: execute for the first LFN only
        """
        return self.__executeIfReplicaExists(
            storageElementName, lfn, "getURL", protocol=protocol
        )
    ...
    def getReplica(self, lfn, storageElementName, localPath=False):
        ...
        """
        return self.__executeIfReplicaExists(
            storageElementName, lfn, "getFile", localPath=localPath
        )
