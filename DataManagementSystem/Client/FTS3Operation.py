...
class FTS3Operation(JSerializable):
    ...
    def _checkSEAccess(seName, accessType, vo=None):
        ...
        if not status[accessType]:
            return S_ERROR(
                errno.EACCES,
                "%s does not have %s in Active or Degraded" % (seName, accessType),
            )
    ...
    def _updateRmsOperationStatus(self):
        ...
        if not res["OK"]:
            ...
            res = self.reqClient.putRequest(
                request, useFailoverProxy=False, retryMainService=3
            )
            if not res["OK"]:
        ...
        for ftsFile in self.ftsFiles:
            ...
            if ftsFile.status == "Canceled":
                log.info(
                    "File canceled, setting it Failed in RMS",
                    "%s %s" % (ftsFile.lfn, ftsFile.targetSE),
                )
                defunctRmsFileIDs.add(ftsFile.rmsFileID)
                continue
        ...
        for ftsFile in self.ftsFiles:
            ...
            opFile.Status = (
                "Failed" if ftsFile.rmsFileID in defunctRmsFileIDs else "Done"
            )
...
class FTS3TransferOperation(FTS3Operation):
    ...
    def prepareNewJobs(self, maxFilesPerJob=100, maxAttemptsPerFile=10):
        ...
        for targetSE, ftsFiles in filesGroupedByTarget.iteritems():
            ...
            # { sourceSE : [FTSFiles] }
            res = FTS3Utilities.selectUniqueSource(
                ftsFiles, self.fts3Plugin, allowedSources=sourceSEs
            )
            ...
            for ftsFile, errMsg in failedFiles.iteritems():
                log.error(
                    "Error when selecting random sources",
                ...
                if cmpError(errMsg, errno.ENOENT):
                    log.error(
                        "The file does not exist, setting it Defunct",
                        "%s" % ftsFile.lfn,
                    )
                    ftsFile.status = "Defunct"
            ...
            for sourceSE, ftsFiles in uniqueTransfersBySource.iteritems():
                ...
                for ftsFilesChunk in breakListIntoChunks(ftsFiles, maxFilesPerJob):
                    ...
                    newJob = self._createNewJob(
                        "Transfer", ftsFilesChunk, targetSE, sourceSE=sourceSE
                    )
    ...
    def __needsMultiHopStaging(self, sourceSEName, destSEName):
        ...
        tpcProtocols = self.fts3Plugin.selectTPCProtocols(
            sourceSEName=sourceSEName, destSEName=destSEName
        )

        res = dstSE.generateTransferURLsBetweenSEs(
            "/%s/fakeLFN" % self.vo, srcSE, protocols=tpcProtocols
        )
    ...
    def _callback(self):
        ...
        if status != "Scheduled":
            ...
            # and we do not do anything, keep ourselves pending
                ...
                return S_ERROR(
                    "Request with id %s is not Scheduled:%s" % (self.rmsReqID, status)
                )
        ...
        for target, ftsFileList in ftsFilesByTarget.iteritems():
            ...
            for ftsFile in ftsFileList:
                ...
                res = returnSingleResult(
                    targetSE.getURL(ftsFile.lfn, protocol=registrationProtocols)
                )
        ...
        return self.reqClient.putRequest(
            request, useFailoverProxy=False, retryMainService=3
        )
    ...
    def prepareNewJobs(self, maxFilesPerJob=100, maxAttemptsPerFile=10):
        ...
        for targetSE, ftsFiles in filesGroupedByTarget.iteritems():
            ...
            for ftsFilesChunk in breakListIntoChunks(ftsFiles, maxFilesPerJob):
                ...
                newJob = self._createNewJob(
                    "Staging", ftsFilesChunk, targetSE, sourceSE=targetSE
                )
                newJobs.append(newJob)
