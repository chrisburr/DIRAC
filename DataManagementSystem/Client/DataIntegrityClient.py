...
class DataIntegrityClient(Client):
    ...
    def reportProblematicReplicas(self, replicaTuple, se, reason):
        ...
        gLogger.info(
            "The following %s files had %s at %s" % (len(replicaTuple), reason, se)
        )
    ...
    def __updateReplicaToChecked(self, problematicDict):
        ...
        gLogger.info(
            "%s replica (%d) is updated to Checked status" % (prognosis, fileID)
        )
    ...
    def _reportProblematicFiles(self, lfns, reason):
        ...
        for lfn in sorted(lfns):
            gLogger.info(lfn)
        res = self.setFileProblematic(
            lfns, reason, sourceComponent="DataIntegrityClient"
        )
        if not res["OK"]:
