...
from DIRAC.DataManagementSystem.Client.CmdDirCompletion.AbstractFileSystem import (
    DFCFileSystem,
    UnixLikeFileSystem,
)
from DIRAC.DataManagementSystem.Client.CmdDirCompletion.DirectoryCompletion import (
    DirectoryCompletion,
)
...
class FileCatalogClientCLI(CLI):
            ...
            if fileList:
                if forceNonEmpty:
                    ...
                    if result["Value"]["Failed"]:
                        print(
                            "Error: failed to remove %d files"
                            % len(result["Value"]["Failed"])
                        )
        ...
        if result["OK"]:
            if result["Value"]["Successful"]:
                ...
                if newdir in result["Value"]["Failed"]:
                    print(
                        "Failed to create directory:", result["Value"]["Failed"][newdir]
                    )
        else:
                ...
                if result["Value"]["Successful"]:
                    for entry in result["Value"]["Successful"][path]["Files"]:
                        ...
                        if _long:
                            ...
                            repDict = result["Value"]["Successful"][path]["Files"][
                                entry
                            ].get("Replicas", {})
                            if fileDict:
                    ...
                    for entry in result["Value"]["Successful"][path]["SubDirs"]:
                        ...
                        if _long:
                            dirDict = result["Value"]["Successful"][path]["SubDirs"][
                                entry
                            ]
                            if dirDict:
                    ...
                    if "Datasets" in result["Value"]["Successful"][path]:
                        for entry in result["Value"]["Successful"][path]["Datasets"]:
                            ...
                            if _long:
                                dsDict = result["Value"]["Successful"][path][
                                    "Datasets"
                                ][entry]["Metadata"]
                                if dsDict:
    ...
    def complete_ls(self, text, line, begidx, endidx):
        ...
        if 1 + index_cnt <= len(args) <= 2 + index_cnt:
            # If last char is ' ',
            # this can be a new parameter.
            if (len(args) == 1 + index_cnt) or (
                len(args) == 2 + index_cnt and (not line.endswith(" "))
            ):
                cur_path = ""
    ...
    def complete_chown(self, text, line, begidx, endidx):
        ...
        if (1 + index_counter) <= len(args) <= (2 + index_counter):
            # If last char is ' ',
            # this can be a new parameter.
            if (len(args) == 1 + index_counter) or (
                len(args) == 2 + index_counter and (not line.endswith(" "))
            ):
                cur_path = ""
                    ...
                    if result["OK"]:
                        if result["Value"]["Successful"]:
                            print(
                                "Logical Size:",
                                int_with_commas(
                                    result["Value"]["Successful"][path]["LogicalSize"]
                                ),
                                "Files:",
                                result["Value"]["Successful"][path]["LogicalFiles"],
                                "Directories:",
                                result["Value"]["Successful"][path][
                                    "LogicalDirectories"
                                ],
                            )
                            if _long:
                                ...
                                if (
                                    "PhysicalSize"
                                    in result["Value"]["Successful"][path]
                                ):
                                    print()
                                    totalSize = result["Value"]["Successful"][path][
                                        "PhysicalSize"
                                    ]["TotalSize"]
                                    totalFiles = result["Value"]["Successful"][path][
                                        "PhysicalSize"
                                    ]["TotalFiles"]
                                    for se, sdata in result["Value"]["Successful"][
                                        path
                                    ]["PhysicalSize"].items():
                                        if not se.startswith("Total"):
                                            ...
                                            values.append(
                                                (se, int_with_commas(size), str(nfiles))
                                            )
                                    # print '='*60
                            ...
                            if "QueryTime" in result["Value"]:
                                print(
                                    "Query time %.2f sec" % result["Value"]["QueryTime"]
                                )
                        else:
                            print(
                                "Directory size failed:",
                                result["Value"]["Failed"][path],
                            )
                    else:
    ...
    def setMeta(self, argss):
        ...
        if len(argss) < 3 or len(argss) % 2 != 1:
            print(
                "Error: command requires at least 3 arguments (or odd number of arguments > 3), %d given"
                % len(argss)
            )
            return
        path = argss[0]
        ...
        if dirFlag:
            ...
            if result["Value"]:
                ...
                for meta, value in result["Value"].items():
                    setFlag = (
                        metaDict[meta] != "OwnParameter"
                        and metaTypeDict[meta] == "MetaSet"
                    )
                    prefix = ""
                    if setFlag:
                        prefix = "+"
                    ...
                    try:
                        ...
                        if name not in typeDict:
                            if name not in typeDictfm:
                                ...
                                print(
                                    "No support for meta data at File level yet: %s"
                                    % name
                                )
                            return
                        mtype = typeDict[name]
    ...
    def dataset_annotate(self, argss):
        ...
        result = returnSingleResult(
            self.fc.addDatasetAnnotation({datasetName: annotation})
        )
        if not result["OK"]:
        ...
        if not result["OK"]:
            ...
            if not changeDict:
                ...
                for par in changeDict:
                    print(
                        "   ", par, ": ", changeDict[par][0], "->", changeDict[par][1]
                    )
