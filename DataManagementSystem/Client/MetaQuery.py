...
class MetaQuery(object):
    ...
    def setMetaQuery(self, queryList, metaTypeDict=None):
        ...
        for arg in queryList:
            ...
            if "," in value:
                valueList = [
                    x.replace("'", "").replace('"', "") for x in value.split(",")
                ]
                mvalue = valueList
                ...
                if mtype[0:5].lower() == "float":
                    mvalue = [
                        float(x) for x in valueList if x not in ["Missing", "Any"]
                    ]
                    mvalue += [x for x in valueList if x in ["Missing", "Any"]]
                if operation == "=":
                    operation = "in"
                        ...
                        if op in metaDict[name]:
                            if isinstance(metaDict[name][op], list):
                                if isinstance(value, list):
                                    metaDict[name][op] = list(
                                        set(metaDict[name][op] + value)
                                    )
                                else:
                                    metaDict[name][op] = list(
                                        set(metaDict[name][op].append(value))
                                    )
                            else:
                                if isinstance(value, list):
                                    metaDict[name][op] = list(
    ...
    def applyQuery(self, userMetaDict):
        ...
        for meta, value in self.__metaQueryDict.items():
            ...
            try:
                ...
                return S_ERROR(
                    "Illegal type for metadata %s: %s in user data"
                    % (meta, str(userValue))
                )
            ...
            for operation, operand in getOperands(value):
                try:
                    ...
                    return S_ERROR(
                        "Illegal type for metadata %s: %s in filter"
                        % (meta, str(operand))
                    )
