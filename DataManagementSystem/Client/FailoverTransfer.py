...
class FailoverTransfer(object):
    ...
    def transferAndRegisterFile(
        ...
        for se in destinationSEList:
            ...
            for sleeptime in (10, 60, 300, 600):
                ...
                result = DataManager(
                    catalogs=fileCatalog, masterCatalogOnly=masterCatalogOnly
                ).putAndRegister(
            ...
            if not result["OK"]:
                self.log.error(
                    "dm.putAndRegister failed with message", result["Message"]
                )
            ...
            if "register" not in errorDict:
                self.log.error(
                    "dm.putAndRegister failed with unknown error", str(errorDict)
                )
            ...
            if not result["OK"]:
                self.log.error(
                    "Failed to set registration request",
                    "SE %s and metadata: \n%s" % (se, fileMetaDict),
                )
                errorList.append(
        ...
        self.log.error(
            "Failed to upload output data file",
    ...
    def transferAndRegisterFileFailover(
        ...
        result = self._setFileReplicationRequest(
            lfn, targetSE, fileMetaDict, sourceSE=failover["Value"]["uploadedSE"]
        )
        if not result["OK"]:
        ...
        self.log.info(
            "Attempting to set replica removal request",
    ...
    def _setFileReplicationRequest(self, lfn, targetSE, fileMetaDict, sourceSE=""):
        ...
        self.log.info(
            "Setting ReplicateAndRegister request", "for %s to %s" % (lfn, targetSE)
        )
    ...
    def _setRegistrationRequest(self, lfn, targetSE, fileDict, catalog):
        ...
        for cat in catalog:
            ...
            regFile.ChecksumType = fileDict.get(
                "ChecksumType", self.defaultChecksumType
            )
