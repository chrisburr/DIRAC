...
class ConsistencyInspector(object):
    ...
    def __init__(self, interactive=True, transClient=None, dm=None, fc=None, dic=None):
        ...
        self.transClient = (
            TransformationClient() if transClient is None else transClient
        )
    ...
    def __logVerbose(self, msg, msg1=""):
        ...
        if self._verbose:
            newMsg = (
                "[ConsistencyChecks] " + ("[%s] " % str(self.prod)) if self.prod else ""
            )
    ...
    def getReplicasPresenceFromDirectoryScan(self, lfns):
        ...
        for dirN in sorted(dirs):
            ...
            gLogger.verbose(
                "Obtained %d files in %.1f seconds"
        ...
        self.__write(" (%.1f seconds)\n" % (time.time() - startTime))
        gLogger.info(
            "Found %d files with replicas and %d without"
    ...
    def __compareLFNLists(self, lfns, lfnsFound):
        ...
        startTime = time.time()
        self.__logVerbose(
            "Comparing list of %d LFNs with second list of %d"
    ...
    def _getFilesFromDirectoryScan(self, dirs):
        ...
        if not res["OK"]:
            if "No such file or directory" not in res["Message"]:
                gLogger.error(
                    "Error getting files from directories %s:" % dirs, res["Message"]
                )
            return []
        if res["Value"]:
            lfnsFound = res["Value"]
    ...
    def __getDirectories(self):
        ...
        if self.directories:
            ...
            if printout:
                gLogger.always(
                    "Expanded list of %d directories:\n%s"
                    % (len(directories), "\n".join(directories))
                )
            return directories
        else:
    ...
    def compareChecksum(self, lfns):
        ...
        if lfnsLeft:
            self.__write(
                "Get replicas for %d files (chunks of %d): "
        ...
        nReps = 0
        for lfn in replicas:
            csDict.setdefault(lfn, {})["FCChecksum"] = metadata.get(lfn, {}).get(
                "Checksum"
            )
        ...
        gLogger.notice(
            "Getting checksum of %d replicas in %d SEs" % (nReps, len(seFiles))
        )
        checkSum = {}
        lfnNotExisting = {}
        lfnNoInfo = {}
            ...
            for surlChunk in breakListIntoChunks(seFiles[se], chunkSize):
                ...
                if not metadata["OK"]:
                    ...
                    for lfn in metadata["Successful"]:
                        checkSum.setdefault(lfn, {})[se] = metadata["Successful"][lfn][
                            "Checksum"
                        ]
            if notFound:
    ...
    def set_prod(self, value):
        ...
        if value:
            ...
            if not res["OK"]:
                S_ERROR(
                    errno.ENOENT,
                    "Couldn't find transformation %d: %s" % (value, res["Message"]),
                )
            else:
    ...
    def __checkPhysicalFileMetadata(self, lfns, se):
        ...
        gLogger.info(
            "Checking the integrity of %s physical files at %s" % (len(lfns), se)
        )
    ...
    def _getCatalogDirectoryContents(self, lfnDirs):
        ...
        def _getDirectoryContent(directory):
            ...
            gLogger.debug(
                "Files in %s: %d"
                % (directory, len(res["Value"]["Successful"][directory]["Files"]))
            )
        ...
        for lfnDir in lfnDirs:
            ...
            if not dirContent["OK"]:
                ...
                gLogger.debug(
                    "Content of directory %s: %d files"
