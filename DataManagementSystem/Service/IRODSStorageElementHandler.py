...
class IRODSStorageElementHandler(RequestHandler):
    ...
    def transfer_fromClient(self, fileID, token, fileSize, fileHelper):
        ...
        try:
            ...
            rcDisconnect(conn)
            return S_ERROR(
                "Cannot open to write destination file %s: %s" % (file_path, str(error))
            )

        if "NoCheckSum" in token:
            fileHelper.disableCheckSum()

        result = fileHelper.networkToDataSink(
            irodsFile, maxFileSize=(MAX_STORAGE_SIZE * 1024 * 1024)
        )
        irodsFile.close()
        rcDisconnect(conn)
        if not result["OK"]:
    ...
    def transfer_bulkFromClient(self, fileID, token, ignoredSize, fileHelper):
        ...
        try:
            ...
            gLogger.exception(
                "Could not set permissions of destination directory.", dir_path, error
            )
        return S_OK()
        ...
        if self.__confirmToken(token, fileID, "x"):
            try:
                ...
                if str(error).find("No such file") >= 0:
                    ...
                    return S_ERROR(
                        "Failed to remove file %s with exception %s" % (fileID, error)
                    )
        else:
    ...
    def export_getDirectorySize(self, fileID):
        ...
        if os.path.exists(dir_path):
            try:
                ...
                gLogger.exception(
                    "Exception while getting size of directory", dir_path, error
                )
    ...
    def __getDirectorySize(self, dir_path):
        ...
        raise NotImplementedError(
            "IRODSStorageElement__getDirectorySize is not implemented "
        )
    ...
    def __getUserDetails(self):
        ...
        irodsUser = gConfig.getValue(
            "%s/UserCredentials/%s/iRodsUser" % (cfgPath, diracUser), diracUser
        )
        irodsHome = gConfig.getValue(
            "%s/UserCredentials/%s/iRodsHome" % (cfgPath, diracUser), ""
        )
        irodsGroup = gConfig.getValue(
            "%s/UserCredentials/%s/iRodsGroup" % (cfgPath, diracUser), ""
        )
        irodsPassword = gConfig.getValue(
            "%s/UserCredentials/%s/iRodsPassword" % (cfgPath, diracUser), ""
        )
