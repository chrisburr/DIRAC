...
class DataIntegrityHandler(RequestHandler):
    ...
    def export_removeProblematic(fileID):
        ...
        if isinstance(fileID, list):
            ...
            fileIDs = [int(fileID)]
        gLogger.info(
            "DataIntegrityHandler.removeProblematic: Attempting to remove problematic."
        )
        res = gDataIntegrityDB.removeProblematic(fileIDs)
        if not res["OK"]:
            gLogger.error(
                "DataIntegrityHandler.removeProblematic: Failed to remove problematic.",
                res["Message"],
            )
        return res
    ...
    def export_getProblematic():
        ...
        if not res["OK"]:
            gLogger.error(
                "DataIntegrityHandler.getProblematic: Failed to get problematic file to resolve.",
                res["Message"],
            )
        return res
    ...
    def export_getPrognosisProblematics(prognosis):
        ...
        gLogger.info(
            "DataIntegrityHandler.getPrognosisProblematics: Getting files with %s prognosis."
            % prognosis
        )
        res = gDataIntegrityDB.getPrognosisProblematics(prognosis)
        if not res["OK"]:
            gLogger.error(
                "DataIntegrityHandler.getPrognosisProblematics: Failed to get prognosis files.",
                res["Message"],
            )
        return res
    ...
    def export_setProblematicStatus(fileID, status):
        ...
        gLogger.info(
            "DataIntegrityHandler.setProblematicStatus: Setting file %s status to %s."
            % (fileID, status)
        )
        res = gDataIntegrityDB.setProblematicStatus(fileID, status)
        if not res["OK"]:
            gLogger.error(
                "DataIntegrityHandler.setProblematicStatus: Failed to set status.",
                res["Message"],
            )
        return res
    ...
    def export_incrementProblematicRetry(fileID):
        ...
        gLogger.info(
            "DataIntegrityHandler.incrementProblematicRetry: Incrementing retries for file %s."
            % (fileID)
        )
        res = gDataIntegrityDB.incrementProblematicRetry(fileID)
        if not res["OK"]:
            gLogger.error(
                "DataIntegrityHandler.incrementProblematicRetry: Failed to increment retries.",
                res["Message"],
            )
        return res
    ...
    def export_insertProblematic(source, fileMetadata):
        ...
        gLogger.info(
            "DataIntegrityHandler.insertProblematic: Inserting problematic file to integrity DB."
        )
        res = gDataIntegrityDB.insertProblematic(source, fileMetadata)
        if not res["OK"]:
            gLogger.error(
                "DataIntegrityHandler.insertProblematic: Failed to insert.",
                res["Message"],
            )
        return res
    ...
    def export_changeProblematicPrognosis(fileID, newPrognosis):
        ...
        gLogger.info(
            "DataIntegrityHandler.changeProblematicPrognosis: Changing problematic prognosis."
        )
        res = gDataIntegrityDB.changeProblematicPrognosis(fileID, newPrognosis)
        if not res["OK"]:
            gLogger.error(
                "DataIntegrityHandler.changeProblematicPrognosis: Failed to update.",
                res["Message"],
            )
        return res
    ...
    def export_getTransformationProblematics(transID):
        ...
        gLogger.info(
            "DataIntegrityHandler.getTransformationProblematics: Getting problematics for transformation."
        )
        res = gDataIntegrityDB.getTransformationProblematics(transID)
        if not res["OK"]:
            gLogger.error(
                "DataIntegrityHandler.getTransformationProblematics: Failed.",
                res["Message"],
            )
        return res
    ...
    def export_getProblematicsSummary():
        ...
        gLogger.info(
            "DataIntegrityHandler.getProblematicsSummary: Getting problematics summary."
        )
        res = gDataIntegrityDB.getProblematicsSummary()
        if res["OK"]:
            for prognosis, statusDict in res["Value"].items():
                gLogger.info(
                    "DataIntegrityHandler.getProblematicsSummary: %s." % prognosis
                )
                for status, count in statusDict.items():
                    gLogger.info(
                        "DataIntegrityHandler.getProblematicsSummary: \t%-10s %-10s."
                        % (status, str(count))
                    )
        else:
            gLogger.error(
                "DataIntegrityHandler.getProblematicsSummary: Failed to get summary.",
                res["Message"],
            )
        return res
    ...
    def export_getDistinctPrognosis():
        ...
        gLogger.info(
            "DataIntegrityHandler.getDistinctPrognosis: Getting distinct prognosis."
        )
        res = gDataIntegrityDB.getDistinctPrognosis()
        if res["OK"]:
            for prognosis in res["Value"]:
                gLogger.info(
                    "DataIntegrityHandler.getDistinctPrognosis: \t%s." % prognosis
                )
        else:
            gLogger.error(
                "DataIntegrityHandler.getDistinctPrognosis: Failed to get unique prognosis.",
                res["Message"],
            )
        return res
