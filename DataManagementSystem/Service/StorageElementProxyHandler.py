...
from DIRAC.DataManagementSystem.private.HttpStorageAccessHandler import (
...
def initializeStorageElementProxyHandler(serviceInfo):
    ...
    if HTTP_FLAG:
        ...
        gLogger.info(
            "Creating HTTP server thread, port:%d, path:%s" % (HTTP_PORT, HTTP_PATH)
        )
...
class StorageElementProxyHandler(RequestHandler):
    ...
    def __proxyWrapper(self, se, name, args, kargs):
        ...
        method = (
            getattr(storageElement, name) if hasattr(storageElement, name) else None
        )
        if not method:
    ...
    def __uploadFile(self, se, pfn):
        ...
        if not res["OK"]:
            gLogger.error(
                "prepareFile: Failed to put local file to storage.", res["Message"]
            )
    ...
    def __prepareFile(self, se, pfn):
        ...
        if not res["OK"]:
            gLogger.error(
                "prepareFile: Failed to get local copy of file.", res["Message"]
            )
            return res
        return S_OK()
    ...
    def __prepareSecurityDetails(self):
        ...
        try:
            ...
            gLogger.debug(
                "Getting proxy for %s@%s (%s)" % (clientUsername, clientGroup, clientDN)
            )
