...
def initializeFileCatalogHandler(serviceInfo):
    ...
    dbLocation = getServiceOption(
        serviceInfo, "Database", "DataManagement/FileCatalogDB"
    )
    ...
    gMonitor.registerActivity(
        "RemoveFile",
        "Amount of removeFile calls",
        "FileCatalogHandler",
        "calls/min",
        gMonitor.OP_SUM,
    )
    gMonitor.registerActivity(
        "RemoveFileSuccessful",
    ...
    gMonitor.registerActivity(
        "AddReplica",
        "Amount of addReplica calls",
        "FileCatalogHandler",
        "calls/min",
        gMonitor.OP_SUM,
    )
    gMonitor.registerActivity(
        "AddReplicaSuccessful",
...
class FileCatalogHandler(RequestHandler):
    ...
    def export_changePathOwner(self, lfns, recursive=False):
        ...
        return gFileCatalogDB.changePathOwner(
            lfns, self.getRemoteCredentials(), recursive
        )
    ...
    def export_changePathGroup(self, lfns, recursive=False):
        """Get replica info for the given list of LFNs"""
        return gFileCatalogDB.changePathGroup(
            lfns, self.getRemoteCredentials(), recursive
        )
    ...
    def export_changePathMode(self, lfns, recursive=False):
        """Get replica info for the given list of LFNs"""
        return gFileCatalogDB.changePathMode(
            lfns, self.getRemoteCredentials(), recursive
        )
    ...
    def export_addFile(self, lfns):
        ...
        if res["OK"]:
            gMonitor.addMark(
                "AddFileSuccessful", len(res.get("Value", {}).get("Successful", []))
            )
            gMonitor.addMark(
                "AddFileFailed", len(res.get("Value", {}).get("Failed", []))
            )
    ...
    def export_removeFile(self, lfns):
        ...
        if res["OK"]:
            gMonitor.addMark(
                "RemoveFileSuccessful", len(res.get("Value", {}).get("Successful", []))
            )
            gMonitor.addMark(
                "RemoveFileFailed", len(res.get("Value", {}).get("Failed", []))
            )
    ...
    def export_addReplica(self, lfns):
        ...
        if res["OK"]:
            gMonitor.addMark(
                "AddReplicaSuccessful", len(res.get("Value", {}).get("Successful", []))
            )
            gMonitor.addMark(
                "AddReplicaFailed", len(res.get("Value", {}).get("Failed", []))
            )
    ...
    def export_removeReplica(self, lfns):
        ...
        if res["OK"]:
            ...
            gMonitor.addMark(
                "RemoveReplicaFailed", len(res.get("Value", {}).get("Failed", []))
            )
    ...
    def export_getFileAncestors(self, lfns, depths):
        ...
        return gFileCatalogDB.getFileAncestors(
            lfnDict, dList, self.getRemoteCredentials()
        )
    ...
    def export_getFileDescendents(self, lfns, depths):
        ...
        if not isinstance(dList, ListType):
            dList = [depths]
        lfnDict = dict.fromkeys(lfns, True)
        return gFileCatalogDB.getFileDescendents(
            lfnDict, dList, self.getRemoteCredentials()
        )
    ...
    def export_listDirectory(self, lfns, verbose):
        ...
        return gFileCatalogDB.listDirectory(
            lfns, self.getRemoteCredentials(), verbose=verbose
        )
    ...
    def export_getDirectorySize(self, lfns, longOut=False, fromFiles=False):
        """Get the size of the supplied directory"""
        return gFileCatalogDB.getDirectorySize(
            lfns, longOut, fromFiles, self.getRemoteCredentials()
        )
    ...
    def export_getDirectoryReplicas(self, lfns, allStatus=False):
        ...
        return gFileCatalogDB.getDirectoryReplicas(
            lfns, allStatus, self.getRemoteCredentials()
        )
    ...
    def export_deleteMetadataField(self, fieldName):
        ...
        result = gFileCatalogDB.dmeta.deleteMetadataField(
            fieldName, self.getRemoteCredentials()
        )
        error = ""
        if not result["OK"]:
            error = result["Message"]
        result = gFileCatalogDB.fmeta.deleteMetadataField(
            fieldName, self.getRemoteCredentials()
        )
        if not result["OK"]:
            if error:
    ...
    def export_getMetadataFields(self):
        ...
        if not resultDir["OK"]:
            return resultDir
        resultFile = gFileCatalogDB.fmeta.getFileMetadataFields(
        ...
        return S_OK(
            {
                "DirectoryMetaFields": resultDir["Value"],
    ...
    def export_setMetadata(self, path, metadatadict):
        ...
        return gFileCatalogDB.setMetadata(
            path, metadatadict, self.getRemoteCredentials()
        )
    ...
    def export_setMetadataBulk(self, pathMetadataDict):
        """Set metadata parameter for the given path"""
        return gFileCatalogDB.setMetadataBulk(
            pathMetadataDict, self.getRemoteCredentials()
        )
    ...
    def export_removeMetadata(self, pathMetadataDict):
        ...
        return gFileCatalogDB.removeMetadata(
            pathMetadataDict, self.getRemoteCredentials()
        )
    ...
    def export_getDirectoryUserMetadata(self, path):
        ...
        return gFileCatalogDB.dmeta.getDirectoryMetadata(
            path, self.getRemoteCredentials()
        )
    ...
    def export_getFileUserMetadata(self, path):
        ...
        return gFileCatalogDB.fmeta.getFileUserMetadata(
            path, self.getRemoteCredentials()
        )
    ...
    def export_findFilesByMetadata(self, metaDict, path="/"):
        ...
        result = gFileCatalogDB.fmeta.findFilesByMetadata(
            metaDict, path, self.getRemoteCredentials()
        )
        if not result["OK"]:
            return result
    ...
    def export_getReplicasByMetadata(self, metaDict, path="/", allStatus=False):
        """Find all the files satisfying the given metadata set"""
        return gFileCatalogDB.fileManager.getReplicasByMetadata(
    ...
    def export_findFilesByMetadataWeb(self, metaDict, path, startItem, maxItems):
        ...
        resultDetails = gFileCatalogDB.getFileDetails(
            lfnsResultList, self.getRemoteCredentials()
        )
    ...
    def export_getCompatibleMetadata(self, metaDict, path="/"):
        ...
        return gFileCatalogDB.dmeta.getCompatibleMetadata(
            metaDict, path, self.getRemoteCredentials()
        )
    ...
    def export_addMetadataSet(self, setName, setDict):
        ...
        return gFileCatalogDB.dmeta.addMetadataSet(
            setName, setDict, self.getRemoteCredentials()
        )
    ...
    def export_getMetadataSet(self, setName, expandFlag):
        """Add a new metadata set"""
        return gFileCatalogDB.dmeta.getMetadataSet(
            setName, expandFlag, self.getRemoteCredentials()
        )
    ...
    def export_addDataset(self, datasets):
        ...
        return gFileCatalogDB.datasetManager.addDataset(
            datasets, self.getRemoteCredentials()
        )
    ...
    def export_removeDataset(self, datasets):
        ...
        return gFileCatalogDB.datasetManager.removeDataset(
            datasets, self.getRemoteCredentials()
        )
    ...
    def export_checkDataset(self, datasets):
        """Check the given dynamic dataset for changes since its definition"""
        return gFileCatalogDB.datasetManager.checkDataset(
            datasets, self.getRemoteCredentials()
        )
    ...
    def export_updateDataset(self, datasets):
        ...
        return gFileCatalogDB.datasetManager.updateDataset(
            datasets, self.getRemoteCredentials()
        )
    ...
    def export_getDatasets(self, datasets):
        ...
        return gFileCatalogDB.datasetManager.getDatasets(
            datasets, self.getRemoteCredentials()
        )
    ...
    def export_getDatasetParameters(self, datasets):
        """Get parameters of the given dynamic dataset as they are stored in the database"""
        return gFileCatalogDB.datasetManager.getDatasetParameters(
            datasets, self.getRemoteCredentials()
        )
    ...
    def export_getDatasetAnnotation(self, datasets):
        ...
        return gFileCatalogDB.datasetManager.getDatasetAnnotation(
            datasets, self.getRemoteCredentials()
        )
    ...
    def export_freezeDataset(self, datasets):
        ...
        return gFileCatalogDB.datasetManager.freezeDataset(
            datasets, self.getRemoteCredentials()
        )
    ...
    def export_releaseDataset(self, datasets):
        ...
        return gFileCatalogDB.datasetManager.releaseDataset(
            datasets, self.getRemoteCredentials()
        )
    ...
    def export_getDatasetFiles(self, datasets):
        ...
        return gFileCatalogDB.datasetManager.getDatasetFiles(
            datasets, self.getRemoteCredentials()
        )
