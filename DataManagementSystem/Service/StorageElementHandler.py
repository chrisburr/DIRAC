...
def getDiskSpace(path, total=False):
    ...
    try:
        ...
        return S_ERROR(
            errno.EIO, "Error while getting the available disk space: %s" % repr(e)
        )
...
def getTotalDiskSpace():
    ...
    maxTotalSpace = (
        min(totalSpace, MAX_STORAGE_SIZE) if MAX_STORAGE_SIZE else totalSpace
    )
...
def initializeStorageElementHandler(serviceInfo):
    ...
    MAX_STORAGE_SIZE = convertSizeUnits(
        getServiceOption(serviceInfo, "MaxStorageSize", MAX_STORAGE_SIZE), "MB", "B"
    )
...
class StorageElementHandler(RequestHandler):
    ...
    def export_createDirectory(self, dir_path):
        ...
        gLogger.info(
            "StorageElementHandler.createDirectory: Attempting to create %s." % path
        )
        if os.path.exists(path):
            if os.path.isfile(path):
                errStr = "Supplied path exists and is a file"
                gLogger.error(
                    "StorageElementHandler.createDirectory: %s." % errStr, path
                )
                return S_ERROR(errStr)
            gLogger.info(
                "StorageElementHandler.createDirectory: %s already exists." % path
            )
            return S_OK()
