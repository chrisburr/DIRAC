    ...
    if result["OK"]:
        if result["Value"]["Successful"]:
            for entry in result["Value"]["Successful"][path]["Files"]:
                size = result["Value"]["Successful"][path]["Files"][entry]["MetaData"][
                    "Size"
                ]
...
def removeRemoteDirectory(fc, lfn):
    ...
    for storageElement in sorted(storageElements):
        ...
        if not res["OK"]:
            return S_ERROR(
                "Failed to clean storage directory at all SE:" + res["Message"]
            )
...
def doUpload(fc, dm, result, source_dir, dest_dir, storage, delete, nthreads):
    ...
    if delete:
        lfns = [
            dest_dir + "/" + filename for filename in result["Value"]["Delete"]["Files"]
        ]
        if len(lfns) > 0:
            res = removeRemoteFiles(dm, lfns)
            if not res["OK"]:
                gLogger.fatal(
                    "Deleting of files: " + lfns + " -X- [FAILED]" + res["Message"]
                )
                DIRAC.exit(1)
            else:
    ...
    res = runInParallel(
        arguments=[dm, source_dir, dest_dir, storage],
...
def uploadListOfFiles(dm, source_dir, dest_dir, storage, listOfFiles, tID):
    ...
    for filename in listOfFiles:
        res = uploadLocalFile(
            dm, dest_dir + "/" + filename, source_dir + "/" + filename, storage
        )
        if not res["OK"]:
            log.fatal(
                threadLine
...
def doDownload(dm, result, source_dir, dest_dir, delete, nthreads):
    ...
    if delete:
        for filename in result["Value"]["Delete"]["Files"]:
            ...
            if not res["OK"]:
                gLogger.fatal(
                    "Deleting of file: " + filename + " -X- [FAILED] " + res["Message"]
                )
                DIRAC.exit(1)
            else:
