...
Script.setUsageMessage(
    "\n".join(
        [
            __doc__,
            "Usage:",
            " %s [option|cfgfile] requestName LFN localFile targetSE"
            % Script.scriptName,
            "Arguments:",
...
if __name__ == "__main__":
    ...
    if not putRequest["OK"]:
        gLogger.error(
            "unable to put request '%s': %s" % (requestName, putRequest["Message"])
        )
        DIRAC.exit(-1)

    gLogger.always("Request '%s' has been put to ReqDB for execution." % requestName)
    gLogger.always(
        "You can monitor its status using command: 'dirac-rms-request %s'" % requestName
    )
    DIRAC.exit(0)
