...
if __name__ == "__main__":
    ...
    for se in allSEs:
        baseSE = gConfig.getOption("/Resources/StorageElements/%s/BaseSE" % se).get(
            "Value"
        )
        if baseSE:
    ...
    if not ret["OK"] or not ret.get("Value", ""):
        gLogger.error(
            "Aborting, Bad Proxy:", ret.get("Message", "Proxy does not belong to a VO!")
        )
        exit(1)
    vo = ret["Value"]
    ...
    for src, dst in ((x, y) for x in fromSE for y in targetSE):
        try:
            fts3TpcProto = fts3Plugin.selectTPCProtocols(
                sourceSEName=src, destSEName=dst
            )
        ...
        proto = ",".join(
            ses[dst].negociateProtocolWithOtherSE(ses[src], thirdPartyProtocols)[
                "Value"
            ]
        )
        if ftsOnly:
