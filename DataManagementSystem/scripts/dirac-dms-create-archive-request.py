...
class CreateArchiveRequest(object):
    ...
    def __init__(self):
        ...
        self.options = [
            ...
            (
                "P",
                "Path",
                "LFN path to folder, all files in the folder will be archived",
            ),
            (
                "N",
                "Name",
                "Name of the Tarball, if not given: Path_Tars/Path_N.tar"
                " will be used to store tarballs",
            ),
            (
                "L",
                "List",
                "File containing list of LFNs to archive, requires Name to be given",
            ),
            (
                "",
                "MaxFiles",
                "Maximum number to put in one tarball: Default %d" % MAX_FILES,
            ),
            (
                "",
                "MaxSize",
                "Maximum number of Bytes to put in one tarball: Default %d" % MAX_SIZE,
            ),
        ...
        for switch in Script.getUnprocessedSwitches():
            for short, longOption, doc in self.options:
                if switch[0] == short or switch[0].lower() == longOption.lower():
                    sLog.verbose(
                        "Found switch %r with value %r" % (longOption, switch[1])
                    )
    ...
    def getLFNList(self):
        ...
        if self.switches.get("List"):
            if os.path.exists(self.switches.get("List")):
                self.lfnList = list(
                    set(
                        [
                            line.split()[0]
                            for line in open(self.switches.get("List"))
                            .read()
                            .splitlines()
                        ]
                    )
            ...
            metaDict = (
                {"SE": self.sourceSEs[0]} if self.switches.get("SourceOnly") else {}
            )
    ...
    def putOrRunRequests(self):
        ...
        if self.dryRun:
            ...
            for reqID, req in enumerate(self.requests):
                ...
                for opID, op in enumerate(req):
                    sLog.notice(
                        "        Operation %d: %s #lfn %d" % (opID, op.Type, len(op))
                    )
            return 0
        for request in self.requests:
            ...
            if not putRequest["OK"]:
                sLog.error(
                    "unable to put request %r: %s"
                    % (request.RequestName, putRequest["Message"])
                )
                continue
            requestIDs.append(str(putRequest["Value"]))
            sLog.always(
                "Request %r has been put to ReqDB for execution." % request.RequestName
            )

        if requestIDs:
            sLog.always(
                "%d requests have been put to ReqDB for execution" % len(requestIDs)
            )
            sLog.always("RequestID(s): %s" % " ".join(requestIDs))
            sLog.always(
                "You can monitor the request status using the command: dirac-rms-request <requestName/ID>"
            )
            return 0
    ...
    def checkSwitches(self):
        ...
        if not self.name and self.lfnFolderPath:
            self.switches["AutoName"] = os.path.join(
                os.path.dirname(self.lfnFolderPath),
        ...
        if (
            self.switches.get("RemoveReplicas")
            and self.switches.get("ArchiveSE") is None
        ):
    ...
    def splitLFNsBySize(self):
        ...
        for lfn, info in self.metaData["Successful"].iteritems():
            if (
                totalSize > self.switches["MaxSize"]
                or len(lfnChunk) >= self.switches["MaxFiles"]
            ):
                self.lfnChunks.append(lfnChunk)
                sLog.notice(
                    "Created Chunk of %s lfns with %s bytes"
                    % (len(lfnChunk), totalSize)
                )
                lfnChunk = []
                totalSize = 0
            lfnChunk.append(lfn)
        ...
        self.lfnChunks.append(lfnChunk)
        sLog.notice(
            "Created Chunk of %s lfns with %s bytes" % (len(lfnChunk), totalSize)
        )

        self.replicaSEs = set(
            [
                seItem
                for se in self.fcClient.getReplicas(self.lfnList)["Value"][
                    "Successful"
                ].values()
    ...
    def run(self):
        ...
        from DIRAC.RequestManagementSystem.private.RequestValidator import (
            RequestValidator,
        )
    ...
    def checkArchive(self, archiveLFN):
        ...
        if not archiveLFN or not hasAccess["OK"] or not hasAccess["Value"]:
            ...
            raise ValueError(
                '%s is not a valid path, parameter "Name" must be correct' % archiveLFN
            )
    ...
    def _checkReplicaSites(self, request, lfnChunk):
        ...
        for lfn, replInfo in resReplica["Value"]["Successful"].iteritems():
            if sourceSE in replInfo:
                ...
                sLog.notice(
                    "WARN: LFN %r not found at source, only at: %s"
                    % (lfn, ",".join(replInfo.keys()))
                )
                notAt.append(lfn)
