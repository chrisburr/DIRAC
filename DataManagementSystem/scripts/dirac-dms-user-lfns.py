...
Script.registerSwitch(
    "D:", "Days=", "Match files older than number of days [%s]" % days
)
Script.registerSwitch(
    "M:", "Months=", "Match files older than number of months [%s]" % months
)
Script.registerSwitch(
    ...
    if not res["OK"]:
        ...
        if not subdirs and not files:
            ...
            for filename in sorted(files):
                fileOK = False
                if (not withMetadata) or isOlderThan(
                    files[filename]["MetaData"]["CreationDate"], totalDays
                ):
            ...
            if len(files) or len(subdirs):
                gLogger.notice(
                    ...
                    % (
                        currentDir,
                        len(files),
                        " matching" if withMetadata or wildcard else "",
                        len(subdirs),
                    )
                )
...
if emptyDirsFlag:
    ...
    outputFile.close()
    gLogger.notice(
        "%d empty directories have been put in %s" % (len(emptyDirs), outputFileName)
    )
