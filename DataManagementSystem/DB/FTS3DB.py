...
class FTS3DB(object):
    ...
    def getActiveJobs(self, limit=20, lastMonitor=None, jobAssignmentTag="Assigned"):
        ...
        try:
            ...
            if jobAssignmentTag:
                jobAssignmentTag += "_%s" % datetime.datetime.utcnow().strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
    ...
    def updateFileStatus(self, fileStatusDict, ftsGUID=None):
        ...
        for fileID, valueDict in fileStatusDict.iteritems():
            ...
            try:
                ...
                session.rollback()
                self.log.exception(
                    "updateFileFtsStatus: unexpected exception", lException=e
                )
    ...
    def cancelNonExistingJob(self, operationID, ftsGUID):
        ...
        try:
            ...
            session.rollback()
            self.log.exception(
                "cancelNonExistingJob: unexpected exception", lException=e
            )
    ...
    def getNonFinishedOperations(self, limit=20, operationAssignmentTag="Assigned"):
        ...
        try:
            ...
            if operationIDs:
                ...
                if operationAssignmentTag:
                    operationAssignmentTag += (
                        "_%s" % datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
                    )
        ...
        try:
            ...
            ftsOps = (
                ...
                .filter(
                    ...
                    < (
                        func.date_sub(
                            func.utc_timestamp(), text("INTERVAL %d HOUR" % kickDelay)
                        )
                    )
                )
                .filter(~FTS3Operation.assignment.is_(None))
        ...
        try:
            ...
            ftsOps = (
                ...
                .filter(
                    ...
                    < (
                        func.date_sub(
                            func.utc_timestamp(), text("INTERVAL %d DAY" % deleteDelay)
                        )
                    )
                )
