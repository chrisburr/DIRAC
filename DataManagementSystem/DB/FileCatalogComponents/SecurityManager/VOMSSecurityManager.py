...
class VOMSSecurityManager(SecurityManagerBase):
    ...
    def __testPermissionOnFile(self, paths, permission, credDict, noExistStrategy=None):
        ...
        for filename in paths:
            res = self.__getFilePermission(
                filename, credDict, noExistStrategy=noExistStrategy
            )
            if not res["OK"]:
    ...
    def __getDirectoryPermission(
        self, path, credDict, recursive=True, noExistStrategy=None
    ):
        ...
        if not res["OK"]:
            ...
            if recursive:
                return self.__getDirectoryPermission(
                    ...
                    credDict,
                    recursive=recursive,
                    noExistStrategy=noExistStrategy,
                )
    ...
    def __testPermissionOnDirectory(
        self, paths, permission, credDict, recursive=True, noExistStrategy=None
    ):
        ...
        for dirName in paths:
            res = self.__getDirectoryPermission(
                dirName, credDict, recursive=recursive, noExistStrategy=noExistStrategy
            )
            if not res["OK"]:
    ...
    def __testPermissionOnParentDirectory(
        self, paths, permission, credDict, recursive=True, noExistStrategy=None
    ):
    ...
    def __getFileOrDirectoryPermission(
        self, path, credDict, recursive=False, noExistStrategy=None
    ):
    ...
    def __testPermissionOnFileOrDirectory(
        self, paths, permission, credDict, recursive=False, noExistStrategy=None
    ):
    ...
    def __policyRemoveDirectory(self, paths, credDict):
        ...
        nonExistingDirectories = set(
            path
            for path in res["Value"]["Successful"]
        ...
        res = self.__testPermissionOnParentDirectory(
            existingDirs, "Write", credDict, recursive=False
        )
        if not res["OK"]:
            return res
    ...
    def __policyRemoveFile(self, paths, credDict):
        ...
        res = self.__testPermissionOnParentDirectory(
            existingFiles, "Write", credDict, recursive=False
        )
        if not res["OK"]:
            return res
    ...
    def __policyReadForFileAndDirectory(self, paths, credDict):
        ...
        return self.__testPermissionOnParentDirectory(
            paths, "Read", credDict, recursive=True
        )
    ...
    def __policyWriteForFileAndDirectory(self, paths, credDict):
        ...
        return self.__testPermissionOnParentDirectory(
            paths, "Write", credDict, recursive=True
        )
    ...
    def __policyReadForReplica(self, paths, credDict):
        ...
        :returns: Successful with True of False, and Failed.
        """
        return self.__testPermissionOnFile(
            paths, "Read", credDict, noExistStrategy=True
        )
    ...
    def __policyWriteForReplica(self, paths, credDict):
        ...
        """
        return self.__testPermissionOnFile(
            paths, "Write", credDict, noExistStrategy=True
        )
    ...
    def __policyChangePathMode(self, paths, credDict):
        ...
        return self.__testPermissionOnFileOrDirectory(
            paths, "Write", credDict, recursive=False, noExistStrategy=True
        )
