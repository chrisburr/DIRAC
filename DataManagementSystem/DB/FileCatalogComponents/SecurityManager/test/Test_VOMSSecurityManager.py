...
def setupTree():
    ...
    def setupFiles():
        ...
        fileTree["/users/usr1/sub1/usr1_secret.txt"] = makeNode(
            "usr1", "grp_user", 0o700
        )
...
nonExistingFiles = [
    "/realData/futurRun/futur_data.txt",
    "/fakeBaseDir/fake_file.txt",
    "/fake_base.txt",
]
...
class mock_DirectoryManager(object):
    ...
    def exists(self, lfns):
        return S_OK(
            {
                "Successful": dict((lfn, lfn in directoryTree) for lfn in lfns),
                "Failed": {},
            }
        )
    ...
    def getDirectoryParameters(self, path):
        return (
            S_OK(directoryTree[path])
            if path in directoryTree
            else S_ERROR("Directory not found")
        )
...
class mock_FileManager(object):
    ...
    def exists(self, lfns):
        return S_OK(
            {"Successful": dict((lfn, lfn in fileTree) for lfn in lfns), "Failed": {}}
        )
...
class BaseCaseMixin(object):
    ...
    def callForDirectories(self, methodName):
        ...
        self.existingRet = self.securityManager.hasAccess(
            methodName, directoryTree.keys(), self.credDict
        )
        self.nonExistingRet = self.securityManager.hasAccess(
            methodName, nonExistingDirectories, self.credDict
        )
    ...
    def callForFiles(self, methodName):
        ...
        self.existingRet = self.securityManager.hasAccess(
            methodName, fileTree.keys(), self.credDict
        )
        self.nonExistingRet = self.securityManager.hasAccess(
            methodName, nonExistingFiles, self.credDict
        )
    ...
    def compareResult(self):
        ...
        for testSet, real, expected in [
            ...
            for dic in ["Successful", "Failed"]:
                ...
                self.assertTrue(
                    notExpected == set(),
                    "(%s) Returned more keys in %s than expected %s"
                ...
                self.assertTrue(
                    notReturned == set(),
                    "Some keys in %s are missing %s" % (dic, notReturned),
                )
...
class TestNonExistingUser(BaseCaseMixin, unittest.TestCase):
    ...
    def test_removeDirectory(self):
        ...
        self.expectedExistingRet = S_OK(
            {"Successful": dict.fromkeys(directoryTree, False), "Failed": {}}
        )

        self.expectedNonExistingRet = S_OK(
            {"Successful": dict.fromkeys(nonExistingDirectories, True), "Failed": {}}
        )
    ...
    def test_createDirectory(self):
        ...
        self.expectedNonExistingRet = S_OK(
            {"Successful": dict.fromkeys(nonExistingDirectories, False), "Failed": {}}
        )
    ...
    def test_addFile(self):
        ...
        self.expectedExistingRet = S_OK(
            {"Successful": dict.fromkeys(fileTree, False), "Failed": {}}
        )

        self.expectedNonExistingRet = S_OK(
            {"Successful": dict.fromkeys(nonExistingFiles, False), "Failed": {}}
        )
    ...
    def test_removeFile(self):
        ...
        self.expectedNonExistingRet = S_OK(
            {"Successful": dict.fromkeys(nonExistingFiles, True), "Failed": {}}
        )
...
class TestAdminGrpAnonUser(BaseCaseMixin, unittest.TestCase):
    ...
    def test_listDirectory(self):
        ...
        self.expectedExistingRet = S_OK(
            {"Successful": dict.fromkeys(directoryTree, True), "Failed": {}}
        )
    ...
    def test_addFile(self):
        ...
        self.expectedExistingRet = S_OK(
            {"Successful": dict.fromkeys(fileTree, True), "Failed": {}}
        )
...
if __name__ == "__main__":
    ...
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(TestAdminGrpAnonUser)
    )
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(TestAdminGrpAdminUser)
    )
