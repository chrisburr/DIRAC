...
class DirectorySecurityManagerWithDelete(DirectorySecurityManager):
    ...
    def hasAccess(self, opType, paths, credDict):
        ...
        res = super(DirectorySecurityManagerWithDelete, self).hasAccess(
            opType, paths, credDict
        )
    ...
    def getPathPermissions(self, paths, credDict):
        ...
        if hasattr(self, "opType") and self.opType.lower() != "delete":
            return super(DirectorySecurityManagerWithDelete, self).getPathPermissions(
                paths, credDict
            )
        ...
        if paths:
            ...
            res = super(DirectorySecurityManagerWithDelete, self).getPathPermissions(
                paths, credDict
            )
            if not res["OK"]:
                return res
