...
class FileManagerPs(FileManagerBase):
    ...
    def _getDirectoryFiles(
        ...
        result = self.db.executeStoredProcedureWithCursor(
            "ps_get_all_info_for_files_in_dir",
            (dirID, specificFiles, formatedFileNames, allStatus, fStatus),
        )
        ...
        for row in rows:
            ...
            files[fileName] = dict(
                (key, rowDict.get(key, "Unknown metadata field")) for key in metadata
            )
    ...
    def __insertMultipleFiles(self, allFileValues, wantedLfns):
        ...
        for lfn in wantedLfns:
            ...
            fileDescStrings.append(
                "(DirID = %s AND FileName = '%s')" % (dirID, fileName)
            )
    ...
    def getLFNForGUID(self, guids, connection=False):
        ...
        formatedGuids = stringListToString(guids)
        result = self.db.executeStoredProcedureWithCursor(
            "ps_get_lfns_from_guids", (formatedGuids,)
        )
    ...
    def __deleteFiles(self, fileIDs, connection=False):
        ...
        result = self.db.executeStoredProcedureWithCursor(
            "ps_delete_files", (formatedFileIds,)
        )
        if not result["OK"]:
            return result
    ...
    def _getFileReplicas(
        self, fileIDs, fields_input=None, allStatus=False, connection=False
    ):
        ...
        for chunks in breakListIntoChunks(fileIDs, 1000):
            ...
            for row in rows:
                ...
                replicas[fileID][se] = dict(
                    (key, rowDict.get(key, "Unknown metadata field")) for key in fields
                )
    ...
    def countFilesInDir(self, dirId):
        ...
        result = self.db.executeStoredProcedure(
            "ps_count_files_in_dir", (dirId, "ret1"), outputIds=[1]
        )
        if not result["OK"]:
            return result
