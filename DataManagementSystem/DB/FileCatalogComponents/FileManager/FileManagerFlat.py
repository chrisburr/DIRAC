...
class FileManagerFlat(FileManagerBase):
    ...
    def _findFiles(self, lfns, metadata=["FileID"], connection=False):
        ...
        for dirPath in directoryIDs.keys():
            ...
            res = self._getDirectoryFiles(
                directoryIDs[dirPath], fileNames, metadata, connection=connection
            )
            if not res["OK"] or not res["Value"]:
                error = res.get("Message", "No such file or directory")
    ...
    def _getDirectoryFiles(
        ...
        req = "SELECT FileName,%s FROM FC_Files WHERE DirID=%d" % (
    ...
    def _getFileIDFromGUID(self, guid, connection=False):
        ...
        if not isinstance(guid, (list, tuple)):
            guid = [guid]
        req = (
            "SELECT FileID,GUID FROM FC_Files WHERE GUID IN (%s)"
    ...
    def __deleteFileReplicas(self, fileIDs, connection=False):
        ...
        if not fileIDs:
            return S_OK()
        req = "DELETE FROM FC_Replicas WHERE FileID in (%s)" % (
    ...
    def _insertReplicas(self, lfns, master=False, connection=False):
        ...
        if insertTuples:
            ...
            req = "INSERT INTO FC_Replicas (%s) VALUES %s" % (
                fields,
                ",".join(insertTuples.values()),
            )
            res = self.db._update(req, connection)
            if not res["OK"]:
    ...
    def __existsReplica(self, fileID, seID, connection=False):
        ...
        if isinstance(seID, six.string_types):
            ...
            if not res["OK"]:
                return res
            seID = res["Value"]
        req = "SELECT FileID FROM FC_Replicas WHERE FileID=%d AND SEID=%d" % (
            fileID,
            seID,
        )
    ...
    def __deleteReplicas(self, replicaTuples, connection=False):
        ...
        req = "DELETE FROM FC_Replicas WHERE (FileID,SEID) IN (%s)" % intListToString(
            deleteTuples
        )
        return self.db._update(req, connection)
    ...
    def _setReplicaStatus(self, fileID, se, status, connection=False):
        ...
        if not res["OK"]:
            return res
        statusID = res["Value"]
        return self._setReplicaParameter(
            fileID, se, "Status", statusID, connection=connection
        )
    ...
    def _setReplicaHost(self, fileID, se, newSE, connection=False):
        ...
        newSE = res["Value"]
        return self._setReplicaParameter(
            fileID, se, "SEID", newSE, connection=connection
        )

    def _setReplicaParameter(
        self, fileID, seID, paramName, paramValue, connection=False
    ):
        connection = self._getConnection(connection)
        if isinstance(seID, six.string_types):
            res = self.db.seManager.findSE(seID)
            ...
            seID = res["Value"]
        req = (
            "UPDATE FC_Replicas SET %s='%s', ModificationDate=UTC_TIMESTAMP() WHERE FileID=%d AND SEID=%d;"
            % (paramName, paramValue, fileID, seID)
        )
        return self.db._update(req, connection)
    ...
    def _setFileParameter(self, fileID, paramName, paramValue, connection=False):
        ...
        if not isinstance(fileID, (list, tuple)):
            fileID = [fileID]
        req = (
            "UPDATE FC_Files SET %s='%s', ModificationDate=UTC_TIMESTAMP() WHERE FileID IN (%s)"
