...
class FileManagerBase(object):
    ...
    def getFileCounters(self, connection=False):
        ...
        req = (
            "SELECT COUNT(FileID) FROM FC_Files WHERE DirID NOT IN ( SELECT DirID FROM %s)"
            % treeTable
        )
        res = self.db._query(req, connection)
        if not res["OK"]:
    ...
    def _getDirectoryFiles(
        self, dirID, fileNames, metadata, allStatus=False, connection=False
    ):
        """To be implemented on derived class"""
        return S_ERROR("To be implemented on derived class")
    ...
    def _addFiles(self, lfns, credDict, connection=False):
        ...
        if masterLfns:
            ...
            if not res["OK"]:
                for (
                    lfn
                ) in masterLfns.keys():  # pylint: disable=consider-iterating-dictionary
                    failed[lfn] = res["Message"]
                    masterLfns.pop(lfn)
            else:
        ...
        if extraLfns:
            res = self._findFiles(
                extraLfns.keys(), ["FileID", "DirID"], connection=connection
            )
            if not res["OK"]:
                for lfn in lfns.keys():
            ...
            if extraLfns:
                res = self._insertReplicas(
                    extraLfns, master=False, connection=connection
                )
                if not res["OK"]:
                    for (
                        lfn
        ...
        for directoryID in directorySEDict.keys():
            ...
            for seID in dirDict.keys():
                ...
                for dirID in parentIDs:
                    insertTuples.append(
                        "(%d,%d,%d,%d,UTC_TIMESTAMP())" % (dirID, seID, size, files)
                    )
    ...
    def _populateFileAncestors(self, lfns, connection=False):
        ...
        for lfn, lfnDict in lfns.iteritems():
            ...
            res = self._insertFileAncestors(
                originalFileID, toInsert, connection=connection
            )
            if not res["OK"]:
    ...
    def _insertFileAncestors(self, fileID, ancestorDict, connection=False):
        ...
        if not ancestorTuples:
            return S_OK()
        req = (
            "INSERT INTO FC_FileAncestors (FileID, AncestorID, AncestorDepth) VALUES %s"
    ...
    def _getFileAncestors(self, fileIDs, depths=[], connection=False):
        connection = self._getConnection(connection)
        req = (
            "SELECT FileID, AncestorID, AncestorDepth FROM FC_FileAncestors WHERE FileID IN (%s)"
                ...
                if not result["OK"]:
                    ...
                    if result["Value"]["Successful"]:
                        ...
                        for aID in result["Value"]["Successful"]:
                            resDict[result["Value"]["Successful"][aID]] = relDict[id_][
                                aID
                            ]
    ...
    def _getExistingMetadata(self, lfns, connection=False):
        ...
        res = self._findFiles(
            lfns, ["FileID", "Size", "Checksum", "GUID"], connection=connection
        )
        if not res["OK"]:
            return res
    ...
    def _checkExistingMetadata(self, existingLfns, lfns):
        ...
        if not res["OK"]:
            ...
            for fileID, lfn in fileIDLFNs.iteritems():
                ...
                if (
                    (existingGuid != newGuid)
                    or (existingSize != newSize)
                    or (existingChecksum != newChecksum)
                ):
    ...
    def _checkUniqueGUID(self, lfns, connection=False):
        ...
        for guid, fileID in res["Value"].iteritems():
            # resolve this to LFN
            failed[guidLFNs[guid]] = (
                "GUID already registered for another file %s" % fileID
            )
        return failed
    ...
    def setFileStatus(self, lfns, connection=False):
        ...
        for lfn in res["Value"]["Successful"].keys():
            ...
            res = self._setFileParameter(
                fileID, "Status", status, connection=connection
            )
            if not res["OK"]:
                failed[lfn] = res["Message"]
            else:
    ...
    def getPathPermissions(self, paths, credDict, connection=False):
        ...
        uid, gid = res["Value"]
        res = self._findFiles(
            paths, metadata=["Mode", "UID", "GID"], connection=connection
        )
        if not res["OK"]:
            return res
        successful = {}
    ...
    def getReplicasByMetadata(
        self, metaDict, path, allStatus, credDict, connection=False
    ):
    ...
    def getReplicaStatus(self, lfns, connection=False):
        ...
        if fileIDLFNs:
            res = self._getFileReplicas(
                fileIDLFNs.keys(), allStatus=True, connection=connection
            )
            if not res["OK"]:
                return res
            for fileID, seDict in res["Value"].iteritems():
