...
from DIRAC.DataManagementSystem.DB.FileCatalogComponents.FileManager.FileManagerBase import (
    FileManagerBase,
)
...
class FileManager(FileManagerBase):
    ...
    def _getDirectoryFiles(
        self, dirID, fileNames, metadata_input, allStatus=False, connection=False
    ):
        ...
        req = (
            "SELECT FileName,DirID,FileID,Size,UID,GID,Status FROM FC_Files WHERE DirID=%d"
            % (dirID)
        )
        if not allStatus:
            statusIDs = []
        ...
        for fileName, dirID, fileID, size, uid, gid, status in fileNameIDs:
            ...
            if "Status" in metadata:
                files[fileName]["Status"] = self._getIntStatus(status).get(
                    "Value", status
                )
    ...
    def _getDirectoryFileIDs(self, dirID, requestString=False):
        ...
        req = (
            "SELECT FileID,DirID,FileName FROM FC_Files WHERE DirID IN ( %s )"
            % dirListString
        )
    ...
    def _getFileMetadataByID(self, fileIDs, connection=False):
        ...
        req = (
            "SELECT FileID,Size,UID,GID,Status FROM FC_Files WHERE FileID in ( %s )"
            % stringIDs
        )
        result = self.db._query(req, connection)
        if not result["OK"]:
        ...
        req = (
            "SELECT FileID,GUID,CreationDate from FC_FileInfo WHERE FileID in ( %s )"
            % stringIDs
        )
        result = self.db._query(req, connection)
        if not result["OK"]:
    ...
    def _insertFiles(self, lfns, uid, gid, connection=False):
        ...
        for lfn in lfns.keys():
            ...
            insertTuples.append(
                "(%d,%d,%d,%d,%d,'%s')"
                % (dirID, size, s_uid, s_gid, statusID, fileName)
            )
        ...
        req = "INSERT INTO FC_Files (DirID,Size,UID,GID,Status,FileName) VALUES %s" % (
            ",".join(insertTuples)
        )
        ...
        if insertTuples:
            ...
            req = "INSERT INTO FC_FileInfo (%s) VALUES %s" % (
                fields,
                ",".join(insertTuples),
            )
            ...
            if not res["OK"]:
                ...
                result = self._updateDirectoryUsage(
                    directorySESizeDict, "+", connection=connection
                )
                if not result["OK"]:
                    gLogger.warn(
                        "Failed to insert FC_DirectoryUsage", result["Message"]
                    )
    ...
    def _getFileIDFromGUID(self, guid, connection=False):
        ...
        if not isinstance(guid, (list, tuple)):
            guid = [guid]
        req = (
            "SELECT FileID,GUID FROM FC_FileInfo WHERE GUID IN (%s)"
            % stringListToString(guid)
        )
        res = self.db._query(req, connection)
        if not res["OK"]:
            return res
    ...
    def getLFNForGUID(self, guids, connection=False):
        ...
        req += (
            " JOIN FC_Files f on fi.FileID = f.FileID WHERE GUID IN (%s)"
    ...
    def __deleteFiles(self, fileIDs, connection=False):
        ...
        for table in ["FC_Files", "FC_FileInfo"]:
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to remove files from table %s" % table, res["Message"]
                )
                failed.append(table)
        if failed:
            return S_ERROR(
                "Failed to remove files from %s" % stringListToString(failed)
            )
        return S_OK()
    ...
    def _insertReplicas(self, lfns, master=False, connection=False):
        ...
        req = "INSERT INTO FC_Replicas (FileID,SEID,Status) VALUES %s" % (
            ",".join(
                [
                    "(%d,%d,%d)" % (tuple_[0], tuple_[1], statusID)
                    for tuple_ in insertTuples
                ]
            )
        )
        res = self.db._update(req, connection)
        ...
        for lfn in lfns.keys():
            ...
            if repID:
                ...
                insertReplicas.append(
                    "(%d,'%s',UTC_TIMESTAMP(),UTC_TIMESTAMP(),'%s')"
                    % (repID, replicaType, pfn)
                )
        if insertReplicas:
            req = (
                "INSERT INTO FC_ReplicaInfo (RepID,RepType,CreationDate,ModificationDate,PFN) VALUES %s"
                % (",".join(insertReplicas))
            )
            res = self.db._update(req, connection)
            if not res["OK"]:
    ...
    def _getRepIDsForReplica(self, replicaTuples, connection=False):
        ...
        req = (
            "SELECT RepID,FileID,SEID FROM FC_Replicas WHERE (FileID,SEID) IN (%s)"
            % intListToString(queryTuples)
        )
        res = self.db._query(req, connection)
        if not res["OK"]:
            return res
    ...
    def _deleteReplicas(self, lfns, connection=False):
        ...
        successful = {}
        res = self._findFiles(
            lfns.keys(), ["DirID", "FileID", "Size"], connection=connection
        )
    ...
    def __deleteReplicas(self, repIDs, connection=False):
        ...
        for table in ["FC_Replicas", "FC_ReplicaInfo"]:
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to remove replicas from table %s" % table, res["Message"]
                )
                failed.append(table)
        if failed:
            return S_ERROR(
                "Failed to remove replicas from %s" % stringListToString(failed)
            )
        return S_OK()
    ...
    def _setReplicaParameter(self, fileID, se, paramName, paramValue, connection=False):
        ...
        repID = res["Value"]
        req = (
            "UPDATE FC_ReplicaInfo SET %s='%s', ModificationDate = UTC_TIMESTAMP() WHERE RepID IN (%d)"
            % (paramName, paramValue, repID)
        )
        return self.db._update(req, connection)
    ...
    def _setFileParameter(self, fileID, paramName, paramValue, connection=False):
        ...
        if paramName in ["UID", "GID", "Status", "Size"]:
            ...
            if "select" in fileIDString.lower():
                tmpreq = (
                    "UPDATE FC_Files as FF1, ( %s ) as FF2 %%s WHERE FF1.FileID=FF2.FileID"
                    % fileIDString
                )
            else:
            ...
            if "select" in fileIDString.lower():
                ...
                req += (
                    " SET ModificationDate=UTC_TIMESTAMP() WHERE FF1.FileID=FF2.FileID"
                )
            else:
                req = (
                    "UPDATE FC_FileInfo SET ModificationDate=UTC_TIMESTAMP() WHERE FileID IN (%s)"
                    % fileIDString
                )
        else:
            # Different statement for the fileIDString with SELECT is for performance optimization
            # since in this case the MySQL engine manages to use index on FileID.
            if "select" in fileIDString.lower():
                req = "UPDATE FC_FileInfo as FF1, ( %s ) as FF2 SET %s='%s', " % (
                    fileIDString,
                    paramName,
                    paramValue,
                )
                ...
                req = (
                    "UPDATE FC_FileInfo SET %s='%s', ModificationDate=UTC_TIMESTAMP() WHERE FileID IN (%s)"
    ...
    def _getFileReplicas(
        self, fileIDs, fields_input=["PFN"], allStatus=False, connection=False
    ):
        """Get replicas for the given list of files specified by their fileIDs"""
        fields = list(fields_input)
        connection = self._getConnection(connection)
        res = self.__getFileIDReplicas(
            fileIDs, allStatus=allStatus, connection=connection
        )
        if not res["OK"]:
            return res
    ...
    def __getFileIDReplicas(self, fileIDs, allStatus=False, connection=False):
        ...
        req = (
            "SELECT FileID,SEID,RepID,Status FROM FC_Replicas WHERE FileID IN (%s)"
    ...
    def _getDirectoryReplicas(self, dirID, allStatus=False, connection=False):
        ...
        if not self.db.lfnPfnConvention or self.db.lfnPfnConvention == "Weak":
            ...
            req += (
                " WHERE FF.FileID=FR.FileID AND FR.RepID=FI.RepID AND FF.DirID=%d "
                % dirID
            )
            if replicaStatusIDs:
    ...
    def repairFileTables(self, connection=False):
        ...
        for fileID in fileIDsToAdd:
            guid = makeGuid()
            insertTuples.append(
                "(%d,'%s',UTC_TIMESTAMP(),UTC_TIMESTAMP(),%d)"
