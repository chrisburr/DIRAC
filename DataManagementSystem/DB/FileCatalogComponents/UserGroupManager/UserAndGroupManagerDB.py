...
class UserAndGroupManagerDB(UserAndGroupManagerBase):
    ...
    def __addUser(self, uname):
        ...
        waitTime = time.time()
        gLogger.debug(
            "UserGroupManager AddUser lock created. Waited %.3f seconds. %s"
    ...
    def _refreshUsers(self):
        ...
        waitTime = time.time()
        gLogger.debug(
            "UserGroupManager RefreshUsers lock created. Waited %.3f seconds."
            % (waitTime - startTime)
        )
        ...
        if not res["OK"]:
            gLogger.debug(
                "UserGroupManager RefreshUsers lock released. Used %.3f seconds."
                % (time.time() - waitTime)
            )
            self.lock.release()
            return res
    ...
    def __addGroup(self, group):
        ...
        waitTime = time.time()
        gLogger.debug(
            "UserGroupManager AddGroup lock created. Waited %.3f seconds. %s"
    ...
    def _refreshGroups(self):
        ...
        waitTime = time.time()
        gLogger.debug(
            "UserGroupManager RefreshGroups lock created. Waited %.3f seconds."
            % (waitTime - startTime)
        )
        res = self.db._query(req)
        if not res["OK"]:
            gLogger.debug(
                "UserGroupManager RefreshGroups lock released. Used %.3f seconds."
                % (time.time() - waitTime)
            )
            self.lock.release()
            return res
