...
from DIRAC.DataManagementSystem.DB.FileCatalogComponents.DirectoryMetadata.DirectoryMetadata import (
    DirectoryMetadata,
)
...
class MultiVODirectoryMetadata(DirectoryMetadata):
    ...
    def addMetadataField(self, pName, pType, credDict):
        ...
        return super(MultiVODirectoryMetadata, self).addMetadataField(
            fname, pType, credDict
        )
    ...
    def deleteMetadataField(self, pName, credDict):
        ...
        """
        fname = _getMetaName(pName, credDict)
        return super(MultiVODirectoryMetadata, self).deleteMetadataField(
            fname, credDict
        )
    ...
    def setMetadata(self, dPath, metaDict, credDict):
        ...
        return super(MultiVODirectoryMetadata, self).setMetadata(
            dPath, fMetaDict, credDict
        )
    ...
    def removeMetadata(self, dPath, metaList, credDict):
        ...
        result = super(MultiVODirectoryMetadata, self).removeMetadata(
            dPath, metaList, credDict
        )
        if not result["OK"]:
    ...
    def setMetaParameter(self, dPath, metaName, metaValue, credDict):
        ...
        return super(MultiVODirectoryMetadata, self).setMetaParameter(
            dPath, fname, metaValue, credDict
        )
    ...
    """
        ...
        result = super(MultiVODirectoryMetadata, self).getDirectoryMetadata(
            path, credDict, inherited, ownData
        )
        if not result["OK"]:
            return result
    ...
    def findDirIDsByMetadata(self, metaDict, dPath, credDict):
        ...
        """
        fMetaDict = _getMetaNameDict(metaDict, credDict)
        return super(MultiVODirectoryMetadata, self).findDirIDsByMetadata(
            fMetaDict, dPath, credDict
        )
