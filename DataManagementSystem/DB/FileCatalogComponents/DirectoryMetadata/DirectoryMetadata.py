...
class DirectoryMetadata(object):
    ...
    def addMetadataField(self, pName, pType, credDict):
        ...
        if pName in result["Value"]:
            ...
            return S_ERROR(
                "Attempt to add an existing metadata with different type: %s/%s"
        ...
        req = (
            "CREATE TABLE FC_Meta_%s ( DirID INTEGER NOT NULL, Value %s, PRIMARY KEY (DirID), INDEX (Value) )"
            % (pName, valueType)
        )
        ...
        result = self.db.insertFields(
            "FC_MetaFields", ["MetaName", "MetaType"], [pName, pType]
        )
        if not result["OK"]:
            return result
    ...
    def getMetadataSet(self, metaSetName, expandFlag, credDict):
        ...
        req = (
            "SELECT S.MetaKey,S.MetaValue FROM FC_MetaSets as S, FC_MetaSetNames as N "
        )
    ...
    def setMetadata(self, dPath, metaDict, credDict):
        ...
        for metaName, metaValue in metaDict.iteritems():
            ...
            result = self.db.insertFields(
                "FC_Meta_%s" % metaName, ["DirID", "Value"], [dirID, metaValue]
            )
            if not result["OK"]:
    ...
    def removeMetadata(self, dPath, metaData, credDict):
        ...
        for meta in metaData:
            if meta in metaFields:
                ...
                # Meta parameter case
                req = "DELETE FROM FC_DirMeta WHERE MetaKey='%s' AND DirID=%d" % (
                    meta,
                    dirID,
                )
                result = self.db._update(req)
        ...
        if failedMeta:
            ...
            result = S_ERROR(
                "Failed to remove %d metadata, e.g. %s"
                % (len(failedMeta), failedMeta[metaExample])
            )
    ...
    def getDirectoryMetaParameters(self, dpath, credDict, inherited=True):
        ...
        if len(pathIDs) > 1:
            ...
            req = (
                "SELECT DirID,MetaKey,MetaValue from FC_DirMeta where DirID in (%s)"
                % pathString
            )
        else:
            req = (
                "SELECT DirID,MetaKey,MetaValue from FC_DirMeta where DirID=%d " % dirID
            )
        result = self.db._query(req)
        if not result["OK"]:
            return result
    ...
    def getDirectoryMetadata(self, path, credDict, inherited=True, ownData=True):
        ...
        for meta in metaFields:
            req = "SELECT Value,DirID FROM FC_Meta_%s WHERE DirID in (%s)" % (
                meta,
                pathString,
            )
            result = self.db._query(req)
            ...
            if len(result["Value"]) > 1:
                return S_ERROR(
                    "Metadata conflict for %s for directory %s" % (meta, path)
                )
            if result["Value"]:
    ...
    def __transformMetaParameterToData(self, metaName):
        ...
        req = "INSERT INTO FC_Meta_%s (DirID,Value) VALUES %s" % (
            metaName,
            ", ".join(insertValueList),
        )
        result = self.db._update(req)
        if not result["OK"]:
            return result
            ...
            for operation, operand in value.iteritems():
                if operation in [">", "<", ">=", "<="]:
                    if isinstance(operand, list):
                        return S_ERROR(
                            "Illegal query: list of values for comparison operation"
                        )
    ...
    def __checkDirsForMetadata(self, metaName, value, pathString):
        ...
        if selectString:
            ...
            req = "SELECT M.DirID FROM FC_Meta_%s AS M WHERE M.DirID IN (%s)" % (
                metaName,
                pathString,
            )
        result = self.db._query(req)
    ...
    def findDirIDsByMetadata(self, queryDict, path, credDict):
        ...
        if finalMetaDict:
            pathSelection = ""
            if pathDirID:
                result = self.db.dtree.getSubdirectoriesByID(
                    pathDirID, includeParent=True, requestString=True
                )
                if not result["OK"]:
                    return result
    ...
    def findFileIDsByMetadata(self, metaDict, path, credDict, startItem=0, maxItems=25):
        ...
        dirList = result["Value"]
        return self.db.dtree.getFileIDsInDirectoryWithLimits(
            dirList, credDict, startItem, maxItems
        )
