...
class DatasetManager(object):
    ...
    def __findFullPathDatasets(self, datasets, connection):
        ...
        for dirPath in directoryIDs:
            ...
            wheres.append(
                "( DirID=%d AND DatasetName IN (%s) )"
                % (dirID, stringListToString(dsNames))
            )

        req = (
            "SELECT DatasetName,DirID,DatasetID FROM FC_MetaDatasets WHERE %s"
            % " OR ".join(wheres)
        )
    ...
    def __findNoPathDatasets(self, nodirDatasets, connection):
        ...
        req += "( %s ) GROUP BY DatasetName,DatasetID" % stringListToString(
            nodirDatasets
        )
        result = self.db._query(req, connection)
        if not result["OK"]:
        ...
        if dsIDs:
            req = (
                "SELECT DatasetName,DatasetID,DirID FROM FC_MetaDatasets WHERE DatasetID in (%s)"
                % ",".join(dsIDs.values())
            )
            result = self.db._query(req, connection)
            if not result["OK"]:
    ...
    def addDatasetAnnotation(self, datasets, credDict):
        ...
        for dataset, annotation in datasets.iteritems():
            if dataset in datasetDict:
                req = (
                    "REPLACE FC_DatasetAnnotations (Annotation,DatasetID) VALUE ('%s',%d)"
    ...
    def getDatasetAnnotation(self, datasets, credDict={}):
        ...
        if datasets:
            ...
            req = (
                "SELECT DatasetID, Annotation FROM FC_DatasetAnnotations WHERE DatasetID in (%s)"
                % idString
            )
        else:
    ...
    def __removeDataset(self, datasetName, credDict):
        ...
        req = (
            "SELECT DatasetID FROM FC_MetaDatasets WHERE DatasetName='%s'" % datasetName
        )
        result = self.db._query(req)
        if not result["OK"]:
            return result
    ...
    def __checkDataset(self, datasetName, credDict):
        """Check that the dataset parameters correspond to the actual state"""
        req = (
            "SELECT MetaQuery,DatasetHash,TotalSize,NumberOfFiles FROM FC_MetaDatasets"
        )
    ...
    def getDatasetsInDirectory(self, dirID, verbose=False, connection=False):
        ...
        req = "SELECT %s FROM FC_MetaDatasets WHERE DirID=%s" % (
            parameterString,
            str(dirID),
        )
        result = self.db._query(req)
    ...
    def __getDatasetParameters(self, datasetName, credDict):
        ...
        req = "SELECT %s FROM FC_MetaDatasets WHERE DatasetName='%s' AND DirID=%d" % (
            parameterString,
            dsName,
            dirID,
        )
    ...
    def setDatasetStatus(self, datasetName, status):
        ...
        intStatus = result["Value"]
        req = (
            "UPDATE FC_MetaDatasets SET Status=%d, ModificationDate=UTC_TIMESTAMP() "
            % intStatus
        )
        req += "WHERE DatasetName='%s'" % datasetName
        result = self.db._update(req)
    ...
    def __freezeDataset(self, datasetName, credDict):
        ...
        req = (
            "INSERT INTO FC_MetaDatasetFiles (DatasetID,FileID) VALUES %s" % valueString
        )
        result = self.db._update(req)
        if not result["OK"]:
            return result
