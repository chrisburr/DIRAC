...
class DirectoryTreeBase(object):
    ...
    def _setDirectoryParameter(self, path, pname, pvalue):
        ...
        req = (
            "UPDATE FC_DirectoryInfo SET %s=%d, "
    ...
    def _changeDirectoryParameter(
        self, paths, directoryFunction, fileFunction, recursive=False
    ):
        ...
        for path, attribute in arguments.iteritems():
            ...
            if recursive:
                ...
                if not result["OK"]:
                    ...
                    continue
                dirID = result["Value"]
                result = self.getSubdirectoriesByID(
                    dirID, requestString=True, includeParent=True
                )
                if not result["OK"]:
                    failed[path] = result["Message"]
                    continue
                ...
                result = self.db.fileManager.getFileIDsInDirectory(
                    subDirQuery, requestString=True
                )
                if not result["OK"]:
                    failed[path] = result["Message"]
                    continue
    ...
    def getFileIDsInDirectoryWithLimits(
        self, dirID, credDict, startItem=1, maxItems=25
    ):
        ...
        req = (
            "SELECT COUNT( DirID ) FROM FC_Files USE INDEX (DirID) WHERE DirID IN ( %s )"
            % dirListString
        )
        ...
        req = "SELECT FileID FROM FC_Files WHERE DirID IN ( %s ) LIMIT %s, %s " % (
            dirListString,
            startItem,
            maxItems,
        )
    ...
    def _getDirectoryContents(self, path, details=False):
        ...
        if not result["OK"]:
            return result
        files = result["Value"]
        result = self.db.datasetManager.getDatasetsInDirectory(
            directoryID, verbose=details
        )
        if not result["OK"]:
            return result
    ...
    def getDirectoryReplicas(self, lfns, allStatus=False):
        ...
        for path in paths:
            ...
            if not result["OK"]:
                ...
                continue
            directoryID = result["Value"]
            result = self.db.fileManager.getDirectoryReplicas(
                directoryID, path, allStatus
            )
            if not result["OK"]:
                failed[path] = result["Message"]
                continue
        ...
        if longOutput:
            ...
            if rawFileTables:
                resultPhysical = self._getDirectoryPhysicalSize(
                    resultDict["Successful"], connection
                )
            else:
            ...
            for lfn in resultPhysical["Value"]["Successful"]:
                resultDict["Successful"][lfn]["PhysicalSize"] = resultPhysical["Value"][
                    "Successful"
                ][lfn]
        connection.close()
        resultDict["QueryTime"] = time.time() - start
    ...
    def _getDirectoryLogicalSizeFromUsage(self, lfns, connection):
        ...
        for path in paths:
            ...
            dirID = result["Value"]
            req = (
                "SELECT SESize, SEFiles FROM FC_DirectoryUsage WHERE SEID=0 AND DirID=%d"
                % dirID
            )
        ...
        for path in paths:
            ...
            if path == "/":
                ...
                if not result["OK"]:
                    ...
                    reqDir = dirString.replace(
                        "SELECT DirID FROM", "SELECT count(*) FROM"
                    )
            ...
            if not result["OK"]:
                ...
                for seID, seName, seSize, seFiles in result["Value"]:
                    if seSize or seFiles:
                        ...
                        req = (
                            "DELETE FROM FC_DirectoryUsage WHERE SEID=%d AND DirID=%d"
                            % (seID, dirID)
                        )
                        result = self.db._update(req)
                        if not result["OK"]:
        ...
        for path in paths:
            if path == "/":
                ...
                if not result["OK"]:
                    ...
                    req += (
                        "WHERE R.SEID=S.SEID AND F.FileID=R.FileID AND F.DirID=T.DirID "
                    )
                    req += "GROUP BY S.SEID"
    ...
    def __rebuildDirectoryUsageLeaves(self):
        ...
        gLogger.verbose(
            "Starting rebuilding Directory Usage, number of visible directories %d"
            % len(dirIDs)
        )
        ...
        for dirID in dirIDs:
            ...
            for seSize, seFiles, seID in result["Value"]:
                ...
                result = self.db.insertFields(
                    "FC_DirectoryUsage", insertFields, insertValues
                )
                if not result["OK"]:
                    if "Duplicate" in result["Message"]:
                        req = (
                            "UPDATE FC_DirectoryUsage SET SESize=%d, SEFiles=%d, LastUpdate=UTC_TIMESTAMP()"
                            % (seSize, seFiles)
                        )
            ...
            req = "SELECT SUM(Size),COUNT(Size) from FC_Files WHERE DirID=%d " % int(
                dirID
            )
            result = self.db._query(req)
            if not result["OK"]:
    ...
    def __rebuildDirectoryUsage(self, directoryID):
        ...
        req = (
            "SELECT SEID,SESize,SEFiles from FC_DirectoryUsage WHERE DirID=%d"
            % directoryID
        )
        result = self.db._query(req)
        if not result["OK"]:
    ...
    def getDirectoryCounters(self, connection=False):
        ...
        req = (
            "SELECT COUNT(DirID) FROM %s WHERE Parent NOT IN ( SELECT DirID from %s )"
            % (treeTable, treeTable)
        )
        req += " AND DirID <> 1"
        res = self.db._query(req, connection)
        ...
        req = (
            "SELECT COUNT(DirID) FROM %s WHERE DirID NOT IN ( SELECT Parent from %s )"
            % (treeTable, treeTable)
        )
        ...
        req = (
            "SELECT COUNT(DirID) FROM %s WHERE DirID NOT IN ( SELECT DirID FROM FC_DirectoryInfo )"
            % treeTable
        )
        res = self.db._query(req, connection)
        if not res["OK"]:
        ...
        req = (
            "SELECT COUNT(DirID) FROM FC_DirectoryInfo WHERE DirID NOT IN ( SELECT DirID FROM %s )"
            % treeTable
        )
        res = self.db._query(req, connection)
        if not res["OK"]:
