...
class DirectoryClosure(DirectoryTreeBase):
    ...
    def findDir(self, path, connection=False):
        ...
        result = self.db.executeStoredProcedure(
            "ps_find_dir", (dpath, "ret1", "ret2"), outputIds=[1, 2]
        )
        if not result["OK"]:
            return result
    ...
    def getDirectoryPath(self, dirID):
        ...
        result = self.db.executeStoredProcedure(
            "ps_get_dirName_from_id", (dirID, "out"), outputIds=[1]
        )
        if not result["OK"]:
            return result
    ...
    def getPathIDsByID(self, dirID):
        ...
        result = self.db.executeStoredProcedureWithCursor(
            "ps_get_parentIds_from_id", (dirID,)
        )
    ...
    def getSubdirectoriesByID(self, dirID, requestString=False, includeParent=False):
        ...
        if requestString:
            reqStr = (
                "SELECT ChildID FROM FC_DirectoryClosure WHERE ParentID = %s" % dirID
            )
        ...
        result = self.db.executeStoredProcedureWithCursor(
            "ps_get_sub_directories", (dirID, includeParent)
        )
        if not result["OK"]:
            return result
        if not result["Value"]:
    ...
    def getAllSubdirectoriesByID(self, dirIdList):
        ...
        dIds = intListToString(dirs)
        result = self.db.executeStoredProcedureWithCursor(
            "ps_get_multiple_sub_directories", (dIds,)
        )
    ...
    def makeDirectory(self, path, credDict, status=1):
        ...
        if parentDirId or path == "/":
            result = self.db.executeStoredProcedureWithCursor(
                packageName="ps_insert_dir",
                parameters=(parentDirId, dpath, l_uid, l_gid, self.db.umask, status),
            )
    ...
    def _getDirectoryPhysicalSize(self, lfns, connection):
        ...
        return self.__getPhysicalSize(
            lfns, "ps_calculate_dir_physical_size", connection
        )

    def _changeDirectoryParameter(
        self, paths, directoryFunction, _fileFunction, recursive=False
    ):
