...
from DIRAC.DataManagementSystem.DB.FileCatalogComponents.DirectoryManager.DirectoryTreeBase import (
    DirectoryTreeBase,
)
...
class DirectoryLevelTree(DirectoryTreeBase):
    ...
    def findDirs(self, paths, connection=False):
        ...
        req = (
            "SELECT DirName,DirID from FC_DirectoryLevelTree WHERE DirName in (%s)"
            % dpaths
        )
        result = self.db._query(req, connection)
        if not result["OK"]:
    ...
    def __getNumericPath(self, dirID, connection=False):
        ...
        req = "SELECT LEVEL,%s FROM FC_DirectoryLevelTree WHERE DirID=%d" % (
            epathString,
            dirID,
        )
        result = self.db._query(req, connection)
    ...
    def makeDir(self, path):
        ...
        if parentDirID:
            ...
            lPath = "LPATH%d" % (level)
            req = (
                " SELECT @tmpvar:=max(%s)+1 FROM FC_DirectoryLevelTree WHERE Parent=%d FOR UPDATE; "
                % (lPath, parentDirID)
            )
            resultLock = self.db._query("START TRANSACTION; ", conn)
            result = self.db._query(req, conn)
            req = "UPDATE FC_DirectoryLevelTree SET %s=@tmpvar WHERE DirID=%d; " % (
                lPath,
                dirID,
            )
            result = self.db._update(req, conn)
    ...
    def getDirectoryPaths(self, dirIDList):
        ...
        req = (
            "SELECT DirID,DirName FROM FC_DirectoryLevelTree WHERE DirID in ( %s )"
            % dirListString
        )
        result = self.db._query(req)
        if not result["OK"]:
    ...
    def getPathIDs(self, path):
        ...
        req = (
            "SELECT DirID FROM FC_DirectoryLevelTree WHERE DirName in (%s) ORDER BY DirID"
            % ",".join(pathString)
        )
        result = self.db._query(req)
        if not result["OK"]:
    ...
    def getPathIDsByID(self, dirID):
        ...
        for lev in xrange(level):
            sel = " AND ".join(
                ["Level=%d" % lev]
                + ["LPATH%d=%d" % (ll + 1, lpaths[ll]) for ll in range(lev)]
            )
            lpathSelects.append(sel)
        selection = "(" + ") OR (".join(lpathSelects) + ")"
        req = (
            "SELECT Level,DirID from FC_DirectoryLevelTree WHERE %s ORDER BY Level"
            % selection
        )
        result = self.db._query(req)
        if not result["OK"]:
    ...
    def getSubdirectoriesByID(self, dirID, requestString=False, includeParent=False):
        ...
        if level > 0:
            ...
            req += (
                " JOIN (SELECT %s FROM FC_DirectoryLevelTree WHERE DirID=%d) AS F2 ON "
                % (pathString, dirID)
            )
            sPaths = []
            for i in range(1, level + 1):
    ...
    def countSubdirectories(self, dirId, includeParent=True):
        result = self.getSubdirectoriesByID(
            dirId, requestString=True, includeParent=includeParent
        )
        if not result["OK"]:
            return result
    ...
    def getAllSubdirectoriesByID(self, dirList):
        ...
        while parentList:
            ...
            req = (
                "SELECT DirID from FC_DirectoryLevelTree WHERE Parent in ( %s )"
                % dirListString
            )
            result = self.db._query(req)
            if not result["OK"]:
    ...
    def recoverOrphanDirectories(self, credDict):
        ...
        req = (
            "SELECT DirID,Parent,Level FROM %s WHERE Parent NOT IN ( SELECT DirID from %s )"
            % (treeTable, treeTable)
        )
        result = self.db._query(req)
        if not result["OK"]:
        ...
        for parentPath, dirDict in parentDict.iteritems():
            ...
            req = "UPDATE FC_DirectoryLevelTree SET Parent=%s WHERE DirID IN (%s)" % (
                parentID,
                dirString,
            )
            result = self.db._update(req)
            ...
            result = self.db._query(
                "LOCK TABLES FC_DirectoryLevelTree WRITE", connection
            )
            if not result["OK"]:
    ...
    def __rebuildLevelIndexes(self, parentID, connection=False):
        ...
        for dirID in subIDList:
            indexList[-1] += 1
            lpaths = [
                "LPATH%d=%d" % (i + 1, indexList[i]) for i in range(parentLevel + 1)
            ]
            lpathString = "SET " + ",".join(lpaths)
            req = "UPDATE FC_DirectoryLevelTree %s WHERE DirID=%s" % (
                lpathString,
                dirID,
            )
