...
class FileMetadata(object):
    ...
    def addMetadataField(self, pName, pType, credDict):
        ...
        result = self.db.insertFields(
            "FC_FileMetaFields", ["MetaName", "MetaType"], [pName, pType]
        )
        if not result["OK"]:
            return result
    ...
    def setMetadata(self, path, metaDict, credDict):
        ...
        for metaName, metaValue in metaDict.iteritems():
            if metaName not in metaFields:
                result = self.__setFileMetaParameter(
                    fileID, metaName, metaValue, credDict
                )
            else:
                result = self.db.insertFields(
                    "FC_FileMeta_%s" % metaName,
                    ["FileID", "Value"],
                    [fileID, metaValue],
                )
                if not result["OK"]:
    ...
    def removeMetadata(self, path, metadata, credDict):
        ...
        for meta in metadata:
            if meta in metaFields:
                ...
                # Meta parameter case
                req = "DELETE FROM FC_FileMeta WHERE MetaKey='%s' AND FileID=%d" % (
                    meta,
                    fileID,
                )
                result = self.db._update(req)
    ...
    def _getFileUserMetadataByID(self, fileIDList, credDict, connection=False):
        ...
        for meta in metaFields:
            req = "SELECT Value,FileID FROM FC_FileMeta_%s WHERE FileID in (%s)" % (
                meta,
                stringIDs,
            )
        ...
        req = (
            "SELECT FileID,MetaKey,MetaValue from FC_FileMeta where FileID in (%s)"
            % stringIDs
        )
        result = self.db._query(req, conn=connection)
        if not result["OK"]:
    ...
    def getFileUserMetadata(self, path, credDict):
        ...
        metaTypeDict = {}
        for meta in metaFields:
            req = "SELECT Value,FileID FROM FC_FileMeta_%s WHERE FileID=%d" % (
                meta,
                fileID,
            )
            result = self.db._query(req)
    ...
    def __getFileMetaParameters(self, fileID, credDict):
        ...
        req = (
            "SELECT FileID,MetaKey,MetaValue from FC_FileMeta where FileID=%d " % fileID
        )
        result = self.db._query(req)
        if not result["OK"]:
            return result
    ...
    def __transformMetaParameterToData(self, metaName):
        ...
        req = "INSERT INTO FC_FileMeta_%s (FileID,Value) VALUES %s" % (
            metaName,
    ...
    def __buildStandardMetaQuery(self, standardMetaDict):
        ...
        for infield, invalue in standardMetaDict.iteritems():
            ...
            if infield in FILES_TABLE_METAKEYS:
                ...
                for operation, operand in result["Value"]:
                    queriesFiles.append(
                        "%s.%s %s %s" % (tableIndex, field, operation, operand)
                    )
        ...
        for table, condition in tablesAndConditions:
            if table == "FC_FileInfo":
                ...
                if table in leftJoinTables:
                    ...
                    tables.append(
                        "INNER JOIN %s M%d USING( FileID )" % (table, counter)
                    )
                table = "M%d" % counter
    ...
    def findFilesByMetadata(self, metaDict, path, credDict):
        ...
        fileMetaDict = dict(
            item for item in metaDict.iteritems() if item[0] in fileMetaKeys
        )
        ...
        if dirFlag != "None":
            ...
            if fileMetaDict:
                ...
                result = self.db.dtree.getFileLFNsInDirectoryByDirectory(
                    dirList, credDict
                )
                if not result["OK"]:
                    return result
