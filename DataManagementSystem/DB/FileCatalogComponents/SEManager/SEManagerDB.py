...
from DIRAC.DataManagementSystem.DB.FileCatalogComponents.SEManager.SEManagerBase import (
    SEManagerBase,
)
...
class SEManagerDB(SEManagerBase):
    def _refreshSEs(self, connection=False):
        ...
        if seNames == set(self.db.seNames) and (
            (time.time() - self.lastUpdate) < self.seUpdatePeriod
        ):
            return S_OK()
        ...
        waitTime = time.time()
        gLogger.debug(
            "SEManager RefreshSEs lock created. Waited %.3f seconds."
            % (waitTime - startTime)
        )
        ...
        gLogger.debug(
            "SEManager RefreshSEs lock released. Used %.3f seconds."
            % (time.time() - waitTime)
        )
    ...
    def __addSE(self, seName, connection=False):
        ...
        waitTime = time.time()
        gLogger.debug(
            "SEManager AddSE lock created. Waited %.3f seconds. %s"
            % (waitTime - startTime, seName)
        )
        if seName in self.db.seNames.keys():
            seid = self.db.seNames[seName]
            gLogger.debug(
                "SEManager AddSE lock released. Used %.3f seconds. %s"
                % (time.time() - waitTime, seName)
            )
            self.lock.release()
            return S_OK(seid)
        connection = self.db._getConnection()
        res = self.db.insertFields(
            "FC_StorageElements", ["SEName"], [seName], connection
        )
        if not res["OK"]:
            gLogger.debug(
                "SEManager AddSE lock released. Used %.3f seconds. %s"
    ...
    def __removeSE(self, seName, connection=False):
        ...
        waitTime = time.time()
        gLogger.debug(
            "SEManager RemoveSE lock created. Waited %.3f seconds. %s"
            % (waitTime - startTime, seName)
        )
        ...
        if not res["OK"]:
            gLogger.debug(
                "SEManager RemoveSE lock released. Used %.3f seconds. %s"
                % (time.time() - waitTime, seName)
            )
            self.lock.release()
            return res
