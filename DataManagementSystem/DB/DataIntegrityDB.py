...
class DataIntegrityDB(DB):
    ...
    def getTransformationProblematics(self, transID):
        ...
        req = (
            "SELECT LFN,FileID FROM Problematics WHERE Status = 'New' AND LFN LIKE '%%/%08d/%%';"
            % transID
        )
        res = self._query(req)
        if not res["OK"]:
    ...
    def incrementProblematicRetry(self, fileID):
        ...
        req = (
            "UPDATE Problematics SET Retries=Retries+1, LastUpdate=UTC_TIMESTAMP() WHERE FileID = %s;"
            % (fileID)
        )
