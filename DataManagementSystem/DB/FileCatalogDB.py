...
class FileCatalogDB(DB):
    ...
    def setConfig(self, databaseConfig):
        ...
        for compAttribute, componentType in [
            ...
            result = self.__loadCatalogComponent(
                componentType, databaseConfig[componentType]
            )
            if not result["OK"]:
                return result
    ...
    def __loadCatalogComponent(self, componentType, componentName):
        ...
        componentModule = "DataManagementSystem.DB.FileCatalogComponents.%s.%s" % (
            componentType,
            componentName,
        )
        ...
        if not result["OK"]:
            gLogger.error(
                "Failed to load catalog component",
                "%s: %s" % (componentName, result["Message"]),
            )
            return result
    ...
    def getReplicas(self, lfns, allStatus, credDict):
        ...
        res = self.fileManager.getReplicas(
            res["Value"]["Successful"], allStatus=allStatus
        )
        if not res["OK"]:
            return res
        failed.update(res["Value"]["Failed"])
    ...
    def getFileDetails(self, lfnList, credDict):
        ...
        result = self.fileManager._getFileMetadataByID(
            fileIDDict.keys(), connection=connection
        )
        if not result["OK"]:
            return result
        ...
        result = self.fmeta._getFileUserMetadataByID(
            fileIDDict.keys(), credDict, connection=connection
        )
        if not result["OK"]:
            return result
    ...
    def removeDirectory(self, lfns, credDict):
        ...
        dirIdList = [
            successful[p]["DirID"] for p in successful if "DirID" in successful[p]
        ]
    ...
    def getDirectorySize(self, lfns, longOutput, fromFiles, credDict):
        ...
        res = self.dtree.getDirectorySize(
            res["Value"]["Successful"], longOutput, fromFiles
        )
        if not res["OK"]:
            return res
        failed.update(res["Value"]["Failed"])
        ...
        return S_OK(
            {"Successful": successful, "Failed": failed, "QueryTime": queryTime}
        )
    ...
    def repairCatalog(self, credDict={}):
        ...
        resultDict = {}
        resultDict["RecoverOrphanDirectories"] = self.dtree.recoverOrphanDirectories(
            credDict
        )
