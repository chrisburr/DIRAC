...
def selectUniqueSource(ftsFiles, fts3Plugin, allowedSources=None):
    ...
    for ftsFile in ftsFiles:
        ...
        try:
            uniqueSource = fts3Plugin.selectSourceSE(
                ftsFile, replicaDict, allowedSources
            )
            groupBySource.setdefault(uniqueSource, []).append(ftsFile)
        except ValueError as e:
            _log.info(
                "No allowed replica source for file", "%s: %s" % (ftsFile.lfn, repr(e))
            )
            continue
...
def getFTS3Plugin(vo=None):
    ...
    """
    pluginName = opHelper(vo=vo).getValue(
        "DataManagement/FTSPlacement/FTS3/FTS3Plugin", "Default"
    )

    objLoader = ObjectLoader()
    _class = objLoader.loadObject(
        "DataManagementSystem.private.FTS3Plugins.%sFTS3Plugin" % pluginName,
        "%sFTS3Plugin" % pluginName,
    )
...
class FTS3ServerPolicy(object):
    ...
    def __init__(self, serverDict, serverPolicy="Random"):
        ...
        if not hasattr(self, methName):
            self.log.error(
                "Unknown server policy %s. Using Random instead" % serverPolicy
            )
    ...
    def chooseFTS3Server(self):
        ...
        while not fts3Server and attempt < self._maxAttempts:
            ...
            if not res["OK"]:
                self.log.warn(
                    "Error getting the RSS status for %s: %s" % (fts3Server, res)
                )
                fts3Server = None
                attempt += 1
                continue
