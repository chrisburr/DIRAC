...
class DefaultFTS3Plugin(object):
    ...
    def selectTPCProtocols(
        self, ftsJob=None, sourceSEName=None, destSEName=None, **kwargs
    ):
        """
    ...
    def selectSourceSE(self, ftsFile, replicaDict, allowedSources):
        ...
        allowedReplicaSource = (
            (set(replicaDict) & allowedSourcesSet) if allowedSourcesSet else replicaDict
        )
        ...
        randSource = random.choice(
            list(allowedReplicaSource)
        )  # one has to convert to list
        return randSource
