...
from DIRAC.DataManagementSystem.private.FTS3Plugins.DefaultFTS3Plugin import (
    DefaultFTS3Plugin,
)
...
def mock__checkSourceReplicas(ftsFiles, preferDisk=False):
    ...
    for ftsFile in ftsFiles:
        if hasattr(ftsFile, "fakeAttr_possibleSources"):
            succ[ftsFile.lfn] = dict.fromkeys(
                getattr(ftsFile, "fakeAttr_possibleSources")
            )
        else:
...
class TestFTS3ServerPolicy(unittest.TestCase):
    ...
    def testCorrectServerPolicyIsUsed(
        self, mockFailoverFunc, mockRandomFunc, mockSequenceFunc, mockFTSServerStatus
    ):
...
if __name__ == "__main__":
    ...
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(TestFTS3ServerPolicy)
    )
