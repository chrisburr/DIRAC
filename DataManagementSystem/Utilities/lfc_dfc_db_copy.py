...
def getUsersGroupsAndSEs(queryQueue, name_id_se):
    ...
    connection = cx_Oracle.Connection(
        prodDbAccount, prodDbAccountPassword, prodDbTNS, threaded=True
    )
...
def getDirAndFileData(fileQueue, dirQueue, dirClosureQueryQueue):
    ...
    directoryCommand = (
        "SELECT %s from CNS_FILE_METADATA START WITH fileid=3 \
                    CONNECT BY NOCYCLE PRIOR  fileid=parent_fileid order by lvl asc,\
                     parent_fileid"
        % (",".join(directoryParameters))
    )
    ...
    if fromdbR_.execute(directoryCommand):
        ...
        while not done:
            print(
                "getDirAndFileData step %s elapsed time %s dir %s files %s"
            ...
            if rows == []:
                ...
                for row in rows:
                    ...
                    dfc_cdate = datetime.utcfromtimestamp(row[CNS_FILE_CTIME]).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    dfc_mdate = datetime.utcfromtimestamp(row[CNS_FILE_MTIME]).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    ...
                    if not isDir(cns_file_mode):
                        ...
                        sqlDirClosure = (
                            "INSERT INTO FC_DirectoryClosure \
                            (ParentID, ChildID, Depth )\
                            VALUES (%s,%s, 0);"
                            % (cns_file_id, cns_file_id)
                        )
    ...
    print(
        "getDirAndFileData done. elapsed time %s dir %s files %s"
        % (time.time() - startTime, dirCounter, fileCounter)
    )
    fromdbR_.close()
    connection.close()
...
def getReplicaData(replicaQueue, name_id_se):
    ...
    getReplicaCommand = "SELECT %s from CNS_FILE_REPLICA" % (
        ",".join(replicaParameters)
    )
    ...
    if fromdbR_.execute(getReplicaCommand):
        ...
        while not done:
            print(
                "getReplicaData step %s elapsed time %s replica %s"
            ...
            if rows == []:
                ...
                for row in rows:
                    ...
                    # Modification time
                    dfc_mdate = datetime.utcfromtimestamp(row[CNS_REP_MTIME]).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    ...
                    try:
                        dfc_cdate = datetime.utcfromtimestamp(
                            row[CNS_REP_CTIME]
    ...
    print(
        "getReplicaData done. elapsed time %s replicas %s"
        % (time.time() - startTime, replicaCounter)
    )
...
def loadDataInMySQL(queryQueue, workerId):
    ...
    con = mdb.connect(
        ...
        port=dfc_db_port,
        user=dfc_db_user,
        passwd=dfc_db_passwd,
        db=dfc_db_name,
        unix_socket=dfc_unix_socket,
    )
    cur = con.cursor()
    ...
    while True:
        ...
        try:
            ...
            print(
                "worker %s : EXCEPTION %s\nworker %s :QUERY %s"
                % (workerId, e, workerId, next_query)
            )
...
def loadTupleDataInMySQL(queryQueue, workerId, querybase):
    ...
    while True:
        ...
        if next_tuple is None:
            print(
                "worker %s : got poison pill. elapsed time %s"
        ...
        if queryExecuted % 10000 == 0:
            print(
                "worker %s (%s) elapsed time %s"
                % (workerId, queryExecuted, time.time() - workerStart)
            )
            logfile.write(
                "worker %s (%s) elapsed time %s "
                % (workerId, queryExecuted, time.time() - workerStart)
...
def updateAdminID():
    ...
    updateQueries = {
        "Update FC_Files": "Update FC_Files set UID = 1, GID = 1 where UID = 0 and GID = 0",
...
def databaseIntegrityCheck():
    ...
    integrityQueries = {
        "Useless Users": " SELECT u.* from FC_Users u\
                                           LEFT OUTER JOIN\
                                               (select distinct(UID)\
...
def compareNumbers():
    ...
    if lfc_filesDir != (dfc_files + dfc_dir + 1):  # /grid folder
        print(
            "ERROR !  lfc_filesDir != (dfc_files + dfc_dir + 1) %s != %s"
            % (lfc_filesDir, dfc_files + dfc_dir + 1)
        )

    if lfc_replicas != dfc_replicas:
        print(
            "ERROR ! lfc_replicas != dfc_replicas %s != %s"
            % (lfc_replicas, dfc_replicas)
        )
...
if __name__ == "__main__":
    ...
    workerUgsProcess = multiprocessing.Process(
        target=loadDataInMySQL, args=(ugsQueryQueue, "ugs")
    )
   ...
   Filename, GUID, Checksum, ChecksumType, Type, CreationDate, ModificationDate, Mode) Values "
    workerFileProcess = multiprocessing.Process(
        target=loadTupleDataInMySQL, args=(fileQueryQueue, "file", fileBaseQuery)
    )
   ...
   ModificationDate, Mode, Status)  values "
    workerDirProcess = multiprocessing.Process(
        target=loadTupleDataInMySQL, args=(dirQueryQueue, "dir", dirBaseQuery)
    )
    workerDirProcess.start()

    # process to execute the query for directory closure
    workerDirClosureProcess = multiprocessing.Process(
        target=loadDataInMySQL, args=(dirClosureQueryQueue, "dirClosure")
    )
   ...
   CreationDate, ModificationDate, PFN) values "
    workerReplicaProcess = multiprocessing.Process(
        target=loadTupleDataInMySQL,
        args=(replicaQueryQueue, "replica", replicaBaseQuery),
    )
    ...
    ugsDumpProcess = multiprocessing.Process(
        target=getUsersGroupsAndSEs, args=(ugsQueryQueue, name_id_se)
    )
    ...
    fileDumpProcess = multiprocessing.Process(
        target=getDirAndFileData,
        args=(fileQueryQueue, dirQueryQueue, dirClosureQueryQueue),
    )
    ...
    replicaDumpProcess = multiprocessing.Process(
        target=getReplicaData, args=(replicaQueryQueue, name_id_se)
    )
