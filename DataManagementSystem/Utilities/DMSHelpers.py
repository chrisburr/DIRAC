...
def _getConnectionIndex(connectionLevel, default=None):
    ...
    return {"LOCAL": LOCAL, "PROTOCOL": PROTOCOL, "DOWNLOAD": DOWNLOAD}.get(
        connectionLevel
    )
...
class DMSHelpers(object):
    ...
    def getSiteSEMapping(self):
        ...
        if not storageElements["OK"]:
            gLogger.warn(
                "Problem retrieving storage elements", storageElements["Message"]
            )
    ...
    def getTiers(self, withStorage=False, tier=None):
        ...
        sites = sorted(
            self.getShortSiteNames(withStorage=withStorage, tier=tier).values()
        )
        ...
        if result["OK"]:
            for site in self.siteSEMapping[LOCAL] if withStorage else self.siteSet:
                ...
                if isinstance(tier, six.integer_types) and (
                    grid != "LCG"
                    or gConfig.getValue(
                        "/Resources/Sites/%s/%s/MoUTierLevel" % (grid, site), 999
                    )
                    != tier
                ):
                    continue
    ...
    def isSEForJobs(self, storageElement, checkSE=True):
        ...
        if self.notForJobSEs is None:
            seList = resolveSEGroup(
                self.__opsHelper.getValue("DataManagement/SEsNotToBeUsedForJobs", [])
            )
    ...
    def getSEsAtCountry(self, country, connectionLevel=None):
        ...
        if not self.siteSet:
            self.getSiteSEMapping()
        siteList = [
            site for site in self.siteSet if siteCountryName(site) == country.lower()
        ]
        if not siteList:
    ...
    def getAllSEsInGroupAtSite(self, seGroup, site):
        ...
        if not foundSEs:
            gLogger.warn(
                "No SE found at that site", "in group %s at %s" % (seGroup, site)
            )
            return S_OK()
