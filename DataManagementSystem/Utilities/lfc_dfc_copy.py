...
def processDir(
    initPath, writerQueue, recursive=False, host=None, fcInit=None, dfcInit=None
):
    ...
    if resultList["OK"]:
        ...
        if recursive:
            for path in paths:
                result = processDir(
                    path, writerQueue, recursive=True, host=host, fcInit=fc, dfcInit=dfc
                )
                if result["OK"]:
...
def finalizeDirectory(task, result):
    ...
    if result["OK"]:
        ...
        if "Path" in result:
            random.shuffle(lfcHosts)
            print(
                "Requeueing task for directory %s, lfc %s"
