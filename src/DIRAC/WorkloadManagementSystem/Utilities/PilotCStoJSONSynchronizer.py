...
class PilotCStoJSONSynchronizer(object):
            ...
            for site in gridSection["Value"]:
                ...
                for ce in ceList["Value"]:
                    ...
                    ceType = gConfig.getValue(
                        "/Resources/Sites/" + grid + "/" + site + "/CEs/" + ce + "/CEType"
                    )
                    if ceType is None:
                        # Skip but log it
                    ...
                    res = gConfig.getSections(
                        "/Resources/Sites/" + grid + "/" + site + "/CEs/" + ce + "/Queues/"
                    )
                    if not res["OK"]:
                        # Skip but log it
    ...
    def _getPilotOptionsPerSetup(self, setup, pilotDict):
        ...
        if "CommandExtensions" in pilotDict["Setups"][setup]:
            ...
            pilotDict["Setups"][setup]["CommandExtensions"] = pilotDict["Setups"][setup][
                "CommandExtensions"
            ].split(", ")
    ...
    def syncScripts(self):
        ...
        if self.pilotVORepo:
            ...
            self.log.info(
                "The /Operations/<Setup>/Pilot/pilotVORepo option is not defined, using Vanilla DIRAC pilot"
            )
