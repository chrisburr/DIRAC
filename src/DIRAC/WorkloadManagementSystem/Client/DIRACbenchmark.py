    ...
    for i in range(copies):
        ...
        processes.append(
            multiprocessing.Process(
                target=singleDiracBenchmarkProcess, args=(results[i], iterations, measuredCopies)
            )
        )
...
"""
    ...
    print(
        result["copies"], result["sum"], result["arithmetic_mean"], result["geometric_mean"], result["median"]
    )
    print(" ".join([str(i) for i in result["raw"]]))
    sys.exit(0)
