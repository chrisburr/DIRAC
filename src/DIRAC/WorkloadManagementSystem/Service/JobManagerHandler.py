...
class JobManagerHandler(RequestHandler):
        ...
        if enablePilotsLogging:
            try:
                result = ObjectLoader().loadObject(
                    "WorkloadManagementSystem.DB.PilotsLoggingDB", "PilotsLoggingDB"
                )
                if not result["OK"]:
                    return result
    ...
    def export_submitJob(self, jobDesc):
        ...
        if nJobs > 0:
            ...
            if nJobs > self.maxParametricJobs:
                ...
                return S_ERROR(
                    EWMSJDL, "Number of parametric jobs exceeds the limit of %d" % self.maxParametricJobs
                )
        ...
        for (
            jobDescription
        ) in jobDescList:  # jobDescList because there might be a list generated by a parametric job
    ...
    def export_confirmBulkSubmission(self, jobIDs):
        ...
        jobUpdateStatusList = list(
            jobID for jobID in jobList if js_dict[jobID]["Status"] == JobStatus.SUBMITTING
        )
    ...
    def export_removeJob(self, jobIDs):
        ...
        if validJobList:
            ...
            for jobID in validJobList:
                ...
                if not resultTQ["OK"]:
                    self.log.warn(
                        "Failed to remove job from TaskQueueDB", "(%d): %s" % (jobID, resultTQ["Message"])
                    )
                    error_count += 1
                else:
                    count += 1
