...
def sendJobAccounting(job, status, minorStatus):
    ...
    try:
        ...
        gLogger.exception(
            "JobWrapper failed sending job accounting for [status:minorStatus] [%s:%s]"
            % (status, minorStatus),
            lException=exc,
        )
    ...
    if "InputSandbox" in arguments["Job"]:
        ...
        try:
            ...
            job.sendJobAccounting(
                status=rescheduleResult, minorStatus=JobMinorStatus.DOWNLOADING_INPUT_SANDBOX
            )
            return 1
        except Exception as exc:  # pylint: disable=broad-except
    ...
    if "InputData" in arguments["Job"]:
        if arguments["Job"]["InputData"]:
            try:
                ...
                job.sendJobAccounting(
                    status=rescheduleResult, minorStatus=JobMinorStatus.INPUT_DATA_RESOLUTION
                )
                return 1
            except Exception as exc:  # pylint: disable=broad-except
