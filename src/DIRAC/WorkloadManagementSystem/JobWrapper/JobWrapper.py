...
class JobWrapper(object):
    ...
    def execute(self):
        ...
        if "Thread" in EXECUTION_RESULT:
            ...
            if not threadResult["OK"]:
                ...
                self.__report(
                    status=JobStatus.FAILED, minorStatus=JobMinorStatus.APP_THREAD_FAILED, sendFlag=True
                )
            ...
            self.__report(
                status=JobStatus.FAILED, minorStatus=JobMinorStatus.APP_THREAD_NOT_COMPLETE, sendFlag=True
            )
        ...
        if outputs:
            ...
            if not watchdog.checkError and not status:
                ...
                self.__report(
                    status=JobStatus.COMPLETING, minorStatus=JobMinorStatus.APP_SUCCESS, sendFlag=True
                )
            elif not watchdog.checkError:
                self.__report(
                    status=JobStatus.COMPLETING, minorStatus=JobMinorStatus.APP_ERRORS, sendFlag=True
                )
                if status in (
                    DErrno.EWMSRESC,
                    DErrno.EWMSRESC & 255,
    ...
    def resolveInputData(self):
        ...
        self.__report(
            status=JobStatus.RUNNING, minorStatus=JobMinorStatus.INPUT_DATA_RESOLUTION, sendFlag=True
        )
        ...
        if not inputData:
            msg = (
                "Job Wrapper cannot resolve local replicas of input data with null job input data parameter "
            )
            self.log.error(msg)
            return S_ERROR(msg)
        else:
    ...
    def processJobOutputs(self):
        ...
        if fileList and self.jobID:
            ...
            if not result_sbUpload["OK"]:
                ...
                if not self.failedFlag:
                    self.__report(
                        status=JobStatus.COMPLETING, minorStatus=JobMinorStatus.OUTPUT_SANDBOX_UPLOADED
                    )
    ...
    def transferInputSandbox(self, inputSandbox):
        ...
        if os.path.exists("%s/inputsandbox" % (self.root)):
            ...
            for inputFile in sandboxFiles:
                if os.path.exists("%s/inputsandbox/%s" % (self.root, inputFile)):
                    self.log.info(
                        "Getting InputSandbox file %s from local directory for testing" % (inputFile)
                    )
        ...
        if lfns:
            ...
            if not download["OK"]:
                ...
                self.__report(
                    status=JobStatus.RUNNING, minorStatus=JobMinorStatus.FAILED_DOWNLOADING_INPUT_SANDBOX_LFN
                )
            ...
            for lfn in lfns:
                if os.path.exists(
                    "%s/%s" % (self.root, os.path.basename(download["Value"]["Successful"][lfn]))
                ):
    ...
    def sendFailoverRequest(self):
        ...
        if result["OK"]:
            ...
            self.log.warn(
                "JobReportFailure", "Could not generate a forwardDISET operation: %s" % result["Message"]
            )
            self.log.warn(
                "JobReportFailure", "The job won't fail, but the jobLogging info might be incomplete"
            )
        ...
        if len(request):
            ...
            if not isValid["OK"]:
                ...
                if reqToJSON["OK"]:
                    ...
                    self.log.error(
                        "Something went wrong creating the JSON from request", reqToJSON["Message"]
                    )
                return S_ERROR()
            else:
...
class ExecutionThread(threading.Thread):
    ...
    def run(self):
        ...
        log.info(
            "EXECUTION_RESULT[Thread] after Execution of spObject.systemCall", str(EXECUTION_RESULT["Thread"])
        )
