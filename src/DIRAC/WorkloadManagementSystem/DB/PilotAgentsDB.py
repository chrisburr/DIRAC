...
AND SubmissionTime < DATE_SUB(UTC_TIMESTAMP(),INTERVAL %d DAY)"
    ...
    def storePilotOutput(self, pilotRef, output, error):
        ...
        req = "INSERT INTO PilotOutput (PilotID,StdOutput,StdError) VALUES (%d,%s,%s)" % (
            pilotID,
            e_output,
            e_error,
        )
    ...
    def getPilotsForTaskQueue(self, taskQueueID, gridType=None, limit=None):
        ...
        if gridType:
            req = "SELECT PilotID FROM PilotAgents WHERE TaskQueueID=%s AND GridType='%s' " % (
                taskQueueID,
                gridType,
            )
        else:
    ...
    def getPilotsForJobID(self, jobID):
        ...
        if result["Value"]:
            ...
            return S_OK(pilotList)
        self.log.verbose(
            "PilotID for job not found: either not matched yet, or already deleted", "id=%s" % jobID
        )
        return S_OK([])
    ...
    def getPilotSummary(self, startdate="", enddate=""):
        ...
        for st in st_list:
            ...
            req = (
                "SELECT DestinationSite,count(DestinationSite) FROM PilotAgents " + "WHERE Status='%s' " % st
            )
            if startdate:
