...
class JobDB(DB):
    ...
    def getAttributesForJobList(self, jobIDList, attrList=None):
        ...
        if attrList:
            ...
            if missingAttr:
                return S_ERROR(
                    "JobDB.getAttributesForJobList: Unknown Attribute(s): %s" % ", ".join(missingAttr)
                )
    ...
    def getDistinctJobAttributes(
        self, attribute, condDict=None, older=None, newer=None, timeStamp="LastUpdateTime"
    ):
                    ...
                    if exactTime:
                        for att in ("StartExecTime", "SubmissionTime"):
                            ...
                            if startTime:
                                break
                        startTime = (
                            datetime.datetime.strptime(startTime, "%Y-%m-%d %H:%M:%S") if startTime else now
                        )
                        for att in ("EndExecTime", "HeartBeatTime"):
                            ...
                            if lastTime:
                                break
                        lastTime = (
                            datetime.datetime.strptime(lastTime, "%Y-%m-%d %H:%M:%S") if lastTime else now
                        )
    ...
    def getJobsAttributes(self, jobIDs, attrList=None):
        ...
        cmd = "SELECT %s FROM Jobs WHERE JobID IN (%s)" % (
            attrNames,
            ",".join(str(jobID) for jobID in jobIDs),
        )
        res = self._query(cmd)
        if not res["OK"]:
            return res
    ...
    def getJobOptParameters(self, jobID, paramList=None):
        ...
        if paramList:
            ...
            for x in paramList:
                ...
                paramNameList.append(ret["Value"])
            paramNames = ",".join(paramNameList)
            cmd = "SELECT Name, Value from OptimizerParameters WHERE JobID=%s and Name in (%s)" % (
                jobID,
                paramNames,
            )
        else:
    ...
    ############################################################################
    def selectJobs(
        self, condDict, older=None, newer=None, timeStamp="LastUpdateTime", orderAttribute=None, limit=None
    ):
    ...
    def setJobAttribute(self, jobID, attrName, attrValue, update=False, myDate=None):
        ...
        if update:
            cmd = "UPDATE Jobs SET %s=%s,LastUpdateTime=UTC_TIMESTAMP() WHERE JobID=%s" % (
                attrName,
                value,
                jobID,
            )
    ...
    #############################################################################
    def setJobStatus(
        self, jobID, status="", minorStatus="", applicationStatus="", minor=None, application=None
    ):
    ...
    def getSiteSummaryWeb(self, selectDict, sortList, startItem, maxItems):
        ...
        result = self.getCounters(
            "Jobs", ["Site", "Status"], {}, newer=last_update, timeStamp="LastUpdateTime"
        )
        ...
        for siteFullName in resultDict:
            ...
            if total_finished > 0:
                efficiency = float(siteDict[JobStatus.DONE] + siteDict[JobStatus.COMPLETED]) / float(
                    total_finished
                )
