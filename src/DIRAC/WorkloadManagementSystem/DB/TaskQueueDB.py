...
class TaskQueueDB(DB):
    ...
    def _checkTaskQueueDefinition(self, tqDefDict):
        ...
        for field in singleValueDefFields:
            ...
            if field in ["CPUTime"]:
                if not isinstance(tqDefDict[field], six.integer_types):
                    return S_ERROR(
                        "Mandatory field %s value type is not valid: %s" % (field, type(tqDefDict[field]))
                    )
            else:
        ...
        for field in multiValueDefFields:
            ...
            if not isinstance(tqDefDict[field], (list, tuple)):
                return S_ERROR(
                    "Multi value field %s value type is not valid: %s" % (field, type(tqDefDict[field]))
                )
            ...
            if isinstance(value, (list, tuple)):
                for subValue in value:
                    if not isinstance(subValue, validTypes):
                        return S_ERROR(
                            "List contained type %s is not valid -> %s" % (type(subValue), validTypes)
                        )
                if escapeValues:
    ...
    def __createTaskQueue(self, tqDefDict, priority=1, connObj=False):
        ...
        for field in multiValueDefFields:
            ...
            if not result["OK"]:
                ...
                self.cleanOrphanedTaskQueues(connObj=connObj)
                return S_ERROR(
                    "Can't insert values %s for field %s: %s" % (str(values), field, result["Message"])
                )
    ...
    def cleanOrphanedTaskQueues(self, connObj=False):
        ...
        for mvField in multiValueDefFields:
            result = self._update(
                "DELETE FROM `tq_TQTo%s` WHERE TQId in ( %s )" % (mvField, ",".join(orphanedTQs)),
                conn=connObj,
            )
            if not result["OK"]:
                return result
    ...
    def __setTaskQueueEnabled(self, tqId, enabled=True, connObj=False):
        ...
        if not result["OK"]:
            self.log.error(
                "Error setting TQ state", "TQ %s State %s: %s" % (tqId, enabled, result["Message"])
            )
            return result
    ...
    def __hackJobPriority(self, jobPriority):
        jobPriority = min(
            max(int(jobPriority), self.__jobPriorityBoundaries[0]), self.__jobPriorityBoundaries[1]
        )
    ...
    def insertJob(self, jobId, tqDefDict, jobPriority, skipTQDefCheck=False):
        ...
        try:
            ...
            if not result["OK"]:
                self.log.error(
                    "Error inserting job in TQ", "Job %s TQ %s: %s" % (jobId, tqId, result["Message"])
                )
                return result
            if newTQ:
        ...
        for _ in range(self.__maxMatchRetry):
            ...
            for tqId, tqOwnerDN, tqOwnerGroup in tqList:
                ...
                while jobTQList:
                    ...
                    if retVal["Value"]:
                        self.log.info(
                            "Extracted job with prio from TQ", "(%s : %s : %s)" % (jobId, prio, tqId)
                        )
                        return S_OK(
                            {"matchFound": True, "jobId": jobId, "taskQueueId": tqId, "tqMatch": tqMatchDict}
                        )
...
WHERE j.JobId = %s AND t.TQId = j.TQId"
    ...
    def getTaskQueueForJobs(self, jobIDs, connObj=False):
        ...
        retVal = self._query(
            "SELECT JobId,TQId FROM `tq_Jobs` WHERE JobId in (%s) " % jobString, conn=connObj
        )
    ...
    def __getOwnerForTaskQueue(self, tqId, connObj=False):
        retVal = self._query(
            "SELECT OwnerDN, OwnerGroup from `tq_TaskQueues` WHERE TQId=%s" % tqId, conn=connObj
        )
        if not retVal["OK"]:
            return retVal
        data = retVal["Value"]
    ...
    def deleteTaskQueueIfEmpty(self, tqId, tqOwnerDN=False, tqOwnerGroup=False, connObj=False):
        ...
        if tqToDel:
            for mvField in multiValueDefFields:
                retVal = self._update(
                    "DELETE FROM `tq_TQTo%s` WHERE TQId = %s" % (mvField, tqId), conn=connObj
                )
                if not retVal["OK"]:
                    return retVal
