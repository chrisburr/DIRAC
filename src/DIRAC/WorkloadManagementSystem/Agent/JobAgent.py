...
class JobAgent(AgentModule):
    ...
    def execute(self):
            ...
            if re.search("No match found", jobRequest["Message"]):
                ...
                if self.matchFailedCount > self.stopAfterFailedMatches:
                    return self.__finish(
                        "Nothing to do for more than %d cycles" % self.stopAfterFailedMatches
                    )
        ...
        for param in matcherParams:
            if param not in matcherInfo:
                jobReport.setJobStatus(status="Failed", minor="Matcher did not return %s" % (param))
                return self.__finish("Matcher Failed")
            elif not matcherInfo[param]:
                jobReport.setJobStatus(status="Failed", minor="Matcher returned null %s" % (param))
                return self.__finish("Matcher Failed")
            else:
        ...
        if not parameters["OK"]:
            jobReport.setJobStatus(status="Failed", minor="Could Not Extract JDL Parameters")
        ...
        try:
            ...
            jobReport.setJobStatus(status="Matched", minor="Job Received by Agent", sendFlag=False)
    ...
    def computeCPUWorkLeft(self, processors=1):
        ...
        if result["OK"]:
            ...
            self.log.warn(
                "There were errors calculating time left using the Timeleft utility", result["Message"]
            )
    ...
    def _submitJob(
        ...
        jobReport.setJobStatus(status="Matched", minor="Submitting To CE")
