...
def test__getPilotOptions(mocker):
    ...
    mocker.patch(
        "DIRAC.WorkloadManagementSystem.Agent.SiteDirector.gConfig.getValue", side_effect=mockGCReply
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"OK": True, "Value": {"1": {"Jobs": 10, "Sites": ["Site1"]}, "2": {"Jobs": 20}}},
            True,
            True,
            set(["Site1"]),
        ),
...
def test__ifAndWhereToSubmit(mocker, mockMatcherReturnValue, expected, anyExpected, sitesExpected):
    ...
    mocker.patch(
        "DIRAC.WorkloadManagementSystem.Agent.SiteDirector.CSGlobals.getSetup", side_effect=mockCSGlobalReply
    )
    mocker.patch("DIRAC.WorkloadManagementSystem.Agent.SiteDirector.AgentModule", side_effect=mockAM)
    sd = SiteDirector()
    sd.log = gLogger
...
def test__updatePilotStatus(mocker, pilotRefs, pilotDict, pilotCEDict, expected):
    ...
    mocker.patch("DIRAC.WorkloadManagementSystem.Agent.SiteDirector.AgentModule", side_effect=mockAM)
    mocker.patch(
        "DIRAC.WorkloadManagementSystem.Agent.SiteDirector.pilotAgentsDB", side_effect=mockPilotAgentsDB
    )
    sd = SiteDirector()
    sd.log = gLogger
    sd.am_getOption = mockAM
