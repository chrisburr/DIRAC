...
class SiteDirector(AgentModule):
        ...
        for site in resourceDict:
            for ce in resourceDict[site]:
                ...
                for queue in qDict:
                    ...
                    self.queueDict[queueName]["ParametersDict"]["Setup"] = gConfig.getValue(
                        "/DIRAC/Setup", "unknown"
                    )
                    ...
                    if queueName in self.queueCECache and self.queueCECache[queueName]["Hash"] == queueHash:
                        ...
                        result = ceFactory.getCE(
                            ceName=ce, ceType=ceDict["CEType"], ceParametersDict=ceQueueDict
                        )
                        if not result["OK"]:
                    ...
                    if "BundleProxy" in self.queueDict[queueName]["ParametersDict"]:
                        if self.queueDict[queueName]["ParametersDict"]["BundleProxy"].lower() in [
                            "true",
                            "yes",
                            "1",
                        ]:
    ...
    def submitPilots(self):
        ...
        for queueName, queueDictionary in queueDictItems:
            ...
            self.log.verbose(
                "Getting pilot proxy", "for %s/%s %d long" % (self.pilotDN, self.pilotGroup, cpuTime)
            )
    ...
    def _allowedToSubmit(self, queue, anySite, jobSites, testSites):
        ...
        if failedCount != 0:
            self.log.warn(
                "queue failed recently ==> number of cycles skipped", "%s ==> %d" % (queue, 10 - failedCount)
            )
    ...
    def _submitPilotsToQueue(self, pilotsToSubmit, ce, queue):
        ...
        """
        self.log.info(
            "Going to submit pilots", "(a maximum of %d pilots to %s queue)" % (pilotsToSubmit, queue)
        )
    ...
    def _writePilotScript(
        self, workingDirectory, pilotOptions, proxy=None, pilotExecDir="", envVariables=None
    ):
