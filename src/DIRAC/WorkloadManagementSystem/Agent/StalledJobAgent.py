...
class StalledJobAgent(AgentModule):
    ...
    def initialize(self):
        ...
        watchdogCycle = max(
            watchdogCycle, gConfig.getValue(cfgPath(wrapperSection, "MinCheckingTime"), 20 * 60)
        )
    ...
    def execute(self):
        ...
        for minor in self.minorStalledStatuses:
            result = self.jobDB.selectJobs(
                {"Status": JobStatus.FAILED, "MinorStatus": minor, "AccountedFlag": "False"}
            )
            if not result["OK"]:
    ...
    def _updateJobStatus(self, job, status, minorStatus=None):
        ...
        if minorStatus:
            self.log.debug(
                "self.jobDB.setJobAttribute(%s,'MinorStatus','%s',update=True)" % (job, minorStatus)
            )
        ...
        result = self.logDB.addLoggingRecord(
            job, status=status, minorStatus=minorStatus, source="StalledJobAgent"
        )
        if not result["OK"]:
    ...
    def _sendAccounting(self, jobID):
        ...
        if result["OK"]:
            ...
            self.log.error(
                "Failed to send accounting report", "Job: %d, Error: %s" % (int(jobID), result["Message"])
            )
        return result
