                ...
                if invalidSites:
                    ...
                    if not self.ex_getOption("AllowInvalidSites", True):
                        return self.__holdJob(
                            jobState, "Requested site(s) %s are invalid" % ",".join(invalidSites)
                        )
                if bannedSites:
                    self.jobLog.debug("Banned site(s) %s ignored" % ",".join(bannedSites))
                    if not usableSites:
                        return self.__holdJob(
                            jobState, "Requested site(s) %s are inactive" % ",".join(bannedSites)
                        )
        ...
        if jobType in Operations().getValue("Transformations/DataProcessing", []):
            ...
            if stageLFNs:
                ...
                return self.__sendToTQ(
                    jobState, jobManifest, userSites, userBannedSites, onlineSites=onlineSites
                )
