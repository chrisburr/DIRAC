...
def main():
    ...
    Script.registerSwitch(
        "V:", "vo=", "Get resources for the given VO. If not set, taken from the proxy", setVOName
    )
    ...
    def printSEInfo(voName):
        ...
        for se in DMSHelpers(
            ...
            records.append(
                (se, status, ",".join([seProtocol["Protocol"] for seProtocol in seObject.protocolOptions]))
            )
