...
def getRoleFromGID(groupID, path=None):
    ...
    if res == 0:
        ...
        gLogger.error(
            errStr, "%s %s%s" % (groupID, ("(%s) " % path) if path else "", lfc.sstrerror(lfc.cvar.serrno))
        )
        return S_ERROR()
...
class LcgFileCatalogClient(FileCatalogClientBase):
    ...
    def hasAccess(self, lfns, opType):
        ...
        failed = perms["Failed"]
        successful = dict(
            (path, perms["Successful"][path].get(opType, False)) for path in perms["Successful"]
        )
                ...
                if not res["OK"]:
                    ...
                    if not res["OK"]:
                        ...
                        try:
                            ...
                            print(
                                "key not found: __getACLInformation returned incomplete dictionary", KeyError
                            )
                            failed[path] = lfcPerm
                            continue
    ...
    def __createDataset(self, datasetName, lfns):
        ...
        for link, error in res["Value"]["Failed"].items():
            gLogger.error(
                "LcgFileCatalogClient.__createDataset: Failed to create link", "for %s: %s" % (link, error)
            )
