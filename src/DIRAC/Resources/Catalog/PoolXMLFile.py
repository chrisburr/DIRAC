...
def getGUID(fileNames, directory=""):
    ...
    if not generated:
        ...
        gLogger.info(
            "GUIDs not found from POOL XML Catalogue (and were generated) for: %s" % ", ".join(generated)
        )
...
def getType(fileNames, directory=""):
    ...
    gLogger.verbose(
        "Will look for POOL XML Catalog file types in %s for %s" % (directory, ", ".join(fileNames))
    )
