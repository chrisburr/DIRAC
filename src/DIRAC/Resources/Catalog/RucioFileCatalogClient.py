            ...
            try:
                ...
                for meta in self.client.get_metadata_bulk(dids):
                    ...
                    if meta["did_type"] in ["DATASET", "CONTAINER"]:
                        nlinks = len(
                            [child for child in self.client.list_content(meta["scope"], meta["name"])]
                        )
                        successful[lfn] = {
                            "Checksum": "",
                            "ChecksumType": "",
                ...
                if meta["did_type"] == "FILE":
                    ...
                    try:
                        self.client.detach_dids(
                            dsnScope, dsnName, [{"scope": did["scope"], "name": did["name"]}]
                        )
                ...
                if meta["did_type"] == "DATASET":
                    try:
                        self.client.set_metadata(
                            scope=did["scope"], name=did["name"], key="lifetime", value=1
                        )
                        resDict["Successful"][lfn] = True
                    except Exception as err:
                        resDict["Failed"][lfn] = str(err)
