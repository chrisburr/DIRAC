...
class ServerBackend(AbstractBackend):
    ...
    def __init__(self, backendParams=None):
        super(ServerBackend, self).__init__(
            ServerHandler, BaseFormatter, backendParams, level=DEFAULT_SERVER_LEVEL
        )
    ...
    def _setHandlerParameters(self, backendParams=None):
        ...
        if backendParams is not None:
            self._handlerParams["sleepTime"] = backendParams.get(
                "SleepTime", self._handlerParams["sleepTime"]
            )
            self._handlerParams["interactive"] = backendParams.get(
                "Interactive", self._handlerParams["interactive"]
            )
