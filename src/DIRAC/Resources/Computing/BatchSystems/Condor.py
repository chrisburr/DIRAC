...
class Condor(object):
    ...
    def killJob(self, **kwargs):
        ...
        for job in jobIDList:
            sp = subprocess.Popen(
                shlex.split("condor_rm %s" % job), stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            output, error = sp.communicate()
            status = sp.returncode
            if status != 0:
    ...
    def getCEStatus(self, **kwargs):
        ...
        sp = subprocess.Popen(
            shlex.split("condor_q -submitter %s" % user), stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        output, error = sp.communicate()
        status = sp.returncode
