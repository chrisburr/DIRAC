...
def test_getScaledCPU(mocker, batch, requiredVariables, returnValue, expected):
    """Test getScaledCPU()"""
    mocker.patch(
        "DIRAC.Resources.Computing.BatchSystems.TimeLeft.TimeLeft.runCommand", return_value=S_OK(returnValue)
    )
    tl = TimeLeft()
    res = tl.getScaledCPU()
    assert res == 0
    ...
    batchPlugin = __import__(
        batchSystemPath, globals(), locals(), [batchSystemName]
...
@pytest.mark.parametrize(
    ...
    [
        (
            "LSF",
            {"bin": "/usr/bin", "hostNorm": 10.0, "cpuLimit": 1000, "wallClockLimit": 1000},
            LSF_OUT,
            True,
            9400.0,
        ),
