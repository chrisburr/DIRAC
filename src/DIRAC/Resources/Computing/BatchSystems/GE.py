...
class GE(object):
    def submitJob(self, **kwargs):
        ...
        for _i in range(int(nJobs)):
            ...
            cmd += (
                "qsub -o %(OutputDir)s -e %(ErrorDir)s -N DIRACPilot %(SubmitOptions)s %(Executable)s"
                % kwargs
            )
    ...
    def killJob(self, **kwargs):
        ...
        for job in jobIDList:
            sp = subprocess.Popen(
                shlex.split("qdel %s" % job), stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            output, error = sp.communicate()
            status = sp.returncode
            if status != 0:
    ...
    def getJobStatus(self, **kwargs):
        ...
        sp = subprocess.Popen(
            shlex.split("qstat -u %s" % user), stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        output, error = sp.communicate()
        status = sp.returncode
        ...
        sp = subprocess.Popen(
            shlex.split("qstat -u %s -s -z" % user), stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        output, error = sp.communicate()
        status = sp.returncode
