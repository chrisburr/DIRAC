...
class ARCComputingElement(ComputingElement):
    ...
    def __writeXRSL(self, executableFile, processors=1):
        ...
        nProcessors = (
            processors if processors > 1 else self.ceParameters.get("NumberOfProcessors", processors)
        )
...
%(xrslExtraString)s
    ...
    def submitJob(self, executableFile, proxy, numberOfJobs=1, processors=1):
        ...
        endpoint = arc.Endpoint(
            str(self.ceHost + ":2811/jobs"), arc.Endpoint.JOBSUBMIT, "org.nordugrid.gridftpjob"
        )
        ...
        for __i in range(numberOfJobs):
            ...
            if result == arc.SubmissionStatus.NONE:
                ...
                if result.isSet(arc.SubmissionStatus.NO_SERVICES):  # pylint: disable=no-member
                    self.log.warn(
                        "%s no services are running on CE? (open GGUS ticket to site admins" % message
                    )
                ...
                if result.isSet(
                    ...
                    self.log.warn(
                        "%s SUBMITTER_PLUGIN_NOT_LOADED : ARC library installation problem?" % message
                    )
    ...
    def killJob(self, jobIDList):
        ...
        for chunk in breakListIntoChunks(jobs, 100):
            ...
            if not job_supervisor.Cancel():
                ...
                return S_ERROR(
                    "Failed to kill at least one of these jobs: %s. CE(?) not reachable?" % errorString
                )
