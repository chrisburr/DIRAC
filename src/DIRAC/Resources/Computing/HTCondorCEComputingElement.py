...
""" % dict(
    ...
    def _reset(self):
        ...
        self.extraSubmitString = str(
            self.ceParameters.get("ExtraSubmitString", "").encode().decode("unicode_escape")
        )
        ...
        self.remoteScheddOptions = (
            "" if self.useLocalSchedd else "-pool %s:9619 -name %s " % (self.ceName, self.ceName)
        )
    ...
    def submitJob(self, executableFile, proxy, numberOfJobs=1):
        ...
        scheddOptions = (
            [] if self.useLocalSchedd else ["-pool", "%s:9619" % self.ceName, "-remote", self.ceName]
        )
