...
@pytest.mark.parametrize(
    ...
    [
        (
            False,
            ["ShouldTransferFiles = YES", "WhenToTransferOutput = ON_EXIT_OR_EVICT"],
            ["universe = vanilla"],
        ),
        (
            True,
            [],
            ["ShouldTransferFiles = YES", "WhenToTransferOutput = ON_EXIT_OR_EVICT", "universe = grid"],
        ),
    ],
)
...
def test_submitJob(setUp, mocker, localSchedd, expected):
    ...
    writeSubMock = mocker.patch(
        MODNAME + ".HTCondorCEComputingElement._HTCondorCEComputingElement__writeSub",
