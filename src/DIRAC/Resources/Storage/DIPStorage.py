...
class DIPStorage(StorageBase):
    ...
    def __putFile(self, src_file, dest_url):
        ...
        if srcDict["Protocol"] in ["dips", "dip"]:
            ...
            res = transferClient.receiveFile(
                srcDict["FileName"], os.path.join(srcDict["Path"], srcDict["FileName"])
            )
            if not res["OK"]:
                return res
    ...
    def isFile(self, path):
        ...
        for url in urls:
            ...
            if res["OK"]:
                ...
                gLogger.error(
                    "DIPStorage.isFile: Failed to get metadata for url", "%s: %s" % (url, res["Message"])
                )
                failed[url] = res["Message"]
        resDict = {"Failed": failed, "Successful": successful}
        return S_OK(resDict)
        ...
        for url in urls:
            ...
            if res["OK"]:
                if res["Value"]["Exists"]:
                    if res["Value"]["Type"] == "File":
                        gLogger.debug(
                            "DIPStorage.getFileMetadata: Successfully obtained metadata for %s." % url
                        )
                        successful[url] = res["Value"]
                    else:
    ...
    def isDirectory(self, path):
        ...
        failed = {}
        gLogger.debug(
            "DIPStorage.isDirectory: Attempting to determine whether %s paths are directories." % len(urls)
        )
        serviceClient = RPCClient(self.url)
        for url in urls:
            res = serviceClient.getMetadata(url)
    ...
    def getDirectoryMetadata(self, path):
        ...
        failed = {}
        gLogger.debug(
            "DIPStorage.getFileMetadata: Attempting to obtain metadata for %s directories." % len(urls)
        )
        serviceClient = RPCClient(self.url)
        for url in urls:
            res = serviceClient.getMetadata(url)
    ...
    def createDirectory(self, path):
        ...
        for url in urls:
            ...
            if res["OK"]:
                gLogger.debug(
                    "DIPStorage.createDirectory: Successfully created directory on storage: %s" % url
                )
                successful[url] = True
            else:
                gLogger.error(
    ...
    def putDirectory(self, path):
        ...
        failed = {}
        gLogger.debug(
            "DIPStorage.putDirectory: Attemping to put %s directories to remote storage." % len(urls)
        )
        transferClient = TransferClient(self.url)
    ...
    def removeDirectory(self, path, recursive=False):
        ...
        for url in urls:
            ...
            if res["OK"]:
                gLogger.debug(
                    "DIPStorage.removeDirectory: Successfully removed directory on storage: %s" % url
                )
    ...
    def getDirectory(self, path, localPath=False):
        ...
        successful = {}
        gLogger.debug(
            "DIPStorage.getDirectory: Attempting to get local copies of %s directories." % len(urls)
        )
        transferClient = TransferClient(self.url)
        for src_dir in urls:
            if localPath:
