...
class RFIOStorage(StorageBase):
            ...
            if res["OK"]:
                ...
                if returncode in [0, 1]:
                    ...
                    if res["OK"]:
                        ...
                        errStr = (
                            "RFIOStorage.removeFile. Completely failed to remove files from the nameserver."
                        )
                        gLogger.error(errStr, res["Message"])
                        for pfn in urls:
                            failed[pfn] = errStr
    ...
    def getDirectory(self, path, localPath=False):
        ...
        failed = {}
        gLogger.debug(
            "RFIOStorage.getDirectory: Attempting to get local copies of %s directories." % len(urls)
        )
        for src_directory in urls:
            ...
            if res["OK"]:
                if res["Value"]["AllGot"]:
                    gLogger.debug(
                        "RFIOStorage.getDirectory: Successfully got local copy of %s" % src_directory
                    )
    ...
    def putDirectory(self, path):
        ...
        failed = {}
        gLogger.debug(
            "RFIOStorage.putDirectory: Attemping to put %s directories to remote storage." % len(urls)
        )
    ...
    def createDirectory(self, path):
        ...
        for url in urls:
            ...
            if res["OK"]:
                gLogger.debug(
                    "RFIOStorage.createDirectory: Successfully created directory on storage: %s" % url
                )
                successful[url] = True
            else:
    ...
    def getDirectoryMetadata(self, path):
        ...
        urls = res["Value"]
        gLogger.debug(
            "RFIOStorage.getDirectoryMetadata: Attempting to get metadata for %s directories." % len(urls)
        )
        res = self.isDirectory(urls)
        if not res["OK"]:
            return res
