...
class StorageElementItem(object):
    ...
    def __init__(self, name, plugins=None, vo=None, hideExceptions=False):
        ...
        self.useProxy = (
            gConfig.getValue(
                "/Resources/StorageElements/%s/AccessProtocol.1/Protocol" % name, "UnknownProtocol"
            )
            in proxiedProtocols
        )
    ...
    def getOccupancy(self, unit="MB", **kwargs):
        ...
        if occupancyPlugin:
            res = ObjectLoader().loadObject(
                "Resources.Storage.OccupancyPlugins.%s" % occupancyPlugin, occupancyPlugin
            )
            if not res["OK"]:
    ...
    def isValid(self, operation=None):
        ...
        log.verbose(
            "Determining if the StorageElement %s is valid for operation '%s'" % (self.name, operation)
        )
    ...
    def __getAllProtocols(self, protoType):
        ...
        """
        return set(
            reduce(lambda x, y: x + y, [plugin.protocolParameters[protoType] for plugin in self.storages])
        )
    ...
    def negociateProtocolWithOtherSE(self, sourceSE, protocols=None):
        ...
        log.debug(
            "Negotiating protocols between %s and %s (protocols %s)" % (sourceSE.name, self.name, protocols)
        )
        ...
        if protocols:
            ...
            commonProtocols = sorted(
                commonProtocols & set(protocolList), key=lambda x: getIndexInList(x, protocolList)
            )
        ...
        for plugin in self.storages:
            ...
            if methodName == "putFile" and inputProtocol:
                if inputProtocol not in pluginParameters["InputProtocols"]:
                    log.debug(
                        "Plugin %s not appropriate for %s protocol as input." % (pluginName, inputProtocol)
                    )
                    continue
    ...
    def __executeMethod(self, lfn, *args, **kwargs):
        ...
        if args:
            ...
            if methDefaultArgs:
                ...
                args = args[1:]
            log.verbose(
                "put it in kwargs, but dirty and might be dangerous!args %s kwargs %s" % (args, kwargs)
            )
        ...
        for storage in filteredPlugins:
            ...
            if not urlDict:
                ...
                log.verbose(
                    "Attempting to perform '%s' for %s physical files" % (self.methodName, len(urlDict))
                )
                fcn = None
                ...
                if not res["OK"]:
                    ...
                    for lfn in urlDict.values():
                        if lfn not in failed:
                            failed[lfn] = ""
                        failed[lfn] = (
                            "%s %s" % (failed[lfn], res["Message"]) if failed[lfn] else res["Message"]
                        )
    ...
    def addAccountingOperation(self, lfns, startDate, elapsedTime, storageParameters, callRes):
        ...
        if self.methodName not in (
            self.readMethods + self.writeMethods + self.removeMethods + self.stageMethods
        ):
            return
