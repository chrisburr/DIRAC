...
class GFAL2_StorageBase(StorageBase):
        ...
        try:
            ...
            if useChecksum:
                ...
                if destSize == remoteSize:
                    ...
                    errStr = (
                        "File sizes don't match. Something went wrong. Removing local file %s" % dest_file
                    )
    ...
    def prestageFile(self, path, lifetime=86400):
        ...
        self.log.debug(
            "GFAL2_StorageBase.prestageFile: Attempting to issue stage requests for %s file(s)." % len(urls)
        )
    ...
    def prestageFileStatus(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.prestageFileStatus: Checking the staging status for %s file(s)." % len(urls)
        )
    ...
    def isDirectory(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.isDirectory: checking whether %s path(s) are directory(ies)." % len(urls)
        )
    ...
    def getDirectorySize(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.getDirectorySize: Attempting to get size of %s directories" % len(urls)
        )
    ...
    def __getSingleDirectorySize(self, path):
        ...
        self.log.debug(
            "GFAL2_StorageBase.__getSingleDirectorySize: Attempting to get the size of directory %s" % path
        )
    ...
    def __getSingleDirectoryMetadata(self, path):
        ...
        """
        self.log.debug(
            "GFAL2_StorageBase.__getSingleDirectoryMetadata: Fetching metadata of directory %s." % path
        )
