...
def loadConfiguration():
    ...
    rConfig = diracInstall.ReleaseConfig(
        instName=diracInstall.cliParams.installation, globalDefaultsURL=diracInstall.cliParams.globalDefaults
    )
    ...
    for opName in ("release", "globalDefaults", "useVersionsDir"):
        try:
            opVal = rConfig.getInstallationConfig("LocalInstallation/%s" % (opName[0].upper() + opName[1:]))
        except KeyError:
            continue
    ...
    res = rConfig.loadProjectRelease(
        diracInstall.cliParams.release,
...
if __name__ == "__main__":
    ...
    result = releaseConfig.getModulesToInstall(
        diracInstall.cliParams.release, diracInstall.cliParams.extensions
    )
    ...
    ddeLocation = os.path.join(
        diracInstall.cliParams.targetPath, "DIRAC", "Core", "scripts", "dirac-deploy-scripts.py"
    )
    if not os.path.isfile(ddeLocation):
        ddeLocation = os.path.join(
            diracInstall.cliParams.targetPath, "DIRAC", "Core", "scripts", "dirac_deploy_scripts.py"
        )
