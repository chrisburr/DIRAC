...
def ssl_verify_callback_print_error(ok, store):
    ...
    if errnum:
        print(
            "SSL DEBUG ERRNUM %s ERRMSG %s" % (errnum, m2.x509_get_verify_error(errnum))
...
def getM2SSLContext(ctx=None, **kwargs):
    ...
    if kwargs.get("skipCACheck", False):
        ...
        # Do validate peer
        ctx.set_verify(
            SSL.verify_peer | SSL.verify_fail_if_no_peer_cert, VERIFY_DEPTH, callback=verify_callback
        )
        # Set CA location
