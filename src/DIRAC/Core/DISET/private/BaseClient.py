...
class BaseClient(object):
    ...
    def __discoverCredentialsToUse(self):
        ...
        * Proxy Chain
        ...
        if self.KW_PROXY_CHAIN in self.kwargs:
            try:
                self.kwargs[self.KW_PROXY_STRING] = self.kwargs[self.KW_PROXY_CHAIN].dumpAllToString()[
                    "Value"
                ]
    ...
    def __findServiceURL(self):
        ...
        try:
            ...
            return S_ERROR(
                "Cannot get URL for %s in setup %s: %s" % (self._destinationSrv, self.setup, repr(e))
            )
        if not urls:
...
===============================================================""" % (
    ...
    def _connect(self):
        ...
        try:
            ...
            if not retVal["OK"]:
                gLogger.warn(
                    "Issue getting socket:", "%s : %s : %s" % (transport, self.__URLTuple, retVal["Message"])
                )
                # We try at most __nbOfRetry each URLs
