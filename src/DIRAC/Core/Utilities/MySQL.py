...
class MySQL(object):
    ...
    def __init__(
        self, hostName="localhost", userName="dirac", passwd="dirac", dbName="", port=3306, debug=False
    ):
        """
    ...
    def _except(self, methodName, x, err, cmd=""):
        ...
        try:
            ...
            self.log.error(
                "%s (%s): %s" % (methodName, self._safeCmd(cmd), err), "%d: %s" % (e.args[0], e.args[1])
            )
    ...
    def _createTables(self, tableDict, force=False):
        ...
        if not isinstance(tableDict, dict):
            return S_ERROR(
                DErrno.EMYSQL, "Argument is not a dictionary: %s( %s )" % (type(tableDict), tableDict)
            )
        ...
        for table in tableList:
            ...
            if not isinstance(thisTable, dict):
                return S_ERROR(
                    DErrno.EMYSQL,
                    "Table description is not a dictionary: %s( %s )" % (type(thisTable), thisTable),
                )
            ...
            for table in tableList:
                ...
                if "PrimaryKey" in thisTable:
                    if isinstance(thisTable["PrimaryKey"], six.string_types):
                        ...
                        cmdList.append(
                            "PRIMARY KEY ( %s )"
                            % ", ".join(["`%s`" % str(f) for f in thisTable["PrimaryKey"]])
                        )
    ...
    def _getFields(
        ...
        return self.getFields(
            tableName, outFields, condDict, limit, conn, older, newer, timeStamp, orderAttribute
        )
    ...
    def executeStoredProcedureWithCursor(self, packageName, parameters):
        ...
        try:
            ...
            execStr = "call %s(%s);" % (
                ...
                ",".join(
                    [
                        '"%s"' % param if isinstance(param, six.string_types) else str(param)
