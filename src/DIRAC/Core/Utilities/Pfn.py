...
def default_pfnunparse(pfnDict):
    ...
    try:
        if not isinstance(pfnDict, dict):
            return S_ERROR(
                "pfnunparse: wrong type for pfnDict argument, expected a dict, got %s" % type(pfnDict)
            )
...
def srm_pfnparse(pfn):
    ...
    if not pfn:
        return S_ERROR(
            "wrong 'pfn' argument value in function call, expected non-empty string, got %s" % str(pfn)
        )
