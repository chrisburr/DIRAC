        ...
        try:
            ...
            # or a class?
                ...
                if not callable(taskObj):
                    raise TypeError(
                        "__call__ operator not defined not in %s class" % taskObj.__class__.__name__
                    )
                # ## call it at least
...
class ProcessPool(object):
    ...
    def __spawnWorkingProcess(self):
        ...
        try:
            worker = WorkingProcess(
                self.__pendingQueue, self.__resultsQueue, self.__stopEvent, self.__keepRunning
            )
    ...
    def createAndQueueTask(
        ...
        """
        task = ProcessTask(
            taskFunction, args, kwargs, taskID, callback, exceptionCallback, usePoolCallbacks, timeOut
        )
        ...
        while True:
            ...
            if self.__resultsQueue.empty():
                if self.__resultsQueue.qsize():
                    log.warn(
                        "Results queue is empty but has non zero size", "%d" % self.__resultsQueue.qsize()
                    )
