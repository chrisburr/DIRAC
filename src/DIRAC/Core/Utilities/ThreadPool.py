...
class ThreadedJob(object):
    def __init__(
        self, oCallable, args=None, kwargs=None, sTJId=None, oCallback=None, oExceptionCallback=None
    ):
...
if __name__ == "__main__":
    ...
    def generateWork(iWorkUnits):
        for iNumber in [random.randint(1, 20) for _ in range(iWorkUnits)]:
            oTJ = ThreadedJob(
                doSomething, args=(iNumber,), oCallback=showResult, oExceptionCallback=showException
            )
            OTP.queueJob(oTJ)
