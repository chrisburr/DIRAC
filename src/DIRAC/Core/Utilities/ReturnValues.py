...
class SErrorException(Exception):
  """Exception class for use with `convertReturnValue`"""
...
def unwrap(result):
  """Unwrap an S_OK/S_ERROR response

  This method assits with using exceptions in DIRAC code by raising
  :exc:`SErrorException` if `result` is an error. This can then by propogated
