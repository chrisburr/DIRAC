...
class GraphData(object):
    ...
    def makeNumKeys(self):
        ...
        if self.key_type == "string":
            ...
            self.all_num_keys = [
                date2num(datetime.datetime.fromtimestamp(to_timestamp(key))) for key in self.all_keys
            ]
    ...
    def getPlotNumData(self, label=None, zipFlag=True):
        ...
        if self.plotdata:
            if zipFlag:
                return zip(
                    self.plotdata.getNumKeys(), self.plotdata.getValues(), self.plotdata.getPlotErrors()
                )
            else:
            ...
            for label in self.subplots:
                arrays.append(
                    numpy.array(
                        [x[1] for x in self.subplots[label].getPlotDataForNumKeys(self.all_num_keys, True)]
                    )
                )
...
class PlotData(object):
    ...
    def initialize(self):
        ...
        if self.key_type == "string":
            ...
            self.num_keys = [
                date2num(datetime.datetime.fromtimestamp(to_timestamp(key))) for key in self.keys
            ]
        elif self.key_type == "numeric":
