...
class X509Chain(object):
    ...
    * You are delegating
    ...
    def generateProxyToString(
        self, lifetime, diracGroup=False, strength=1024, limited=False, proxyKey=False, rfc=True
    ):
        """
        ...
        if not proxyKey:
            ...
            proxyKey.assign_rsa(
                M2Crypto.RSA.gen_key(strength, 65537, callback=M2Crypto.util.quiet_genparam_callback)
            )
    ...
    def generateProxyToFile(
        self, filePath, lifetime, diracGroup=False, strength=1024, limited=False, rfc=True
    ):
        """
