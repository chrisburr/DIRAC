...
class TransformationDB(DB):
    ...
    def addTransformation(
        ...
        if inheritedFrom:
            ...
            if not res["OK"]:
                gLogger.error(
                    "Failed to update parent transformation agent type, now deleting", res["Message"]
                )
                return self.deleteTransformation(transID, connection=connection)
        ...
        if addFiles and inputMetaQuery:
            ...
            if filesToAdd:
                ...
                if not res["OK"]:
                    gLogger.error(
                        "Failed to add files to transformation", "%s %s" % (transID, res["Message"])
                    )
    ...
    def __addAdditionalTransformationParameter(self, transID, paramName, paramValue, connection=False):
        req = "DELETE FROM AdditionalParameters WHERE TransformationID=%d AND ParameterName='%s'" % (
            transID,
            paramName,
        )
        res = self._update(req, connection)
    ...
    def getTransformationFiles(
        ...
        if not fileIDs:
            ...
            for row in transFiles:
                ...
                rList = [lfn] + [
                    str(item) if not isinstance(item, six.integer_types) else item for item in row
                ]
                webList.append(rList)
    ...
    def getTransformationStats(self, transName, connection=False):
        ...
        res = self.getCounters(
            "TransformationFiles", ["TransformationID", "Status"], {"TransformationID": transID}
        )
        if not res["OK"]:
            return res
        statusDict = dict(
    ...
    def __assignTransformationFile(self, transID, taskID, se, fileIDs, connection=False):
        ...
        req = "INSERT INTO TransformationFileTasks (TransformationID,FileID,TaskID) VALUES %s" % ",".join(
            fileTuples
        )
        res = self._update(req, connection)
        if not res["OK"]:
    ...
    def __deleteTransformationFileTask(self, transID, taskID, connection=False):
        ...
        """
        req = "DELETE FROM TransformationFileTasks WHERE TransformationID=%d AND TaskID=%d" % (
            transID,
            taskID,
        )
        return self._update(req, connection)
    ...
    def reserveTask(self, transName, taskID, connection=False):
        ...
        if not res["OK"]:
            return S_ERROR(
                "Failed to set Reserved status for job %d - failed to update the time stamp" % int(taskID)
            )
        return S_OK()
    ...
    def deleteTransformationMetaQuery(self, transName, queryType, author="", connection=False):
        ...
        queryType = res["Value"]
        req = "DELETE FROM TransformationMetaQueries WHERE TransformationID=%d AND QueryType=%s;" % (
            transID,
            queryType,
        )
        res = self._update(req, connection)
    ...
    def __addDirectory(self, path, force):
        ...
        if path not in res["Value"]["Successful"]:
            ...
            return res
        gLogger.info(
            "TransformationDB.addDirectory: Obtained %s files in %s seconds." % (path, time.time() - start)
        )
        successful = []
        failed = []
