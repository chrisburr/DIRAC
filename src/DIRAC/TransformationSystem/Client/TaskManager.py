    ...
    def _multiOperationsBody(self, transJson, taskDict, ownerDN, ownerGroup):
        ...
        for taskID, task in list(taskDict.items()):
            ...
            if not task.get("InputData"):
                self._logError(
                    "Error creating request for task", "%s, No input data" % taskID, transID=transID
                )
                taskDict.pop(taskID)
                continue
            files = []
...
class WorkflowTasks(TaskBase):
    ...
    def __prepareTasksBulk(self, transBody, taskDict, owner, ownerGroup, ownerDN):
        ...
        self._logVerbose(
            "Setting job owner:group to %s:%s" % (owner, ownerGroup), transID=transID, method=method
        )
        oJob.setOwner(owner)
        ...
        for taskID in sorted(taskDict):
            ...
            for paramName, paramValue in paramsDict.items():
                if paramName not in ("InputData", "Site", "TargetSE"):
                    if paramValue:
                        self._logVerbose(
                            "Setting %s to %s" % (paramName, paramValue), transID=transID, method=method
                        )
            ...
            if self.outputDataModule:
                ...
                if not res["OK"]:
                    self._logError(
                        "Failed to generate output data", res["Message"], transID=transID, method=method
                    )
                    continue
        ...
        if taskDict:
            self._logInfo(
                "Prepared %d tasks" % len(taskDict), transID=transID, method=method, reftime=startTime
            )
    ...
    def __prepareTasks(self, transBody, taskDict, owner, ownerGroup, ownerDN):
        ...
        for taskID, paramsDict in taskDict.items():
            ...
            if not templateOK:
                ...
                self._logVerbose(
                    "Job owner:group to %s:%s" % (owner, ownerGroup), transID=transID, method=method
                )
                transGroup = str(transID).zfill(8)
                self._logVerbose(
                    "Adding default transformation group of %s" % (transGroup), transID=transID, method=method
                ...
                if not oJobTemplate.workflow.findParameter("JOB_ID"):
                    oJobTemplate._addParameter(
                        oJobTemplate.workflow, "JOB_ID", "string", "00000000", "Initial JOB_ID"
                    )
            ...
            if not sites:
                ...
                if not res["OK"]:
                    self._logError(
                        "Could not set the site: %s" % res["Message"], transID=transID, method=method
                    )
                    paramsDict["TaskObject"] = ""
                    continue
    ...
    def _handleInputs(self, oJob, paramsDict):
        ...
        if inputData:
            ...
            if not res["OK"]:
                self._logError(
                    "Could not set the inputs: %s" % res["Message"], transID=transID, method="_handleInputs"
                )
    ...
    def _handleRest(self, oJob, paramsDict):
        ...
        for paramName, paramValue in paramsDict.items():
            if paramName not in ("InputData", "Site", "TargetSE"):
                if paramValue:
                    self._logDebug(
                        "Setting %s to %s" % (paramName, paramValue), transID=transID, method="_handleRest"
                    )
    ...
    def _checkSickTransformations(self, transID):
        ...
        clinicPath = "Hospital"
        if transID in set(
            int(x) for x in self.opsH.getValue(os.path.join(clinicPath, "Transformations"), [])
        ):
            return clinicPath
    ...
    def updateTransformationReservedTasks(self, taskDicts):
        transID = None
        jobNames = [
            self._transTaskName(taskDict["TransformationID"], taskDict["TaskID"]) for taskDict in taskDicts
        ]
