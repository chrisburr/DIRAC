...
class TransformationClient(Client):
    ...
    def getTransformations(
        ...
        while True:
            res = rpcClient.getTransformations(
                condDict, older, newer, timeStamp, orderAttribute, limit, extraParams, offsetToApply
            )
            if not res["OK"]:
                return res
            else:
                gLogger.verbose(
                    "Result for limit %d, offset %d: %d" % (limit, offsetToApply, len(res["Value"]))
                )
                if res["Value"]:
    ...
    def moveFilesToDerivedTransformation(self, transDict, resetUnused=True):
        ...
        if not res["OK"]:
            log.error(
                " Error getting Unused files from transformation", "%d: %s" % (parentProd, res["Message"])
            )
            return res
            ...
            for lfnChunk in breakListIntoChunks(lfnList, 5000):
                ...
                if not res["OK"]:
                    ...
                    if not res["OK"]:
                        log.error(
                            " Error setting status in transformation",
                            " %d: status %s for %d files" % (parentProd, oldStatus, len(lfnChunk)),
                            res["Message"],
                        )
                else:
                    log.info(
        ...
        if res["OK"]:
            ...
            if assignedFiles:
                ...
                if res["OK"]:
                    parentFiles = res["Value"]
                    processedLfns = [
                        lfnDict["LFN"] for lfnDict in parentFiles if lfnDict["Status"] == "Processed"
                    ]
                    if processedLfns:
    ...
    def setFileStatusForTransformation(self, transName, newLFNsStatus=None, lfns=None, force=False):
        ...
        if tsFiles:
            ...
            tsFilesAsDict = dict(
                (tsFile["LFN"], [tsFile["Status"], tsFile["ErrorCount"], tsFile["FileID"]])
