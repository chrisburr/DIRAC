...
class PluginUtilities(object):
    ...
    def groupByReplicas(self, files, status):
        ...
        flush = status == "Flush"
        self.logVerbose(
            "groupByReplicas: %d files, groupSize %d, flush %s" % (len(files), self.groupSize, flush)
        )
    ...
    def groupBySize(self, files, status):
        ...
        flush = status == "Flush"
        self.logVerbose(
            "groupBySize: %d files, groupSize: %d, flush: %s" % (len(files), self.groupSize, flush)
        )
        ...
        for groupSE in (True, False):
            ...
            self.logVerbose(
                "groupBySize: %d tasks created with groupSE %s" % (len(tasks) - nTasks, str(groupSE))
            )
    ...
    def _normaliseShares(originalShares):
        ...
        return dict(
            [(site, 100.0 * float(share) / total if total else 0.0) for site, share in originalShares.items()]
        )
