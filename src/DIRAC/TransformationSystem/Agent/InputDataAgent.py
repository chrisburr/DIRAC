        ...
        for transDict in result["Value"]:
            ...
            if not res["OK"]:
                if cmpError(res, ENOENT):
                    self.log.info(
                        "InputDataAgent.execute: No input data query found for transformation", transID
                    )
                else:
                    self.log.error(
            ...
            self.log.verbose(
                "Using input data query for transformation", "%d: %s" % (transID, str(inputDataQuery))
            )
            start = time.time()
            ...
            if lfnList:
                ...
                if not result["OK"]:
                    self.log.warn(
                        "InputDataAgent.execute: failed to add lfns to transformation", result["Message"]
                    )
                    ...
                    if result["Value"]["Successful"]:
                        ...
                        self.log.info(
                            "InputDataAgent.execute: Added files to transformation", "(%d)" % len(addedLfns)
                        )
