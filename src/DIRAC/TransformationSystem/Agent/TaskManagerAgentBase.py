...
class TaskManagerAgentBase(AgentModule, TransformationAgentsUtilities):
    ...
    def execute(self):
        ...
        if resProxy["OK"]:  # there is a shifterProxy
            ...
            self.log.info(
                "ShifterProxy: Tasks will be submitted with the credentials %s:%s" % (owner, ownerGroup)
            )
        ...
        while True:
            ...
            try:
                ...
                for operation in operations:
                    ...
                    if not res["OK"]:
                        self._logError(
                            "Failed to %s: %s" % (operation, res["Message"]), method=method, transID=transID
                        )
                    self._logInfo(
    ...
    def checkReservedTasks(self, transIDOPBody, clients):
        ...
        self._logDebug(
            "getTransformationTasks(%s) return value:" % condDict, res, method=method, transID=transID
        )
        if not res["OK"]:
    ...
    def submitTasks(self, transIDOPBody, clients):
        ...
        if not tasksToSubmit["OK"]:
            self._logError(
                "Failed to obtain tasks:", tasksToSubmit["Message"], method=method, transID=transID
            )
    ...
    def _prepareAndSubmitAndUpdateTasks(self, transID, transBody, tasks, owner, ownerDN, ownerGroup, clients):
        ...
        if not res["OK"]:
            self._logError(
                "Failed to update DB after task submission:", res["Message"], method=method, transID=transID
            )
            return res
