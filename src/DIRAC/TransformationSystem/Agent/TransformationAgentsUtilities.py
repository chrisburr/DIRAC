...
class TransformationAgentsUtilities(object):
    ...
    def _logVerbose(self, message, param="", method="execute", transID="None", reftime=None):
        """verbose"""
        if self.debug:
            gLogger.getSubLogger("(V) " + self.__prefixForLogging(transID, method, reftime)).info(
                message, param
            )
        else:
    ...
    def _logException(
        self, message, param="", lException=False, method="execute", transID="None", reftime=None
    ):
        """exception"""
        gLogger.getSubLogger(self.__prefixForLogging(transID, method, reftime)).exception(
            message, param, lException
        )
