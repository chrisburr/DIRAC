...
class ValidateOutputDataAgent(AgentModule):
    ...
    def updateWaitingIntegrity(self):
        ...
        for transDict in transDicts:
            ...
            if not res["OK"]:
                ...
                if not res["OK"]:
                    gLogger.error(
                        "Failed to update status of transformation %s to ValidatedOutput" % (transID)
                    )
                else:
                    gLogger.info("Updated status of transformation %s to ValidatedOutput" % (transID))
            else:
                gLogger.info(
                    "%d problematic files for transformation %s were found" % (len(res["Value"]), transID)
                )
        return
    ...
    def finalizeCheck(self, transID):
        ...
        if not res["OK"]:
            gLogger.error(
                "Failed to determine whether there were associated problematic files", res["Message"]
            )
            newStatus = ""
        elif res["Value"]:
            gLogger.info(
