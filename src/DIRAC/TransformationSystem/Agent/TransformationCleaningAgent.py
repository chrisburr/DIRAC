...
class TransformationCleaningAgent(AgentModule):
    ...
    def initialize(self):
        ...
        self.log.info(
            "Will search for directories in the following locations: %s" % str(self.directoryLocations)
        )
        # # transformation metadata
    ...
    def finalize(self):
        ...
        if transformationIDs:
            ...
            jobGroupsToCheck = [
                str(transDict["TransformationID"]).zfill(8) for transDict in toClean + toArchive
            ]
    ...
    def cleanContent(self, directory):
        ...
        if not filesFound:
            ...
            return S_OK()
        self.log.info(
            "Attempting to remove %d possible remnants from the catalog and storage" % len(filesFound)
        )
    ...
    def removeTransformationOutput(self, transID):
        ...
        if not res["OK"]:
            self.log.error(
                'Problem obtaining directories for transformation %s with result "%s"' % (transID, res)
            )
            return S_OK()
      ...
      and its files from the storage for transformation %s"
        ...
        if not res["OK"]:
            self.log.error(
                "Failed to update status of transformation %s to RemovedFiles" % (transID), res["Message"]
            )
            return res
    ...
    def archiveTransformation(self, transID):
        ...
        if not res["OK"]:
            self.log.error(
                "Failed to update status of transformation %s to Archived" % (transID), res["Message"]
            )
            return res
    ...
    def cleanTransformation(self, transID):
        ...
        if not res["OK"]:
            self.log.error(
                "Failed to update status of transformation %s to Cleaned" % (transID), res["Message"]
            )
            return res
    ...
    def __removeWMSTasks(self, transJobIDs):
        ...
        for jobList in breakListIntoChunks(jobIDs, 500):
            ...
            if res["OK"]:
                ...
                self.log.error(
                    "Failed to kill %s jobs because not authorized" % len(res["NonauthorizedJobIDs"])
                )
                allRemove = False
            ...
            if res["OK"]:
                ...
                self.log.error(
                    "Failed to remove %s jobs because not authorized" % len(res["NonauthorizedJobIDs"])
                )
                allRemove = False
            elif "FailedJobIDs" in res:
