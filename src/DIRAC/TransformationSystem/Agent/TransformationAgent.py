...
class TransformationAgent(AgentModule, TransformationAgentsUtilities):
        ...
        while True:
            ...
            try:
                ...
                if not transID:
                    transID = "None"
                self._logInfo(
                    "Processed transformation in %.1f seconds" % (time.time() - startTime), transID=transID
                )
    ...
    def processTransformation(self, transDict, clients):
        ...
        if created:
            self._logInfo(
                "Successfully created %d tasks for transformation." % created, method=method, transID=transID
            )
        else:
    ...
    def _getTransformationFiles(self, transDict, clients, statusList=None, replicateOrRemove=False):
        ...
        if not transFiles:
            ...
            if transDict["Status"] == "Flush":
                ...
                if not res["OK"]:
                    ...
                    self._logInfo(
                        "Updated transformation status to 'Active'.", method=method, transID=transID
                    )
            return S_OK()
        ...
        if not kickTrans and skipIfNoNewUnused and noUnusedDelay:
            nextStamp = self.unusedTimeStamp.setdefault(transID, now) + datetime.timedelta(
                hours=noUnusedDelay
            )
            skip = now < nextStamp
            if (
                ...
                self._logInfo(
                    "No new '%s' files found for transformation." % ",".join(statusList),
                    method=method,
                    transID=transID,
                )
                return S_OK()
    ...
    def __getDataReplicas(self, transDict, lfns, clients, forJobs=True):
        ...
        if newLFNs:
            startTime = time.time()
            self._logInfo(
                "Getting replicas for %d files from catalog" % len(newLFNs), method=method, transID=transID
            )
            newReplicas = {}
    ...
    def _getDataReplicasDM(self, transID, lfns, clients, forJobs=True, ignoreMissing=False):
        ...
        problematicLfns = [
            lfn for lfn in lfns if lfn not in replicas["Successful"] and lfn not in replicas["Failed"]
        ]
        if problematicLfns:
        ...
        for lfn, replicaDict in replicas["Successful"].items():
            for se in replicaDict:
                ...
                if forJobs and "failover" in se.lower():
                    self._logVerbose(
                        "Ignoring failover replica for %s." % lfn, method=method, transID=transID
                    )
                else:
    ...
    def __cleanReplicas(self, transID, lfns):
        ...
        if toRemove:
            self._logInfo(
                "Remove %d files from cache" % len(toRemove), method="__cleanReplicas", transID=transID
            )
    ...
    def __removeFilesFromCache(self, transID, lfns):
        ...
        if removed:
            self._logInfo(
                "Removed %d replicas from cache" % removed, method="__removeFilesFromCache", transID=transID
            )
            self.__writeCache(transID)
    ...
    def __generatePluginObject(self, plugin, clients):
        ...
        try:
            ...
            self._logException(
                "Failed to create %s()" % plugin, lException=e, method="__generatePluginObject"
            )
            return S_ERROR()
