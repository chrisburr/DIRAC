...
class DataRecoveryAgent(AgentModule):
    ...
    def treatTransformation(self, transID, transInfoDict):
        ...
        tInfo = TransformationInfo(
            transID, transInfoDict, self.enabled, self.tClient, self.fcClient, self.jobMon
        )
    ...
    def checkAllJobs(self, jobs, tInfo, tasksDict=None, lfnTaskDict=None):
        ...
        for counter, job in enumerate(jobs.values()):
            if counter % self.printEveryNJobs == 0:
                self.log.notice(
                    "Checking Jobs %d/%d: %3.1fs" % (counter, nJobs, float(time.time() - jobCheckStart))
                )
            while True:
                try:
        ...
        self.log.notice(
            "Checking Jobs Done: %d/%d: %3.1fs" % (counter, nJobs, float(time.time() - jobCheckStart))
        )
