...
class ReqClient(Client):
    ...
    def putRequest(self, request, useFailoverProxy=True, retryMainService=0):
        ...
        self.log.warn(
            "putRequest: unable to set request '%s' at RequestManager" % request.RequestName,
    ...
    def getRequest(self, requestID=0):
        ...
        if not getRequest["OK"]:
            self.log.error(
                "getRequest: unable to get request", "'%s' %s" % (requestID, getRequest["Message"])
            )
            return getRequest
    ...
    def getBulkRequests(self, numberOfRequest=10, assigned=True):
        ...
        if not getRequests["OK"]:
            self.log.error(
                "getRequests: unable to get '%s' requests: %s" % (numberOfRequest, getRequests["Message"])
            )
            return getRequests
        # No Request returned
    ...
    def finalizeRequest(self, requestID, jobID, useCertificates=True):
        ...
        if not res["OK"]:
            ...
            if digest["OK"]:
                ...
                if not res["OK"]:
                    self.log.info(
                        "finalizeRequest: Failed to set job %d parameter: %s" % (jobID, res["Message"])
                    )
                    return res
            else:
                self.log.error(
            ...
            if newJobStatus:
                self.log.info(
                    "finalizeRequest: Updating job status for %d to %s/Requests done" % (jobID, newJobStatus)
                )
            else:
                self.log.info(
