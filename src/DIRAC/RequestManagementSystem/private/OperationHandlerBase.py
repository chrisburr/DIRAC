...
class OperationHandlerBase(object):
    ...
    def setOperation(self, operation):
        ...
        self.log = gLogger.getSubLogger(
            "pid_%s/%s/%s/%s"
            % (os.getpid(), self.request.RequestName, self.request.Order, self.operation.Type)
        )
    ...
    def getProxyForLFN(self, lfn):
        ...
        ownerRole = (
            "/%s" % dirMeta["OwnerRole"] if not dirMeta["OwnerRole"].startswith("/") else dirMeta["OwnerRole"]
        )
