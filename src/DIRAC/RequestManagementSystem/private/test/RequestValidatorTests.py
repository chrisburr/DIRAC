...
class RequestValidatorTests(unittest.TestCase):
    ...
    def testValidator(self):
        ...
        ret = validator.validate(self.request)
        self.assertEqual(
            ret, {"Message": "Operation #0 in request 'test_request' hasn't got Type set", "OK": False}
        )
        ...
        self.assertEqual(
            ret,
            {
                "Message": "Operation #0 of type 'ReplicateAndRegister' hasn't got files to process.",
                "OK": False,
            },
        )
        ...
        self.assertEqual(
            ret,
            {
                "Message": "Operation #0 of type 'ReplicateAndRegister' is missing TargetSE attribute.",
                "OK": False,
            },
        )
        ...
        self.assertEqual(
            ret,
            {"Message": "File in operation #0 is missing Checksum (abcdef) or ChecksumType ()", "OK": False},
        )
        ...
        self.assertEqual(
            ret,
            {"Message": "File in operation #0 is missing Checksum () or ChecksumType (ADLER32)", "OK": False},
        )
