...
def main():
    ...
    if transID:
        ...
        if res["Value"]["Failed"]:
            gLogger.error(
                "No request found for jobs %s" % ",".join(sorted(str(job) for job in res["Value"]["Failed"]))
            )
    ...
    if status and not requests:
        ...
        if not res["OK"]:
            ...
            DIRAC.exit(2)
        requests = [
            reqID for reqID, _st, updTime in res["Value"] if updTime > since and updTime <= until and reqID
        ]
