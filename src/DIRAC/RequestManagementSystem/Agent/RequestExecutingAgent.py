    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.__rmsMonitoring:
            ...
            gMonitor.registerActivity(
                "Iteration", "Agent Loops", "RequestExecutingAgent", "Loops/min", gMonitor.OP_SUM
            )
            gMonitor.registerActivity(
    ...
    def putAllRequests(self):
        ...
        for requestID in self.__requestCache.keys():
            ...
            if not reset["OK"]:
                ...
                self.log.debug(
                    "putAllRequests: request %s has been put back with its initial state" % requestID
                )
        return S_OK()
            ...
            for request in requestsToExecute:
                ...
                while True:
                    if not self.processPool().getFreeSlots():
                        ...
                        self.log.info(
                            "spawning task for request", "'%s/%s'" % (request.RequestID, request.RequestName)
                        )
