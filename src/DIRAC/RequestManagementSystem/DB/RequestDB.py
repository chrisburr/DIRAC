...
class RequestDB(object):
        ...
        try:
            ...
            try:
                if hasattr(request, "RequestID"):
                    ...
                    status = (
                        session.query(Request._Status).filter(Request.RequestID == request.RequestID).one()
                    )
        ...
        try:
            if getJobID:
                reqQuery = session.query(
                    Request.RequestID, Request._Status, Request._LastUpdate, Request.JobID
    ...
    def getDBSummary(self):
        ...
        try:
            requestQuery = (
                session.query(Request._Status, func.count(Request.RequestID)).group_by(Request._Status).all()
            )
                ...
                if key == "ToDate":
                    ...
                    if key == "Type":
                        summaryQuery = summaryQuery.join(Request.__operations__).group_by(
                            Request.RequestID, Operation.Type
                        )
                        tableName = "Operation"
                    elif key == "Status":
                        key = "_Status"
    ...
    def getRequestIDsForJobs(self, jobIDs):
        ...
        try:
            ...
            reqDict["Failed"] = dict(
                (jobid, "Request not found") for jobid in jobIDs - set(reqDict["Successful"])
            )
        except Exception as e:
