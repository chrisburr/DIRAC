...
class AccountingDB(DB):
    def __init__(self, name="Accounting/AccountingDB", readOnly=False):
        ...
        gMonitor.registerActivity(
            "insertiontime", "Record insertion time", "Accounting", "seconds", gMonitor.OP_MEAN
        )
        gMonitor.registerActivity(
        ...
        for pythonClassName in sorted(objectsLoaded):
            ...
            for setup in setupsList:
                ...
                retVal = self.registerType(
                    typeName, definitionKeyFields, definitionAccountingFields, bucketsLength
                )
                if not retVal["OK"]:
    ...
    def registerType(self, name, definitionKeyFields, definitionAccountingFields, bucketsLength):
        ...
        if self.__readOnly:
            if tables:
                self.log.notice(
                    "ReadOnly mode: Skipping create of tables for %s. Removing from memory catalog" % name
                )
    ...
    def deleteType(self, typeName):
        ...
        if not retVal["OK"]:
            return retVal
        retVal = self._update(
            "DELETE FROM `%s` WHERE name='%s'" % (_getTableName("catalog", "Types"), typeName)
        )
    ...
    def __getIdForKeyValue(self, typeName, keyName, keyValue, conn=False):
        ...
        retVal = self._query(
            "SELECT `id` FROM `%s` WHERE `value`=%s" % (_getTableName("key", typeName, keyName), keyValue),
            conn=conn,
        )
        if not retVal["OK"]:
            return retVal
    ...
    def __insertInQueueTable(self, typeName, startTime, endTime, valuesList):
        ...
        if len(sqlFields) != len(sqlValues):
            ...
            return S_ERROR(
                "Fields mismatch for record %s. %s fields and %s expected" % (typeName, numRcv, numExp)
            )
    ...
    def deleteRecord(self, typeName, startTime, endTime, valuesList):
        ...
        for i in range(len(sqlValues)):
            ...
            if needToRound:
                compVal = [
                    "`%s`.`%s`" % (mainTable, self.dbCatalog[typeName]["typeFields"][i]),
                    "%f" % sqlValues[i],
                ]
                ...
                sqlCond.append(
                    "`%s`.`%s`=%s" % (mainTable, self.dbCatalog[typeName]["typeFields"][i], sqlValues[i])
                )
        retVal = self._getConnection()
        if not retVal["OK"]:
            return retVal
    ...
    def retrieveRawRecords(self, typeName, startTime, endTime, condDict, orderFields, connObj=False):
        ...
        return self.__queryType(
            typeName, startTime, endTime, selectFields, condDict, False, orderFields, "type"
        )
    ...
    def __queryType(
        ...
        for keyName in condDict:
            ...
            if keyName in self.dbCatalog[typeName]["keys"]:
                List.appendUnique(
                    sqlLinkList,
                    "`%s`.`%s` = `%s`.`id`" % (tableName, keyName, _getTableName("key", typeName, keyName)),
                )
            ...
            for keyValue in condDict[keyName]:
                ...
                if keyName in self.dbCatalog[typeName]["keys"]:
                    sqlORList.append(
                        "`%s`.`value` = %s" % (_getTableName("key", typeName, keyName), keyValue)
                    )
                else:
    ...
    def __compactBucketsForType(self, typeName):
        ...
        for bPos in range(len(self.dbBucketsLength[typeName]) - 1):
            ...
            self.log.info(
                "[COMPACT] Finished compaction %d of %d" % (bPos, len(self.dbBucketsLength[typeName]) - 1)
            )
        # return self.__commitTransaction( connObj )
        return S_OK()
    ...
    def __slowCompactBucketsForType(self, typeName):
        ...
        for bPos in range(len(self.dbBucketsLength[typeName]) - 1):
            ...
            self.log.info(
                "[COMPACT] Finised compaction %d of %d" % (bPos, len(self.dbBucketsLength[typeName]) - 1)
            )
        # return self.__commitTransaction(connObj)
        return S_OK()
        ...
        for sqlQuery in sqlQueries:
            ...
            for entry in rawData:
                ...
                if rebucketedRecords % 1000 == 0:
                    ...
                    expectedEnd = str(
                        datetime.timedelta(seconds=int((numRecords - rebucketedRecords) / blockAvg))
                    )
                    self.log.info(
