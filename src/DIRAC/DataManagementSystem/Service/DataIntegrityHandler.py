...
class DataIntegrityHandler(RequestHandler):
    ...
    def export_removeProblematic(self, fileID):
        ...
        if not res["OK"]:
            self.log.error(
                "DataIntegrityHandler.removeProblematic: Failed to remove problematic.", res["Message"]
            )
        return res
    ...
    def export_getPrognosisProblematics(self, prognosis):
        ...
        self.log.info(
            "DataIntegrityHandler.getPrognosisProblematics: Getting files with %s prognosis." % prognosis
        )
    ...
    def export_setProblematicStatus(self, fileID, status):
        ...
        self.log.info(
            "DataIntegrityHandler.setProblematicStatus: Setting file %s status to %s." % (fileID, status)
        )
    ...
    def export_incrementProblematicRetry(self, fileID):
        ...
        self.log.info(
            "DataIntegrityHandler.incrementProblematicRetry: Incrementing retries for file %s." % (fileID)
        )
    ...
    def export_changeProblematicPrognosis(self, fileID, newPrognosis):
        ...
        if not res["OK"]:
            self.log.error(
                "DataIntegrityHandler.changeProblematicPrognosis: Failed to update.", res["Message"]
            )
        return res
        ...
        if res["OK"]:
            for prognosis, statusDict in res["Value"].items():
                ...
                for status, count in statusDict.items():
                    self.log.info(
                        "DataIntegrityHandler.getProblematicsSummary: \t%-10s %-10s." % (status, str(count))
                    )
        else:
            self.log.error(
                "DataIntegrityHandler.getProblematicsSummary: Failed to get summary.", res["Message"]
            )
        return res
    ...
    def export_getDistinctPrognosis(self):
        ...
        if res["OK"]:
            ...
            self.log.error(
                "DataIntegrityHandler.getDistinctPrognosis: Failed to get unique prognosis.", res["Message"]
            )
        return res
