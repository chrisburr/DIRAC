...
class MoveReplicaFailure(ReqOpsTestCase):
    ...
    def test__dmTransfer(self):
        ...
        for sourceSE in self.op.sourceSEList:
            successful[sourceSE] = (
                "dips://" + sourceSE.lower() + ":9148/DataManagement/StorageElement" + self.File.LFN
            )
    ...
    def test__dmRemoval(self):
        ...
        res = {
            ...
            "Value": {
                "Successful": {},
                "Failed": {self.File.LFN: "Write access not permitted for this credential"},
            },
        }
        self.mr.dm.removeReplica.return_value = res
        ...
        resvalue = dict(
            [(targetSE, "Write access not permitted for this credential") for targetSE in targetSEs]
        )
        self.assertEqual(res["Value"], {self.File.LFN: resvalue})
