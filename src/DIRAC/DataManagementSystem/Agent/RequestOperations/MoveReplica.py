...
class MoveReplica(DMSRequestOperationsBase):
    ...
    def __call__(self):
        ...
        for opFile in waitingFiles:
            ...
            if not res["OK"]:
                ...
                self.log.info(
                    "todo: %s replicas to delete from %s sites" % (len(toRemoveDict), len(targetSEs))
                )
    ...
    def dmTransfer(self, opFile):
        ...
        if noReplicas:
            ...
            self.log.error(
                "Unable to replicate", "%s, replicas corrupted at %s" % (lfn, someReplicasCorrupted[lfn])
            )
        ...
        for targetSE in self.operation.targetSEList:
            ...
            if res["OK"]:
                if lfn in res["Value"]["Successful"]:
                    ...
                    self.log.error(
                        "Failed to replicate and register", "File %s at %s: %s" % (lfn, targetSE, reason)
                    )
                    opFile.Error = reason
            else:
