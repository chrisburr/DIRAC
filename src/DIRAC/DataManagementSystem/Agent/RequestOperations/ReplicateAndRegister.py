...
def filterReplicas(opFile, logger=None, dataManager=None):
    ...
    if not replicas:
        ...
        if allReplicas["OK"]:
            ...
            log.verbose(
                "File has no%s replica in File Catalog" % ("" if noReplicas else " active"), opFile.LFN
            )
        else:
            return allReplicas
        ...
        for opFile in self.getWaitingFilesList():
            ...
            if validReplicas:
                ...
                if noMetaReplicas:
                    ...
                    self.log.warn(
                        "unable to schedule %s, could not get a PFN at %s" % (opFile.LFN, ",".join(noPFN))
                    )
            ...
            for targetSE in self.operation.targetSEList:
                ...
                if res["OK"]:
                    ...
                    if lfn in res["Value"]["Successful"]:
                        ...
                        self.log.error(
                            "Failed to replicate and register", "File %s at %s:" % (lfn, targetSE), reason
                        )
                        opFile.Error = reason
