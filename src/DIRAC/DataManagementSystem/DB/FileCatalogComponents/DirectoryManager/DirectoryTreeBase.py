...
class DirectoryTreeBase(object):
    ...
    def _setDirectoryParameter(self, path, pname, pvalue):
        ...
        req = (
            ...
            "ModificationDate=UTC_TIMESTAMP() WHERE DirID IN ( %s )" % (pname, pvalue, dirIDString)
        )
        result = self.db._update(req)
        return result
    ...
    def getDirectorySize(self, lfns, longOutput=False, rawFileTables=False, recursiveSum=True):
        ...
        if rawFileTables:
            resultLogical = self._getDirectoryLogicalSize(
    ...
    def __rebuildDirectoryUsage(self, directoryID):
        ...
        for seID in resultDict:
            ...
            req = (
                "UPDATE FC_DirectoryUsage SET SESize=SESize+%d, SEFiles=SEFiles+%d WHERE DirID=%d AND SEID=%d"
            )
    ...
    def getDirectoryCounters(self, connection=False):
        ...
        req = "SELECT COUNT(DirID) FROM %s WHERE Parent NOT IN ( SELECT DirID from %s )" % (
            treeTable,
            treeTable,
        )
        req += " AND DirID <> 1"
        ...
        req = "SELECT COUNT(DirID) FROM %s WHERE DirID NOT IN ( SELECT Parent from %s )" % (
            treeTable,
            treeTable,
        )
