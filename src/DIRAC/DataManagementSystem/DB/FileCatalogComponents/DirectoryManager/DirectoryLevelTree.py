...
class DirectoryLevelTree(DirectoryTreeBase):
    ...
    def getPathIDs(self, path):
        ...
        req = "SELECT DirID FROM FC_DirectoryLevelTree WHERE DirName in (%s) ORDER BY DirID" % ",".join(
            pathString
        )
        result = self.db._query(req)
        if not result["OK"]:
    ...
    def getPathIDsByID(self, dirID):
        ...
        for lev in range(level):
            sel = " AND ".join(
                ["Level=%d" % lev] + ["LPATH%d=%d" % (ll + 1, lpaths[ll]) for ll in range(lev)]
            )
    ...
    def getSubdirectoriesByID(self, dirID, requestString=False, includeParent=False):
        ...
        if level > 0:
            ...
            req += " JOIN (SELECT %s FROM FC_DirectoryLevelTree WHERE DirID=%d) AS F2 ON " % (
                pathString,
                dirID,
            )
            sPaths = []
    ...
    def recoverOrphanDirectories(self, credDict):
        ...
        req = "SELECT DirID,Parent,Level FROM %s WHERE Parent NOT IN ( SELECT DirID from %s )" % (
            treeTable,
            treeTable,
        )
        result = self.db._query(req)
