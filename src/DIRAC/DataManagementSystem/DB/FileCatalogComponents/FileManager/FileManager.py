...
class FileManager(FileManagerBase):
        ...
        for lfn in lfns.keys():
            ...
            if repID:
                ...
                insertReplicas.append(
                    "(%d,'%s',UTC_TIMESTAMP(),UTC_TIMESTAMP(),'%s')" % (repID, replicaType, pfn)
                )
        if insertReplicas:
    ...
    def _getRepIDsForReplica(self, replicaTuples, connection=False):
        ...
        req = "SELECT RepID,FileID,SEID FROM FC_Replicas WHERE (FileID,SEID) IN (%s)" % intListToString(
            queryTuples
        )
        res = self.db._query(req, connection)
        if not res["OK"]:
    ...
    def _setFileParameter(self, fileID, paramName, paramValue, connection=False):
        ...
        if paramName in ["UID", "GID", "Status", "Size"]:
            ...
            if "select" in fileIDString.lower():
                tmpreq = (
                    "UPDATE FC_Files as FF1, ( %s ) as FF2 %%s WHERE FF1.FileID=FF2.FileID" % fileIDString
                )
            else:
    ...
    def __getFileIDReplicas(self, fileIDs, allStatus=False, connection=False):
        ...
        req = "SELECT FileID,SEID,RepID,Status FROM FC_Replicas WHERE FileID IN (%s)" % (
    ...
    def repairFileTables(self, connection=False):
        ...
        for fileID in fileIDsToAdd:
            guid = makeGuid()
            insertTuples.append(
                "(%d,'%s',UTC_TIMESTAMP(),UTC_TIMESTAMP(),%d)" % (int(fileID), guid, self.db.umask)
            )
