...
class UserAndGroupManagerDB(UserAndGroupManagerBase):
    ...
    def __addUser(self, uname):
        ...
        waitTime = time.time()
        gLogger.debug(
            "UserGroupManager AddUser lock created. Waited %.3f seconds. %s" % (waitTime - startTime, uname)
        )
        if uname in self.db.users.keys():
            uid = self.db.users[uname]
            gLogger.debug(
                "UserGroupManager AddUser lock released. Used %.3f seconds. %s"
    ...
    def __removeUser(self, uname):
        ...
        waitTime = time.time()
        gLogger.debug(
            "UserGroupManager RemoveUser lock created. Waited %.3f seconds. %s"
            % (waitTime - startTime, uname)
        )
    ...
    def _refreshUsers(self):
        ...
        waitTime = time.time()
        gLogger.debug(
            "UserGroupManager RefreshUsers lock created. Waited %.3f seconds." % (waitTime - startTime)
        )
        req = "SELECT UID,UserName from FC_Users"
        res = self.db._query(req)
        if not res["OK"]:
            gLogger.debug(
                "UserGroupManager RefreshUsers lock released. Used %.3f seconds." % (time.time() - waitTime)
            )
            self.lock.release()
            return res
        self.db.users = {}
    ...
    def __addGroup(self, group):
        ...
        waitTime = time.time()
        gLogger.debug(
            "UserGroupManager AddGroup lock created. Waited %.3f seconds. %s" % (waitTime - startTime, group)
        )
        if group in self.db.groups.keys():
            gid = self.db.groups[group]
            gLogger.debug(
                "UserGroupManager AddGroup lock released. Used %.3f seconds. %s"
    ...
    def __removeGroup(self, group):
        ...
        waitTime = time.time()
        gLogger.debug(
            "UserGroupManager RemoveGroup lock created. Waited %.3f seconds. %s"
            % (waitTime - startTime, group)
        )
    ...
    def _refreshGroups(self):
        ...
        waitTime = time.time()
        gLogger.debug(
            "UserGroupManager RefreshGroups lock created. Waited %.3f seconds." % (waitTime - startTime)
        )
        res = self.db._query(req)
        if not res["OK"]:
            gLogger.debug(
                "UserGroupManager RefreshGroups lock released. Used %.3f seconds." % (time.time() - waitTime)
            )
            self.lock.release()
            return res
        self.db.groups = {}
