...
class DataIntegrityClient(Client):
    ...
    def setFileProblematic(self, lfn, reason, sourceComponent=""):
        ...
        if isinstance(lfn, list):
            ...
            errStr = (
                "DataIntegrityClient.setFileProblematic: Supplied file info must be list or a single LFN."
            )
    ...
    def setReplicaProblematic(self, replicaTuple, sourceComponent=""):
        ...
        if isinstance(replicaTuple, tuple):
            ...
            return S_ERROR(errStr)
        gLogger.info(
            "DataIntegrityClient.setReplicaProblematic: Attempting to update %s replicas." % len(replicaTuple)
        )
        replicaDict = {}
    ...
    def resolveCatalogPFNSizeMismatch(self, problematicDict):
        ...
        if catalogSize == bookkeepingSize:
            gLogger.info(
                "CatalogPFNSizeMismatch replica (%d) found to mismatch the bookkeeping also" % fileID
            )
        ...
        gLogger.info(
            "CatalogPFNSizeMismatch replica (%d) all sizes found mismatch. Updating retry count" % fileID
        )
