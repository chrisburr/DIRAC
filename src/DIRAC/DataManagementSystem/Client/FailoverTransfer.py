...
class FailoverTransfer(object):
    ...
    def transferAndRegisterFile(
        ...
        for se in destinationSEList:
            ...
            for sleeptime in (10, 60, 300, 600):
                ...
                ).putAndRegister(lfn, localPath, se, guid=fileGUID, checksum=fileChecksum)
            ...
            if not result["Value"]["Failed"]:
                self.log.info(
                    "dm.putAndRegister successfully uploaded and registered", "%s to %s" % (fileName, se)
                )
            ...
            if not result["OK"]:
                self.log.error(
                    "Failed to set registration request", "SE %s and metadata: \n%s" % (se, fileMetaDict)
                )
                errorList.append(
    ...
    def transferAndRegisterFileFailover(
        ...
        self.log.info(
            "Attempting to set replica removal request", "for LFN %s at failover SE %s" % (lfn, failoverSE)
        )
