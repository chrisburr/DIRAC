...
class DataManager(object):
    ...
    def __init__(self, catalogs=None, masterCatalogOnly=False, vo=False):
        ...
        if catalogs is None:
            catalogs = []
        catalogsToUse = (
            FileCatalog(vo=self.voName).getMasterCatalogNames()["Value"] if masterCatalogOnly else catalogs
        )
    ...
    def __cleanDirectory(self, folder):
        ...
        if not areDirs["OK"]:
            return areDirs
        listOfFolders = [
            aDir for aDir in areDirs["Value"]["Successful"] if areDirs["Value"]["Successful"][aDir]
        ]
    ...
    def __replicate(self, lfn, destSEName, sourceSEName="", destPath="", localCache=""):
        ...
        for candidateSEName in possibleSourceSEs:
            ...
            if not res["OK"]:
                log.verbose(
                    "The storage element is not currently valid.", "%s %s" % (candidateSEName, res["Message"])
                )
                continue
            else:
            ...
            if seFileSize != catalogSize:
                log.debug(
                    "Catalog size and physical file size mismatch.", "%s %s" % (catalogSize, seFileSize)
                )
                continue
            else:
    ...
    def __registerReplica(self, replicaTuples, catalog):
        ...
        for storageElementName, replicaTuple in seDict.items():  # can be an iterator
            ...
            if not res["OK"]:
                ...
                for lfn, url in replicaTuple:
                    res = returnSingleResult(
                        destStorageElement.getURL(lfn, protocol=self.registrationProtocol)
                    )
                    if not res["OK"]:
                        failed[lfn] = res["Message"]
                    else:
    ...
    def __removeFile(self, lfnDict):
        ...
        if completelyRemovedFiles:
            ...
            if not res["OK"]:
                failed.update(
                    dict.fromkeys(
                        completelyRemovedFiles, "Failed to remove file from the catalog: %s" % res["Message"]
                    )
                )
            else:
                failed.update(res["Value"]["Failed"])
    ...
    def __filterReplicasForJobs(self, replicaDict):
        ...
        # Get a cache of SE statuses for long list of replicas
        seStatus = dict(
            (se, (self.dmsHelper.isSEForJobs(se), self.dmsHelper.isSEArchive(se))) for se in seList
        )
        # Beware, there is a del below
        for lfn, replicas in list(replicaDict["Successful"].items()):
