...
class CreateArchiveRequest(object):
    ...
    def __init__(self):
        ...
        self.options = [
            ...
            (
                "N",
                "Name",
                "Name of the Tarball, if not given: Path_Tars/Path_N.tar" " will be used to store tarballs",
            ),
    ...
    def registerSwitchesAndParseCommandLine(self):
        ...
        for _short, longOption, _doc in self.options:
            ...
            if defaultValue:
                sLog.verbose(
                    "Found default value in the CS for %r with value %r" % (longOption, defaultValue)
                )
    ...
    def createRequest(self, requestName, archiveLFN, lfnChunk):
        ...
        if self.switches.get("CheckMigration"):
            ...
            migrationTarget = (
                self.targetSE if self.switches.get("ReplicateTarball") else self.switches["TarballSE"]
            )
    ...
    def _checkReplicaSites(self, request, lfnChunk):
        ...
        for lfn, replInfo in resReplica["Value"]["Successful"].items():
            if sourceSE in replInfo:
                ...
                sLog.notice(
                    "WARN: LFN %r not found at source, only at: %s" % (lfn, ",".join(replInfo.keys()))
                )
                notAt.append(lfn)
