...
class ProductionDB(DB):
    ...
    def addProduction(self, prodName, prodDescription, authorDN, authorGroup, connection=False):
        ...
        if res["OK"]:
            return S_ERROR(
                "Production with name %s already exists with ProductionID = %d" % (prodName, res["Value"])
            )
    ...
    def __setProductionStatus(self, prodID, status, connection=False):
        ...
        """
        req = "UPDATE Productions SET Status='%s', LastUpdate=UTC_TIMESTAMP() WHERE ProductionID=%d" % (
            status,
            prodID,
        )
    ...
    def _getProductionID(self, prodName, connection=False):
        ...
        if not res["OK"]:
            gLogger.error(
                "Failed to obtain production ID for production", "%s: %s" % (prodName, res["Message"])
            )
            return res
        elif not res["Value"]:
