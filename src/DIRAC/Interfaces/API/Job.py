...
class Job(API):
    ...
    def setExecutable(
        self, executable, arguments="", logFile="", modulesList=None, parameters=None, paramValues=None
    ):
        """Helper function.
    ...
    def setInputSandbox(self, files):
        ...
        if isinstance(files, list) and files:
            ...
            return self._reportError(
                "Expected file string or list of files for input sandbox contents", **kwargs
            )
    ...
    def setOutputSandbox(self, files):
        ...
        if isinstance(files, list) and files:
            ...
            kwargs = {"files": files}
            return self._reportError(
                "Expected file string or list of files for output sandbox contents", **kwargs
            )
    ...
    def setInputDataPolicy(self, policy, dataScheduling=True):
        ...
        if not jobPolicy:
            return self._reportError(
                "Could not get value for Operations option %s/%s" % (csSection, finalPolicy),
                __name__,
                **kwargs
            )
    ...
    def setOutputData(self, lfns, outputSE=None, outputPath=""):
        ...
        if isinstance(lfns, list) and lfns:
            ...
            return self._reportError(
                "Expected file name string or list of file names for output data", **kwargs
            )
    ...
    def setPlatform(self, platform):
        ...
        if not platform.lower() == "any":
            ...
            if platform in availablePlatforms["Value"]:
                self._addParameter(
                    self.workflow, "Platform", "JDL", platform, "Platform ( Operating System )"
                )
            else:
        ...
        if not isinstance(timeInSecs, int):
            try:
                ...
                if not re.search("{{", timeInSecs):
                    return self._reportError(
                        "Expected numerical string or int for CPU time in seconds", **kwargs
                    )
    ...
    def setNumberOfProcessors(
        self, numberOfProcessors=None, minNumberOfProcessors=None, maxNumberOfProcessors=None
    ):
        """Helper function.
        ...
        if environmentDict:
            ...
            for var, val in environmentDict.items():
                try:
                    ...
                    return self._reportError(
                        "Expected string for environment variable key value pairs", **kwargs
                    )
