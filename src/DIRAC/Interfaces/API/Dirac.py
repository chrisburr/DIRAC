...
class Dirac(API):
    ...
    def submitJob(self, job, mode="wms"):
        ...
        if isinstance(job, six.string_types):
            ...
            try:
                ...
                if not result["OK"]:
                    self.log.error(
                        'Pre-submission checks failed for job with message: "%s"' % (result["Message"])
                    )
                    return result
        ...
        if mode.lower() == "local":
            ...
            result = WMSClient(useCertificates=self.useCertificates).submitJob(
                jdlAsString, jobDescriptionObject
            )
            if not result["OK"]:
    ...
    def getInputDataCatalog(self, lfns, siteName="", fileName="pool_xml_catalog.xml", ignoreMissing=False):
        ...
        if not inputDataPolicy:
            return self._errorReport(
                "Could not retrieve /DIRAC/Operations/InputDataPolicy/InputDataModule for VO"
            )
    ...
    def runLocal(self, job):
        ...
        if inputData:
            ...
            self.log.info(
                "Job has input data requirement, will attempt to resolve data for %s" % DIRAC.siteName()
            )
            ...
            for isFile in sandbox:
                ...
                if os.path.exists(os.path.join(os.getcwd(), os.path.basename(isFile))):
                    ...
                    if os.path.isabs(isFile) and os.path.exists(isFile):
                        ...
                        self.log.debug(
                            "Input Sandbox %s found in the submission directory, copying it" % isFile
                        )
                        ...
                        self.log.debug(
                            "Input Sandbox %s is a file, found in the tmp directory, copying it" % isFile
                        )
                        shutil.copy(os.path.join(tmpdir, isFile), os.getcwd())
                    else:
                        self.log.verbose(
                            "perhaps the file %s is in an LFN, so we attempt to download it." % isFile
                        )
                        getFile = self.getFile(isFile)
                        if not getFile["OK"]:
                            self.log.warn(
                                "Failed to download %s with error: %s" % (isFile, getFile["Message"])
                            )
                ...
                if tarfile.is_tarfile(basefname):
                    try:
                        ...
                        return S_ERROR(
                            "Could not untar or extract %s with exception %s" % (basefname, repr(x))
                        )
        ...
        result = systemCall(
            0, cmdSeq=shlex.split(command), env=executionEnv, callbackFunction=self.__printOutput
        )
        if not result["OK"]:
            return result
    ...
    def getJobDebugOutput(self, jobID):
        ...
        if not result["OK"]:
            self.log.info(
                "Could not obtain status information for jobID %s, please check this is valid." % jobID
            )
        ...
        try:
            ...
            for param, value in paramsDict.items():  # can be an iterator
                if re.search("{", value):
                    ...
                    rawValues = (
                        value.replace("{", "").replace("}", "").replace('"', "").replace("LFN:", "").split()
                    )
                    valueList = []
