...
def AgentOptionsTest(agentPath, options, mocker):
    ...
    for name, member in inspect.getmembers(agentModule):
        LOG.info(
            "Mocking? %s, %s, %s, isclass(%s)", name, callable(member), type(member), inspect.isclass(member)
        )
...
def checkAgentOptions(getOptionMock, systemName, agentName, ignoreOptions=None, extension="DIRAC"):
    ...
    configFilePath = os.path.join(
        os.path.dirname(os.path.dirname(DIRAC.__file__)), extension, systemName, "ConfigTemplate.cfg"
    )
    ...
    for option, value in optionsDict.items():
        ...
        if not isinstance(value, bool) and not value:  # empty string, list, dict ...
            assert any(
                call(option, null) in getOptionMock.call_args_list for null in ({}, set(), [], "", 0, None)
            )
        else:
            assert (
