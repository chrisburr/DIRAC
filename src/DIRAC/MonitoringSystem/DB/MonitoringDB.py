...
class MonitoringDB(ElasticDB):
    ...
    def __init__(self, name="Monitoring/MonitoringDB", readOnly=False):
        ...
        try:
            ...
            super(MonitoringDB, self).__init__(
                dbname=name.split("/")[1], fullName=name, indexPrefix=indexPrefix
            )
    ...
    def __getRawData(self, typeName, condDict, size=-1):
        ...
        if condDict.get("startTime") and condDict.get("endTime"):
            query = self._Q(
                "range",
                timestamp={"lte": condDict.get("endTime") * 1000, "gte": condDict.get("startTime") * 1000},
            )
        ...
        if hits and "hits" in hits and hits["hits"]:
            ...
            for resObj in hits["hits"]:
                records.append(
                    dict([(paramName, getattr(resObj["_source"], paramName)) for paramName in paramNames])
                )
            return S_OK(records)
