...
class StorageManagerHandler(RequestHandler):
    ...
    def export_getStagedReplicas(cls):
        ...
        if not res["OK"]:
            gLogger.error(
                "getStagedReplicas: Failed to obtain Staged/StageSubmitted replicas", res["Message"]
            )
        return res
  ...
  '''
    ...
    def export_getReplicasWithStatus(cls, status):
        ...
        if not res["OK"]:
            gLogger.error(
                "getReplicasWithStatus: Failed to get replicas with %s status" % status, res["Message"]
            )
        return res
    ...
    def export_getStageSubmittedReplicas(cls):
        ...
        if not res["OK"]:
            gLogger.error(
                "getStageSubmittedReplicas: Failed to obtain StageSubmitted replicas", res["Message"]
            )
        return res
