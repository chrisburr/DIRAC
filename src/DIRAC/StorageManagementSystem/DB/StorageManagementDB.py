...
class StorageManagementDB(DB):
    ...
    def __updateTaskStatus(self, taskIDs, newTaskStatus, force=False, connection=False):
        ...
        for record in resSelect["Value"]:
            ...
            gLogger.verbose(
                "%s.%s_DB: to_update Tasks =  %s" % (self._caller(), "__updateTaskStatus", record)
            )
        ...
        if len(taskIDs) > 0:
            ...
            if not resSelect1["OK"]:
                ...
                for record in resSelect1["Value"]:
                    gLogger.verbose(
                        "%s.%s_DB: updated Tasks = %s" % (self._caller(), "__updateTaskStatus", record)
                    )
    ...
    def updateReplicaStatus(self, replicaIDs, newReplicaStatus, connection=False):
        ...
        for record in resSelect["Value"]:
            ...
            gLogger.verbose(
                "%s.%s_DB: to_update CacheReplicas =  %s" % (self._caller(), "updateReplicaStatus", record)
            )
        if len(replicaIDs) > 0:
    ...
    def getTaskSummary(self, jobID, connection=False):
        ...
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB.getTaskSummary: Failed to get Replica summary for task.", res["Message"]
            )
            return res
        replicaInfo = {}
    ...
    def _createTask(self, source, callbackMethod, sourceTaskID, connection=False):
        ...
        if not resSelect["OK"]:
            ...
            gLogger.verbose(
                "%s.%s_DB: inserted Tasks = %s" % (self._caller(), "_createTask", resSelect["Value"][0])
            )
    ...
    def _getExistingReplicas(self, storageElement, lfns, connection=False):
        ...
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB._getExistingReplicas: Failed to get existing replicas.", res["Message"]
            )
            return res
        existingReplicas = {}
    ...
    def _insertReplicaInformation(self, lfn, storageElement, rType, connection=False):
        ...
        if not res["OK"]:
            gLogger.error(
                "_insertReplicaInformation: Failed to insert to CacheReplicas table.", res["Message"]
            )
            return res
    ...
    def getStagedReplicas(self, connection=False):
        ...
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB.getStagedReplicas: Failed to get eligible TaskReplicas", res["Message"]
            )
            return res
        goodTasks = []
    ...
    def getWaitingReplicas(self, connection=False):
        ...
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB.getWaitingReplicas: Failed to get eligible TaskReplicas", res["Message"]
            )
            return res
        badTasks = []
        goodTasks = []
    ...
    def getOfflineReplicas(self, connection=False):
        ...
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB.getOfflineReplicas: Failed to get eligible TaskReplicas", res["Message"]
            )
            return res
        badTasks = []
        goodTasks = []
    ...
    def setStageComplete(self, replicaIDs):
        ...
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB.setStageComplete: Failed to set StageRequest completed.", res["Message"]
            )
            return res

        for record in resSelect["Value"]:
            gLogger.verbose(
                "%s.%s_DB: to_update StageRequests =  %s" % (self._caller(), "setStageComplete", record)
            )
        ...
        for record in resSelect1["Value"]:
            gLogger.verbose(
                "%s.%s_DB: updated StageRequests = %s" % (self._caller(), "setStageComplete", record)
            )
  ...
  '''
    ...
    def setTasksDone(self, taskIDs):
        ...
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB.setTasksDone: Failed to set Tasks status to Done.", res["Message"]
            )
            return res
