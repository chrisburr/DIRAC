...
class RequestFinalizationAgent(AgentModule):
    ...
    def callbackDoneTasks(self):
        ...
        doneTasks = res["Value"]
        gLogger.info(
            "RequestFinalization.callbackDoneTasks: Obtained %s tasks in the 'Done' status." % len(doneTasks)
        )
        ...
        if not res["OK"]:
            gLogger.fatal(
                "RequestFinalization.callbackDoneTasks: Failed to remove Done tasks.", res["Message"]
            )
        return res
    ...
    def callbackStagedTasks(self):
        ...
        for taskID, (_source, callback, sourceTask) in stagedTasks.items():
            if callback and sourceTask:
                ...
                if not res["OK"]:
                    ...
                    gLogger.info(
                        "RequestFinalization.callbackStagedTasks, Task = %s: %s" % (sourceTask, res["Value"])
                    )
        ...
        if not res["OK"]:
            gLogger.fatal(
                "RequestFinalization.callbackStagedTasks: Failed to remove staged Tasks.", res["Message"]
            )
        return res
    ...
    def __performCallback(self, status, callback, sourceTask):
        ...
        gLogger.debug(
            "RequestFinalization.__performCallback: Attempting to invoke %s service method" % method
        )
