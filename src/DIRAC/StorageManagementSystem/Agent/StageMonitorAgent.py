...
class StageMonitorAgent(AgentModule):
    ...
    def __getStageSubmittedReplicas(self):
        ...
        if not res["Value"]:
            return S_ERROR(
                "Could not obtain request IDs for replicas %s from StageRequests table" % list(replicaIDs)
            )
    ...
    def __wakeupOldRequests(self, oldRequests):
        ...
        if not res["OK"]:
            gLogger.error(
                "StageMonitor.__wakeupOldRequests: Failed to resubmit old requests.", res["Message"]
            )
            return res
        return S_OK()
