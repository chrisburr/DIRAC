...
class StageRequestAgent(AgentModule):
    ...
    def submitStageRequests(self):
        ...
        * Offline -> StageSubmitted (if there are not more Waiting replicas)
        ...
        gLogger.info(
            "StageRequest.submitStageRequests: Obtained %s replicas for staging." % len(allReplicaInfo)
        )
        for storageElement, seReplicaIDs in seReplicas.items():
            gLogger.debug("Staging at %s:" % storageElement, seReplicaIDs)
            self._issuePrestageRequests(storageElement, seReplicaIDs, allReplicaInfo)
    ...
    def _getMissingReplicas(self):
        ...
        if not res["OK"]:
            gLogger.fatal(
                "StageRequest._getMissingReplicas: Failed to get associated Replicas.", res["Message"]
            )
    ...
    def _getOnlineReplicas(self):
        ...
        * Waiting -> Offline (if the file is not found Cached)
        ...
        if not len(allReplicaInfo):
            ...
            return res
        gLogger.info(
            "StageRequest._getOnlineReplicas: Obtained %s replicas Waiting for staging." % len(allReplicaInfo)
        )
        replicasToStage = []
        for storageElement, seReplicaIDs in res["Value"]["SEReplicas"].items():
        ...
        if not res["OK"]:
            gLogger.fatal(
                "StageRequest._getOnlineReplicas: Failed to get associated Replicas.", res["Message"]
            )
    ...
    def _getOfflineReplicas(self):
        ...
        if not res["OK"]:
            gLogger.fatal(
                "StageRequest._getOfflineReplicas: Failed to get associated Replicas.", res["Message"]
            )
    ...
    def _issuePrestageRequests(self, storageElement, seReplicaIDs, allReplicaInfo):
        ...
        if lfnRepIDs:
            ...
            gLogger.debug(
                "StageRequest._issuePrestageRequests: StorageElement.prestageStorageFile: res=", res
            )
        ...
        if stageRequestMetadata:
            ...
            if not res["OK"]:
                gLogger.error(
                    "StageRequest._issuePrestageRequests: Failed to insert replica status.", res["Message"]
                )
        return
    ...
    def __addAssociatedReplicas(self, replicasToStage, seReplicas, allReplicaInfo):
        ...
        if not res["OK"]:
            gLogger.fatal(
                "StageRequest.__addAssociatedReplicas: Failed to get associated Replicas.", res["Message"]
            )
            return res
    ...
    def __checkIntegrity(self, storageElement, seReplicaIDs, allReplicaInfo):
        ...
        for lfn, metadata in res["Value"]["Successful"].items():
            ...
            if metadata["Size"] != allReplicaInfo[lfnRepIDs[lfn]]["Size"]:
                gLogger.error(
                    "StageRequest.__checkIntegrity: LFN StorageElement size does not match FileCatalog", lfn
                )
                ...
                gLogger.error(
                    "StageRequest.__checkIntegrity: LFN is declared Unavailable by the StorageElement", lfn
                )
        ...
        if terminalReplicaIDs:
            gLogger.info(
                "StageRequest.__checkIntegrity: %s replicas are terminally failed." % len(terminalReplicaIDs)
            )
            res = self.stagerClient.updateReplicaFailure(terminalReplicaIDs)
            if not res["OK"]:
                gLogger.error(
                    "StageRequest.__checkIntegrity: Failed to update replica failures.", res["Message"]
                )
        if onlineReplicaIDs:
