...
class ComponentInstaller(object):
    ...
    def printStartupStatus(self, rDict):
        ...
        try:
            for comp in rDict:
                records.append(
                    [comp, rDict[comp]["RunitStatus"], rDict[comp]["Timeup"], str(rDict[comp]["PID"])]
                )
    ...
    def getSoftwareComponents(self, extensions):
        ...
        remainingTypes = [
            cType for cType in self.componentTypes if cType not in ["service", "agent", "executor"]
        ]
        resultIndexes = {}
                    ...
                    for component in softDict[compType][system]:
                        ...
                        try:
                            ...
                            if compDir in runitDict:
                                ...
                                resultDict[compType][system][component]["Timeup"] = runitDict[compDir][
                                    "Timeup"
                                ]
                                try:
                                    resultDict[compType][system][component]["PID"] = int(
                                        runitDict[compDir]["PID"]
                                    )
                                except ValueError:
                                    resultDict[compType][system][component]["PID"] = -1
                                try:
                                    resultDict[compType][system][component]["CPU"] = float(
                                        runitDict[compDir]["CPU"]
                                    )
                                except ValueError:
                                    resultDict[compType][system][component]["CPU"] = -1
                                try:
                                    resultDict[compType][system][component]["MEM"] = float(
                                        runitDict[compDir]["MEM"]
                                    )
                                except ValueError:
                                    resultDict[compType][system][component]["MEM"] = -1
                                try:
                                    resultDict[compType][system][component]["RSS"] = float(
                                        runitDict[compDir]["RSS"]
                                    )
                                except ValueError:
                                    resultDict[compType][system][component]["RSS"] = -1
                                try:
                                    resultDict[compType][system][component]["VSZ"] = float(
                                        runitDict[compDir]["VSZ"]
                                    )
                                except ValueError:
    ...
    def checkComponentModule(self, componentType, system, module):
        ...
        if componentType == "agent":
            ...
            loader = ModuleLoader(
                "Service", PathFinder.getServiceSection, RequestHandler, moduleSuffix="Handler"
            )
    ...
    def setupSite(self, scriptCfg, cfg=None):
        ...
        if setupAddConfiguration:
            ...
            for system, agent in setupAgents:
                if not self.addDefaultOptionsToCS(None, "agent", system, agent, extensions, overwrite=True)[
                    "OK"
                ]:
                    # If we are not allowed to write to the central CS, add the configuration to the local file
                    ...
                    if not res["OK"]:
                        gLogger.warn(
                            "Can't write to the specific component CFG", "for %s : %s" % (system, agent)
                        )
            for system, executor in setupExecutors:
                if not self.addDefaultOptionsToCS(
                    ...
                    if not res["OK"]:
                        gLogger.warn(
                            "Can't write to the specific component CFG", "for %s : %s" % (system, executor)
                        )
        else:
...
exec svlogd .
  """
    ...
    def installComponent(
        self, componentType, system, component, extensions, componentModule="", checkModule=True
    ):
        """
  ...
  """
    ...
    def getAvailableSQLDatabases(self, extensions):
        ...
        for extension in extensions + [""]:
            databases = glob.glob(
                os.path.join(
                    rootPath, ("%sDIRAC" % extension).replace("DIRACDIRAC", "DIRAC"), "*", "DB", "*.sql"
                )
            )
    ...
    def getAvailableESDatabases(self, extensions):
        ...
        for extension in extensions + [""]:
            ...
            pyDBs = glob.glob(
                os.path.join(
                    rootPath, ("%sDIRAC" % extension).replace("DIRACDIRAC", "DIRAC"), "*", "DB", "*DB.py"
                )
            )
    ...
    def installDatabase(self, dbName):
        ...
        for extension in CSGlobals.getCSExtensions():
            dbFileInExtension = glob.glob(
                os.path.join(rootPath, "%sDIRAC" % extension, "*", "DB", "%s.sql" % dbName)
            )
