...
class ProxyManagerClient(object):
    ...
    def __refreshUserCache(self, validSeconds=0):
        ...
        for record in data:
            ...
            self.__usersCache.add(
                cacheKey, self.__getSecondsLeftToExpiration(record["expirationtime"]), record
            )
        return S_OK()
    ...
    def downloadProxyToFile(
        ...
        retVal = self.downloadProxy(
            userDN, userGroup, limited, requiredTimeLeft, cacheTime, proxyToConnect, token
        )
        if not retVal["OK"]:
            return retVal
        chain = retVal["Value"]
    ...
    def renewProxy(
        self, proxyToBeRenewed=None, minLifeTime=3600, newProxyLifeTime=43200, proxyToConnect=None
    ):
