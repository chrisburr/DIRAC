            ...
            for system in systems:
                ...
                for cType in ("agent", "service"):
                    ...
                    result = gConfig.getSections(
                        "/Systems/%s/%s/%s" % (system, instance, "%ss" % cType.capitalize())
                    )
                    if not result["OK"]:
                        self.log.warn(
    ...
    def __addMissingDefinedComponents(self, compDictList):
        ...
        for compDict in compDictList:
            compDict = self.__setStatus(
                compDict, "Error", "Component is not up or hasn't connected to register yet"
            )
            cD.append(compDict)
            ...
            if "Status" not in dbComp:
                ...
                if dbComp["Type"] == "service":
                    if "Port" not in dbComp:
                        ...
                        compDict["Port"] for compDict in compDictList if "Port" in compDict
                    ]:
                        self.__setStatus(
                            compDictList[-1],
                            "Error",
