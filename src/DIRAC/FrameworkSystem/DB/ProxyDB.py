...
class ProxyDB(DB):
    ...
    def __addUserNameToTable(self, tableName):
        ...
        for userName, userDN in data:
            if not userName:
                ...
                userName = result["Value"]
                result = self._update(
                    "UPDATE `%s` SET UserName=%s WHERE UserDN=%s" % (tableName, userName, userDN)
                )
                if not result["OK"]:
    ...
    def renewFromMyProxy(self, userDN, userGroup, lifeTime=None, chain=None):
        ...
        if chainGroup != userGroup:
            return S_ERROR(
                "Mismatch between renewed proxy group and expected: %s vs %s" % (userGroup, chainGroup)
            )
    ...
    def getCredentialsAboutToExpire(self, requiredSecondsLeft, onlyPersistent=True):
        ...
        cmd += (
            " WHERE TIMESTAMPDIFF( SECOND, ExpirationTime, UTC_TIMESTAMP() ) < %d and " % requiredSecondsLeft
        )
    ...
    def sendExpirationNotifications(self):
        ...
        notifLimits = sorted(
            [int(x) for x in self.getCSOption("NotificationTimes", ProxyDB.NOTIFICATION_TIMES)]
        )
