...
class ComponentSupervisionAgent(AgentModule):
    ...
    def sendNotification(self):
        ...
        for address in self.addressTo:
            res = self.nClient.sendMail(
                address, self.emailSubject, emailBody, self.addressFrom, localAttempt=False
            )
            if not res["OK"]:
    ...
    def on_terminate(self, componentName, process):
        ...
        self.log.info(
            "%s's process with ID: %s has been terminated successfully" % (componentName, process.pid)
        )
    ...
    def execute(self):
        ...
        for instanceType in ("executor", "agent", "service"):
            for name, options in getattr(self, instanceType + "s").items():
                ...
                if not res["OK"]:
                    self.logError(
                        "Failure when checking %s" % instanceType, "%s, %s" % (name, res["Message"])
                    )

        res = self.componentControl()
        if not res["OK"]:
            if (
                "Stopped does not exist" not in res["Message"]
                and "Running does not exist" not in res["Message"]
            ):
    ...
    def checkForCheckingJobs(self, executorName):
        ...
        if not resJobs["OK"]:
            self.logError(
                "Could not get jobs for this executor", "%s: %s" % (executorName, resJobs["Message"])
            )
            return resJobs
        if resJobs["Value"]:
    ...
    def _getDefaultComponentStatus(self):
        ...
        if defaultStatus["Run"].intersection(defaultStatus["Down"]):
            self.logError(
                "Overlap in configuration", str(defaultStatus["Run"].intersection(defaultStatus["Down"]))
            )
