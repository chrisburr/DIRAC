...
class TestComponentSupervisionAgent(unittest.TestCase):
    ...
    def test_execute(self):
        ...
        self.restartAgent.agents = {
            agentOne: {
                "PollingTime": agentOnePollingTime,
                "LogFileLocation": agentOneLogLoc,
                "PID": agentOnePID,
            },
            agentTwo: {
                "PollingTime": agentTwoPollingTime,
        ...
        calls = [
            call(agentOne, self.restartAgent.agents[agentOne]),
    ...
    def test_checkURLs_1(self):
        ...
        for i in [1, 2]:
            urls.append(
                "%(prot)s://%(host)s:%(port)s/Sys/Serv%(i)s"
                % dict(i=i, host=host, port=port[i], prot=prot[i])
            )
        for i in [1]:
            tempurls.append(
                "%(prot)s://%(host)s:%(port)s/Sys/Serv%(i)s"
