...
class ServiceInterface(object):
    ...
    def registerActivities(self, sourceDict, activitiesDict, componentExtraInfo):
        ...
        for name in activitiesDict:
            ...
            retVal = rrdManager.create(
                activitiesDict[name]["type"], rrdFile, activitiesDict[name]["bucketLength"]
            )
            if not retVal["OK"]:
                return retVal
    ...
    def commitMarks(self, sourceId, activitiesDict, componentExtraInfo):
        ...
        for acName in activitiesDict:
            ...
            if len(entries) > 0:
                ...
                if not retDict["OK"]:
                    gLogger.error(
                        "There was an error updating", "%s:%s activity [%s]" % (sourceId, acName, rrdFile)
                    )
                else:
    ...
    def plotView(self, viewRequest):
        ...
        return self.generatePlots(
            viewRequest["fromSecs"], viewRequest["toSecs"], viewDefinition, viewRequest["size"]
        )
