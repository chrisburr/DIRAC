...
class MonitoringCatalog(object):
    ...
    def __dbExecute(self, query, values=False):
        ...
        if not executed:
            self.log.error(
                "Could not execute query, big mess ahead", "query: %s, values: %s" % (query, values)
            )
        return cursor
    ...
    def queryField(self, field, definedFields):
        ...
        """
        retList = self.__select(
            field, "sources, activities", definedFields, "sources.id = activities.sourceId"
        )
        return retList
    ...
    def registerView(self, viewName, viewData, varFields):
        ...
        self.__insert(
            "views",
            {"id": "NULL"},
            {"name": viewName, "definition": viewData, "variableFields": ", ".join(varFields)},
        )
        return S_OK()
    ...
    def getActivities(self, dbCond):
        ...
        return self.__select(
            "id, name, category, unit, type, description, bucketLength", "activities", dbCond
        )
