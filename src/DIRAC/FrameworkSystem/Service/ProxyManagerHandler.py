...
class ProxyManagerHandler(RequestHandler):
    ...
    def export_completeDelegationUpload(self, requestId, pemChain):
        ...
        if not retVal["OK"]:
            gLogger.error(
                "Upload proxy failed", "id: %s user: %s message: %s" % (requestId, userId, retVal["Message"])
            )
    ...
    def __checkProperties(self, requestedUserDN, requestedUserGroup):
        ...
        if Properties.PRIVATE_LIMITED_DELEGATION in credDict["properties"]:
            ...
            if Properties.PRIVATE_LIMITED_DELEGATION not in Registry.getPropertiesForGroup(
                requestedUserGroup
            ):
    ...
    def export_generateToken(self, requesterDN, requesterGroup, tokenUses):
        ...
        credDict = self.getRemoteCredentials()
        self.__proxyDB.logAction(
            "generate tokens", credDict["DN"], credDict["group"], requesterDN, requesterGroup
        )
    ...
    def export_getProxyWithToken(self, userDN, userGroup, requestPem, requiredLifetime, token):
        ...
        if not result["OK"]:
            return result
        self.__proxyDB.logAction(
            "download proxy with token", credDict["DN"], credDict["group"], userDN, userGroup
        )
    ...
    def export_getVOMSProxyWithToken(
        ...
        if not result["OK"]:
            return result
        self.__proxyDB.logAction(
            "download voms proxy with token", credDict["DN"], credDict["group"], userDN, userGroup
        )
