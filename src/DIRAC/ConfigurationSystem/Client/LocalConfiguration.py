...
class LocalConfiguration(object):
    ...
    def __registerBasicOptions(self):
        ...
        self.registerCmdOpt(
            "s:", "section=", "Set base section for relative parsed options", self.__setSectionByCmd
        )
        self.registerCmdOpt(
            "c:", "cert=", "Use server certificate to connect to Core Services", self.__setUseCertByCmd
        )
    ...
    def __addUserDataToConfiguration(self):
        ...
        for optionName, optionValue in self.parsedOptionList:
            ...
            for definedOptionTuple in self.commandOptionList:
                if optionName == definedOptionTuple[0].replace(":", "") or optionName == definedOptionTuple[
                    1
                ].replace("=", ""):
                    if definedOptionTuple[3]:
                        ...
                        if not isinstance(retVal, dict):
                            errorsList.append(
                                "Callback for switch '%s' does not return S_OK or S_ERROR" % optionName
                            )
                        elif not retVal["OK"]:
                            errorsList.append(retVal["Message"])
                    else:
        ...
        if iLastOpt + 1 < len(self.commandOptionList):
            ...
            for iPos in range(iLastOpt + 1, len(self.commandOptionList)):
                ...
                if optionTuple[0].endswith(":"):
                    ...
                    gLogger.notice(
                        "  -%s --%s : %s"
                        % (optionTuple[0].ljust(2), optionTuple[1].ljust(22), optionTuple[2])
                    )
