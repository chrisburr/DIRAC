        ...
        for siteName in siteNames:
            ...
            for ce in ces.get("Value", []):
                ...
                if submissionMode == "Unknown" and newSubmissionMode:
                    addToChangeSet(
                        (ceSection, "SubmissionMode", submissionMode, newSubmissionMode), changeSet
                    )
                ...
                for queue, queueInfo in ceInfo["Queues"].items():
                    ...
                    if newNOP != numberOfProcessors:
                        addToChangeSet(
                            (queueSection, "NumberOfProcessors", numberOfProcessors, newNOP), changeSet
                        )
                        if newNOP > 1:
                            ...
                            if localCEType_inner:
                                ...
                                newLocalCEType = "Pool"
                            addToChangeSet(
                                (queueSection, "LocalCEType", localCEType, newLocalCEType), changeSet
                            )
                        else:
                            ...
                            newTag = ",".join(
                                sorted(set(tag.split(",")).difference({"MultiProcessor"}))
                            ).strip(",")
