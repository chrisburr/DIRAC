    ...
    def syncCSWithVOMS(self):
        ...
        for dn in self.vomsUserDict:
                ...
                if not diracName:
                    ...
                    if self.syncPlugin:
                        try:
                            ...
                            self.log.error(
                                "Error validating new user", "nickname %s\n error %s" % (newDiracName, e)
                            )
            ...
            if newDNForExistingUser:
                userDict["DN"] = ",".join(
                    [dn, diracUserDict.get(diracName, newAddedUserDict.get(diracName))["DN"]]
                )
                modified = True
            ...
            if diracName in diracUserDict:
                ...
                for key in userDict:
                    if key == "Groups":
                        ...
                        removedGroups = set(diracUserDict.get(diracName, {}).get(key, [])) - set(
                            userDict[key]
                        )
                        if addedGroups:
