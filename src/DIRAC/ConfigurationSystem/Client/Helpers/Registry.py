...
def getVOMSAttributeForGroup(group):
    ...
    return gConfig.getValue(
        "%s/Groups/%s/VOMSRole" % (gBaseRegistrySection, group), getDefaultVOMSAttribute()
    )
        ...
        for vo in voNames:
            ...
            if result["OK"]:
                ...
                for server in serverList:
                    ...
                    DN = gConfig.getValue(
                        "%s/VO/%s/VOMSServers/%s/DN" % (gBaseRegistrySection, vo, server), ""
                    )
                    CA = gConfig.getValue(
                        "%s/VO/%s/VOMSServers/%s/CA" % (gBaseRegistrySection, vo, server), ""
                    )
                    port = gConfig.getValue(
                        "%s/VO/%s/VOMSServers/%s/Port" % (gBaseRegistrySection, vo, server), 0
                    )
...
def getDNFromProxyProviderForUserID(proxyProvider, userID):
    ...
    for DN in result["Value"]:
        ...
        if proxyProvider in result["Value"]:
            return S_OK(DN)
    return S_ERROR(
        errno.ENODATA, "No DN found for %s proxy provider for user ID %s" % (proxyProvider, userID)
    )
