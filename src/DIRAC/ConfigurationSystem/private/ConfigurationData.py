...
class ConfigurationData(object):
    ...
    def getPropagationTime(self):
        try:
            return int(
                self.extractOptionFromCFG("%s/PropagationTime" % self.configurationPath, self.mergedCFG)
            )
        except BaseException:
            return 300
    ...
    def getSlavesGraceTime(self):
        try:
            return int(
                self.extractOptionFromCFG("%s/SlavesGraceTime" % self.configurationPath, self.mergedCFG)
            )
        except BaseException:
            return 600
        ...
        if os.path.isfile(configurationFile):
            ...
            try:
                with zipfile.ZipFile(backupFile, "w", zipfile.ZIP_DEFLATED) as zf:
                    zf.write(
                        configurationFile, "%s.backup.%s" % (os.path.split(configurationFile)[1], backupName)
                    )
            except Exception:
                gLogger.exception()
