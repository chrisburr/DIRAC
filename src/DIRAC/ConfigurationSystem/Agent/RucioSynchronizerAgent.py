        ...
        try:
                ...
                for rse in rses:
                    if rse in allSEs:
                        ...
                        try:
                            ...
                            self.log.error(
                                "Cannot create RSE attribute OccupancyLFN for %s : %s", rse, str(err)
                            )
                    if rse in primarySEs:
                        try:
                            ...
                            self.log.error(
                                "Cannot create RSE attribute PrimaryDataSE for %s : %s", rse, str(err)
                            )
                    else:
                        try:
                            ...
                            self.log.error(
                                "Cannot remove RSE attribute PrimaryDataSE for %s : %s", rse, str(err)
                            )
            ...
            for account in getAllUsers():
                ...
                if account not in listAccounts:
                    ...
                    try:
                        client.add_identity(
                            account=account, identity=dn, authtype="X509", email=email, default=True
                        )
                    except Exception as err:
                        self.log.error("Cannot create account %s : %s", account, str(err))
                    for rse in rses:
            ...
            for group in groups:
                ...
                for dn in getDNsInGroup(group):
                    try:
                        ...
                        self.log.error(
                            "Cannot create identity %s for account %s : %s" % (dn, group, str(err))
                        )
            ...
            if not result["OK"]:
                ...
                for host in hosts:
                    ...
                    try:
                        ...
                        self.log.error(
                            "Cannot create identity %s for account dirac_srv : %s" % (dn, str(err))
                        )
                        self.log.error(format_exc())
