...
class VOMS2CSAgent(AgentModule):
    ...
    def execute(self):
        ...
        for vo in self.voList:
            ...
            self.log.info(
                "Performing VOMS sync", "for VO %s with credentials %s@%s" % (vo, voAdminUser, voAdminGroup)
            )
            ...
            if not result["OK"]:
                self.log.error(
                    "Failed to perform VOMS to CS synchronization:", "VO %s: %s" % (vo, result["Message"])
                )
                continue
            ...
            if self.makeFCEntry and newUsers:
                ...
                if not result["OK"]:
                    self.log.error(
                        "Failed to create user home directories:", "VO %s: %s" % (vo, result["Message"])
                    )
                else:
