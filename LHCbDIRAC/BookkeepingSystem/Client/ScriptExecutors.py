...
def executeFileSisters(dmScript, level=1):
  ...
  for prod, lfnList in prodLfns.items():
    ...
    for lfn, ancList in result['Value']['Successful'].items():
...
def _getJobsEISFromAncestors(lfnList):
  ...
  for lfnChunk in breakListIntoChunks(lfnList, 100):
    ...
    for lfn, ancDict in ancWithMetadata.items():
      job = _jobFromLfn(lfn)
      for anc, meta in ancDict.items():
  ...
  if ancToCheck:
    ...
    for job, ancJobs in ancestors.items():
      if job not in jobEventInputStat:
        try:
          # This ancestor job is  already known
