...
class TransformationPlugin(DIRACTransformationPlugin):
  ...
  def __byRun(self, param='', plugin='LHCbStandard', requireFlush=False, forceFlush=False):
    ...
    for runID in list(runNumbers):
      runFiles = sum(len(lfns) for lfns in runFileDict[runID].values())
    ...
    for run in runList:
      ...
      for paramValue in sorted(paramDict):
        ...
        if fromSEs:
          ...
          tasks = []
          for okSEs, taskLfns in okDict.items():
            if addAncestors:
  ...
  def _LHCbDSTBroadcast(self):
    ...
    for runID, runLfns in runFileDict.items():
  ...
  def _LHCbMCDSTBroadcastRandom(self):
    ...
    if excludedFileTypes:
      excludedLfns = []
      for lfn, fileType in self.util.getMetadataFromTSorBK(self.transReplicas, 'FileType').items():
  ...
  def _RemoveReplicasKeepDestination(self, minKeep=None):
    ...
    tasks = []
    for runID, runLfns in runFileDict.items():
      replicas = dict((lfn, ses) for lfn, ses in self.transReplicas.items() if lfn in runLfns)
      existingSEs = set(se for ses in replicas.values() for se in ses if se not in keepSEs)
      ...
      if destinationSE is None:
        ...
        replicasWithKeep = dict((lfn, ses) for lfn, ses in replicas.items() if destinationSE in ses)
        replicasNoKeep = dict((lfn, ses) for lfn, ses in replicas.items() if destinationSE not in ses)
  ...
  def _removeReplicas(self, replicas=None, fromSEs=None, keepSEs=None, mandatorySEs=None, minKeep=999):
    ...
    storageElementGroups = {}
    notInKeepSEs = []
    for replicaSE, lfns in getFileGroups(replicas).items():
  ...
  def __getAncestorLFNs(self, lfns):
    ...
    if not ancestors['OK']:
      ...
      return ancestors
    ancestors = [anc['FileName'] for ancList in ancestors['Value']['Successful'].values() for anc in ancList]
  ...
  def __addAncestors(self, pluginMethod=None):
    ...
    for targetSE, lfns in self._alreadyProcessedLFNs.items():
      ancestors = self.__getAncestorLFNs(lfns)
      if not ancestors['OK']:
        return ancestors
      ancestors = ancestors['Value']
