...
from DIRAC.ResourceStatusSystem.Utilities.InfoGetter import (
    getPolicyActionsThatApply,
...
class PDP(object):
        ...
        if decisionParams is not None:
            ...
            if standardParamsDict["element"] is not None:
                self.log = gLogger.getSubLogger(
                    "PDP/%s" % standardParamsDict["element"]
                )
                if standardParamsDict["name"] is not None:
                    self.log = gLogger.getSubLogger(
                        "PDP/%s/%s"
                        % (standardParamsDict["element"], standardParamsDict["name"])
                    )
                    self.log.verbose(
    ...
    def _runPolicies(self, policies):
        ...
        for policyDict in policies:
            ...
            if not policyInvocationResult["Status"] in validStatus:
                _msg = (
                    "runPolicies ( not valid status ) %s"
                    % policyInvocationResult["Status"]
                )
                self.log.error(_msg)
                return S_ERROR(_msg)
    ...
    def _combineSinglePolicyResults(self, singlePolicyRes):
        ...
        if not singlePolicyRes:
            policyCombined["Reason"] = (
                "No policy applies to %(element)s, %(name)s, %(elementType)s"
                % self.decisionParams
            )
        ...
        machineStatus = self.rssMachine.setState(
            self.decisionParams["status"], noWarn=True
        )
    ...
    def _findPoliciesToCombine(self, singlePolicyRes):
        ...
        return [
            policyResult
            for policyResult in singlePolicyRes
