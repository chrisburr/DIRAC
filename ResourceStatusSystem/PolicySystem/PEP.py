...
class PEP(object):
    ...
    def enforce(self, decisionParams):
        ...
        if not resDecisions["OK"]:
            self.log.error(
                "Something went wrong, not enforcing policies", "%s" % decisionParams
            )
            return resDecisions
    ...
    def __isNotUpdated(self, decisionParams):
        ...
        if decisionParams["element"] == "Site":
            unchangedRow = self.clients["SiteStatus"].getSiteStatuses(
                [decisionParams["name"]]
            )
        else:
        ...
        if not unchangedRow["Value"]:
            msg = (
                "%(name)s  ( %(status)s / %(statusType)s ) has been updated after PEP started running"
                % selectParams
            )
            self.log.error(msg)
            return S_ERROR(msg)
