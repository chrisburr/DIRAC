...
class EmailAction(BaseAction):
    def __init__(
        self, name, decisionParams, enforcementResult, singlePolicyResults, clients=None
    ):
    ...
    def run(self):
        ...
        if self.decisionParams["element"] == "Site":
            ...
            if not siteName["OK"]:
                self.log.error(
                    "Resource %s does not exist at any site: %s"
                ...
                siteName = (
                    siteName["Value"]
                    if isinstance(siteName["Value"], six.string_types)
                    else siteName["Value"][0]
                )
        ...
        with sqlite3.connect(self.cacheFile) as conn:
            ...
            try:
                conn.execute(
                    """CREATE TABLE IF NOT EXISTS ResourceStatusCache(
