...
class ElementStatusBase(object):
    ...
    lastchecktime = Column(
        "LastCheckTime", DateTime, nullable=False, server_default="1000-01-01 00:00:00"
    )
    tokenowner = Column(
        "TokenOwner", String(16), nullable=False, server_default="rs_svc"
    )
    ...
    def addOrModify(self, table, params):
        ...
        try:
            ...
            if params.get("Status"):
                if (
                    params.get("Status") != res.status
                ):  # we update dateEffective iff we change the status
                    changeDE = True
