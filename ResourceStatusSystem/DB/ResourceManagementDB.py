...
class DowntimeCache(rmsBase):
    ...
    def fromDict(self, dictionary):
        ...
        self.gocdbservicetype = dictionary.get(
            "GOCDBServiceType", self.gocdbservicetype
        )
...
class JobCache(rmsBase):
    ...
    efficiency = Column(
        "Efficiency", Float(asdecimal=False), nullable=False, server_default="0"
    )
...
class PilotCache(rmsBase):
    ...
    vo = Column(
        "VO", String(64), nullable=False, primary_key=True, server_default="all"
    )
    status = Column("Status", String(16), nullable=False)
    pilotjobeff = Column(
        "PilotJobEff", Float(asdecimal=False), nullable=False, server_default="0"
    )
    pilotsperjob = Column(
        "PilotsPerJob", Float(asdecimal=False), nullable=False, server_default="0"
    )
    lastchecktime = Column("LastCheckTime", DateTime, nullable=False)
...
class PolicyResult(rmsBase):
    ...
    statustype = Column(
        "StatusType", String(16), nullable=False, server_default="", primary_key=True
    )
...
class SpaceTokenOccupancyCache(rmsBase):
    ...
    guaranteed = Column(
        "Guaranteed", Float(asdecimal=False), nullable=False, server_default="0"
    )
...
class TransferCache(rmsBase):
    ...
    destinationname = Column(
        "DestinationName", String(64), nullable=False, primary_key=True
    )
...
class ResourceManagementDB(object):
    ...
    def insert(self, table, params):
        ...
        )  # FIXME: should we use this flag elsewhere?

        found = False
        for ext in self.extensions:
            try:
                tableRow_o = getattr(
                    __import__(ext + __name__, globals(), locals(), [table]), table
                )()
                found = True
                break
            except (ImportError, AttributeError):
    ...
    def select(self, table, params):
        ...
        columnNames = [
            column.lower() for column in params.get("Meta", {}).get("columns", [])
        ]
        ...
        try:
            ...
            if wholeTable:
                ...
                selectionResToList = [
                    [getattr(res, col) for col in columnNames] for res in selectionRes
                ]
    ...
    def addOrModify(self, table, params):
        ...
        try:
            ...
            for columnName, columnValue in params.iteritems():
                if (
                    columnName == "LastCheckTime" and not columnValue
                ):  # we always update lastCheckTime
                    columnValue = datetime.datetime.utcnow().replace(microsecond=0)
                if (
                    columnName == "DateEffective" and not columnValue
                ):  # we always update DateEffective, if there
                    columnValue = datetime.datetime.utcnow().replace(microsecond=0)
                if columnValue:
