...
def getPoliciesThatApply(decisionParams):
    ...
    for policyName, policySetup in policiesConfig.iteritems():
        ...
        gLogger.debug(
            "PolicyMatch for decisionParams %s: %s" % (decisionParams, str(policyMatch))
        )
    ...
    gLogger.debug(
        "policies that apply (before post-processing): %s" % str(policiesThatApply)
    )
    policiesThatApply = postProcessingPolicyList(policiesThatApply)
    gLogger.debug(
        "policies that apply (after post-processing): %s" % str(policiesThatApply)
    )
...
def getPolicyActionsThatApply(
    decisionParams, singlePolicyResults, policyCombinedResults
):
    """
...
def _getComputingElementsByDomainName(targetDomain=None):
    ...
    if unknownDomains:
        gLogger.warn(
            "Domains %s belong to the policy parameters but not to the CS domains"
            % unknownDomains
        )
    ...
    if not knownDomains:
        gLogger.warn(
            "Policy parameters domain names do not match with any CS domain names"
        )
        return S_OK([])

    for domainName in knownDomains:
        gLogger.info(
            "Fetching the list of Computing Elements belonging to domain %s"
            % domainName
        )
        ...
        for site in domainSites:
            siteCEs = gConfig.getSections(
                "%s/%s/%s/CEs" % (_basePath, domainName, site)
            )
            if not siteCEs["OK"]:
                # return siteCEs
...
def _filterPolicies(decisionParams, policyMatchParams):
    ...
    if elementType and elementType.upper() == "CE" and "domain" in policyMatchParams:
        ...
        if result["OK"]:
            ...
            if name not in ces:
                gLogger.info(
                    "ComputingElement %s NOT found in domains %s" % (name, domains)
                )
                return False
            else:
                gLogger.info(
                    "ComputingElement %s found in domains %s" % (name, domains)
                )
        else:
            gLogger.warn(
                "unable to verify if ComputingElement %s is in domains %s"
                % (name, domains)
            )
            return False
