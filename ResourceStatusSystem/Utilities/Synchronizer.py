...
class Synchronizer(object):
    ...
    def __syncComputingElements(self):
        ...
        cesStatusTuples = [
            (se, statusType) for se in cesCS for statusType in statusTypes
        ]
    ...
    def __syncFileCatalogs(self):
        ...
        # For each ( se, statusType ) tuple not present in the DB, add it.
        catalogsStatusTuples = [
            (se, statusType) for se in catalogsCS for statusType in statusTypes
        ]
    ...
    def __syncFTS(self):
        ...
        ftsDB = self.rStatus.selectStatusElement(
            "Resource", "Status", elementType="FTS", meta={"columns": ["Name"]}
        )
        if not ftsDB["OK"]:
            return ftsDB
        ...
        # For each ( se, statusType ) tuple not present in the DB, add it.
        ftsStatusTuples = [
            (se, statusType) for se in ftsCS for statusType in statusTypes
        ]
    ...
    def __syncStorageElements(self):
        ...
        # For each ( se, statusType ) tuple not present in the DB, add it.
        sesStatusTuples = [
            (se, statusType) for se in sesCS for statusType in statusTypes
        ]
    ...
    def __syncQueues(self):
        ...
        queuesDB = self.rStatus.selectStatusElement(
            "Node", "Status", elementType="Queue", meta={"columns": ["Name"]}
        )
        if not queuesDB["OK"]:
            return queuesDB
        ...
        # For each ( se, statusType ) tuple not present in the DB, add it.
        queueStatusTuples = [
            (se, statusType) for se in queuesCS for statusType in statusTypes
        ]
