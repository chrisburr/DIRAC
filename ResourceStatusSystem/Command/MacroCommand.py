...
class MacroCommand(Command):
    ...
    def setArgs(self, argsListIn=None):
        ...
        if len(self.args) == len(self.commands):
            commArgs = [
                (self.commands[x], self.args[x]) for x in range(len(self.commands))
            ]
        elif len(self.args) == 1:
            commArgs = [
                (self.commands[x], self.args[0]) for x in range(len(self.commands))
            ]
        else:
            gLogger.error(
                "Tuples or `args` provided are nor 1 nor the same number of the commands"
            )
    ...
    def setClient(self, clientListIn=None):
        ...
        if len(self.clients) == len(self.commands):
            commArgs = [
                (self.commands[x], self.clients[x]) for x in range(len(self.commands))
            ]
        elif len(self.args) == 1:
            commArgs = [
                (self.commands[x], self.clients[0]) for x in range(len(self.commands))
            ]
        else:
            gLogger.error(
                "`clients` provided are nor 1 nor the same number of the commands"
            )
