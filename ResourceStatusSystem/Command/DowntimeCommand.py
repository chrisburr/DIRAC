...
class DowntimeCommand(Command):
    ...
    def _cleanCommand(self, element, elementNames):
        ...
        for elementName in elementNames:
            ...
            for dt in uniformResult:
                ...
                if (
                    dt["EndDate"] < currentDate
                    or dt["Link"] not in gDTLinkList["Value"]
                ):
    ...
    def _prepareCommand(self):
        ...
        if element == "Site":
            ...
            if not gocSite[
                "OK"
            ]:  # The site is most probably is not a grid site - not an issue, of course
        ...
        # The DIRAC se names mean nothing on the grid, but their hosts do mean.
            ...
            try:
                ...
                self.log.error(
                    "Failure instantiating StorageElement object", elementName
                )
    ...
    def doNew(self, masterParams=None):
        ...
        try:
            results = self.gClient.getStatus(
                element, name=elementNames, startingInHours=hours
            )
            ...
            if dt["gOCDBServiceType"] and gOCDBServiceType:
                if gOCDBServiceType.lower() != downDic["SERVICE_TYPE"].lower():
                    self.log.warn(
                        ...
                        % (gOCDBServiceType, downDic["SERVICE_TYPE"], dt["Name"]),
                    )
    ...
    def doMaster(self):
        ...
        self.log.verbose(
            "Processing Sites", ", ".join(gocSites if gocSites else ["NONE"])
        )
        ...
        self.log.verbose(
            "Processing Resources", ", ".join(resources if resources else ["NONE"])
        )
