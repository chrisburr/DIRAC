...
class FreeDiskSpaceCommand(Command):
    ...
    def _storeCommand(self, results):
        ...
        if not res["OK"]:
            self.log.error(
                "Error calling addOrModifySpaceTokenOccupancyCache", res["Message"]
            )
            return res
    ...
    def doMaster(self):
        ...
        for name in DMSHelpers().getStorageElements():
            try:
                ...
                if not diskSpace["OK"]:
                    self.log.warn(
                        "Unable to calculate free/total disk space", "name: %s" % name
                    )
                    self.log.warn(diskSpace["Message"])
                    continue
            except Exception as excp:  # pylint: disable=broad-except
                self.log.error(
                    "Failed to get SE FreeDiskSpace information ==> SE skipped", name
                )
                self.log.exception(
                    "Operation finished with exception: ", lException=excp
                )
