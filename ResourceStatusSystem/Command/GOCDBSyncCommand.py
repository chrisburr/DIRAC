        ...
        for downtimes in result["Value"]:
            ...
            for dtElement in downtimeElements:
                ...
                if localDowntimeID == GOCDBDowntimeID:
                    ...
                    if (
                        localDBdict["FORMATED_START_DATE"]
                        != GOCDBdict["FORMATED_START_DATE"]
                    ):
                        result = self.rmClient.addOrModifyDowntimeCache(
                            downtimeID=localDBdict["DowntimeID"],
                            startDate=GOCDBdict["FORMATED_START_DATE"],
                        )
                        gLogger.verbose(
                            "The start date of %s has been changed!" % downtimes[3]
                        )
                    ...
                    if (
                        localDBdict["FORMATED_END_DATE"]
                        != GOCDBdict["FORMATED_END_DATE"]
                    ):
                        result = self.rmClient.addOrModifyDowntimeCache(
                            downtimeID=localDBdict["DowntimeID"],
                        ...
                        gLogger.verbose(
                            "The end date of %s has been changed!" % downtimes[3]
                        )
