...
def registerSwitches():
    ...
    Script.registerSwitch(
        "",
        "init",
        "Initialize the element to the status in the CS ( applicable for StorageElements )",
    )
    Script.registerSwitch(
        "",
        "element=",
        "Element family to be Synchronized ( Site, Resource or Node ) or `all`",
    )
    Script.registerSwitch(
...
from DIRAC.ResourceStatusSystem.Utilities import (
    Synchronizer,
    CSHelpers,
    RssConfiguration,
)
    ...
    for se in DMSHelpers().getStorageElements():
        ...
        for statusType, status in opts.iteritems():
            ...
            if status not in statuses:
                subLogger.error(
                    "%s not a valid status for %s - %s" % (status, se, statusType)
                )
                continue
        ...
        for statusType in statusTypesList:
            ...
            if not result["OK"]:
                subLogger.error(
                    "Error in backtracking for %s,%s,%s" % (se, statusType, status)
                )
                subLogger.error(result["Message"])
