...
def registerSwitches():
    ...
    switches = (
        ...
        (
            "name=",
            "Name (or comma-separeted list of names) of the element where the change applies",
        ),
        (
            "statusType=",
            "StatusType (or comma-separeted list of names), if none applies to all possible statusTypes",
        ),
...
def setStatus(switchDict, tokenOwner):
    ...
    if not elements:
        subLogger.warn(
            "Nothing found for %s, %s, %s"
            % (switchDict["element"], switchDict["name"], switchDict["statusType"])
        )
        return S_OK()
    ...
    for status, statusType in elements:
        ...
        if switchDict["status"] == status:
            subLogger.notice(
                "Status for %s (%s) is already %s. Ignoring.."
