...
def parseSwitches():
    ...
    if "status" in switches and switches["status"] is not None:
        ...
        if switches["status"] not in (
            ...
            error(
                "'%s' is an invalid argument for switch 'status'" % switches["status"]
            )
...
def tabularPrint(table):
    ...
    output = printTable(
        columns_names, records, numbering=False, columnSeparator=" | ", printOut=False
    )
