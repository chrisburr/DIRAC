        ...
        try:
            for rse in client.list_rses():
                ...
                if resStatus["OK"]:
                    ...
                    availabilityRead = (
                        True
                        if seAccessValue["ReadAccess"] in ["Active", "Degraded"]
                        else False
                    )
                    availabilityWrite = (
                        True
                        if seAccessValue["WriteAccess"] in ["Active", "Degraded"]
                        else False
                    )
                    availabilityDelete = (
                        True
                        if seAccessValue["RemoveAccess"] in ["Active", "Degraded"]
                        else False
                    )
                    isUpdated = False
                    if dictSe["availability_read"] != availabilityRead:
                        self.log.info(
                            "Set availability_read for %s to %s",
                            thisSe,
                            availabilityRead,
                        )
                        client.update_rse(
                    ...
                    if dictSe["availability_write"] != availabilityWrite:
                        self.log.info(
                            "Set availability_write for %s to %s",
                            thisSe,
                            availabilityWrite,
                        )
                        client.update_rse(
                    ...
                    if dictSe["availability_delete"] != availabilityDelete:
                        self.log.info(
                            "Set availability_delete for %s to %s",
                            thisSe,
                            availabilityDelete,
                        )
                        client.update_rse(
                            thisSe, {"availability_delete": availabilityDelete}
                        )
                        isUpdated = True
