...
class ElementInspectorAgent(AgentModule):
    ...
    def initialize(self):
        ...
        maxNumberOfThreads = self.am_getOption(
            "maxNumberOfThreads", self.__maxNumberOfThreads
        )
        ...
        if not res["OK"]:
            self.log.error(
                "Failed to load ResourceStatusClient class: %s" % res["Message"]
            )
            return res
        rsClass = res["Value"]
        ...
        if not res["OK"]:
            self.log.error(
                "Failed to load ResourceManagementClient class: %s" % res["Message"]
            )
            return res
        rmClass = res["Value"]
    ...
    def execute(self):
        ...
        self.log.info(
            "Needed %d threads to process %d elements" % (numberOfThreads, queueSize)
        )
    ...
    def getElementsToBeChecked(self):
        ...
        for element in elements["Value"]:
            ...
            if utcnow <= elemDict["LastCheckTime"] + datetime.timedelta(
                minutes=timeToNextCheck
            ):
                continue
        ...
        while True:
            ...
            if oldStatus != newStatus:
                self.log.info(
                    ...
                    % (element["name"], statusType, newStatus, reason, oldStatus)
                )
