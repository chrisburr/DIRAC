...
def sla(mocker):
    mocker.patch(
        "DIRAC.ResourceStatusSystem.Agent.SummarizeLogsAgent.AgentModule", new=mockAM
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            response1,
            True,
            (101, {("RRCKI-BUFFER", "WriteAccess"): [dict(zip(columns, el101))]}),
        ),
        (
            response2,
            True,
            (102, {("RRCKI-BUFFER", "WriteAccess"): [dict(zip(columns, el101))]}),
        ),
        (
            response3,
            True,
...
def test__summarizeLogs(
    rsClientMockSelectStatusElementReturnValue, expected, expectedValue, sla
):
    rsClientMock.selectStatusElement.return_value = (
        rsClientMockSelectStatusElementReturnValue
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            (None, None),
            [],
            {"OK": True, "Value": [], "Columns": ["Status", "TokenOwner"]},
            True,
        ),
        (
            (None, None),
...
def test__registerLogs(
    key, logs, rsClientMockSelectStatusElementReturnValue, expected, sla
):
    rsClientMock.selectStatusElement.return_value = (
        rsClientMockSelectStatusElementReturnValue
    )
