...
class CacheFeederAgent(AgentModule):
    ...
    def loadCommand(self, commandModule, commandDict):
        ...
        commandObject = self.cCaller.commandInvocation(
            commandTuple, pArgs=commandArgs, clients=self.clients
        )
                ...
                try:
                    ...
                    if not results["OK"]:
                        self.log.error(
                            ...
                            "%s: %s" % (commandModule, results["Message"]),
                        )
                        continue
                    ...
                    self.log.exception(
                        "Failed to execute command, with exception: %s" % commandModule,
                        lException=excp,
                    )
