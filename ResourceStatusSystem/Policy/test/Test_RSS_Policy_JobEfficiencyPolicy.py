...
class JobEfficiencyPolicy_Success(JobEfficiencyPolicy_TestCase):
    ...
    def test_evaluate(self):
        ...
        res = module._evaluate(
            {"OK": True, "Value": [{"Completed": 0, "Done": 0, "Failed": 0}]}
        )
        self.assertTrue(res["OK"])
        self.assertEqual("Unknown", res["Value"]["Status"])
        ...
        res = module._evaluate(
            {"OK": True, "Value": [{"Completed": 0, "Done": 0, "Failed": 1}]}
        )
        self.assertTrue(res["OK"])
        self.assertEqual("Unknown", res["Value"]["Status"])
        self.assertEqual("Not enough jobs to take a decision", res["Value"]["Reason"])
        ...
        res = module._evaluate(
            {"OK": True, "Value": [{"Completed": 0, "Done": 0, "Failed": 10}]}
        )
        self.assertTrue(res["OK"])
        ...
        res = module._evaluate(
            {"OK": True, "Value": [{"Completed": 0, "Done": 8, "Failed": 2}]}
        )
        self.assertTrue(res["OK"])
        ...
        res = module._evaluate(
            {"OK": True, "Value": [{"Completed": 10, "Done": 9, "Failed": 1}]}
        )
        self.assertTrue(res["OK"])
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(
        JobEfficiencyPolicy_TestCase
    )
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(JobEfficiencyPolicy_Success)
    )
