...
class PilotEfficiencyPolicy_Success(PilotEfficiencyPolicy_TestCase):
    ...
    def test_evaluate(self):
        ...
        res = module._evaluate(
            {
                "OK": True,
                "Value": [{"Aborted": 0, "Deleted": 0, "Done": 0, "Failed": 0}],
            }
        )
        self.assertTrue(res["OK"])
        self.assertEqual("Unknown", res["Value"]["Status"])
        ...
        res = module._evaluate(
            {
                "OK": True,
                "Value": [{"Aborted": 10, "Deleted": 0, "Done": 10, "Failed": 0}],
            }
        )
        self.assertTrue(res["OK"])
        ...
        res = module._evaluate(
            {
                "OK": True,
                "Value": [{"Aborted": 0, "Deleted": 0, "Done": 30, "Failed": 10}],
            }
        )
        self.assertTrue(res["OK"])
        ...
        res = module._evaluate(
            {
                "OK": True,
                "Value": [{"Aborted": 0, "Deleted": 0, "Done": 19, "Failed": 1}],
            }
        )
        self.assertTrue(res["OK"])
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(
        PilotEfficiencyPolicy_TestCase
    )
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(PilotEfficiencyPolicy_Success)
    )
