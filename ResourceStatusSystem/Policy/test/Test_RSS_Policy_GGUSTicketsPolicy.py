...
class GGUSTicketsPolicy_Success(GGUSTicketsPolicy_TestCase):
    ...
    def test_evaluate(self):
        ...
        self.assertEqual("Error", res["Value"]["Status"])
        self.assertEqual(
            "Expected OpenTickets key for GGUSTickets", res["Value"]["Reason"]
        )
        ...
        self.assertRaises(
            KeyError, module._evaluate, {"OK": True, "Value": [{"OpenTickets": 1}]}
        )

        res = module._evaluate(
            {"OK": True, "Value": [{"OpenTickets": 1, "Tickets": "1a"}]}
        )
        self.assertTrue(res["OK"])
...
if __name__ == "__main__":
    ...
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(GGUSTicketsPolicy_Success)
    )
