...
class JobRunningWaitingRatioPolicy_Success(JobRunningWaitingRatioPolicy_TestCase):
    ...
    def test_evaluate(self):
        ...
        res = module._evaluate(
            {"OK": True, "Value": [{"Running": 0, "Waiting": 0, "Staging": 0}]}
        )
        self.assertTrue(res["OK"])
        self.assertEqual("Unknown", res["Value"]["Status"])
        ...
        res = module._evaluate(
            {"OK": True, "Value": [{"Running": 10, "Waiting": 10, "Staging": 10}]}
        )
        self.assertTrue(res["OK"])
        ...
        res = module._evaluate(
            {"OK": True, "Value": [{"Running": 10, "Waiting": 10, "Staging": 0}]}
        )
        self.assertTrue(res["OK"])
        ...
        res = module._evaluate(
            {"OK": True, "Value": [{"Running": 20, "Waiting": 10, "Staging": 0}]}
        )
        self.assertTrue(res["OK"])
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(
        JobRunningWaitingRatioPolicy_TestCase
    )
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(
            JobRunningWaitingRatioPolicy_Success
        )
    )
