...
class ResourceStatus(object):
    ...
    def getElementStatus(self, elementName, elementType, statusType=None, default=None):
        ...
        if elementType not in allowedParameters:
            return S_ERROR(
                "%s in not in the list of the allowed parameters: %s"
                % (elementType, allowedParameters)
            )
        ...
        if self.rssFlag:
            ...
            return self.__getCSElementStatus(
                elementName, elementType, statusType, default
            )

    def setElementStatus(
        self, elementName, elementType, statusType, status, reason=None, tokenOwner=None
    ):
        ...
        if self.rssFlag:
            return self.__setRSSElementStatus(
                elementName, elementType, statusType, status, reason, tokenOwner
            )
        else:
            return self.__setCSElementStatus(
                elementName, elementType, statusType, status
            )
    ...
    def __updateRssCache(self):
        ...
        for ti in range(5):
            rawCache = self.rssClient.selectStatusElement(
                "Resource", "Status", meta=meta
            )
            if rawCache["OK"]:
                break
            self.log.warn(
                "Can't get resource's status", rawCache["Message"] + "; trial %d" % ti
            )
            sleep(math.pow(ti, 2))
            self.rssClient = ResourceStatusClient()
    ...
    def __setRSSElementStatus(
        self, elementName, elementType, statusType, status, reason, tokenOwner
    ):
        """
        ...
        try:
            ...
            if not res["OK"]:
                _msg = "Error updating Element (%s,%s,%s)" % (
                    elementName,
                    statusType,
                    status,
                )
    ...
    def __setCSElementStatus(self, elementName, elementType, statusType, status):
        ...
        csAPI = CSAPI()
        csAPI.setOption(
            "%s/%s/%s/%s" % (cs_path, elementName, elementType, statusType), status
        )
