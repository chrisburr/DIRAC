...
class CSAPI(object):
    ...
    def addCEtoSite(self, siteName, ceName, optionsDict=None):
        ...
        self.__csMod.createSection(
            cfgPath(
                self.__baseResources, "Sites", siteName.split(".")[0], siteName, "CEs"
            )
        )
        self.__csMod.createSection(
            cfgPath(
    ...
    def __describeEntity(self, mask, hosts=False):
        ...
        if mask:
            entities = [
                entity
                for entity in self.__csMod.getSections(csSection)
                if entity in mask
            ]
        else:
        ...
        for entity in entities:
            ...
            if not hosts:
                ...
                for group in groupsDict:
                    if (
                        "Users" in groupsDict[group]
                        and entity in groupsDict[group]["Users"]
                    ):
    ...
    def __removeUserFromGroup(self, group, username):
        ...
        """
        usersInGroup = self.__csMod.getValue(
            "%s/Groups/%s/Users" % (self.__baseSecurity, group)
        )
        if usersInGroup is not None:
            ...
            self.__csMod.setOptionValue(
                "%s/Groups/%s/Users" % (self.__baseSecurity, group), ",".join(userList)
            )
    ...
    def addUser(self, username, properties):
        ...
        for userGroup in properties["Groups"]:
            if userGroup not in groups:
                gLogger.error(
                    "User group is not a valid group", "%s %s" % (username, userGroup)
                )
                return S_OK(False)
    ...
    def modifyUser(self, username, properties, createIfNonExistant=False):
        ...
        for prop in properties:
            ...
            if not prevVal or prevVal != properties[prop]:
                gLogger.info(
                    "Setting %s property for user %s to %s"
        ...
        if "Groups" in properties:
            ...
            for userGroup in properties["Groups"]:
                if userGroup not in groups:
                    gLogger.error(
                        "User group is not a valid group",
    ...
    def modifyGroup(self, groupname, properties, createIfNonExistant=False):
        ...
        for prop in properties:
            ...
            if not prevVal or prevVal != properties[prop]:
                gLogger.info(
                    "Setting %s property for group %s to %s"
    ...
    def addShifter(self, shifters=None):
        ...
        for shifter in shifters:
            ...
            self.__csMod.setOptionValue(
                section + "/" + shifter + "/" + "User", shifters[shifter]["User"]
            )
            self.__csMod.setOptionValue(
                section + "/" + shifter + "/" + "Group", shifters[shifter]["Group"]
            )
    ...
    def modifyHost(self, hostname, properties, createIfNonExistant=False):
        ...
        for prop in properties:
            ...
            if not prevVal or prevVal != properties[prop]:
                gLogger.info(
                    "Setting %s property for host %s to %s"
    ...
    def commitChanges(self, sortUsers=True):
        ...
        if self.csModified:
            ...
            if not retVal["OK"]:
                gLogger.error(
                    "Can't commit new configuration data", "%s" % retVal["Message"]
                )
                return retVal
    ...
    def modifyValue(self, optionPath, newValue):
        ...
        if prevVal is None:
            return S_ERROR(
                "Trying to set %s to %s but option does not exist"
                % (optionPath, newValue)
            )
        gLogger.verbose(
