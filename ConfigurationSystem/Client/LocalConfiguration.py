...
class LocalConfiguration(object):
    ...
    def __setOptionValue(self, optionPath, value):
        gConfigurationData.setOptionInCFG(
            self.__getAbsolutePath(optionPath), str(value)
        )

    def __registerBasicOptions(self):
        self.registerCmdOpt(
            "o:", "option=", "Option=value to add", self.__setOptionByCmd
        )
        self.registerCmdOpt(
            "s:",
            "section=",
            "Set base section for relative parsed options",
            self.__setSectionByCmd,
        )
        self.registerCmdOpt(
            "c:",
            "cert=",
            "Use server certificate to connect to Core Services",
            self.__setUseCertByCmd,
        )
        self.registerCmdOpt(
            "d", "debug", "Set debug mode (-ddd is extra debug)", self.__setDebugMode
        )
    ...
    def __checkMandatoryOptions(self):
        try:
            ...
            for optionPath in self.mandatoryEntryList:
                ...
                if not gConfigurationData.extractOptionFromCFG(optionPath):
                    gLogger.fatal(
                        "Missing mandatory local configuration option", optionPath
                    )
    ...
    def __parseCommandLine(self):
        ...
        for optionTuple in self.commandOptionList:
            if shortOption.find(optionTuple[0]) < 0:
                ...
                if optionTuple[0]:
                    gLogger.error(
                        "Short option -%s has been already defined" % optionTuple[0]
                    )
            if not optionTuple[1] in longOptionList:
                ...
                if optionTuple[1]:
                    gLogger.error(
                        "Long option --%s has been already defined" % optionTuple[1]
                    )

        try:
            opts, args = getopt.gnu_getopt(
                sys.argv[self.firstOptionIndex :], shortOption, longOptionList
            )
          ...
          To modify the local configuration use '--cfg <configfile>' instead."""
                ...
                if os.environ.get("DIRAC_DEPRECATED_FAIL", None):
                    raise NotImplementedError(
                        "ERROR: using deprecated config file passing option."
                    )
    ...
    def __loadCFGFiles(self):
        ...
        if "DIRACSYSCONFIG" in os.environ:
            diracSysConfigFiles = (
                os.environ["DIRACSYSCONFIG"].replace(" ", "").split(",")
            )
        ...
        for fileName in self.additionalCFGFiles:
            ...
            if not retVal["OK"]:
                gLogger.debug(
                    "Could not load file %s: %s" % (fileName, retVal["Message"])
                )
    ...
    def __addUserDataToConfiguration(self):
        ...
        for optionName, optionValue in self.parsedOptionList:
            ...
            for definedOptionTuple in self.commandOptionList:
                if optionName == definedOptionTuple[0].replace(
                    ":", ""
                ) or optionName == definedOptionTuple[1].replace("=", ""):
    ...
    def __setOptionByCmd(self, value):
        ...
        if len(valueList) < 2:
            ...
            return S_ERROR(
                "-o expects a option=value argument.\nFor example %s -o Port=1234"
                % sys.argv[0]
            )
    ...
    def showLicense(self, dummy=False):
        ...
        try:
            ...
            sys.stdout.write(
                "Can't find GPLv3 license at %s. Somebody stole it!\n" % lpath
            )
            sys.stdout.write(
                "Please check out http://www.gnu.org/licenses/gpl-3.0.html for more info\n"
            )
        DIRAC.exit(0)
        ...
        for iPos, iVal in enumerate(self.commandOptionList):
            ...
            if optionTuple[0].endswith(":"):
                ...
                gLogger.notice(
                    "  -%s --%s : %s"
                    % (
                        optionTuple[0].ljust(2),
                        optionTuple[1].ljust(22),
                        optionTuple[2],
                    )
                )
            iLastOpt = iPos
