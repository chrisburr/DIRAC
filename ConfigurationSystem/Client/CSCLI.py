...
class CSCLI(CLI):
    ...
    def _tryConnection(self):
        ...
        try:
            ...
            gLogger.error(
                "Couldn't connect to master CS server",
    ...
    def do_options(self, args):
        ...
        try:
            ...
            for option in optionsList:
                ...
                self.printPair(
                    option, self.modificator.getValue("%s/%s" % (section, option)), "="
                )
        except BaseException:
            _showTraceback()
    ...
    def do_writeToServer(self, dummy):
        ...
        try:
            ...
            if choice in ("yes", "y"):
                print(
                    "Uploading changes to %s (It may take some seconds)..."
                    % self.masterURL
                )
    ...
    def do_removeOption(self, args):
        ...
        try:
            ...
            optionPath = argsList[0].strip()
            choice = raw_input(
                "Are you sure you want to delete %s? yes/no [no]: " % optionPath
            )
            choice = choice.lower()
    ...
    def do_rollbackToVersion(self, args):
        ...
        try:
            ...
            choice = raw_input(
                "Do you really want to rollback to version %s? yes/no [no]: " % version
            )
            choice = choice.lower()
            if choice in ("yes", "y"):
