...
def divideFullName(entityName):
    ...
    if len(fields) < 2:
        raise RuntimeError(
            "Service (%s) name must be with the form system/service" % entityName
        )
    return tuple(fields)
...
def getComponentSection(
    componentName, componentTuple=False, setup=False, componentCategory="Services"
):
...
def getServiceFailoverURL(serviceName, serviceTuple=False, setup=False):
    ...
    systemSection = getSystemSection(serviceName, serviceTuple, setup=setup)
    url = gConfigurationData.extractOptionFromCFG(
        "%s/FailoverURLs/%s" % (systemSection, serviceTuple[1])
    )
    if not url:
        return ""
    if len(url.split("/")) < 5:
...
def getGatewayURLs(serviceName=""):
    ...
    if not siteName:
        return False
    gatewayList = gConfigurationData.extractOptionFromCFG(
        "/DIRAC/Gateways/%s" % siteName
    )
    if not gatewayList:
        return False
    if serviceName:
        gatewayList = [
            "%s/%s" % ("/".join(gw.split("/")[:3]), serviceName)
            for gw in List.fromChar(gatewayList, ",")
        ]
