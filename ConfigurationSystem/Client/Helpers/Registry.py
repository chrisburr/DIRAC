...
def __getGroupsWithAttr(attrName, value):
    ...
    return (
        S_OK(groups)
        if groups
        else S_ERROR("No groups found for %s=%s" % (attrName, value))
    )
...
def findDefaultGroupForUser(userName):
    ...
    return (
        S_OK(userGroups[0])
        if userGroups
        else S_ERROR("User %s has no groups" % userName)
    )
...
def getUserOption(userName, optName, defaultValue=""):
    ...
    return gConfig.getValue(
        "%s/Users/%s/%s" % (gBaseRegistrySection, userName, optName), defaultValue
    )
...
def getGroupOption(groupName, optName, defaultValue=""):
    ...
    """
    return gConfig.getValue(
        "%s/Groups/%s/%s" % (gBaseRegistrySection, groupName, optName), defaultValue
    )
...
def getHostOption(hostName, optName, defaultValue=""):
    ...
    """
    return gConfig.getValue(
        "%s/Hosts/%s/%s" % (gBaseRegistrySection, hostName, optName), defaultValue
    )
...
def getVOOption(voName, optName, defaultValue=""):
    ...
    """
    return gConfig.getValue(
        "%s/VO/%s/%s" % (gBaseRegistrySection, voName, optName), defaultValue
    )
...
def getVOMSVOForGroup(group):
    ...
    vomsVO = gConfig.getValue(
        "%s/Groups/%s/VOMSVO" % (gBaseRegistrySection, group), getDefaultVOMSVO()
    )
    if not vomsVO:
...
def getVOs():
    ...
    return (
        S_OK([voName])
        if voName
        else gConfig.getSections("%s/VO" % gBaseRegistrySection)
    )
    ...
    if result["OK"]:
        ...
        for vo in voNames:
            ...
            result = gConfig.getSections(
                "%s/VO/%s/VOMSServers" % (gBaseRegistrySection, vo)
            )
            if result["OK"]:
                ...
                for server in serverList:
                    ...
                    DN = gConfig.getValue(
                        "%s/VO/%s/VOMSServers/%s/DN"
                        % (gBaseRegistrySection, vo, server),
                        "",
                    )
                    CA = gConfig.getValue(
                        "%s/VO/%s/VOMSServers/%s/CA"
                        % (gBaseRegistrySection, vo, server),
                        "",
                    )
                    port = gConfig.getValue(
                        "%s/VO/%s/VOMSServers/%s/Port"
                        % (gBaseRegistrySection, vo, server),
                        0,
                    )
...
def getDNProperty(userDN, value, defaultValue=None):
    ...
    if not result["OK"]:
        return result
    pathDNProperties = "%s/Users/%s/DNProperties" % (
        gBaseRegistrySection,
        result["Value"],
    )
    ...
    if result["OK"]:
        for section in result["Value"]:
            if userDN == gConfig.getValue("%s/%s/DN" % (pathDNProperties, section)):
                return S_OK(
                    gConfig.getValue(
                        "%s/%s/%s" % (pathDNProperties, section, value), defaultValue
                    )
                )
...
def isDownloadableGroup(groupName):
    ...
    :return: boolean
    """
    if getGroupOption(groupName, "DownloadableProxy") in [
        False,
        "False",
        "false",
        "no",
