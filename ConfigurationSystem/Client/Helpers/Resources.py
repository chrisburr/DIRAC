...
def getGOCSiteName(diracSiteName):
    ...
    if not gocDBName:
        return S_ERROR(
            "No GOC site name for %s in CS (Not a grid site ?)" % diracSiteName
        )
...
def getDIRACSiteName(gocSiteName):
    ...
    tmpList = [
        (
            ...
            gConfig.getValue(
                cfgPath(
                    gBaseResourcesSection, "Sites", site.split(".")[0], site, "Name"
                )
            ),
        )
...
def getGOCFTSName(diracFTSName):
    ...
    if not gocFTSName:
        return S_ERROR(
            "No GOC FTS server name for %s in CS (Not a grid site ?)" % diracFTSName
        )
            ...
            for ce in ces:
                ...
                if ceTypeList:
                    ceType = gConfig.getValue(
                        "/Resources/Sites/%s/%s/CEs/%s/CEType" % (grid, site, ce), ""
                    )
                ...
                if community:
                    ...
                    if comList and community.lower() not in [
                        cl.lower() for cl in comList
                    ]:
                        continue
...
def getCompatiblePlatforms(originalPlatforms):
    ...
    platformsDict = dict(
        (k, v.replace(" ", "").split(",")) for k, v in result["Value"].items()
    )  # can be an iterator
