...
def getSiteUpdates(vo, bdiiInfo=None, log=None, glue2=True):
    ...
    for site in ceBdiiDict:
        ...
        for siteName in siteNames:
            siteSection = cfgPath(
                "/Resources", "Sites", siteName.split(".")[0], siteName
            )
            ...
            addToChangeSet(
                (siteSection, "Description", description, newdescription), changeSet
            )
            ...
            for ce in ces.get("Value", []):
                ...
                if result["OK"]:
                    ...
                    if ceBdiiDict[site]["CEs"].get(ce, None):
                        log.notice(
                            "Adding new CE", "%s to site %s/%s" % (ce, siteName, site)
                        )
                ...
                newarch = (
                    ceBdiiDict[site]["CEs"][ce]
                    .get("GlueHostArchitecturePlatformType", "")
                    .strip()
                )
                ...
                for queue in queues:
                    ...
                    if newMaxCPUTime == "4" * len(
                        newMaxCPUTime
                    ) or newMaxCPUTime == "9" * len(newMaxCPUTime):
                        newMaxCPUTime = ""
                    wallTime = queueInfo.get("GlueCEPolicyMaxWallClockTime", "")
                    if wallTime == "4" * len(wallTime) or wallTime == "9" * len(
                        wallTime
                    ):
                        wallTime = ""
                    ...
                    addToChangeSet(
                        (queueSection, "maxCPUTime", maxCPUTime, newMaxCPUTime),
                        changeSet,
                    )
                    ...
                    if newNOP != numberOfProcessors:
                        ...
                        if newNOP > 1:
                            ...
                            newTag = ",".join(
                                sorted(set(tag.split(",")).union({"MultiProcessor"}))
                            ).strip(",")
                            addToChangeSet(
                                (queueSection, "Tag", tag, newTag), changeSet
                            )
                            ...
                            newTag = ",".join(
                                sorted(
                                    set(tag.split(",")).difference({"MultiProcessor"})
                                )
                            ).strip(",")
                            changeSet.add((queueSection, "Tag", tag, newTag))
                    if maxTotalJobs == "Unknown":
                        newTotalJobs = min(
                            1000, int(int(queueInfo.get("GlueCEInfoTotalCPUs", 0)) / 2)
                        )
                        newWaitingJobs = max(2, int(newTotalJobs * 0.1))
                        newTotalJobs = str(newTotalJobs)
                        newWaitingJobs = str(newWaitingJobs)
                        addToChangeSet(
                            (queueSection, "MaxTotalJobs", "", newTotalJobs), changeSet
                        )
                        addToChangeSet(
                            (queueSection, "MaxWaitingJobs", "", newWaitingJobs),
                            changeSet,
                        )
                    ...
                    if queueDict.get("VO", ""):
                        VOs = set(
                            [q.strip() for q in queueDict.get("VO", "").split(",") if q]
                        )
                    if vo not in VOs:
                        VOs.add(vo)
                        VOs = list(VOs)
