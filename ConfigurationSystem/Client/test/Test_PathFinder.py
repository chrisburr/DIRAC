...
class TestGetComponentSection(TestPathFinder):
    def test_success(self):
        result = getComponentSection(
            "WorkloadManagement/SandboxStoreHandler", False, False, "Services"
        )
        correctResult = (
            "/Systems/WorkloadManagement/" + self.wm + "/Services/SandboxStoreHandler"
        )
    ...
    def test_sucessComponentStringDoesNotExist(self):
        ...
        correctResult = (
            ...
            + self.wm
            + "/NonRonsumersNon/SimpleLogConsumer"
        )
        self.assertEqual(result, correctResult)
...
if __name__ == "__main__":
    ...
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(TestGetComponentSection)
    )
