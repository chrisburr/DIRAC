...
class VOMS2CSSynchronizer(object):
    def __init__(
        ...
        if syncPluginName:
            ...
            _class = objLoader.loadObject(
                "ConfigurationSystem.Client.SyncPlugins.%sSyncPlugin" % syncPluginName,
                "%sSyncPlugin" % syncPluginName,
            )
    ...
    def syncCSWithVOMS(self):
        ...
        if not result["OK"]:
            self.log.error(
                "Could not retrieve user information from VOMS", result["Message"]
            )
            return result

        self.vomsUserDict = result["Value"]
        message = "There are %s user entries in VOMS for VO %s" % (
            len(self.vomsUserDict),
            self.vomsVOName,
        )
        ...
        self.log.info(
            ...
            ": there are %s registered users in DIRAC VO %s"
            % (len(diracUserDict), self.vo),
        )
        ...
        for dn in self.vomsUserDict:
            ...
            if dn in newDNs:
                ...
                if not diracName:
                    ...
                    if (
                        self.vomsUserDict[dn]["suspended"]
                        or self.vomsUserDict[dn]["certSuspended"]
                    ):
                    ...
                    if self.syncPlugin:
                        try:
                            self.syncPlugin.verifyAndUpdateUserInfo(
                                newDiracName, userDict
                            )
                        except ValueError as e:
                            self.log.error(
                                "Error validating new user",
                                "nickname %s\n error %s" % (newDiracName, e),
                            )
                            self.adminMsgs["Errors"].append(
                                "Error validating new user %s: %s\n  %s"
                                % (newDiracName, userDict, e)
                            )
                            continue
                    ...
                    if self.autoAddUsers:
                        self.log.info(
                            "Adding new user %s: %s" % (newDiracName, str(userDict))
                        )
            ...
            userDict = {
                "DN": dn,
                "CA": self.vomsUserDict[dn]["CA"],
                "Email": self.vomsUserDict[dn].get(
                    "mail", self.vomsUserDict[dn].get("emailAddress")
                )
                or knownEmail,
            }
            ...
            if (
                not suspendedInVOMS
                and self.vo in suspendedVOList
                and self.autoLiftSuspendedStatus
            ):
                newList = []
            ...
            if newDNForExistingUser:
                userDict["DN"] = ",".join(
                    [
                        dn,
                        diracUserDict.get(diracName, newAddedUserDict.get(diracName))[
                            "DN"
                        ],
                    ]
                )
            ...
            if not existingGroups and diracName in allDiracUsers:
                ...
                if groups["OK"]:
                    self.log.info(
                        "Found groups for user %s %s" % (diracName, groups["Value"])
                    )
        ...
        for user in diracUserDict:
            ...
            if (
                not dnSet.intersection(set(self.vomsUserDict))
                and user not in nonVOUserDict
            ):
                ...
                if removedGroups:
                    self.log.info(
                        "Checking user for deletion", "%s: %s" % (user, existingGroups)
                    )
                    self.log.info(
                        "User has groups in other VOs", "%s: %s" % (user, nonVOGroups)
                    )
                    ...
                    if self.autoModifyUsers:
                        ...
                        if result["OK"] and result["Value"]:
                            self.log.info(
                                "Modified user %s: %s" % (user, str(userDict))
                            )
                            self.voChanged = True
                            message = "\n  Modified user %s:\n" % user
                            modMsg = "    Removed from group(s) %s\n" % ",".join(
        ...
        for user in diracUserDict:
            ...
            for dn in dnSet:
                if dn in obsoletedDNs and user not in oldUsers:
                    ...
                    if nonVOGroups:
                        self.log.verbose(
                            "User has groups in other VOs",
                            "%s: %s" % (user, nonVOGroups),
                        )
                        continue
                    ...
                    if self.autoModifyUsers:
                        ...
                        if modDNSet:
                            ...
                            if result["OK"] and result["Value"]:
                                self.log.info(
                                    "Modified user %s: dropped DN %s" % (user, dn)
                                )
                                self.adminMsgs["Info"].append(
                                    "Modified user %s: dropped DN %s" % (user, dn)
                                )
    ...
    def getVOUserReport(self):
        ...
        output = printTable(
            fields, records, sortField="Group", printOut=False, numbering=False
        )
