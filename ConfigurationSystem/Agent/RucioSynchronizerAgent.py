            ...
            if res["OK"]:
                ...
                for key in [
                    ...
                    if key == "Protocol":
                        ...
                        params["domains"]["lan"]["write"] = write_protocols.get(
                            value, 0
                        )
                        params["domains"]["wan"]["write"] = write_protocols.get(
                            value, 0
                        )
                        params["domains"]["lan"]["delete"] = write_protocols.get(
                            value, 0
                        )
                        params["domains"]["wan"]["delete"] = write_protocols.get(
                            value, 0
                        )
                        params["domains"]["wan"][
                            "third_party_copy"
                        ] = write_protocols.get(value, 0)
        ...
        try:
            ...
            if result["OK"]:
                ...
                for se in result["Value"]:
                    ...
                    for params in result["Value"][se]:
                        ...
                        if (
                            prot not in existing_protocols
                            and prot[0] in valid_protocols
                        ):
                            ...
                            try:
                                ...
                                self.log.info(
                                    "Protocol %s already exists on %s",
                                    params["scheme"],
                                    se,
                                )
                            except Exception as err:
                                self.log.error(
                                    "Cannot create protocol on RSE %s : %s",
                                    se,
                                    str(err),
                                )
                        else:
                                ...
                                if prot == (
                                    ...
                                    if update:
                                        data = {
                                            ...
                                            "read_lan": params["domains"]["lan"][
                                                "read"
                                            ],
                                            "read_wan": params["domains"]["wan"][
                                                "read"
                                            ],
                                            "write_lan": params["domains"]["lan"][
                                                "write"
                                            ],
                                            "write_wan": params["domains"]["wan"][
                                                "write"
                                            ],
                                            "delete_lan": params["domains"]["lan"][
                                                "delete"
                                            ],
                                            "delete_wan": params["domains"]["wan"][
                                                "delete"
                                            ],
                                            "third_party_copy": params["domains"][
                                                "wan"
                                            ]["write"],
                                        }
                                        self.log.info(
                    ...
                    for prot in existing_protocols:
                        if prot not in protocols_to_create:
                            ...
                            client.delete_protocols(
                                se, scheme=prot[0], hostname=prot[1], port=prot[2]
                            )
            else:
            ...
            if newRSE:
                ...
                for src_rse, dest_rse in permutations(rses, r=2):
                    try:
                        client.add_distance(
                            src_rse, dest_rse, {"ranking": 1, "distance": 10}
                        )
                    except Exception as err:
                        self.log.error(
                            "Cannot add distance for %s:%s : %s",
                            src_rse,
                            dest_rse,
                            str(err),
                        )
            ...
            if result["OK"]:
                ...
                for rse in rses:
                    try:
                        self.log.info(
                            "Setting productionSEshare for %s : %s",
                            rse,
                            rseDict.get(rse, 0),
                        )
                        client.add_rse_attribute(
                            rse, "productionSEshare", rseDict.get(rse, 0)
                        )
                    except Exception as err:
                ...
                for dataLevel in result["Value"]:
                    ...
                    for rse in rses:
                        try:
                            ...
                            client.add_rse_attribute(
                                rse, "%sShare" % dataLevel, rseDict.get(rse, 0)
                            )
                        except Exception as err:
                            self.log.error(
                                "Cannot create %sShare for %s", dataLevel, rse
                            )
            else:
                ...
                for rse in rses:
                    if rse in allSEs:
                        ...
                        try:
                            ...
                            self.log.error(
                                "Cannot create RSE attribute OccupancyLFN for %s : %s",
                                rse,
                                str(err),
                            )
                    if rse in primarySEs:
                        try:
                            ...
                            self.log.error(
                                "Cannot create RSE attribute PrimaryDataSE for %s : %s",
                                rse,
                                str(err),
                            )
                    else:
                        try:
                            ...
                            self.log.error(
                                "Cannot remove RSE attribute PrimaryDataSE for %s : %s",
                                rse,
                                str(err),
                            )
            ...
            for account in getAllUsers():
                ...
                if account not in listAccounts:
                    ...
                    try:
                        ...
                        self.log.error(
                            "Cannot create account %s : %s", account, str(err)
                        )
                    try:
                        client.add_identity(
                            account=account,
                    ...
                    try:
                        ...
                        self.log.error(
                            "Cannot create identity %s for account %s : %s",
                            dn,
                            account,
                            str(err),
                        )
            ...
            for group in groups:
                if group not in listAccounts:
                    ...
                    try:
                        ...
                        self.log.error(
                            "Cannot create account %s : %s" % (group, str(err))
                        )
                    for rse in rses:
                        client.set_local_account_limit(account, rse, 1000000000000000)
            ...
            if not result["OK"]:
                ...
                for host in hosts:
                    ...
                    try:
                        ...
                        self.log.error(
                            "Cannot create identity %s for account dirac_srv : %s"
                            % (dn, str(err))
                        )
                        self.log.error(format_exc())
