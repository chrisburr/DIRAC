...
class Bdii2CSAgent(AgentModule):
    ...
    def initialize(self):
        ...
        self.alternativeBDIIs = self.am_getOption(
            "AlternativeBDIIs", self.alternativeBDIIs
        )
        ...
        if not self.voName or (
            len(self.voName) == 1 and self.voName[0].lower() == "all"
        ):
        ...
        for vo in self.voName:
            ...
            if body:
                ...
                if self.addressTo and self.addressFrom:
                    ...
                    if not result["OK"]:
                        self.log.error(
                            "Can not send new site notification mail", result["Message"]
                        )
    ...
    def __getBdiiCEInfo(self, vo):
        ...
        if not mainResult["OK"]:
            self.log.error(
                "Failed getting information from default bdii", mainResult["Message"]
            )
        ...
        for bdii in reversed(self.alternativeBDIIs):
            ...
            if resultAlt["OK"]:
                ...
                self.log.error(
                    "Failed getting information from %s " % bdii, resultAlt["Message"]
                )
        ...
        if (
            not totalResult["Value"] and message
        ):  # Dict is empty and we have an error message
    ...
    def __purgeSites(self, ceBdiiDict):
        ...
        for site in list(ceBdiiDict):
            ...
            for ce in ces:
                ...
                if not res["OK"]:
                    self.log.error(
                        "Failed to get DIRAC site name for ce",
                        "%s: %s" % (ce, res["Message"]),
                    )
                    continue
        ...
        if changeSet:
            ...
            if self.dryRun:
                ...
                if not result["OK"]:
                    ...
                    self.log.info(
                        "Successfully committed %d changes to CS" % len(changeList)
                    )
                return result
        else:
