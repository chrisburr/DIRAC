...
class GOCDB2CSAgent(AgentModule):
    ...
    def updatePerfSONARConfiguration(self):
        ...
        if not result["OK"]:
            log.error(
                "__getPerfSONAREndpoints() failed with message: %s" % result["Message"]
            )
        ...
        if not result["OK"]:
            log.error(
                "__addDIRACSiteName() failed with message: %s" % result["Message"]
            )
    ...
    def __getPerfSONAREndpoints(self):
        ...
        for endpointType in ["Latency", "Bandwidth"]:
            ...
            if not result["OK"]:
                log.error(
                    "getServiceEndpointInfo() failed with message: %s"
                    % result["Message"]
                )
                return S_ERROR(
                    "Could not fetch %s endpoints from GOCDB" % endpointType.lower()
                )

            log.debug(
                "Number of %s endpoints: %s"
    ...
    def __addDIRACSiteName(self, inputList):
        ...
        if not result["OK"]:
            log.error(
                "getDIRACGOCDictionary() failed with message: %s" % result["Message"]
            )
    ...
    def __updateConfiguration(self, setElements=None, delElements=None):
        ...
        for path, value in setElements.iteritems():
            ...
            try:
                ...
                if not result["OK"]:
                    log.error(
                        "createSection() failed with message: %s" % result["Message"]
                    )
            except Exception as e:
                log.error(
                    "Exception in createSection(): %s" % repr(e).replace(",)", ")")
                )
            ...
            if value is not None:
                try:
                    ...
                    if not result["OK"]:
                        log.error(
                            "setOption() failed with message: %s" % result["Message"]
                        )
                except Exception as e:
                    log.error(
                        "Exception in setOption(): %s" % repr(e).replace(",)", ")")
                    )
