...
class VOMS2CSAgent(AgentModule):
    ...
    def initialize(self):
        ...
        self.autoModifyUsers = self.am_getOption(
            "AutoModifyUsers", self.autoModifyUsers
        )
        self.autoDeleteUsers = self.am_getOption(
            "AutoDeleteUsers", self.autoDeleteUsers
        )
        self.autoLiftSuspendedStatus = self.am_getOption(
            "AutoLiftSuspendedStatus", self.autoLiftSuspendedStatus
        )
    ...
    def execute(self):
        ...
        for vo in self.voList:
            ...
            voAdminGroup = getVOOption(
                vo, "VOAdminGroup", getVOOption(vo, "DefaultGroup")
            )

            self.log.info(
                "Performing VOMS sync",
            ...
            autoLiftSuspendedStatus = getVOOption(
                vo, "AutoLiftSuspendedStatus", self.autoLiftSuspendedStatus
            )
            ...
            if not result["OK"]:
                self.log.error(
                    "Failed to perform VOMS to CS synchronization:",
            ...
            if csapi.csModified:
                ...
                if self.dryRun:
                    ...
                    if not result["OK"]:
                        self.log.error(
                            "Could not commit configuration changes", result["Message"]
                        )
                        return result
                    self.log.notice("Configuration committed", "for VO %s" % vo)
            else:
                self.log.info(
                    "No changes to the CS recorded at this cycle", "for VO %s" % vo
                )
            ...
            if self.makeFCEntry and newUsers:
                ...
                if not result["OK"]:
                    self.log.error(
                        "Failed to create user home directories:",
                        "VO %s: %s" % (vo, result["Message"]),
                    )
                else:
