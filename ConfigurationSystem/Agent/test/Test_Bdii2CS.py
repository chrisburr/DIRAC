...
ALTBDII = {
    "site2": {
        "CEs": {
            "ce2": {"Queues": {"queue2": "SomeOtherValues"}},
...
class Bdii2CSTests(unittest.TestCase):
    def setUp(self):
        with patch(
            "DIRAC.ConfigurationSystem.Agent.Bdii2CSAgent.AgentModule.__init__",
            new=Mock(),
        ), patch(
            "DIRAC.ConfigurationSystem.Agent.Bdii2CSAgent.AgentModule.am_getModuleParam",
            new=Mock(return_value="fullName"),
        ):
            self.agent = Bdii2CSAgent.Bdii2CSAgent(
                agentName="Configuration/testing", loadName="Configuration/testing"
            )
    ...
    def test__getBdiiCEInfo_success(self):
        ...
        with patch(
            ...
            ret = self.agent._Bdii2CSAgent__getBdiiCEInfo(
                "vo"
            )  # pylint: disable=no-member
            infoMock.assert_any_call(
                "vo", host=self.agent.host, glue2=self.agent.glue2Only
            )
            infoMock.assert_any_call("vo", host="server2", glue2=self.agent.glue2Only)
        self.assertTrue(ret["OK"])
        self.assertEqual(expectedResult, ret["Value"])
        self.assertEqual(
            ret["Value"]["site2"]["CEs"]["ce2"]["Queues"]["queue2"], "SomeValues"
        )
