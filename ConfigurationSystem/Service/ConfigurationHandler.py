...
class ConfigurationHandler(RequestHandler):
    ...
    def export_commitNewData(self, sData):
        ...
        updatePilotCStoJSONFileFlag = Operations().getValue(
            "Pilot/UpdatePilotCStoJSONFile", True
        )
        ...
        if updatePilotCStoJSONFileFlag and gServiceInterface.isMaster():
            if gPilotSynchronizer is None:
                try:
                    ...
                    self.log.exception(
                        "Failed to import PilotCStoJSONSynchronizer", repr(exc)
                    )
                    return S_ERROR(
                        DErrno.EIMPERR, "Failed to import PilotCStoJSONSynchronizer"
                    )
    ...
    def export_getVersionContents(cls, versionList):
        ...
        for version in versionList:
            ...
            if retVal["OK"]:
                ...
                return S_ERROR(
                    "Can't get contents for version %s: %s"
                    % (version, retVal["Message"])
                )
    ...
    def export_rollbackToVersion(self, version):
        ...
        if not retVal["OK"]:
            return S_ERROR(
                "Can't get contents for version %s: %s" % (version, retVal["Message"])
            )
        credDict = self.getRemoteCredentials()
        if "DN" not in credDict or "username" not in credDict:
            return S_ERROR("You must be authenticated!")
        return gServiceInterface.updateConfiguration(
