...
class CheckConfig(object):
    ...
    def _setSwitches(self):
        Script.registerSwitch(
            "S:",
            "system=",
            "Systems to check, by default all of them are checked",
            self._setSystems,
        )
        Script.registerSwitch(
            "M",
            "modified",
            "Show entries which differ from the default",
            self._setShowModified,
        )
        Script.registerSwitch(
            "A",
            "added",
            "Show entries which do not exist in ConfigTemplate",
            self._setShowAdded,
        )
        Script.registerSwitch(
            "U",
    ...
    def _printDiff(self, entry, level=""):
        ...
        if diffType == "modSec":
            ...
            if self.showMissingSections:
                LOG.notice(
                    "Section %r not found in current configuration: %s"
                    % (fullPath, pformat(changes))
                )
        elif diffType == "addOpt":
            if self.showMissingOptions:
                LOG.notice(
                    "Option %r not found in current configuration. Default value is %r"
                    % (fullPath, changes)
                )
        else:
