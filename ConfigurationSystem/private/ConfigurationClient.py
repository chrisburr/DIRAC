...
class ConfigurationClient(object):
    ...
    def getOption(self, optionPath, typeValue=None):
        ...
        if requestedType in (list, tuple, set):
            ...
            try:
                ...
                return S_ERROR(
                    "Can't convert value (%s) to Boolean \n%s"
                    % (str(optionValue), repr(e))
                )
    ...
    def getConfigurationTree(self, root="", *filters):
        ...
        if option["OK"]:
            ...
            if not options["OK"]:
                return S_ERROR(
                    "getOptionsDict() failed with message: %s" % options["Message"]
                )
            ...
            if not sections["OK"]:
                return S_ERROR(
                    "getSections() failed with message: %s" % sections["Message"]
                )
