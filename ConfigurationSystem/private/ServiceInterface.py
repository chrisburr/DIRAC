...
class ServiceInterface(threading.Thread):
    ...
    def publishSlaveServer(self, sSlaveURL):
        ...
        if bNewSlave:
            gConfigurationData.setServers(
                "%s, %s" % (self.sURL, ", ".join(self.dAliveSlaveServers.keys()))
            )
    ...
    def __processResults(self, id_, result):
        if result["OK"]:
            ...
            gLogger.warn(
                "Failed to update configuration on",
    ...
    def updateConfiguration(self, sBuffer, committer="", updateVersionOption=False):
        ...
        if sRemoteName != sLocalName:
            return S_ERROR(
                "Names differ: Server is %s and remote is %s"
    ...
    def __getCfgBackups(self, basePath, date="", subPath=""):
        ...
        for entry in fsEntries:
            ...
            if os.path.isdir(entryPath):
                backupsList.extend(
                    self.__getCfgBackups(basePath, date, "%s/%s" % (subPath, entry))
                )
    ...
    def __getPreviousCFG(self, oRemoteConfData):
        backupsList = self.__getCfgBackups(
            gConfigurationData.getBackupDir(), date=oRemoteConfData.getVersion()
        )
        if not backupsList:
            return S_ERROR(
                "Could not AutoMerge. Could not retrieve original committer's version"
            )
        ...
        gLogger.info(
            "Loaded client original version %s" % prevRemoteConfData.getVersion()
        )
    ...
    def _checkConflictsInModifications(self, realModList, reqModList, parentSection=""):
        realModifiedSections = dict(
            [
                ...
                if modAc[0].find("Sec") == len(modAc[0]) - 3
            ]
        )
        reqOptionsModificationList = dict(
            [
                ...
                if modAc[0].find("Opt") == len(modAc[0]) - 3
            ]
        )
        for modAc in reqModList:
            action = modAc[0]
            ...
            if action == "addSec":
                if objectName in realModifiedSections:
                    return S_ERROR(
                        "Section %s/%s already exists" % (parentSection, objectName)
                    )
            elif action == "delSec":
                if objectName in realModifiedSections:
                    return S_ERROR(
                        "Section %s/%s cannot be deleted. It has been modified."
                        % (parentSection, objectName)
                    )
            elif action == "modSec":
                if objectName in realModifiedSections:
                    result = self._checkConflictsInModifications(
                        realModifiedSections[objectName],
                        modAc[3],
                        "%s/%s" % (parentSection, objectName),
                    )
                    if not result["OK"]:
                        return result
        for modAc in realModList:
    ...
    def __mergeIndependentUpdates(self, oRemoteConfData):
        ...
        result = self._checkConflictsInModifications(
            prevCliToCurSrvModList, prevCliToCurCliModList
        )
        if not result["OK"]:
