...
class ConfigurationData(object):
    ...
    def sync(self):
        ...
        localServers = self.extractOptionFromCFG(
            "%s/Servers" % self.configurationPath,
            self.localCFG,
    ...
    def getCommentFromCFG(self, path, cfg=False):
        ...
        try:
            levelList = [
                level.strip() for level in path.split("/") if level.strip() != ""
            ]
    ...
    def getName(self):
        return self.extractOptionFromCFG(
            "%s/Name" % self.configurationPath, self.mergedCFG
        )

    def exportName(self):
        return self.setOptionInCFG(
            "%s/Name" % self.configurationPath, self.getName(), self.remoteCFG
        )
    ...
    def getPropagationTime(self):
        try:
            return int(
                self.extractOptionFromCFG(
                    "%s/PropagationTime" % self.configurationPath, self.mergedCFG
                )
            )
        except BaseException:
            return 300
    ...
    def getSlavesGraceTime(self):
        try:
            return int(
                self.extractOptionFromCFG(
                    "%s/SlavesGraceTime" % self.configurationPath, self.mergedCFG
                )
            )
        except BaseException:
            return 600
    ...
    def setServers(self, sServers):
        self.setOptionInCFG(
            "%s/Servers" % self.configurationPath, sServers, self.remoteCFG
        )
        self.sync()
    ...
    def setMasterServer(self, sURL):
        self.setOptionInCFG(
            "%s/MasterServer" % self.configurationPath, sURL, self.remoteCFG
        )
        self.sync()
    ...
    def isMaster(self):
        value = self.extractOptionFromCFG(
            "%s/Master" % self.configurationPath, self.localCFG
        )
    ...
    def __backupCurrentConfiguration(self, backupName):
        ...
        backupPath = os.path.join(
            self.getBackupDir(), str(today.year), "%02d" % today.month
        )
        mkDir(backupPath)
        backupFile = os.path.join(
            backupPath, configurationFilename.replace(".cfg", ".%s.zip" % backupName)
        )
        if os.path.isfile(configurationFile):
            ...
            try:
                ...
                gLogger.exception()
                gLogger.error(
                    "Cannot backup configuration data file", "file %s" % backupFile
                )
        else:
    ...
    def writeRemoteConfigurationToDisk(self, backupName=False):
        configurationFile = os.path.join(
            DIRAC.rootPath, "etc", "%s.cfg" % self.getName()
        )
        try:
