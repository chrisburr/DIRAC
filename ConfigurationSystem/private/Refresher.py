    ...
    if retVal["OK"]:
        ...
        if localVersion < dataDict["newestVersion"]:
            gLogger.debug(
                ...
                "Updating to version %s..." % dataDict["newestVersion"],
            )
            ...
            gEventDispatcher.triggerEvent(
                "CSNewVersion", dataDict["newestVersion"], threaded=True
            )
        return S_OK()
    return retVal
...
class Refresher(threading.Thread):
    ...
    def refreshConfigurationIfNeeded(self):
        if (
            not self.__refreshEnabled
            or self.__automaticUpdate
            or not gConfigurationData.getServers()
        ):
            return
        ...
        for sServer in randomServerList:
            ...
            if dRetVal["OK"]:
                ...
                gLogger.warn(
                    ...
                    "Error while updating from %s: %s" % (sServer, dRetVal["Message"]),
                )
