...
class RegressionTestCase(IntegrationTest):
    ...
    def setUp(self):
        ...
        try:
            exeScriptLoc = find_all("exe-script.py", rootPath, "/DIRAC/tests/Workflow")[
                0
            ]
            helloWorldLoc = find_all(
                "helloWorld.py", rootPath, "/DIRAC/tests/Workflow"
            )[0]
        except IndexError:  # we are in Jenkins
            exeScriptLoc = find_all(
                "exe-script.py", os.environ["WORKSPACE"], "/DIRAC/tests/Workflow"
            )[0]
            helloWorldLoc = find_all(
                "helloWorld.py", os.environ["WORKSPACE"], "/DIRAC/tests/Workflow"
            )[0]
        ...
        try:
            helloWorldXMLLocation = find_all(
                "helloWorld.xml", rootPath, "/DIRAC/tests/Workflow/Regression"
            )[0]
        except IndexError:  # we are in Jenkins
            helloWorldXMLLocation = find_all(
                "helloWorld.xml",
                os.environ["WORKSPACE"],
                "/DIRAC/tests/Workflow/Regression",
            )[0]
