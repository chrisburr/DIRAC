...
class UserJobTestCase(IntegrationTest):
    ...
    def setUp(self):
        ...
        try:
            self.exeScriptLocation = find_all(
                "exe-script.py", rootPath, integration_test_dir
            )[0]
            self.helloWorld = find_all("helloWorld.py", rootPath, integration_test_dir)[
                0
            ]
            self.mpExe = find_all("mpTest.py", rootPath, "/DIRAC/tests/Utilities")[0]
            self.mpExeFlex = find_all(
                "mpTest-flexible.py", rootPath, "/DIRAC/tests/Utilities"
            )[0]
        except IndexError:  # we are in Jenkins
            self.exeScriptLocation = find_all(
                "exe-script.py", os.environ["WORKSPACE"], integration_test_dir
            )[0]
            self.helloWorld = find_all(
                "helloWorld.py", os.environ["WORKSPACE"], integration_test_dir
            )[0]
            self.mpExe = find_all(
                "mpTest.py", os.environ["WORKSPACE"], "/DIRAC/tests/Utilities"
            )[0]
            self.mpExeFlex = find_all(
                "mpTest-flexible.py", os.environ["WORKSPACE"], "/DIRAC/tests/Utilities"
            )[0]
...
class MPSuccessMinMax(UserJobTestCase):
    def test_min2(self):
        ...
        j.setInputSandbox(
            find_all("mpTest-flexible.py", rootPath, "DIRAC/tests/Utilities")[0]
        )
        j.setNumberOfProcessors(
            minNumberOfProcessors=2
        )  # This requires at least 2 processors
        j.setLogLevel("DEBUG")
        try:
            # This is the standard location in Jenkins
            j.setInputSandbox(
    ...
    def test_min2max4(self):
        ...
        j.setNumberOfProcessors(
            minNumberOfProcessors=2, maxNumberOfProcessors=4
        )  # This requires 2 to 4 processors
        j.setLogLevel("DEBUG")
        try:
            # This is the standard location in Jenkins
            j.setInputSandbox(
    ...
    def test_min1(self):
        ...
        )  # This requires 1 to infinite processors
        j.setLogLevel("DEBUG")
        try:
            # This is the standard location in Jenkins
            j.setInputSandbox(
...
if __name__ == "__main__":
    ...
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(HelloWorldPlusSuccess)
    )
