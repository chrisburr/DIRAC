...
Script.registerSwitch(
    "D:", "softwareDistModule=", "set the software dist module to update."
)
...
if cFile:
    ...
    if os.path.isfile(
        os.path.expandvars("$WORKSPACE") + "/PilotInstallDIR/etc/dirac.cfg"
    ):
        localConfigFile = (
            os.path.expandvars("$WORKSPACE") + "/PilotInstallDIR/etc/dirac.cfg"
        )
    elif os.path.isfile(
        os.path.expandvars("$WORKSPACE") + "/ServerInstallDIR/etc/dirac.cfg"
    ):
        localConfigFile = (
            os.path.expandvars("$WORKSPACE") + "/ServerInstallDIR/etc/dirac.cfg"
        )
