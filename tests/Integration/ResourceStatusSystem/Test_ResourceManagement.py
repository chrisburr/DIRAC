...
class ResourceManagementClientChain(TestClientResourceManagementTestCase):
    ...
    def test_DowntimeCache(self):
        ...
        res = self.rmClient.deleteDowntimeCache(
            "TestName12345"
        )  # just making sure it's not there (yet)
        self.assertTrue(res["OK"])
        ...
        res = self.rmClient.addOrModifyDowntimeCache(
            "TestName12345", "element", "name", severity="changedSeverity"
        )
        self.assertTrue(res["OK"])
    ...
    def test_GGUSTicketsCache(self):
        ...
        res = self.rmClient.deleteGGUSTicketsCache(
            "TestName12345"
        )  # just making sure it's not there (yet)
        self.assertTrue(res["OK"])

        # TEST addOrModifyGGUSTicketsCache
        res = self.rmClient.addOrModifyGGUSTicketsCache(
            "TestName12345", "link", 0, "tickets", datetime.datetime.now()
        )
        self.assertTrue(res["OK"])
    ...
    def test_JobCache(self):
        ...
        res = self.rmClient.deleteJobCache(
            "TestName12345"
        )  # just making sure it's not there (yet)
        self.assertTrue(res["OK"])

        # TEST addOrModifyJobCache
        res = self.rmClient.addOrModifyJobCache(
            "TestName12345", "maskstatus", 50.89, "status", datetime.datetime.now()
        )
        self.assertTrue(res["OK"])
    ...
    def test_PilotCache(self):
        ...
        res = self.rmClient.deletePilotCache(
            "TestName12345"
        )  # just making sure it's not there (yet)
        self.assertTrue(res["OK"])

        # TEST addOrModifyPilotCache
        res = self.rmClient.addOrModifyPilotCache(
            "TestName12345", "CE", 0.0, 25.5, "status", datetime.datetime.now()
        )
        self.assertTrue(res["OK"])
        ...
        self.assertEqual(
            res["Value"][0][6], "all"
        )  # default value for vo, as the last element
    ...
    def test_PolicyResult(self):
        ...
        res = self.rmClient.selectPolicyResult(
            "element", "TestName12345", "policyName", "statusType"
        )
        self.assertTrue(res["OK"])
        # check if the name that we got is equal to the previously added 'TestName12345'
    ...
    def test_SpaceTokenOccupancy(self):
        ...
        res = self.rmClient.deleteSpaceTokenOccupancyCache(
            "endpoint", "token"
        )  # just making sure it's not there (yet)
        self.assertTrue(res["OK"])
    ...
    def test_Transfer(self):
        ...
        res = self.rmClient.addOrModifyTransferCache(
            "sourcename", "destinationname", value=200.0
        )
        self.assertTrue(res["OK"])
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(
        TestClientResourceManagementTestCase
    )
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(ResourceManagementClientChain)
    )
