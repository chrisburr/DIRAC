...
class ResourceStatusClientChain(TestClientResourceStatusTestCase):
    def test_addAndRemove(self):
        ...
        res = rssClient.selectStatusElement(
            "Resource", "Log", "TestName1234", meta={"columns": ["name"]}
        )
        # check if the select query was executed properly
        self.assertTrue(res["OK"])
        self.assertEqual(res["Value"], [])
        ...
        self.assertEqual(res["Value"][0][2], "Active")
        print(
            "inserted lastCheckTime and DateEffective: %s, %s"
            % (res["Value"][0][7], res["Value"][0][4])
        )
        ...
        # (it should NOT be there)
        res = rssClient.selectStatusElement(
            "Site", "Log", "TestName1234", meta={"columns": ["name"]}
        )
        # check if the select query was executed properly
        self.assertTrue(res["OK"])
        self.assertEqual(res["Value"], [])
        ...
        time.sleep(
            3
        )  # just for seeing a difference between lastCheckTime and DateEffective
        print(
            "modifing once more the previously entered element, but this time we only modify the reason"
        )
        res = rssClient.modifyStatusElement(
            "Resource",
            "Status",
        ...
        )  # this is the last one (in this order)
        self.assertEqual(res["Value"][3][1], "statusType")
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(
        TestClientResourceStatusTestCase
    )
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(ResourceStatusClientChain)
    )
