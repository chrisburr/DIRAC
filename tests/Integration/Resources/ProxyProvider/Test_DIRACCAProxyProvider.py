...
class DIRACCAPPTest(unittest.TestCase):
    ...
    def test_getProxy(self):
        for dn, res in [
            (
                "/C=FR/O=DIRAC/OU=DIRAC TEST/CN=DIRAC test user/emailAddress=testuser@diracgrid.org",
                True,
            ),
    ...
    def test_generateProxyDN(self):
        ...
        self.assertEqual(
            issuer,
            "/C=FR/O=DIRAC/OU=DIRAC TEST/CN=John Doe/emailAddress=john.doe@nowhere.net",
        )
