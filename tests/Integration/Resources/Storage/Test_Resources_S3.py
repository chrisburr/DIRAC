...
local_path = (
    download_dir
) = putDir = createDir = putFile = isFile = listDir = getFile = rmDir = None
    ...
    for root, _dirs, files in os.walk(local_path):
        for fn in files:
            filesToRemove.append(
                os.path.join(target_path, root.replace(local_path, "").strip("/"), fn)
            )
        ...
        for root, _dirs, files in os.walk(os.path.join(local_path, dirName)):
            for fn in files:
                filesInFolderAandB.append(
                    os.path.join(
                        DESTINATION_PATH, root.replace(local_path, "").strip("/"), fn
                    )
                )
...
def test_getFile(setuptest):
    ...
    assert expectedNbOfFiles == sum(
        [len(files) for _, _, files in os.walk(download_dir)]
    )
