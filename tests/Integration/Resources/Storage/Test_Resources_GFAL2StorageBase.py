...
class basicTest(unittest.TestCase):
    ...
    def setUp(self, pluginToTest):
        ...
        if pluginProtocol in specSE.localAccessProtocolList:
            ...
            print(
                "Plugin %s is not available for read. Use a generic SE" % pluginToTest
            )
        ...
        if pluginProtocol in specSE.localWriteProtocolList:
            ...
            print(
                "Plugin %s is not available for write. Use a generic SE" % pluginToTest
            )
            self.writeSE = genericSE

        # Make sure we are testing the specific plugin at least for one
        self.assertTrue(
            self.readSE == specSE or self.writeSE == specSE,
    ...
    def testWorkflow(self):
        ...
        putDir = {
            os.path.join(DESTINATION_PATH, "Workflow/FolderA"): os.path.join(
                self.LOCAL_PATH, "Workflow/FolderA"
            ),
            os.path.join(DESTINATION_PATH, "Workflow/FolderB"): os.path.join(
                self.LOCAL_PATH, "Workflow/FolderB"
            ),
        }
        ...
        putFile = {
            os.path.join(DESTINATION_PATH, "Workflow/FolderA/File1"): os.path.join(
                self.LOCAL_PATH, "Workflow/File1"
            ),
            os.path.join(DESTINATION_PATH, "Workflow/FolderAA/File1"): os.path.join(
                self.LOCAL_PATH, "Workflow/File1"
            ),
            os.path.join(DESTINATION_PATH, "Workflow/FolderBB/File2"): os.path.join(
                self.LOCAL_PATH, "Workflow/File2"
            ),
            os.path.join(DESTINATION_PATH, "Workflow/FolderB/File2"): os.path.join(
                self.LOCAL_PATH, "Workflow/File2"
            ),
            os.path.join(DESTINATION_PATH, "Workflow/File3"): os.path.join(
                self.LOCAL_PATH, "Workflow/File3"
            ),
        }
        ...
        self.assertEqual(
            any(
                ...
                for dictKey in res["Value"]["Successful"][
                    os.path.join(DESTINATION_PATH, "Workflow/FolderA")
                ]["Files"].keys()
            ),
            True,
        ...
        self.assertEqual(
            any(path in resKey for path in isFile for resKey in res.keys()), True
        )
...
@unittest.skipIf(
    "GFAL2_SRM2" not in AVAILABLE_PLUGINS,
...
@unittest.skipIf(
    "GFAL2_HTTPS" not in AVAILABLE_PLUGINS,
...
@unittest.skipIf(
    "GFAL2_XROOT" not in AVAILABLE_PLUGINS,
...
@unittest.skipIf(
    "XROOT" not in AVAILABLE_PLUGINS,
