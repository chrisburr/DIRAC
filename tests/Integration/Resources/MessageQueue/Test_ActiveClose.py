...
from DIRAC.Resources.MessageQueue.StompMQConnector import (
    StompMQConnector as MyStompConnector,
)
...
class Test_MQProducers_1(Test_MQProducers):
    @mock.patch(
        "DIRAC.Resources.MessageQueue.MQCommunication.getMQParamsFromCS",
        side_effect=pseudoCS,
    )
...
class Test_MQProducers_4(Test_MQProducers):
    @mock.patch(
        "DIRAC.Resources.MessageQueue.StompMQConnector.StompMQConnector.reconnect",
        side_effect=pseudoReconnect,
    )
    @mock.patch(
        "DIRAC.Resources.MessageQueue.MQConnectionManager.createMQConnector",
        side_effect=pseudocreateMQConnector,
    )
    @mock.patch(
        "DIRAC.Resources.MessageQueue.MQCommunication.getMQParamsFromCS",
        side_effect=pseudoCS,
    ...
    def test_success(
        self, mock_getMQParamsFromCS, mock_createMQConnector, mock_reconnect
    ):
        result = createProducer(mqURI="mardirac3.in2p3.fr::Queue::test1")
        self.assertTrue(result["OK"])
        producer = result["Value"]
