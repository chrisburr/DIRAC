...
class testDB(TestSystemLoggingDBTestCase):
    def test_addAndRemove(self):
        ...
        message = tupleToMessage(
            (systemName, level, time, msgTest, variableText, frameInfo, subSystemName)
        )
        site = "somewehere"
        ...
        for k in xrange(records):
            result = db.insertMessage(
                message, site, nodeFQDN, userDN, userGroup, remoteAddress
            )
        ...
        result = db._queryDB(
            showFieldList=["VariableText", "SiteName"],
            count=True,
        ...
        result = db.insertMessage(
            message, longSite, nodeFQDN, userDN, userGroup, remoteAddress
        )
