...
class testDB(ProxyDBTestCase):
    ...
    def test_getRemoveProxy(self):
        ...
        self.assertTrue(
            bool(db._query(cmd)["Value"][0][0] == 0), "GetProxy method didn't delete the last proxy."
        )
        ...
        result = db.getProxy(
            "/C=DN/O=DIRACCA/OU=None/CN=user_ca/emailAddress=user_ca@diracgrid.org", "group_1", 1800
        )
        self.assertTrue(result["OK"], "\n" + result.get("Message", "Error message is absent."))
        ...
        cmd = "INSERT INTO ProxyDB_Proxies(UserName, UserDN, UserGroup, Pem, ExpirationTime) VALUES "
        cmd += '("user", "%s", "%s", "%s", TIMESTAMPADD(SECOND, 43200, UTC_TIMESTAMP()))' % (
            dn,
            group,
            proxyStr,
        )
