...
class ProxyDBTestCase(unittest.TestCase):
    ...
    def createProxy(self, userName, group, time, vo=None, role=None):
        ...
        if not vo:
            ...
            result = chain.generateProxyToFile(
                self.proxyPath, time * 3600, diracGroup=group
            )
            if not result["OK"]:
                return result
        else:
        ...
        with open(cls.caConfigFile, "r") as caCFG:
            for line in caCFG:
                if re.findall("=", re.sub(r"#.*", "", line)):
                    field = (
                        re.sub(r"#.*", "", line).replace(" ", "").rstrip().split("=")[0]
                    )
                    line = (
                        "dir = %s #PUT THE RIGHT DIR HERE!\n" % (cls.caPath)
                        if field == "dir"
                        else line
                    )
                    val = (
                        re.sub(r"#.*", "", line).replace(" ", "").rstrip().split("=")[1]
                    )
                    if field in fields:
                        for i in fields:
                            if cfgDict.get(i):
    ...
    def setUp(self):
        ...
        db._update(
            'DELETE FROM ProxyDB_Proxies WHERE UserName IN ("user_ca", "user", "user_1", "user_2", "user_3")'
        )
        db._update(
...
class testDB(ProxyDBTestCase):
    ...
    def test_getUsers(self):
        ...
        field = (
            '("%%s", "/C=CC/O=DN/O=DIRAC/CN=%%s", %%s "PEM", TIMESTAMPADD(SECOND, %%s, UTC_TIMESTAMP()))%s'
            % ""
        )
        # Fill table for test
        ...
        for table, values, fields in [
            ...
            result = db._update(
                "INSERT INTO %s%s VALUES %s ;" % (table, fields, ", ".join(values))
            )
            self.assertTrue(
        ...
        for user, exp, expect, log in [
            ...
            self.assertEqual(
                set(expect),
                set(usersList),
                str(usersList) + ", when expected " + str(expect),
            )
    ...
    def test_getRemoveProxy(self):
        ...
        result = db.getProxiesContent(
            {"UserName": ["user_ca", "user", "user_1" "user_2", "user_3"]}, {}
        )
        self.assertTrue(
            result["OK"], "\n" + result.get("Message", "Error message is absent.")
        )
        self.assertTrue(
            bool(int(result["Value"]["TotalRecords"]) == 0), "In DB present proxies."
        )
        ...
        self.assertTrue(
            ...
            "GetProxy method didn't delete the last proxy.",
        )
        ...
        self.assertTrue(
            bool(int(result["Value"]["TotalRecords"]) == 1),
            "Generated proxy must be one.",
        )
        ...
        for user, dn, group, vo, time, res, log in [
            ...
            if vo:
                self.assertTrue(
                    bool(chain.isVOMS().get("Value")),
                    "Cannot create proxy with VOMS extension",
                )
            ...
            result = chain.generateChainFromRequestString(
                resDict["request"], time * 3500
            )
            self.assertTrue(
                result["OK"], "\n" + result.get("Message", "Error message is absent.")
            )
            if not chain.isVOMS().get("Value") and vo:
                gLogger.info(
                    "voms-proxy-fake command not working as expected, so proxy have no VOMS extention"
                )
                res = not res
            ...
            self.assertTrue(
                bool(db._query(cmd)["Value"][0][0] == (res and group and 1) or 0),
                "ProxyDB_Proxies must " + (res and "contain proxy" or "be empty"),
            )
            cmd = (
                'SELECT COUNT( * ) FROM ProxyDB_CleanProxies WHERE UserName="%s"' % user
            )
            self.assertTrue(
                bool(db._query(cmd)["Value"][0][0] == (res and not group and 1) or 0),
        ...
        self.assertTrue(
            ...
            "ProxyDB_CleanProxies must contain proxy",
        )
        ...
        for res, group, reqtime, log in [
            ...
            if res:
                ...
                self.assertEqual(
                    "group_1",
                    result["Value"],
                    "Group must be group_1, not " + result["Value"],
                )
            else:
                gLogger.info("Msg: %s" % (result["Message"]))
        ...
        gLogger.info("* Check that DB is clean..")
        result = db.deleteProxy(
            "/C=CC/O=DN/O=DIRAC/CN=user", proxyProvider="Certificate"
        )
        self.assertTrue(
            result["OK"], "\n" + result.get("Message", "Error message is absent.")
        )
        result = db.getProxiesContent(
            {"UserName": ["user_ca", "user", "user_2", "user_3"]}, {}
        )
        self.assertTrue(
            result["OK"], "\n" + result.get("Message", "Error message is absent.")
        )
        ...
        result = ca._forceGenerateProxyForDN(
            "/C=CC/O=DN/O=DIRAC/CN=user", 12 * 3600, group="group_1"
        )
        self.assertTrue(
            result["OK"], "\n" + result.get("Message", "Error message is absent.")
        )
        ...
        cmd = "INSERT INTO ProxyDB_Proxies(UserName, UserDN, UserGroup, Pem, ExpirationTime) VALUES "
        cmd += (
            '("user", "%s", "%s", "%s", TIMESTAMPADD(SECOND, 43200, UTC_TIMESTAMP()))'
            % (dn, group, proxyStr)
        )
        result = db._update(cmd)
        self.assertTrue(
        ...
        self.assertEqual(
            "group_1", result["Value"], "Group must be group_1, not " + result["Value"]
        )
        ...
        for vomsuser in ["user", "user_1"]:
            ...
            cmd = "INSERT INTO ProxyDB_Proxies(UserName, UserDN, UserGroup, Pem, ExpirationTime) VALUES "
            cmd += '("%s", "/C=CC/O=DN/O=DIRAC/CN=%s", "group_1", "%s", ' % (
                vomsuser,
                vomsuser,
                proxyStr,
            )
        ...
        for table, user, count in [
            ("ProxyDB_Proxies", "user", 1),
            ("ProxyDB_CleanProxies", "user_ca", 1),
        ]:
