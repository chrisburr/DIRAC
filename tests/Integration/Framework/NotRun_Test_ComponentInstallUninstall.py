...
class ComponentInstallationChain(TestComponentInstallation):
    def testComponent(self):
        ...
        if cfg.isSection(
            "Systems/Framework/" + self.frameworkSetup + "/Services/Notification/"
        ) and cfg.isOption(
        ...
        cfg = self.csClient.getCurrentCFG()["Value"]
        if cfg.isSection(
            "Systems/Framework/" + self.frameworkSetup + "/Services/Notification2/"
        ) and cfg.isOption(
        ...
        if not service1Present and not service2Present:
            ...
            self.assertTrue(
                ...
                and not cfg.isSection(
                    "Systems/Framework/"
                    + self.frameworkSetup
                    + "/Services/Notification2/"
                )
                and not cfg.isOption(
                    "Systems/Framework/" + self.frameworkSetup + "/URLs/Notification"
                )
            )
    ...
    def testDatabase(self):
        ...
        self.assertTrue(
            cfg.isSection(
                "Systems/DataManagement/" + self.frameworkSetup + "/Databases/FTS3DB/"
            )
        )
...
if __name__ == "__main__":
    ...
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(ComponentInstallationChain)
    )
