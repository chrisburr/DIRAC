...
class TransformationClientChain(TestClientTransformationTestCase):
    ...
    def test_addTasksAndFiles(self):
        ...
        # add tasks - with lfns
        res = self.transClient.addTaskForTransformation(
            transID, ["/aa/lfn.1.txt", "/aa/lfn.2.txt"]
        )
        ...
        res = self.transClient.addFilesToTransformation(
            transID,
            ["/aa/lfn.1.txt", "/aa/lfn.2.txt", "/aa/lfn.3.txt", "/aa/lfn.4.txt"],
        )
        self.assertTrue(res["OK"])
        ...
        self.assertTrue(res["OK"])
        res = self.transClient.addTaskForTransformation(
            transID, ["/aa/lfn.3.txt", "/aa/lfn.4.txt"]
        )
        self.assertTrue(res["OK"])
        res = self.transClient.getTransformationTasks({"TransformationID": transID})
        self.assertTrue(res["OK"])
    ...
    def test_mix(self):
        ...
        res = self.transClient.setTransformationParameter(
            transID, "aParamName", "aParamValue"
        )
        self.assertTrue(res["OK"])
        ...
        res = self.transClient.getTransformationFiles(
            {"TransformationID": transID, "LFN": lfns}
        )
        self.assertTrue(res["OK"])
        for f in res["Value"]:
        ...
        res = self.transClient.setFileStatusForTransformation(
            transID, "Unused", lfns, force=True
        )
        self.assertTrue(res["OK"])
        res = self.transClient.getTransformationFiles(
            {"TransformationID": transID, "LFN": lfns}
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(
        TestClientTransformationTestCase
    )
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(TransformationClientChain)
    )
