...
class TestClientFTS3(unittest.TestCase):
    ...
    def test_01_operation(self):
        ...
        op = self.generateOperation(
            "Transfer", 3, ["Target1", "Target2"], sources=["Source1", "Source2"]
        )
        ...
        for ftsFile in op4.ftsFiles:
            if ftsFile.fileID % 2:
                ...
                self.assertTrue(
                    ftsFile.status
                    == FTS3File.FINAL_STATES[ftsFile.fileID % nbFinalStates]
                )
                self.assertTrue(ftsFile.error == "Tough luck")
    ...
    def test_05_cancelNotFoundJob(self):
        ...
        fileStatusDict = {
            file1ID: {"status": "Finished", "ftsGUID": None},
            file2ID: {"status": "Staging"},
        }
