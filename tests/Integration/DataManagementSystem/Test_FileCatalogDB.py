...
class FileCase(FileCatalogDBTestCase):
    def test_fileOperations(self):
        ...
        result = self.db.addFile(
            {testFile: {"PFN": "testfile", "SE": "testSE", "Size": 123, "GUID": "1000", "Checksum": "1"}},
            credDict,
        )
        self.assertTrue(
            result["OK"], "addFile failed when adding existing file with different parem %s" % result
        )
        self.assertTrue(
        ...
        result = self.db.addFile(
            {
                testFile
                + "2": {"PFN": "testfile", "SE": "testSE", "Size": 123, "GUID": "1000", "Checksum": "0"}
            },
            credDict,
        )
        ...
        self.assertTrue(
            result["OK"], "setFileStatus failed when setting existing status of existing file %s" % result
        )
        self.assertTrue(
            testFile in result["Value"]["Successful"],
...
class ReplicaCase(FileCatalogDBTestCase):
    def test_replicaOperations(self):
        ...
        self.assertTrue(
            result["OK"], "addReplica failed when adding Replica to non existing Replica %s" % result
        )
        self.assertTrue(
            nonExistingFile in result["Value"]["Failed"],
        ...
        self.assertTrue(
            result["OK"],
            "setReplicaStatus failed when setting existing status of existing Replica %s" % result,
        )
        self.assertTrue(
            testFile in result["Value"]["Successful"],
        ...
        self.assertTrue(
            result["OK"],
            "setReplicaStatus failed when setting non-existing status of existing Replica %s" % result,
        )
        self.assertTrue(
            testFile in result["Value"]["Failed"],
        ...
        self.assertTrue(
            result["OK"],
            "setReplicaStatus failed when setting existing status of non-existing Replica %s" % result,
        )
        self.assertTrue(
            testFile in result["Value"]["Failed"],
            "setReplicaStatus failed: %s should be in Failed (%s)" % (testFile, result),
        ...
        self.assertTrue(
            result["OK"],
            "setReplicaStatus failed when setting existing status of non-existing File %s" % result,
        )
        self.assertTrue(
            nonExistingFile in result["Value"]["Failed"],
        ...
        self.assertTrue(
            result["OK"],
            "getReplicaStatus failed when getting existing status of existing Replica %s" % result,
        )
        self.assertTrue(
            testFile in result["Value"]["Successful"],
        ...
        self.assertTrue(
            result["OK"], "getReplicaStatus failed when getting status of non existing File %s" % result
        )
        self.assertTrue(
            nonExistingFile in result["Value"]["Failed"],
        ...
        self.assertTrue(
            result["OK"], "removeReplica failed when removing replica of non existing File %s" % result
        )
        self.assertTrue(
            nonExistingFile in result["Value"]["Successful"],
...
class DirectoryUsageCase(FileCatalogDBTestCase):
    ...
    def getAndCompareDirectorySize(self, dirList, recursiveSum=True):
        ...
        self.assertTrue(
            retEquals,
            "Calc and table results different with recursiveSum %s: %s %s"
    ...
    def checkNonRecursiveDirectorySize(self, curDir):
        ...
        # should be equal to the recursive size of curdir
            ...
            self.assertEqual(
                rDirPhys.pop("TotalFiles", 0), nrDirPhys.pop("TotalFiles", 0) + physicalSizeTotalFiles
            )
            self.assertEqual(
                rDirPhys.pop("TotalSize", 0), nrDirPhys.pop("TotalSize", 0) + physicalSizeTotalSize
            )
    ...
    def test_directoryUsage(self):
        ...
        self.assertEqual(
            d1l, (2, f1Size + f2Size), "Unexpected size %s, expected %s" % (d1l, (2, f1Size + f2Size))
        )
        ...
        ret = self.db.addReplica(
            {f1: {"PFN": "f1se2", "SE": "se2"}, f2: {"PFN": "f1se3", "SE": "se3"}}, credDict
        )
        ...
        self.assertEqual(d1s1, (1, f1Size), "Unexpected size %s, expected %s" % (d1s1, (1, f1Size)))
        self.assertEqual(
            d1s2, (2, f1Size + f2Size), "Unexpected size %s, expected %s" % (d1s2, (2, f1Size + f2Size))
        )
