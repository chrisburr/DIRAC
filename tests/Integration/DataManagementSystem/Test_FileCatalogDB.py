...
class SECase(FileCatalogDBTestCase):
    def test_seOperations(self):
        ...
        if isAdmin:
            ...
            ret = self.db.addSE(seName, credDict)
            self.assertEqual(
                ret["Value"], seId, "addSE failed when adding existing SE: %s" % ret
            )
            ...
            self.assertEqual(
                ret["OK"],
                False,
                "addSE should fail when adding new SE as non admin: %s" % ret,
            )

        # remove it
        ret = self.db.deleteSE(seName, credDict)
        self.assertEqual(
            ret["OK"], True if isAdmin else False, "deleteE failed %s" % ret
        )
...
class UserGroupCase(FileCatalogDBTestCase):
    def test_userOperations(self):
        ...
        self.assertEqual(
            result["OK"],
            expectedRes,
            "AddUser failed when adding new user: %s" % result,
        )
        ...
        self.assertEqual(
            result["OK"],
            expectedRes,
            "AddUser failed when adding existing user: %s" % result,
        )
        ...
        if isAdmin:
            ...
            self.assertEqual(
                testUser in result["Value"], expectedRes, "getUsers failed: %s" % result
            )
    ...
    def test_groupOperations(self):
        ...
        self.assertEqual(
            result["OK"],
            expectedRes,
            "AddGroup failed when adding new user: %s" % result,
        )
        result = self.db.addGroup(testGroup, credDict)
        self.assertEqual(
            result["OK"],
            expectedRes,
            "AddGroup failed when adding existing user: %s" % result,
        )
...
class FileCase(FileCatalogDBTestCase):
    def test_fileOperations(self):
        ...
        self.assertTrue(
            result["OK"],
            "addFile failed when adding existing file with same param %s" % result,
        )
        self.assertTrue(
        ...
        self.assertTrue(
            result["OK"],
            "addFile failed when adding existing file with different parem %s" % result,
        )
        self.assertTrue(
        ...
        self.assertTrue(
            result["OK"], "addFile failed when adding existing file %s" % result
        )
        self.assertTrue(
        ...
        self.assertTrue(
            testFile in result["Value"]["Failed"],
            "setFileStatus should have failed %s" % result,
        )
        ...
        self.assertTrue(
            testFile in result["Value"]["Successful"],
            "isFile : %s should be in Successful %s" % (testFile, result),
        )
        self.assertTrue(
            result["Value"]["Successful"][testFile],
            "isFile : %s should be seen as a file %s" % (testFile, result),
        )
        self.assertTrue(
        ...
        result = self.db.changePathOwner(
            {testFile: "toto", nonExistingFile: "tata"}, credDict
        )
        ...
        result = self.db.changePathGroup(
            {testFile: "toto", nonExistingFile: "tata"}, credDict
        )
        ...
        result = self.db.changePathMode(
            {testFile: 0o44, nonExistingFile: 0o44}, credDict
        )
        ...
        self.assertEqual(
            result["Value"]["Successful"][testFile],
            123,
            "getFileSize got incorrect file size %s" % result,
        )
        self.assertTrue(
            nonExistingFile in result["Value"]["Failed"],
        ...
        self.assertTrue(
            testFile in result["Value"]["Successful"],
            "removeFile : %s should be in Successful %s" % (testFile, result),
        )
        self.assertTrue(
            result["Value"]["Successful"][testFile],
            "removeFile : %s should be in True %s" % (testFile, result),
        )
        self.assertTrue(
...
class ReplicaCase(FileCatalogDBTestCase):
    def test_replicaOperations(self):
        ...
        # Adding new replica
        result = self.db.addReplica(
            {testFile: {"PFN": "testFile", "SE": "otherSE"}}, credDict
        )
        self.assertTrue(
            result["OK"], "addReplica failed when adding new Replica %s" % result
        )
        self.assertTrue(
            testFile in result["Value"]["Successful"],
            "addReplica failed when adding new Replica %s" % result,
        )
        ...
        result = self.db.addReplica(
            {nonExistingFile: {"PFN": "Idontexist", "SE": "otherSE"}}, credDict
        )
        self.assertTrue(
            result["OK"],
            "addReplica failed when adding Replica to non existing Replica %s" % result,
        )
        self.assertTrue(
            nonExistingFile in result["Value"]["Failed"],
        ...
        result = self.db.setReplicaStatus(
            {testFile: {"Status": "Trash", "SE": "otherSE"}}, credDict
        )
        self.assertTrue(
            result["OK"],
        ...
        result = self.db.getReplicas(
            [testFile, nonExistingFile], allStatus=True, credDict=credDict
        )
        ...
        self.assertTrue(
            result["OK"],
            "removeReplica failed when removing master Replica %s" % result,
        )
        self.assertTrue(
            testFile in result["Value"]["Successful"],
            "removeReplica failed when removing master Replica %s" % result,
        ...
        self.assertTrue(
            result["OK"],
            "removeReplica failed when removing non existing Replica %s" % result,
        )
        self.assertTrue(
            testFile in result["Value"]["Successful"],
            "removeReplica failed when removing new Replica %s" % result,
        )

        # removing non existing replica of non existing file
        result = self.db.removeReplica(
            {nonExistingFile: {"SE": "nonExistingSe3"}}, credDict
        )
        self.assertTrue(
            result["OK"],
        ...
        self.assertTrue(
            result["OK"], "removeReplica failed when removing last Replica %s" % result
        )
        self.assertTrue(
            testFile in result["Value"]["Successful"],
            "removeReplica failed when removing last Replica %s" % result,
        )
...
class DirectoryCase(FileCatalogDBTestCase):
    def test_directoryOperations(self):
        ...
        self.assertTrue(
            result["OK"], "addDirectory failed when adding new directory %s" % result
        )
        ...
        self.assertTrue(
            result["OK"],
            "addDirectory failed when adding existing directory %s" % result,
        )
        self.assertTrue(
        ...
        self.assertTrue(
            testDir in result["Value"]["Successful"],
            "isDirectory : %s should be in Successful %s" % (testDir, result),
        )
        self.assertTrue(
        ...
        self.assertTrue(
            ...
            % (nonExistingDir, result),
        )

        result = self.db.getDirectorySize(
            [testDir, nonExistingDir], False, False, credDict
        )
        ...
        result = self.db.getDirectorySize(
            [testDir, nonExistingDir], False, True, credDict
        )
        ...
        self.assertEqual(
            result["Value"]["Successful"][parentDir]["SubDirs"].keys(),
            [testDir],
        ...
        self.assertEqual(
            result["Value"]["Successful"][testDir]["Files"].keys(),
            [testFile.split("/")[-1]],
...
class DirectoryUsageCase(FileCatalogDBTestCase):
    ...
    def getAndCompareDirectorySize(self, dirList):
        ...
        self.assertTrue(
            retEquals, "Calc and table results different %s %s" % (succTable, succCalc)
        )
    ...
    def test_directoryUsage(self):
        ...
        self.assertEqual(
            d1s1, (1, f1Size), "Unexpected size %s, expected %s" % (d1s1, (1, f1Size))
        )
        self.assertEqual(
            d1s2, (1, f2Size), "Unexpected size %s, expected %s" % (d1s2, (1, f2Size))
        )
        self.assertEqual(
            d1l,
            (2, f1Size + f2Size),
            "Unexpected size %s, expected %s" % (d1l, (2, f1Size + f2Size)),
        )

        ret = self.db.addReplica(
            {f1: {"PFN": "f1se2", "SE": "se2"}, f2: {"PFN": "f1se3", "SE": "se3"}},
            credDict,
        )
        ...
        self.assertEqual(
            d1s2,
            (2, f1Size + f2Size),
            "Unexpected size %s, expected %s" % (d1s2, (2, f1Size + f2Size)),
        )
        self.assertEqual(
            d1s3, (1, f2Size), "Unexpected size %s, expected %s" % (d1s3, (1, f2Size))
        )
        self.assertEqual(
            d1l,
            (2, f1Size + f2Size),
        ...
        self.assertEqual(
            d1s1, (0, 0), "Unexpected size %s, expected %s" % (d1s1, (0, 0))
        )
        self.assertEqual(
            d1s2, (1, f2Size), "Unexpected size %s, expected %s" % (d1s2, (1, f2Size))
        )
        ...
        self.assertEqual(
            d1l, (1, f2Size), "Unexpected size %s, expected %s" % (d1l, (1, f2Size))
        )
        ...
        self.assertEqual(
            d1s2, (0, 0), "Unexpected size %s, expected %s" % (d1s2, (0, 0))
        )
        self.assertEqual(
            d1s3, (1, f2Size), "Unexpected size %s, expected %s" % (d1s3, (1, f2Size))
        )
        ...
        self.assertEqual(
            d2s3, (1, f3Size), "Unexpected size %s, expected %s" % (d2s3, (1, f3Size))
        )
        self.assertEqual(
            d1l,
            (2, f1Size + f2Size),
        ...
        self.assertEqual(
            d2l, (1, f3Size), "Unexpected size %s, expected %s" % (d2l, (1, f3Size))
        )
        ...
        self.assertEqual(
            d1s3, (0, 0), "Unexpected size %s, expected %s" % (d1s3, (0, 0))
        )
        self.assertEqual(
            d2s3, (0, 0), "Unexpected size %s, expected %s" % (d2s3, (0, 0))
        )
        # This one is silly... there are no replicas of f1, but since the file is still there,
        # the logical size does not change
        self.assertEqual(
...
if __name__ == "__main__":
    ...
    for setup in all_combinations:
        ...
        suite.addTest(
            unittest.defaultTestLoader.loadTestsFromTestCase(DirectoryUsageCase)
        )
