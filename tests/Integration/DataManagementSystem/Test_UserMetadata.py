...
class testMetadata(TestUserMetadataBasicTestCase):
    ...
    def test_fileCatalogClient(self):
        try:
            ...
            self.fail(
                " FileCatalogClient('DataManagement/MultiVOFileCatalog') raised ExceptionType unexpectedly!"
            )
    ...
    def test_metaIndex(self):
        ...
        self.assertDictContainsSubset(
            {"MetaInt6": "INT"}, result["Value"]["FileMetaFields"]
        )
        self.assertDictContainsSubset(
            {"TestDirectory6": "INT"}, result["Value"]["DirectoryMetaFields"]
        )
