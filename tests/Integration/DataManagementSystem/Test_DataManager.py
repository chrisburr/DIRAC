...
def test_putAndRegisterReplicate(dm, tempFile):
    ...
    lfn = os.path.join(
        DESTINATION_PATH, "putAndRegisterReplicate/testFile.%s" % time.time()
    )
    diracSE = "SE-1"
    putRes = dm.putAndRegister(lfn, tempFile, diracSE)
    replicateRes = dm.replicateAndRegister(
        lfn, "SE-2"
    )  # ,sourceSE='',destPath='',localCache='')
    removeRes = dm.removeFile(lfn)
...
def test_putAndRegisterGetReplicaMetadata(dm, tempFile):
    ...
    lfn = os.path.join(
        DESTINATION_PATH, "putAndRegisterGetReplicaMetadata/testFile.%s" % time.time()
    )
    diracSE = "SE-1"
    putRes = dm.putAndRegister(lfn, tempFile, diracSE)
...
def test_putAndRegsiterGetAccessUrl(dm, tempFile):
    ...
    lfn = os.path.join(
        DESTINATION_PATH, "putAndRegisterGetAccessUrl/testFile.%s" % time.time()
    )
    diracSE = "SE-1"
    putRes = dm.putAndRegister(lfn, tempFile, diracSE)
...
def test_putAndRegisterRemoveReplica(dm, tempFile):
    ...
    lfn = os.path.join(
        DESTINATION_PATH, "putAndRegisterRemoveReplica/testFile.%s" % time.time()
    )
    diracSE = "SE-1"
    putRes = dm.putAndRegister(lfn, tempFile, diracSE)
