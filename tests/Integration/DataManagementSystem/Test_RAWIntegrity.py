...
class RAWIntegrityDBTest(unittest.TestCase):
    ...
    def test_03_fileManipulation(self):
        ...
        testFile = {"LFN": "lfn", "PFN": "pfn", "Size": 123, "SE": "se", "GUID": "guid", "Checksum": "checksum"}

        # adding a file
        res = self.db.addFile(
            testFile["LFN"], testFile["PFN"], testFile["Size"], testFile["SE"], testFile["GUID"], testFile["Checksum"]
        )
        self.assertTrue(res["OK"], res)
    ...
    def test_04_webQueries(self):
        ...
        for i in range(1, 5):
            res = self.db.addFile("lfn%s" % i, "pfn%s" % i, 5 - i, "se%s" % (i % 2), "GUID%s" % i, "Checksum%s" % i)
            self.assertTrue(res["OK"], res)
...
class RAWIntegrityAgentTest(unittest.TestCase):
    ...
    def setUp(self, _mockFC):  # pylint: disable=arguments-differ
        ...
        self.failMetadata = RAWIntegrityAgentTest.dbFile("/lhcb/fail_seMetadata/failedMetadata.txt", size=2)
        ...
        self.onSEMetadata = RAWIntegrityAgentTest.dbFile("/lhcb/allGood/ButOnSEMetadata.txt", size=3, se="se2")
        ...
        self.errorAddFile = RAWIntegrityAgentTest.dbFile("/lhcb/error_addFile/errorAddFile.txt", size=8, se="se3")
        # All fine but on se3 with errorAddFile -> goes to migrated
        self.onSeAddFile = RAWIntegrityAgentTest.dbFile("/lhcb/allGood/ButOnSEAddFile.txt", size=8, se="se3")
        # Cannot perform the removal -> goes to Registered
        self.failRemove = RAWIntegrityAgentTest.dbFile("/lhcb/removal/fail_removeFile/failRemove.txt", size=9)
        ...
        self.onSeRemove = RAWIntegrityAgentTest.dbFile("/lhcb/removal/allGood/ButOnSeRemove.txt", size=9, se="se4")
    ...
    def test_01_executeWithNoError(self):
        ...
        pbMetadataFiles = set([self.failMetadata, self.errorMetadata, self.onSEMetadata, self.badChecksum])
    ...
    def _analyseResults(self, allActiveFiles, pbMetadataFiles, pbRegisterFiles, pbRemoveFiles):
        ...
        migratedFiles = (allCopiedFiles | attemptsRegister | attemptsRemove) - (pbRegisterFiles | pbRemoveFiles)
        ...
        self.assertEqual(
            sorted(RAWIntegrityAgentTest.genericSE.successfulRemove), sorted([f.lfn for f in migratedFiles])
        )

        # There are also some removal failures
        self.assertEqual(sorted(RAWIntegrityAgentTest.genericSE.failedRemove), sorted([f.lfn for f in pbRemoveFiles]))

        # The attempts should be all the possible remove
        self.assertEqual(sorted(RAWIntegrityAgentTest.genericSE.removeCalls), sorted([f.lfn for f in attemptsRemove]))
        ...
        self.assertEqual(sorted(self.agent.fileCatalog.failedAdd), sorted([f.lfn for f in pbRegisterFiles]))

        self.assertEqual(sorted(self.agent.fileCatalog.successfullAdd), sorted([f.lfn for f in successfullRegister]))

        self.assertEqual(sorted(self.agent.fileCatalog.attemptsAdd), sorted([f.lfn for f in attemptsRegister]))
