...
class TestProductionRequestTestCaseChain(TestProductionRequestTestCase):
    ...
    def _run_mix_test(self, dumps_func):
        ...
        # add
        res = self.reqClient.createProductionRequest(
            {
                "ParentID": "",
                "MasterID": "",
        ...
        # Adding production
        res = self.reqClient.addProductionToRequest(
            {"ProductionID": 123, "RequestID": firstReq, "Used": 0, "BkEvents": 0}
        )
        self.assertTrue(res["OK"])
        ...
        res = self.reqClient.addProductionToRequest(
            {"ProductionID": 456, "RequestID": firstReq + 1, "Used": 1, "BkEvents": 0}
        )
        self.assertTrue(res["OK"])
