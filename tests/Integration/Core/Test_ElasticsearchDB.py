...
class ElasticTestChain(ElasticTestCase):
    ...
    def test_getDocTypes(self):
        ...
        if "_doc" in result["Value"]:
            ...
            self.assertEqual(
                list(result["Value"]["properties"]), [u"Color", u"timestamp", u"Product", u"quantity"]
            )
                        ...
                        "filter": [
                            {
                                "bool": {
                                    "must": [
                                        {"range": {"timestamp": {"gte": 1423497057518, "lte": 1423501337292}}}
                                    ]
                                }
                            }
                        ]
                        ...
                        "filter": [
                            {
                                "bool": {
                                    "must": [
                                        {"range": {"timestamp": {"gte": 1423399451544, "lte": 1423631917911}}}
                                    ]
                                }
                            }
                        ]
