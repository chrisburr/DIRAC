...
def test_chainWithParameter():
    ...
    result = tqDB.deleteTaskQueue(tq)
    assert result["OK"] is False  # This will fail because of the foreign key
    result = tqDB.deleteTaskQueueIfEmpty(
        tq
    )  # this won't delete anything, as both 123 and 125 are in
...
def test_chainWithSites():
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000}, numQueuesToGet=4
    )
    assert result["OK"] is True
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "centos7"}, numQueuesToGet=4
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    assert res == {tq_job1, tq_job2}
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Site": "DIRAC.Test.org"},
        numQueuesToGet=4,
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Site": "LCG.CERN.ch"}, numQueuesToGet=4
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    assert res == {tq_job2}
    ...
    result = tqDB.deleteTaskQueueIfEmpty(
        tq_job1
    )  # this won't delete anything, as 127 is in
    assert result["OK"] is True  # but still it won't fail
    assert result["Value"] is False
...
def test_chainWithPlatforms():
    ...
    # ubuntu
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "ubuntu"}, numQueuesToGet=4
    )
    assert result["OK"] is True
    ...
    # slc6
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "slc6"}, numQueuesToGet=4
    )
    assert result["OK"] is True
    ...
    # slc5
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "slc5"}, numQueuesToGet=4
    )
    assert result["OK"] is True
    ...
    # ANY platform
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "ANY"}, numQueuesToGet=5
    )
    assert result["OK"] is True
    ...
    # ANY platform
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "ANY"}, numQueuesToGet=6
    )
    assert result["OK"] is True
    # this should match whatever
    res = set([int(x[0]) for x in result["Value"]])
    ...
    # no platform at all
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000}, numQueuesToGet=6
    )
    assert result["OK"] is True
    # this should match whatever
    res = set([int(x[0]) for x in result["Value"]])
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "slc5"}, numQueuesToGet=6
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    assert res == {tq_job6}
    ...
    # slc6
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "slc6"}, numQueuesToGet=6
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    ...
    # slc5, slc6
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": ["slc5", "slc6"]},
        numQueuesToGet=6,
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    ...
    # slc5, slc6, ubuntu
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": ["slc5", "slc6", "ubuntu"]},
        numQueuesToGet=6,
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    ...
    # ANY platform
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "ANY"}, numQueuesToGet=7
    )
    assert result["OK"] is True
    # this should match whatever
    res = set([int(x[0]) for x in result["Value"]])
    ...
    # NO platform
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000}, numQueuesToGet=7
    )
    assert result["OK"] is True
    # this should match whatever
    assert int(result["Value"][0][0]) in [
        tq_job1,
        tq_job2,
        tq_job3,
        tq_job4,
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "slc5"}, numQueuesToGet=7
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    ...
    # slc6
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "slc6"}, numQueuesToGet=7
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    ...
    # centos8 (> centos7)
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "centos8"}, numQueuesToGet=7
    )
    assert result["OK"] is True
...
def test_chainWithTags():
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Tag": "ANY"}, numQueuesToGet=6
    )
    assert result["OK"] is True
    # this should match whatever
    res = set([int(x[0]) for x in result["Value"]])
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Tag": ["MultiProcessor", "aNy"]},
        numQueuesToGet=6,
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000}, numQueuesToGet=5
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    assert res == {tq_job5}
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Tag": ""}, numQueuesToGet=5
    )
    assert result["OK"] is True
    res = set([int(x[0]) for x in result["Value"]])
    assert res == {tq_job5}
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Tag": "MultiProcessor"}, numQueuesToGet=4
    )
    assert result["OK"] is True
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Tag": ["MultiProcessor", "GPU"]},
        numQueuesToGet=4,
    )
    assert result["OK"] is True
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "RequiredTag": "MultiProcessor"},
        numQueuesToGet=4,
    )
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Tag": "4Processors"}, numQueuesToGet=4
    )
    assert result["OK"] is True
...
def test_chainWithTagsAndPlatforms():
    ...
    # No Tag, Platform = "ANY"
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "ANY"}, numQueuesToGet=4
    )
    assert result["OK"] is True
    ...
    result = tqDB.matchAndGetTaskQueue(
        {"Setup": "aSetup", "CPUTime": 50000, "Platform": "ANY", "Tag": "ANY"},
        numQueuesToGet=4,
    )
    assert result["OK"] is True
    # this should match whatever
