...
class WMSChain(TestWMSTestCase):
    def test_FullChain(self):
        ...
        self.assertEqual(
            res["Value"], res["JobID"], msg="Got %s, expected %s" % (str(res["Value"]), res["JobID"])
        )
        jobID = res["JobID"]
        jobID = res["Value"]
...
class JobMonitoring(TestWMSTestCase):
    def test_JobStateUpdateAndJobMonitoring(self):
        ...
        self.assertEqual(
            res["Value"]["MinorStatus"], "MinorStatus", msg="Got %s" % str(res["Value"]["MinorStatus"])
        )
        self.assertEqual(
            res["Value"]["ApplicationStatus"],
            "app status",
...
class JobMonitoringMore(TestWMSTestCase):
    def test_JobStateUpdateAndJobMonitoringMultuple(self):
        ...
        self.assertTrue(res["OK"], res.get("Message"))
        self.assertTrue(sorted(res["Value"]) in [["Received"], sorted(["Received", "Waiting"])], res["Value"])
        res = jobMonitor.getMinorStates()
        self.assertTrue(res["OK"], res.get("Message"))
        self.assertTrue(
            sorted(res["Value"]) in [["Job accepted"], sorted(["Job accepted", "Job Rescheduled"])],
            res["Value"],
        )
        self.assertTrue(res["OK"], res.get("Message"))
        ...
        try:
            self.assertTrue(
                res["Value"].get("Received") + res["Value"].get("Waiting") >= int(len(lfnss) * len(types))
            )
        except TypeError:
            pass
