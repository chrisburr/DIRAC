...
from DIRAC.WorkloadManagementSystem.Client.WMSAdministratorClient import (
...
def parametricJob():
    ...
    job.setExecutable(
        exeScriptLocation,
        arguments=": testing %(args)s %(iargs)s",
        logFile="helloWorld_%n.log",
    )
    return job
...
class TestWMSTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        jca = JobCleaningAgent(
            "WorkloadManagement/JobCleaningAgent", "WorkloadManagement/JobCleaningAgent"
        )
        jca.initialize()
...
class WMSChain(TestWMSTestCase):
    def test_FullChain(self):
        ...
        self.assertTrue(res["OK"], res.get("Message"))
        self.assertTrue(
            isinstance(res["Value"], int), msg="Got %s" % type(res["Value"])
        )
        self.assertEqual(
            res["Value"],
            res["JobID"],
        ...
        # updating the status
        res = jobStateUpdate.setJobStatus(
            jobID, "Running", "Executing Minchiapp", "source"
        )
        self.assertTrue(res["OK"], res.get("Message"))
        ...
        self.assertEqual(
            res["Value"],
            {jobID: {"MinorStatus": "Job Rescheduled", "JobID": jobID}},
        ...
        self.assertEqual(
            res["Value"],
            {jobID: {"ApplicationStatus": "Unknown", "JobID": jobID}},
            msg="Got %s" % str(res["Value"]),
        )
    ...
    def test_ParametricChain(self):
        ...
        self.assertEqual(
            set(jobNames), set(["parametric_helloWorld_%s" % nJob for nJob in range(3)])
        )
...
class JobMonitoring(TestWMSTestCase):
    def test_JobStateUpdateAndJobMonitoring(self):
        ...
        res = jobStateUpdate.setJobStatus(jobID, "Matched", "matching", "source")
        self.assertTrue(res["OK"], res.get("Message"))
        res = jobStateUpdate.setJobParameters(
            jobID, [("par1", "par1Value"), ("par2", "par2Value")]
        )
        time.sleep(5)
        self.assertTrue(res["OK"], res.get("Message"))
        ...
        self.assertTrue(res["OK"], res.get("Message"))
        self.assertEqual(
            res["Value"], {"par1": "par1Value"}, msg="Got %s" % str(res["Value"])
        )
        ...
        self.assertTrue(res["OK"], res.get("Message"))
        self.assertEqual(
            res["Value"]["Status"], "Done", msg="Got %s" % str(res["Value"]["Status"])
        )
        self.assertEqual(
            res["Value"]["MinorStatus"],
            "MinorStatus",
...
class JobMonitoringMore(TestWMSTestCase):
    def test_JobStateUpdateAndJobMonitoringMultuple(self):
        ...
        self.assertTrue(res["OK"], res.get("Message"))
        self.assertEqual(
            sorted(res["Value"]), sorted(["Unknown"]), msg="Got %s" % str(res["Value"])
        )
        ...
        res = jobMonitor.getJobGroups(
            None, datetime.datetime.utcnow() - datetime.timedelta(days=365)
        )
        self.assertTrue(res["OK"], res.get("Message"))
        ...
        self.assertTrue(res["OK"], res.get("Message"))
        self.assertTrue(
            sorted(res["Value"]) in [["Received"], sorted(["Received", "Waiting"])]
        )
        res = jobMonitor.getMinorStates()
        self.assertTrue(res["OK"], res.get("Message"))
        self.assertTrue(
            sorted(res["Value"])
            in [["Job accepted"], sorted(["Job accepted", "Job Rescheduled"])]
        )
        self.assertTrue(res["OK"], res.get("Message"))
