...
from DIRAC.tests.Integration.WorkloadManagementSystem.Test_Client_WMS import (
    helloWorldJob,
    createFile,
)
...
def test_MySQLandES_jobParameters():
    ...
    res = jobStateUpdateClient.setJobParameter(
        jobID, "ParName-fromMySQL", "ParValue-fromMySQL"
    )
    assert res["OK"], res["Message"]

    res = jobMonitoringClient.getJobParameter(
        jobID, "ParName-fromMySQL"
    )  # This will be looked up in MySQL only
    assert res["OK"], res["Message"]
    assert res["Value"] == {"ParName-fromMySQL": "ParValue-fromMySQL"}, res["Value"]

    res = jobMonitoringClient.getJobParameters(
        jobID
    )  # This will be looked up in MySQL only
    assert res["OK"], res["Message"]
    assert isinstance(res["Value"], dict), res["Value"]
    assert res["Value"] == {jobID: {"ParName-fromMySQL": "ParValue-fromMySQL"}}, res[
        "Value"
    ]
    ...
    # Now we insert (in ES)
    res = jobStateUpdateClient.setJobParameter(
        jobID, "ParName-fromES", "ParValue-fromES"
    )
    time.sleep(2)  # sleep to give time to ES to index
    assert res["OK"], res["Message"]

    res = jobMonitoringClient.getJobParameter(
        jobID, "ParName-fromES"
    )  # This will be in ES
    assert res["OK"], res["Message"]
    ...
    res = jobMonitoringClient.getJobAttributes(
        jobID
    )  # these will still be all in MySQL
    assert res["OK"], res["Message"]
