...
def test_insertAndRemoveJobIntoDB():
    ...
    res = jobDB.insertNewJobIntoDB(
        jdl, "owner", "/DN/OF/owner", "ownerGroup", "someSetup"
    )
...
def test_getCounters():
    ...
    res = jobDB.getCounters(
        "Jobs", ["Status", "MinorStatus"], {}, "2007-04-22 00:00:00"
    )
    assert res["OK"] is True, res["Message"]
...
def test_heartBeatLogging():
    ...
    res = jobDB.removeInfoFromHeartBeatLogging(
        status="Done", delTime=delTime, maxLines=100
    )
    assert res["OK"] is True, res["Message"]
