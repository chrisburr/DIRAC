...
class ReqDBMix(ReqDBTestCase):
    def test01Stress(self):
        ...
        for i in xrange(self.stressRequests):
            ...
            self.assertEqual(
                put["OK"], True, put["Message"] if "Message" in put else "OK"
            )
        ...
        for reqID in reqIDs:
            ...
            if "Message" in get:
                print(get["Message"])
            self.assertEqual(
                get["OK"], True, get["Message"] if "Message" in get else "OK"
            )
        ...
        for reqID in reqIDs:
            ...
            self.assertEqual(
                delete["OK"], True, delete["Message"] if "Message" in delete else "OK"
            )
    ...
    def test02Scheduled(self):
        ...
        self.assertEqual(
            peek["OK"], True, peek["Message"] if "Message" in peek else "OK"
        )
        ...
        self.assertEqual(
            getFTS["Value"].RequestName, "FTSTest", "wrong request selected"
        )
    ...
    def test03Dirty(self):
        ...
        put = db.putRequest(r)
        self.assertEqual(
            put["OK"], True, "1. putRequest failed: %s" % put.get("Message", "")
        )
        reqID = put["Value"]

        r = db.getRequest(reqID)
        self.assertEqual(
            r["OK"], True, "1. getRequest failed: %s" % r.get("Message", "")
        )
        r = r["Value"]
        ...
        put = db.putRequest(r)
        self.assertEqual(
            put["OK"], True, "2. putRequest failed: %s" % put.get("Message", "")
        )
        reqID = put["Value"]

        r = db.getRequest(reqID)
        self.assertEqual(
            r["OK"], True, "2. getRequest failed: %s" % r.get("Message", "")
        )
        ...
        put = db.putRequest(r)
        self.assertEqual(
            put["OK"], True, "3. putRequest failed: %s" % put.get("Message", "")
        )
        reqID = put["Value"]

        r = db.getRequest(reqID)
        self.assertEqual(
            r["OK"], True, "3. getRequest failed: %s" % r.get("Message", "")
        )
        r = r["Value"]
