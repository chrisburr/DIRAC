...
def checkAgentOptions(
    getOptionMock, systemName, agentName, ignoreOptions=None, extension="DIRAC"
):
    ...
    configFilePath = os.path.join(
        os.path.dirname(os.path.dirname(DIRAC.__file__)),
        extension,
        systemName,
        "ConfigTemplate.cfg",
    )
    ...
    for option, value in optionsDict.iteritems():
        if any(ignoreOp in option for ignoreOp in ignoreOptions):
            LOG.info(
                "From Agent: ignoring option %r with value %r, (%s)",
                option,
                value,
                type(value),
            )
            continue
        LOG.info(
            "Looking for call to option %r with value %r, (%s)",
            option,
            value,
            type(value),
        )
    ...
    for opCall in getOptionMock.call_args_list:
        ...
        if optionName in ignoreOptions:
            LOG.info(
                "From Template: ignoring option %r with %r", optionName, optionValue
            )
            continue
        ...
        assert (
            ...
            or [optionsDict[optionName]] == optionValue
        )
