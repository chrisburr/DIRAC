...
class SystemAdministratorClientCLI(CLI):
    ...
    def do_show(self, args):
        ...
        if option == "software":
                    ...
                    for system in rDict[compType]:
                        ...
                        for component in components:
                            ...
                            if rDict[compType][system][component]["Installed"]:
                                ...
                                record += [
                                    str(
                                        rDict[compType][system][component][
                                            "RunitStatus"
                                        ]
                                    )
                                ]
                                record += [
                                    str(rDict[compType][system][component]["Timeup"])
                                ]
                                record += [
                                    str(rDict[compType][system][component]["PID"])
                                ]
                ...
                for parameter in result["Value"].iteritems():
                    if parameter[0] == "Extension":
                        ...
                        for extension in extensions:
                            ...
                            records.append(
                                ["%sVersion" % extensionName, str(extensionVersion)]
                            )
                    else:
            ...
            if not result["OK"]:
                self._errMsg(
                    "Error retrieving the list of hosts: %s" % (result["Message"])
                )
            else:
                ...
                for element in hostList:
                    gLogger.notice(
                        "|"
                        + element["HostName"].center(32)
                        + "|"
                        + element["CPU"].center(34)
                        + "|"
                    )
                gLogger.notice(("-" * 69))
                gLogger.notice("")
            ...
            if len(argss) > 2:
                if argss[0] in ["service", "agent"]:
                    ...
                    compModule = argss[2]
                    client = SystemAdministratorClient(self.host, self.port)
                    result = client.getComponentDocumentation(
                        compType, compSystem, compModule
                    )
                    if result["OK"]:
            ...
            if len(argss) > 1:
                ...
                if size:
                    ...
                    result = client.getDataForAGivenPeriod(
                        host, component, initialDate, endingDate
                    )
    ...
    def getErrors(self, argss):
        ...
        if not result["OK"]:
            ...
            for cname in result["Value"]:
                ...
                records.append(
                    [system, component, str(errors_1), str(errors_24), lastError]
                )
            records.sort()
            printTable(fields, records)
    ...
    def getInstallations(self, argss):
        ...
        client = ComponentMonitoringClient()
        result = client.getInstallations(
            installationFilter, componentFilter, hostFilter, True
        )
        if not result["OK"]:
            self._errMsg(
                "Could not retrieve the installations: %s" % (result["Message"])
            )
            installations = None
        else:
        ...
        if installations:
            ...
            for i, installation in enumerate(installations):
                ...
                if installation["UnInstallationTime"]:
                    uninstalledOn = installation["UnInstallationTime"].strftime(
                        "%d-%m-%Y %H:%M"
                    )
                    isInstalled = "No"
                else:
                ...
                if display == "table":
                    ...
                    gLogger.notice(
                        "Host: ".rjust(20) + installation["Host"]["HostName"]
                    )
                    gLogger.notice("Name: ".rjust(20) + installation["Instance"])
                    gLogger.notice(
                        "Module: ".rjust(20) + installation["Component"]["Module"]
                    )
                    gLogger.notice(
                        "System: ".rjust(20) + installation["Component"]["System"]
                    )
                    gLogger.notice(
                        "Type: ".rjust(20) + installation["Component"]["Type"]
                    )
                    gLogger.notice(
                        "Installed on: ".rjust(20)
                        + installation["InstallationTime"].strftime("%d-%m-%Y %H:%M")
                    )
                    if installedBy != "":
    ...
    def do_install(self, args):
        ...
        if option == "db":
            ...
            if not instance:
                ...
                self._errMsg(
                    "\tAdd new instance with 'add instance %s <instance_name>'" % system
                )
                return
            ...
            if dbType == "MySQL":
                if not gComponentInstaller.mysqlPassword:
                    gComponentInstaller.mysqlPassword = "LocalConfig"
                gComponentInstaller.getMySQLPasswords()
                result = client.installDatabase(
                    database, gComponentInstaller.mysqlRootPwd
                )
                if not result["OK"]:
                    self._errMsg(result["Message"])
                    return
            ...
            result = gComponentInstaller.addDatabaseOptionsToCS(
                gConfig, system, database, hostSetup, overwrite=True
            )
            if not result["OK"]:
                self._errMsg(result["Message"])
                return
            gLogger.notice(
                "Database %s from %s/%s installed successfully"
        ...
        # DIRAC components
            ...
            gLogger.notice(
                "%s %s_%s is installed, runit status: %s"
            ...
            if component == "ComponentMonitoring":
                ...
                if not result["OK"]:
                    self._errMsg(
                        "Error registering installation into database: %s"
                        % result["Message"]
                    )
                    return
    ...
    def do_uninstall(self, args):
        ...
        if option == "db":
            ...
            result = MonitoringUtilities.monitorUninstallation(
                system, component, hostname=hostname, cpu=cpu
            )
            if not result["OK"]:
                self._errMsg(result["Message"])
                return
                ...
                if not result["Value"]:
                    ...
                    if not result["OK"]:
                        ...
                        for installation in host["Installations"]:
                            result = client.removeInstallations(
                        ...
                        if not result["OK"]:
                            ...
                            gLogger.notice(
                                "Host %s was successfully removed" % hostname
                            )
        else:
            if option == "-f":
                force = True
        ...
        if option != "mysql":
            ...
            if not result["OK"]:
                ...
                for comp in result["Value"]:
                    gLogger.notice(
                        (comp.rjust(32), ":", result["Value"][comp]["RunitStatus"])
                    )
        else:
    ...
    def do_update(self, args):
        ...
        if not result["OK"]:
            ...
            gLogger.notice(
                "Think of updating /Operations/<vo>/<setup>/Pilot/Versions section in the CS"
            )
    ...
    def do_add(self, args):
        ...
        if option == "instance" or option == "system":
            ...
            if not result["OK"]:
                self._errMsg(result["Message"])
            hostSetup = result["Value"]["Setup"]
            instanceName = gConfig.getValue(
                "/DIRAC/Setups/%s/%s" % (hostSetup, system), ""
            )
            if instanceName:
                if instanceName == instance:
                    gLogger.notice(
                        "System %s already has instance %s defined in %s Setup"
                        % (system, instance, hostSetup)
                    )
                else:
                    self._errMsg(
                        "System %s already has instance %s defined in %s Setup"
            ...
            if not result["OK"]:
                ...
                gLogger.notice(
                    "%s system instance %s added successfully" % (system, instance)
                )
        else:
            gLogger.notice("Unknown option:", option)
    ...
    def do_showall(self, args):
        ...
        if not resultAll["OK"]:
                    ...
                    for system in rDict[compType]:
                        ...
                        for component in components:
                            ...
                            if rDict[compType][system][component]["Installed"]:
                                ...
                                if (
                                    resultInfo["OK"]
                                    and host in resultInfo["Value"]
                                    and resultInfo["Value"][host]["OK"]
                                ):
            ...
            if silentHosts:
                print(
                    "\n %d out of %d hosts did not respond"
                    % (len(silentHosts), len(respondingHosts))
                )
