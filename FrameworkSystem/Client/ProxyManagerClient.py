...
class ProxyManagerClient(object):
    ...
    def uploadProxy(self, proxy=None, restrictLifeTime=0, rfcIfPossible=False):
        ...
        if isinstance(proxy, X509Chain):
            ...
            if not result["OK"]:
                return S_ERROR(
                    "Can't load %s: %s " % (proxyLocation, result["Message"])
                )
        ...
        retVal = chain.generateChainFromRequestString(
            reqDict["request"], lifetime=chainLifeTime, rfc=rfcIfPossible
        )
        if not retVal["OK"]:
            return retVal
        # Upload!
    ...
    def downloadProxy(
        ...
        if proxyToConnect:
            rpcClient = RPCClient(
                "Framework/ProxyManager", proxyChain=proxyToConnect, timeout=120
            )
        else:
            rpcClient = RPCClient("Framework/ProxyManager", timeout=120)
        if token:
    ...
    def getPilotProxyFromDIRACGroup(
        self, userDN, userGroup, requiredTimeLeft=43200, proxyToConnect=None
    ):
    ...
    def getPilotProxyFromVOMSGroup(
        self, userDN, vomsAttr, requiredTimeLeft=43200, proxyToConnect=None
    ):
        """Download a pilot proxy with VOMS extensions depending on the group
    ...
    def getPayloadProxyFromDIRACGroup(
        self, userDN, userGroup, requiredTimeLeft, token=None, proxyToConnect=None
    ):
    ...
    def getPayloadProxyFromVOMSGroup(
        self, userDN, vomsAttr, token, requiredTimeLeft, proxyToConnect=None
    ):
    ...
    def renewProxy(
        ...
        userDN = (
            proxyToRenewDict["chain"].getIssuerCert()["Value"].getSubjectDN()["Value"]
        )
