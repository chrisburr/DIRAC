...
class ComponentInstaller(object):
    ...
    def loadDiracCfg(self):
        ...
        self.instance = self.localCfg.getOption(
            cfgInstallPath("InstanceName"), self.setup
        )
        self.logLevel = self.localCfg.getOption(cfgInstallPath("LogLevel"), "INFO")
        self.linkedRootPath = self.localCfg.getOption(
            cfgInstallPath("RootPath"), rootPath
        )
        useVersionsDir = self.localCfg.getOption(
        ...
        self.instancePath = self.localCfg.getOption(
            cfgInstallPath("InstancePath"), rootPath
        )
        if useVersionsDir:
        ...
        self.runitDir = self.localCfg.getOption(
            cfgInstallPath("RunitDir"), self.runitDir
        )
        gLogger.verbose("Using Runit Dir at", self.runitDir)

        self.startDir = os.path.join(self.instancePath, "startup")
        self.startDir = self.localCfg.getOption(
            cfgInstallPath("StartupDir"), self.startDir
        )
        gLogger.verbose("Using Startup Dir at", self.startDir)

        self.controlDir = os.path.join(self.instancePath, "control")
        self.controlDir = self.localCfg.getOption(
            cfgInstallPath("ControlDir"), self.controlDir
        )
        gLogger.verbose("Using Control Dir at", self.controlDir)

        # Now some MySQL default values
        self.mysqlRootPwd = self.localCfg.getOption(
            cfgInstallPath("Database", "RootPwd"), self.mysqlRootPwd
        )
        ...
        self.mysqlUser = self.localCfg.getOption(
            cfgInstallPath("Database", "User"), self.mysqlUser
        )
        if self.mysqlUser:
        ...
        self.mysqlPassword = self.localCfg.getOption(
            cfgInstallPath("Database", "Password"), self.mysqlPassword
        )
        if self.mysqlPassword:
            gLogger.verbose(
                "Reading %s MySQL Password from local configuration " % self.mysqlUser
            )
        else:
        ...
        if self.mysqlPort:
            gLogger.verbose(
                "Using MySQL Port from local configuration ", self.mysqlPort
            )
        else:
            gLogger.warn("Using port '3306' as MySQL port")
            self.mysqlPort = 3306

        self.mysqlRootUser = self.localCfg.getOption(
            cfgInstallPath("Database", "RootUser"), ""
        )
        if self.mysqlRootUser:
            gLogger.verbose(
                "Using MySQL root user from local configuration ", self.mysqlRootUser
            )
        else:
            gLogger.warn("Using 'root' as root MySQL user")
            self.mysqlRootUser = "root"

        self.mysqlMode = self.localCfg.getOption(
            cfgInstallPath("Database", "MySQLMode"), ""
        )
        if self.mysqlMode:
            gLogger.verbose("Configuring MySQL server as %s" % self.mysqlMode)

        # Now some noSQL defaults
        self.noSQLHost = self.localCfg.getOption(
            cfgInstallPath("NoSQLDatabase", "Host"), ""
        )
        if self.noSQLHost:
        ...
        self.noSQLPort = self.localCfg.getOption(
            cfgInstallPath("NoSQLDatabase", "Port"), 0
        )
        if self.noSQLPort:
            gLogger.verbose(
                "Using NoSQL Port from local configuration ", self.noSQLPort
            )
        else:
            gLogger.warn("Using the default port 9200")
            self.noSQLPort = 9200

        self.noSQLUser = self.localCfg.getOption(
            cfgInstallPath("NoSQLDatabase", "User"), self.noSQLUser
        )
        if self.noSQLUser:
        ...
        self.noSQLPassword = self.localCfg.getOption(
            cfgInstallPath("NoSQLDatabase", "Password"), self.noSQLPassword
        )
        if self.noSQLPassword:
            gLogger.verbose(
                "Reading %s NoSQL Password from local configuration " % self.noSQLUser
            )
        else:
            gLogger.warn("NoSQL password not found")

        self.noSQLSSL = self.localCfg.getOption(
            cfgInstallPath("NoSQLDatabase", "SSL"), self.noSQLSSL
        )
        if self.noSQLSSL:
    ...
    def _getCentralCfg(self, installCfg):
        ...
        adminGroupName = self.localCfg.getOption(
            cfgInstallPath("AdminGroupName"), "dirac_admin"
        )
        hostDN = self.localCfg.getOption(cfgInstallPath("HostDN"), "")
        defaultGroupName = self.localCfg.getOption(
            cfgInstallPath("DefaultGroupName"), "dirac_user"
        )
        ...
        if adminUserName:
            if not (adminUserDN and adminUserEmail):
                gLogger.error(
                    "AdminUserName is given but DN or Mail is missing it will not be configured"
                )
            else:
                for section in [
                ...
                if centralCfg["Registry"].existsKey(
                    "DefaultGroup"
                ):  # pylint: disable=unsubscriptable-object,no-member
                    centralCfg["Registry"].deleteKey(
                        "DefaultGroup"
                    )  # pylint: disable=unsubscriptable-object,no-member
                centralCfg[
                    "Registry"
                ].addKey(  # pylint: disable=unsubscriptable-object,no-member
                ...
                centralCfg["Registry"]["Users"][
                    adminUserName
                ].addKey(  # pylint: disable=unsubscriptable-object
                    "DN", adminUserDN, ""
                )
                ...
                for group in [adminGroupName, defaultGroupName]:
                    ...
                    users = centralCfg["Registry"]["Groups"][
                        group
                    ].getOption(  # pylint: disable=unsubscriptable-object
                        "Users", []
                    )
                    ...
                    if not centralCfg["Registry"]["Groups"][
                        group
                    ].isOption(  # pylint: disable=unsubscriptable-object
                        "Properties"
                    ):
                        centralCfg["Registry"]["Groups"][
                            group
        ...
        if hostDN:
            ...
            if centralCfg["Registry"]["Hosts"][self.host].existsKey(
                "DN"
            ):  # pylint: disable=unsubscriptable-object
                centralCfg["Registry"]["Hosts"][self.host].deleteKey(
                    "DN"
                )  # pylint: disable=unsubscriptable-object
            centralCfg["Registry"]["Hosts"][self.host].addKey(
                "DN", hostDN, ""
            )  # pylint: disable=unsubscriptable-object
            if not centralCfg["Registry"]["Hosts"][
                self.host
            ].isOption(  # pylint: disable=unsubscriptable-object
                "Properties"
            ):
                centralCfg["Registry"]["Hosts"][
                    self.host
                ].addKey(  # pylint: disable=unsubscriptable-object
                    "Properties", "", ""
                )
            properties = centralCfg["Registry"]["Hosts"][
                self.host
            ].getOption(  # pylint: disable=unsubscriptable-object
                "Properties", []
            )
        ...
        # Operations
        if adminUserEmail:
            operationsCfg = self.__getCfg(
                cfgPath("Operations", "Defaults", "EMail"), "Production", adminUserEmail
            )
            centralCfg = centralCfg.mergeWith(operationsCfg)
            operationsCfg = self.__getCfg(
                cfgPath("Operations", "Defaults", "EMail"), "Logging", adminUserEmail
            )
            centralCfg = centralCfg.mergeWith(operationsCfg)
    ...
    def addOptionToDiracCfg(self, option, value):
        ...
        return S_ERROR(
            "Could not merge %s=%s with local configuration" % (option, value)
        )
    ...
    def removeComponentOptionsFromCS(self, system, component, mySetup=None):
            ...
            if remove:
                ...
                if not isRenamed and cType == "service":
                    result = self._removeOptionFromCS(
                        cfgPath("Systems", system, compInstance, "URLs", component)
                    )
                    if not result["OK"]:
        ...
        return S_OK(
            "Instances of this component still exist. It won't be completely removed"
        )
    ...
    def addDefaultOptionsToCS(
        ...
        addOptions = True
        componentSection = cfgPath(
            "Systems", system, compInstance, sectionName, component
        )
        if not overwrite:
            if gConfig_o:
    ...
    def addDefaultOptionsToComponentCfg(
        self, componentType, systemName, component, extensions
    ):
        """
        ...
        # Add the component options now
        result = self.getComponentCfg(
            componentType, system, component, compInstance, extensions
        )
        if not result["OK"]:
            return result
        compCfg = result["Value"]
    ...
    def addCfgToComponentCfg(self, componentType, systemName, component, cfg):
        ...
        newCfg.createNewSection(
            cfgPath(sectionPath, component), "Added by ComponentInstaller", cfg
        )
    ...
    def getComponentCfg(
        ...
        if addDefaultOptions:
            ...
            for ext in extensionsDIRAC + ["DIRAC"]:
                cfgTemplatePath = os.path.join(
                    rootPath, ext, "%sSystem" % system, "ConfigTemplate.cfg"
                )
        ...
        if componentType == "service":
            ...
            if port and self.host:
                ...
                failoverUrlsPath = cfgPath(
                    "Systems", system, compInstance, "FailoverURLs"
                )
                cfg.createNewSection(failoverUrlsPath)
                cfg.setOption(
                    cfgPath(urlsPath, component),
                    "dips://%s:%d/%s/%s" % (self.host, port, system, component),
                )

        return S_OK(cfg)

    def addDatabaseOptionsToCS(
        self, gConfig_o, systemName, dbName, mySetup=None, overwrite=False
    ):
        """
    ...
    def removeDatabaseOptionsFromCS(self, gConfig_o, system, dbName, mySetup=None):
        ...
        if not exists:
            result = self._removeSectionFromCS(
                cfgPath("Systems", system, compInstance, "Databases", dbName)
            )
            if not result["OK"]:
                return result
    ...
    def getAvailableSystems(self, extensions):
        ...
        for extension in extensions:
            ...
            for system in [
                os.path.basename(k).split("System")[0] for k in glob.glob(extensionPath)
            ]:
    ...
    def getSoftwareComponents(self, extensions):
        ...
        remainingTypes = [
            ...
            if cType not in ["service", "agent", "executor"]
        ]
        resultIndexes = {}
            ...
            for sys in systemList:
                    ...
                    for agent in agentList:
                        if os.path.splitext(agent)[1] == ".py":
                            ...
                            with io.open(agentFile, "rt") as afile:
                                body = afile.read()
                            if (
                                body.find("AgentModule") != -1
                                or body.find("OptimizerModule") != -1
                            ):
                ...
                try:
                    ...
                    for service in serviceList:
                        if (
                            service.find("Handler") != -1
                            and os.path.splitext(service)[1] == ".py"
                        ):
                            if system not in services:
                                services[system] = []
                            if (
                                system == "Configuration"
                                and service == "ConfigurationHandler.py"
                            ):
                                service = "ServerHandler.py"
                            services[system].append(
                                service.replace(".py", "").replace("Handler", "")
                            )
                except OSError:
                    pass
                try:
                ...
                for cType in remainingTypes:
                    try:
                        remainDir = os.path.join(
                            rootPath, extension, sys, cType.title()
                        )
                        ...
                        for remainder in remainList:
                            if os.path.splitext(remainder)[1] == ".py":
                                ...
                                remainders[cType][system].append(
                                    remainder.replace(".py", "")
                                )
                    except OSError:
                        pass
        ...
        try:
            if componentTupleList:
                ...
                for componentTuple in componentTupleList:
                    cList.extend(
                        glob.glob(os.path.join(self.startDir, "_".join(componentTuple)))
                    )
            else:
    ...
    def getComponentModule(self, system, component, compType):
        ...
        self.instance = gConfig.getValue(
            cfgPath("DIRAC", "Setups", self.setup, system), ""
        )
        if not self.instance:
            return S_OK(component)
        module = gConfig.getValue(
            cfgPath("Systems", system, self.instance, compType, component, "Module"), ""
        )
        if not module:
            module = component
        return S_OK(module)
                    ...
                    for component in softDict[compType][system]:
                        ...
                        try:
                            ...
                            if compDir in runitDict:
                                resultDict[compType][system][component][
                                    "RunitStatus"
                                ] = runitDict[compDir]["RunitStatus"]
                                resultDict[compType][system][component][
                                    "Timeup"
                                ] = runitDict[compDir]["Timeup"]
                                try:
                                    resultDict[compType][system][component][
                                        "PID"
                                    ] = int(runitDict[compDir]["PID"])
                                except ValueError:
                                    resultDict[compType][system][component]["PID"] = -1
                                try:
                                    resultDict[compType][system][component][
                                        "CPU"
                                    ] = float(runitDict[compDir]["CPU"])
                                except ValueError:
                                    resultDict[compType][system][component]["CPU"] = -1
                                try:
                                    resultDict[compType][system][component][
                                        "MEM"
                                    ] = float(runitDict[compDir]["MEM"])
                                except ValueError:
                                    resultDict[compType][system][component]["MEM"] = -1
                                try:
                                    resultDict[compType][system][component][
                                        "RSS"
                                    ] = float(runitDict[compDir]["RSS"])
                                except ValueError:
                                    resultDict[compType][system][component]["RSS"] = -1
                                try:
                                    resultDict[compType][system][component][
                                        "VSZ"
                                    ] = float(runitDict[compDir]["VSZ"])
                                except ValueError:
    ...
    def checkComponentModule(self, componentType, system, module):
        ...
        if componentType == "agent":
            ...
            loader = ModuleLoader(
                "Executor", PathFinder.getExecutorSection, ExecutorModule
            )
        else:
    ...
    def runsvctrlComponent(self, system, component, mode):
        ...
        startCompDirs = glob.glob(
            os.path.join(self.startDir, "%s_%s" % (system, component))
        )
    ...
    def setupSite(self, scriptCfg, cfg=None):
        ...
        if cfg:
            try:
                ...
                if self.instancePath != self.basePath:
                    ...
                    diracCfg.setOption(
                        cfgPath("LocalSite", "InstancePath"), self.instancePath
                    )
        ...
        setupSystems = self.localCfg.getOption(
            cfgInstallPath("Systems"), ["Configuration", "Framework"]
        )
        setupDatabases = self.localCfg.getOption(cfgInstallPath("Databases"), [])
        setupServices = [
            k.split("/")
            for k in self.localCfg.getOption(cfgInstallPath("Services"), [])
        ]
        setupAgents = [
            k.split("/") for k in self.localCfg.getOption(cfgInstallPath("Agents"), [])
        ]
        setupExecutors = [
            k.split("/")
            for k in self.localCfg.getOption(cfgInstallPath("Executors"), [])
        ]
        setupWeb = self.localCfg.getOption(cfgInstallPath("WebPortal"), False)
        setupConfigurationMaster = self.localCfg.getOption(
            cfgInstallPath("ConfigurationMaster"), False
        )
        setupPrivateConfiguration = self.localCfg.getOption(
            cfgInstallPath("PrivateConfiguration"), False
        )
        setupConfigurationName = self.localCfg.getOption(
            cfgInstallPath("ConfigurationName"), self.setup
        )
        setupAddConfiguration = self.localCfg.getOption(
            cfgInstallPath("AddConfiguration"), True
        )
        ...
        if self.basePath != self.instancePath:
            ...
            if os.path.realpath(instanceEtcDir) != os.path.realpath(etcDir):
                error = "Instance etc (%s) is not the same as DIRAC etc (%s)" % (
                    instanceEtcDir,
                    etcDir,
                )
                if self.exitOnError:
        ...
        if setupServices or setupAgents or setupExecutors or setupWeb:
            ...
            if not result["OK"]:
                if self.exitOnError:
                    gLogger.error(
                        "Failed to verify runsvdir running", result["Message"]
                    )
                    DIRAC.exit(-1)
        ...
        if ["Configuration", "Server"] in setupServices and setupConfigurationMaster:
            ...
            from DIRAC.ConfigurationSystem.Client.ConfigurationData import (
                gConfigurationData,
            )

            gLogger.notice("Installing Master Configuration Server")
            cfg = self.__getCfg(
                cfgPath("DIRAC", "Setups", self.setup), "Configuration", self.instance
            )
            self._addCfgToDiracCfg(cfg)
            cfg = self.__getCfg(cfgPath("DIRAC", "Configuration"), "Master", "yes")
            cfg.setOption(
                cfgPath("DIRAC", "Configuration", "Name"), setupConfigurationName
            )
            ...
            instanceCfg = self.__getCfg(
                instanceOptionPath, "Configuration", self.instance
            )
            ...
            self.addDefaultOptionsToComponentCfg(
            ...
            self.setupComponent(
                "service", "Configuration", "Server", [], checkModule=False
            )
        ...
        localServers = self.localCfg.getOption(
            cfgPath("DIRAC", "Configuration", "Servers")
        )
        masterServer = gConfig.getValue(
            cfgPath("DIRAC", "Configuration", "MasterServer"), ""
        )
        initialCfg = self.__getCfg(
            cfgPath("DIRAC", "Configuration"), "Servers", localServers
        )
        masterCfg = self.__getCfg(
            cfgPath("DIRAC", "Configuration"), "Servers", masterServer
        )
        ...
        if setupAddConfiguration:
            ...
            for system, service in setupServices:
                if not self.addDefaultOptionsToCS(
                    None, "service", system, service, extensions, overwrite=True
                )["OK"]:
                    ...
                    res = self.addDefaultOptionsToComponentCfg(
                        "service", system, service, extensions
                    )
                    if not res["OK"]:
                        gLogger.warn("Can't write to the specific component CFG")
            for system, agent in setupAgents:
                if not self.addDefaultOptionsToCS(
                    None, "agent", system, agent, extensions, overwrite=True
                )["OK"]:
                    # If we are not allowed to write to the central CS, add the configuration to the local file
                    gLogger.warn(
                        "Can't write to central CS, so adding to the specific component CFG"
                    ...
                    res = self.addDefaultOptionsToComponentCfg(
                        "agent", system, agent, extensions
                    )
                    if not res["OK"]:
                        gLogger.warn(
                            "Can't write to the specific component CFG",
                            "for %s : %s" % (system, agent),
                        )
            for system, executor in setupExecutors:
                if not self.addDefaultOptionsToCS(
                    None, "executor", system, executor, extensions, overwrite=True
                )["OK"]:
                    # If we are not allowed to write to the central CS, add the configuration to the local file
                    gLogger.warn(
                        "Can't write to central CS, so adding to the specific component CFG"
                    ...
                    res = self.addDefaultOptionsToComponentCfg(
                        "executor", system, executor, extensions
                    )
                    if not res["OK"]:
                        gLogger.warn(
                            "Can't write to the specific component CFG",
        ...
        if setupDatabases:
            ...
            for dbName in setupDatabases:
                ...
                if dbName not in installedDatabases:
                    ...
                    gLogger.notice(
                        "Database %s from %s/%s installed" % (dbName, extension, system)
                    )
                else:
                    gLogger.notice("Database %s already installed" % dbName)

                dbSystem = dbDict[dbName]["System"]
                result = self.addDatabaseOptionsToCS(
                    None, dbSystem, dbName, overwrite=True
                )
                if not result["OK"]:
                    gLogger.error(
                        "Database %s CS registration failed: %s"
                        % (dbName, result["Message"])
                    )
  ...
  """
    ...
    def installComponent(
        ...
        if checkModule:
            ...
            if not result["OK"]:
                if (
                    not self.checkComponentSoftware(
                        componentType, system, cModule, extensions
                    )["OK"]
                    and componentType != "executor"
                ):
                    error = "Software for %s %s/%s is not installed" % (
                        componentType,
                        system,
                        component,
                    )
        ...
        result = gConfig.getOption(
            "DIRAC/Setups/%s/%s" % (CSGlobals.getSetup(), system)
        )
        if not result["OK"]:
            return result
        ...
        bashVars = ""
        if compCfg.isSection(
            "Systems/%s/%s/%s/%s/Environment"
            % (system, self.instance, section, component)
        ):
            dictionary = compCfg.getAsDict()
            bashSection = dictionary["Systems"][system][self.instance][section][
                component
            ]["BashVariables"]
        ...
        try:
            componentCfg = os.path.join(
                self.linkedRootPath, "etc", "%s_%s.cfg" % (system, component)
            )
...
"""
            ...
            error = "Failed to prepare self.setup for %s %s/%s" % (
                componentType,
                system,
                component,
            )
    ...
    def setupComponent(
        ...
        """
        result = self.installComponent(
            componentType, system, component, extensions, componentModule, checkModule
        )
        if not result["OK"]:
            return result
        ...
        if not os.path.lexists(startCompDir):
            ...
            while (time.time() - 10) < start:
                ...
                return S_ERROR(
                    "Failed to find supervise/ok for component %s_%s"
                    % (system, component)
                )
        ...
        while (time.time() - 20) < start:
            ...
            if (
                result["Value"]
                and result["Value"]["%s_%s" % (system, component)]["RunitStatus"]
                == "Run"
            ):
                break
            time.sleep(1)
        ...
        resDict["RunitStatus"] = result["Value"]["%s_%s" % (system, component)][
            "RunitStatus"
        ]
    ...
    def setupPortal(self):
        ...
        while (time.time() - 10) < start:
            ...
            if (
                result["Value"]
                and result["Value"]["%s_%s" % ("Web", "WebApp")]["RunitStatus"] == "Run"
            ):
                break
            time.sleep(1)
  ...
  """
    ...
    def installDatabase(self, dbName):
        ...
        for cmd in [
            ...
            "GRANT %s ON `%s`.* TO '%s'@'%s'"
            % (perms, dbName, self.mysqlUser, self.mysqlHost),
    ...
    def execMySQL(self, cmd, dbName="mysql", localhost=False):
        ...
        if dbName not in self.db:
            ...
            if localhost:
                dbHost = "localhost"
            self.db[dbName] = MySQL(
                dbHost, self.mysqlRootUser, self.mysqlRootPwd, dbName, self.mysqlPort
            )
    ...
    def _addMySQLToDiracCfg(self):
        ...
        if not self.mysqlPassword:
            return S_ERROR(
                "Missing %s in %s"
                % (cfgInstallPath("Database", "Password"), self.cfgFile)
            )
