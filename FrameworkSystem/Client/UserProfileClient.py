...
class UserProfileClient(object):
    ...
    def __generateTypeDest(self, dataObj):
        ...
        if isinstance(dataObj, dict):
            return "d%se" % "".join(
                [
                    "%s%s"
                    % (self.__generateTypeDest(k), self.__generateTypeDest(dataObj[k]))
                    for k in dataObj
                ]
            )
        return ""
    ...
    def checkTypeRe(self, dataObj, typeRE):
        ...
        if not re.match(typeRE, typeDesc):
            return S_ERROR(
                "Stored data does not match typeRE: %s vs %s" % (typeDesc, typeRE)
            )
        return S_OK()
    ...
    def retrieveVarFromUser(self, ownerName, ownerGroup, varName, dataTypeRE=False):
        rpcClient = self.__getRPCClient()
        result = rpcClient.retrieveProfileVarFromUser(
            ownerName, ownerGroup, self.profile, varName
        )
        if not result["OK"]:
            return result
        return self.__decodeVar(result["Value"], dataTypeRE)
