...
class CLIParams(object):
    ...
    def registerCLISwitches(self):
        ...
        Script.registerSwitch(
            "S",
            "summary",
            "Enable summary output when generating proxy",
            self.setSummary,
        )
        Script.registerSwitch(
        ...
        Script.registerSwitch(
            "r",
            "rfc",
            "Create an RFC proxy, true by default, deprecated flag",
            self.setRFC,
        )
        Script.registerSwitch(
...
def generateProxy(params):
    ...
    if not retVal["OK"]:
        return S_ERROR(
            "Cannot load certificate %s: %s" % (params.certLoc, retVal["Message"])
        )
    ...
    retVal = testChain.loadKeyFromFile(
        params.keyLoc, password=params.userPasswd
    )  # XXX why so commented?
    if not retVal["OK"]:
    ...
    if params.checkWithCS:
        ...
        if not params.diracGroup:
            ...
            if not result["OK"]:
                gLogger.warn(
                    "Could not get a default group for DN %s: %s"
        ...
        if params.diracGroup not in groups:
            return S_ERROR(
                "Requested group %s is not valid for DN %s"
                % (params.diracGroup, userDN)
            )
        gLogger.info(
            "Creating proxy for %s@%s (%s)" % (username, params.diracGroup, userDN)
        )
    if params.summary:
