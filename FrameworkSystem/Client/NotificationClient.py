...
class NotificationClient(Client):
    ...
    def sendMail(
        ...
        addresses = (
            [addresses] if isinstance(addresses, six.string_types) else list(addresses)
        )
        for address in addresses:
            ...
            result = self._getRPC().sendMail(
                address, subject, body, str(fromAddress), avoidSpam
            )
            if not result["OK"]:
                self.log.error(
    ...
    def sendSMS(self, userName, body, fromAddress=None):
        ...
        if not result["OK"]:
            self.log.error(
                "Could not send SMS via central Notification service", result["Message"]
            )
        else:
            self.log.verbose(result["Value"])
    ...
    def newAlarm(
        self, subject, status, notifications, assignee, body, priority, alarmKey=""
    ):
    ...
    def addNotificationForUser(self, user, message, lifetime=604800, deferToMail=True):
        ...
        return self._getRPC().addNotificationForUser(
            user, message, lifetime, deferToMail
        )
