...
class MonitoringClient(object):
    ...
    def registerActivity(
        self, name, description, category, unit, operation, bucketLength=60
    ):
        """
    ...
    def addMark(self, name, value=1):
        ...
        if not self.__enabled:
            return
        if name not in self.activitiesDefinitions:
            raise MonitoringClientActivityNotDefined(
                "You must register activity %s before adding marks to it" % name
            )
    ...
    def __disabled(self):
        return gConfig.getValue(
            "%s/DisableMonitoring" % self.cfgSection, "false"
    ...
    def __sendData(self, secsTimeout=False):
        from DIRAC.FrameworkSystem.private.monitoring.ServiceInterface import (
            gServiceInterface,
        )
    ...
    def __sendMarks(self, rpcClient):
        ...
        retDict = rpcClient.commitMarks(
            self.sourceId, self.marksToSend, self.__compCommitExtraDict
        )
        if not retDict["OK"]:
        ...
        if len(retDict["Value"]) > 0:
            ...
            for acName in retDict["Value"]:
                if acName in self.activitiesDefinitions:
                    self.definitionsToSend[acName] = dict(
                        self.activitiesDefinitions[acName]
                    )
                    acMissedMarks[acName] = self.marksToSend[acName]
                else:
                    self.logger.debug(
                        "Server reported unregistered activity that does not exist"
                    )
    ...
    def getComponentsStatusWebFormatted(
        self, condDict={}, sortingList=[], startItem=0, maxItems=0
    ):
