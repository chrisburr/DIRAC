...
class BundleDeliveryClient(Client):
    ...
    def __setHash(self, bundleID, dirToSyncTo, bdHash):
        ...
        try:
            ...
            self.log.error(
                "Could not save hash after synchronization",
                "%s: %s" % (fileName, str(e)),
            )
    ...
    def syncDir(self, bundleID, dirToSyncTo):
        ...
        if os.path.isdir(dirToSyncTo):
            for p in [os.W_OK, os.R_OK]:
                if not os.access(dirToSyncTo, p):
                    self.log.error(
                        "%s does not have the permissions to update %s"
                        % (getpass.getuser(), dirToSyncTo)
                    )
                    return S_ERROR(
                        "%s does not have the permissions to update %s"
                        % (getpass.getuser(), dirToSyncTo)
        ...
        self.log.info(
            "Current hash for bundle %s in dir %s is '%s'"
            % (bundleID, dirToSyncTo, currentHash)
        )
