...
class ServiceInterface(object):
    ...
    def registerActivities(self, sourceDict, activitiesDict, componentExtraInfo):
        ...
        for name in activitiesDict:
            ...
            gLogger.info(
                "Received activity", "%s [%s]" % (name, str(activitiesDict[name]))
            )
            rrdFile = acCatalog.registerActivity(sourceId, name, activitiesDict[name])
            if not rrdFile:
                return S_ERROR("Could not register activity %s" % name)
            retVal = rrdManager.create(
                activitiesDict[name]["type"],
                rrdFile,
                activitiesDict[name]["bucketLength"],
            )
            if not retVal["OK"]:
                return retVal
    ...
    def commitMarks(self, sourceId, activitiesDict, componentExtraInfo):
        ...
        for acName in activitiesDict:
            ...
            if not acInfo:
                ...
                gLogger.warn(
                    "Cant find rrd filename", "%s:%s activity" % (sourceId, acName)
                )
                continue
            rrdFile = acInfo[6]
            ...
            if len(entries) > 0:
                ...
                retDict = rrdManager.update(
                    acInfo[4], rrdFile, acInfo[7], entries, long(acInfo[8])
                )
                if not retDict["OK"]:
                    gLogger.error(
    ...
    def fieldValue(self, field, definedFields):
        ...
        """
        retList = self.__createCatalog().queryField(
            "DISTINCT %s" % field, definedFields
        )
        return S_OK(retList)
    ...
    def __generateGroupPlots(self, fromSecs, toSecs, viewDescription, size):
        ...
        for groupPlot in plotList:
            retVal = self.plotCache.groupPlot(
                fromSecs, toSecs, groupPlot, viewDescription["stacked"], size
            )
            if not retVal["OK"]:
    ...
    def __generatePlots(self, fromSecs, toSecs, viewDescription, size):
        ...
        for activity in acList:
            activity.setLabel(viewDescription["label"])
            retVal = self.plotCache.plot(
                fromSecs, toSecs, activity, viewDescription["stacked"], size
            )
            if not retVal["OK"]:
                gLogger.error("There was a problem ploting", retVal["Message"])
                return retVal
    ...
    def saveView(self, viewName, viewDescription):
        ...
        acCatalog = self.__createCatalog()
        return acCatalog.registerView(
            viewName, DEncode.encode(viewDescription), viewDescription["variable"]
        )
    ...
    def plotView(self, viewRequest):
        ...
        if len(neededVarFields) > 0:
            if "varData" not in viewRequest:
                return S_ERROR(
                    "Missing variable fields %s!" % ", ".join(neededVarFields)
                )
            missingVarFields = []
            for neededField in neededVarFields:
                if neededField in viewRequest["varData"]:
                    viewDefinition["definition"][neededField] = viewRequest["varData"][
                        neededField
                    ]
                else:
            ...
            if len(missingVarFields) > 0:
                return S_ERROR(
                    "Missing required fields %s!" % ", ".join(missingVarFields)
                )
        return self.generatePlots(
            viewRequest["fromSecs"],
            viewRequest["toSecs"],
            viewDefinition,
            viewRequest["size"],
        )
    ...
    def __cmdb__writeComponent(self, sourceId):
        ...
        if not result["OK"]:
            gLogger.error(
                "Cannot register component in ComponentMonitoringDB", result["Message"]
            )
            return False
    ...
    def __cmdb__writeHeartbeat(self, sourceId):
        ...
        if not result["OK"]:
            gLogger.error(
                "Cannot heartbeat component in ComponentMonitoringDB", result["Message"]
            )
    ...
    def __cmdb_heartbeatComponent(self, sourceId, componentExtraInfo):
        ...
        if sourceId not in ServiceInterface.__sourceToComponentIdMapping:
            if not self.__cmdb__loadComponentFromActivityDB(sourceId):
                return False
        if ServiceInterface.__sourceToComponentIdMapping[sourceId]["type"] not in (
            "service",
            "agent",
        ):
            return True
