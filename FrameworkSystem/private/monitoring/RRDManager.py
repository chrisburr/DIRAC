...
class RRDManager(object):
    ...
    def __init__(self, rrdLocation, graphLocation):
        ...
        self.rrdExec = gConfig.getValue(
            "%s/RRDExec" % getServiceSection("Framework/Monitoring"), "rrdtool"
        )
    ...
    def __getLastUpdateTime(self, rrdFile):
        ...
        if retTuple[0]:
            return S_ERROR(
                "Failed to fetch last update %s : %s" % (rrdFile, retTuple[2])
            )
        ...
        for i in range(0, len(rrdUpdates), maxRRDArgs):
            ...
            if not retVal["OK"]:
                self.log.warn(
                    ...
                    "%s rrd: %s" % (rrdFile, retVal["Message"]),
                )
    ...
    def __generateRRDGraphVar(self, entryName, activity, timeSpan, plotWidth):
        ...
        varStr = "'DEF:ac%sRAW=%s/%s:value:AVERAGE'" % (
            entryName,
            self.rrdLocation,
            rrdFile,
        )
        if rrdType in ("mean", "rate"):
            varStr += " 'CDEF:%s=ac%sRAW,UN,0,ac%sRAW,IF'" % (
                entryName,
                entryName,
                entryName,
            )
            ...
            varStr += " 'CDEF:%s=ac%sRAW,UN,0,ac%sRAW,%s,*,IF'" % (
                entryName,
                entryName,
                entryName,
                scale,
            ...
            scale = yScaleFactor * bucketLength
            varStr += " 'CDEF:ac%sNOTUN=ac%sRAW,UN,0,ac%sRAW,%s,*,IF'" % (
                entryName,
                entryName,
                entryName,
                scale,
            )
            varStr += " 'CDEF:%s=PREV,UN,ac%sNOTUN,PREV,ac%sNOTUN,+,IF'" % (
                entryName,
                entryName,
                entryName,
            )
    ...
    def __graphTimeComment(self, fromEpoch, toEpoch):
        comStr = (
            " 'COMMENT:Generated on %s UTC'"
            % Time.toString().replace(":", r"\:").split(".")[0]
        )
        comStr += " 'COMMENT:%s'" % (
            "From %s to %s" % (Time.fromEpoch(fromEpoch), Time.fromEpoch(toEpoch))
        ).replace(":", r"\:")
        return comStr
    ...
    def groupPlot(
        self, fromSecs, toSecs, activitiesList, stackActivities, size, graphFilename=""
    ):
        """
        ...
        if not graphFilename:
            graphFilename = "%s.png" % self.__generateName(
                fromSecs, toSecs, activitiesList, stackActivities
            )
        ...
        for idActivity in range(len(activitiesList)):
            ...
            rrdCmd += " %s" % self.__generateRRDGraphVar(
                idActivity, activity, plotTimeSpan, self.__sizesList[size][0]
            )
            if stackActivities:
    ...
    def plot(self, fromSecs, toSecs, activity, stackActivities, size, graphFilename=""):
        ...
        if not graphFilename:
            graphFilename = "%s.png" % self.__generateName(
                fromSecs, toSecs, activity, stackActivities
            )
        graphVar = self.__generateRRDGraphVar(
            0, activity, plotTimeSpan, self.__sizesList[size][0]
        )
        rrdCmd = "%s graph %s/%s" % (self.rrdExec, self.graphLocation, graphFilename)
        rrdCmd += " -s %s" % fromSecs
        rrdCmd += " -e %s" % toSecs
