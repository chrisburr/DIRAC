...
class MonitoringCatalog(object):
    ...
    def __dbExecute(self, query, values=False):
        ...
        if not executed:
            self.log.error(
                "Could not execute query, big mess ahead",
                "query: %s, values: %s" % (query, values),
            )
        return cursor
    ...
    def activitiesQuery(self, selDict, sortList, start, limit):
        ...
        if sortList:
            ...
            extraSQL = "ORDER BY %s" % ",".join(
                ["%s %s" % sorting for sorting in sortList]
            )
        if limit:
            if start:
        ...
        retList = self.__select(
            ...
            "sources, activities",
            selDict,
            "sources.id = activities.sourceId",
            extraSQL,
        )
    ...
    def registerView(self, viewName, viewData, varFields):
        ...
        if len(retList) > 0:
            return S_ERROR(
                "View specification already defined with name '%s'" % retList[0][0]
            )
        self.__insert(
            "views",
            {"id": "NULL"},
    ...
    def getViewById(self, viewId):
        ...
        if isinstance(viewId, six.string_types):
            return self.__select(
                "definition, variableFields", "views", {"name": viewId}
            )
        else:
    ...
    def getActivities(self, dbCond):
        return self.__select(
            "id, name, category, unit, type, description, bucketLength",
            "activities",
            dbCond,
        )
