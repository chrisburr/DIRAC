...
from DIRAC.Core.Utilities.RabbitMQAdmin import (
    setUsersPermissions,
    addUsersWithoutPasswords,
)
from DIRAC.ResourceStatusSystem.Utilities import CSHelpers
from DIRAC import gLogger
from DIRAC.ConfigurationSystem.Client.Helpers.Operations import (
    Operations,
)  # for getValidPilotGroupName()
...
class RabbitMQSynchronizer(object):
    def __init__(self):
        ...
        self._accessProperty = (
            getAllowedHostProperty()
        )  # only host with this property are allowed to connect
    ...
    for host in hosts:
        if hostHasProperties(host, [accessProperty]):
            ...
            if retVal["OK"]:
                ...
                gLogger.error(
                    "Could not find a correct DN for host: %s. It will be ignored."
                    % host
                )
    return DNs
