...
class SystemLoggingDB(DB):
    ...
    def __init__(self):
        ...
        self.tableDict = {
            ...
            "ClientIPs": {
                ...
                "UniqueIndexes": {
                    "Client": ["ClientIPNumberString", "ClientFQDN", "SiteID"]
                },
                "Engine": "InnoDB",
            },
            "Systems": {
        ...
        if not result["OK"]:
            gLogger.error(
                "Failed to check/create the database tables", result["Message"]
            )
    ...
    def __buildTableList(self, showFieldList):
        ...
        if len(showFieldList):
            ...
            if (
                tableList.count("Sites")
                and tableList.count("MessageRepository")
                and not tableList.count("ClientIPs")
            ):
    ...
    def _queryDB(
        ...
        try:
            condition = self.buildCondition(
                condDict=condDict, older=older, newer=newer, timeStamp="MessageTime"
            )
        ...
        if not showFieldList:
            ...
            errorString = (
                "The showFieldList variable should be a string or a list of strings"
            )
    ...
    def __insertIntoAuxiliaryTable(self, tableName, outFields, inFields, inValues):
        ...
        if not result["OK"]:
            self.log.error(
                "__insertIntoAuxiliaryTable failed to query DB", result["Message"]
            )
            return S_ERROR()
        ...
        if not result["OK"] and "Duplicate entry" not in result["Message"]:
            self.log.error(
                "__insertIntoAuxiliaryTable failed to insert data into DB",
                result["Message"],
            )
        ...
        for i, item in enumerate(inValues):
            if item != insertedValues[i]:
                error = 'Inserted Value does not match %s: "%s" != "%s"' % (
                    inFields[i],
                    item,
                    insertedValues[i],
                )
    ...
    def insertMessage(self, message, site, nodeFQDN, userDN, userGroup, remoteAddress):
        ...
        outFields = ["UserDNID"]
        result = self.__insertIntoAuxiliaryTable(
            "UserDNs", outFields, inFields, inValues
        )
        if not result["OK"]:
            return result
        ...
        result = self.__insertIntoAuxiliaryTable(
            "ClientIPs", outFields, inFields, inValues
        )
        if not result["OK"]:
            return result
        messageList.append(result["Value"])
        ...
        outFields = ["SystemID"]
        result = self.__insertIntoAuxiliaryTable(
            "Systems", outFields, inFields, inValues
        )
        if not result["OK"]:
            return result
        ...
        result = self.__insertIntoAuxiliaryTable(
            "SubSystems", outFields, inFields, inValues
        )
        if not result["OK"]:
            return result
        ...
        result = self.__insertIntoAuxiliaryTable(
            "FixedTextMessages", outFields, inFields, inValues
        )
        if not result["OK"]:
            return result
        messageList.append(result["Value"])
    ...
    def _insertDataIntoAgentTable(self, agentName, data):
        ...
        if not result["OK"]:
            ...
            if not result["OK"]:
                return result
        cmd = (
            "UPDATE LOW_PRIORITY AgentPersistentData SET AgentData='%s' WHERE AgentID='%s'"
