...
class ComponentMonitoringDB(DB):
    ...
    def __registerIfNotThere(self, compDict):
        ...
        result = self._query(
            "SELECT id FROM `%s` WHERE %s" % (tableName, " AND ".join(sqlCond))
        )
        if not result["OK"]:
            self.log.error(
                "Cannot register component", "%s: %s" % (compLogName, result["Message"])
            )
            return result
        if len(result["Value"]):
    ...
    def __updateVersionHistoryIfNeeded(self, compId, compDict):
        ...
        for field in ("version", "DIRACVersion", "platform"):
            ...
            sqlCond.append("%s = '%s'" % (field, value))
        result = self._query(
            "SELECT CompId FROM `%s` WHERE %s" % (tableName, " AND ".join(sqlCond))
        )
        if not result["OK"]:
            return result
        if len(result["Value"]):
    ...
    def heartbeat(self, compDict):
        ...
        if "startTime" in compDict:
            sqlUpdateFields.append(
                "StartTime='%s'" % self.__datetime2str(compDict["startTime"])
            )
        return self._update(
            "UPDATE `%s` SET %s WHERE Id=%s"
            % (
                self.__getTableName("Components"),
    ...
    def __getComponents(self, condDict):
        ...
        for field in condDict:
            ...
            if isinstance(val, six.string_types):
                ...
                sqlWhere.append(
                    "( %s )" % " OR ".join(["%s='%s'" % (field, v) for v in val])
                )
        if sqlWhere:
            ...
            sqlWhere = ""
        result = self._query(
            "SELECT %s FROM `%s` %s" % (mainFields, compTable, sqlWhere)
        )
        if not result["OK"]:
            return result
        records = []
    ...
    def __getComponentDefinitionFromCS(self, system, setup, instance, cType, component):
        ...
        if cType == "service":
            ...
            if not result["OK"]:
                ...
                compDict["Message"] = (
                    "Component seems to be defined wrong in CS: %s" % result["Message"]
                )
                return compDict
            try:
    ...
    def __componentMatchesCondition(
    ...
    def getComponentsStatus(self, conditionDict={}):
            ...
            for system in systems:
                ...
                for cType in ("agent", "service"):
                    ...
                    result = gConfig.getSections(
                        "/Systems/%s/%s/%s"
                        % (system, instance, "%ss" % cType.capitalize())
                    )
                    if not result["OK"]:
                        self.log.warn(
                    ...
                    for component in components:
                        componentName = "%s/%s" % (system, component)
                        compDict = self.__getComponentDefinitionFromCS(
                            system, setup, instance, cType, component
                        )
                        if self.__componentMatchesCondition(
                            compDict, requiredComponents, conditionDict
                        ):
                            statusSet.addUniqueToSet(requiredComponents, compDict)
                # Walk the URLs
                result = gConfig.getOptionsDict(
                    "/Systems/%s/%s/URLs" % (system, instance)
                )
                if not result["OK"]:
                    self.log.warn(
                        "There doesn't to be defined the URLs section for %s in %s instance"
                        % (system, instance)
                    )
                else:
        ...
        return S_OK(
            (
                statusSet.getRequiredComponents(),
...
class StatusSet:
                ...
                for key in component:
                    ...
                    if key == "Host":
                        result = Network.checkHostsMatch(
                            component[key], potentiallyMoreRestrictiveComponent[key]
                        )
    ...
    def __addMissingDefinedComponents(self, compDictList):
        ...
        for compDict in compDictList:
            compDict = self.__setStatus(
                compDict,
                "Error",
                "Component is not up or hasn't connected to register yet",
            )
            cD.append(compDict)
    ...
    def __addFoundDefinedComponent(self, compDictList):
        ...
        for unmatchedComp in unmatched:
            self.__setStatus(
                unmatchedComp, "Error", "There is no component up with this properties"
            )
