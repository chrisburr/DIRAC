...
class NotificationDB(DB):
    ...
    def __checkAlarmField(self, name, value):
        ...
        if name == "status":
            if value not in self.__validAlarmStatus:
                return S_ERROR(
                    "Status %s is invalid. Valid ones are: %s"
                    % (value, self.__validAlarmStatus)
                )
        elif name == "priority":
            if value not in self.__validAlarmPriorities:
                return S_ERROR(
                    "Type %s is invalid. Valid ones are: %s"
                    % (value, self.__validAlarmPriorities)
                )
        elif name == "assignee":
    ...
    def newAlarm(self, alarmDef):
        ...
        if "alarmKey" in alarmDef:
            ...
            result = self._query(
                "SELECT AlarmId FROM `ntf_Alarms` WHERE AlarmKey=%s" % alarmKey
            )
            if not result["OK"]:
                return result
            if result["Value"]:
                return S_ERROR(
                    "Oops, alarm with id %s has the same alarm key!"
                    % result["Value"][0][0]
                )
        else:
        ...
        sqlInsert = "INSERT INTO `ntf_Alarms` (%s) VALUES (%s)" % (
            ",".join(sqlFieldsName),
            ",".join(sqlFieldsValue),
        )
    ...
    def deleteAlarmsByAlarmKey(self, alarmKeyList):
        ...
        self.log.info(
            "Trying to delete alarms with:\n alamKey %s\n  alarmId %s"
            % (alarmKeyList, alarmsIdList)
        )
    ...
    def deleteAlarmsByAlarmId(self, alarmIdList):
        ...
        try:
            ...
            return S_ERROR(
                "At least one alarmId is not a number: %s" % str(alarmIdList)
            )
        ...
        for tableName in tablesToCheck:
            delSql = "DELETE FROM `%s` WHERE AlarmId in ( %s )" % (
                tableName,
                alamsSQLList,
            )
            ...
            if not result["OK"]:
                self.log.error(
                    ...
                    "from table %s: %s" % (tableName, result["Message"]),
                )
        return S_OK()
    ...
    def updateAlarm(self, updateReq):
        ...
        if "alarmKey" in updateReq:
            ...
            if not result["OK"]:
                self.log.error(
                    "Could not get alarm id for key",
                    " %s: %s" % (alarmKey, result["Value"]),
                )
                return result
            updateReq["id"] = result["Value"]
            self.log.info(
                "Retrieving alarm key %s maps to id %s" % (alarmKey, updateReq["id"])
            )
        # Check fields
        ...
        if not validReq:
            return S_OK(
                "Requirement needs at least one of %s"
                % " ".join(self.__updateAlarmAtLeastOneField)
            )
        ...
        result = self._query(
            "SELECT AlarmId FROM `ntf_Alarms` WHERE AlarmId=%d" % alarmId
        )
        if not result["OK"]:
            return result
        if not result["Value"]:
        ...
        logSQL = "INSERT INTO `ntf_AlarmLog` (%s) VALUES (%s)" % (
            ",".join(sqlFields),
            ",".join(sqlValues),
        )
        ...
        if not result["OK"] or not result["Value"]:
            self.log.error(
                "Could not retrieve default notifications for alarm", "%s" % alarmId
            )
            return S_OK(alarmId)
        ...
        for follower in followers:
            result = self.modifyFollowerForAlarm(
                alarmId, follower, notificationsDict, overwrite=False
            )
            if not result["OK"]:
                varMsg = "\nFollower: %s\nAlarm: %s\nError: %s" % (
                    follower,
    ...
    def __generateAlarmLogMessage(self, alarmLog, showOnlyLast=False):
        ...
        for iD in logToShow:
            ...
            msg.append(
                " On       : %s" % data["timestamp"].strftime("%Y/%m/%d %H:%M:%S")
            )
    ...
    def __generateAlarmInfoMessage(self, alarmInfo):
        ...
        msg += "   Priority          : %s\n" % alarmInfo["priority"]
        msg += "   Assignee          : %s\n" % alarmInfo["assignee"]
        msg += "   Creation date     : %s UTC\n" % alarmInfo["creationtime"].strftime(
            "%Y/%m/%d %H:%M:%S"
        )
        msg += "   Last modificaiton : %s UTC\n" % alarmInfo["modtime"].strftime(
            "%Y/%m/%d %H:%M:%S"
        )
        msg += "   Body:\n\n%s" % alarmInfo["body"]
        return msg
    ...
    def getAlarms(
        self, condDict={}, sortList=False, start=0, limit=0, modifiedAfter=None
    ):
        ...
        if modifiedAfter:
            condSQL.append(
                "ModTime >= %s" % modifiedAfter.strftime("%Y-%m-%d %H:%M:%S")
            )
        if condSQL:
            selSQL = "%s WHERE %s" % (selSQL, " AND ".join(condSQL))
        if sortList:
            selSQL += " ORDER BY %s" % ", ".join(
    ...
    def getAlarmLog(self, alarmId):
        ...
        sqlSel = (
            "SELECT %s FROM `ntf_AlarmLog` WHERE AlarmId=%d ORDER BY Timestamp ASC"
            % (",".join(self.__alarmLogFields), alarmId)
        )
    ...
    def modifyFollowerForAlarm(self, alarmId, user, notificationsDict, overwrite=True):
        ...
        for k in notificationsDict:
            if notificationsDict[k]:
                ...
                break
        selSQL = (
            "SELECT Notification, Mail, SMS FROM `ntf_AlarmFollowers` WHERE AlarmId=%d AND User=%s"
            % (alarmId, user)
        )
        result = self._query(selSQL)
        if not result["OK"]:
        ...
        for k in self.__validAlarmNotifications:
            if notificationsDict[k]:
                ...
                modSQL.append("%s=0" % k)
        return self._update(
            "UPDATE `ntf_AlarmFollowers` SET %s WHERE %s" % (modSQL, sqlCond)
        )

    def getSubscribersForAlarm(self, alarmId):
        selSQL = (
            "SELECT User, Mail, Notification, SMS FROM `ntf_AlarmFollowers` WHERE AlarmId=%d"
            % alarmId
        )
        result = self._query(selSQL)
        if not result["OK"]:
    ...
    def getUserAsignees(self, assignee):
        ...
        escAG = result["Value"]
        sqlSel = (
            "SELECT User FROM `ntf_AssigneeGroups` WHERE AssigneeGroup = %s" % escAG
        )
        result = self._query(sqlSel)
        if not result["OK"]:
            return result
    ...
    def setAssigneeGroup(self, groupName, usersList):
        ...
        sqlSel = (
            "SELECT User FROM `ntf_AssigneeGroups` WHERE AssigneeGroup = %s" % escGroup
        )
        result = self._query(sqlSel)
        if not result["OK"]:
            return result
        ...
        if usersToAdd:
            sqlInsert = (
                "INSERT INTO `ntf_AssigneeGroups` ( AssigneeGroup, User ) VALUES %s"
                % ",".join(usersToAdd)
            )
            result = self._update(sqlInsert)
            if not result["OK"]:
    ...
    def getAssigneeGroups(self):
        result = self._query(
            "SELECT AssigneeGroup, User from `ntf_AssigneeGroups` ORDER BY User"
        )
        if not result["OK"]:
            return result
        agDict = {}
    ...
    def getAssigneeGroupsForUser(self, user):
        ...
        user = result["Value"]
        result = self._query(
            "SELECT AssigneeGroup from `ntf_AssigneeGroups` WHERE User=%s" % user
        )
        if not result["OK"]:
            return result
    ...
    def addNotificationForUser(self, user, message, lifetime=0, deferToMail=1):
        if user not in Registry.getAllUsers():
            return S_ERROR("%s is an unknown user" % user)
        self.log.info(
            "Adding a notification for user %s (msg is %s chars)" % (user, len(message))
        )
        result = self._escapeString(user)
        if not result["OK"]:
            return result
        ...
        sqlInsert = "INSERT INTO `ntf_Notifications` (%s) VALUES (%s) " % (
            ",".join(sqlFields),
            ",".join(sqlValues),
        )
        result = self._update(sqlInsert)
    ...
    def markNotificationsSeen(self, user, seen=True, msgIds=False):
        ...
        if seen:
            ...
            seen = 0
        updateSQL = "UPDATE `ntf_Notifications` SET Seen=%d WHERE User=%s" % (
            seen,
            user,
        )
        escapedIDs = []
    ...
    def getNotifications(self, condDict={}, sortList=False, start=0, limit=0):
        ...
        selSQL = "SELECT %s FROM `ntf_Notifications`" % ",".join(
        ...
        if eSortList:
            selSQL += " ORDER BY %s" % ", ".join(
                ["%s %s" % (sort[0], sort[1]) for sort in eSortList]
            )
        else:
    ...
    def purgeExpiredNotifications(self):
        ...
        delConds = [
            "(Seen=1 OR DeferToMail=0)",
            "(TIMESTAMPDIFF( SECOND, UTC_TIMESTAMP(), Expiration ) < 0 )",
        ]
        ...
        selSQL = (
            "SELECT Id, User, Message FROM `ntf_Notifications` WHERE %s"
            % " AND ".join(deferCond)
        )
        result = self._query(selSQL)
        if not result["OK"]:
        ...
        return self._update(
            "DELETE FROM `ntf_Notifications` WHERE Id in (%s)" % ",".join(ids)
        )
