...
class UserProfileDB(DB):
    ...
    def __getFieldsCached(self, tableName, outFields, condDict):
        ...
        if len(data) > 0:
            objId = data[0][0]
            self.updateFields(
                tableName, ["LastAccess"], ["UTC_TIMESTAMP()"], {"Id": objId}
            )
    ...
    def __getObjId(self, objValue, varName, tableName, insertIfMissing=True):
        ...
        result = self.insertFields(
            tableName, [varName, "LastAccess"], [objValue, "UTC_TIMESTAMP()"]
        )
        if not result["OK"]:
            return result
    ...
    def __webProfileReadAccessDataCond(
        self, userIds, ownerIds, sqlProfileName, sqlVarName=False, match=False
    ):
        permCondSQL = []
        sqlCond = []
        ...
        permCondSQL.append(
            '`up_ProfilesData`.GroupId=%s AND `up_ProfilesData`.ReadAccess="GROUP"'
            % userIds[1]
        )
        permCondSQL.append(
            '`up_ProfilesData`.VOId=%s AND `up_ProfilesData`.ReadAccess="VO"'
            % userIds[2]
        )
    ...
    def retrieveVarById(self, userIds, ownerIds, profileName, varName):
        ...
        sqlCond = self.__webProfileReadAccessDataCond(
            userIds, ownerIds, sqlProfileName, sqlVarName, True
        )
        ...
        return S_ERROR(
            "No data for userIds %s profileName %s varName %s"
            % (userIds, profileName, varName)
        )
    ...
    def retrieveUserProfilesById(self, userIds):
        ...
        selectSQL = (
            "SELECT Profile, varName, data FROM `up_ProfilesData` WHERE %s" % sqlCond
        )
        result = self._query(selectSQL)
        if not result["OK"]:
            return result
    ...
    def retrieveVarPermsById(self, userIds, ownerIds, profileName, varName):
        ...
        sqlCond = self.__webProfileReadAccessDataCond(
            userIds, ownerIds, sqlProfileName, sqlVarName
        )
        selectSQL = "SELECT %s FROM `up_ProfilesData` WHERE %s" % (
            ", ".join(self.__permAttrs),
            sqlCond,
        )
        result = self._query(selectSQL)
        if not result["OK"]:
    ...
    def retrieveVar(
        self, userName, userGroup, ownerName, ownerGroup, profileName, varName
    ):
        """
        Get a data entry for a profile
        """
    ...
    def storeVar(self, userName, userGroup, profileName, varName, data, perms=None):
        ...
        try:
            ...
            if not result["OK"]:
                return result
            userIds = result["Value"]
            return self.storeVarByUserId(
                userIds, profileName, varName, data, perms=perms
            )
        finally:
            pass
    ...
    def listVarsById(self, userIds, profileName, filterDict=None):
        ...
        sqlVars2Get = [
            ...
            "`up_Groups`.UserGroup",
            "`up_VOs`.VO",
            "`up_ProfilesData`.VarName",
        ]
        sqlQuery = (
            "SELECT %s FROM `up_Users`, `up_Groups`, `up_VOs`, `up_ProfilesData` WHERE %s"
            % (", ".join(sqlVars2Get), " AND ".join(sqlCond))
        )
    ...
    def retrieveAllHashTagsById(self, userIds):
        ...
        result = self.getFields(
            "up_HashTags",
            ["HashTag", "TagName"],
            {"UserId": userIds[0], "GroupId": userIds[1], "VOId": userIds[2]},
        )
        if not result["OK"]:
            return result
        data = result["Value"]
...
def testUserProfileDB():
    ...
    gConfig.setOptionValue(
        "/Systems/Framework/Test/Databases/UserProfileDB/Password", pwd
    )
