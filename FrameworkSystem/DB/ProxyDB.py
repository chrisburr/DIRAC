...
from DIRAC.Core.Security.X509Chain import (
    X509Chain,
    isPUSPdn,
...
class ProxyDB(DB):
    ...
    def __addUserNameToTable(self, tableName):
        ...
        """
        result = self._update(
            "ALTER TABLE `%s` ADD COLUMN UserName VARCHAR(64) NOT NULL" % tableName
        )
        if not result["OK"]:
            return result
        ...
        for userName, userDN in data:
            if not userName:
                ...
                try:
                    ...
                    self.log.error(
                        "Could not escape username or DN", "%s %s" % (userName, userDN)
                    )
                    continue
                userName = result["Value"]
                result = self._update(
                    "UPDATE `%s` SET UserName=%s WHERE UserDN=%s"
                    % (tableName, userName, userDN)
                )
                if not result["OK"]:
                    self.log.error(
                        "Could update username for DN",
                        "%s: %s" % (userDN, result["Message"]),
                    )
                    continue
    ...
    def __checkDBVersion(self):
        ...
        for tableName in (
            ...
            if "UserName" not in [row[0] for row in result["Value"]]:
                self.log.notice(
                    "Username missing in table %s schema. Adding it" % tableName
                )
    ...
    def generateDelegationRequest(self, proxyChain, userDN):
        ...
        retVal = self._query(
            "SELECT Id FROM `ProxyDB_Requests` WHERE Pem='%s'" % reqStr
        )
        if not retVal["OK"]:
            return retVal
        data = retVal["Value"]
    ...
    def completeDelegation(self, requestId, userDN, delegatedPem):
        ...
        if retVal["Value"]:
            ...
            self.log.warn(
                "Proxies with DIRAC group extensions must be not allowed to be uploaded:",
                retVal["Value"],
            )
    ...
    def __storeProxy(self, userDN, chain, proxyProvider=None):
        ...
        self.log.info(
            "Storing proxy for credentials %s (%d:%02d:%02d:%02d left)"
        ...
        dValues = {
            ...
            "ExpirationTime": "TIMESTAMPADD( SECOND, %d, UTC_TIMESTAMP() )"
            % int(remainingSecs),
        }
    ...
    def purgeExpiredProxies(self, sendNotifications=True):
        ...
        for tableName in (
            ...
            purged += result["Value"]
            self.log.info(
                "Purged %s expired proxies from %s" % (result["Value"], tableName)
            )
    ...
    def deleteProxy(self, userDN, userGroup=None, proxyProvider=None):
        ...
        for table in ["ProxyDB_Proxies", "ProxyDB_VOMSProxies"]:
            result = self._update(
                "%s %s"
                % (req % table, userGroup and "AND UserGroup=%s" % userGroup or "")
            )
            if not result["OK"]:
    ...
    def __getPemAndTimeLeft(
        self, userDN, userGroup=None, vomsAttr=None, proxyProvider=None
    ):
        ...
        cmd = (
            "SELECT Pem, TIMESTAMPDIFF( SECOND, UTC_TIMESTAMP(), ExpirationTime ) from %s "
            % sTable
        )
        cmd += (
            "WHERE UserDN=%s AND TIMESTAMPDIFF( SECOND, UTC_TIMESTAMP(), ExpirationTime ) > 0"
            % (sUserDN)
        )
        if proxyProvider:
            ...
            if record[0]:
                if proxyProvider:
                    ...
                    result = chain.generateProxyToString(
                        record[1], diracGroup=userGroup, rfc=True
                    )
                    if not result["OK"]:
                        return result
    ...
    def renewFromMyProxy(self, userDN, userGroup, lifeTime=None, chain=None):
        ...
        if not retVal["OK"]:
            return S_ERROR(
                "Can't retrieve DIRAC Group from renewed proxy: %s" % retVal["Message"]
            )
    ...
    def __getPUSProxy(
        self, userDN, userGroup, requiredLifetime, requestedVOMSAttr=False
    ):
        ...
        puspURL = (
            "%s?voms=%s:%s&proxy-renewal=false&disable-voms-proxy=false"
            "&rfc-proxy=true&cn-label=user:%s"
            % (puspServiceURL, vomsVO, vomsAttribute, user)
        )
    ...
    def __generateProxyFromProxyProvider(self, userDN, proxyProvider):
        ...
        """
        gLogger.info(
            "Getting proxy from proxyProvider",
            '(for "%s" DN by "%s")' % (userDN, proxyProvider),
        )
        ...
        if result["OK"]:
            return S_OK(
                {"proxy": proxyStr, "chain": chain, "remainingSecs": remainingSecs}
            )
        return result
    ...
    def __getProxyFromProxyProviders(self, userDN, userGroup, requiredLifeTime):
        ...
        if userGroup not in result["Value"]:
            return S_ERROR(
                "Cannot generate proxy: Invalid group %s for user" % userGroup
            )
        result = Registry.getProxyProvidersForDN(userDN)
        ...
        if result["OK"]:
            ...
            for proxyProvider in providers:
                ...
                result = self.__getPemAndTimeLeft(
                    userDN, userGroup, proxyProvider=proxyProvider
                )
                if result["OK"] and (
                    not requiredLifeTime or result["Value"][1] > requiredLifeTime
                ):
                    return result
                if len(providers) == 1:
                    return S_ERROR(
                        'Cannot generate proxy: No proxy providers found for "%s"'
                        % userDN
                    )
                self.log.verbose(
                    "Try to generate proxy from %s proxy provider" % proxyProvider
                )
                ...
                if result["OK"]:
                    ...
                    result = chain.generateProxyToString(
                        remainingSecs, diracGroup=userGroup, rfc=True
                    )
                    if result["OK"]:
        ...
        return S_ERROR(
            "Cannot generate proxy%s" % (errMsgs and ": " + ", ".join(errMsgs) or "")
        )
    ...
    def getProxy(self, userDN, userGroup, requiredLifeTime=None):
        ...
        retVal = self.__getPemAndTimeLeft(userDN, userGroup)
        errMsg = "Can't get proxy%s: " % (
            requiredLifeTime and " for %s seconds" % requiredLifeTime or ""
        )
        if not retVal["OK"]:
            errMsg += "%s, try to generate new" % retVal["Message"]
            retVal = self.__getProxyFromProxyProviders(
                userDN, userGroup, requiredLifeTime=requiredLifeTime
            )
        ...
        if self.__useMyProxy:
            if requiredLifeTime:
                if timeLeft < requiredLifeTime:
                    retVal = self.renewFromMyProxy(
                        userDN, userGroup, lifeTime=requiredLifeTime, chain=chain
                    )
                    if not retVal["OK"]:
                        return S_ERROR(
                            "%s; the proxy lifetime from MyProxy is less than required."
                            % errMsg
                        )
                    chain = retVal["Value"]
    ...
    def __getVOMSAttribute(self, userGroup, requiredVOMSAttribute=False):
        ...
        if requiredVOMSAttribute:
            return S_OK(
                {
                    "attribute": requiredVOMSAttribute,
                    "VOMSVO": Registry.getVOMSVOForGroup(userGroup),
                }
            )
        ...
        return S_OK(
            {
                "attribute": csVOMSMapping,
                "VOMSVO": Registry.getVOMSVOForGroup(userGroup),
            }
        )

    def getVOMSProxy(
        self, userDN, userGroup, requiredLifeTime=None, requestedVOMSAttr=None
    ):
        """Get proxy string from the Proxy Repository for use with userDN
        in the userGroup
        ...
        if retVal["OK"]:
            ...
            if retVal["OK"]:
                ...
                if retVal["OK"]:
                    ...
                    if (
                        requiredLifeTime
                        and requiredLifeTime <= vomsTime
                        and requiredLifeTime <= remainingSecs
                    ):
                        return S_OK((chain, min(vomsTime, remainingSecs)))

        if isPUSPdn(userDN):
            # Get the Per User SubProxy if one is requested
            result = self.__getPUSProxy(
                userDN, userGroup, requiredLifeTime, requestedVOMSAttr
            )
            if not result["OK"]:
                return result
            pemData = result["Value"][0]
            ...
            if attrs[0]:
                ...
                if not retVal["OK"]:
                    return S_ERROR(
                        "Cannot append voms extension: %s" % retVal["Message"]
                    )
                chain = retVal["Value"]
    ...
    def getUsers(self, validSecondsLeft=0, userMask=None):
        ...
        if validSecondsLeft:
            ...
            sqlCond.append(
                "TIMESTAMPDIFF( SECOND, UTC_TIMESTAMP(), ExpirationTime ) > %d"
                % validSecondsLeft
            )
    ...
    def getCredentialsAboutToExpire(self, requiredSecondsLeft, onlyPersistent=True):
        ...
        cmd += (
            " WHERE TIMESTAMPDIFF( SECOND, ExpirationTime, UTC_TIMESTAMP() ) < %d and "
            % requiredSecondsLeft
        )
    ...
    def setPersistencyFlag(self, userDN, userGroup, persistent=True):
        ...
        if sqlInsert:
            ...
            if not result["OK"]:
                self.log.error(
                    "setPersistencyFlag: Can not retrieve username for DN", userDN
                )
                return result
            try:
            ...
            cmd += " VALUES( %s, %s, %s, '', UTC_TIMESTAMP(), 'True' )" % (
                sUserName,
                sUserDN,
                sUserGroup,
            )
            ...
            cmd = (
                "UPDATE `ProxyDB_Proxies` SET PersistentFlag='%s' WHERE UserDN=%s AND UserGroup=%s"
                % (sqlFlag, sUserDN, sUserGroup)
            )
    ...
    def getProxiesContent(self, selDict, sortList, start=0, limit=0):
                ...
                if isinstance(fVal, (dict, tuple, list)):
                    sqlWhere.append(
                        ...
                        % (
                            ...
                            ", ".join(
                                [
                                    self._escapeString(str(value))["Value"]
                                    for value in fVal
                                ]
                            ),
                        )
                    ...
                    sqlWhere.append(
                        "%s = %s" % (field, self._escapeString(str(fVal))["Value"])
                    )
            sqlOrder = []
            if sortList:
                for sort in sortList:
                    ...
                    if sort[0] not in fields:
                        if table == "ProxyDB_CleanProxies" and sort[0] in [
                            "UserGroup",
                            "PersistentFlag",
                        ]:
                            continue
        ...
        return S_OK(
            {"ParameterNames": fields, "Records": data, "TotalRecords": totalRecords}
        )
    ...
    def getLogsContent(self, selDict, sortList, start=0, limit=0):
        ...
        if selDict:
            ...
            if "beforeDate" in selDict:
                qr.append(
                    "Timestamp < %s"
                    % self._escapeString(selDict["beforeDate"])["Value"]
                )
                del selDict["beforeDate"]
            if "afterDate" in selDict:
                qr.append(
                    "Timestamp > %s" % self._escapeString(selDict["afterDate"])["Value"]
                )
            ...
            for field in selDict:
                qr.append(
                    ...
                    % " OR ".join(
                        [
                            "%s=%s" % (field, self._escapeString(str(value))["Value"])
        ...
        if sortList:
            cmd += " ORDER BY %s" % ", ".join(
                ["%s %s" % (sort[0], sort[1]) for sort in sortList]
            )
        if limit:
            cmd += " LIMIT %d,%d" % (start, limit)
        retVal = self._query(cmd)
    ...
    def generateToken(
        self, requesterDN, requesterGroup, numUses=1, lifeTime=0, retries=10
    ):
        ...
        """
        if not lifeTime:
            lifeTime = gConfig.getValue(
                "/DIRAC/VOPolicy/TokenLifeTime", self.__defaultTokenLifetime
            )
        maxUses = gConfig.getValue(
            "/DIRAC/VOPolicy/TokenMaxUses", self.__defaultTokenMaxUses
        )
        ...
        token = m.hexdigest()
        fieldsSQL = ", ".join(
            ("Token", "RequesterDN", "RequesterGroup", "ExpirationTime", "UsesLeft")
        )
        valuesSQL = ", ".join(
            (
        ...
        insertSQL = "INSERT INTO `ProxyDB_Tokens` ( %s ) VALUES ( %s )" % (
            fieldsSQL,
            valuesSQL,
        )
    ...
    def useToken(self, token, requesterDN, requesterGroup):
        ...
        updateSQL = (
            "UPDATE `ProxyDB_Tokens` SET UsesLeft = UsesLeft - 1 WHERE %s" % sqlCond
        )
    ...
    def sendExpirationNotifications(self):
        ...
        notifLimits = sorted(
            [
                int(x)
                for x in self.getCSOption(
                    "NotificationTimes", ProxyDB.NOTIFICATION_TIMES
                )
            ]
        )
        sqlSel = "UserDN, UserGroup, TIMESTAMPDIFF( SECOND, UTC_TIMESTAMP(), ExpirationTime )"
        sqlCond = "TIMESTAMPDIFF( SECOND, UTC_TIMESTAMP(), ExpirationTime ) < %d" % max(
            notifLimits
        )
        ...
        for row in data:
            ...
            for notifLimit in notifLimits:
                ...
                if notKey not in notifDone:
                    values = (
                        "( %s, %s, %d, TIMESTAMPADD( SECOND, %s, UTC_TIMESTAMP() ) )"
                        % (sUserDN, sGroup, notifLimit, lTime)
                    )
                    cmd = (
                    ...
                    if not result["OK"]:
                        gLogger.error(
                            "Could not mark notification as sent", result["Message"]
                        )
                else:
                    values = (
                        "LifeLimit = %d, ExpirationTime = TIMESTAMPADD( SECOND, %s, UTC_TIMESTAMP() )"
                        % (notifLimit, lTime)
                    )
                    cmd = (
                        "UPDATE `ProxyDB_ExpNotifs` SET %s WHERE UserDN = %s AND UserGroup = %s"
                        % (values, sUserDN, sGroup)
                    )
                    result = self._update(cmd)
                    if not result["OK"]:
                        gLogger.error(
...
""" % (
        ...
        result = self.__notifClient.sendMail(
            userEMail, msgSubject, msgBody, fromAddress=fromAddr
        )
        if not result["OK"]:
    ...
    def __storeProxyOld(self, userDN, userGroup, chain):
        ...
        if sqlInsert:
            ...
            for key in dValues:
                sqlFields.append(key)
                sqlValues.append(dValues[key])
            cmd = "INSERT INTO `ProxyDB_Proxies` ( %s ) VALUES ( %s )" % (
                ", ".join(sqlFields),
                ", ".join(sqlValues),
            )
        else:
            ...
            for k in dValues:
                if k in ("UserDN", "UserGroup"):
                    ...
                    sqlSet.append("%s = %s" % (k, dValues[k]))
            cmd = "UPDATE `ProxyDB_Proxies` SET %s WHERE %s" % (
                ", ".join(sqlSet),
                " AND ".join(sqlWhere),
            )
