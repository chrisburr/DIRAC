...
class ProxyInit(object):
    ...
    def getIssuerCert(self):
        ...
        if not resultProxyChainFromFile["OK"]:
            gLogger.error(
                "Could not load the proxy: %s" % resultProxyChainFromFile["Message"]
            )
            sys.exit(1)
    ...
    def certLifeTimeCheck(self):
        minLife = Registry.getGroupOption(
            self.__piParams.diracGroup, "SafeCertificateLifeTime", 2592000
        )
        resultIssuerCert = self.getIssuerCert()
        resultRemainingSecs = (
            resultIssuerCert.getRemainingSecs()
        )  # pylint: disable=no-member
        if not resultRemainingSecs["OK"]:
            gLogger.error(
                "Could not retrieve certificate expiration time",
                resultRemainingSecs["Message"],
            )
            return
        ...
        if minLife > lifeLeft:
            ...
            msg = (
                "Your certificate will expire in less than %d days. Please renew it!"
                % daysLeft
            )
            sep = "=" * (len(msg) + 4)
            msg = "%s\n  %s  \n%s" % (sep, msg, sep)
    ...
    def uploadProxy(self):
        ...
        if userDN in self.__uploadedInfo:
            ...
            if expiry:
                if (
                    issuerCert.getNotAfterDate()["Value"]
        ...
        upParams.proxyLifeTime = (
            issuerCert.getRemainingSecs()["Value"] - 300
        )  # pylint: disable=no-member
    ...
    def printInfo(self):
        ...
        if not resultProxyInfoAsAString["OK"]:
            gLogger.error(
                "Failed to get the new proxy info: %s"
                % resultProxyInfoAsAString["Message"]
            )
        else:
    ...
    def doTheMagic(self):
        ...
        if not resultProxyWithVOMS["OK"]:
            if "returning a valid AC for the user" in resultProxyWithVOMS["Message"]:
                ...
                gLogger.notice(
                    "Failed to add VOMS extension: no standard grid interface available"
                )
            else:
                gLogger.error(resultProxyWithVOMS["Message"])
