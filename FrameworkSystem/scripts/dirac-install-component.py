...
Script.registerSwitch(
    "m:", "module=", "Python module name for the component code", setModule
)
...
for compType in availableComponents:
    if (
        ...
        and compOrMod in availableComponents[compType][system]
    ):
...
if not cType:
    gLogger.error(
        "Component %s/%s is not available for installation" % (system, component)
    )
    DIRACexit(1)
...
if not result["OK"]:
    ...
    result = gComponentInstaller.installComponent(
    ...
    if not result["OK"]:
        ...
        if not result["OK"]:
            ...
            DIRACexit(1)
        gLogger.notice(
            "Successfully completed the installation of %s/%s" % (system, component)
        )
        DIRACexit()
