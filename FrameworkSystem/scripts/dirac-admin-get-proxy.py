...
class Params(object):
    ...
    def registerCLISwitches(self):
        ...
        Script.registerSwitch(
            "a",
            "voms",
            "Get proxy with VOMS extension mapped to the DIRAC group",
            self.automaticVOMS,
        )
        Script.registerSwitch(
...
if userDN.find("/") != 0:
    ...
    if not retVal["OK"]:
        gLogger.notice(
            "Cannot discover DN for username %s\n\t%s" % (userName, retVal["Message"])
        )
        DIRAC.exit(2)
    DNList = retVal["Value"]
    if len(DNList) > 1:
...
if not result["OK"]:
    gLogger.notice(
        "Proxy file cannot be written to %s: %s" % (params.proxyPath, result["Message"])
    )
    DIRAC.exit(2)
