...
class ErrorMessageMonitor(AgentModule):
    ...
    def execute(self):
        ...
        result = self.systemLoggingDB.getCounters(
            "FixedTextMessages", ["ReviewedMessage"], condDict
        )
        if not result["OK"]:
            return result
        ...
        result = self.systemLoggingDB._queryDB(
            showFieldList=returnFields,
            groupColumn="FixedTextString,FixedTextID,SystemName",
            condDict=condDict,
        )
        if not result["OK"]:
            self.log.error(
                "Failed to obtain the non reviewed Strings", result["Message"]
            )
            return S_OK()
        ...
        if self._mailAddress:
            result = self.notification.sendMail(
                self._mailAddress, self._subject, mailBody
            )
            if not result["OK"]:
        ...
        result = self.systemLoggingDB.updateFields(
            "FixedTextMessages", ["ReviewedMessage"], [1], condDict=condDict
        )
        if not result["OK"]:
        ...
        self.log.info(
            "The messages have been sent for review",
