...
class TopErrorMessagesReporter(AgentModule):
    ...
    def initialize(self):
        ...
        if not mailList:
            errString = (
                "There are no valid users in the list of email where to send the report"
            )
    ...
    def execute(self):
        ...
        cmd = (
            ...
            + " GROUP BY FixedTextID,SystemName,SubSystemName HAVING entries > %s"
            % self._threshold
        ...
        if not result["OK"]:
            ...
            if result["Value"]:
                self.log.debug(
                    "date value: %s" % fromString(result["Value"][0][0][1:-1])
                )
        ...
        self.log.debug(
            "limitDate: %s\t" % limitDate + "lastMailSentDate: %s\n" % lastMailSentDate
        )
        if lastMailSentDate > limitDate:
            self.log.info(
                "The previous report was sent less " + " than %s days ago" % self.__days
            )
            return S_OK()
