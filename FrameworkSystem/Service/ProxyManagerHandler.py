...
class ProxyManagerHandler(RequestHandler):
    ...
    def initializeHandler(cls, serviceInfoDict):
        ...
        gThreadScheduler.addPeriodicTask(
            900, cls.__proxyDB.purgeExpiredTokens, elapsedTime=900
        )
        gThreadScheduler.addPeriodicTask(
            900, cls.__proxyDB.purgeExpiredRequests, elapsedTime=900
        )
    ...
    types_requestDelegationUpload = [
        six.integer_types,
        [six.string_types, bool, type(None)],
    ]
    ...
    def export_requestDelegationUpload(self, requestedUploadTime, diracGroup=None):
        ...
        if diracGroup:
            ...
            self.log.warn(
                "Proxy with DIRAC group or VOMS extensions must be not allowed to be uploaded."
            )
        credDict = self.getRemoteCredentials()
        userDN = credDict["DN"]
        ...
        if userGroup not in groupsAvailable:
            return S_ERROR(
                "%s is not a valid group for user %s" % (userGroup, userName)
            )
        clientChain = credDict["x509Chain"]
        clientSecs = clientChain.getIssuerCert()["Value"].getRemainingSecs()["Value"]
        requestedUploadTime = min(
            requestedUploadTime, clientSecs
    ...
    def export_completeDelegationUpload(self, requestId, pemChain):
        ...
        if not retVal["OK"]:
            gLogger.error(
                "Upload proxy failed",
                "id: %s user: %s message: %s" % (requestId, userId, retVal["Message"]),
            )
    ...
    def export_getRegisteredUsers(self, validSecondsRequired=0):
        ...
        if Properties.PROXY_MANAGEMENT not in credDict["properties"]:
            return self.__proxyDB.getUsers(
                validSecondsRequired, userMask=credDict["username"]
            )
    ...
    def __checkProperties(self, requestedUserDN, requestedUserGroup):
        ...
        if Properties.PRIVATE_LIMITED_DELEGATION in credDict["properties"]:
            ...
            if (
                Properties.PRIVATE_LIMITED_DELEGATION
                not in Registry.getPropertiesForGroup(requestedUserGroup)
            ):
    ...
    def export_getProxy(self, userDN, userGroup, requestPem, requiredLifetime):
        ...
        self.__proxyDB.logAction(
            "download proxy", credDict["DN"], credDict["group"], userDN, userGroup
        )
        return self.__getProxy(
            userDN, userGroup, requestPem, requiredLifetime, forceLimited
        )
    ...
    def __getProxy(self, userDN, userGroup, requestPem, requiredLifetime, forceLimited):
        ...
        """
        retVal = self.__proxyDB.getProxy(
        ...
        requiredLifetime = int(
            min(secsLeft, requiredLifetime * self.__maxExtraLifeFactor)
        )
    ...
    def export_getVOMSProxy(
        self, userDN, userGroup, requestPem, requiredLifetime, vomsAttribute=None
    ):
        """Get a proxy for a userDN/userGroup
        ...
        self.__proxyDB.logAction(
            "download voms proxy", credDict["DN"], credDict["group"], userDN, userGroup
        )
        return self.__getVOMSProxy(
            userDN, userGroup, requestPem, requiredLifetime, vomsAttribute, forceLimited
        )
    ...
    def __getVOMSProxy(
        ...
        return chain.generateChainFromRequestString(
            requestPem, lifetime=requiredLifetime, requireLimited=forceLimited
        )
    ...
    def export_setPersistency(self, userDN, userGroup, persistentFlag):
        ...
        credDict = self.getRemoteCredentials()
        self.__proxyDB.logAction(
            "set persistency to %s" % bool(persistentFlag),
            credDict["DN"],
            credDict["group"],
            userDN,
            userGroup,
    ...
    def export_deleteProxyBundle(self, idList):
        ...
        if errorInDelete:
            return S_ERROR(
                "Could not delete some proxies: %s" % ",".join(errorInDelete)
            )
        return S_OK(deleted)
    ...
    def export_deleteProxy(self, userDN, userGroup):
        ...
        if not retVal["OK"]:
            return retVal
        self.__proxyDB.logAction(
            "delete proxy", credDict["DN"], credDict["group"], userDN, userGroup
        )
        return S_OK()
    ...
    def export_generateToken(self, requesterDN, requesterGroup, tokenUses):
        ...
        return self.__proxyDB.generateToken(
            requesterDN, requesterGroup, numUses=tokenUses
        )
    ...
    def export_getProxyWithToken(
        self, userDN, userGroup, requestPem, requiredLifetime, token
    ):
        """Get a proxy for a userDN/userGroup
        ...
        gLogger.info(
            "Trying to use token %s by %s:%s"
            % (token, credDict["DN"], credDict["group"])
        )
        if not result["OK"]:
            return result
        if not result["Value"]:
        ...
        self.__proxyDB.logAction(
            "used token", credDict["DN"], credDict["group"], userDN, userGroup
        )
    ...
    def export_getVOMSProxyWithToken(
        self, userDN, userGroup, requestPem, requiredLifetime, token, vomsAttribute=None
    ):
        """Get a proxy for a userDN/userGroup
        ...
        return self.__getVOMSProxy(
            userDN, userGroup, requestPem, requiredLifetime, vomsAttribute, True
        )
