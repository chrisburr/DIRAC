...
def initializeMonitoringHandler(serviceInfo):
    ...
    # Get data location
    dataPath = gConfig.getValue(
        "%s/DataLocation" % monitoringSection, "data/monitoring"
    )
    dataPath = dataPath.strip()
    if dataPath[0] != "/":
        dataPath = os.path.realpath(
            "%s/%s" % (gConfig.getValue("/LocalSite/InstancePath", rootPath), dataPath)
        )
    ...
    gMonitor.registerActivity(
        "drawnplots", "Drawn plot images", "Monitoring plots", "plots", gMonitor.OP_SUM
    )
    return S_OK()
...
class MonitoringHandler(RequestHandler):
    ...
    def export_registerActivities(
        self, sourceDict, activitiesDict, componentExtraInfo={}
    ):
        """
        Registers new activities
        """
        return gServiceInterface.registerActivities(
            sourceDict, activitiesDict, componentExtraInfo
        )
    ...
    def export_commitMarks(self, sourceId, activitiesDict, componentExtraInfo={}):
        ...
        activitiesDict = {
            metric: strToIntDict(value) for metric, value in activitiesDict.iteritems()
        }
        ...
        return gServiceInterface.commitMarks(
            sourceId, activitiesDict, componentExtraInfo
        )
    ...
    def export_getActivitiesContents(self, selDict, sortList, start, limit):
        ...
        result = gServiceInterface.getActivitiesContents(
            selDict, sortList, start, limit
        )
        if not result["OK"]:
            return result
