...
class NotificationHandler(RequestHandler):
    ...
    def export_sendMail(self, address, subject, body, fromAddress, avoidSpam=False):
        ...
        eMail._fromAddress = (
            gConfig.getValue("%s/FromAddress" % csSection) or eMail._fromAddress
        )
        result = eMail._send()
        if not result["OK"]:
            gLogger.warn(
            ...
            gLogger.info(
                "Mail sent successfully to %s with subject %s" % (address, subject)
            )
    ...
    def export_sendSMS(self, userName, body, fromAddress):
        ...
        if not smsSwitch:
            return S_ERROR(
                "No SMS switch is defined in CS path %s/SMSSwitch" % csSection
            )
    ...
    def export_addNotificationForUser(
        self, user, message, lifetime=604800, deferToMail=True
    ):
        """Create a group of users to be used as an assignee for an alarm"""
        try:
