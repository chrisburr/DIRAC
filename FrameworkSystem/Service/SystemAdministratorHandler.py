...
class SystemAdministratorHandler(RequestHandler):
            ...
            for system in statusDict[compType]:
                for component in statusDict[compType][system]:
                    ...
                    if not result["OK"]:
                        ...
                        statusDict[compType][system][component]["Module"] = result[
                            "Value"
                        ]
    ...
    def export_installComponent(
        self, componentType, system, component, componentModule=""
    ):
    ...
    def export_addDatabaseOptionsToCS(self, system, database, overwrite=False):
        ...
        return gComponentInstaller.addDatabaseOptionsToCS(
            gConfig, system, database, overwrite=overwrite
        )

    types_addDefaultOptionsToCS = [basestring, basestring, basestring]

    def export_addDefaultOptionsToCS(
        self, componentType, system, component, overwrite=False
    ):
    ...
    def export_updateSoftware(self, version, rootPath="", gridVersion=""):
        ...
        webPortal = gConfig.getValue(
            "/LocalInstallation/WebApp", False
    ...
    def export_setProject(self, projectName):
        ...
        diracCFG.setOption(
            "/LocalInstallation/Project", projectName, "Project to install"
        )
        try:
            ...
            result.update(
                {
                    "Extensions": ";".join(
                        [
                            "%s:%s" % (key, value)
                            for (key, value) in infoResult["Value"]
                            .get("Extensions")
                            .iteritems()
                        ]
                    )
    ...
    def export_getComponentDocumentation(self, cType, system, module):
        ...
        try:
            importedModule = importlib.import_module(
                "DIRAC.%sSystem.%s.%s" % (system, cType.capitalize(), module)
            )
            return S_OK(importedModule.__doc__)
        except Exception as _e:
    ...
    def __deleteOldSoftware(keepLast):
        ...
        if versionsDirectory.endswith(
            ...
            try:
                ...
                gLogger.error(
                    "Can not delete old DIRAC versions from the file system", repr(e)
                )
        else:
