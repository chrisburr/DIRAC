        ...
        def makeProperty(self, name, value, readOnly=False):
            ...
            if hasattr(self, "_" + name) or hasattr(self, name):
                raise AttributeError(
                    "_%s or %s is already defined as a member" % (name, name)
                )
            ...
            fset = (
                None if readOnly else lambda self, value: self._setProperty(name, value)
            )
...
class Node(object):
    ...
    def addEdge(self, edge):
        ...
        if not isinstance(edge, Edge):
            raise TypeError(
                "supplied edge argument should be an Edge instance or subclass"
            )
        if edge not in self:
...
class Edge(object):
    ...
    def __init__(self, fromNode, toNode, rwAttrs=None, roAttrs=None):
        ...
        self.makeProperty(
            "name", "%s-%s" % (self.fromNode.name, self.toNode.name), True
        )
        self.makeProperty("visited", False)
        rwAttrs = rwAttrs if isinstance(rwAttrs, dict) else {}
        for attr, value in rwAttrs.items():
