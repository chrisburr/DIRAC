...
class ExecutorQueues:
    ...
    def pushTask(self, eType, taskId, ahead=False):
        self.__log.verbose(
            "Pushing task %s into waiting queue for executor %s" % (taskId, eType)
        )
        self.__lock.acquire()
        try:
            if taskId in self.__taskInQueue:
                if self.__taskInQueue[taskId] != eType:
                    errMsg = (
                        "Task %s cannot be queued because it's already queued for %s"
                        % (taskId, self.__taskInQueue[taskId])
                    )
    ...
    def popTask(self, eTypes):
        ...
        for eType in eTypes:
            try:
                ...
                self.__lastUse[eType] = time.time()
                self.__log.verbose(
                    "Popped task %s from executor %s waiting queue" % (taskId, eType)
                )
                return (taskId, eType)
            except IndexError:
                continue
...
class ExecutorDispatcher:
    ...
    def setCallbacks(self, callbacksObj):
        if not isinstance(callbacksObj, ExecutorDispatcherCallbacks):
            return S_ERROR(
                "Callbacks object does not inherit from ExecutorDispatcherCallbacks"
            )
    ...
    def addExecutor(self, eId, eTypes, maxTasks=1):
        self.__log.verbose(
            "Adding new %s executor to the pool %s" % (eId, ", ".join(eTypes))
        )
        ...
        if not isFrozen:
            ...
            if self.__failedOnTooFrozen:
                self.__cbHolder.cbTaskError(
                    taskId,
                    eTask.taskObj,
                    "Retried more than 10 times. Last error: %s" % errMsg,
                )
            return False
        return True
        ...
        while iP < len(self.__taskFreezer):
            ...
            try:
                ...
                try:
                    ...
                    self.__log.notice(
                        "Removing task %s from the freezer. Somebody has removed the task"
                        % taskId
                    )
                    self.__taskFreezer.pop(iP)
                    continue
    ...
    def __dispatchTask(self, taskId, defrozeIfNeeded=True):
        ...
        if not result["OK"]:
            self.__log.warn(
                "Error while calling dispatch callback: %s" % result["Message"]
            )
            ...
            self.removeTask(taskId)
            self.__cbHolder.cbTaskError(
                taskId, taskObj, "Could not dispatch task: %s" % result["Message"]
            )
        ...
        if eType not in self.__execTypes:
            if self.__freezeOnUnknownExecutor:
                self.__log.verbose(
                    "Executor type %s has not connected. Freezing task %s"
                    % (eType, taskId)
                )
                self.__freezeTask(
                    taskId,
                    "Unknown executor %s type" % eType,
                    eType=eType,
                    freezeTime=0,
                )
                return S_OK()
            self.__log.verbose(
                "Executor type %s has not connected. Forgetting task %s"
                % (eType, taskId)
            )
            return self.removeTask(taskId)
    ...
    def __getNextExecutor(self, taskId):
        try:
            ...
            self.__log.error(
                "Task was deleted prematurely while being dispatched", "%s" % taskId
            )
            return S_ERROR(msg)
        try:
            result = self.__cbHolder.cbDispatch(
                taskId, eTask.taskObj, tuple(eTask.pathExecuted)
            )
        except BaseException:
    ...
    def __taskReceived(self, taskId, eId):
        ...
        if not self.__states.removeTask(taskId, eId):
            self.__log.info(
                "Executor %s says it's processed task %s but it didn't have it"
                % (eId, taskId)
            )
            return S_OK()
        if eTask.eType not in self.__idMap[eId]:
            ...
            self.__log.error(
                "Executor type invalid. Redoing task",
                "Type %s, Task %s" % (eId, taskId),
            )
    ...
    def freezeTask(self, eId, taskId, freezeTime, taskObj=False):
        ...
        try:
            ...
            self.__log.error(
                "Task seems to have been removed while being processed!", "%s" % taskId
            )
            self.__sendTaskToExecutor(eId, eType)
            return S_OK()
        self.__freezeTask(
            taskId,
            "Freeze request by %s executor" % eType,
            eType=eType,
    ...
    def retryTask(self, eId, taskId):
        ...
        if not self.__states.removeTask(taskId, eId):
            ...
            return S_OK()
        self.__log.verbose(
            "Executor %s did NOT process task %s, retrying" % (eId, taskId)
        )
        try:
            self.__tasks[taskId].retries += 1
        except KeyError:
            self.__log.error(
                "Task seems to have been removed while waiting for retry!",
                "%s" % taskId,
            )
            return S_OK()
        return self.__dispatchTask(taskId)
    ...
    def __fillExecutors(self, eType, defrozeIfNeeded=True):
        ...
        while eId:
            ...
            if not result["OK"]:
                self.__log.error(
                    "Could not send task to executor", "%s" % result["Message"]
                )
            else:
                if not result["Value"]:
    ...
    def __msgTaskToExecutor(self, taskId, eId, eType):
        ...
        try:
            result = self.__cbHolder.cbSendTask(
                taskId, self.__tasks[taskId].taskObj, eId, eType
            )
        except BaseException:
