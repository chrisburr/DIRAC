...
def getSiteSEMapping(gridName="", withSiteLocalSEMapping=False):
    ...
    if gridName:
        mapping = dict(
            (site, mapping[site]) for site in mapping if siteGridName(site) == gridName
        )
    return S_OK(mapping)
    ...
    return S_OK(
        dict(
            (
                se,
                getSitesForSE(
                    se, gridName=gridName, withSiteLocalSEMapping=withSiteLocalSEMapping
                ).get("Value", []),
            )
...
def getSEsForSite(siteName, withSiteLocalSEMapping=False):
    ...
    result = DMSHelpers().getSEsForSite(
        siteName, connectionLevel="DOWNLOAD" if withSiteLocalSEMapping else "LOCAL"
    )
    if not result["OK"]:
        return S_OK([])
    return result
