...
class ObjectLoader(object):
    ...
    def __recurseImport(
        self, modName, parentModule=None, hideExceptions=False, fullName=False
    ):
        ...
        return self.__recurseImport(
            modName[1:], impModule, hideExceptions=hideExceptions, fullName=fullName
        )
    ...
    def loadObject(self, importString, objName=False, hideExceptions=False):
        ...
        try:
            ...
            return S_ERROR(
                DErrno.EIMPERR,
                "%s does not contain a %s object" % (importString, objName),
            )
        ...
        for rootModule in self.__rootModules:
            ...
            for _modLoader, modName, isPkg in pkgutil.walk_packages(
                ...
                result = self.__recurseImport(
                    modName, parentModule=parentModule, fullName=fullName
                )
                if not result["OK"]:
                    if continueOnError:
                        gLogger.error(
                ...
                try:
                    ...
                    gLogger.warn(
                        "%s does not contain a %s object" % (fullName, modName)
                    )
                    continue
