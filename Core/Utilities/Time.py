...
def timeThis(method):
    ...
    def timed(*args, **kw):
        ...
        print(
            "%s Exec time ===> function %r %s -> %2.2f sec"
            % (pre, method.__name__, argsLen, te - ts)
        )
        return result
...
def time(myDateTime=None):
    ...
    if not isinstance(myDateTime, _dateTimeType):
        myDateTime = dateTime()
    return myDateTime - datetime.datetime(
...
def fromString(myDate=None):
    ...
    if StringTypes.__contains__(type(myDate)):
        if myDate.find(" ") > 0:
            ...
            try:
                return datetime.date(
                    int(dateTuple[0]), int(dateTuple[1]), int(dateTuple[2])
                )
            except BaseException:
                return None
...
class timeInterval:
    ...
    def __init__(self, initialDateTime, intervalTimeDelta):
        ...
        """
        if not isinstance(initialDateTime, _dateTimeType) or not isinstance(
            intervalTimeDelta, _timeType
        ):
            self.__error = True
            return None
        self.__error = False
