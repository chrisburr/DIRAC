...
# call stack
DIRAC_DEBUG_DENCODE_CALLSTACK = bool(
    os.environ.get("DIRAC_DEBUG_DENCODE_CALLSTACK", False)
)
    ...
    * all the method in DENCODE_WARNING_IGNORED_METHODS (see ignoreEncodeWarning)
    ...
    for frameRecord in reversed(frames):
        ...
        # Else, if we are answering an RPC call
            ...
            if (
                funcName in DENCODE_WARNING_IGNORED_METHODS
                and className in DENCODE_WARNING_IGNORED_METHODS[funcName]
            ):
                return
            else:
...
def encodeDateTime(oValue, eList):
    ...
    if isinstance(oValue, _dateTimeType):
        ...
        raise Exception(
            "Unexpected type %s while encoding a datetime object" % str(type(oValue))
        )
...
def decodeDateTime(data, i):
    ...
    if dataType == "a":
        ...
        raise Exception(
            "Unexpected type %s while decoding a datetime object" % dataType
        )
    return (dtObject, i)
