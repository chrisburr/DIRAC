...
class Legend(object):
    ...
    def getLegendSize(self):
        ...
        legend_text_padding = self.prefs.get(
            "legend_text_padding", self.prefs["text_padding"]
        )
        if legend_position in ["right", "left"]:
            ...
            nColumns = min(
                self.prefs["legend_max_columns"], int(legend_width / self.column_width)
            )
            nLabels = len(self.labels)
            ...
            legend_height = min(
                legend_height, (nRows * (self.text_size + text_padding) + text_padding)
            )
    ...
    def __get_column_width(self):
        ...
        self.column_width = (
            columnwidth
            if columnwidth <= self.prefs["legend_width"]
    ...
    def draw(self):
        ...
        nLabels = len(self.labels)
        nColumns = min(
            self.prefs["legend_max_columns"], int(ax_xsize / self.column_width)
        )
