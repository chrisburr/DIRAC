...
class BarGraph(PlotBase):
    ...
    def __init__(self, data, ax, prefs, *args, **kw):
        ...
        if "span" in self.prefs:
            ...
            if self.gdata.key_type == "time":
                ...
                self.width = (max(self.gdata.all_keys) - min(self.gdata.all_keys)) / (
                    nKeys - 1
                )
    ...
    def draw(self):
        ...
        if "starttime" in self.prefs and "endtime" in self.prefs:
            start_plot = date2num(
                datetime.datetime.fromtimestamp(to_timestamp(self.prefs["starttime"]))
            )
            end_plot = date2num(
                datetime.datetime.fromtimestamp(to_timestamp(self.prefs["endtime"]))
            )
