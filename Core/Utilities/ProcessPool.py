...
class ProcessTask(object):
    ...
    def hasCallback(self):
        ...
        """
        return (
            self.__resultCallback or self.__exceptionCallback or self.__usePoolCallbacks
        )
...
class ProcessPool(object):
    ...
    def getNumWorkingProcesses(self):
        ...
        try:
            counter = len(
                [
                    pid
                    for pid, worker in self.__workersDict.iteritems()
                    if worker.isWorking()
                ]
            )
        finally:
    ...
    def __spawnNeededWorkingProcesses(self):
        ...
        while (
            self.hasPendingTasks()
            and self.getNumIdleProcesses() == 0
            and len(self.__workersDict) < self.__maxSize
        ):
            if self.__draining or self.__stopEvent.is_set():
                return
            self.__spawnWorkingProcess()
    ...
    def queueTask(self, task, blocking=True, usePoolCallbacks=False):
        ...
        if not isinstance(task, ProcessTask):
            raise TypeError(
                "Tasks added to the process pool must be ProcessTask instances"
            )
        ...
        while True:
            if (
                not log.debug(
                    ...
                    % (self.__resultsQueue.qsize(), processed)
                )
                and processed == 0
                and self.__resultsQueue.qsize()
            ):
                log.debug(
                    "Process results, queue size = %d" % self.__resultsQueue.qsize()
                )
            start = time.time()
            self.__cleanDeadProcesses()
            ...
            if not self.__pendingQueue.empty():
                self.__spawnNeededWorkingProcesses()
                log.debug(
                    "__spawnNeededWorkingProcesses", "t=%.2f" % (time.time() - start)
                )
            time.sleep(0.1)
            ...
            try:
                ...
                if task.usePoolCallbacks():
                    if self.__poolExceptionCallback and task.exceptionRaised():
                        self.__poolExceptionCallback(
                            task.getTaskID(), task.taskException()
                        )
