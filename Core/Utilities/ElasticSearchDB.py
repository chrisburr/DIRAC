...
class ElasticSearchDB(object):
    ...
    def update(self, index, query=None, updateByQuery=True, id=None):
        ...
        sLog.debug(
            "Updating %s with %s, updateByQuery=%s, id=%s"
        ...
        try:
            if updateByQuery:
                ...
                esDSLQueryResult = self.client.index(
                    index=index, doc_type="_doc", body=query, id=id
                )
            return S_OK(esDSLQueryResult)
        except RequestError as re:
            return S_ERROR(re)
    ...
    def createIndex(self, indexPrefix, mapping=None, period="day"):
        ...
        if period is not None:
            fullIndex = self.generateFullIndexName(
                indexPrefix, period
            )  # we have to create an index each day...
        else:
        ...
        try:
            ...
            try:
                self.client.indices.create(
                    index=fullIndex, body={"mappings": mapping}
                )  # ES7
            except RequestError as re:
                if re.error == "mapper_parsing_exception":
                    self.client.indices.create(
                        index=fullIndex, body={"mappings": {"_doc": mapping}}
                    )  # ES6
            return S_OK(fullIndex)
        except Exception as e:  # pylint: disable=broad-except
    ...
    def index(self, indexName, body=None, docID=None):
        ...
        try:
            res = self.client.index(
                index=indexName, doc_type="_doc", body=body, id=docID
            )
    ...
    def bulk_index(self, indexPrefix, data=None, mapping=None, period="day"):
        ...
        docs = []
        for row in data:
            body = {"_index": indexName, "_source": {}, "_type": "_doc"}
            body["_source"] = row
    ...
    def getUniqueValue(self, indexName, key, orderBy=False):
        ...
        if orderBy:
            query.aggs.bucket(
                key, "terms", field=key, size=self.RESULT_SIZE, order=orderBy
            ).metric(key, "cardinality", field=key)
        else:
            query.aggs.bucket(key, "terms", field=key, size=self.RESULT_SIZE).metric(
