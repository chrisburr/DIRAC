...
try:
    MySQLdb.server_init(
        ["--defaults-file=/opt/dirac/etc/my.cnf", "--datadir=/opt/mysql/db"], ["mysqld"]
    )
...
class MySQL(object):
    ...
    class ConnectionPool(object):
        ...
        def __getWithRetry(self, dbName, totalRetries, retriesLeft):
            ...
            if lastName != dbName:
                try:
                    ...
                    if retriesLeft > 0:
                        return self.__getWithRetry(
                            dbName, totalRetries, retriesLeft - 1
                        )
                    return S_ERROR(
                        DErrno.EMYSQL, "Could not select db %s: %s" % (dbName, excp)
                    )
                try:
            ...
            try:
                ...
                if len(self.__spares) < self.__maxSpares:
                    ...
                    try:
                        ...
                        gLogger.warn(
                            "ProgrammingError exception while closing MySQL connection: %s"
                            % exc
                        )
                    except Exception as exc:
                        gLogger.warn(
                            "Exception while closing MySQL connection: %s" % exc
                        )
            except KeyError:
                pass
        ...
        def transactionStart(self, dbName):
            ...
            try:
                return S_OK(
                    self.__execute(conn, "START TRANSACTION WITH CONSISTENT SNAPSHOT")
                )
            except MySQLdb.MySQLError as excp:
        ...
        def transactionRollback(self, dbName):
            ...
            try:
                ...
                return S_ERROR(
                    DErrno.EMYSQL, "Could not rollback transaction: %s" % excp
                )
    ...
    def _except(self, methodName, x, err, cmd=""):
        ...
        try:
            ...
            self.log.error(
                "%s (%s): %s" % (methodName, self._safeCmd(cmd), err),
                "%d: %s" % (e.args[0], e.args[1]),
            )
            return S_ERROR(
                DErrno.EMYSQL, "%s: ( %d: %s )" % (err, e.args[0], e.args[1])
            )
        except Exception as e:
            self.log.error(
                "%s (%s): %s" % (methodName, self._safeCmd(cmd), err), repr(e)
            )
    ...
    def __escapeString(self, myString):
        ...
        try:
            ...
            for func in ["TIMESTAMPDIFF", "TIMESTAMPADD"]:
                if myString.strip().startswith(
                    "%s(" % func
                ) and myString.strip().endswith(")"):
                    args = (
                        myString.strip()[:-1]
                        .replace("%s(" % func, "")
                        .strip()
                    ...
                    return S_ERROR(
                        DErrno.EMYSQL, "__escape_string: Could not escape string"
                    )
            ...
            return self._except(
                "__escape_string", x, "Could not escape string", myString
            )
    ...
    def _transaction(self, cmdList, conn=None):
        ...
        if not isinstance(cmdList, list):
            return S_ERROR(
                DErrno.EMYSQL,
                "_transaction: wrong type (%s) for cmdList" % type(cmdList),
            )
    ...
    def _createViews(self, viewsDict, force=False):
        ...
        if force:
            ...
            for viewName, viewDict in viewsDict.iteritems():
                ...
                viewQuery = [
                    "CREATE OR REPLACE VIEW `%s`.`%s` AS" % (self.__dbName, viewName)
                ]

                columns = ",".join(
                    [
                        "%s AS %s" % (colDef, colName)
    ...
    def _createTables(self, tableDict, force=False):
        ...
        if not isinstance(tableDict, dict):
            return S_ERROR(
                DErrno.EMYSQL,
                "Argument is not a dictionary: %s( %s )" % (type(tableDict), tableDict),
            )
        ...
        if tableList:
            return S_ERROR(
                DErrno.EMYSQL, "Recursive Foreign Keys in %s" % ", ".join(tableList)
            )
            ...
            for table in tableList:
                ...
                if "PrimaryKey" in thisTable:
                    if isinstance(thisTable["PrimaryKey"], six.string_types):
                        ...
                        cmdList.append(
                            "PRIMARY KEY ( %s )"
                            % ", ".join(
                                ["`%s`" % str(f) for f in thisTable["PrimaryKey"]]
                            )
                        )
                ...
                if "UniqueIndexes" in thisTable:
                    ...
                    for index in indexDict:
                        indexedFields = "`, `".join(indexDict[index])
                        cmdList.append(
                            "UNIQUE INDEX `%s` ( `%s` )" % (index, indexedFields)
                        )
                if "ForeignKeys" in thisTable:
                    thisKeys = thisTable["ForeignKeys"]
                    for key, auxTable in thisKeys.iteritems():
    ...
    def _getFields(
        ...
        if inFields is not None:
            try:
                condDict.update(
                    [(inFields[k], inValues[k]) for k in range(len(inFields))]
                )
            except Exception as x:
    ...
    def getCounters(
        ...
        cmd = "SELECT %s, COUNT(*) FROM %s %s GROUP BY %s ORDER BY %s" % (
            attrNames,
            table,
            cond,
            attrNames,
    ...
    def getDistinctAttributeValues(
        ...
        cmd = "SELECT DISTINCT( %s ) FROM %s %s ORDER BY %s" % (
            attributeName,
            table,
            cond,
            attributeName,
    ...
    def getFields(
        ...
        return self._query(
            "SELECT %s FROM %s %s" % (quotedOutFields, table, condition), conn
        )
    ...
    def updateFields(
        ...
        updateString = ",".join(
            [
                "%s = %s" % (_quotedList([updateFields[k]]), updateValues[k])
                for k in range(len(updateFields))
            ]
        )

        return self._update(
            "UPDATE %s SET %s %s" % (table, updateString, condition), conn
        )

    #############################################################################
    def insertFields(
        self, tableName, inFields=None, inValues=None, conn=None, inDict=None
    ):
        """
        Insert a new row in "tableName" assigning the values "inValues" to the
        fields "inFields".
        ...
        return self._update(
            "INSERT INTO %s %s VALUES %s" % (table, inFieldString, inValueString), conn
        )
