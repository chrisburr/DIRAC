...
class ThreadPool(threading.Thread):
    def __init__(
        self, iMinThreads, iMaxThreads=0, iMaxQueuedRequests=0, strictLimits=True
    ):
    ...
    def __spawnWorkingThread(self):
        self.__workingThreadsList.append(
            WorkingThread(self.__pendingQueue, self.__resultsQueue)
        )
    ...
    def __spawnNeededWorkingThreads(self):
        ...
        while (
            self.__countWaitingThreads() == 0
            and len(self.__workingThreadsList) < self.__maxThreads
        ):
            self.__spawnWorkingThread()
    ...
    def queueJob(self, oTJob, blocking=True):
        if not isinstance(oTJob, ThreadedJob):
            raise TypeError(
                "Jobs added to the thread pool must be ThreadedJob instances"
            )
        try:
