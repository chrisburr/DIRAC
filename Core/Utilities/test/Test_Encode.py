...
from DIRAC.Core.Utilities.DEncode import (
    encode as disetEncode,
    decode as disetDecode,
    g_dEncodeFunctions,
)
from DIRAC.Core.Utilities.JEncode import (
...
nestedStrategy = recursive(
    initialStrategies, lambda x: lists(x) | dictionaries(text(), x) | tuples(x)
)

# This strategy does not return tuples
nestedStrategyJson = recursive(
    initialJsonStrategies, lambda x: lists(x) | dictionaries(text(), x)
)
...
@fixture(
    scope="function", params=enc_dec_imp_without_json, ids=enc_dec_ids_without_json
)
    ...
    def __eq__(self, other):
        return all(
            [
                ...
                for attr in self._attrToSerialize
            ]
        )
