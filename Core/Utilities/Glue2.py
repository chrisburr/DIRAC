...
def getGlue2CEInfo(vo, host):
    ...
    filt = (
        "(&(objectClass=GLUE2Policy)(|(GLUE2PolicyRule=VO:%s)(GLUE2PolicyRule=vo:%s)))"
        % (vo, vo)
    )
    polRes = __ldapsearchBDII(
        filt=filt, attr=None, host=host, base="o=glue", selectionString="GLUE2"
    )
    ...
    for policyValues in polRes:
        ...
        siteName = (
            policyValues["attr"]["dn"].split("GLUE2DomainID=")[1].split(",", 1)[0]
        )
        listOfSitesWithPolicies.add(siteName)
        if shareID is None:  # policy not pointing to ComputingInformation
            sLog.debug(
                "Policy %s does not point to computing information" % (policyID,)
            )
            continue
    ...
    for shareInfo in shareRes["Value"]:
        ...
        if "GLUE2ComputingShare" not in shareInfo["objectClass"]:
            sLog.debug(
                "Share %r is not a ComputingShare: \n%s" % (shareID, pformat(shareInfo))
            )
            continue
    ...
    if not siteInfo["OK"]:
        sLog.error(
            "Could not get CE info for", "%s: %s" % (shareID, siteInfo["Message"])
        )
        return siteInfo
    ...
    for siteName, shareInfoDicts in shareInfoLists.items():
        ...
        for shareInfoDict in shareInfoDicts:
            ...
            ceInfo["MaxWaitingJobs"] = shareInfoDict.get(
                "GLUE2ComputingShareMaxWaitingJobs", "-1"
            )  # This is not used
            ceInfo["Queues"] = {}
            queueInfo = {}
            ...
            queueInfo["GlueCEPolicyMaxCPUTime"] = str(
                int(shareInfoDict.get("GLUE2ComputingShareMaxCPUTime", 86400)) / 60
            )
            queueInfo["GlueCEPolicyMaxWallClockTime"] = str(
                int(shareInfoDict.get("GLUE2ComputingShareMaxWallTime", 86400)) / 60
            )
            ...
            try:
                ...
                if numberOfProcs != maxNOPfromGLUE:
                    sLog.info(
                        "Limited NumberOfProcessors for",
                        "%s from %s to %s" % (siteName, maxNOPfromGLUE, numberOfProcs),
                    )
            except ValueError:
                sLog.error(
            ...
            try:
                if not shareEndpoints and shareInfoDict["GLUE2ShareID"].startswith(
                    "urn:ogf"
                ):
                    exeInfo = dict(exeInfo)  # silence pylint about tuples
                    queueInfo["GlueCEImplementationName"] = "ARC"
                    managerName = (
                        exeInfo.pop("MANAGER", "").split(" ", 1)[0].rsplit(":", 1)[1]
                    )
                    managerName = (
                        managerName.capitalize()
                        if managerName == "condor"
                        else managerName
                    )
                    ...
                    ceName = (
                        shareInfoDict["GLUE2ShareID"]
                        .split("ComputingShare:")[1]
                        .split(":")[0]
                    )
                    cesDict.setdefault(ceName, {})
                    existingQueues = dict(cesDict[ceName].get("Queues", {}))
...
def __getGlue2ExecutionEnvironmentInfoForSite(sitename, foreignKeys, exeInfos):
    ...
    exeInfos = [
        ...
        if exeInfo["attr"]["GLUE2ResourceID"] in foreignKeys
    ]
    # take the CE with the lowest MainMemory
    exeInfo = sorted(
        exeInfos,
        key=lambda k: int(k["attr"]["GLUE2ExecutionEnvironmentMainMemorySize"]),
    )
    if not exeInfo:
        sLog.error(
            "SCHEMA PROBLEM: Did not find execution info for site",
            sitename + " and keys: " + " ".join(foreignKeys),
        )
        return S_OK()
