    ...
    def wrapped_fcn(*args, **kwargs):
        ...
        if (userName or userDN) and userGroup:
            ...
            if not proxyResults["OK"]:
                return proxyResults
            originalUserProxy, useServerCertificate, executionLock = proxyResults[
                "Value"
            ]
            ...
            try:
                ...
                _restoreProxyState(
                    originalUserProxy, useServerCertificate, executionLock
                )
        else:
...
class UserProxy(object):
    ...
    def __exit__(self, _type, _value, _traceback):
        ...
        if self.result["OK"] and self.result["Value"]:
            originalUserProxy, useServerCertificate, executionLock = self.result[
                "Value"
            ]
...
def _putProxy(
    ...
    executionLock = (
        LockRing().getLock("_UseUserProxy_", recursive=True)
    ...
    os.environ["X509_USER_PROXY"], originalUserProxy = result["Value"], os.environ.get(
        "X509_USER_PROXY"
    )
...
def _restoreProxyState(
    originalUserProxy=None, useServerCertificate=None, executionLock=None
):
