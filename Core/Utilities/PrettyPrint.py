...
def printTable(
    fields, records, sortField="", numbering=True, printOut=True, columnSeparator=" "
):
    ...
    for rec in records:
        if nFields != len(rec):
            out = "Incorrect data structure to print, nFields %d, nRecords %d" % (
                nFields,
                len(rec),
            )
            if printOut:
        ...
        for record in recordList:
            if isinstance(record[i], list):
                ...
                if isinstance(fieldValue, list):
                    fieldWidths[i] = max(
                        max(len(item) for item in fieldValue), fieldWidths[i]
                    )
                else:
    ...
    for i in range(nFields):
        ...
        length = __writeField(
            stringBuffer, fieldList[i], fieldWidths[i], columnSeparator, lastColumn
        )
        topLength += length
    stringBuffer.write("\n")
    if columnSeparator == " ":
        ...
        if listMode:
            ...
            for ll in range(1, listMode):
                ...
                for i, (fieldValue, fieldWidth) in enumerate(zip(record, fieldWidths)):
                    ...
                    __writeField(
                        stringBuffer, value, fieldWidth, columnSeparator, lastColumn
                    )
