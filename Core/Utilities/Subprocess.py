...
class Watchdog(object):
    ...
    def __call__(self, timeout=0):
        ...
        if timeout:
            ...
            ret = {
                "OK": False,
                "Message": "Timeout after %s seconds" % timeout,
                "Value": (1, "", ""),
            }
        try:
...
class Subprocess(object):
    ...
    def killChild(self, recursive=True):
        ...
        if recursive:
            for gcpid in getChildrenPIDs(
                self.childPID, lambda cpid: os.kill(cpid, signal.SIGSTOP)
            ):
                try:
    ...
    def pythonCall(self, function, *stArgs, **stKeyArgs):
        ...
        if pid == 0:
            ...
            try:
                if len(readSeq) == 0:
                    self.log.debug(
                        "Timeout limit reached for pythonCall", function.__name__
                    )
                    self.__killPid(pid)
    ...
    def __readFromFile(self, fd, baseLength):
        ...
        try:
            ...
            while fd in select.select([fd], [], [], 1)[0]:
                ...
                if len(dataString) + baseLength > self.bufferLimit:
                    self.log.error(
                        "DataString is getting too long (%s): %s "
                        % (len(dataString), dataString[-10000:])
                    )
                    break
        except Exception as x:
    ...
    def __readFromSystemCommandOutput(self, fd, bufferIndex):
        ...
        if retDict["OK"]:
            ...
            return self.__generateSystemCommandError(
                exitStatus, "%s for '%s' call" % (retDict["Message"], self.cmdSeq)
            )
        ...
        try:
            ...
            while (0, 0) == exitStatus or exitStatus is None:
                ...
                if self.timeout and time.time() - initialTime > self.timeout:
                    ...
                    return self.__generateSystemCommandError(
                        exitStatus,
                        "Timeout (%d seconds) for '%s' call" % (self.timeout, cmdSeq),
                    )
                time.sleep(0.01)
                exitStatus = self.__poll(self.child.pid)
    ...
    def __callLineCallback(self, bufferIndex):
        ...
        nextLineIndex = self.bufferList[bufferIndex][0][
            ...
            try:
                self.callback(
                    ...
                    self.bufferList[bufferIndex][0][
                        self.bufferList[bufferIndex][1] : self.bufferList[bufferIndex][
                            1
                        ]
                        + nextLineIndex
                    ],
                ...
                self.log.exception(
                    "Exception while calling callback function",
...
def systemCall(timeout, cmdSeq, callbackFunction=None, env=None, bufferLimit=52428800):
    ...
    if timeout > 0 and USE_WATCHDOG:
        ...
        result = spObject.systemCall(
            cmdSeq, callbackFunction=callbackFunction, env=env, shell=False
        )
    return result
...
def shellCall(timeout, cmdSeq, callbackFunction=None, env=None, bufferLimit=52428800):
    ...
    if timeout > 0 and USE_WATCHDOG:
        ...
        spObject = Subprocess(timeout, bufferLimit=bufferLimit)
        result = spObject.systemCall(
            cmdSeq, callbackFunction=callbackFunction, env=env, shell=True
        )
    return result
...
def pythonCall(timeout, function, *stArgs, **stKeyArgs):
    ...
    if timeout > 0 and USE_WATCHDOG:
        ...
        pyCall = Watchdog(
            spObject.pythonCall, args=(function,) + stArgs, kwargs=stKeyArgs
        )
        spObject.log.verbose("Subprocess Watchdog timeout set to %d" % timeout)
