...
try:
    ...
    class ListDummy:
        def fromChar(self, inputString, sepChar=","):
            if not (
                isinstance(inputString, six.string_types)
            ...
            return [
                fieldString.strip()
                for fieldString in inputString.split(sepChar)
                if len(fieldString.strip()) > 0
            ]
...
class CFG(object):
    ...
    def createNewSection(self, sectionName, comment="", contents=False):
        ...
        if sectionName == "":
            raise ValueError(
                "Creating a section with empty name! You shouldn't do that!"
            )
        if sectionName.find("/") > -1:
            ...
            return parentSection.createNewSection(
                recDict["levelsBelow"], comment, contents
            )
    ...
    def setOption(self, optionName, value, comment=""):
        ...
        if optionName == "":
            raise ValueError(
                "Creating an option with empty name! You shouldn't do that!"
            )
    ...
    def copyKey(self, oldName, newName):
        ...
        if not result:
            raise KeyError(
                "%s does not exist" % "/".join(List.fromChar(oldName, "/")[:-1])
            )
        oldCfg = result["value"]
        ...
        if oldEnd in oldCfg.__dataDict:
            ...
            if not result:
                raise KeyError(
                    "%s does not exist" % "/".join(List.fromChar(newName, "/")[:-1])
                )
            newCfg = result["value"]
    ...
    def listOptions(self, ordered=True):
        ...
        if ordered:
            return [
                ...
                if isinstance(self.__dataDict[sKey], six.string_types)
            ]
        else:
            return [
                sKey
                for sKey in self.__dataDict.keys()
                if isinstance(self.__dataDict[sKey], six.string_types)
            ]
    ...
    def listSections(self, ordered=True):
        ...
        if ordered:
            return [
                sKey
                for sKey in self.__orderedList
                if not isinstance(self.__dataDict[sKey], six.string_types)
            ]
        else:
            return [
                sKey
    ...
    def isSection(self, key):
        ...
        return key in self.__dataDict and not isinstance(
            self.__dataDict[key], six.string_types
        )
    ...
    def isOption(self, key):
        ...
        return key in self.__dataDict and isinstance(
            self.__dataDict[key], six.string_types
        )
    ...
    def getRecursive(self, path, levelsAbove=0):
        ...
        """
        pathList = [
            dirName.strip() for dirName in path.split("/") if not dirName.strip() == ""
        ]
    ...
    def appendToOption(self, optionName, value):
        ...
        if not result:
            raise KeyError(
                "%s does not exist" % "/".join(List.fromChar(optionName, "/")[:-1])
            )
        cfg = result["value"]
        end = result["levelsBelow"]
    ...
    def serialize(self, tabLevelString=""):
        ...
        for entryName in self.__orderedList:
            ...
            if entryName in self.listSections():
                ...
                cfgString += self.__dataDict[entryName].serialize(
                    "%s%s" % (tabLevelString, indentation)
                )
                cfgString += "%s}\n" % tabLevelString
                ...
                raise ValueError(
                    "Oops. There is an entry in the order which is not a section nor an option"
                )
        return cfgString
    ...
    def mergeWith(self, cfgToMergeWith):
        ...
        for option in cfgToMergeWith.listOptions():
            mergedCFG.setOption(
                option, cfgToMergeWith[option], cfgToMergeWith.getComment(option)
            )
        for section in self.listSections():
            if section in cfgToMergeWith.listSections():
                ...
                mergedCFG.createNewSection(
                    section, cfgToMergeWith.getComment(section), oSectionCFG
                )
            else:
                mergedCFG.createNewSection(
                    section, self.getComment(section), self[section].clone()
                )
        for section in cfgToMergeWith.listSections():
            if section not in self.listSections():
                mergedCFG.createNewSection(
                    section, cfgToMergeWith.getComment(section), cfgToMergeWith[section]
                )
        return mergedCFG
        ...
        for newOption in newOptions:
            ...
            if newOption not in oldOptions:
                ...
                if iPos != self.__orderedList.index(newOption) and not ignoreOrder:
                    ...
                    newerCfg.getComment(newOption) != self.getComment(newOption)
                    and not ignoreComments
                ):
                    modified = True
                if modified:
                    modList.append(
        ...
        for modAction in modList:
            ...
            if action == "addSec":
                if key in self.listSections():
                    return S_ERROR(
                        "Section %s/%s already exists" % (parentSection, key)
                    )
                ...
                if key not in self.listSections():
                    return S_ERROR(
                        "Section %s/%s does not exist" % (parentSection, key)
                    )
                self.deleteKey(key)
            elif action == "modSec":
                if key not in self.listSections():
        ...
        for line in data.split("\n"):
            ...
            for index in range(len(line)):
                if line[index] == "{":
                    ...
                    currentLevel.appendToOption(
                        valueList[0].strip(), ", %s" % "+=".join(valueList[1:]).strip()
                    )
                    currentlyParsedString = ""
                    currentComment = ""
                    break
