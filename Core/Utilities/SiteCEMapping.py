...
def getSites(gridName=None):
    ...
    for grid in gridTypes:
        ...
        if not sites["OK"]:
            gLogger.error(
                "Problem retrieving /Resources/Sites/%s section" % grid,
                sites["Message"],
            )
            return sites
...
def getSiteCEMapping(gridName=None):
    ...
    for candidate in sites["Value"]:
        ...
        candidateCEs = gConfig.getValue(
            "/Resources/Sites/%s/%s/CE" % (grid, candidate), []
        )
        if candidateCEs:
    ...
    for candidate in sites["Value"]:
        ...
        for ce in siteCEs:
            if ce in ceSiteMapping:
                ...
                gLogger.error(
                    "CE %s already has a defined site %s but it is also defined for %s"
                    % (ce, current, candidate)
                )
            else:
...
def getCEsForSite(siteName):
    ...
    return S_OK(
        gConfig.getValue("/Resources/Sites/%s/%s/CE" % (gridName, siteName), [])
    )
