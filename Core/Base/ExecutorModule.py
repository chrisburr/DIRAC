...
class ExecutorModule(object):
    ...
    def _ex_initialize(cls, exeName, loadName):
        ...
        cls.__defaults["WorkDirectory"] = os.path.join(
            cls.__basePath, "work", *exeName.split("/")
        )
        ...
        try:
            ...
            gLogger.exception(
                "Exception while initializing %s" % loadName, lException=excp
            )
    ...
    def _ex_processTask(self, taskId, taskStub):
        ...
        if not result["OK"]:
            self.log.verbose(
                "Task %s: Cannot serialize: %s" % (str(taskId), result["Message"])
            )
            return result
        ...
        if not self.__freezeTime and self.__fastTrackEnabled:
            ...
            if not result["OK"]:
                self.log.error(
                    "FastTrackDispatch failed for job",
