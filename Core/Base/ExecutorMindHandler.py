...
class ExecutorMindHandler(RequestHandler):
    ...
    MSG_DEFINITIONS = {
        "ProcessTask": {
            "taskId": (types.IntType, types.LongType),
    ...
    def __sendTask(self, taskId, taskObj, eId, eType):
        try:
            ...
            gLogger.exception(
                "Exception while executing prepareToSend: %s" % str(excp),
                lException=excp,
            )
        ...
        try:
            ...
            gLogger.exception(
                "Exception while serializing task %s" % taskId, lException=excp
            )
    ...
    def msg_TaskDone(self, msgObj):
        ...
        try:
            ...
            gLogger.exception(
                "Exception while deserializing task %s" % taskId, lException=excp
            )
        ...
        result = self.__eDispatch.taskProcessed(
            self.srv_getTransportID(), msgObj.taskId, taskObj
        )
        if not result["OK"]:
            gLogger.error(
                "There was a problem processing task",
    ...
    def msg_TaskFreeze(self, msgObj):
        ...
        if not result["OK"]:
            return result
        taskObj = result["Value"]
        result = self.__eDispatch.freezeTask(
            self.srv_getTransportID(), msgObj.taskId, msgObj.freezeTime, taskObj
        )
        if not result["OK"]:
            gLogger.error(
                "There was a problem freezing task",
                "%s: %s" % (taskId, result["Message"]),
            )
        return S_OK()
    ...
    def msg_TaskError(self, msgObj):
        ...
        try:
            ...
            gLogger.exception(
                "Exception when processing task %s" % msgObj.taskId, lException=excp
            )
        return S_OK()
