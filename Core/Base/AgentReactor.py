...
class AgentReactor(object):
    ...
    def loadAgentModules(self, modulesList, hideExceptions=False):
        ...
        for agentName in self.__agentModules:
            ...
            try:
                instanceObj = agentData["classObj"](
                    agentName, agentData["loadName"], self.__baseAgentName
                )
                ...
                if not hideExceptions:
                    gLogger.exception(
                        "Can't load agent %s" % agentName, lException=excp
                    )
    ...
    def __finalize(self):
        ...
        for agentName in self.__agentModules:
            try:
                ...
                gLogger.exception(
                    "Failed to execute finalize for Agent: %s" % agentName,
                    lException=excp,
                )
