...
class AgentModule(object):
    ...
    def __init__(self, agentName, loadName, baseAgentName=False, properties={}):
        ...
        (
            self.__moduleProperties["system"],
            self.__moduleProperties["agentName"],
        ) = agentName.split("/")
    ...
    def __getCodeInfo(self):
        ...
        docVar = "__doc__"
        try:
            self.__agentModule = __import__(
                self.__class__.__module__, globals(), locals(), versionVar
            )
    ...
    def am_initialize(self, *initArgs):
        ...
        if not result["OK"]:
            return S_ERROR(
                "Error while initializing %s: %s" % (agentName, result["Message"])
            )
        ...
        if pollingTime > 3600:
            ...
            self.log.notice(
                " Polling time: %s seconds" % self.am_getOption("PollingTime")
            )
    ...
    def am_getShifterProxyLocation(self):
        return os.path.join(
            self.__basePath, str(self.am_getOption("shifterProxyLocation"))
        )
    ...
    def am_getOption(self, optionName, defaultValue=None):
        ...
        for section in (
            self.__moduleProperties["section"],
    ...
    def __initializeMonitor(self):
        ...
        self.monitor.registerActivity(
            "CPU", "CPU Usage", "Framework", "CPU,%", self.monitor.OP_MEAN, 600
        )
        self.monitor.registerActivity(
            "MEM", "Memory Usage", "Framework", "Memory,MB", self.monitor.OP_MEAN, 600
        )
        # Component monitor
    ...
    def am_secureCall(self, functor, args=(), name=False):
        ...
        try:
            ...
            self.log.exception(
                "Agent exception while calling method %s" % name, lException=e
            )
    ...
    def am_go(self):
        ...
        self.log.notice("-" * 40)
        self.log.notice(
            "Starting cycle for module %s" % self.__moduleProperties["fullName"]
        )
        ...
        self.log.notice("-" * 40)
        self.log.notice(
            "Agent module %s run summary" % self.__moduleProperties["fullName"]
        )
        self.log.notice(
            " Executed %s times previously" % self.__moduleProperties["cyclesDone"]
        )
        self.log.notice(" Cycle took %.2f seconds" % elapsedTime)
        averageElapsedTime = (
            self.__moduleProperties["totalElapsedTime"]
