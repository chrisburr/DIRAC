...
class ExecutorReactor(object):
    ...
    class MindCluster(object):
        ...
        def addModule(self, name, exeClass):
            ...
            self.__reconnectSleep = max(
                self.__reconnectSleep, exeClass.ex_getOption("ReconnectSleep")
            )
            self.__reconnectRetries = max(
                self.__reconnectRetries, exeClass.ex_getOption("ReconnectRetries")
            )
        ...
        def connect(self):
            ...
            result = self.__msgClient.connect(
                executorTypes=list(self.__modules.keys()),
        ...
        def __disconnected(self, msgClient):
            ...
            while True:
                ...
                gLogger.notice(
                    "Failed to reconnect. Sleeping for %d seconds"
                    % self.__reconnectSleep
                )
        ...
        def __processTask(self, msgObj):
            ...
            if not result["OK"]:
                return self.__sendExecutorError(
                    eType,
                    taskId,
                    "Can't generate %s message: %s" % (msgName, result["Message"]),
                )
        ...
        def __moduleProcess(self, eType, taskId, taskStub, fastTrackLevel=0):
            ...
            try:
                ...
                gLogger.exception(
                    "Error while processing task %s" % taskId, lException=excp
                )
            ...
            if fastTrackType:
                if fastTrackLevel < 10 and fastTrackType in self.__modules:
                    gLogger.notice(
                        "Fast tracking task %s to %s" % (taskId, fastTrackType)
                    )
                    return self.__moduleProcess(
                        fastTrackType, taskId, taskStub, fastTrackLevel + 1
                    )
                else:
                    gLogger.notice(
                        "Stopping %s fast track. Sending back to the mind" % (taskId)
                    )
    ...
    def go(self):
        for name in self.__executorModules:
            ...
            result = exeClass._ex_initialize(
                name, self.__executorModules[name]["loadName"]
            )
            if not result["OK"]:
                return result
