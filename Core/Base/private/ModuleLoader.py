...
class ModuleLoader(object):
    ...
    def loadModules(self, modulesList, hideExceptions=False):
        ...
        for modName in modulesList:
            ...
            if modName.find("/") > -1:
                gLogger.verbose(
                    "Module %s seems to be a valid name. Try to load it!" % modName
                )
            ...
            if result["OK"]:
                ...
                for modName in result["Value"]:
                    result = self.loadModule(
                        "%s/%s" % (system, modName), hideExceptions=hideExceptions
                    )
                    if not result["OK"]:
                        return result
            ...
            for entry in os.listdir(parentPath):
                ...
                result = self.loadModule(
                    modName, hideExceptions=hideExceptions, parentModule=parentModule
                )
    ...
    def loadModule(self, modName, hideExceptions=False, parentModule=False):
        ...
        if loadGroup:
            gLogger.info(
                "Found load group %s. Will load %s" % (modName, ", ".join(loadGroup))
            )
            for loadModName in loadGroup:
                if loadModName.find("/") == -1:
                    loadModName = "%s/%s" % (modList[0], loadModName)
                result = self.loadModule(
                    loadModName, hideExceptions=hideExceptions, parentModule=False
                )
                if not result["OK"]:
                    return result
            return S_OK()
        ...
        if loadName not in self.__loadedModules:
            ...
            if handlerPath:
                gLogger.info(
                    "Trying to %s from CS defined path %s" % (loadName, handlerPath)
                )
                ...
                if not result["OK"]:
                    return S_ERROR(
                        "Cannot load user defined handler %s: %s"
                ...
                result = self.__recurseImport(
                    modImport, parentModule, hideExceptions=hideExceptions
                )
            else:
                ...
                for rootModule in rootModulesToLook:
                    ...
                    result = self.__recurseImport(
                        importString, hideExceptions=hideExceptions
                    )
                    # Error while loading
                    if not result["OK"]:
                        return result
            ...
            try:
                ...
                gLogger.exception(
                    "%s module does not have a %s class!" % (location, module)
                )
                return S_ERROR("Cannot load %s" % module)
            # Check if it's subclass
            if not issubclass(modClass, self.__superClass):
                return S_ERROR(
                    "%s has to inherit from %s" % (loadName, self.__superClass.__name__)
                )
    ...
    def __recurseImport(self, modName, parentModule=None, hideExceptions=False):
        gLogger.debug(
            "importing recursively %s, parentModule=%s, hideExceptions=%s"
            % (modName, parentModule, hideExceptions)
        )
        ...
        try:
            ...
            strExcp = str(excp)
            if strExcp.find("No module named") == 0 and strExcp.find(modName[0]) == len(
                strExcp
            ) - len(modName[0]):
                return S_OK()
