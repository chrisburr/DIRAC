...
def createClient(serviceName):
    ...
    handlerClassPath = "%sSystem.Service.%s.%s" % (
        systemName,
        handlerModuleName,
        handlerClassName,
    )
    ...
    locations = [
        ...
        if folder != "DIRAC"
        and folder.endswith("DIRAC")
        and os.path.isdir(os.path.join(basepath, folder))
    ]
    ...
    def genFunc(funcName, arguments, handlerClassPath, doc):
        ...
        if arguments and ":param " not in doc:
            parameterDoc = "\n".join(
                ":param %(par)s: %(par)s" % dict(par=par) for par in arguments
            )
            func.__doc__ += "\n\n" + parameterDoc
        return func
    ...
    def addFunctions(clientCls):
        ...
        for location in locations:
            ...
            for node in ast.iter_child_nodes(handlerAst):
                ...
                if not (
                    isinstance(node, ast.ClassDef) and node.name == handlerClassName
                ):
                    continue
                for member in ast.iter_child_nodes(node):
                    ...
                    attrDict[funcName] = genFunc(
                        funcName,
                        arguments,
                        fullHandlerClassPath,
