...
class Parameter(object):
    ...
    def getValueTypeCorrected(self):
        ...
        if self.isTypeString():
            return (
                '"""' + str(self.value).replace('"', r"\"").replace("'", r"\'") + '"""'
            )
        return self.value
...
class ParameterCollection(list):
    ...
    def appendCopy(self, opt, prefix="", postfix=""):
        if isinstance(opt, ParameterCollection):
            for p in opt:
                self.appendOrOverwrite(
                    Parameter(name=prefix + p.getName() + postfix, parameter=p)
                )
        elif isinstance(opt, Parameter):
            self.appendOrOverwrite(
                Parameter(name=prefix + opt.getName() + postfix, parameter=opt)
            )
        else:
            raise TypeError(
                "Can not append object type "
    ...
    def linkUp(self, opt, prefix="", postfix="", objname="self"):
        ...
        if isinstance(opt, ParameterCollection):
            ...
            for s in opt:
                ...
                if par is None:
                    print(
                        "ERROR ParameterCollection.linkUp() can not find parameter with the name=%s"
                        % (s)
                    )
                else:
                    par.link(objname, prefix + p.getName() + postfix)
    ...
    def unlink(self, opt):
        ...
        if isinstance(opt, ParameterCollection):
            ...
            for s in opt:
                ...
                if par is None:
                    print(
                        "ERROR ParameterCollection.unlink() can not find parameter with the name=%s"
                        % (s)
                    )
                else:
                    par.unlink()
    ...
    def remove(self, name_or_ind):
        ...
        if isinstance(name_or_ind, list) and isinstance(name_or_ind[0], str):
            for s in name_or_ind:
                ...
                if par is None:
                    print(
                        "ERROR ParameterCollection.remove() can not find parameter with the name=%s"
                        % (s)
                    )
                else:
    ...
    def find(self, name_or_ind):
        ...
        elif isinstance(name_or_ind, int) or isinstance(
            name_or_ind, long
        ):  # we given index
            ...
            while True:
                for substitute_var in substitute_vars:
                    ...
                    if v_other is not None and not v_other.isLinked():
                        ...
                        print(
                            "Leaving %s variable for dynamic resolution"
                            % substitute_var
                        )
                ...
                if recurrency > recurrency_max:
                    ...
                    print(
                        "ERROR! reached maximum recurrency level",
                        recurrency,
