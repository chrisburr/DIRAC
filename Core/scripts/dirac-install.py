...
class ReleaseConfig(object):
    class CFG:
        ...
        def __parse(self, cfgData, cIndex=0):
            ...
            while cIndex < len(cfgData):
                ...
                if opFound > -1:
                    ...
                    if not childName:
                        raise Exception(
                            "No section name defined for opening in line %s" % numLine
                        )
        ...
        def isSection(self, obList):
            ...
            """
            return (
                self.__exists([ob.strip() for ob in obList.split("/") if ob.strip()])
                == 2
            )
        ...
        def get(self, opName, defaultValue=None):
            ...
            try:
                value = self.__get(
                    [op.strip() for op in opName.split("/") if op.strip()]
                )
            except KeyError:
        ...
        def toString(self, tabs=0):
            ...
            lines = [
                "%s%s = %s" % ("  " * tabs, opName, self.data[opName])
                for opName in self.data
            ]
    ...
    def __loadObjectDefaults(self, rootPath, objectName):
        ...
        if self.globalDefaults.get("%s/SkipDefaults" % basePath, False):
            ...
            defaultsLocation = self.globalDefaults.get(
                "%s/DefaultsLocation" % basePath, ""
            )

        if not defaultsLocation:
            self.__dbgMsg(
                "No defaults file defined for %s %s"
                % (rootPath.lower()[:-1], objectName)
            )
        else:
    ...
    def getModuleVersionFromLocalCfg(self, moduleName):
        ...
        """
        return self.globalDefaults.get(
            "Installations/%s/LocalInstallation/%s" % (self.instName, moduleName), ""
        )
    ...
    def getTarsLocation(self, project, module=None):
        ...
        if module:
            ...
            differntSourceUrl = self.globalDefaults.get(
                "Projects/%s/%s" % (project, module), ""
            )
        ...
        return S_ERROR(
            "Don't know how to find the installation tarballs for project %s" % project
        )
    ...
    def __loadReleaseConfig(
        self, project, release, releaseMode, sourceURL=None, relLocation=None
    ):
        """
        ...
        if release in self.prjRelCFG[project]:
            self.__dbgMsg(
                "Release config for %s:%s has already been loaded" % (project, release)
            )
            return S_OK()
        ...
        if relLocation:
            ...
            if releaseMode:
                try:
                    relcfgLoc = self.globalDefaults.get(
                        "Projects/%s/Releases" % project
                    )
                except KeyError:
    ...
    def dumpReleasesToPath(self):
        ...
        for project in self.prjRelCFG:
            ...
            for release in prjRels:
                ...
                fd = open(
                    os.path.join(
                        cliParams.targetPath, "releases-%s-%s.cfg" % (project, release)
                    ),
                    "w",
                )
    ...
    def __checkCircularDependencies(self, key, routePath=None):
        ...
        for lKey in self.projectsLoadedBy[key]:
            if lKey in routePath:
                ...
                return S_ERROR(
                    "Circular dependency found for %s: %s" % ("%s:%s" % lKey, route)
                )
    ...
    def loadProjectRelease(
        self, releases, project=None, sourceURL=None, releaseMode=None, relLocation=None
    ):
        """
        ...
        for release in releases:
            ...
            result = self.__loadReleaseConfig(
                project, release, releaseMode, sourceURL, relLocation
            )
            if not result["OK"]:
                return result
            relCFG = result["Value"]
            ...
            if initialDeps:
                self.__dbgMsg(
                    ...
                    % (
                        project,
                        release,
                        ", ".join(["%s:%s" % (k, initialDeps[k]) for k in initialDeps]),
                    )
                )
                ...
                for sKey in subDep:
                    ...
                    for pKey in relDeps:
                        if pKey[0] == prj and pKey[1] != vrs:
                            errMsg = (
                                "%s is required with two different versions ( %s and %s ) \
              starting with %s:%s"
                                % (prj, pKey[1], vrs, project, release)
                            )
            ...
            if project in relDeps and relDeps[project] != release:
                errMsg = (
                    "%s:%s requires itself with a different version through dependencies ( %s )"
            ...
            for project, version in relDeps:
                ...
                if modules["OK"]:
                    for dependency in modules["Value"]:
                        ...
                        self.diracBaseModules[dependency]["Version"] = modules["Value"][
                            dependency
                        ]
                        ...
                        if not res["OK"]:
                            ...
                            self.diracBaseModules[dependency]["sourceUrl"] = res[
                                "Value"
                            ][1]
    ...
    def getModulesForRelease(self, release, project=None):
        ...
        if release not in self.prjRelCFG[project]:
            return S_ERROR(
                "Version %s has not been loaded for project %s" % (release, project)
            )
        config = self.prjRelCFG[project][release]
        if not config.isSection("Releases/%s" % release):
            return S_ERROR(
                "Release %s is not defined for project %s" % (release, project)
            )
        # Defined Modules explicitly in the release
        modules = self.getReleaseOption(
            project, release, "Releases/%s/Modules" % release
        )
        if modules:
            dMods = {}
            for entry in [
                entry.split(":") for entry in modules.split(",") if entry.strip()
            ]:  # pylint: disable=no-member
                if len(entry) == 1:
        ...
        if project != "DIRAC":
            for modName in modules:
                if modName.find(project) != 0:
                    return S_ERROR(
                        "Module %s does not start with the name %s" % (modName, project)
                    )
        return S_OK(modules)
    ...
    def getModSource(self, release, modName, project=None):
        ...
        modTpl = [
            field.strip() for field in modLocation.split("|") if field.strip()
        ]  # pylint: disable=no-member
        if len(modTpl) == 1:
    ...
    def getExternalsVersion(self, release=None):
        ...
        try:
            return self.prjRelCFG["DIRAC"][release].get(
                "Releases/%s/Externals" % release
            )
        except KeyError:
            return False
    ...
    def getModulesToInstall(self, release, extensions=None):
        ...
        for project, relVersion in projects:
            try:
                requiredModules = self.prjRelCFG[project][relVersion].get(
                    "RequiredExtraModules"
                )
                requiredModules = [
                    modName.strip()
                    for modName in requiredModules.split("/")
                    if modName.strip()
                ]
            except KeyError:
                requiredModules = []
            ...
            self.__dbgMsg(
                "Discovering modules to install for %s (%s)" % (project, relVersion)
            )
            ...
            try:
                ...
                modNames = [
                    mod.strip() for mod in defaultMods.split(",") if mod.strip()
                ]
            except KeyError:
                modNames = []
            for extension in extensions:
                ...
                if "DIRAC" not in extension:
                    ...
                    if dextension in modVersions:
                        ...
                        extraFound.append(extension)
            modNameVer = [
                "%s:%s" % (modName, modVersions[modName]) for modName in modNames
            ]
            self.__dbgMsg(
                "Modules to be installed for %s are: %s"
                % (project, ", ".join(modNameVer))
            )
        ...
        for modName in extensions:
            if modName.split(":")[0] not in extraFound:
                return S_ERROR(
                    "No module %s defined. You sure it's defined for this release?"
                    % modName
                )
...
def urlretrieveTimeout(url, fileName="", timeout=0):
    ...
    try:
        ...
        try:
            ...
            logWARN(
                "Content-Length parameter not returned, skipping expectedBytes check"
            )
        ...
        if receivedBytes != expectedBytes and expectedBytes > 0:
            logERROR(
                "File should be %s bytes but received %s"
...
def downloadAndExtractTarball(tarsURL, pkgName, pkgVer, checkHash=True, cache=False):
    ...
    postInstallScript = os.path.join(
        cliParams.targetPath, pkgName, "dirac-postInstall.py"
    )
    if os.path.isfile(postInstallScript):
        os.chmod(postInstallScript, executablePerms)
        logNOTICE("Executing %s..." % postInstallScript)
        if os.system(
            "python '%s' > '%s.out' 2> '%s.err'"
            % (postInstallScript, postInstallScript, postInstallScript)
        ):
            logERROR(
                "Post installation script %s failed. Check %s.err"
...
def fixBuildPaths():
    ...
    try:
        ...
        replaceCmd = "grep -rIl '%s' %s | xargs sed -i'.org' 's:%s:%s:g'" % (
            buildPath,
            binaryPath,
            buildPath,
            cliParams.targetPath,
...
def fixPythonShebang():
    ...
    try:
        replaceCmd = (
            "grep -rIl '#!/usr/bin/python' %s/bin |\
     xargs sed -i'.org' 's:#!/usr/bin/python:#!/usr/bin/env python:g'"
            % binaryPath
        )
        os.system(replaceCmd)
    except BaseException:
...
def runExternalsPostInstall():
    ...
    if cliParams.diracOS or cliParams.diracOSVersion:
        ...
        postInstallPath = os.path.join(
            cliParams.targetPath, cliParams.platform, "postInstall"
        )
    ...
    for scriptName in os.listdir(postInstallPath):
        if not scriptName.endswith(postInstallSuffix):
            logDEBUG(
                "%s does not have the %s suffix. Skipping.."
                % (scriptName, postInstallSuffix)
            )
            continue
        ...
        if os.system(
            "'%s' > '%s.out' 2> '%s.err'" % (scriptPath, scriptPath, scriptPath)
        ):
            logERROR(
                "Post installation script %s failed. Check %s.err"
...
def checkPlatformAliasLink():
    ...
    if cliParams.platform in platformAlias:
        os.symlink(
            os.path.join(cliParams.targetPath, platformAlias[cliParams.platform]),
...
def installExternalRequirements(extType):
    ...
    reqScript = os.path.join(
        cliParams.targetPath, "scripts", "dirac-externals-requirements"
    )
    bashrcFile = os.path.join(cliParams.targetPath, "bashrc")
    if cliParams.useVersionsDir:
        bashrcFile = os.path.join(cliParams.basePath, "bashrc")
    if os.path.isfile(reqScript):
        ...
        if os.system('bash -c "source %s; %s"' % (bashrcFile, command)):
            logERROR(
                "Requirements installation script %s failed. Check %s.err"
                % (reqScript, reqScript)
            )
    return True
...
cmdOpts = (
    ...
    (
        "  ",
        "createLink",
        "create version symbolic link from the versions directory. This is equivalent to the \
...
def loadConfiguration():
    ...
    optList, args = getopt.getopt(
        sys.argv[1:], "".join([opt[0] for opt in cmdOpts]), [opt[1] for opt in cmdOpts]
    )
    ...
    releaseConfig = ReleaseConfig(
        instName=cliParams.installation, globalDefaultsURL=cliParams.globalDefaults
    )
    if cliParams.debug:
    ...
    for o, v in optList:
        if o in ("-r", "--release"):
            ...
            cliParams.userEnvVariables = dict(
                zip(
                    [name.split(":::")[0] for name in v.replace(" ", "").split(",")],
    ...
    if cliParams.useVersionsDir:
        ...
        cliParams.targetPath = os.path.join(
            cliParams.targetPath,
            "versions",
            "%s_%s" % (cliParams.release, int(time.time())),
        )
        try:
    ...
    result = releaseConfig.loadProjectRelease(
        cliParams.release, project=cliParams.project, sourceURL=cliParams.installSource
    )
    if not result["OK"]:
        return result
...
def compileExternals(extVersion):
    ...
    buildCmd = os.path.join(
        cliParams.targetPath, "DIRAC", "Core", "scripts", "dirac-compile-externals.py"
    )
    buildCmd = "%s -t '%s' -D '%s' -v '%s' -i '%s'" % (
        buildCmd,
...
def getPlatform():
    ...
    platformPath = os.path.join(
        cliParams.targetPath, "DIRAC", "Core", "Utilities", "Platform.py"
    )
    try:
    ...
    Platform = imp.load_module(
        "Platform", platFD, platformPath, ("", "r", imp.PY_SOURCE)
    )
    platFD.close()
...
def installLCGutils(releaseConfig):
    ...
    if lcgVer:
        ...
        if not downloadAndExtractTarball(
            tarsURL + "/../lcgBundles", "DIRAC-lcg", verString, False, cache=True
        ):
            logERROR(
            ...
            logERROR(
                "Please, check the availability of the LCG software bindings for you \
    ...
    try:
        ...
        if not os.path.exists(bashrcFile):
            ...
            if "X509_CERT_DIR" in os.environ:
                ...
                if os.path.isdir("/etc/grid-security/certificates") and os.listdir(
            ...
            lines.append(
                ...
                % os.path.join(proPath, "etc", "grid-security", "vomsdir")
            )
            lines.extend(
                [
                    ...
                    "export DIRACBIN=%s"
                    % os.path.join("$DIRAC", cliParams.platform, "bin"),
                    "export DIRACSCRIPTS=%s" % os.path.join("$DIRAC", "scripts"),
                    "export DIRACLIB=%s"
                    % os.path.join("$DIRAC", cliParams.platform, "lib"),
                    "export TERMINFO=%s"
            ...
            lines.extend(
                [
                    ...
                    "export GFAL_CONFIG_DIR=%s"
                    % os.path.join("$DIRAC", cliParams.platform, "etc/gfal2.d"),
                    "export  GFAL_PLUGIN_DIR=%s"
            ...
            lines.extend(
                ["# ARC Computing Element", "export ARC_PLUGIN_PATH=$DIRACLIB/arc"]
            )

            # Add the lines required for fork support for xrootd
            lines.extend(["# Fork support for xrootd", "export XRD_RUNFORKHANDLER=1"])
            ...
            if cliParams.userEnvVariables:
                ...
                for envName, envValue in cliParams.userEnvVariables.items():
                    lines.extend(
                        [
                            "( echo $%s | grep -q $%s ) || export %s=$%s:$%s"
                            % (envName, envValue, envName, envName, envValue)
                        ]
                    )
...
def createCshrc():
    ...
    try:
        ...
        if not os.path.exists(cshrcFile):
            lines = [
                "# DIRAC cshrc file, used by clients to set up the environment",
            ...
            lines.extend(
                [
                    "# Gfal2 configuration and plugins",
                    "setenv GFAL_CONFIG_DIR %s"
                    % os.path.join("$DIRAC", cliParams.platform, "etc/gfal2.d"),
                    "setenv  GFAL_PLUGIN_DIR %s"
                    % os.path.join("$DIRACLIB", "gfal2-plugins"),
                ]
            )
            # add DIRACPLAT environment variable for client installations
            ...
            # Add the lines required for ARC CE support
            lines.extend(
                ["# ARC Computing Element", "setenv ARC_PLUGIN_PATH $DIRACLIB/arc"]
            )

            # Add the lines required for fork support for xrootd
            lines.extend(["# Fork support for xrootd", "setenv XRD_RUNFORKHANDLER 1"])
...
def installDiracOS(releaseConfig):
    ...
    tarsURL = cliParams.installSource
    if not tarsURL:
        tarsURL = releaseConfig.getDiracOsLocation(
            useVanillaDiracOS=(diracos.lower() == "diracos")
        )
    if not tarsURL:
        tarsURL = releaseConfig.getTarsLocation("DIRAC")["Value"]
        ...
        if not os.path.exists(bashrcFile):
            ...
            lines.extend(
                [
                    ...
                    "export TERMINFO=%s"
                    % __getTerminfoLocations(
                        os.path.join("$DIRACOS", "usr", "share", "terminfo")
                    ),
                    "export RRD_DEFAULT_FONT=%s"
                    % os.path.join(
                        "$DIRACOS", "usr", "share", "fonts", "DejaVuSansMono-Roman.ttf"
                    ),
                ]
            )
...
def checkoutFromGit(moduleName, sourceURL, tagVersion, destinationDir=None):
    ...
    if os.path.exists(fDirName + "/" + moduleName):
        cmd = "ln -s %s/%s %s" % (
            codeRepo,
            moduleName,
            os.path.join(cliParams.targetPath, moduleName),
        )
    else:
    ...
    if not result["OK"]:
        ...
        if cliParams.modules:
            ...
            for i in cliParams.modules:
                modsOrder.append(i)
                modsToInstall[i] = (
                    cliParams.modules[i]["sourceUrl"],
                    cliParams.modules[i]["Version"],
                )
        else:
    ...
    if not cliParams.externalsOnly:
        ...
        if releaseConfig:
            result = releaseConfig.getModulesToInstall(
                cliParams.release, cliParams.extensions
            )
            if not result["OK"]:
                logERROR(result["Message"])
                sys.exit(1)
        ...
        ddeLocation = os.path.join(
            cliParams.targetPath, "DIRAC", "Core", "scripts", "dirac-deploy-scripts.py"
        )
