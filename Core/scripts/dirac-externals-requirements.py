...
Script.registerSwitch(
    "t:", "type=", "Installation type. 'server' by default.", setInstallType
)
...
for entry in os.listdir(rootPath):
    ...
    if not reqList:
        gLogger.verbose(
            "%s does not have requirements for %s installation" % (entry, instType)
        )
        continue
    for req in reqList:
        reqName = False
        ...
        if reqName not in reqDict:
            ...
            gLogger.notice(
                "Skipping %s, it's already requested by %s"
