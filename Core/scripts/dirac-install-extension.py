...
cmdOpts = (
    ...
    (
        "e:",
        "extensions=",
        "Extensions to install (comma separated). Several -e options can be given",
    ),
...
def loadConfiguration():
    ...
    for opName in ("release", "globalDefaults", "useVersionsDir"):
        ...
        if isinstance(getattr(diracInstall.cliParams, opName), six.string_types):
            ...
            setattr(
                diracInstall.cliParams,
                opName,
                opVal.lower() in ("y", "yes", "true", "1"),
            )
    ...
    if diracInstall.cliParams.useVersionsDir:
        ...
        diracInstall.cliParams.targetPath = os.path.join(
            diracInstall.cliParams.targetPath, "pro"
        )

    diracInstall.logNOTICE(
        "Destination path for installation is %s" % diracInstall.cliParams.targetPath
    )
...
if __name__ == "__main__":
    ...
    ddeLocation = os.path.join(
        diracInstall.cliParams.targetPath,
        "DIRAC",
        "Core",
        "scripts",
