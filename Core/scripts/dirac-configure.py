...
def setServer(optionValue):
    ...
    DIRAC.gConfig.setOptionValue(
        cfgInstallPath("ConfigurationServer"), configurationServer
    )
    return DIRAC.S_OK()
...
Script.registerSwitch(
    "e:", "Extensions=", "Set <extensions> as DIRAC extensions", setExtensions
)
Script.registerSwitch(
    "C:",
    "ConfigurationServer=",
...
Script.registerSwitch(
    "I", "IncludeAllServers", "include all Configuration Servers", setAllServers
)
Script.registerSwitch(
    "n:", "SiteName=", "Set <sitename> as DIRAC Site Name", setSiteName
)
Script.registerSwitch("N:", "CEName=", "Determiner <sitename> from <cename>", setCEName)
Script.registerSwitch("V:", "VO=", "Set the VO name", setVO)

Script.registerSwitch(
    "W:", "gateway=", "Configure <gateway> as DIRAC Gateway for the site", setGateway
)

Script.registerSwitch(
    "U", "UseServerCertificate", "Configure to use Server Certificate", setServerCert
)
Script.registerSwitch(
    "H", "SkipCAChecks", "Configure to skip check of CAs", setSkipCAChecks
)
Script.registerSwitch(
    "D", "SkipCADownload", "Configure to skip download of CAs", setSkipCADownload
)
Script.registerSwitch(
    "M",
    "SkipVOMSDownload",
...
Script.registerSwitch(
    "v", "UseVersionsDir", "Use versions directory", setUseVersionsDir
)

Script.registerSwitch(
    "A:", "Architecture=", "Configure /Architecture=<architecture>", setArchitecture
)
Script.registerSwitch(
    "L:", "LocalSE=", "Configure LocalSite/LocalSE=<localse>", setLocalSE
)
...
if includeAllServers:
    ...
    DIRAC.gConfig.setOptionValue(
        "/DIRAC/Configuration/Servers", ",".join(DIRAC.gConfig.getServersList())
    )
...
for vo in vomsDict:
    ...
    vomsDirPath = os.path.join(
        DIRAC.rootPath, "etc", "grid-security", "vomsdir", voName
    )
    ...
    for vomsHost in vomsDict[vo].get("Servers", {}):
        ...
        try:
            ...
            with open(hostFilePath, "wb") as fd:
                fd.write("%s\n%s\n" % (DN, CA))
            vomsesLines.append(
                '"%s" "%s" "%s" "%s" "%s" "24"' % (voName, vomsHost, port, DN, voName)
            )
