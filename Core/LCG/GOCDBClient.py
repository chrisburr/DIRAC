        ...
        if child.nodeName == "SCOPES":
            for subchild in child.childNodes:
                if subchild.childNodes:
                    handler.setdefault("SCOPES", []).append(
                        subchild.childNodes[0].nodeValue.encode("utf-8")
                    )
            continue
                ...
                if subchild.childNodes:
                    ...
                    for subsubchild in subchild.childNodes:
                        if subsubchild.childNodes:
                            dct[
                                subsubchild.nodeName.encode("utf-8")
                            ] = subsubchild.childNodes[0].nodeValue.encode("utf-8")
...
class GOCDBClient(object):
    ...
    def getStatus(
        self, granularity, name=None, startDate=None, startingInHours=None, timeout=None
    ):
        """
        ...
        if startingInHours is not None:
            ...
            if resXML_startDate is None:
                ...
                resStartDate = self._downTimeXMLParsing(
                    resXML_startDate, granularity, name, startDateMax
                )
    ...
    def getServiceEndpointInfo(self, granularity, entity):
        ...
        """
        assert isinstance(granularity, six.string_types) and isinstance(
            entity, six.string_types
        )
        try:
            ...
            _msg = "Exception getting information for %s %s: %s" % (
                granularity,
                entity,
                e,
            )
    ...
    def getCurrentDTLinkList(self):
        ...
        gResourceDT = self._downTimeXMLParsing(
            gDTPage, "Resource"
    ...
    def getHostnameDowntime(self, hostname, startDate=None, ongoing=False):
        ...
        try:
            response = requests.get(
                "https://goc.egi.eu/gocdbpi/public/?method=get_downtime&topentity="
                + params,
                verify=caPath,
            )
    ...
    def _getServiceEndpointCurlDownload(self, granularity, entity):
        ...
        :params:
          ...
          :attr:`entity` : a string. Actual name of the entity.
        """
        if not isinstance(granularity, six.string_types) or not isinstance(
            entity, six.string_types
        ):
    ...
    def _downTimeXMLParsing(self, dt, siteOrRes, entities=None, startDateMax=None):
        ...
        for dtElement in downtimeElements:
            ...
            try:
                dtDict[
                    str(dtElement.getAttributeNode("PRIMARY_KEY").nodeValue)
                    + " "
                    + elements["ENDPOINT"]
                ] = elements
                dtDict[
        ...
        if startDateMax is not None:
            for dtID in list(dtDict):
                startDateMaxFromKeys = datetime(
                    *time.strptime(
                        dtDict[dtID]["FORMATED_START_DATE"], "%Y-%m-%d %H:%M"
                    )[0:5]
                )
