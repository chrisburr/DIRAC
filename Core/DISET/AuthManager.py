...
class AuthManager(object):
    ...
    def authQuery(self, methodQuery, credDict, defaultProperties=False):
        ...
        # Is it a host?
        if (
            self.KW_EXTRA_CREDENTIALS in credDict
            and credDict[self.KW_EXTRA_CREDENTIALS] == self.KW_HOSTS_GROUP
        ):
    ...
    def getHostNickName(self, credDict):
        ...
        if not retVal["OK"]:
            gLogger.warn(
                "Cannot find hostname for DN %s: %s"
                % (credDict[self.KW_DN], retVal["Message"])
            )
            return False
        credDict[self.KW_USERNAME] = retVal["Value"]
        credDict[self.KW_PROPERTIES] = Registry.getPropertiesForHost(
            credDict[self.KW_USERNAME], []
        )
        return True
    ...
    def getValidPropertiesForMethod(self, method, defaultProperties=False):
        ...
        if authProps:
            self.__authLogger.debug(
                "Method %s has no properties defined using %s" % (method, defaultPath)
            )
            return authProps
        self.__authLogger.debug(
    ...
    def forwardedCredentials(self, credDict):
        ...
        """
        if self.KW_EXTRA_CREDENTIALS in credDict and isinstance(
            credDict[self.KW_EXTRA_CREDENTIALS], (tuple, list)
        ):
            if self.KW_DN in credDict:
                retVal = Registry.getHostnameForDN(credDict[self.KW_DN])
                if retVal["OK"]:
                    hostname = retVal["Value"]
                    if Properties.TRUSTED_HOST in Registry.getPropertiesForHost(
                        hostname, []
                    ):
                        return True
        return False
    ...
    def isUserSuspended(self, credDict):
        ...
        if self.KW_USERNAME not in credDict or self.KW_GROUP not in credDict:
            return False
        suspendedVOList = Registry.getUserOption(
            credDict[self.KW_USERNAME], "Suspended", []
        )
