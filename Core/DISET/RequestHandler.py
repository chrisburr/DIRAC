...
class RequestHandler(object):
    ...
    def _rh_executeAction(self, proposalTuple):
        ...
        result = self.__trPool.send(
            self.__trid, retVal
        )  # this will delete the value from the S_OK(value)
        del retVal
        return result
    ...
    def __doFileTransfer(self, sDirection):
        ...
        if "transfer_%s" % sDirection not in dir(self):
            self.__trPool.send(
                self.__trid, S_ERROR("Service can't transfer files %s" % sDirection)
            )
            return
        ...
        try:
            try:
                ...
                if sDirection == "fromClient":
                    ...
                    fileHelper.setDirection("toClient")
                    uRetVal = self.transfer_listBulk(
                        fileInfo[0], fileInfo[1], fileHelper
                    )
                else:
                ...
                if uRetVal["OK"] and not fileHelper.finishedTransmission():
                    gLogger.error(
                        "You haven't finished receiving/sending the file", str(fileInfo)
                    )
    ...
    def __doRPC(self, method):
        ...
        if not retVal["OK"]:
            raise RequestHandler.ConnectionError(
                "Error while receiving arguments %s %s"
                % (self.srv_getFormattedRemoteCredentials(), retVal["Message"])
            )
        args = retVal["Value"]
    ...
    def __RPCCallFunction(self, method, args):
        ...
        try:
            ...
            gLogger.exception(
                "Uncaught exception when serving RPC",
                "Function %s" % method,
                lException=e,
            )
    ...
    def __checkExpectedArgumentTypes(self, method, args):
        ...
        try:
            ...
            for iIndex in range(min(len(oTypesList), len(args))):
                ...
                if mismatch:
                    sError = (
                        "Type mismatch in parameter %d (starting with param 0) Received %s, expected %s"
    ...
    __connectionCallbackTypes = {"new": [basestring, dict], "connected": [], "drop": []}
    ...
    def _rh_executeConnectionCallback(self, methodName, args=False):
        ...
        try:
            ...
            gLogger.exception(
                "Uncaught exception when serving Connect",
    ...
    def _rh_executeMessageCallback(self, msgObj):
        ...
        try:
            try:
                ...
                gLogger.exception(
                    "Uncaught exception when serving message", methodName, lException=e
                )
    ...
    def __logRemoteQuery(self, method, args):
        ...
        gLogger.notice(
            "Executing action",
            "%s %s(%s)"
            % (self.srv_getFormattedRemoteCredentials(), method, argsString),
        )
    ...
    def __logRemoteQueryResponse(self, retVal, elapsedTime):
        ...
        gLogger.notice(
            "Returning response",
            "%s (%.2f secs) %s"
            % (self.srv_getFormattedRemoteCredentials(), elapsedTime, argsString),
        )
