...
class Service(object):
    ...
    def __init__(self, serviceData):
        ...
        self._authMgr = AuthManager(
            "%s/Authorization" % PathFinder.getServiceSection(serviceData["loadName"])
        )
    ...
    def initialize(self):
        ...
        self._serviceInfoDict = {
            ...
            "csPaths": [
                PathFinder.getServiceSection(svcName) for svcName in self._validNames
            ],
        }
        # Call static initialization function
        try:
            self._handler["class"]._rh__initializeClass(
                dict(self._serviceInfoDict),
                self._lockManager,
                self._msgBroker,
                self._monitor,
            )
    ...
    def _loadHandlerInit(self):
        ...
        try:
            handlerInitMethods.append(
                getattr(self._svcData["moduleObj"], "initialize%s" % handlerName)
            )
    ...
    def _loadActions(self):
            ...
            for attribute in handlerAttributeList:
                ...
                if actionType == "RPC":
                    ...
                    typeAttr = "types_%s_%s" % (
                        Service.SVC_VALID_ACTIONS[actionType],
                        exportedName,
                    )
                    authAttr = "auth_%s_%s" % (
                        Service.SVC_VALID_ACTIONS[actionType],
                ...
                if typeAttr in handlerAttributeList:
                    ...
                    gLogger.verbose(
                        "|- Found type definition %s: %s" % (typeAttr, str(obj))
                    )
        ...
        for actionType in Service.SVC_VALID_ACTIONS:
            ...
            if not referedAction:
                continue
            gLogger.verbose(
                "Action %s is a meta action for %s" % (actionType, referedAction)
            )
            authRules[actionType] = []
            ...
            gLogger.verbose(
                "Meta action %s props are %s" % (actionType, authRules[actionType])
            )
    ...
    def _initMonitoring(self):
        ...
        self._monitor.registerActivity(
            "CPU", "CPU Usage", "Framework", "CPU,%", MonitoringClient.OP_MEAN, 600
        )
        self._monitor.registerActivity(
            "MEM",
            "Memory Usage",
        ...
        self._monitor.registerActivity(
            "MaxFD", "Max File Descriptors", "Framework", "fd", MonitoringClient.OP_MEAN
        )
    ...
    def handleConnection(self, clientTransport):
        ...
        if useThreadPoolExecutor:
            ...
            self._threadPool.generateJobAndQueueIt(
                self._processInThread, args=(clientTransport,)
            )
    ...
    def _instantiateHandler(self, trid, proposalTuple=None):
        ...
        if proposalTuple:
            ...
            clientParams["clientVersion"] = (
                proposalTuple[3] if len(proposalTuple) > 3 else None
            )
    ...
    def _processProposal(self, trid, proposalTuple, handlerObj):
        ...
        if messageConnection:
            ...
            if (
                self._msgBroker.getNumConnections()
