...
class BaseTransport(object):
    ...
    def __init__(self, stServerAddress, bServerMode=False, **kwargs):
        ...
        if "keepAliveLapse" in kwargs:
            try:
                ...
                pass
        self.iListenQueueSize = max(
            self.iListenQueueSize, int(kwargs.get("SocketBacklog", 0))
        )
    ...
    def receiveData(self, maxBufferSize=0, blockAfterKeepAlive=True, idleReceive=False):
        ...
        try:
            ...
            isKeepAlive = (
                self.byteStream.find(BaseTransport.keepAliveMagic, 0, keepAliveMagicLen)
                == 0
            )
            ...
            while iSeparatorPosition == -1 and not isKeepAlive:
                ...
                iSeparatorPosition = self.byteStream.find(":", 0, 10)
                isKeepAlive = (
                    self.byteStream.find(
                        BaseTransport.keepAliveMagic, 0, keepAliveMagicLen
                    )
                    == 0
                )
                # Over the limit?
                if (
                    maxBufferSize
                    and len(self.byteStream) > maxBufferSize
                    and iSeparatorPosition == -1
                ):
