...
class SocketInfoFactory(object):
    ...
    def __socketConnect(self, hostAddress, timeout, retries=2):
        addrs = socket.getaddrinfo(
            hostAddress[0], hostAddress[1], 0, socket.SOCK_STREAM
        )
        errs = []
        for addr in addrs:
    ...
    def __sockConnect(self, hostAddress, sockType, timeout, retries):
        ...
        if timeout:
            ...
            osSocket.settimeout(
                tsocket
            )  # we try to connect 3 times with 1 second timeout
        try:
            ...
            if e.args[0] == "timed out":
                osSocket.close()
                if retries:
                    return self.__sockConnect(
                        hostAddress, sockType, timeout, retries - 1
                    )
                else:
    ...
    def __connect(self, socketInfo, hostAddress):
        ...
        if "proxyChain" in socketInfo.infoDict:
            sessionHash.update(
                "|%s" % socketInfo.infoDict["proxyChain"].dumpAllToString()["Value"]
            )
        ...
        for _ in xrange(1):  # TODO: this retry can be reduced.
            ...
            if not connected:
                return S_ERROR(
                    ...
                    % (hostAddress, ",".join([e for e in errorsList]))
                )
    ...
    def getListeningSocket(
        self, hostAddress, listeningQueueSize=128, reuseAddress=True, **kwargs
    ):
        try:
