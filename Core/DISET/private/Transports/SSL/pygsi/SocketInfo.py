...
class SocketInfo:
    ...
    def __init__(self, infoDict, sslContext=None):
        ...
        if sslContext:
            ...
            if self.infoDict["clientMode"]:
                if (
                    "useCertificates" in self.infoDict
                    and self.infoDict["useCertificates"]
                ):
    ...
    def gatherPeerCredentials(self):
        ...
        if isProxyChain:
            if peerChain.isPUSP()["Value"]:
                identitySubject = peerChain.getCertInChain(-2)[
                    "Value"
                ].getSubjectNameObject()["Value"]
            else:
                identitySubject = peerChain.getIssuerCert()[
                    "Value"
                ].getSubjectNameObject()["Value"]
        else:
            identitySubject = peerChain.getCertInChain(0)[
                "Value"
            ].getSubjectNameObject()["Value"]
        credDict = {}
        credDict = {
            "DN": identitySubject.one_line(),
    ...
    def __getCAStore(self):
        ...
        try:
            if (
                not SocketInfo.__cachedCAsCRLs
                or time.time() - SocketInfo.__cachedCAsCRLsLastLoaded > 900
            ):
                ...
                for fileName in os.listdir(casPath):
                    ...
                    # Try to load CA Cert
                    try:
                        caCert = GSI.crypto.load_certificate(
                            GSI.crypto.FILETYPE_PEM, pemData
                        )
                        if caCert.has_expired():
                            continue
                        caID = (
                            caCert.get_subject().one_line(),
                    ...
                    if (
                        "IgnoreCRLs" not in self.infoDict
                        or not self.infoDict["IgnoreCRLs"]
                    ):
                        # Try to load CRL
                        try:
                            ...
                            if fileName.find(".r0") == len(fileName) - 2:
                                gLogger.exception(
                                    "LOADING %s ,Exception: %s" % (filePath, str(e))
                                )

                gLogger.debug("Loaded %s CAs [%s CRLs]" % (casFound, crlsFound))
                SocketInfo.__cachedCAsCRLs = (
                    [casDict[k][1] for k in casDict],
    ...
    def __createContext(self):
        ...
        if not self.__getValue("skipCACheck", False):
            self.sslContext.set_verify(
                GSI.SSL.VERIFY_PEER | GSI.SSL.VERIFY_FAIL_IF_NO_PEER_CERT,
                None,
                gsiEnable,
            ...
            self.sslContext.set_verify(
                GSI.SSL.VERIFY_NONE, None, gsiEnable
            )  # Demand a certificate
        return S_OK()
