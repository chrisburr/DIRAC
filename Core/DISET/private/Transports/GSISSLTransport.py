...
from DIRAC.Core.DISET.private.Transports.SSL.pygsi.SocketInfoFactory import (
    gSocketInfoFactory,
)
...
class SSLTransport(BaseTransport):
    ...
    def initAsClient(self):
        retVal = gSocketInfoFactory.getSocket(
            self.stServerAddress, **self.extraArgsDict
        )
        if not retVal["OK"]:
            return retVal
        ...
        if not self.oSocket.session_reused():
            gLogger.debug(
                "New session connecting to server at %s" % str(self.stServerAddress)
            )
    ...
    def initAsServer(self):
        ...
        retVal = gSocketInfoFactory.getListeningSocket(
    ...
    def handshake(self):
        ...
        if not self.oSocket.session_reused():
            gLogger.debug(
                "New session connecting from client at %s"
