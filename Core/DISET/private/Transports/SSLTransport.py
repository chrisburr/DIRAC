...
from DIRAC.Core.Security.X509Certificate import (
    X509Certificate,
...
def checkSanity(urlTuple, kwargs):
    ...
    if "useCertificates" in kwargs and kwargs["useCertificates"]:
        ...
        if not isinstance(kwargs["proxyString"], six.string_types):
            gLogger.error(
                "proxyString parameter is not a valid type",
                str(type(kwargs["proxyString"])),
            )
    ...
    if not retVal["OK"]:
        gLogger.error(
            "Can't verify proxy or certificate file",
            "%s:%s" % (certFile, retVal["Message"]),
        )
        ...
        if retVal["Value"]:
            ...
            if notAfter["OK"]:
                ...
                notAfter = "unknown"
            gLogger.error(
                "PEM file has expired",
                "%s is not valid after %s" % (certFile, notAfter),
            )
            return S_ERROR(
                "PEM file %s has expired, not valid after %s" % (certFile, notAfter)
            )
