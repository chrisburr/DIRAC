...
class MessageBroker(object):
    ...
    def __startListeningThread(self):
        threadDead = (
            self.__listeningForMessages
            and self.__listenThread is not None
            and not self.__listenThread.isAlive()
        )
    ...
    def __listenAutoReceiveConnections(self):
        while self.__listeningForMessages:
            ...
            try:
                ...
                gLogger.exception(
                    "Exception while selecting persistent connections", lException=e
                )
                continue
            for sock in inList:
    ...
    def __receiveMsgDataAndQueue(self, trid):
        ...
        if useThreadPoolExecutor:
            ...
            self.__threadPool.generateJobAndQueueIt(
                self.__processIncomingData, args=(trid, result)
            )
        return S_OK()
    ...
    def __processIncomingRequest(self, trid, msg):
        ...
        if "attrs" in msg:
            ...
            if not isinstance(attrs, (tuple, list)):
                return S_ERROR(
                    "Message args has to be a tuple or a list, not %s" % type(attrs)
                )
        else:
            attrs = None
        ...
        try:
            ...
            gLogger.exception(
                "Exception while processing message %s" % msg["name"], lException=e
            )
            return S_ERROR(
                "Exception while processing message %s: %s" % (msg["name"], str(e))
            )
    ...
    def __waitForMessageResponse(self, msgId):
        ...
        while (
            "result" not in respCallback
            and time.time() - respCallback["creationTime"] < 30
        ):
...
def getGlobalMessageBroker():
    ...
    if not gMessageBroker:
        gMessageBroker = MessageBroker(
            "GlobalMessageBroker", transportPool=getGlobalTransportPool()
        )
