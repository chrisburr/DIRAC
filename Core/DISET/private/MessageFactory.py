...
class MessageFactory(object):
    ...
    def createMessage(self, serviceName, msgName, attrs=None):
        ...
        if msgName not in self.__definitions[serviceName]:
            return S_ERROR(
                "Could not find message definition %s for service %s"
                % (msgName, serviceName)
            )
    ...
    def __loadHandler(self, serviceName):
        ...
        loadedObjs = loadObjects(
            "%sSystem/Service" % sysName,
            reFilter=re.compile(r"^%s\.py$" % svcHandlerName),
        )
        if svcHandlerName not in loadedObjs:
            return S_ERROR(
                "Could not find %s for getting messages definition" % serviceName
            )
    ...
    def __loadMessagesForAncestry(self, handlerClass):
        ...
        if not isinstance(msgDefs, dict):
            return S_ERROR(
                "Message definitions for service %s is not a dict"
...
class Message(object):
    ...
    def isOK(self):
        for k in self.__order:
            if k not in self.__values:
                return False
            if self.__fDef[k] is not None and not isinstance(
                self.__values[k], self.__fDef[k]
            ):
                return False
        return True
    ...
    def dumpAttrs(self):
        try:
            return S_OK(
                (self.__waitForAck, tuple(self.__values[k] for k in self.__order))
            )
    ...
    def __setattr__(self, k, v):
        ...
        if self.__fDef[k] is not None and type(v) not in self.__fDef[k]:
            raise AttributeError(
                "%s is to be of type %s for attr %s, and is of type %s"
                % (v, self.__fDef[k], k, type(v))
            )
        self.__values[k] = v
    ...
    def __getattr__(self, k):
        if k not in self.__values:
            if k not in self.__fDef:
                raise AttributeError(
                    "No %s attribute for message %s" % (k, self.__name)
                )
...
def loadObjects(path, reFilter=None, parentClass=None):
    ...
    parentModuleList = ["%sDIRAC" % ext for ext in CSGlobals.getCSExtensions()] + [
        "DIRAC"
    ]
    objectsToLoad = {}
        ...
        for objFile in os.listdir(objDir):
            if reFilter.match(objFile):
                ...
                if pythonClassName not in objectsToLoad:
                    gLogger.info(
                        "Adding to message load queue %s/%s/%s"
    ...
    for pythonClassName in objectsToLoad:
        ...
        try:
            ...
            gLogger.exception(
                "Can't load type %s/%s: %s" % (parentModule, pythonClassName, str(e))
            )
            continue
        ...
        if parentClass and not issubclass(objClass, parentClass):
            gLogger.warn(
                "%s is not a subclass of %s. Skipping" % (objClass, parentClass)
            )
            continue
