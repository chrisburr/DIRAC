...
class FileHelper(object):
    ...
    def networkToDataSink(self, dataSink, maxFileSize=0):
        if "write" not in dir(dataSink):
            return S_ERROR(
                "%s data sink object does not have a write method" % str(dataSink)
            )
        self.__oMD5 = hashlib.md5()
        self.bReceivedEOF = False
        ...
        try:
            ...
            while not self.receivedEOF():
                if maxFileSize > 0 and receivedBytes > maxFileSize:
                    ...
                    return S_ERROR(
                        "Received file exceeded maximum size of %s bytes"
                        % (maxFileSize)
                    )
    ...
    def DataSourceToNetwork(self, dataSource):
        if "read" not in dir(dataSource):
            return S_ERROR(
                "%s data source object does not have a read method" % str(dataSource)
            )
        self.__oMD5 = hashlib.md5()
        iPacketSize = self.packetSize
        self.__fileBytes = 0
    ...
    def __createTar(self, fileList, wPipe, compress, autoClose=True):
        ...
        with tarfile.open(name="Pipe", mode=tarMode, fileobj=filePipe) as tar:
            for entry in fileList:
                tar.add(
                    os.path.realpath(entry), os.path.basename(entry), recursive=True
                )
        if autoClose:
            try:
                filePipe.close()
    ...
    def bulkToNetwork(self, fileList, compress=True, onthefly=True):
        if not onthefly:
            try:
                ...
                return S_ERROR(
                    "Can't create temporary file to pregenerate the bulk: %s" % str(e)
                )
            ...
            thrd = threading.Thread(
                target=self.__createTar, args=(fileList, wPipe, compress)
            )
            thrd.start()
    ...
    def networkToBulk(self, destDir, compress=True, maxFileSize=0):
        ...
        rPipe, wPipe = os.pipe()
        thrd = threading.Thread(
            target=self.__receiveToPipe, args=(wPipe, retList, maxFileSize)
        )
        thrd.start()
        try:
