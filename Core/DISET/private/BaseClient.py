...
class BaseClient(object):
    ...
    def __discoverExtraCredentials(self):
        ...
        * delegation:
        ...
        self.__extraCredentials = (
            self.VAL_EXTRA_CREDENTIALS_HOST if self.__useCertificates else ""
        )
        ...
        delegatedDN = (
            self.kwargs.get(self.KW_DELEGATED_DN) or self.__threadConfig.getDN()
        )
        delegatedGroup = (
    ...
    def __findServiceURL(self):
        ...
        try:
            ...
            return S_ERROR(
                "Cannot get URL for %s in setup %s: %s"
        ...
        try:
            failoverUrlsStr = getServiceFailoverURL(
                self._destinationSrv, setup=self.setup
            )
            if failoverUrlsStr:
        ...
        if (
            len(self.__bannedUrls) > 0 and self.__nbOfUrls > 2
        ):  # when we have multiple services then we can
            ...
            if retVal["OK"]:
                ...
                if found:
                    ...
                    if nexturl:  # an url found which is in different host
                        sURL = nexturl
        gLogger.debug(
            "Discovering URL for service", "%s -> %s" % (self._destinationSrv, sURL)
        )
        return S_OK(sURL)
...
===============================================================""" % (
    ...
    def _connect(self):
        ...
        try:
            ...
            if not retVal["OK"]:
                gLogger.warn(
                    "Issue getting socket:",
                    "%s : %s : %s" % (transport, self.__URLTuple, retVal["Message"]),
                )
                # We try at most __nbOfRetry each URLs
                if self.__retry < self.__nbOfRetry * self.__nbOfUrls - 1:
                    ...
                    if url not in self.__bannedUrls:
                        gLogger.warn(
                            "Non-responding URL temporarily banned", "%s" % url
                        )
                    ...
                    gLogger.info(
                        "Retry connection",
                        ": %d to %s" % (self.__retry, self.serviceURL),
                    )
                    ...
                    if len(self.__bannedUrls) == self.__nbOfUrls:
                        ...
                        self.__retryDelay = (
                            3.0 / self.__nbOfUrls if self.__nbOfUrls > 1 else 2
                        )
                        gLogger.info(
                            "Waiting %f seconds before retry all service(s)"
                            % self.__retryDelay
                        )
    ...
    def _serializeStConnectionInfo(stConnectionInfo):
        ...
        """
        serializedTuple = [
            list(x) if isinstance(x, tuple) else x for x in stConnectionInfo
        ]
    ...
    def _proposeAction(self, transport, action):
        ...
        if not self.__initStatus["OK"]:
            return self.__initStatus
        stConnectionInfo = (
            (self.__URLTuple[3], self.setup, self.vo),
            action,
            self.__extraCredentials,
            DIRAC.version,
        )
        ...
        retVal = transport.sendData(
            S_OK(BaseClient._serializeStConnectionInfo(stConnectionInfo))
        )
        if not retVal["OK"]:
            return retVal
        ...
        if (
            serverReturn["OK"]
            and "Value" in serverReturn
            and isinstance(serverReturn["Value"], dict)
        ):
