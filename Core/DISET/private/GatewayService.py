...
class GatewayService(Service):
        ...
        super(GatewayService, self).__init__(
            {
                ...
                "standalone": True,
                "moduleObj": sys.modules[
                    DIRAC.Core.DISET.private.GatewayService.GatewayService.__module__
                ],
    ...
    def initialize(self):
        ...
        if not self._url:
            return S_ERROR(
                "Could not build service URL for %s" % GatewayService.GATEWAY_NAME
            )
        ...
        if useThreadPoolExecutor:
            ...
            self._threadPool = ThreadPool(
                1, max(0, self._cfg.getMaxThreads()), self._cfg.getMaxWaitingPetitions()
            )
            self._threadPool.daemonize()

        self._msgBroker = MessageBroker(
            "%sMSB" % GatewayService.GATEWAY_NAME, threadPool=self._threadPool
        )
    ...
    def _receiveAndCheckProposal(self, trid):
        ...
        if not retVal["OK"]:
            gLogger.error(
                ...
                "%s %s"
                % (
                    self._createIdentityString(credDict, clientTransport),
                    retVal["Message"],
                ),
            )
    ...
    def __getClientInitArgs(self, trid, proposalTuple):
        ...
        if not result["OK"]:
            gLogger.warn(
                "Could not get proxy for %s: %s" % (idString, result["Message"])
            )
            return result
    ...
    def __requestDelegation(self, clientTransport, credDict):
        ...
        if not retVal["OK"]:
            ...
            return retVal
        gLogger.info(
            "Sending delegation request for %s"
        ...
        if not retVal["OK"]:
            retVal = S_ERROR(
                "Error in receiving delegated proxy: %s" % retVal["Message"]
            )
            clientTransport.sendData(retVal)
            return retVal
    ...
    def _executeAction(self, trid, proposalTuple, clientInitArgs):
        ...
        if not retVal["OK"]:
            ...
            clientTransport.sendData(
                S_ERROR(
                    "Error while receiving file description: %s" % retVal["Message"]
                )
            )
            return
        if actionType == "FileTransfer":
            ...
            gLogger.info(
                "Forwarding %s/%s action to %s for %s"
                % (actionType, actionMethod, targetService, idString)
            )
            retVal = self.__forwardRPCCall(
                targetService, clientInitArgs, actionMethod, retVal["Value"]
            )
        elif actionType == "Connection" and actionMethod == "new":
            gLogger.info(
                "Initiating a messaging connection to %s for %s"
                % (targetService, idString)
            )
            retVal = self._msgForwarder.addClient(
                trid, targetService, clientInitArgs, retVal["Value"]
            )
        else:
            gLogger.warn(
                "Received an invalid %s/%s action from %s"
    ...
    def __forwardFileTransferCall(
        self, targetService, clientInitArgs, method, params, clientTransport
    ):
...
class TransferRelay(TransferClient):
    ...
    def sendDataToService(self, srvMethod, params, data):
        self.infoMsg(
            "Sending header request to %s" % self.getDestinationService(), str(params)
        )
    ...
    def forwardFromClient(self, clientFileHelper, params):
        ...
        return self.sendDataToService(
            "FromClient", (fileId, token, receivedBytes), dataReceived
        )
    ...
    def forwardBulkFromClient(self, clientFileHelper, params):
        ...
        receivedBytes = clientFileHelper.getTransferedBytes()
        return self.sendDataToService(
            "BulkFromClient", (fileId, token, receivedBytes), dataReceived
        )
...
class MessageForwarder(object):
    ...
    def __srvDisconnect(self, srvEndCli):
        ...
        gLogger.info(
            "Service %s disconnected messaging connection"
            % self.__byClient[cliTrid]["srvName"]
        )
    ...
    def cliDisconnect(self, cliTrid):
        if cliTrid not in self.__byClient:
            ...
            return
        gLogger.info(
            "Client to %s disconnected messaging connection"
            % self.__byClient[cliTrid]["srvName"]
        )
    ...
    def msgFromClient(self, cliTrid, msgObj):
        gLogger.info(
            "Message %s to %s service"
            % (msgObj.getName(), self.__byClient[cliTrid]["srvName"])
        )
