...
class MessageClient(BaseClient):
    ...
    def createMessage(self, msgName):
        return self.__msgBroker.getMsgFactory().createMessage(
            self.getServiceName(), msgName
        )
    ...
    def connect(self, **extraParams):
        ...
        try:
            ...
            self.__checkResult(
                transport.sendData(S_OK((self.__uniqueName, self.__connectExtraParams)))
            )
    ...
    def __cbDisconnect(self, trid):
        ...
        if self.__trid != trid:
            gLogger.error(
                "OOps. trid's don't match. This shouldn't happen!",
                "(%s vs %s)" % (self.__trid, trid),
            )
    ...
    def __cbRecvMsg(self, trid, msgObj):
        ...
        for cb in self.__specialCallbacks["msg"]:
            try:
                ...
                if not isReturnStructure(result):
                    gLogger.error(
                        "Callback for message does not return S_OK/S_ERROR",
                        msgObj.getName(),
                    )
                ...
                gLogger.exception(
                    "Exception while processing callbacks", msgObj.getName()
                )
        if msgName not in self.__callbacks:
        ...
        try:
            ...
            if not isReturnStructure(result):
                gLogger.error(
                    "Callback for message does not return S_OK/S_ERROR", msgName
                )
                return S_ERROR("No response")
            return result
        except BaseException:
