...
class ServiceReactor(object):
    ...
    def __createListeners(self):
        for serviceName in self.__services:
            ...
            if protocol not in gProtocolDict:
                return S_ERROR(
                    "Protocol %s is not known for service %s" % (protocol, serviceName)
                )
            self.__listeningConnections[serviceName] = {
                "port": port,
                "protocol": protocol,
            }
            transportArgs = {}
            ...
            transport = gProtocolDict[protocol]["transport"](
                ("", port), bServerMode=True, **transportArgs
            )
            retVal = transport.initAsServer()
            if not retVal["OK"]:
                return S_ERROR(
                    "Cannot start listening connection for service %s: %s"
                    % (serviceName, retVal["Message"])
                )
    ...
    def serve(self):
        ...
        for svcName in self.__listeningConnections:
            gLogger.always(
                "Listening at %s" % self.__services[svcName].getConfig().getURL()
            )
            ...
            for svcName in self.__listeningConnections:
                ...
                for i in range(1, clones):
                    p = multiprocessing.Process(
                        target=self.__startCloneProcess, args=(svcName, i)
                    )
    ...
    def stopAllProcess(self):
        ...
        for process in self.__processes:
            gLogger.info(
                "Stopping: PID=%d, name=%s, parentPid=%d"
                % (process.pid, process.name, process._parent_pid)
            )
            if process.is_alive():
                process.terminate()
