...
NO_LATER_THAN_2050_IN_SEC = int(
    (datetime.strptime("2049-12-31", "%Y-%M-%d") - datetime.now()).total_seconds()
)
...
def test_loadChainFromString(
    cert_content_type,
    get_X509Chain_class,
    indirect=("hostcertcontent", "usercertcontent"),
):
...
def test_getCertList(cert_file, get_X509Chain_class):
    ...
    assert (
        len(x509Chain.getCertList()["Value"]) == x509Chain.getNumCertsInChain()["Value"]
    )
...
def test_getCertInChain(get_proxy):
    ...
    assert certSubject == proxyChain.getCertInChain(certPos=-1)[
        "Value"
    ].getSubjectDN().get("Value")
...
@given(
    lifetime=integers(
        min_value=TWENTY_YEARS_IN_SEC, max_value=NO_LATER_THAN_2050_IN_SEC
    )
)
...
def test_delegation(get_X509Request, get_proxy, diracGroup, lifetime):
    ...
    assert (
        proxyChain.getNumCertsInChain()["Value"] + 1
        == delegatedProxy.getNumCertsInChain()["Value"]
    )
    ...
    assert proxyChain.getDIRACGroup(ignoreDefault=True) == delegatedProxy.getDIRACGroup(
        ignoreDefault=True
    )
    ...
    if "pygsi" not in x509Req.__class__.__module__:
        assert (
            proxyChain.getNotAfterDate()["Value"]
            >= delegatedProxy.getNotAfterDate()["Value"]
        )
