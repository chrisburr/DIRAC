...
class VOMS(BaseSecurity):
    ...
    def getVOMSAttributes(self, proxy, switch="all"):
        ...
        if not result["OK"]:
            return S_ERROR(
                DErrno.EVOMS,
                "Failed to extract info from proxy: %s" % result["Message"],
            )
        ...
        try:
            ...
            if option == "fqan":
                return S_OK(
                    "\n".join(
                        [
                            f.replace("/Role=NULL", "").replace("/Capability=NULL", "")
                            for f in data["fqan"]
                        ]
                    )
                )
    ...
    def getVOMSESLocation(self):
        ...
        if "DIRAC_VOMSES" not in os.environ and "X509_VOMSES" not in os.environ:
            os.environ["X509_VOMSES"] = os.path.join(
    ...
    def setVOMSAttributes(self, proxy, attribute=None, vo=None):
        ...
        if not vo:
            return S_ERROR(
                DErrno.EVOMS,
                "No vo specified, and can't get default in the configuration",
            )
        ...
        if not result["OK"]:
            ...
            return S_ERROR(
                DErrno.EVOMS, "Failed to call voms-proxy-init: %s" % result["Message"]
            )
        ...
        if status:
            ...
            return S_ERROR(
                DErrno.EVOMS,
                "Failed to set VOMS attributes. Command: %s; StdOut: %s; StdErr: %s"
                % (cmd, output, error),
            )
