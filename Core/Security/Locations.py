...
def getHostCertificateAndKeyLocation(specificLocation=None):
    ...
    * Alternative exotic options, with ``prefix`` in  ``server``, ``host``, ``dirac``, ``service``:
    ...
    for fileType, envVar in (
        ("cert", "DIRAC_X509_HOST_CERT"),
        ("key", "DIRAC_X509_HOST_KEY"),
    ):
    ...
    for fileType in ("cert", "key"):
        ...
        retVal = gConfig.getOption(
            "%s/%sFile" % (g_SecurityConfPath, fileType.capitalize())
        )
        if retVal["OK"]:
        ...
        for filePrefix in ("server", "host", "dirac", "service"):
            ...
            for path in paths:
                filePath = os.path.realpath(
                    "%s/%s%s.pem" % (path, filePrefix, fileType)
                )
