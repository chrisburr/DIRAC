    ...
    * You are delegating
    ...
    def __getProxyExtensionList(diracGroup=False, rfcLimited=False):
        ...
        ext = M2Crypto.X509.new_extension(
            "proxyCertInfo", "critical, language:%s" % (policyOID), critical=1
        )
        extStack.push(ext)
        ...
        if diracGroup and isinstance(diracGroup, six.string_types):
            ...
            dGext = M2Crypto.X509.new_extension(
                DIRAC_GROUP_OID, str("ASN1:IA5:%s" % diracGroup)
            )
            extStack.push(dGext)
    ...
    def generateProxyToString(
        ...
        if not proxyKey:
            ...
            proxyKey.assign_rsa(
                M2Crypto.RSA.gen_key(
                    strength, 65537, callback=M2Crypto.util.quiet_genparam_callback
                )
            )
        ...
        res = X509Certificate.generateProxyCertFromIssuer(
            issuerCert, proxyExtensions, proxyKey, lifetime=lifetime
        )
        if not res["OK"]:
            return res
        proxyCert = res["Value"]
    ...
    def getDIRACGroup(self, ignoreDefault=False):
        ...
        if self.isPUSP()["Value"]:
            return self._certList[self.__firstProxyStep - 2].getDIRACGroup(
    ...
    def generateChainFromRequestString(
        ...
        """
        try:
            req = M2Crypto.X509.load_request_string(
                pemData, format=M2Crypto.X509.FORMAT_PEM
            )
        ...
        return self.generateProxyToString(
            lifetime, diracGroup, 1024, limited, req.get_pubkey()
        )
