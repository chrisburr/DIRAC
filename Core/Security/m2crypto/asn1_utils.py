...
def decodeDIRACGroup(m2cert):
    ...
    diracGroupUTF8Str, _rest = der_decode(
        diracGroupOctetString, asn1Spec=asn1char.IA5String()
    )
...
def decodeVOMSExtension(m2cert):
    ...
    vomsExtensionSeqOfSeq, _rest = der_decode(
        vomsExtensionOctetString, asn1Spec=_ACSequenceOfSequence()
    )
    ...
    for rdnName in certAttrInfo["issuer"]["v2Form"]["issuerName"][0]["directoryName"][
        "rdnSequence"
    ]:
    ...
    fqanAttrObj = [
        attrObj
        for attrObj in certAttrInfo["attributes"]
    ...
    #
    fqanObj, _rest = der_decode(
        fqanAttrObj["values"][0], asn1Spec=rfc3281.IetfAttrSyntax()
    )

    # We retrieve the VO and the VOMS server
    voName, _vomsServer, _vomsPort = (
        fqanObj["policyAuthority"][0]["uniformResourceIdentifier"].asOctets().split(":")
    )
    ...
    tagExtensionObj = [
        extObj
        for extObj in certAttrInfo["extensions"]
    ...
    if tagExtensionObj:
        ...
        tagContainersObj, _rest = der_decode(
            tagExtensionObj["extnValue"], asn1Spec=_TagContainers()
        )
