...
class X509Chain(object):
    ...
    def loadChainFromFile(self, chainLocation):
        ...
        try:
            ...
            return S_ERROR(
                DErrno.EOF, "%s: %s" % (chainLocation, repr(e).replace(",)", ")"))
            )
    ...
    def loadKeyFromString(self, pemData, password=False):
        ...
        self.__loadedPKey = False
        try:
            self.__keyObj = crypto.load_privatekey(
                crypto.FILETYPE_PEM, pemData, password
            )
        except Exception as e:
            return S_ERROR(
                DErrno.ECERTREAD,
                "%s (Probably bad pass phrase?)" % repr(e).replace(",)", ")"),
            )
        self.__loadedPKey = True
        return S_OK()
    ...
    def generateProxyToString(
        ...
        if rfc:
            ...
            proxyCert.add_extensions(
                self.__getProxyExtensionList(
                    diracGroup, rfc and not limited, rfc and limited
                )
            )
        else:
        ...
        proxyString = "%s%s" % (
            crypto.dump_certificate(crypto.FILETYPE_PEM, proxyCert),
            crypto.dump_privatekey(crypto.FILETYPE_PEM, proxyKey),
        )
        for i in range(len(self.__certList)):
            proxyString += crypto.dump_certificate(
                crypto.FILETYPE_PEM, self.__certList[i]
            )
    ...
    def generateProxyToFile(
        ...
        """
        retVal = self.generateProxyToString(
            lifeTime, diracGroup, strength, limited, rfc
        )
        if not retVal["OK"]:
            return retVal
        try:
            ...
            return S_ERROR(
                DErrno.EWF, "%s :%s" % (filePath, repr(e).replace(",)", ")"))
            )
        try:
            os.chmod(filePath, stat.S_IRUSR | stat.S_IWUSR)
        except Exception as e:
            return S_ERROR(
                DErrno.ESPF, "%s :%s" % (filePath, repr(e).replace(",)", ")"))
            )
        return S_OK()
    ...
    def __checkProxyness(self):
        self.__hash = False
        self.__firstProxyStep = (
            len(self.__certList) - 2
        )  # -1 is user cert by default, -2 is first proxy step
        self.__isProxy = True
        self.__isRFC = None
    ...
    def getDIRACGroup(self, ignoreDefault=False):
        ...
        if self.isPUSP()["Value"]:
            return self.getCertInChain(self.__firstProxyStep - 2)[
                "Value"
            ].getDIRACGroup(ignoreDefault=ignoreDefault)
    ...
    def generateProxyRequest(self, bitStrength=1024, limited=False):
        ...
        if not bitStrength:
            return S_ERROR(
                DErrno.EX509,
                "bitStrength has to be greater than 1024 (%s)" % bitStrength,
            )
        x509 = self.getCertInChain(0)["Value"]
        return x509.generateProxyRequest(bitStrength, limited)

    def generateChainFromRequestString(
        self, pemData, lifetime=86400, requireLimited=False, diracGroup=False, rfc=True
    ):
        """
        ...
        try:
            ...
            return S_ERROR(
                DErrno.ECERTREAD,
                "Can't load request data: %s" % repr(e).replace(",)", ")"),
            )
        limited = requireLimited and self.isLimitedProxy().get("Value", False)
        return self.generateProxyToString(
            lifetime, diracGroup, 1024, limited, rfc, req.get_pubkey()
        )
    ...
    def dumpAllToFile(self, filename=False):
        ...
        try:
            ...
            return S_ERROR(
                DErrno.EWF, "%s :%s" % (filename, repr(e).replace(",)", ")"))
            )
        try:
            os.chmod(filename, stat.S_IRUSR | stat.S_IWUSR)
        except Exception as e:
            return S_ERROR(
                DErrno.ESPF, "%s :%s" % (filename, repr(e).replace(",)", ")"))
            )
    ...
    def getCredentials(self, ignoreDefault=False):
        ...
        if self.__isProxy:
            credDict["identity"] = (
                self.__certList[self.__firstProxyStep + 1].get_subject().one_line()
            )
            ...
            if retVal["OK"]:
                ...
                if retVal["OK"] and diracGroup in retVal["Value"]:
                    ...
                    credDict["groupProperties"] = Registry.getPropertiesForGroup(
                        diracGroup
                    )
        else:
