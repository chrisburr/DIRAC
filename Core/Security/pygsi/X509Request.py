...
class X509Request(object):
    ...
    def dumpRequest(self):
        ...
        try:
            reqStr = GSI.crypto.dump_certificate_request(
                GSI.crypto.FILETYPE_PEM, self.__reqObj
            )
    ...
    def dumpPKey(self):
        ...
        if not self.__valid:
            return S_ERROR(DErrno.ENOCERT)
        try:
            pkeyStr = GSI.crypto.dump_privatekey(
                GSI.crypto.FILETYPE_PEM, self.__pkeyObj
            )
        except Exception as e:
    ...
    def loadAllFromString(self, pemData):
        try:
            self.__reqObj = GSI.crypto.load_certificate_request(
                GSI.crypto.FILETYPE_PEM, pemData
            )
        except Exception as e:
            return S_ERROR(DErrno.ENOCERT, str(e))
        try:
            self.__pkeyObj = GSI.crypto.load_privatekey(
                GSI.crypto.FILETYPE_PEM, pemData
            )
        except Exception as e:
    ...
    def generateChainFromResponse(self, pemData):
        ...
        if not self.__valid:
            return S_ERROR(DErrno.ENOCERT)
        try:
            certList = GSI.crypto.load_certificate_chain(
                GSI.crypto.FILETYPE_PEM, pemData
            )
        except Exception as e:
            return S_ERROR(DErrno.ENOCERT, str(e))
    ...
    def checkChain(self, chain):
        ...
        chainPubKey = GSI.crypto.dump_publickey(
            GSI.crypto.FILETYPE_PEM, lastCert.getPublicKey()["Value"]
        )
