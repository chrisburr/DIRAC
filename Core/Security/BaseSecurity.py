...
class BaseSecurity(object):
    def __init__(self, server=False, serverCert=False, serverKey=False, timeout=False):
        ...
        if not server:
            self._secServer = gConfig.getValue(
                "/DIRAC/VOPolicy/MyProxyServer", "myproxy.cern.ch"
            )
        else:
        ...
        if serverCert:
            ...
            if ckLoc:
                ...
                self._secCertLoc = (
                    "%s/etc/grid-security/servercert.pem" % DIRAC.rootPath
                )
        if serverKey:
        ...
        self._secRunningFromTrustedHost = gConfig.getValue(
            "/DIRAC/VOPolicy/MyProxyTrustedHost", "True"
        ).lower() in ("y", "yes", "true")
    ...
    def _getUsername(self, proxyChain):
        ...
        if not credDict["validDN"]:
            return S_ERROR(
                DErrno.EDISET, "DN %s is not known in dirac" % credDict["subject"]
            )
        if not credDict["validGroup"]:
            return S_ERROR(
                DErrno.EDISET,
                "Group %s is invalid for DN %s"
