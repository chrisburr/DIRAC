...
class BaseAccountingType(object):
    ...
    def checkType(self):
        ...
        if not self.definitionAccountingFields:
            raise Exception(
                "definitionAccountingFields has to be filled prior to utilization"
            )
    ...
    def checkValues(self):
        ...
        for i in range(len(self.valuesList)):
            ...
            if key in self.valueFieldsList and not isinstance(
                self.valuesList[i], six.integer_types + (float,)
            ):
