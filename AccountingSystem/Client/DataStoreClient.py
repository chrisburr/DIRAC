...
class DataStoreClient(Client):
    ...
    def __init__(self, retryGraceTime=0, **kwargs):
        ...
        self.__failoverEnabled = not gConfig.getValue(
            "/LocalSite/DisableFailover", False
        )
    ...
    def addRegister(self, register):
        ...
        if not self.__checkBaseType(register.__class__):
            return S_ERROR(
                "register is not a valid type (has to inherit from BaseAccountingType"
            )
        ...
        try:
            while registersList:
                ...
                if retVal["OK"]:
                    ...
                    if (
                        self.__failoverEnabled
                        and time.time() - self.__lastSuccessfulCommit
                        > self.__maxTimeRetrying
                    ):
                        gLogger.warn(
                            "Error sending accounting record", retVal["Message"]
                        )
...
def _sendToFailover(rpcStub):
    ...
    try:
        request = Request()
        request.RequestName = "Accounting.DataStore.%s.%s" % (
            time.time(),
            random.random(),
        )
    ...
    # We catch all the exceptions, because it should never crash
        ...
        return S_ERROR(
            ERMSUKN, "Exception sending accounting failover request: %s" % repr(e)
        )
