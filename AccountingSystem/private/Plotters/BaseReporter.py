...
class BaseReporter(DBUtils):
    ...
    def generate(self, reportRequest):
        reportRequest["groupingFields"] = self._translateGrouping(
        ...
        if (
            "retrieveReportData" in reportRequest["extraArgs"]
            and reportRequest["extraArgs"]["retrieveReportData"]
        ):
    ...
    def __generatePlotForReport(self, reportRequest, reportHash, reportData):
        ...
        try:
            ...
            return S_ERROR(
                "Plot function for report %s is not defined"
                % reportRequest["reportName"]
            )
    ...
    def _getTimedData(
        ...
        if self._PARAM_CONVERT_TO_GRANULARITY not in metadataDict:
            ...
            metadataDict[self._PARAM_CONVERT_TO_GRANULARITY]
            not in self._VALID_PARAM_CONVERT_TO_GRANULARITY
        ):
            return S_ERROR(
                "%s field metadata is invalid" % self._PARAM_CONVERT_TO_GRANULARITY
            )
        ...
        # Query!
        timeGrouping = (
            "%%s, %s" % groupingFields[0],
            ["startTime"] + groupingFields[1],
        )
        ...
        for keyField in dataDict:
            ...
            if metadataDict[self._PARAM_CONVERT_TO_GRANULARITY] == "average":
                dataDict[keyField] = self._averageToGranularity(
                    coarsestGranularity, dataDict[keyField]
                )
            if metadataDict[self._PARAM_CONVERT_TO_GRANULARITY] == "sum":
                dataDict[keyField] = self._sumToGranularity(
                    coarsestGranularity, dataDict[keyField]
                )
    ...
    def _getSelectStringForGrouping(self, groupingFields):
        ...
        if len(groupingFields[1]) == 1:
            ...
            return "CONCAT( %s )" % ", ".join(
                ["%s, '-'" % sqlRep for sqlRep in groupingFields[0]]
            )
    ...
    def _findUnitMagic(self, reportDataDict, maxValue, unit, selectedUnits):
        ...
        graphDataDict, maxValue = self._divideByFactor(
            copy.deepcopy(reportDataDict), unitData[1]
        )
        if unitData == baseUnitData:
            reportDataDict = graphDataDict
        else:
            reportDataDict, dummyMaxValue = self._divideByFactor(
                reportDataDict, baseUnitData[1]
            )
    ...
    def __checkPlotMetadata(self, metadata):
        if self._EA_WIDTH in self._extraArgs and self._extraArgs[self._EA_WIDTH]:
            try:
                metadata[self._EA_WIDTH] = min(
                    1600, max(200, int(self._extraArgs[self._EA_WIDTH]))
                )
            except BaseException:
                pass
        if self._EA_HEIGHT in self._extraArgs and self._extraArgs[self._EA_HEIGHT]:
            try:
                metadata[self._EA_HEIGHT] = min(
                    1600, max(200, int(self._extraArgs[self._EA_HEIGHT]))
                )
            except BaseException:
                pass
    ...
    def __checkThumbnailMetadata(self, metadata):
        if (
            self._EA_THUMBNAIL in self._extraArgs
            and self._extraArgs[self._EA_THUMBNAIL]
        ):
            thbMD = dict(metadata)
            thbMD["legend"] = False
    ...
    def _generateTimedStackedBarPlot(self, filename, dataDict, metadata):
        return self.__plotData(
            filename, dataDict, metadata, generateTimedStackedBarPlot
        )
