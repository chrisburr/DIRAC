...
class JobPlotter(BaseReporter):
    ...
    def _translateGrouping(self, grouping):
        if grouping == "Country":
            sqlRepr = (
                'upper( substring( %s, locate( ".", %s, length( %s ) - 4 ) + 1 ) )'
            )
    ...
    def _reportCPUUsed(self, reportRequest):
        ...
        dataDict, granularity = retVal["Value"]
        self.stripDataField(dataDict, 0)
        dataDict = self._fillWithZero(
            granularity, reportRequest["startTime"], reportRequest["endTime"], dataDict
        )
        dataDict = self._accumulate(
            granularity, reportRequest["startTime"], reportRequest["endTime"], dataDict
        )
    ...
    def _plotCPUUsed(self, reportRequest, plotInfo, filename):
        ...
        return self._generateCumulativePlot(
            filename, plotInfo["graphDataDict"], metadata
        )
    ...
    def _plotCPUUsage(self, reportRequest, plotInfo, filename):
        ...
        return self._generateStackedLinePlot(
            filename, plotInfo["graphDataDict"], metadata
        )
    ...
    def _plotDiskSpaceSize(self, reportRequest, plotInfo, filename):
        return self.__plotFieldSizeinMB(
            reportRequest, plotInfo, filename, ("DiskSpace", "Used disk space")
        )
    ...
    def _plotOutputDataSize(self, reportRequest, plotInfo, filename):
        return self.__plotFieldSizeinMB(
            reportRequest, plotInfo, filename, ("OutputDataSize", "Output data")
        )
    ...
    def __plotCumulativeFieldSizeinMB(
        self, reportRequest, plotInfo, filename, fieldTuple
    ):
        metadata = {
    ...
    def _reportOuputDataFiles(self, reportRequest):
        return self.__reportDataFiles(
            reportRequest, ("OutputDataFiles", "Output files")
        )
    ...
    def _plotInputDataFiles(self, reportRequest, plotInfo, filename):
        return self.__plotDataFiles(
            reportRequest, plotInfo, filename, ("InputDataFiles", "Input files")
        )

    def _plotOuputDataFiles(self, reportRequest, plotInfo, filename):
        return self.__plotDataFiles(
            reportRequest, plotInfo, filename, ("OutputDataFiles", "Output files")
        )
