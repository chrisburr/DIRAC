...
class NetworkAgent(AgentModule):
    ...
    def initialize(self):
        ...
        self.bufferTimeout = self.am_getOption(
            "BufferTimeout", NetworkAgent.BUFFER_TIMEOUT
        )
        ...
        self.messagesCountOld = (
            0  # previous number of received messages (used to check connection status)
        )

        self.skippedMessagesCount = (
            0  # number of skipped messages (errors, unsupported metrics, etc.)
        )
        ...
        self.skippedMetricCount = (
            0  # number of skipped metrics (errors, invalid data, etc.)
        )
    ...
    def updateNameDictionary(self):
        ...
        result = gConfig.getConfigurationTree(
            "/Resources/Sites", "Network/", "/Enabled"
        )
        if not result["OK"]:
            self.log.error(
                "getConfigurationTree() failed with message: %s" % result["Message"]
            )
    ...
    def checkConsumers(self):
        ...
        if not self.consumers or self.messagesCount == self.messagesCountOld:
            ...
            for uri in (
                self.am_getOption("MessageQueueURI", "").replace(" ", "").split(",")
            ):
    ...
    def processMessage(self, headers, body):
        ...
        try:
            ...
            metadata["Destination"] = self.nameDictionary[
                body["meta"]["input_destination"]
            ]
        except KeyError as error:
            # messages with unsupported source or destination host name can be safely skipped
            self.skippedMessagesCount += 1
            self.log.debug(
                'Host "%s" does not exist in the host-to-dirac name dictionary (message skipped)'
                % error
            )
            return S_OK()
        ...
        for timestamp in timestamps:
            try:
                ...
                if networkAccountingObjectKey in self.buffer:
                    ...
                    timeDifference = (
                        datetime.now()
                        - self.buffer[networkAccountingObjectKey]["addTime"]
                    )
                    if timeDifference.total_seconds() > 60:
                        self.log.warn(
                            "Object was taken from buffer after %s" % (timeDifference)
                        )
                else:
                    net = Network()
                ...
                if headers["event-type"] == "packet-loss-rate":
                    ...
                    if OWDAvg < 0 or OWDMin < 0 or OWDMax < 0:
                        raise Exception(
                            "Invalid OWD metric (%s, %s, %s)" % (OWDMin, OWDAvg, OWDMax)
                        )
                    else:
                ...
                self.buffer[networkAccountingObjectKey] = {
    ...
    def commitData(self):
        ...
        if removed:
            self.log.warn(
                "Network accounting object(s) has been removed because of missing data"
            )
