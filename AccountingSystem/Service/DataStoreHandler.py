...
class DataStoreHandler(RequestHandler):
    ...
    def initializeHandler(cls, svcInfoDict):
        ...
        if cls.runBucketing:
            ...
            result = (
                cls.__acDB.markAllPendingRecordsAsNotTaken()
            )  # pylint: disable=no-member
            if not result["OK"]:
                return result
            gThreadScheduler.addPeriodicTask(
                60, cls.__acDB.loadPendingRecords
            )  # pylint: disable=no-member
        return S_OK()

    types_registerType = [basestring, list, list, list]

    def export_registerType(
        self, typeName, definitionKeyFields, definitionAccountingFields, bucketsLength
    ):
        """
    ...
    def export_setBucketsLength(self, typeName, bucketsLength):
        ...
        if errorsList:
            return S_ERROR(
                "Error while changing bucketsLength type:\n %s" % "\n ".join(errorsList)
            )
        return S_OK()
    ...
    def export_regenerateBuckets(self, typeName):
        ...
        errorsList = []
        for setup in retVal["Value"]:
            retVal = self.__acDB.regenerateBuckets(
                setup, typeName
            )  # pylint: disable=no-member
            if not retVal["OK"]:
                errorsList.append(retVal["Message"])
        if errorsList:
            return S_ERROR(
                "Error while recalculating buckets for type:\n %s"
                % "\n ".join(errorsList)
            )
        return S_OK()
    ...
    def export_removeRegisters(self, entriesList):
        ...
        for entry in entriesList:
            ...
            for i in xrange(len(entry)):
                if not isinstance(entry[i], expectedTypes[i]):
                    return S_ERROR(
                        "%s field in the records should be %s" % (i, expectedTypes[i])
                    )
        ok = 0
        for entry in entriesList:
            startTime = int(Time.toEpoch(entry[1]))
