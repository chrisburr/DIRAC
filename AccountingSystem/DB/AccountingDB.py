...
class AccountingDB(DB):
    def __init__(self, name="Accounting/AccountingDB", readOnly=False):
        ...
        gMonitor.registerActivity(
            "registeradded", "Register added", "Accounting", "entries", gMonitor.OP_ACUM
        )
        gMonitor.registerActivity(
            "insertiontime",
        ...
        gMonitor.registerActivity(
            "querytime", "Records query time", "Accounting", "seconds", gMonitor.OP_MEAN
        )

        self.__compactTime = datetime.time(
            hour=2, minute=random.randint(0, 59), second=random.randint(0, 59)
        )
        lcd = Time.dateTime()
            ...
            for setup in setupsList:
                ...
                if typeName in self.dbCatalog:
                    ...
                    if bucketsLength != self.dbBucketsLength[typeName]:
                        ...
                        self.log.warn(
                            "Bucket length has changed", "for type %s" % typeName
                        )
                    ...
                    if keyFields != self.dbCatalog[typeName]["keys"]:
                        ...
                        self.log.error(
                            "Definition fields have changed", "Type %s" % typeName
                        )
                    ...
                    if valueFields != self.dbCatalog[typeName]["values"]:
                        ...
                        self.log.error(
                            "Accountable fields have changed", "Type %s" % typeName
                        )
                # Try to re register to check all the tables are there
                retVal = self.registerType(
                    typeName,
                    definitionKeyFields,
                    definitionAccountingFields,
                    bucketsLength,
                ...
                if not retVal["OK"]:
                    self.log.error(
                        "Can't register type", "%s: %s" % (typeName, retVal["Message"])
                    )
    ...
    def loadPendingRecords(self):
        ...
        for typeName in self.dbCatalog:
            ...
            self.log.info(
                "[PENDING] %s in the queue, %d empty slots"
                % (pendingInQueue, emptySlots)
            )
            if emptySlots < 1:
                continue
            ...
            if not result["OK"]:
                ...
                return result
            self.log.info(
                "[PENDING] Got %s pending records for type %s"
                % (len(result["Value"]), typeName)
            )
            dbData = result["Value"]
            ...
            for record in dbData:
                ...
                recordsToProcess.append(
                    (iD, typeName, startTime, endTime, valuesList, now)
                )
                if len(recordsToProcess) % recordsPerSlot == 0:
                    self.__threadPool.generateJobAndQueueIt(
                        self.__insertFromINTable, args=(recordsToProcess,)
                    )
                    recordsToProcess = []
            if recordsToProcess:
                self.__threadPool.generateJobAndQueueIt(
    ...
    def __addToCatalog(self, typeName, keyFields, valueFields, bucketsLength):
        ...
        """
        self.log.verbose(
            "Adding to catalog type %s" % typeName,
            "with length %s" % str(bucketsLength),
        )
        self.dbCatalog[typeName] = {
            "keys": keyFields,
            "values": valueFields,
    ...
    @gSynchro
    def registerType(
        self, name, definitionKeyFields, definitionAccountingFields, bucketsLength
    ):
        """
        Register a new type
        """
        ...
        for field in definitionKeyFields:
            if field in valueFieldsList:
                return S_ERROR(
                    "Key field %s is also in the list of value fields" % field
                )
        for field in definitionAccountingFields:
            if field in keyFieldsList:
                return S_ERROR(
                    "Value field %s is also in the list of key fields" % field
                )
        ...
        if self.__readOnly:
            if tables:
                self.log.notice(
                    "ReadOnly mode: Skipping create of tables for %s. Removing from memory catalog"
                    % name
                )
                self.log.verbose(
                    "Skipping creation of tables %s" % ", ".join([tn for tn in tables])
                )
                try:
        ...
        if tables:
            ...
            if not retVal["OK"]:
                self.log.error(
                    "Can't create type", "%s: %s" % (name, retVal["Message"])
                )
    ...
    def getKeyValues(self, typeName, condDict, connObj=False):
        ...
        for keyName in condDict:
            if keyName in typeKeysList:
                ...
                for value in condDict[keyName]:
                    sqlCond.append(
                        "%s.value = %s" % (keyTable, self._escapeString(value)["Value"])
                    )
        ...
        for keyName in typeKeysList:
            ...
            cmd = "SELECT DISTINCT %s.value FROM %s" % (
                keyTable,
                ", ".join(allKeyTables),
            )
            if sqlCond:
    ...
    def deleteType(self, typeName):
        ...
        retVal = self._update(
            ...
            % (_getTableName("catalog", "Types"), typeName)
        )
    ...
    def __getIdForKeyValue(self, typeName, keyName, keyValue, conn=False):
        ...
        retVal = self._query(
            ...
            % (_getTableName("key", typeName, keyName), keyValue),
            conn=conn,
        )
        if not retVal["OK"]:
            return retVal
        ...
        return S_ERROR(
            "Key id %s for value %s does not exist although it shoud"
            % (keyName, keyValue)
        )
    ...
    def __addKeyValue(self, typeName, keyName, keyValue):
        ...
        self.log.info(
            "Value %s for key %s didn't exist, inserting" % (keyValue, keyName)
        )
    ...
    def calculateBuckets(self, typeName, startTime, endTime, nowEpoch=False):
        ...
        bucketTimeLength = self.calculateBucketLengthForTime(
            typeName, nowEpoch, startTime
        )
        ...
        while currentBucketStart < endTime:
            ...
            bucketTimeLength = self.calculateBucketLengthForTime(
                typeName, nowEpoch, currentBucketStart
            )
        return buckets

    def __insertInQueueTable(self, typeName, startTime, endTime, valuesList):
        sqlFields = ["id", "taken", "takenSince"] + self.dbCatalog[typeName][
            "typeFields"
        ]
        ...
        if len(sqlFields) != len(sqlValues):
            ...
            return S_ERROR(
                "Fields mismatch for record %s. %s fields and %s expected"
                % (typeName, numRcv, numExp)
            )
    ...
    def __insertFromINTable(self, recordTuples):
        ...
        """
        self.log.verbose(
            "Received bundle to process", "of %s elements" % len(recordTuples)
        )
        for record in recordTuples:
            ...
            if not result["OK"]:
                self._update(
                    "UPDATE `%s` SET taken=0 WHERE id=%s"
                    % (_getTableName("in", typeName), iD)
                )
                self.log.error("Can't insert row", result["Message"])
                continue
            result = self._update(
                "DELETE FROM `%s` WHERE id=%s" % (_getTableName("in", typeName), iD)
            )
            if not result["OK"]:
    ...
    def insertRecordDirectly(self, typeName, startTime, endTime, valuesList):
        ...
        for keyPos in range(len(self.dbCatalog[typeName]["keys"])):
            ...
            if not retVal["OK"]:
                return retVal
            self.log.verbose(
                "Value %s for key %s has id %s" % (keyValue, keyName, retVal["Value"])
            )
        ...
        try:
            ...
            if not retVal["OK"]:
                return retVal
            retVal = self.__splitInBuckets(
                typeName, startTime, endTime, valuesList, connObj=connObj
            )
            if not retVal["OK"]:
    ...
    def deleteRecord(self, typeName, startTime, endTime, valuesList):
        ...
        for i in range(len(sqlValues)):
            ...
            if i >= numKeyFields and i - numKeyFields < numValueFields:
                ...
                if (
                    self.dbCatalog[typeName]["definition"]["values"][vIndex][1].find(
                        "FLOAT"
                    )
                    > -1
                ):
            ...
            if needToRound:
                compVal = [
                    "`%s`.`%s`"
                    % (mainTable, self.dbCatalog[typeName]["typeFields"][i]),
                    "%f" % sqlValues[i],
                ]
        ...
        if not retVal["OK"]:
            return retVal
        retVal = self._update(
            "DELETE FROM `%s` WHERE %s" % (mainTable, " AND ".join(sqlCond)),
            conn=connObj,
        )
        if not retVal["OK"]:
            return retVal
        ...
        if numInsertions == 0:
            return S_OK(0)
        sqlValues.append(1)
        retVal = self.__deleteFromBuckets(
            typeName, startTime, endTime, sqlValues, numInsertions, connObj=connObj
        )
        if not retVal["OK"]:
            self.__rollbackTransaction(connObj)
            return retVal
    ...
    def __splitInBuckets(self, typeName, startTime, endTime, valuesList, connObj=False):
        ...
        return self.__writeBuckets(
            typeName, buckets, keyValues, valuesList, connObj=connObj
        )

    def __deleteFromBuckets(
        self, typeName, startTime, endTime, valuesList, numInsertions, connObj=False
    ):
        """
        DeBucketize a record
        """
        # Calculate amount of buckets
        buckets = self.calculateBuckets(
            typeName, startTime, endTime, self.__lastCompactionEpoch
        )
        # Separate key values from normal values
        numKeys = len(self.dbCatalog[typeName]["keys"])
        keyValues = valuesList[:numKeys]
    ...
    def __generateSQLConditionForKeys(self, typeName, keyValues):
        ...
        for keyPos in range(len(self.dbCatalog[typeName]["keys"])):
            ...
            realCondList.append(
                "`%s`.`%s` = %s"
                % (_getTableName("bucket", typeName), keyField, keyValue)
            )
        return " AND ".join(realCondList)

    def __getBucketFromDB(
        self, typeName, startTime, bucketLength, keyValues, connObj=False
    ):
        """
    ...
    def __extractFromBucket(
        ...
        for pos in range(len(self.dbCatalog[typeName]["values"])):
            ...
            sqlValList.append(
                "%s=GREATEST(0,%s-(%s*%s))"
                % (fullFieldName, fullFieldName, value, proportion)
            )
        sqlValList.append(
    ...
    def __checkIncomingFieldsForQuery(
        self, typeName, selectFields, condDict, groupFields, orderFields, tableType
    ):
        ...
        if groupFields:
            missing = self.__checkFieldsExistsInType(
                typeName, groupFields[1], tableType
            )
            if missing:
                return S_ERROR("Group fields %s are not defined" % ", ".join(missing))
        if orderFields:
            missing = self.__checkFieldsExistsInType(
                typeName, orderFields[1], tableType
            )
            if missing:
                return S_ERROR("Order fields %s are not defined" % ", ".join(missing))
        return S_OK()

    def retrieveRawRecords(
        self, typeName, startTime, endTime, condDict, orderFields, connObj=False
    ):
        """
    ...
    def retrieveBucketedData(
        ...
        if len(selectFields) < 2:
            return S_ERROR(
                "selectFields has to be a list containing a string and a list of fields"
            )
    ...
    def __queryType(
        ...
        for rawFieldName in selectFields[1]:
            ...
            if rawFieldName in self.dbCatalog[typeName]["keys"]:
                ...
                List.appendUnique(
                    sqlLinkList,
                    "`%s`.`%s` = `%s`.`id`" % (tableName, rawFieldName, keyTable),
                )
            else:
        ...
        for keyName in condDict:
            ...
            if keyName in self.dbCatalog[typeName]["keys"]:
                List.appendUnique(
                    sqlLinkList,
                    "`%s`.`%s` = `%s`.`id`"
                    % (tableName, keyName, _getTableName("key", typeName, keyName)),
                )
                ...
                for i in range(len(preGenFields[1])):
                    ...
                    if field in self.dbCatalog[typeName]["keys"]:
                        List.appendUnique(
                            sqlLinkList,
                            "`%s`.`%s` = `%s`.`id`"
                            % (tableName, field, _getTableName("key", typeName, field)),
                        )
                        if preGenFields[0] != "%s":
                            # The default grouping was changed
                            preGenFields[1][i] = "`%s`.Value" % _getTableName(
                                "key", typeName, field
                            )
                        else:
                        ...
                        "bucketLength",
                        "entriesInBucket",
                    ]:  # these are not in the dbCatalog
                        preGenFields[1][i] = "`%s`.`%s`" % (tableName, field)
    ...
    def compactBuckets(self, typeFilter=False):
        ...
        for typeName in self.dbCatalog:
            ...
            if self.dbCatalog[typeName]["dataTimespan"] > 0:
                self.log.info(
                    "[COMPACT] Deleting records older that timespan for type %s"
                    % typeName
                )
    ...
    def __selectForCompactBuckets(
        self, typeName, timeLimit, bucketLength, nextBucketLength, connObj=False
    ):
        """
        ...
        # MAGIC bucketing
        sqlGroupList = [
            _bucketizeDataField("`%s`.`startTime`" % tableName, nextBucketLength)
        ]
        for field in self.dbCatalog[typeName]["keys"]:
    ...
    def __deleteForCompactBuckets(
        self, typeName, timeLimit, bucketLength, connObj=False
    ):
        """
    ...
    def __compactBucketsForType(self, typeName):
        ...
        for bPos in range(len(self.dbBucketsLength[typeName]) - 1):
            self.log.info(
                "[COMPACT] Query %d of %d"
                % (bPos + 1, len(self.dbBucketsLength[typeName]) - 1)
            )
            ...
            # Retrieve the data
            retVal = self.__selectForCompactBuckets(
                typeName, timeLimit, bucketLength, nextBucketLength
            )
            if not retVal["OK"]:
                # self.__rollbackTransaction( connObj )
                return retVal
            ...
            for record in bucketsData:
                ...
                if not retVal["OK"]:
                    # self.__rollbackTransaction( connObj )
                    self.log.error(
                        "[COMPACT] Error while compacting data for record",
                        "%s: %s" % (typeName, retVal["Value"]),
                    )
            self.log.info(
                "[COMPACT] Finished compaction %d of %d"
                % (bPos, len(self.dbBucketsLength[typeName]) - 1)
            )
        # return self.__commitTransaction( connObj )
        return S_OK()
    ...
    def __slowCompactBucketsForType(self, typeName):
        ...
        for bPos in range(len(self.dbBucketsLength[typeName]) - 1):
            ...
            while previousRecordsSelected == querySize:
                ...
                result = self.__selectIndividualForCompactBuckets(
                    typeName, timeLimit, bucketLength, querySize
                )
                if not result["OK"]:
                    # self.__rollbackTransaction( connObj )
                    return result
                ...
                for record in bucketsData:
                    ...
                    valuesList = record[:-2]
                    retVal = self.__splitInBuckets(
                        typeName, startTime, endTime, valuesList
                    )
                    if not retVal["OK"]:
                        self.log.error(
            ...
            self.log.info(
                "[COMPACT] Finised compaction %d of %d"
                % (bPos, len(self.dbBucketsLength[typeName]) - 1)
            )
        # return self.__commitTransaction( connObj )
        return S_OK()

    def __selectIndividualForCompactBuckets(
        self, typeName, timeLimit, bucketLength, querySize, connObj=False
    ):
        """
        Nasty SQL query to get ideal buckets using grouping by date calculations and adding value contents
        """
    ...
    def __deleteIndividualForCompactBuckets(self, typeName, bucketsData, connObj=False):
        ...
        for bLimit in range(0, len(bucketsData), deleteQueryLimit):
            ...
            if not result["OK"]:
                self.log.error(
                    "Cannot delete individual records for compaction", result["Message"]
                )
            else:
    ...
    def __deleteRecordsOlderThanDataTimespan(self, typeName):
        ...
        """
        dataTimespan = (
            self.dbCatalog[typeName]["dataTimespan"]
            + self.dbBucketsLength[typeName][-1][1]
        )
            ...
            while deleted >= deleteLimit:
                ...
                if not result["OK"]:
                    self.log.error(
                        ...
                        "Table: %s Timespan: %s Error: %s"
                        % (table, dataTimespan, result["Message"]),
                    )
                    break
                self.log.info(
                    "[COMPACT] Deleted %d records for %s table"
                    % (result["Value"], table)
                )
    ...
    def regenerateBuckets(self, typeName):
        ...
        if self.dbCatalog[typeName]["dataTimespan"] > 0:
            self.log.info(
                "[REBUCKET] Deleting records older that timespan for type %s" % typeName
            )
            self.__deleteRecordsOlderThanDataTimespan(typeName)
        ...
        for iRange in range(len(self.dbBucketsLength[typeName])):
            ...
            sqlQuery = (
                "SELECT %s, %s, COUNT(%s) FROM `%s` WHERE %s AND %s GROUP BY %s, %s"
                % (
                    timeSelectString,
                    sumSelectString,
                    countedField,
        ...
        sqlQueries.append(sqlQuery)
        self.log.info(
            "[REBUCKET] Retrieving data for rebuilding buckets for type %s..."
            % (typeName)
        )
        queryNum = 0
        for sqlQuery in sqlQueries:
            ...
            if not retVal["OK"]:
                self.log.error(
                    "[REBUCKET] Can't retrieve data for rebucketing", retVal["Message"]
                )
                # self.__rollbackTransaction( connObj )
                return retVal
            ...
            for entry in rawData:
                ...
                if rebucketedRecords % 1000 == 0:
                    ...
                    expectedEnd = str(
                        datetime.timedelta(
                            seconds=int((numRecords - rebucketedRecords) / blockAvg)
                        )
                    )
                    self.log.info(
...
def _getTableName(tableType, typeName, keyName=None):
    ...
    if not keyName:
        ...
        raise Exception(
            "Call to _getTableName with tableType as key but with no keyName"
        )
