...
mockObjectSE1.getFileMetadata.return_value = S_OK(
...
mockObjectSE2.getFileMetadata.return_value = S_OK(
    {"Successful": {"/a/lfn/2.txt": {"Cached": 1, "Accessible": True}}, "Failed": {}}
)
mockObjectSE2.getStatus.return_value = S_OK({"DiskSE": False, "TapeSE": True})

mockObjectSE3 = MagicMock()
mockObjectSE3.getFileMetadata.return_value = S_OK(
    {"Successful": {}, "Failed": {"/a/lfn/2.txt": "error"}}
)
mockObjectSE3.getStatus.return_value = S_OK({"DiskSE": False, "TapeSE": True})

mockObjectSE4 = MagicMock()
mockObjectSE4.getFileMetadata.return_value = S_OK(
...
mockObjectSE5.getFileMetadata.return_value = S_OK(
    {"Successful": {"/a/lfn/1.txt": {"Accessible": False}}, "Failed": {}}
)
mockObjectSE5.getStatus.return_value = S_OK({"DiskSE": True, "TapeSE": False})

mockObjectSE6 = MagicMock()
mockObjectSE6.getFileMetadata.return_value = S_OK(
...
class StorageManagerSuccess(ClientsTestCase):
    @patch(
        "DIRAC.StorageManagementSystem.Client.StorageManagerClient.DataManager",
        return_value=dm_mock,
    )
    @patch(
        "DIRAC.StorageManagementSystem.Client.StorageManagerClient.StorageElement",
        return_value=mockObjectSE1,
    )
    def test_getFilesToStage_withFilesToStage(self, _patch, _patched):
        ...
        self.assertIn(
            ...
            [{"SE1": ["/a/lfn/1.txt"]}, {"SE2": ["/a/lfn/1.txt"]}],
        )
    ...
    def test_getFilesToStage_noSuchFile(self, _patch, _patched):
        ...
        self.assertEqual(res["Value"]["offlineLFNs"], {})
        self.assertEqual(
            res["Value"]["absentLFNs"],
            {"/a/lfn/2.txt": "No such file or directory ( 2 : File not at SE2)"},
        )
        self.assertEqual(res["Value"]["failedLFNs"], [])
...
if __name__ == "__main__":
    ...
    suite.addTest(
        unittest.defaultTestLoader.loadTestsFromTestCase(StorageManagerSuccess)
    )
