...
def getFilesToStage(lfnList, jobState=None, checkOnlyTapeSEs=None, jobLog=None):
    ...
    if not lfnList:
        return S_OK(
            {"onlineLFNs": [], "offlineLFNs": {}, "failedLFNs": [], "absentLFNs": {}}
        )
    ...
    if seToLFNs:
        ...
        onlineSites = set(
            dmsHelper.getLocalSiteForSE(se).get("Value") for se in onlineSEs
        ) - {None}
...
def _checkFilesToStage(
    ...
    for lfn in absentLFNs:
        ...
        # FIXME: it is not possible to return here an S_ERROR(), return the message only
        absentLFNs[lfn] = S_ERROR(errno.ENOENT, "File not at %s" % ",".join(seList))[
            "Message"
        ]
