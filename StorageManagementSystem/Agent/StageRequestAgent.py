...
from DIRAC.StorageManagementSystem.DB.StorageManagementDB import (
    THROTTLING_STEPS,
    THROTTLING_TIME,
)
...
class StageRequestAgent(AgentModule):
    ...
    def __cache(self, storageElement):
        ...
        if storageElement not in self.storageElementCache:
            ...
            self.storageElementCache[storageElement] = (
                diskCacheTB * 1000.0 / THROTTLING_STEPS
            )
    ...
    def _issuePrestageRequests(self, storageElement, seReplicaIDs, allReplicaInfo):
        ...
        if lfnRepIDs:
            ...
            if not res["OK"]:
                ...
                for lfn, requestID in res["Value"]["Successful"].iteritems():
                    stageRequestMetadata.setdefault(requestID, []).append(
                        lfnRepIDs[lfn]
                    )
        ...
        if stageRequestMetadata:
            ...
            res = self.stagerClient.insertStageRequest(
                stageRequestMetadata, self.pinLifetime
            )
            if not res["OK"]:
                gLogger.error(
    ...
    def __checkIntegrity(self, storageElement, seReplicaIDs, allReplicaInfo):
        ...
        for lfn, metadata in res["Value"]["Successful"].iteritems():
            ...
            if metadata["Size"] != allReplicaInfo[lfnRepIDs[lfn]]["Size"]:
                ...
                terminalReplicaIDs[
                    lfnRepIDs[lfn]
                ] = "LFN StorageElement size does not match FileCatalog"
                lfnRepIDs.pop(lfn)
                ...
                terminalReplicaIDs[
                    lfnRepIDs[lfn]
                ] = "LFN has been Lost by the StorageElement"
                lfnRepIDs.pop(lfn)
                ...
                terminalReplicaIDs[
                    lfnRepIDs[lfn]
                ] = "LFN is declared Unavailable by the StorageElement"
                lfnRepIDs.pop(lfn)
        ...
        for lfn, reason in res["Value"]["Failed"].iteritems():
            if re.search("File does not exist", reason):
                ...
                terminalReplicaIDs[
                    lfnRepIDs[lfn]
                ] = "LFN does not exist in the StorageElement"
            lfnRepIDs.pop(lfn)
