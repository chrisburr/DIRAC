...
class StageMonitorAgent(AgentModule):
    ...
    def monitorStageRequests(self):
        ...
        for storageElement, seReplicaIDs in seReplicas.iteritems():
            self.__monitorStorageElementStageRequests(
                storageElement, seReplicaIDs, replicaIDs
            )
    ...
    def __monitorStorageElementStageRequests(
        self, storageElement, seReplicaIDs, replicaIDs
    ):
        terminalReplicaIDs = {}
        oldRequests = []
        stagedReplicas = []
        ...
        res = StorageElement(
            storageElement, plugins=self.storagePlugins
        ).getFileMetadata(lfnRepIDs)
        if not res["OK"]:
            gLogger.error(
        ...
        for lfn, reason in prestageStatus["Failed"].iteritems():
            ...
            if re.search("File does not exist", reason):
                ...
                terminalReplicaIDs[
                    lfnRepIDs[lfn]
                ] = "LFN did not exist in the StorageElement"
