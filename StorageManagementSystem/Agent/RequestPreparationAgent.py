...
class RequestPreparationAgent(AgentModule):
    ...
    def execute(self):
        ...
        for lfn, requestedSEs in replicas.items():
            ...
            if not lfnReplicas:
                gLogger.error(
                    "Missing replicas information", "%s %s" % (lfn, requestedSEs)
                )
                continue
    ...
    def __getExistingFiles(self, lfns):
        ...
        if missing:
            ...
            gLogger.warn(
                "RequestPreparation.__getExistingFiles: %s" % reason,
                "\n".join([""] + missing),
            )
    ...
    def __getFileReplicas(self, lfns):
        ...
        return S_OK(
            {"Replicas": replicas, "ZeroReplicas": noReplicas, "Failed": failed}
        )
