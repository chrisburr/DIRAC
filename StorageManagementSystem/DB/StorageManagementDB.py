...
class StorageManagementDB(DB):
    def __init__(self, systemInstance="Default"):
        DB.__init__(
            self, "StorageManagementDB", "StorageManagement/StorageManagementDB"
        )
    ...
    def __updateTaskStatus(self, taskIDs, newTaskStatus, force=False, connection=False):
        ...
        reqSelect = (
            "SELECT TaskID FROM Tasks WHERE TaskID IN (%s) AND Status != '%s';"
        ...
        if not resSelect["OK"]:
            gLogger.error(
                ...
                % (self._caller(), "__updateTaskStatus"),
                "%s. %s" % (reqSelect, resSelect["Message"]),
            )

        req = (
            "UPDATE Tasks SET Status='%s',LastUpdate=UTC_TIMESTAMP() WHERE TaskID IN (%s) AND Status != '%s';"
        ...
        if len(taskIDs) > 0:
            reqSelect1 = "SELECT * FROM Tasks WHERE TaskID IN (%s);" % intListToString(
                taskIDs
            )
    ...
    def updateReplicaStatus(self, replicaIDs, newReplicaStatus, connection=False):
        ...
        reqSelect = (
            "SELECT ReplicaID FROM CacheReplicas WHERE ReplicaID IN (%s) AND Status != '%s';"
        ...
        if not resSelect["OK"]:
            gLogger.error(
                "%s.%s_DB: problem retrieving record:"
                % (self._caller(), "updateReplicaStatus"),
                "%s. %s" % (reqSelect, resSelect["Message"]),
            )
        ...
        if len(replicaIDs) > 0:
            reqSelect1 = (
                "SELECT * FROM CacheReplicas WHERE ReplicaID IN (%s);"
        ...
        for newStatus in tasksInStatus:
            if tasksInStatus[newStatus]:
                res = self.__updateTaskStatus(
                    tasksInStatus[newStatus], newStatus, True, connection=connection
                )
                if not res["OK"]:
                    gLogger.warn(
                        "Failed to update task associated to replicas", res["Message"]
                    )
                    # return res
    ...
    def _checkReplicaUpdate(self, replicaIDs, newReplicaState, connection=False):
        ...
        if not oldReplicaState:
            ...
            req = (
                "SELECT ReplicaID FROM CacheReplicas WHERE Status IN (%s) AND ReplicaID IN (%s)"
    ...
    def updateStageRequestStatus(self, replicaIDs, newStageStatus, connection=False):
        ...
        res = self.updateReplicaStatus(
            toUpdate, newReplicaStatus, connection=connection
        )
        if not res["OK"]:
            gLogger.warn(
                "Failed to update cache replicas associated to stage requests",
                res["Message"],
            )
        return S_OK(toUpdate)
    ...
    def _checkStageUpdate(self, replicaIDs, newStageState, connection=False):
        ...
        if not oldStageState:
            ...
            req = (
                "SELECT ReplicaID FROM StageRequests WHERE StageStatus = '%s' AND ReplicaID IN (%s)"
    ...
    def getTaskInfo(self, taskID, connection=False):
        ...
        if not resDict:
            gLogger.error(
                "StorageManagementDB.getTaskInfo: The supplied task did not exist",
                taskID,
            )
    ...
    def getTaskSummary(self, jobID, connection=False):
        ...
        if not res["OK"]:
            ...
            return res
        replicaInfo = {}
        for lfn, storageElement, pfn, fileSize, status, lastupdate, reason in res[
            "Value"
        ]:
            replicaInfo[lfn] = {
    ...
    def getTasks(
        ...
        if condDict or older or newer:
            ...
            req = "%s %s" % (
                req,
                self.buildCondition(
                    condDict, older, newer, timeStamp, orderAttribute, limit
                ),
            )
        res = self._query(req, connection)
        if not res["OK"]:
    ...
    def _getTaskReplicaIDs(self, taskIDs, connection=False):
        if not taskIDs:
            return S_OK([])
        req = (
            "SELECT DISTINCT(ReplicaID) FROM TaskReplicas WHERE TaskID IN (%s);"
            % intListToString(taskIDs)
        )
        res = self._query(req, connection)
        if not res["OK"]:
    ...
    def _getReplicaIDTasks(self, replicaIDs, connection=False):
        if not replicaIDs:
            return S_OK([])
        req = (
            "SELECT DISTINCT(TaskID) FROM TaskReplicas WHERE ReplicaID IN (%s);"
            % intListToString(replicaIDs)
        )
        res = self._query(req, connection)
        if not res["OK"]:
    ...
    def setRequest(
        self, lfnDict, source, callbackMethod, sourceTaskID, connection=False
    ):
        ...
        res = self._createTask(
            source, callbackMethod, sourceTaskID, connection=connection
        )
        if not res["OK"]:
            return res
        taskID = res["Value"]
        ...
        for se, lfns in lfnDict.iteritems():
            ...
            for lfn in lfns:
                if lfn in existingReplicas:
                    ...
                    res = self._insertReplicaInformation(
                        lfn, se, "Stage", connection=connection
                    )
                    if not res["OK"]:
        ...
        res = self._insertTaskReplicaInformation(
            taskID, allReplicaIDs, connection=connection
        )
        if not res["OK"]:
            self._cleanTask(taskID, connection=connection)
            return res
    ...
    def _createTask(self, source, callbackMethod, sourceTaskID, connection=False):
        ...
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB._createTask: Failed to create task.",
                res["Message"],
            )
            return res
    ...
    def _getExistingReplicas(self, storageElement, lfns, connection=False):
        ...
        connection = self.__getConnection(connection)
        req = (
            "SELECT ReplicaID,LFN,Status FROM CacheReplicas WHERE SE = '%s' AND LFN IN (%s);"
    ...
    def getStagedReplicas(self, connection=False):
        ...
        return self.getCacheReplicas(
            {"Status": "Staged", "TaskID": goodTasks}, connection=connection
        )
    ...
    def getWaitingReplicas(self, connection=False):
        ...
        for taskID, status, _count in res["Value"]:
            if taskID in badTasks:
                ...
                goodTasks.append(taskID)
        return self.getCacheReplicas(
            {"Status": "Waiting", "TaskID": goodTasks}, connection=connection
        )
    ...
    def getOfflineReplicas(self, connection=False):
        ...
        for taskID, status, _count in res["Value"]:
            if taskID in badTasks:
                ...
                goodTasks.append(taskID)
        return self.getCacheReplicas(
            {"Status": "Offline", "TaskID": goodTasks}, connection=connection
        )
    ...
    def getTasksWithStatus(self, status):
        ...
        req = (
            "SELECT TaskID,Source,CallBackMethod,SourceTaskID from Tasks WHERE Status = '%s';"
            % status
        )
        res = self._query(req)
        if not res["OK"]:
    ...
    def updateReplicaInformation(self, replicaTuples):
        ...
        for replicaID, pfn, size in replicaTuples:
            ...
            reqSelect = (
                "SELECT ReplicaID FROM CacheReplicas WHERE ReplicaID = %s and Status != 'Cancelled';"
                % (replicaID)
            )
            resSelect = self._query(reqSelect)
            if not resSelect["OK"]:
    ...
    def getSubmittedStagePins(self):
        ...
        for storageElement, replicas, totalSize in res["Value"]:
            storageRequests[storageElement] = {
    ...
    def insertStageRequest(self, requestDict, pinLifeTime):
        ...
        for requestID, replicaIDs in requestDict.iteritems():
            for replicaID in replicaIDs:
                replicaString = "(%s,'%s',UTC_TIMESTAMP(),%d)," % (
                    replicaID,
                    requestID,
                    pinLifeTime,
                )
        ...
        for requestID, replicaIDs in requestDict.iteritems():
            for replicaID in replicaIDs:
                ...
                reqSelect = (
                    "SELECT * FROM StageRequests WHERE ReplicaID = %s AND RequestID = '%s';"
                    % (replicaID, requestID)
                )
                resSelect = self._query(reqSelect)
                if not resSelect["OK"]:
    ...
    def setStageComplete(self, replicaIDs):
        ...
        reqSelect = (
            "SELECT * FROM StageRequests WHERE ReplicaID IN (%s);"
            % intListToString(replicaIDs)
        )
        resSelect = self._query(reqSelect)
        if not resSelect["OK"]:
        ...
        for record in resSelect["Value"]:
            gLogger.verbose(
                "%s.%s_DB: to_update StageRequests =  %s"
    ...
    def wakeupOldRequests(self, replicaIDs, retryInterval, connection=False):
        ...
        if (replicaIDs) > 0:
            ...
            if len(old_replicaIDs) > 0:
                ...
                req = (
                    "DELETE FROM StageRequests WHERE ReplicaID in (%s);"
                    % intListToString(old_replicaIDs)
                )
                res = self._update(req, connection)
                if not res["OK"]:
  ...
  '''
    ...
    def setTasksDone(self, taskIDs):
        ...
        reqSelect = "SELECT * FROM Tasks WHERE TaskID IN (%s);" % intListToString(
            taskIDs
        )
        resSelect = self._query(reqSelect)
        if not resSelect["OK"]:
        ...
        for record in resSelect["Value"]:
            gLogger.verbose(
                "%s.%s_DB: to_update Tasks =  %s"
                % (self._caller(), "setTasksDone", record)
            )
            # fix, no individual queries
        reqSelect1 = "SELECT * FROM Tasks WHERE TaskID IN (%s);" % intListToString(
            taskIDs
    ...
    def killTasksBySourceTaskID(self, sourceTaskIDs, connection=False):
        ...
        # get the TaskIDs
        req = "SELECT TaskID from Tasks WHERE SourceTaskID IN (%s);" % intListToString(
            sourceTaskIDs
        )
        res = self._query(req)
        if not res["OK"]:
        ...
        if taskIDs:
            ...
            if replicaIDs:
                ...
                req = (
                    "DELETE FROM CacheReplicas WHERE ReplicaID in (%s) AND Links=1;"
                    % intListToString(replicaIDs)
                )
                res = self._update(req, connection)
                if not res["OK"]:
    ...
    def removeStageRequests(self, replicaIDs, connection=False):
        connection = self.__getConnection(connection)
        req = "DELETE FROM StageRequests WHERE ReplicaID in (%s);" % intListToString(
            replicaIDs
        )
        res = self._update(req, connection)
        if not res["OK"]:
    ...
    def removeTasks(self, taskIDs, connection=False):
        ...
        connection = self.__getConnection(connection)
        req = "DELETE FROM TaskReplicas WHERE TaskID IN (%s);" % intListToString(
            taskIDs
        )
        res = self._update(req, connection)
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB.removeTasks. Problem removing entries from TaskReplicas."
            )
            return res
        ...
        if not resSelect["OK"]:
            ...
            for record in resSelect["Value"]:
                gLogger.verbose(
                    "%s.%s_DB: to_delete Tasks =  %s"
                    % (self._caller(), "removeTasks", record)
                )
        ...
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB.removeTasks. Problem removing entries from Tasks."
            )
    ...
    def setOldTasksAsFailed(self, daysOld, connection=False):
        ...
        """
        req = (
            "UPDATE Tasks SET Status='Failed' WHERE DATE_ADD(SubmitTime, INTERVAL %s DAY ) < UTC_TIMESTAMP();"
            % (daysOld)
        )
        res = self._update(req, connection)
        if not res["OK"]:
            gLogger.error(
                "StorageManagementDB.setOldTasksAsFailed. Problem setting old Tasks to Failed."
            )
            return res
        return res
    ...
    def removeUnlinkedReplicas(self, connection=False):
        ...
        req = (
            "SELECT ReplicaID FROM CacheReplicas WHERE Links = 0 AND Status = 'Failed';"
        )
        res = self._query(req, connection)
        if not res["OK"]:
            gLogger.error(
        ...
        req = (
            "DELETE FROM CacheReplicas WHERE ReplicaID IN (%s) AND Links= 0;"
            % intListToString(replicaIDs)
        )
        res = self._update(req, connection)
        if res["OK"]:
