...
class StorageManagerHandler(RequestHandler):
    ...
    def export_updateTaskStatus(self, sourceID, status, successful=[], failed=[]):
        ...
        gLogger.info(
            "updateTaskStatus: Received callback information for ID %s" % sourceID
        )
        gLogger.info("updateTaskStatus: Status = '%s'" % status)
        if successful:
            gLogger.info(
                "updateTaskStatus: %s files successfully staged" % len(successful)
            )
            for lfn, time in successful:
                gLogger.info(
                    "updateTaskStatus: %s %s" % (lfn.ljust(100), time.ljust(10))
                )
        if failed:
    ...
    def export_removeStageRequests(self, replicaIDs):
        ...
        if not res["OK"]:
            gLogger.error(
                "removeStageRequests: Failed to remove StageRequests", res["Message"]
            )
        return res
    ...
    def export_getCacheReplicas(
        ...
        if not res["OK"]:
            gLogger.error(
                "getCacheReplicas: Failed to get Cache replicas", res["Message"]
            )
        return res
    ...
    def export_getStageRequests(
        ...
        if not res["OK"]:
            gLogger.error(
                "getStageRequests: Failed to get Cache replicas", res["Message"]
            )
        return res
    ...
    def export_updateReplicaStatus(self, replicaIDs, newReplicaStatus):
        ...
        if not res["OK"]:
            gLogger.error(
                "updateReplicaStatus: Failed to update replica status", res["Message"]
            )
        return res
    ...
    def export_updateReplicaInformation(self, replicaTuples):
        ...
        if not res["OK"]:
            gLogger.error(
                "updateRelicaInformation: Failed to update replica information",
                res["Message"],
            )
        return res
    ...
    def export_getStagedReplicas(self):
        ...
        if not res["OK"]:
            gLogger.error(
                "getStagedReplicas: Failed to obtain Staged/StageSubmitted replicas",
                res["Message"],
            )
        return res
    ...
    def export_getWaitingReplicas(self):
        ...
        if not res["OK"]:
            gLogger.error(
                "getWaitingReplicas: Failed to obtain Waiting replicas", res["Message"]
            )
        return res
    ...
    def export_getOfflineReplicas(self):
        ...
        if not res["OK"]:
            gLogger.error(
                "getOfflineReplicas: Failed to obtain Offline replicas", res["Message"]
            )
        return res
    ...
    def export_getSubmittedStagePins(self):
        ...
        if not res["OK"]:
            gLogger.error(
                "getSubmittedStagePins: Failed to obtain submitted request summary",
                res["Message"],
            )
        return res
    ...
    def export_insertStageRequest(self, requestReplicas, pinLifetime):
        ...
        if not res["OK"]:
            gLogger.error(
                "insertStageRequest: Failed to insert stage request information",
                res["Message"],
            )
        return res
    ...
    def export_setStageComplete(self, replicaIDs):
        ...
        if not res["OK"]:
            gLogger.error(
                "setStageComplete: Failed to set StageRequest complete", res["Message"]
            )
        return res
  ...
  '''
    ...
    def export_setTasksDone(self, taskIDs):
        ...
        if not res["OK"]:
            gLogger.error(
                "setTasksDone: Failed to set status of tasks to Done", res["Message"]
            )
        return res
    ...
    def export_removeUnlinkedReplicas(self):
        ...
        if not res["OK"]:
            gLogger.error(
                "removeUnlinkedReplicas: Failed to remove unlinked Replicas",
                res["Message"],
            )
        return res
    ...
    def export_updateReplicaFailure(self, replicaFailures):
        ...
        if not res["OK"]:
            gLogger.error(
                "updateRelicaFailure: Failed to update replica failure information",
                res["Message"],
            )
        return res
    ...
    def export_getTasksWithStatus(self, status):
        ...
        if not res["OK"]:
            gLogger.error(
                "getTasksWithStatus: Failed to get tasks with %s status" % status,
                res["Message"],
            )
        return res
    ...
    def export_getReplicasWithStatus(self, status):
        ...
        if not res["OK"]:
            gLogger.error(
                "getReplicasWithStatus: Failed to get replicas with %s status" % status,
                res["Message"],
            )
        return res
    ...
    def export_getStageSubmittedReplicas(self):
        ...
        if not res["OK"]:
            gLogger.error(
                "getStageSubmittedReplicas: Failed to obtain StageSubmitted replicas",
                res["Message"],
            )
        return res
    ...
    def export_wakeupOldRequests(self, oldRequests, retryInterval):
        ...
        if not res["OK"]:
            gLogger.error(
                "wakeupOldRequests: Failed to wake up old requests", res["Message"]
            )
        return res
    ...
    def export_setOldTasksAsFailed(self, daysOld):
        ...
        if not res["OK"]:
            gLogger.error(
                "setOldTasksAsFailed: Failed to set old Tasks to Failed state. ",
                res["Message"],
            )
        return res
    ...
    def export_getAssociatedReplicas(self, replicaIDs):
        ...
        if not res["OK"]:
            gLogger.error(
                "getAssociatedReplicas: Failed to get Associated Replicas. ",
                res["Message"],
            )
        return res
    ...
    def export_getCacheReplicasSummary(self):
        ...
        if not res["OK"]:
            gLogger.error(
                " getCacheReplicasSummary: Failed to retrieve summary from server",
                res["Message"],
            )
        return res
