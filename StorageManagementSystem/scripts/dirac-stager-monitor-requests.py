...
def registerSwitches():
    ...
    switches = (
        ...
        (
            "se=",
            "Filter per Storage Element. If not used, all storage elements will be taken into account.",
        ),
        ("limit=", "Limit the number of entries returned."),
        (
            "showJobs=",
            "Whether to ALSO list the jobs asking for these files to be staged",
        ),
    )
...
def parseSwitches():
    ...
    if "status" in switches and switches["status"] not in (
        ...
        subLogger.error(
            'Found "%s" as Status value. Incorrect value used!' % switches["status"]
        )
        subLogger.error("Please, check documentation below")
        Script.showHelp(exitCode=1)
...
def run():
    ...
    if "limit" in switchDict:
        ...
        res = client.getCacheReplicas(
            queryDict, None, newer, None, None, int(switchDict["limit"])
        )
    else:
