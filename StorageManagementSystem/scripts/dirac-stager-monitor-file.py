...
if cacheReplicaInfo:
    ...
    outStr += "\n%s: %s" % (
        "LastUpdate".ljust(8),
        str(cacheReplicaInfo[replicaID]["LastUpdate"]).ljust(100),
    )
    outStr += "\n%s: %s" % (
        "Reason".ljust(8),
        str(cacheReplicaInfo[replicaID]["Reason"]).ljust(100),
    )
    ...
    if resStageRequests["Records"]:
        ...
        for info in stageRequests.itervalues():
            outStr += "\n%s: %s" % (
                "SRM RequestID".ljust(8),
                info["RequestID"].ljust(100),
            )
            outStr += "\n%s: %s" % (
                "SRM StageStatus".ljust(8),
            ...
            outStr += "\n%s: %s" % (
                "SRM PinExpiryTime".ljust(8),
                str(info["PinExpiryTime"]).ljust(100),
            )
            outStr += "\n%s: %s sec" % (
                "SRM PinLength".ljust(8),
                str(info["PinLength"]).ljust(100),
            )
    else:
